...
| Step | 模型 | 输入 → 输出 |
|------|------|------------|
| 1. Topology + ELK Layout | `claude-sonnet-4-5-20250929` (.env) | 论文 method text → 拓扑 JSON → ELK.js 约束布局 → 骨架 SVG |
...
| `src/pages/api/models.ts` | fallback 模型列表从 `gemini-2.5-flash` 改为 `claude-sonnet-4-5-20250929` / `grok-4` / `gemini-3-pro-image-preview` |

**后端 (3个)**

| 文件 | 改动 |
|------|------|
| `backend/config.py` | `DEFAULT_AI_MODEL` / `DEFAULT_TOPOLOGY_MODEL` 从 `gemini-2.5-flash` → `claude-sonnet-4-5-20250929`; 注释更新为 3-Step |
...
修复后: 前端不再发送 model 字段 → 后端从 .env 读取 `DEFAULT_TOPOLOGY_MODEL=claude-sonnet-4-5-20250929`
