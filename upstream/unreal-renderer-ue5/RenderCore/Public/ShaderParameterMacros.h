...
public:
	...
	inline EUniformBufferBindingFlags GetBindingFlags() const
	{
		return BindingFlags;
	}
...
protected:
	inline FUniformBufferBinding(const FUniformBufferRHIRef& InUniformBuffer, EUniformBufferBindingFlags InBindingFlags)
...
public:
	...
	inline TUniformBufferRef<TBufferStruct> GetUniformBufferRef() const
	{
		return TUniformBufferRef<TBufferStruct>(FUniformBufferBinding::GetUniformBuffer());
	}

	inline TRDGUniformBuffer<TBufferStruct>* operator->() const
	{
...
public:
	...
	inline FRDGUniformBuffer* GetUniformBuffer() const
	{
		return UniformBuffer;
	}
	...
	inline FRDGUniformBuffer* operator->() const
	{
		return UniformBuffer;
	}
...
public:
	...
	inline TRDGUniformBuffer<TBufferStruct>* GetUniformBuffer() const
	{
...
public:
	...
	inline FRDGBuffer* GetBuffer() const { return Buffer; }
	inline ERHIAccess  GetAccess() const { return Access; }
	...
	inline FRDGBuffer* operator->() const
	{
		check(Buffer);
		return Buffer;
	}

	inline operator FRDGBuffer*() const
	{
		return Buffer;
	}

	inline bool operator==(const FRDGBufferAccess& RHS) const
	{
...
public:
	...
	inline FRDGTexture* GetTexture() const { return Texture; }
	inline FRDGTextureSubresourceRange GetSubresourceRange() const { return SubresourceRange; }
	inline ERHIAccess GetAccess() const  { return Access; }
	...
	inline FRDGTexture* operator->() const
	{
		check(Texture);
		return Texture;
	}

	inline operator FRDGTexture*() const
	{
		return Texture;
	}

	inline bool operator==(const FRDGTextureAccess& RHS) const
	{
...
struct FDepthStencilBinding
{
	...
	inline FRDGTexture* GetTexture() const
	{
		return Texture;
	}
	inline FRDGTexture* GetResolveTexture() const
	{
		return ResolveTexture;
	}
	inline ERenderTargetLoadAction GetDepthLoadAction() const
	{
		return DepthLoadAction;
	}
	inline ERenderTargetLoadAction GetStencilLoadAction() const
	{
		return StencilLoadAction;
	}
	inline FExclusiveDepthStencil GetDepthStencilAccess() const
	{
...
class alignas(SHADER_PARAMETER_POINTER_ALIGNMENT) TShaderResourceParameterArray : public TStaticArray<std::conditional_t<SHADER_PARAMETER_POINTER_ALIGNMENT == alignof(TElement), TElement, UE::Core::TAlignedElement<TElement, SHADER_PARAMETER_POINTER_ALIGNMENT>>, NumElements>
{
public:
	inline TShaderResourceParameterArray()
	{
...
/** Adds a shader resource collection.
 *
 * Example:
 *	SHADER_PARAMETER_RESOURCE_COLLECTION(FResourceCollection, MyResourceCollection)
 */
#define SHADER_PARAMETER_RESOURCE_COLLECTION(ShaderType,MemberName) \
	INTERNAL_SHADER_PARAMETER_EXPLICIT(UBMT_RESOURCE_COLLECTION, TShaderResourceParameterTypeInfo<FRHIResourceCollection*>, FRHIResourceCollection*,MemberName,, = nullptr,EShaderPrecisionModifier::Float,TEXT(#ShaderType),false)
...
/** Adds a descriptor range.
 *
 * Example:
 *	SHADER_PARAMETER_DESCRIPTOR_RANGE(FDescriptorRange, MyDescriptorRange)
 */
#define SHADER_PARAMETER_DESCRIPTOR_RANGE(ShaderType,MemberName) \
	INTERNAL_SHADER_PARAMETER_EXPLICIT(UBMT_DESCRIPTOR_RANGE,TShaderResourceParameterTypeInfo<FRHIDescriptorRange*>,FRHIDescriptorRange*,MemberName,,=nullptr,EShaderPrecisionModifier::Float,TEXT(#ShaderType),false)
...
/** Adds an RDG-tracked uniform buffer view binding backed by a UBO on platforms that support it.
 *  On UBO platforms, generates a standalone cbuffer with ElementType[PLATFORM_MAX_UNIFORM_BUFFER_RANGE_VEC4].
 *  On non-UBO platforms, falls back to a Buffer<ElementType> SRV with the same global name
 *
 *  ElementType must be a 16-byte-aligned vector type: float4, uint4, or int4.
 */
#define SHADER_PARAMETER_RDG_UNIFORM_BLOCK(ElementType, MemberName) \
	INTERNAL_SHADER_PARAMETER_EXPLICIT(UBMT_RDG_UNIFORM_BLOCK, TShaderResourceParameterTypeInfo<FRDGBufferSRV*>, FRDGBufferSRV*,MemberName,, = nullptr,EShaderPrecisionModifier::Float,TEXT(#ElementType),false)
