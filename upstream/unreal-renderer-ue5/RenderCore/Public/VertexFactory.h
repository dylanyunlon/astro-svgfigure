...
enum class EVertexFactoryFlags : uint32
{
	...
	SupportsPrimitiveIdStream UE_DEPRECATED(5.9, "EVertexFactoryFlags::SupportsPrimitiveIdStream is not a valid enum entry anymore, please use EVertexFactoryFlags::SupportsGPUScene.") = 1u << 7,
	// Supports GPUScene on all feature levels
	SupportsGPUScene                      = 1u << 7,
	...
	SupportsMeshShading                   = 1u << 18,
	SupportsLumenMeshCards                = 1u << 19,
	SupportsLandscape                     = 1u << 20,
	IncludesFwdShader                     = 1u << 21,
	SupportsTriangleSorting               = 1u << 22,
        // Supports GPUScene on non-mobile feature levels only
	SupportsGPUSceneNonMobile             = 1u << 23
};
...
public:
	...
	bool SupportsMeshShading() const                   { return HasFlags(EVertexFactoryFlags::SupportsMeshShading); }
	...
	bool SupportsGPUScene(FStaticFeatureLevel InFeatureLevel) const
	{
		return HasFlags(EVertexFactoryFlags::SupportsGPUScene) || (InFeatureLevel > ERHIFeatureLevel::ES3_1 && HasFlags(EVertexFactoryFlags::SupportsGPUSceneNonMobile));
	}
...
private:
	...
	/** Tracks whether engine vertex factory initialization has occurred */
	static bool bVertexFactoriesInitialized;
...
#define IMPLEMENT_TEMPLATE_VERTEX_FACTORY_TYPE(TemplatePrefix, FactoryClass, ShaderFilename, Flags) \
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) FVertexFactoryType FactoryClass::StaticType( \
		TEXT(#FactoryClass), \
		TEXT(ShaderFilename), \
		Flags, \
		IMPLEMENT_VERTEX_FACTORY_VTABLE(FactoryClass) \
		); \
		UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) FVertexFactoryType* FactoryClass::GetType() const { return &StaticType; }
...
{																				
public:		
	inline bool operator()(const FVertexFactoryType& A, const FVertexFactoryType& B ) const
	{
...
public:
	...
	*/
	...
	{
		return PrimitiveIdStreamIndex[(int32)InputStreamType];
	}

	inline bool SupportsGPUScene(const FStaticFeatureLevel InFeatureLevel) const
	{
		return GetType()->SupportsGPUScene(InFeatureLevel);
	}

protected:
	void SetPrimitiveIdStreamIndex(EVertexInputStreamType InputStreamType, int32 StreamIndex)
	{
		check(StreamIndex >= TNumericLimits<int8>::Min() && StreamIndex <= TNumericLimits<int8>::Max());
		PrimitiveIdStreamIndex[(int32)InputStreamType] = static_cast<int8>(StreamIndex);
	}
	
	RENDERCORE_API bool AddPrimitiveIdStreamElement(EVertexInputStreamType InputStreamType, FVertexDeclarationElementList& Elements, uint8 AttributeIndex);

	UE_DEPRECATED(5.9, "This method will be removed. Please use AddPrimitiveIdStreamElement without AttributeIndex_Mobile argument")
...
public:
	...
	{
		uint8 StreamIndex = AddVertexStream(Component, InOutStreams);
		return FVertexElement(StreamIndex, Component.Offset, Component.Type, AttributeIndex, Component.Stride, EnumHasAnyFlags(EVertexStreamUsage::Instancing, Component.VertexStreamUsage));
	}

	template<typename VertexStreamListType>
	static uint8 AddVertexStream(const FVertexStreamComponent& Component, VertexStreamListType& InOutStreams)
	{
		...
		return (uint8)InOutStreams.AddUnique(VertexStream);
	}
