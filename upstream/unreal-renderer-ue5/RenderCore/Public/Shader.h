...
#include "ShaderMapAssetAssociation.h" // For FShaderMapAssetPaths
...
struct TShaderTypePermutation
{
	...
	friend inline uint32 GetTypeHash(const TShaderTypePermutation& Var)
	{
...
{
public:
	inline bool operator()(const TShaderTypePermutation<MetaShaderType>& A, const TShaderTypePermutation<MetaShaderType>& B) const
	{
...
public:
	...
	virtual FShaderHash GetShaderHash(int32 ShaderIndex) = 0;

	virtual FShaderHash GetShaderMapHash() const
	{
		/* return invalid hash when not using shader library */
		return FShaderHash();
	}
	...
	/** Returns the sort key for a shader, derived from its output hash. */
	inline uint32 GetSortKey(int32 ShaderIndex) const
	{
		check(IsValidShaderIndex(ShaderIndex));
		return SortKeys[ShaderIndex];
	}
...
protected:
...
#endif
		Size += SortKeys.GetAllocatedSize();
	...
	/** Exchanges an RHI shader. This is meant to be used by a shadermap resource that is transferred to a new shader library instance. */
	inline FRHIShader* ExchangeShader(int32 ShaderIndex, FRHIShader* Shader)
	{
		return RHIShaders[ShaderIndex].exchange(Shader, std::memory_order_acquire);
	}
	
	/** Populate SortKeys from shader hashes. Called by derived constructors. */
	void InitSortKeys();
...
private:
	...
	/** Shader Draw sort keys derived from shader hashes, computed at construction time. */
	TArray<uint32> SortKeys;
};

class FShaderMapResourceCode : public FRefCountedObject
{
public:
#if WITH_EDITORONLY_DATA
	struct FShaderEditorOnlyDataEntry
	{
		// Sets the symbols for this entry to the given set of symbol files; hashes every file in the input set and keeps the lowest-hash version, for determinism.
		void ConditionalSetSymbolFiles(TConstArrayView<FShaderSymbolFile> InSymbolFiles);

		uint64 SymbolFilesHash = 0u; // Cached hash of SymbolFiles used for determinism selection; does not need to be serialized.
		TArray<FShaderSymbolFile> SymbolFiles;
		...
		*   information is available */
		...
		UE_DEPRECATED(5.8, "Warnings are no longer serialized with shadermaps to DDC due to significant overhead. If you need to see shader compile warnings, please enable shader dev mode and force a recompile.")
		...
		{
			return Ar << Entry.DebugInfo << Entry.ShaderTypeHashes << Entry.ShaderStatistics;
		}
	...
	int32 FindShaderIndex(const FShaderHash& InHash) const;

#if WITH_EDITORONLY_DATA

	void AddEditorOnlyData(const FString& DebugName, uint64 ShaderTypeHash, TConstArrayView<FShaderSymbolFile> InSymbolFiles, TArray<FGenericShaderStat>&& ShaderStatistics, const FString& DebugInfo = FString());
	void UpdateEditorOnlyData(int32 Index, const FString& DebugName, uint64 ShaderTypeHash, TConstArrayView<FShaderSymbolFile> InSymbolFiles, const FString& DebugInfo);

	UE_DEPRECATED(5.8, "Shader compiler warnings are no longer stored with shadermap data, and so cannot be logged via this API; please force a recompile of any shaders you wish to see warnings for.")
	inline void LogShaderCompilerWarnings() {}
#endif

	RENDERCORE_API void ToString(FStringBuilderBase& OutString) const;

	/** Returns a ResourceIndex value for a Shader based on its hash. */
	RENDERCORE_API int32 AssignShaderResourceIndex(const FShaderHash& InHash);

	/** A hash describing the total contents of *this. Constructed from the contents of ShaderHashes during Finalize. */
	FShaderHash ResourceHash;

	/** Hashes of all shaders, added during compilation. */
	TArray<FShaderHash> ShaderHashes;

	/** Code resources for all shaders. */
	TArray<FShaderCodeResource> ShaderCodeResources;

#if WITH_EDITOR
	/** Transient shaders hashes array sorted based on shader layout order in ShaderMap. Gets built during ShaderMap Finalize. */
	TArray<FShaderHash> SortedShaderHashes;
#endif

#if WITH_EDITORONLY_DATA
	// Optional array of editor-only data indexed in the same order as ShaderEntries
...
#if WITH_EDITOR
	/** Reorders Shader Data to match the order present in SortedShaderHashes. */
	void SortShaderData();
#endif

	/** Computes ResourceHash. */
	void UpdateHash();
};
...
public:
	...
	RENDERCORE_API virtual FShaderHash GetShaderHash(int32 ShaderIndex) override;
...
{
public:
	inline FShaderKey(const FShaderHash& InMaterialShaderMapHash, const FShaderPipelineType* InShaderPipeline, FVertexFactoryType* InVertexFactoryType, int32 PermutationId, EShaderPlatform InPlatform)
	...
	friend inline uint32 GetTypeHash(const FShaderKey& Id)
	{
		return
			HashCombine(
				HashCombine(GetTypeHash(Id.MaterialShaderMapHash), GetTypeHash(Id.Platform)),
	...
	FShaderHash MaterialShaderMapHash;
	int32 PermutationId;
...
public:
	inline FShaderId() {}
	inline FShaderId(const FShaderType* InType, uint64 InMaterialShaderMapHash, const FHashedName& InShaderPipeline, const FVertexFactoryType* InVertexFactoryType, int32 InPermutationId, EShaderPlatform InPlatform)
		: Type(InType)
...
public:
	...
	RENDERCORE_API const FShaderHash& GetOutputHash() const;

	UE_DEPRECATED(5.9, "Sort Key is no longer stored on FShader. Please use TShaderRef for getting the Sort Key.")
	uint32 GetSortKey() const { return 0; }

	UE_DEPRECATED(5.9, "Finalize requires mutable Code Resource.")
	void Finalize(const FShaderMapResourceCode* Code) { Finalize(const_cast<FShaderMapResourceCode*>(Code)); }
	RENDERCORE_API void Finalize(FShaderMapResourceCode* Code);
	...
	inline const FShaderUniformBufferParameter& GetUniformBufferParameter(const FShaderParametersMetadata* SearchStruct) const
	{
	...
	inline const FShaderUniformBufferParameter& GetUniformBufferParameter(const FHashedName SearchName) const
	{
...
protected:
	...
	*/
	LAYOUT_FIELD_EDITORONLY(FShaderHash, OutputHash);
...
public:
	...
	TShaderRefBase(ShaderType* InShader, const FShaderMapBase& InShaderMap, const FSpecializationContainerType& InSpecConsts)
		: ShaderContent(InShader), ShaderMap(&InShaderMap), SpecConsts(InSpecConsts) { checkSlow(InShader); }
	TShaderRefBase(const TShaderRefBase&) = default;

	template<typename OtherShaderType, typename OtherPointerTableType>
	TShaderRefBase(const TShaderRefBase<OtherShaderType, OtherPointerTableType>& Rhs) : ShaderContent(Rhs.GetShader()), ShaderMap(Rhs.GetShaderMap()), SpecConsts(Rhs.GetSpecializationConstants()) {}
	...
	TShaderRefBase& operator=(const TShaderRefBase<OtherShaderType, OtherPointerTableType>& Rhs)
	{
		...
		SpecConsts = Rhs.GetSpecializationConstants();
		return *this;
	}
	...
	{
		return TShaderRefBase<ShaderType, PointerTableType>(static_cast<ShaderType*>(Rhs.GetShader()), Rhs.GetShaderMap(), Rhs.GetSpecializationConstants());
	}
	...
	{
		return TShaderRefBase<ShaderType, PointerTableType>(reinterpret_cast<ShaderType*>(Rhs.GetShader()), Rhs.GetShaderMap(), Rhs.GetSpecializationConstants());
	}
	...
	/** Returns the sort key for this shader, computed from the shader's output hash. */
	inline uint32 GetSortKey() const
	{
		if (ShaderContent)
		{
			if (FShaderMapResource* Resource = GetResource())
			{
				Resource->GetSortKey(ShaderContent->GetResourceIndex());
			}
		}
		return 0;
	}
	...
	inline FRHIShader* GetRHIShaderBase(EShaderFrequency Frequency, bool bRequired = true) const
	{
		...
		if(ShaderContent)
		{
			...
			{
				UE_LOGF(LogShaders, Log, "Failed to create shader for type %ls with resource index %d.", GetType()->GetName(), ShaderContent->GetResourceIndex());
				return nullptr;
			}
	...
	inline FRHIComputeShader* GetComputeShader(bool bRequired = true) const
	{
		return static_cast<FRHIComputeShader*>(GetRHIShaderBase(SF_Compute, bRequired));
	}
	...
	inline const FSpecializationContainerType& GetSpecializationConstants() const
	{
		return SpecConsts;
	}

	inline void SetSpecializationConstants(FSpecializationContainerType& InSpecConsts)
	{
		SpecConsts = InSpecConsts;
	}

private:
	TShaderRefBase(ShaderType* InShader, const FShaderMapBase* InShaderMap, const FSpecializationContainerType& InSpecConsts)
		: ShaderContent(InShader), ShaderMap(InShaderMap), SpecConsts(InSpecConsts)
	{
...
protected:
	FSpecializationContainerType SpecConsts;
};
...
#undef SHADER_TYPE_ENTRY
	...
	typedef int32(*GetUnspecializedIdType)(const int32 PermutationId);
	typedef FSpecializationContainerType (*GetSpecializationValuesType)(const int32 PermutationId);
	...
	RENDERCORE_API FShaderType(
		...
		int32 TotalSpecializationCount,
	...
	RENDERCORE_API bool IsSpecialization(const int32 PermutationId) const;
	RENDERCORE_API int32 GetUnspecializedId(const int32 PermutationId) const;
	RENDERCORE_API FSpecializationContainerType GetSpecializationValues(const int32 PermutationId) const;
	...
	*/
	...
	inline const FOpenColorIOShaderType* GetOpenColorIOShaderType() const
	{
	...
	inline FOpenColorIOShaderType* GetOpenColorIOShaderType()
	{
		return (ShaderTypeForDynamicCast == EShaderTypeForDynamicCast::OCIO) ? reinterpret_cast<FOpenColorIOShaderType*>(this) : nullptr;
	}
	inline const FComputeKernelShaderType* GetComputeKernelShaderType() const
	{
	...
	inline FComputeKernelShaderType* GetComputeKernelShaderType()
	{
	...
	inline const FNNERuntimeIREEShaderType* GetNNERuntimeIREEShaderType() const
	{
	...
	inline FNNERuntimeIREEShaderType* GetNNERuntimeIREEShaderType()
	{
	...
	inline const FGlobalShaderType* AsGlobalShaderType() const
	{
	...
	inline const FMaterialShaderType* AsMaterialShaderType() const
	{
	...
	inline const FMeshMaterialShaderType* AsMeshMaterialShaderType() const
	{
	...
	inline const FNiagaraShaderType* AsNiagaraShaderType() const
	{
	...
	inline const FOpenColorIOShaderType* AsOpenColorIOShaderType() const
	{
	...
	inline int32 GetSpecializationCount() const
	{
		return TotalSpecializationCount;
	}
...
private:
	...
	GetUnspecializedIdType GetUnspecializedIdRef;
	GetSpecializationValuesType GetSpecializationValuesRef;
...
{
public:
	RENDERCORE_API FShaderTypeRegistration(TFunctionRef<FShaderType& ()> LazyShaderTypeAccessor);
...
struct FShaderCompiledShaderInitializerType
{
	...
	RENDERCORE_API FShaderCompiledShaderInitializerType(
		...
		uint64 InMaterialShaderMapHash,
...
#define SHADER_DECLARE_VTABLE(ShaderClass) \
	...
	static int32 GetUnspecializedIdImpl(const int32 PermutationID) \
	{ return typename ShaderClass::FPermutationDomain(PermutationID).ToDimensionUnspecializedId(); } \
	static FSpecializationContainerType GetSpecializationValuesImpl(const int32 PermutationID) \
	{ \
		if constexpr (ShaderClass::FPermutationDomain::SpecializationCount > 0) {\
			FSpecializationContainerType SpecConsts; \
			SpecConsts.Reserve(ShaderClass::FPermutationDomain::SpecializationCount); \
			typename ShaderClass::FPermutationDomain(PermutationID).ToSpecializationConstantArray(SpecConsts); \
			return SpecConsts; \
		} else { return FSpecializationContainerType(); } \
	} \
...
#define SHADER_TYPE_VTABLE(ShaderClass) \
	...
	ShaderClass::GetUnspecializedIdImpl, \
	ShaderClass::GetSpecializationValuesImpl, \
...
#define IMPLEMENT_SHADER_TYPE(TemplatePrefix,ShaderClass,SourceFilename,FunctionName,Frequency) \
	...
	{ \
		static ShaderClass::ShaderMetaType StaticType( \
			...
			ShaderClass::FPermutationDomain::SpecializationCount, \
...
public:
	...
	inline bool HasMeshShader() const { return AllStages[SF_Mesh] != nullptr; }
	inline bool HasGeometry() const { return AllStages[SF_Geometry] != nullptr; }
	inline bool HasPixelShader() const { return AllStages[SF_Pixel] != nullptr; }

	inline const FShaderType* GetShader(EShaderFrequency Frequency) const
	{
		check(Frequency < SF_NumFrequencies);
		return AllStages[Frequency];
	}

	inline FName GetFName() const { return TypeName; }
	inline TCHAR const* GetName() const { return Name; }
	inline const FHashedName& GetHashedName() const { return HashedName; }
	inline const FHashedName& GetHashedPrimaryShaderFilename() const { return HashedPrimaryShaderFilename; }

	// Returns an array of valid stages, sorted from PS->GS->DS->HS->VS, no gaps if missing stages
	inline const TArray<const FShaderType*>& GetStages() const { return Stages; }
...
#define IMPLEMENT_SHADERPIPELINE_TYPE_VSPS(PipelineName, VertexShaderType, PixelShaderType, bRemoveUnused)	\
	static FShaderPipelineType PipelineName(TEXT(UE_STRINGIZE(PipelineName)), &VertexShaderType::GetStaticType(), nullptr, &PixelShaderType::GetStaticType(), false, bRemoveUnused);
// Only VS
#define IMPLEMENT_SHADERPIPELINE_TYPE_VS(PipelineName, VertexShaderType, bRemoveUnused)	\
	static FShaderPipelineType PipelineName(TEXT(UE_STRINGIZE(PipelineName)), &VertexShaderType::GetStaticType(), nullptr, nullptr, false, bRemoveUnused);
...
#define IMPLEMENT_SHADERPIPELINE_TYPE_VSGSPS(PipelineName, VertexShaderType, GeometryShaderType, PixelShaderType, bRemoveUnused)	\
	static FShaderPipelineType PipelineName(TEXT(UE_STRINGIZE(PipelineName)), &VertexShaderType::GetStaticType(), &GeometryShaderType::GetStaticType(), &PixelShaderType::GetStaticType(), false, bRemoveUnused);
// Vertex+Geometry
#define IMPLEMENT_SHADERPIPELINE_TYPE_VSGS(PipelineName, VertexShaderType, GeometryShaderType, bRemoveUnused)	\
	static FShaderPipelineType PipelineName(TEXT(UE_STRINGIZE(PipelineName)), &VertexShaderType::GetStaticType(), &GeometryShaderType::GetStaticType(), nullptr, false, bRemoveUnused);

// Mesh+Pixel
#define IMPLEMENT_SHADERPIPELINE_TYPE_MSPS(PipelineName, MeshShaderType, PixelShaderType, bRemoveUnused)	\
	static FShaderPipelineType PipelineName(TEXT(UE_STRINGIZE(PipelineName)), &MeshShaderType::GetStaticType(), nullptr, &PixelShaderType::GetStaticType(), true, bRemoveUnused);
...
#define IMPLEMENT_SHADERPIPELINE_TYPE_MSASPS(PipelineName, MeshShaderType, AmplificationShaderType, PixelShaderType, bRemoveUnused)	\
	static FShaderPipelineType PipelineName(TEXT(UE_STRINGIZE(PipelineName)), &MeshShaderType::GetStaticType(), &AmplificationShaderType::GetStaticType(), &PixelShaderType::GetStaticType(), true, bRemoveUnused);
#endif
...
{																				
public:
	inline bool operator()(const FShaderType& A, const FShaderType& B ) const
	{
...
public:
	...
	{
		return GetShader<ShaderType>(UE::ShaderPermutationUtils::ConvertToUnspecializedId(GetShaderPlatform(), PermutationVector));
	}
	...
	UE_DEPRECATED(5.8, "Use overload accepting uint64 shadermap hash")
	inline void GetShaderList(const FShaderMapBase& InShaderMap, const FSHAHash& InMaterialShaderMapHash, TMap<FShaderId, TShaderRef<FShader>>& OutShaders) const
	{
		uint64 TruncatedShaHash;
		memcpy(&TruncatedShaHash, InMaterialShaderMapHash.Hash, sizeof(uint64));
		GetShaderList(InShaderMap, TruncatedShaHash, OutShaders);
	}
	
	/** Builds a list of the shaders in a shader map. */
	RENDERCORE_API void GetShaderList(const FShaderMapBase& InShaderMap, uint64 InMaterialShaderMapHash, TMap<FShaderId, TShaderRef<FShader>>& OutShaders) const;
	...
	/** Sorts the shader map's internal Shaders/ShaderTypes/ShaderPermutations arrays by type name and permutation ID for deterministic ordering. */
	RENDERCORE_API void SortShaders();

	/** Iterates all shaders and shader pipelines in the map, calling Finalize on each to assign their ResourceIndex. */
	RENDERCORE_API void FinalizeShaders(FShaderMapResourceCode* Code);

	template <typename THashBuilder>
	void UpdateHash(THashBuilder& Hasher) const
	{
...
protected:
	...
	LAYOUT_FIELD_EDITORONLY(bool, bSorted)
};
...
public:
	...
	RENDERCORE_API bool Serialize(FShaderSerializeContext& Ctx, uint64* OutCookedShaderMapContentHash = nullptr);
...
public:
	...
	{
		checkf(TShaderRef<ShaderType>::SpecConsts.Num() == ShaderType::FPermutationDomain::SpecializationCount, TEXT("Shader returned without specialization constant values."));
	}
};
