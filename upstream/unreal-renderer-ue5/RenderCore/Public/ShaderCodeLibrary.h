...
{
	FShaderHash Shaders[SF_NumGraphicsFrequencies];
	mutable uint32 Hash;
	...
	friend uint32 GetTypeHash(const FShaderCodeLibraryPipeline &Key)
	{
		if(!Key.Hash)
		{
			for (uint32 Frequency = 0u; Frequency < SF_NumGraphicsFrequencies; ++Frequency)
			{
				Key.Hash = HashCombine(Key.Hash, GetTypeHash(Key.Shaders[Frequency]));
			}
		}
		return Key.Hash;
	}
	...
	RENDERCORE_API void GetPipelineHash(FShaderHash& Output);
...
struct FCompactFullName
{
	...
	RENDERCORE_API FString GetPackageNameOnly() const;
#endif
};
...
struct FStableShaderKeyFuncs : BaseKeyFuncs<TUniquePtr<FStableShaderKeyAndValue>, FStableShaderKeyAndValue>
{
	[[nodiscard]] static FORCEINLINE const FStableShaderKeyAndValue& GetSetKey(const TUniquePtr<FStableShaderKeyAndValue>& Element)
	{
		return *Element;
	}

	[[nodiscard]] static FORCEINLINE bool Matches(const FStableShaderKeyAndValue& A, const FStableShaderKeyAndValue& B)
	{
		return A == B;
	}

	/** Calculates a hash index for a key. */
	[[nodiscard]] static FORCEINLINE uint32 GetKeyHash(const FStableShaderKeyAndValue& Key)
	{
...
{
	/** If bound, invoked from the shader-library deferred deleter's end-of-frame callback when destroying a
	 *  FShaderLibraryInstance whose Resources will trigger FRHIShader destruction. The provided functor does
	 *  the actual destruction. Engine binds this to wrap the destruction.
	 *
	 *  Left unbound by default — if so skip the recreate and destroy directly. */
	RENDERCORE_API extern TUniqueFunction<void(TFunctionRef<void()> DoUnload)> GUnloadShaderLibraryWithRenderStateRecreate;
}

using FSharedShaderCodeRequest = TTSMulticastDelegate<void(const FShaderHash&, FArchive*)>; // thread-safe because it's "broadcasted" concurrently
using FSharedShaderCodeRelease = TMulticastDelegate<void(const FShaderHash&)>;
...
struct FShaderCodeLibrary
{
	...
	static RENDERCORE_API bool ContainsShaderCode(const FShaderHash& Hash);
    static RENDERCORE_API bool ContainsShaderCode(const FShaderHash& Hash, const FString& LogicalLibraryName);

	static RENDERCORE_API TRefCountPtr<FShaderMapResource> LoadResource(const FShaderHash& Hash, FArchive* Ar);

	static RENDERCORE_API bool PreloadShader(const FShaderHash& Hash, FArchive* Ar);
	static RENDERCORE_API bool ReleasePreloadedShader(const FShaderHash& Hash);

	static RENDERCORE_API FVertexShaderRHIRef CreateVertexShader(EShaderPlatform Platform, const FShaderHash& Hash);
	static RENDERCORE_API FPixelShaderRHIRef CreatePixelShader(EShaderPlatform Platform, const FShaderHash& Hash);
	static RENDERCORE_API FGeometryShaderRHIRef CreateGeometryShader(EShaderPlatform Platform, const FShaderHash& Hash);
	static RENDERCORE_API FComputeShaderRHIRef CreateComputeShader(EShaderPlatform Platform, const FShaderHash& Hash);
	static RENDERCORE_API FMeshShaderRHIRef CreateMeshShader(EShaderPlatform Platform, const FShaderHash& Hash);
	static RENDERCORE_API FAmplificationShaderRHIRef CreateAmplificationShader(EShaderPlatform Platform, const FShaderHash& Hash);
	static RENDERCORE_API FRayTracingShaderRHIRef CreateRayTracingShader(EShaderPlatform Platform, const FShaderHash& Hash, EShaderFrequency Frequency);
	...
	static RENDERCORE_API void SafeAssignHash(FRHIShader* InShader, const FShaderHash& Hash);
...
struct FShaderLibraryCooker
{
	...
	static RENDERCORE_API void InitForCooking(bool bNativeFormat, UE::Cook::ICookArtifactReader* CookArtifactReader);
	...
	static RENDERCORE_API void FinishPopulateShaderLibrary(const ITargetPlatform* TargetPlatform, FString const& Name, FString const& SandboxDestinationPath,
		FString const& SandboxMetadataPath, bool bInitializeReferenceTracking = false);

	/** Called by the cooker to report which assets are referenced in the current cook or in incrementalcook oplog. */
	static RENDERCORE_API void UpdateOplogPackages(UE::Cook::Artifact::FUpdateOplogPackagesContext& Context);

	/**
	 * Called by the cooker to ask the library to prune the no-longer referenced shaders from its incremental cook library,
	 * called only after UpdateOplogPackages has been called on every platform.
	 */
	static RENDERCORE_API void PruneShaderLibrary(const ITargetPlatform* TargetPlatform, const FString& Name);
	 ...
	 * @param PlatformSCLCSVPaths path for where to put the information about the shader hashes
	 * @param PackagingSystem Which packaging system is being used by the project's data build. This impacts
	 *        some internal format storage decisions for ShaderLibraries due to runtime performance requirements.
	 ...
	 * @return true if successful
	 */
	static RENDERCORE_API bool SaveShaderLibraryWithoutChunking(const ITargetPlatform* TargetPlatform,
		FString const& Name, FString const& SandboxDestinationPath, FString const& SandboxMetadataPath,
		TArray<FString>& PlatformSCLCSVPaths, UE::Cook::EPackagingSystem PackagingSystem,
		FString& OutErrorMessage, bool& bOutHasData);
	 ...
	 * @return true if successful
	 */
	static RENDERCORE_API bool SaveShaderLibraryChunk(int32 ChunkId, const TSet<FName>& InPackagesInChunk,
		const ITargetPlatform* TargetPlatform, const FString& SandboxDestinationPath,
		const FString& SandboxMetadataPath, UE::Cook::EPackagingSystem PackagingSystem,
		TArray<FString>& OutChunkFilenames, bool& bOutHasData);

	/**
	 * Delete the chunk-specific shaderlibrary files that are still present on disk from a previous incremental cook.
	 * They will be rewritten by SaveShaderLibraryChunk calls.
	 */
	static RENDERCORE_API void DeleteShaderLibraryChunks(const ITargetPlatform* TargetPlatform,
