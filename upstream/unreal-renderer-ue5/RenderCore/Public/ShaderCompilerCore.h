...
inline const int32 ShaderCompileWorkerInputVersion = 33;
// this is for the protocol, not the data, bump if FShaderCompilerOutput or WriteToOutputArchive changes.
inline const int32 ShaderCompileWorkerOutputVersion = 31;
...
{
	UE_DEPRECATED(5.8, "UE::ShaderCompiler::GetBindlessConfiguration has been replaced by FShaderPlatformConfig::GetBindlessConfiguration")
...
extern RENDERCORE_API bool ShouldForceOptimizeRayTracingShaders(FName ShaderFormat);
...
struct FShaderCompilerResourceTable
{
	...
	/** Mapping of bound uniform blocks to their location in resource tables. */
...
struct FShaderCompilerInput
{
	...
	// Uniform buffer that are (dis)allowed to be used for (non) VS frequencies: relies on reflection data to fail 
	// compilation even though the shader may actually compile. This ensures we don't start using a uniform buffer by mistake
	TArray<FString> VSDisallowedUniformBufferNames;
	TArray<FString> NonVSDisallowedUniformBufferNames;
	...
	// Marks the compile job for being used with a ShaderPipeline. This tells the shader backend to remove unused interpolators from the Pixel input and Vertex output (in that order).
	// Therefore, ShaderPipelines compile their Pixel shader first, to determine what input is actually used and then compile the Vertex shader, to remove output interpolators that are no longer needed by the Pixel input.
	// Only the first stage in a ShaderPipeline job enables this attribute when the job is constructed and the next stage propagates its value in CompileShaderPipeline().
...
struct FShaderCompilerOutput
{
	...
	,	bInterpolatorRemovalFailed(false)
	,	bForceDumpDebugInfo(false)
	,	bForceLogDiagnosticMessages(false)
	{
	}
	...
	/** Specifies that the interpolator removal optimization failed for this job.
	 * If the job still succeeded, then all other shaders for the associated pipeline job
	 * must also skip interpolator removal to avoid mismatch between interpolators of shader stages.
	 */
	bool bInterpolatorRemovalFailed;

	/** Specifies to always dump debug information for this job output.
	 * If the compile job was successful, this allows to dump debug information to diagnose failed intermediate results, regardless of r.DumpShaderDebugInfo configuration.
	 * This is primarily used when the cross-compilation pipeline is triggered due to internal compiler errors (usually with FXC).
	 */
	bool bForceDumpDebugInfo;

	/** Specifies to always log diagnostic messages.
	 * If the compile job was successful, this will be printed with Display verbosity.
	 * If r.ShaderDevelopmentMode is enabled, this will be printed with Warning verbosity.
	 */
	bool bForceLogDiagnosticMessages;
	...
	void AddErrorf(UE::Core::TCheckedFormatString<FString::FmtCharType, Types...> Fmt, Types... Args)
	{
		Errors.Emplace(FString::Printf(Fmt, Args...));
	}
	...
	friend FArchive& operator<<(FArchive& Ar, FShaderCompilerOutput& Output)
	{
		...
		Ar << Output.bInterpolatorRemovalFailed;
		Ar << Output.bForceDumpDebugInfo;
		Ar << Output.bForceLogDiagnosticMessages;
...
extern RENDERCORE_API void CompileShader(const TArray<const IShaderFormat*>& ShaderFormats, FShaderCompileJob& Job, int32* CompileCount = nullptr);
extern RENDERCORE_API void CompileShaderPipeline(const TArray<const IShaderFormat*>& ShaderFormats, FShaderPipelineCompileJob* PipelineJob, int32* CompileCount = nullptr);
...
{
	FShaderHash					PathInSourceHash;		// PathInSourceHash doesn't include PathInSource's null terminator, so hash computation can use a string view
	...
	inline bool EqualsPathInSource(const ANSICHAR* InPathInSource, int32 InPathInSourceLen, FShaderHash InPathInSourceHash, const ANSICHAR* InParentPath) const
	{
