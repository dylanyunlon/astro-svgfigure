 ...
 * 
 * @return e.g. EPixelFormatChannelFlags::R for PF_R8
 */
...
RENDERCORE_API bool MobileUsesExtendedGBuffer(const FStaticShaderPlatform Platform);
...
/** Return true the sky light capture can affect the height fog. */
RENDERCORE_API bool SupportsSkyLightCaptureAffectsHeightFogInScattering(const FStaticShaderPlatform Platform);
...
/** Returns whether to simulate mobile for low material quality. */
RENDERCORE_API bool GetMaterialSimulateMobileForLowQuality();

UE_DEPRECATED(5.9, "This function is deprecated and will be removed")
...
RENDERCORE_API bool PlatformSupportsSeparatedSubsurfaceDiffuse(const FStaticShaderPlatform Platform);
...
RENDERCORE_API bool NaniteMeshShadersSupported(EShaderPlatform Platform);
RENDERCORE_API bool NaniteSplineMeshesSupported();
RENDERCORE_API bool NaniteSkinnedMeshesSupported();
RENDERCORE_API bool NaniteTranslucencySupported();
RENDERCORE_API bool NaniteAssembliesSupported();
RENDERCORE_API bool NaniteVoxelsSupported();
RENDERCORE_API bool NaniteCurvesSupported();

RENDERCORE_API bool UseNaniteBindlessShading(EShaderPlatform Platform);
RENDERCORE_API bool UseNaniteBindlessRasterization(EShaderPlatform Platform);
...
RENDERCORE_API bool DoesMaterialQualitySupportNanite(EShaderPlatform ShaderPlatform, int32 MaterialQualityLevel);
...
/**
* Returns true if classic (non-virtual) shadow map techniques are supported on the given shader platform.
*/
RENDERCORE_API bool DoesPlatformSupportClassicShadowMaps(EShaderPlatform Platform);
...
/**
* Returns true if non-Nanite virtual shadow maps are enabled by CVar r.Shadow.Virtual.NonNanite.ProjectEnabled
...
/**
*	(Runtime) Whether platform uses translucent fallback for SLW rendering.
*/
RENDERCORE_API bool IsUsingSingleLayerWaterFallback(FStaticShaderPlatform Platform);
...
/**
 *  Returns true if Hair Complex Transmittance is enabled for all hair surfaces/pixels
 */
RENDERCORE_API bool IsHairComplexTransmittanceEnabled(EShaderPlatform Platform);
...
/**
 *   Checks if inline skinning shaders are enabled for the platform (via r.SkinCache.CompileShaders)
 */
RENDERCORE_API bool AreInlineSkinningShadersEnabled(EShaderPlatform Platform);
...
/**
 * Checks if mesh deformers can be used for a specific platform.
 */
RENDERCORE_API bool AreMeshDeformersAvailable(EShaderPlatform Platform);
...
/**
 * Returns true if we need to create SRVs for all index/vertex buffers.
 * This has a memory overhead for the RHI objects so we only return true if the platform or project settings require this.
 */
...
/**
* Read-only switch to check if compiling explicit 128BitRT BasePass Pixel Shaders is allowed
* They are infrequently necessary, and can save 50k shaders / 15 MiB / platform on large projects
* Check bRequiresExplicit128bitRT in a platform's DataDrivenPlatformInfo.ini to see if that platform requires explicit 128Bit shaders
*/
...
UE_DEPRECATED(5.9, "Ray tracing on-demand is always supported. Remove the call (it always returns true).")
inline bool IsRayTracingEnableOnDemandSupported()
{
	return true;
}
...
RENDERCORE_API bool ShouldCompileMegaLightsShaders(EShaderPlatform ShaderPlatform);
...
namespace Substrate
{
	...
	RENDERCORE_API bool IsRoughDiffuseEnabled(EShaderPlatform InPlatform);
	UE_DEPRECATED(5.7, "Please use IsGlintEnabled(EShaderPlatform InPlatform)")
	...
	RENDERCORE_API bool IsToonProfileEnabled();
	...
	UE_DEPRECATED(5.7, "Please use IsAdvancedVisualizationEnabled(EShaderPlatform InPlatform)")
	RENDERCORE_API bool IsAdvancedVisualizationEnabled();
	RENDERCORE_API bool IsAdvancedVisualizationEnabled(EShaderPlatform InPlatform);
	RENDERCORE_API bool IsMaterialLayeringSupportEnabled();

	RENDERCORE_API bool IsHiddenMaterialAssetConversionEnabled();

	RENDERCORE_API bool AreLightingPassesTiled();
	RENDERCORE_API bool UseTileSize16();
	...
	UE_DEPRECATED(5.8, "GetClosurePerPixel(EShaderPlatform InPlatform, uint8 InMaterialQualityLevel)")
	RENDERCORE_API uint32 GetClosurePerPixel(EShaderPlatform InPlatform);

	inline const uint8 IgnoreMaterialQualityLevelsForClosurePerPixel = 255;
	RENDERCORE_API uint32 GetClosurePerPixel(EShaderPlatform InPlatform, uint8 InMaterialQualityLevel);

	RENDERCORE_API uint32 GetNormalQuality();

	UE_DEPRECATED(5.8, "This function is no longer used. It will always return 1.")
...
inline float EV100ToLuminance(float LuminanceMax, float EV100)
{
...
inline float EV100ToLog2(float LuminanceMax, float EV100)
{
	return EV100 + FMath::Log2(LuminanceMax);
}

inline float LuminanceToEV100(float LuminanceMax, float Luminance)
{
	return FMath::Log2(Luminance / LuminanceMax);
}

inline float LuminanceToEV100(float Luminance)
{
	// LuminanceMax set to 1 for lighting to be unitless (1.0cd/m^2 becomes 1.0 at EV100)
	return FMath::Log2(Luminance);
}

inline float Log2ToEV100(float LuminanceMax, float Log2)
{
...
/**
 * Returns true if the renderer should use parallel tasks
 */
RENDERCORE_API bool ShouldUseParallelRendererTasks();
