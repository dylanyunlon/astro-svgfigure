...
struct FUniformResourceEntry
{
	...
	inline FStringView GetUniformBufferName() const { return FStringView(UniformBufferMemberName, UniformBufferNameLength); }
};
...
public:
	...
	enum class EUsageFlags : uint8
	{
		...
		/** When bindless is enabled, allow this uniform buffer to be accessed via a bindless constant buffer. */
	...
	inline const FString& GetUniformBufferPath() const { return UniformBufferPath; }
	inline const FString& GetUniformBufferInclude() const { return UniformBufferInclude; }
	inline uint32 GetUniformBufferPathHash() const { return UniformBufferPathHash; }
#endif // WITH_EDITOR
