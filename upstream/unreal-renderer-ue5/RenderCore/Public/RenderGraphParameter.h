...
public:
	...
	{
		check(MemberType == UBMT_RDG_BUFFER_SRV || MemberType == UBMT_RDG_UNIFORM_BLOCK);
...
public:
	...
	inline bool HasRenderTargets() const   { return Layout->HasRenderTargets(); }
	inline bool HasExternalOutputs() const { return Layout->HasExternalOutputs(); }
	inline bool HasStaticSlot() const      { return Layout->HasStaticSlot(); }
...
inline static FRHIRenderPassInfo GetRenderPassInfo(TParameterStruct* Parameters)
{
	return FRDGParameterStruct(Parameters, TParameterStruct::FTypeInfo::GetStructMetadata()).GetRenderPassInfo();
}

template <typename TParameterStruct>
inline static bool HasRenderPassInfo(TParameterStruct* Parameters)
{
...
template <typename TParameterStruct>
inline static FUniformBufferStaticBindings GetStaticUniformBuffers(TParameterStruct* Parameters)
{
