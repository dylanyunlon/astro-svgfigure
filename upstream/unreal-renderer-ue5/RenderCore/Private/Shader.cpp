...
void FShaderParameterMap::VerifyBindingsAreComplete(const TCHAR* ShaderTypeName, FShaderTarget Target, const FVertexFactoryType* InVertexFactoryType) const
{
	...
	if (UE_LOG_ACTIVE(LogShaders, Warning))
	{
		...
		{
			FString ErrorMessage = FString("Found unbound parameters being used in shadertype ") + ShaderTypeName + TEXT(" (VertexFactory: ") + VertexFactoryName + TEXT(")\n") + UnBoundParameters;
...
FShaderType::FShaderType(
	...
	, TotalSpecializationCount(InTotalSpecializationCount)
	...
	, GetUnspecializedIdRef(InGetUnspecializedIdRef)
	, GetSpecializationValuesRef(InGetSpecializationValuesRef)
...
FArchive& operator<<(FArchive& Ar,FShaderType*& Ref)
{
	...
	else if(Ar.IsLoading())
	{
		...
		if(ShaderTypeName != NAME_None)
		{
			...
			{
				UE_LOGF(LogShaders, Verbose, "ShaderType '%ls' dependency was not found.", *ShaderTypeName.ToString());
			}
		}
	}
	return Ar;
...
bool FShaderType::IsSpecialization(const int32 PermutationId) const
{
	return PermutationId != (*GetUnspecializedIdRef)(PermutationId);
}

int32 FShaderType::GetUnspecializedId(const int32 PermutationId) const
{
	return (TotalSpecializationCount > 0) ? (*GetUnspecializedIdRef)(PermutationId) : PermutationId;
}

FSpecializationContainerType FShaderType::GetSpecializationValues(const int32 PermutationId) const
{
	return (*GetSpecializationValuesRef)(PermutationId);
}
...
void FShaderType::ModifyCompilationEnvironment(const FShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment) const
{
	...
	switch (Frequency)
	{
		...
		default:
		{
			// not a raytracing shader, specifying a payload type would suggest some confusion has occurred
	...
	{
		checkf(IsRayTracingPayloadRegistered(RayTracingPayloadType), TEXT("Raytracing shader %s is using a payload type (%u) which was never registered"), Name, EnumToUnderlyingType(RayTracingPayloadType));
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FShaderType::Initialize);
...
#endif // WITH_EDITORONLY_DATA
{
	checkSlow(Initializer.OutputHash != FShaderHash());
...
void FShader::Finalize(FShaderMapResourceCode* Code)
{
	const FShaderHash& Hash = GetOutputHash();
	const int32 NewResourceIndex = Code->AssignShaderResourceIndex(Hash);
...
const FShaderHash& FShader::GetOutputHash() const
{
#if WITH_EDITORONLY_DATA
	return OutputHash;
#else
	static const FShaderHash DefaultOutputHash;
	return DefaultOutputHash;
#endif
}
...
void FShader::DumpDebugInfo(const FShaderMapPointerTable& InPtrTable)
{
	...
	UE_LOGF(LogConsoleResponse, Display, "      FShader  :Frequency %ls", GetShaderFrequencyString(GetFrequency()));
	UE_LOGF(LogConsoleResponse, Display, "               :Target %ls", *LegacyShaderPlatformToShaderFormat(GetShaderPlatform()).ToString());
	UE_LOGF(LogConsoleResponse, Display, "               :VFType %ls", VertexFactoryType ? VertexFactoryType->GetName() : TEXT("null"));
	UE_LOGF(LogConsoleResponse, Display, "               :Type %ls", GetType(InPtrTable)->GetName());
	UE_LOGF(LogConsoleResponse, Display, "               :SourceHash %ls", *GetHash().ToString());
	UE_LOGF(LogConsoleResponse, Display, "               :VFSourceHash %ls", *GetVertexFactoryHash().ToString());
	UE_LOGF(LogConsoleResponse, Display, "               :OutputHash %ls", *GetOutputHash().ToString());
}
...
void ShaderMapAppendKey(EShaderPlatform Platform, FShaderKeyGenerator& KeyGen)
{
...

	{
		...
		KeyGen.AppendSeparator();
		KeyGen.Append((CVar && CVar->GetValueOnAnyThread() != 0 && (!IsMobilePlatform(Platform) || !IsMobileDeferredShadingEnabled(Platform) || MobileUsesExtendedGBuffer(Platform))) ? TEXT("CCBN") : TEXT("NoCCBN"));
	}
	...
	{
		static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Material.UseShaderCompilationParameters"));
		KeyGen.AppendSeparator();
		KeyGen.Append((CVar && CVar->GetValueOnAnyThread() != 0) ? TEXT("CSP") : TEXT("NoCSP"));
	}
	...
	if (SupportsSkyLightCaptureAffectsHeightFogInScattering(Platform))
	{
		KeyGen.AppendSeparator();
		KeyGen.Append(TEXT("SkyCapFog"));
	}
	...
	if (IsMobilePlatform(Platform))
	{
		...
		{
			static IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Mobile.EnableClusteredDeferredShadingPermutations"));
			if (CVar)
			{
				KeyGen.AppendSeparator();
				KeyGen.Append(TEXT("Clust"));
				KeyGen.Append(CVar->GetInt());
			}
		}
		...
		
		{
			...
			{
				KeyGen.AppendSeparator();
				KeyGen.Append(MobileUsesExtendedGBuffer(Platform) ? TEXT("MobDShEx") : TEXT("MobDSh"));
			}
			else
			{
	...
	if (IsMetalPlatform(Platform))
	{
		...
		bool bAllowsRuntimeShaderCompiling = false;
		bool bStripIndividualMetalLibs = false;
		
		const bool bIsMac = IsPCPlatform(Platform);
		const TCHAR* RuntimeSettingsClassName = bIsMac ? FMacPlatformProperties<false, false, false>::GetRuntimeSettingsClassName() : FIOSPlatformProperties::GetRuntimeSettingsClassName();

		GConfig->GetBool(RuntimeSettingsClassName, TEXT("UseFastIntrinsics"), bAllowFastIntrinsics, GEngineIni);
		GConfig->GetBool(RuntimeSettingsClassName, TEXT("EnableMathOptimisations"), bEnableMathOptimisations, GEngineIni);
		GConfig->GetBool(RuntimeSettingsClassName, TEXT("ForceFloats"), bForceFloats, GEngineIni);
		GConfig->GetBool(RuntimeSettingsClassName, TEXT("MetalOptimizeBySize"), bMetalOptimizeBySize, GEngineIni);
		GConfig->GetBool(RuntimeSettingsClassName, TEXT("AllowsRuntimeShaderCompiling"), bAllowsRuntimeShaderCompiling, GEngineIni);
		GConfig->GetBool(RuntimeSettingsClassName, TEXT("StripIndividualMetalLibs"), bStripIndividualMetalLibs, GEngineIni);
		...
		if(bAllowsRuntimeShaderCompiling)
		{
			KeyGen.AppendSeparator();
			KeyGen.Append(TEXT("AllowsRuntimeShaderCompiling"));
		}
		
		if(bStripIndividualMetalLibs)
		{
			KeyGen.AppendSeparator();
			KeyGen.Append(TEXT("StripIndividualMetalLibs"));
		}
...

	{
		...
		if (CVar && CVar->GetValueOnAnyThread() > 0)
		{
			...
			static const auto CVarHeightFog = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.SupportsSkyAtmosphereAffectsHeightFog"));
	...
	if (Substrate::IsSubstrateEnabled())
	{
...

		{
			...
			// We append all closure count to the key from low to epic material quality levels for the current platfom.
			KeyGen.Append(Substrate::GetClosurePerPixel(Platform, 0));
			KeyGen.Append(Substrate::GetClosurePerPixel(Platform, 1));
			KeyGen.Append(Substrate::GetClosurePerPixel(Platform, 2));
			KeyGen.Append(Substrate::GetClosurePerPixel(Platform, 3));
		}
...

	{
		...
		if (GetMaterialSimulateMobileForLowQuality())
		{
			KeyGen.AppendSeparator();
			KeyGen.Append(TEXT("MATMOBSIM"));
		}
	}
	...
	{
		const bool VTLightmaps = UseVirtualTextureLightmap(Platform, TargetPlatform);
		const bool VTTextures = UseVirtualTexturing(TargetPlatform);
		...
		KeyGen.AppendDebugText(TEXT("-"));
		KeyGen.Append(VTTextures); // Field repeated to maintain KeyGen layout and avoid unwanted global invalidation.
		KeyGen.AppendDebugText(TEXT("-"));
	...
	{
		const ERHIBindlessConfiguration BindlessConfig = FShaderPlatformConfig::GetBindlessConfiguration(Platform);
	...
	if (DoesRuntimeSupportNanite(Platform, false, true))
	{
		...
		KeyGen.Append(NaniteSplineMeshesSupported() ? 1 : 0);
		KeyGen.AppendDebugText(TEXT("Skinned"));
		KeyGen.Append(NaniteSkinnedMeshesSupported() ? 1 : 0);
		...
		KeyGen.AppendDebugText(TEXT("Curves"));
		KeyGen.Append(NaniteCurvesSupported() ? 1 : 0);
		KeyGen.AppendDebugText(TEXT("Translucency"));
		KeyGen.Append(NaniteTranslucencySupported() ? 1 : 0);
	}
}
...
uint32 GetRayTracingPayloadTypeMaxSize(ERayTracingPayloadType PayloadType)
{
	...
	uint32 Result = 0;
	checkf(IsRayTracingPayloadRegistered(PayloadType), TEXT("Payload type %u has not been registered"), uint32{EnumToUnderlyingType(PayloadType)});
