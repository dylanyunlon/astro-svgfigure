	...
	uint32 NumColumns)
{
	...
	case UBMT_DESCRIPTOR_RANGE:
		Result = TEXT("FDescriptorRange");
		break;
	default:
		UE_LOGF(LogShaders, Fatal, "Unrecognized uniform buffer struct member base type.");
	};
...
const TCHAR* const kShaderParameterMacroNames[] = {
	...
	TEXT("DESCRIPTOR_RANGE"), // UBMT_DESCRIPTOR_RANGE,
	
	TEXT("SHADER_PARAMETER_RDG_UNIFORM_BLOCK"), // UBMT_RDG_UNIFORM_BLOCK,
};
...
EShaderCodeResourceBindingType ParseShaderResourceBindingType(const TCHAR* ShaderType)
{
	...
	else if (ShaderTypeLength == 18 && FCString::Strncmp(ComparedShaderType, TEXT("Texture2DMultiView"), ShaderTypeLength) == 0)
	{
		BindingType = bIsRWResource ? EShaderCodeResourceBindingType::RWTexture2DMultiView : EShaderCodeResourceBindingType::Texture2DMultiView;
	}
	...
	else if (!bIsRWResource && ShaderTypeLength == 23 && FCString::Strncmp(ComparedShaderType, TEXT("FIndirectVirtualTexture"), ShaderTypeLength) == 0)
	{
		BindingType = EShaderCodeResourceBindingType::Buffer;
	}
	else if (!bIsRWResource && ShaderTypeLength == 16 && FCString::Strncmp(ComparedShaderType, TEXT("FDescriptorRange"), ShaderTypeLength) == 0)
	{
		BindingType = EShaderCodeResourceBindingType::DescriptorRange;
	}
...
void FShaderParametersMetadata::InitializeLayout(FRHIUniformBufferLayoutInitializer* OutLayoutInitializer)
{
	...
	if (EnumHasAnyFlags(UsageFlags, EUsageFlags::NoEmulatedUniformBuffer))
	{
	...
	if (EnumHasAnyFlags(UsageFlags, EUsageFlags::BindlessAccessible))
	{
		EnumAddFlags(LayoutInitializer.Flags, ERHIUniformBufferFlags::BindlessAccessible);
	}
	...
	else
	{
		checkf(!EnumHasAnyFlags(BindingFlags, EUniformBufferBindingFlags::Static), TEXT("Uniform buffer of type '%s' and shader name '%s' has no binding slot specified, but the binding model specifies 'Global'."),
			StructTypeName, ShaderVariableName);
	}
	...
	for (int32 i = 0; i < MemberStack.Num(); ++i)
	{
		...
		if (DO_CHECK)
		{
			...
			{
				UE_LOGF(LogRendererCore, Fatal,
					"%lsbool are actually illegal in shader parameter structure, " "because bool type in HLSL means using scalar register to store binary information. " "Boolean information should always be packed explicitly in bitfield to reduce memory footprint, " "and use HLSL comparison operators to translate into clean SGPR, to have minimal VGPR footprint.", *GetMemberErrorPrefix());
			}
			...
			if (BaseType == UBMT_REFERENCED_STRUCT || BaseType == UBMT_RDG_UNIFORM_BUFFER)
			{
				...
				{
					UE_LOGF(LogRendererCore, Fatal, "%lsShader parameter struct reference can only be done in shader parameter structs.", *GetMemberErrorPrefix());
				}
			}
			else if (BaseType == UBMT_RDG_UNIFORM_BLOCK)
			{
				if (UseCase != EUseCase::UniformBuffer)
				{
					UE_LOGF(LogRendererCore, Fatal, "%lsSHADER_PARAMETER_RDG_UNIFORM_BLOCK is only allowed inside a uniform buffer struct (BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT / BEGIN_UNIFORM_BUFFER_STRUCT).", *GetMemberErrorPrefix());
				}
			}
			else if (BaseType == UBMT_NESTED_STRUCT || BaseType == UBMT_INCLUDED_STRUCT)
			{
				...
				{
					UE_LOGF(LogRendererCore, Fatal, "%lsShader parameter struct is not known inline other structures.", *GetMemberErrorPrefix());
				}
				else if (ChildStruct->GetUseCase() != EUseCase::ShaderParameterStruct && UseCase == EUseCase::ShaderParameterStruct)
				{
					UE_LOGF(LogRendererCore, Fatal, "%lsCan only nest or include shader parameter struct define with BEGIN_SHADER_PARAMETER_STRUCT(), but %ls is not.", *GetMemberErrorPrefix(), ChildStruct->GetStructTypeName());
				}
			}

			if (UseCase != EUseCase::ShaderParameterStruct && IsShaderParameterTypeIgnoredByRHI(BaseType))
			{
				UE_LOGF(LogRendererCore, Fatal, "Shader parameter %ls is not allowed in a uniform buffer.", *GetMemberErrorPrefix());
			}
			...
			{
				UE_LOGF(LogRendererCore, Fatal, "%lsNot allowed to be an array.", *GetMemberErrorPrefix());
			}

			// Validate the shader binding type.
			if (bIsRHIResource || (bIsRDGResource && !IsRDGResourceAccessType(BaseType) && BaseType != UBMT_RDG_UNIFORM_BUFFER && BaseType != UBMT_RDG_UNIFORM_BLOCK))
			{
				...
				{
					UE_LOGF(LogRendererCore, Warning, "%lsUnknown shader parameter type %ls.", *GetMemberErrorPrefix(), ShaderType);
				}
				...
				if (BindingType == EShaderCodeResourceBindingType::SamplerState)
				{
					...
					BindingType == EShaderCodeResourceBindingType::Texture2DMultiView ||
					BindingType == EShaderCodeResourceBindingType::Texture2DMS ||
				...
				else if (BindingType == EShaderCodeResourceBindingType::RaytracingAccelerationStructure)
				{
					...
					BindingType == EShaderCodeResourceBindingType::RWTexture2DMultiView ||
				...
				else if (BindingType == EShaderCodeResourceBindingType::DescriptorRange)
				{
					bIsValidBindingType = BaseType == UBMT_DESCRIPTOR_RANGE;
				}
				else
				{
					//unimplemented();
				...
				if (!bIsValidBindingType)
				{
					...
					UE_LOGF(
						...
						"%lsUnable to bind a shader parameter type %ls with a %ls.",
...
// Register a synthetic FUniformBufferEntry for every UBMT_RDG_UNIFORM_BLOCK
static void RegisterUniformBlockEntries(const TCHAR* UniformBufferName, const FString& GlobalPrefix, TConstArrayView<FShaderParametersMetadata::FMember> Members, TMap<FString, FUniformBufferEntry>& UniformBufferMap)
{
	for (const FShaderParametersMetadata::FMember& Member : Members)
	{
		const EUniformBufferBaseType BaseType = Member.GetBaseType();

		if (BaseType == UBMT_RDG_UNIFORM_BLOCK)
		{
			const FString CBufferName = GlobalPrefix.IsEmpty()
				? FString::Printf(TEXT("%s_%s"), UniformBufferName, Member.GetName())
				: FString::Printf(TEXT("%s_%s_%s"), UniformBufferName, *GlobalPrefix, Member.GetName());

			FUniformBufferEntry Entry;
			Entry.MemberNameBuffer = MakeShareable(new TArray<TCHAR>());
			EnumAddFlags(Entry.Flags, ERHIUniformBufferFlags::NoEmulatedUniformBuffer);
			UniformBufferMap.Add(CBufferName, Entry);
		}
		else if (BaseType == UBMT_NESTED_STRUCT && Member.GetNumElements() == 0)
		{
			check(Member.GetStructMetadata());
			const FString NewGlobalPrefix = GlobalPrefix.IsEmpty()
				? FString(Member.GetName())
				: FString::Printf(TEXT("%s_%s"), *GlobalPrefix, Member.GetName());
			RegisterUniformBlockEntries(UniformBufferName, NewGlobalPrefix, Member.GetStructMetadata()->GetMembers(), UniformBufferMap);
		}
		else if (BaseType == UBMT_INCLUDED_STRUCT)
		{
			check(Member.GetStructMetadata());
			RegisterUniformBlockEntries(UniformBufferName, GlobalPrefix, Member.GetStructMetadata()->GetMembers(), UniformBufferMap);
		}
	}
}
...
void FShaderParametersMetadata::AddResourceTableEntries(FShaderResourceTableMap& ResourceTableMap, TMap<FString, FUniformBufferEntry>& UniformBufferMap) const
{
	...
	RegisterUniformBlockEntries(ShaderVariableName, FString(), Members, UniformBufferMap);
}
