...
void IssueScalableLongGPUTask(FRHICommandListImmediate& RHICmdList, int32 NumIteration /* = -1 by default */)
{
	...
	FTextureRHIRef LongTaskRenderTarget = RHICmdList.CreateTexture(Desc);
...
void MeasureLongGPUTaskExecutionTime(FRHICommandListImmediate& RHICmdList)
{
	...
	else
	{
		...
		NumMeasuredIterationsToAchieve500ms = 5;
		UE_LOGF(LogTemp, Display, "Unable to get render query result on a platform supporting RQT_AbsoluteTime queries, defaulting to %d iterations for LongGPUTask", NumMeasuredIterationsToAchieve500ms);
	}
}
