...
FAutoConsoleCommandWithArgsAndOutputDevice GListShaderCmd(
	...
	FConsoleCommandWithArgsAndOutputDeviceDelegate::CreateStatic(
		[](const TArray<FString>& Params, FOutputDevice& Out)
		{
			...
			{
				UE_LOGF(LogShaders, Warning, "Enable r.TrackShaderMaps in ini config to enable the functionality");
				return;
			}
			Out.Logf(TEXT("FriendlyName,OwnerName,ShaderMapHash,VertexFactoryName,ShaderTypeName,ShaderPermutation,UncompressedSizeKb,Hash,UsedForRenderingRHIPtr"));
			FScopeLock SMAccess(&GAllShaderMapsGuard);
			for (FShaderMapBase* ShaderMap : GAllShaderMaps)
			{
				TStringBuilder<FShaderHash::GetStringLen()+1> ShaderMapHash; // +1 for null terminator (appended when calling ToString)
				if (ShaderMap->GetResource())
				{
					ShaderMapHash << ShaderMap->GetResource()->GetShaderMapHash();
				}
				...
				Report.BindLambda(
					[&Out, &ShaderMapHash](
						...
						void* Ptr)
					{
						Out.Logf(TEXT("%s,%s,%s,%s,%s,%d,%.2f,%s,%p"), FriendlyName, OwnerName, ShaderMapHash.ToString(), VertexFactory, ShaderType, Permutation, Bytes / 1024.f, Hash, Ptr);
					}
				);
...
FAutoConsoleCommandWithArgsAndOutputDevice GListShaderMapsCmd(
	...
	FConsoleCommandWithArgsAndOutputDeviceDelegate::CreateStatic(
		[](const TArray<FString>& Params, FOutputDevice& Out)
		{
			...
			for (FShaderMapBase* ShaderMap : GAllShaderMaps)
			{
				FString FriendlyName = "Unknown";
				FString OwnerName = "Unknown";
...
void FShaderMapBase::AssignCopy(const FShaderMapBase& Source)
{
	...
	static TCsvPersistentCustomStat<float>* CsvStatShaderMemoryMB = FCsvProfiler::Get()->GetOrCreatePersistentCustomStatFloat(TEXT("ShaderMemoryMB"), CSV_CATEGORY_INDEX(Shaders));
...
bool FShaderMapBase::Serialize(FShaderSerializeContext& Ctx, uint64* OutCookedShaderMapContentHash)
{
	...
	if (Ar.IsSaving())
	{
...

		{
			...
			if (OutCookedShaderMapContentHash && Ar.IsCooking())
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(FShaderMapBase::ComputeCookedShaderMapContentHash);

				TMemoryHasher<FXxHash64Builder, FXxHash64> Hasher;
				MemoryImageResult.SaveToArchive(Hasher);
				*OutCookedShaderMapContentHash = Hasher.Finalize().Hash;
			}
		...
		if (bShareCode)
		{
			FShaderHash ResourceHash = Code->ResourceHash;
			Ar << ResourceHash;
	...
	else
	{
		...
		if (bShareCode)
		{
			...
			if (!Resource)
			{
				...
				{
					UE_LOGF(LogShaders, Error, "Missing shader resource for hash '%ls' for shader platform '%ls' in the shader library while serializing asset %ls", *ResourceHash.ToString(),
...
FShaderMapContent::FShaderMapContent(EShaderPlatform InPlatform)
	: ShaderHash(128u)
	, ShaderPlatformName(FDataDrivenShaderPlatformInfo::GetName(InPlatform))
#if WITH_EDITORONLY_DATA
	, bSorted(false)
#endif // WITH_EDITORONLY_DATA
{}
...
static void RetrieveShaderInfo(uint32& OutShaderCodeSizeBytes, FRHIShader*& OutRHIShader, FString& OutHash, FShaderMapResource* Resource, FShader* Shader)
{
	if (Resource)
	{
		if (Resource->HasShader(Shader->GetResourceIndex()))
		{
			OutRHIShader = Resource->GetShader(Shader->GetResourceIndex());
		}
		const uint32 Index = Shader->GetResourceIndex();
		OutShaderCodeSizeBytes = Resource->GetShaderSizeBytes(Index);
		OutHash = Resource->GetShaderHash(Index).ToString();
	}
}
...
void FShaderMapContent::DumpShaderList(const FShaderMapBase& InShaderMap, FShaderListReport& Out) const
{
	...
	for (int32 ShaderIndex = 0; ShaderIndex < Shaders.Num(); ++ShaderIndex)
	{
		...
		FString Hash;
		RetrieveShaderInfo(ShaderCodeSizeBytes, RHIShader, Hash, Resource, Shader);
...
void FShaderMapContent::GetShaderList(const FShaderMapBase& InShaderMap, uint64 InMaterialShaderMapHash, TMap<FShaderId, TShaderRef<FShader>>& OutShaders) const
{
	for (int32 ShaderIndex = 0; ShaderIndex < Shaders.Num(); ++ShaderIndex)
	{
		FShader* Shader = Shaders[ShaderIndex].GetChecked();
...
void FShaderMapContent::Finalize(FShaderMapResourceCode* Code)
{
	check(Code);

	// Finalize should be called after sorting, so shaders are sorted when their ResourceIndex is assigned
	SortShaders();
	FinalizeShaders(Code);
}

void FShaderMapContent::SortShaders() 
{
...
void FShaderMapContent::FinalizeShaders(FShaderMapResourceCode* Code)
{
	check(Code);
#if WITH_EDITORONLY_DATA
	checkf(bSorted, TEXT("Finalize called on unsorted ShaderMap content. This can lead to invalid ResourceIndex values."));
#endif
