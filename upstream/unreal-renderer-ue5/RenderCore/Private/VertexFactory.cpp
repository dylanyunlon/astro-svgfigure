...
bool FVertexFactory::AddPrimitiveIdStreamElement(EVertexInputStreamType InputStreamType, FVertexDeclarationElementList& Elements, uint8 AttributeIndex)
{
	if (GetType()->SupportsGPUScene(GMaxRHIFeatureLevel))
	{
		// UniformView path does not use PrimitiveId stream, we still need to set it to a non-negative index
		int32 AddedStreamIndex = TNumericLimits<int8>::Max();
		...
		SetPrimitiveIdStreamIndex(InputStreamType, AddedStreamIndex);
		return true;
	}
	return false;
}
...
{
	return AddPrimitiveIdStreamElement(InputStreamType, Elements, AttributeIndex);
}
