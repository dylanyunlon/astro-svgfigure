...
FString FRDGScope::GetFullPath(FRDGEventName const& PassName)
{
	FString Path;
#if RDG_EVENTS
	TArray<FRDGScope_RHI*, TInlineAllocator<16>> ScopeStack;
	...
	for (FRDGScope* Current = Parent; Current; Current = Current->Parent)
	{
		...
		{
			ScopeStack.Add(RHIScope);
			TotalLen += FCString::Strlen(RHIScope->GetTCHAR(Buffer)) + 1;
		}
	}

	if (ScopeStack.Num())
	{
		const TCHAR* PassNameStr = PassName.GetTCHAR();
		Path.Reserve(TotalLen + FCString::Strlen(PassNameStr));
		for (int32 Index = ScopeStack.Num() - 1; Index >= 0; --Index)
		{
			Path += ScopeStack[Index]->GetTCHAR(Buffer);
