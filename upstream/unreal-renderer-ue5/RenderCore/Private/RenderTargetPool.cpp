...
static int32 GRenderTargetPoolLogCreationSizes = 0;
static FAutoConsoleVariableRef CVarRenderTargetPoolLogCreationSizes(
	TEXT("r.RenderTargetPool.LogCreationSizes"),
	GRenderTargetPoolLogCreationSizes,
	TEXT("Enable/disable warning log for render target pool creation sizes."));

static int32 GRenderTargetPoolLogCreationSizesSpamInterval = 4; // 4 per frames
static FAutoConsoleVariableRef CVarRenderTargetPoolLogCreationSpamInterval(
	TEXT("r.RenderTargetPool.LogCreationSizesSpamInterval"),
	GRenderTargetPoolLogCreationSizesSpamInterval,
	TEXT("The number of logs in a given frame that are generated before we force a print."));

static int32 GRenderTargetPoolLogCreationSizesSizeInterval = 1024 * 1024; // 1 MB
static FAutoConsoleVariableRef CVarRenderTargetPoolLogCreationSizeInterval(
	TEXT("r.RenderTargetPool.LogCreationSizesSizeInterval"),
	GRenderTargetPoolLogCreationSizesSizeInterval,
	TEXT("The total size of accumulated logs during a frame before we force a print."));

static void LogRenderTargetCreationSizeAndFrequencies(const FRHITextureCreateInfo& Desc, const TCHAR* Name)
{
#if !UE_BUILD_SHIPPING && !UE_SERVER
	static FString AccumulatedLogs;
	static uint64 AccumulatedCreatedTargets = 0;
	static uint64 AccumulatedCreatedTargetSizes = 0;
	static uint64 TrackingCurrentFrame = 0;
	
	if (GRenderTargetPoolLogCreationSizes <= 0)
	{
		AccumulatedLogs.Empty();
		AccumulatedCreatedTargets = 0;
		AccumulatedCreatedTargetSizes = 0;
		TrackingCurrentFrame = 0;
		return;
	}

	// Log last frame's results IFF we are over one or more of our target
	// thresholds. Either way, clear the accumulators for the current frame.
	const uint64 CurrentRTFrame = GFrameCounterRenderThread;
	if (TrackingCurrentFrame != CurrentRTFrame)
	{
		if (AccumulatedCreatedTargets >= GRenderTargetPoolLogCreationSizesSpamInterval ||
			AccumulatedCreatedTargetSizes >= GRenderTargetPoolLogCreationSizesSizeInterval)
		{
			UE_LOGF(LogRenderTargetPool, Warning, "FRenderTargetPool::CreateRenderTarget(s) since last frame:\n%ls", *AccumulatedLogs);
		}

		AccumulatedLogs.Empty();
		AccumulatedCreatedTargets = 0;
		AccumulatedCreatedTargetSizes = 0;
		TrackingCurrentFrame = CurrentRTFrame;
	}

	const uint64 CreationSize = RHICalcTexturePlatformSize(Desc).Size;
	AccumulatedCreatedTargets += 1;
	AccumulatedCreatedTargetSizes += CreationSize;
	AccumulatedLogs += FString::Printf(
		TEXT("\tRenderTargetCreation { \"%s\", Size(%.2f KB) }\n"), Name, CreationSize / 1024.0);
#endif
}
...
FPooledRenderTarget* FRenderTargetPool::CreateRenderTarget(FRHICommandListBase& RHICmdList, const FRHITextureCreateInfo& Desc, uint32 DescHash, const TCHAR* Name)
{
	...
	UE_TRACE_LOG_SCOPED_T(Cpu, FRenderTargetPool_CreateTexture, CpuChannel)
		...
		<< FRenderTargetPool_CreateTexture.Width(Desc.Extent.X)
		<< FRenderTargetPool_CreateTexture.Height(Desc.Extent.Y);
#endif

	LogRenderTargetCreationSizeAndFrequencies(Desc, Name);

	FRHITextureCreateDesc CreateDesc(Desc, ERHIAccess::SRVMask, Name);
...
FPooledRenderTarget* FRenderTargetPool::TryFindRenderTarget(const FRHITextureCreateInfo& Desc, uint32 DescHash, T&& Predicate) const
{
	for (uint32 Index = 0, Num = (uint32)PooledRenderTargets.Num(); Index < Num; ++Index)
	{
		if (PooledRenderTargetHashes[Index] == DescHash)
		{
			...
			checkf(Element && Element->GetRHI(), TEXT("Hash was not cleared from the list."));

			// Compare the descriptors to avoid hash collisions.
			if (Element->GetRHI()->GetDesc() != Desc)
			{
				continue;
			}
...
bool FRenderTargetPool::FindFreeElement(FRHICommandListBase& RHICmdList, const FRHITextureCreateInfo& Desc, TRefCountPtr<IPooledRenderTarget>& Out, const TCHAR* Name)
{
	...
	if (Out)
	{
		...
		if (Out->GetRHI() && Out->GetRHI()->GetDesc() == Desc)
		{
...
void FRenderTargetPool::TickPoolElements()
{
	...
	uint32 MinimumPoolSizeInKB;
	{
		...
		MinimumPoolSizeInKB = FMath::Max(CVar->GetValueOnRenderThread(), 0) * 1024;
	}
	...
	while (AllocationLevelInKB > MinimumPoolSizeInKB)
	{
		...
		else
		{
			...
			{
				UE_CLOGF(IsRunningClientOnly() && MinimumPoolSizeInKB != 0, LogRenderTargetPool, Warning, "r.RenderTargetPoolMin exceeded %d/%d MB (ok in editor, bad on fixed memory platform)", (AllocationLevelInKB + 1023) / 1024, MinimumPoolSizeInKB / 1024);
				bCurrentlyOverBudget = true;

#if RHI_ENABLE_RESOURCE_INFO
				if (bLogResourceUsageOnFirstOverflow)
				{
					int32 NumberOfResourcesToShow = 100;
					RHIDumpResourceMemoryToLog(NumberOfResourcesToShow);
					bLogResourceUsageOnFirstOverflow = false;
				}
	...
	if (AllocationLevelInKB <= MinimumPoolSizeInKB)
	{
		if (bCurrentlyOverBudget)
		{
			UE_CLOGF(MinimumPoolSizeInKB != 0, LogRenderTargetPool, Display, "r.RenderTargetPoolMin resolved %d/%d MB", (AllocationLevelInKB + 1023) / 1024, MinimumPoolSizeInKB / 1024);
