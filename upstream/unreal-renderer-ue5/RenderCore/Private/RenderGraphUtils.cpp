...
static TAutoConsoleVariable<bool> CVarRDGBarrierPass(
	TEXT("r.RDG.BarrierPass"),
	true,
	TEXT("Whether to allow AddBarrierPass. AddBarrierPass is only used on plaforms which don't support split barriers and allows to manually batch barriers for optimal GPU dispatch overlap."),
	...
	const FRDGDrawTextureInfo& DrawInfo)
{
	...
	else
	{
		...
		for (uint32 MipIndex = 0; MipIndex < DrawInfo.NumMips; ++MipIndex)
		{
			...
			// CreateForMipLevel leaves NumArraySlices at ArraySize for array sources; narrow to the single slice we
			// bind so FirstArraySlice + NumArraySlices stays in range, and view as Texture2D to match FDrawTexturePS.
			if (InputTexture->Desc.IsTextureArray())
			{
				SRVDesc.NumArraySlices = 1;
				SRVDesc.DimensionOverride = ETextureDimension::Texture2D;
			}
...
void AddClearUAVPass(FRDGBuilder& GraphBuilder, ERHIFeatureLevel::Type FeatureLevel, FRDGTextureUAVRef TextureUAV, const uint32(&ClearValues)[4], FRDGBufferSRVRef RectCoordBufferSRV, uint32 NumRects)
{
	...
	auto* ShaderMap = GetGlobalShaderMap(GShaderPlatformForFeatureLevel[FeatureLevel]);
...
void AddEnqueueCopyPass(FRDGBuilder& GraphBuilder, FRHIGPUTextureReadback* Readback, FRDGTextureRef SourceTexture, FResolveRect Rect, uint32 SourceSlice)
{
	...
	GraphBuilder.AddPass(
		...
		[Readback, SourceTexture, Rect, SourceSlice](FRDGAsyncTask, FRHICommandList& RHICmdList)
	{
		Readback->EnqueueCopy(RHICmdList, SourceTexture->GetRHI(), Rect, SourceSlice);
	});
}
...
FRDGBufferRef FComputeShaderUtils::AddIndirectArgsSetupCsPass1D(FRDGBuilder& GraphBuilder, ERHIFeatureLevel::Type FeatureLevel, FRDGBuffer* InputCountBuffer, const TCHAR* OutputBufferName, uint32 Divisor, uint32 InputCountOffset, uint32 Multiplier)
{
	...
	FRDGBufferRef IndirectArgsBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateIndirectDesc<FRHIDispatchIndirectParameters>(), OutputBufferName);
	{
		...
		// ue-todo: Refactor to use EShaderPlatform directly instead of converting from FeatureLevel
		auto ComputeShader = GetGlobalShaderMap(GShaderPlatformForFeatureLevel[FeatureLevel])->GetShader<FInitIndirectArgs1DCS>();
...
BEGIN_SHADER_PARAMETER_STRUCT(FBarrierPassParameters, )
	RDG_TEXTURE_ACCESS_ARRAY(Textures)
	RDG_BUFFER_ACCESS_ARRAY(Buffers)
END_SHADER_PARAMETER_STRUCT()

void AddBarrierPass(FRDGBuilder& GraphBuilder, ERDGPassFlags PassFlags, FRDGTextureAccessArray Textures, FRDGBufferAccessArray Buffers)
{
	// Skip adding the barrier pass when split barriers are supported, as they intrinsically support overlap.
	if (!GRHIGlobals.SupportsSplitBarriers && CVarRDGBarrierPass.GetValueOnRenderThread())
	{
		FBarrierPassParameters* Parameters = GraphBuilder.AllocParameters<FBarrierPassParameters>();
		Parameters->Textures = MoveTemp(Textures);
		Parameters->Buffers  = MoveTemp(Buffers);
		GraphBuilder.AddPass(RDG_EVENT_NAME("BarrierPass"), Parameters, ERDGPassFlags::NeverCull | PassFlags, [&Parameters](FRDGAsyncTask, FRHICommandList& RHICmdList) {});
	}
}