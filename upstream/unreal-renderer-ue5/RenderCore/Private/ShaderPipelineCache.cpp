...
DECLARE_DWORD_COUNTER_STAT(TEXT("Graphic PSOs Compiled"), STAT_GraphicPSOsCompiled, STATGROUP_PipelineStateCache);
DECLARE_DWORD_COUNTER_STAT(TEXT("Graphic PSOs Skipped"), STAT_GraphicPSOsSkipped, STATGROUP_PipelineStateCache);
DECLARE_DWORD_COUNTER_STAT(TEXT("Compute PSOs Compiled"), STAT_ComputePSOsCompiled, STATGROUP_PipelineStateCache);
DECLARE_DWORD_COUNTER_STAT(TEXT("Compute PSOs Skipped"), STAT_ComputePSOsSkipped, STATGROUP_PipelineStateCache);
...
static TAutoConsoleVariable<int32> CVarPSOFileCachePSOPrecacheThreadPool(
															 TEXT("r.ShaderPipelineCache.PSOPrecacheThreadPoolBatchSize"),
															 500,
															 TEXT("Set the number of PipelineStateObjects to compile in a single batch operation when using the PSO precaching thread pool (only used when PSOPrecaching is enabled and r.ShaderPipelineCache.UsePrecachingThreadPool=1. Defaults to a maximum of 500 per frame, due to async. file IO it is less in practice."),
															 ECVF_Default | ECVF_RenderThreadSafe
															 );
...
static TAutoConsoleVariable<float> CVarPSOFileCachePSOPrecacheThreadPoolBatchTime(
														   TEXT("r.ShaderPipelineCache.PSOPrecacheThreadPoolBatchTime"),
														   16.0f,
														   TEXT("The target time (in ms) to spend precompiling each frame when compiling takes priority or 0.0 to disable. When precompiling is faster the batch size will grow and when slower will shrink to attempt to occupy the full amount. Defaults to 16.0 (max. ms per-frame of precompilation)."),
														   ECVF_Default | ECVF_RenderThreadSafe
														   );
...
static int32 GPSOPrecacheIncludeMode = 2;
static FAutoConsoleVariableRef CVarPSOFileCachePSOPrecacheIncludeMode(
	TEXT("r.ShaderPipelineCache.PSOPrecacheIncludeMode"),
	GPSOPrecacheIncludeMode,
	TEXT("Defines which PSOs from the file cache are compiled depending on the PSOPrecacheResult which is stored during collection:\n") \
	TEXT("0: Include all PSOs independent of PSO precache result\n") \
	TEXT("1: Only include PSOs which were not successfully compiled with PSO precaching (Missed, Untracked & TooLate)\n") \
	TEXT("2: Same as 1, but always precache all PSOs coming from global shaders (default)"),
	ECVF_ReadOnly
);
...
int32 GShaderPipelineCacheUsePrecachingThreadPool = 1;
static FAutoConsoleVariableRef CVarShaderPipelineCacheUsePrecachingThreadPool(
	TEXT("r.ShaderPipelineCache.UsePrecachingThreadPool"),
	GShaderPipelineCacheUsePrecachingThreadPool,
	TEXT("Use the PSO precaching thread pool when PSO precaching is enabled (default 1)"),
	ECVF_Default
);

static bool UsePSOPrecachingThreadPool()
{
	return GShaderPipelineCacheUsePrecachingThreadPool && PipelineStateCache::IsPSOPrecachingEnabled();
}
...
namespace UE
{
	namespace ShaderPipeline
	{
		...
		struct CompileJob
		{
			 ...
			 */
			bool PreloadShaders(TSet<FShaderHash>& OutRequiredShaders, bool& bOutCompatible);
...
public:
	...
	void PrecompilePipelineBatch(FRHICommandListImmediate& RHICmdList);
	bool ReadyForNextBatch() const;
	bool ReadyForAutoSave() const;
	void PollShutdownItems();
	void Flush();
	void Tick(FRHICommandListImmediate& RHICmdList, float DeltaTime);
	...
	if (ShaderPipelineCache != nullptr && CVarPSOFileCacheGameFileMaskEnabled.GetValueOnAnyThread())
	{
		...
		if (!ShaderPipelineCache->ShaderCacheTasks.IsEmpty())
		{
			...
			if (bMaskChanged)
			{
				...
				UE_LOGF(LogRHI, Display, "New ShaderPipelineCacheTask GameUsageMask [%llu=>%llu], queued %d cache(s) for precompile.", OldMask, InMask, PendingPSOCacheCount);

				return OldMask != InMask;
			}
			else
			{
				UE_LOGF(LogRHI, Display, "ShaderPipelineCache::SetGameUsageMaskWithComparison failed to set a new mask because the game mask was not different");
			}
		}
		else
		{
			...
			UE_LOGF(LogRHI, Display, "ShaderPipelineCache::SetGameUsageMaskWithComparison set a new mask but did not attempt to setup any tasks because there are no PSO caching tasks.");
			return OldMask != InMask;
		}
	}
	else
	{
		UE_LOGF(LogRHI, Display, "ShaderPipelineCache::SetGameUsageMaskWithComparison failed to set a new mask because the cache was not open or game mask is not enabled");
	}
...
void FShaderPipelineCache::PauseBatching()
{
	if (ShaderPipelineCache)
    {
        ...
        UE_LOGF(LogRHI, Display, "ShaderPipelineCache: Paused Batching. %d", ShaderPipelineCache->PausedCount);
...
void FShaderPipelineCache::SetBatchMode(BatchMode Mode)
{
	if (ShaderPipelineCache)
	{
		...
		if (UsePSOPrecachingThreadPool())
		{
			BatchSize = CVarPSOFileCachePSOPrecacheThreadPool.GetValueOnAnyThread();
			BatchTime = CVarPSOFileCachePSOPrecacheThreadPoolBatchTime.GetValueOnAnyThread();
		}
		else
		{
			switch (Mode)
		...
		UE_CLOGF(PreviousBatchSize != ShaderPipelineCache->BatchSize || PreviousBatchTime != ShaderPipelineCache->BatchTime, LogRHI, Log,
			"ShaderPipelineCache: Batch mode changed (%d): Size: %d -> %d, Time: %f -> %f", (uint32)Mode, PreviousBatchSize, ShaderPipelineCache->BatchSize, PreviousBatchTime, ShaderPipelineCache->BatchTime);
	}
}
...
void FShaderPipelineCache::ResumeBatching()
{
    ...
    {
        ShaderPipelineCache->PausedCount--;
        UE_LOGF(LogRHI, Display, "ShaderPipelineCache: Resumed Batching. %d", ShaderPipelineCache->PausedCount);
        if (ShaderPipelineCache->PausedCount <= 0)
        {
            UE_LOGF(LogRHI, Display, "ShaderPipelineCache: Batching Resumed.");
...
bool FShaderPipelineCache::OpenPipelineFileCache(EShaderPlatform Platform)
{
	...
	{
		UE_LOGF(LogRHI, Log, "Ignoring request to open static pipeline file cache because r.ShaderPipelineCache.OnlyOpenUserCache is set.");
		return false;
	}
...
bool FShaderPipelineCache::OpenUserPipelineFileCache(EShaderPlatform Platform)
{
	...
	if (ShaderPipelineCache)
	{
		...
		UserCacheTaskKey = UserCacheName + TEXT("_usr");
		if (UE::ShaderPipeline::ShouldLoadUserCache() && !ShaderPipelineCache->GetTask(UserCacheTaskKey))
		{
			bFileOpen = ShaderPipelineCache->OpenPipelineFileCacheInternal(true, UserCacheTaskKey, UserCacheName, Platform);
		}
	}
	return bFileOpen;
}
...
bool FShaderPipelineCache::SavePipelineFileCache(FPipelineFileCacheManager::SaveMode Mode)
{
	if (ShaderPipelineCache)
	{
		...
		UE_CLOGF(!bOK, LogRHI, Warning, "Failed to save shader pipeline cache for using save mode %d.", (uint32)Mode);
...
void FShaderPipelineCache::ShaderLibraryStateChanged(ELibraryState State, EShaderPlatform Platform, FString const& PSOCacheBaseName, int32 ComponentID)
{
	if (ShaderPipelineCache)
	{
		const FString PSOCacheName = UE::ShaderLibrary::Private::GetShaderLibraryNameForChunk(PSOCacheBaseName, ComponentID);
...
bool FShaderPipelineCacheTask::Precompile(FRHICommandListImmediate& RHICmdList, EShaderPlatform Platform, FPipelineCacheFileFormatPSO const& PSO)
{
	...
	if(PSO.Verify())
	{		
		FString GlobalLibraryName(TEXT("Global"));

		auto IsGlobalShader = [GlobalLibraryName](bool bGlobalShader, const FShaderHash& Hash)
		{
			return bGlobalShader && FShaderCodeLibrary::ContainsShaderCode(Hash, GlobalLibraryName);
		};

		auto ShouldCompilePSO = [PSO](bool bGlobalShader)
		{
			// Check if we should skip compiling this PSO due to PSO precaching state
			if (GPSOPrecacheIncludeMode > 0)
			{
				bool bIsPrecached = PSO.PSOPrecacheResult == EPSOPrecacheResult::Complete;

				// Include all global shaders when mode > 1
				if (GPSOPrecacheIncludeMode > 1)
				{
					if (bIsPrecached && !bGlobalShader)
					{
						return false;
					}
				}
				// Otherwise ignore all PSOs which are precached correctly
				else if (bIsPrecached)
				{
					return false;
				}
			}
		...
		if(FPipelineCacheFileFormatPSO::DescriptorType::Graphics == PSO.Type)
		{
...

			{
				...
				if (PSO.GraphicsDesc.MeshShader != FShaderHash())
				{
					...
					if (PSO.GraphicsDesc.AmplificationShader != FShaderHash())
					{
						...
						bOnlyGlobalShaders = IsGlobalShader(bOnlyGlobalShaders, PSO.GraphicsDesc.AmplificationShader);
					}
				}
				else
				{
					...
					if (PSO.GraphicsDesc.VertexShader != FShaderHash())
					{
						...
						bOnlyGlobalShaders = IsGlobalShader(bOnlyGlobalShaders, PSO.GraphicsDesc.VertexShader);
					}
					...
					if (PSO.GraphicsDesc.GeometryShader != FShaderHash())
					{
						...
						bOnlyGlobalShaders = IsGlobalShader(bOnlyGlobalShaders, PSO.GraphicsDesc.GeometryShader);
					}
#endif
				}
				...
				if (PSO.GraphicsDesc.FragmentShader != FShaderHash())
				{
					...
					bOnlyGlobalShaders = IsGlobalShader(bOnlyGlobalShaders, PSO.GraphicsDesc.FragmentShader);
				}
				...
				GraphicsInitializer.bAllowVariableRateShading = PSO.GraphicsDesc.bAllowVariableRateShading;
				GraphicsInitializer.ShadingRate = PSO.GraphicsDesc.ShadingRate;
			...
			if (ShouldCompilePSO(bOnlyGlobalShaders))
			{
				...
				if (UsePSOPrecachingThreadPool())
				{
					// Remove file cache flag because it's going over precaching code path				
					GraphicsInitializer.bFromPSOFileCache = false;
					GraphicsInitializer.StatePrecachePSOHash = RHIComputeStatePrecachePSOHash(GraphicsInitializer);

					static FString BundledGlobalGraphicsPSOCollectorName = TEXT("BundledGlobalGraphicsPSO");
					static FString BundledMeshPassGraphicsPSOCollectorName = TEXT("BundledMeshPassGraphicsPSO");

					// Async compile requests
					FPSOPrecacheRequestParams PrecacheRequestParams;
					PrecacheRequestParams.PrecachePriority = bOnlyGlobalShaders ? EPSOPrecachePriority::High : EPSOPrecachePriority::Medium;
					PrecacheRequestParams.InMemoryMode = bOnlyGlobalShaders ? EPSOPrecacheInMemoryMode::Cached : EPSOPrecacheInMemoryMode::Uncached;
					PrecacheRequestParams.CollectorName = bOnlyGlobalShaders ? *BundledGlobalGraphicsPSOCollectorName : *BundledMeshPassGraphicsPSOCollectorName;
					PipelineStateCache::PrecacheGraphicsPipelineState(GraphicsInitializer, PrecacheRequestParams);
				}
				else
				{
				...
				INC_DWORD_STAT(STAT_GraphicPSOsCompiled);
			}
			else
			{
				INC_DWORD_STAT(STAT_GraphicPSOsSkipped);
			}
		...
		else if(FPipelineCacheFileFormatPSO::DescriptorType::Compute == PSO.Type)
		{
			...
			{
				FComputeShaderRHIRef ComputeShaderRef = FShaderCodeLibrary::CreateComputeShader(Platform, PSO.ComputeDesc.ComputeShader);
				if (ComputeShaderRef.IsValid() && !ComputeShaderRef->HasSpecializationConstants())  // :todo-jn: add specialization support to CS precaching
				{
					bool bIsGlobalShader = IsGlobalShader(true, PSO.ComputeDesc.ComputeShader);

					if (ShouldCompilePSO(bIsGlobalShader))
					{
						FComputePipelineStateInitializer ComputeInitializer(ComputeShaderRef);
						if (UsePSOPrecachingThreadPool())
						{
							static FString BundledGlobalComputePSOCollectorName = TEXT("BundledGlobalComputePSO");
							static FString BundledMeshPassComputePSOCollectorName = TEXT("BundledMeshPassComputePSO");
						
							FPSOPrecacheRequestParams PrecacheRequestParams;
							PrecacheRequestParams.PrecachePriority = bIsGlobalShader ? EPSOPrecachePriority::High : EPSOPrecachePriority::Medium;
							PrecacheRequestParams.InMemoryMode = bIsGlobalShader ? EPSOPrecacheInMemoryMode::Cached : EPSOPrecacheInMemoryMode::Uncached;
							PrecacheRequestParams.CollectorName = bIsGlobalShader ? *BundledGlobalComputePSOCollectorName : *BundledMeshPassComputePSOCollectorName;

							PipelineStateCache::PrecacheComputePipelineState(ComputeInitializer, PrecacheRequestParams);
							bOk = true;
						}
						else
						{
							ComputeInitializer.bFromPSOFileCache = true;
						...
						INC_DWORD_STAT(STAT_ComputePSOsCompiled);
					}
					else
					{
						INC_DWORD_STAT(STAT_ComputePSOsSkipped);
						bOk = true;
					}
				}
			}
	...
	{
		UE_LOGF(LogRHI, Warning, "FShaderPipelineCache::Precompile() - PSO Verify failure - Did not attempt to precompile");
	}
#endif
...
bool UE::ShaderPipeline::CompileJob::PreloadShaders(TSet<FShaderHash>& OutRequiredShaders, bool &bOutCompatible)
{
	static FShaderHash EmptyHash;
	bool bOK = true;
	TArray<FShaderHash> ShadersToUnpreloadInCaseOfFailure;

	auto PreloadShader = [&bOK, &ShadersToUnpreloadInCaseOfFailure](const FShaderHash& ShaderHash, FShaderPipelineCacheArchive* ReadReqs)
	{
		if (bOK && ShaderHash != EmptyHash)
		{
			...
			UE_CLOGF(!bOK, LogRHI, Verbose, "Failed to read shader: %ls", *(ShaderHash.ToString()));
		}
	};
	...
	if (PSO.Type == FPipelineCacheFileFormatPSO::DescriptorType::Graphics)
	{
		...
		if (PSO.GraphicsDesc.MeshShader != EmptyHash)
		{
			OutRequiredShaders.Add(PSO.GraphicsDesc.MeshShader);
			bOK &= FShaderCodeLibrary::ContainsShaderCode(PSO.GraphicsDesc.MeshShader);
			UE_CLOGF(!bOK, LogRHI, Verbose, "Failed to find MeshShader shader: %ls", *(PSO.GraphicsDesc.MeshShader.ToString()));

			if (PSO.GraphicsDesc.AmplificationShader != EmptyHash)
			{
				...
				UE_CLOGF(!bOK, LogRHI, Verbose, "Failed to find AmplificationShader shader: %ls", *(PSO.GraphicsDesc.AmplificationShader.ToString()));
			}
		}
		else if (PSO.GraphicsDesc.VertexShader != EmptyHash)
		{
			...
			UE_CLOGF(!bOK, LogRHI, Verbose, "Failed to find VertexShader shader: %ls", *(PSO.GraphicsDesc.VertexShader.ToString()));

			if (PSO.GraphicsDesc.GeometryShader != EmptyHash)
			{
				...
				UE_CLOGF(!bOK, LogRHI, Verbose, "Failed to find GeometryShader shader: %ls", *(PSO.GraphicsDesc.GeometryShader.ToString()));
			}
		}
		else
		{
			...
			UE_LOGF(LogRHI, Error, "PSO Entry has no vertex shader!");
			bOK = false;
		}

		if (PSO.GraphicsDesc.FragmentShader != EmptyHash)
		{
			...
			UE_CLOGF(!bOK, LogRHI, Verbose, "Failed to find FragmentShader shader: %ls", *(PSO.GraphicsDesc.FragmentShader.ToString()));
		}
	...
	{
		if (PSO.ComputeDesc.ComputeShader != EmptyHash)
		{
		...
		{
			bOK = false;
			UE_LOGF(LogRHI, Error, "Invalid PSO entry in pipeline cache!");
		}
	}
	else if (PSO.Type == FPipelineCacheFileFormatPSO::DescriptorType::RayTracing)
	{
		...
		{
			if (PSO.RayTracingDesc.ShaderHash != EmptyHash)
			{
	...
	if (!bOK)
	{
		for (const FShaderHash& PreloadedShader : ShadersToUnpreloadInCaseOfFailure)
		{
...
void UE::ShaderPipeline::CompileJob::ReleasePreloadedShaders()
{
	if (bShadersPreloaded)
	{
		...
		auto ReleasePreloadedShader = [](const FShaderHash& ShaderHash)
		{
			if (ShaderHash != EmptyHash)
			{
...
void FShaderPipelineCacheTask::PreparePipelineBatch(TDoubleLinkedList<FPipelineCacheFileFormatPSORead*>& PipelineBatch)
{
	...
	while(CurrentNode)
	{
			...
			(PSORead->bReadCompleted || Archive->PollExternalReadDependencies()))
		{
            ...
            // Shaders required.
            TSet<FShaderHash> RequiredShaders;
			...
			else
			{
				if (RequiredShaders.Num())
				{
					...
					OrderedCompileTasks.Add(Hdr);
				}
				else if (bCompatible)
				{
					UE_LOGF(LogRHI, Error, "Invalid PSO entry in pipeline cache: %u!", PSORead->Hash);
				}
...
void FShaderPipelineCacheTask::PrecompilePipelineBatch(FRHICommandListImmediate& RHICmdList)
{
	...
	for(uint32 i = 0; i < (uint32)NumToPrecompile; i++)
	{
		...
		UE_LOGF(LogRHI, Verbose, "Precompiling PSO %u (type %d) (%d/%d)", PSOHash, int(CompileTask.PSO.Type), i+1, NumToPrecompile);
	...
	else
	{
		...
		switch (Mode)
		{
			case 0:
				...
				PausedCount = 1;
				bPaused = true;
				UE_LOGF(LogRHI, Display, "ShaderPipelineCache: Starting paused. %d", PausedCount);
				break;
			case 2:
...
void FShaderPipelineCache::BeginNextPrecompileCacheTaskInternal()
{
	...
	while(!PendingPrecompilePSOCacheKeys.IsEmpty())
	{
		...
		UE_LOGF(LogRHI, Log, "FShaderPipelineCache::BeginNextPrecompileCacheTask() - %ls begining compile.", *CurrentPrecompilingPSOFCKey);			
		...
		{
			UE_LOGF(LogRHI, Warning, "FShaderPipelineCache::BeginNextPrecompileCacheTask() - skipping %ls, not ready for precompile.", *CurrentPrecompilingPSOFCKey);
			CurrentPrecompilingPSOFCKey.Empty();
		}
	}
	...
	{
		// all jobs done.
		UE_LOGF(LogRHI, Log, "FShaderPipelineCache::BeginNextPrecompileCacheTask() - Finished, no jobs remaining.");
		bHasShaderPipelineTask = false;
	}
}
...
void FShaderPipelineCache::Tick(FRHICommandListImmediate& RHICmdList, float DeltaTime)
{
	...
	if(CurrentCacheTask)
	{
		CurrentCacheTask->Tick(RHICmdList, DeltaTime);
	}
}
...
{
	Tick(FRHICommandListImmediate::Get(), DeltaTime);
}

void FShaderPipelineCacheTask::Tick(FRHICommandListImmediate& RHICmdList, float DeltaTime)
{
	...
	if (PrecompileStartTime == 0.0 && (FetchTasks.Num() || OrderedCompileTasks.Num()))
	{
	...
	if (ReadyForPrecompile())
	{
		...
		PrecompilePipelineBatch(RHICmdList);
		...
		if (FShaderPipelineCache::BatchTime > 0.0f)
		{
			...
			else if (ElapsedMs > FShaderPipelineCache::BatchTime)
			{
				...
				{
					UE_LOGF(LogRHI, Warning, "FShaderPipelineCache: Cannot reduce BatchSize below 1 to meet target of %f ms, elapsed time was %f ms)", FShaderPipelineCache::BatchTime, ElapsedMs);
				}
			}
		}
	}
	...
	if (ReadyForNextBatch() && (OrderedCompileTasks.Num() || FetchTasks.Num()))
	{
		...
		if (FetchTasks.Num() < (int32)Num)
		{
			...
			Num -= FetchTasks.Num();
            for (int32 Index = OrderedCompileTasks.Num() - 1; Index >= 0 && Num > 0; --Index)
            {
                bool bHasShaders = true;
                for (FShaderHash const& Hash : OrderedCompileTasks[Index].Shaders)
                {
            	...
            	if (bHasShaders)
            	{
					...
					Entry->Hash = OrderedCompileTasks[Index].Hash;
            		...
            		OrderedCompileTasks.RemoveAtSwap(Index);
                    TotalActiveTasks++;
                    TotalWaitingTasks--;
                    --Num;
            	}
        ...
        {
            UE_LOGF(LogRHI, Warning, "FShaderPipelineCache: Attempting to pre-compile more jobs (%d) than the batch size (%d)", FetchTasks.Num(), FShaderPipelineCache::BatchSize);
        }
	...
	if(CVarPSOFileCacheGameFileMaskEnabled.GetValueOnAnyThread())
	{
		if(TotalActiveTasks + TotalWaitingTasks == 0)
		{
			...
			if(!bAlreadyInSet)
			{
				UE_LOGF(LogRHI, Display, "ShaderPipelineCache: GameUsageMask [%llu] precompile complete.", Mask);
			}
		}
	}
...
void FShaderPipelineCacheTask::BeginPrecompilingPipelineCache()
{
	...
	if(bReady)
	{
		...
		{
			bool bHasShaders = true;
			for (FShaderHash const& Hash : Task.Shaders)
			{
		...
		const int32 NumLocalPreFetchedTasks = LocalPreFetchedTasks.Num();
		OrderedCompileTasks.Reserve(NumLocalPreFetchedTasks);
		for (int32 Index = NumLocalPreFetchedTasks - 1; Index >= 0; --Index)
		{
			OrderedCompileTasks.Add(LocalPreFetchedTasks[Index]);
		}
	...
	{
		UE_LOGF(LogRHI, Display, "FShaderPipelineCache pipeline cache %ls - precompile deferred on UsageMask.", *PSOCacheKey);
	}
}
...
bool FShaderPipelineCacheTask::OnShaderLibraryStateChanged(FShaderPipelineCache::ELibraryState State, EShaderPlatform Platform, FString const& Name, int32 ComponentID)
{
	...
	if (ensure(!Name.IsEmpty()))
	{
		...
		UE_LOGF(LogRHI, Display, "Shader library state change %d, pipeline cache %ls was opened=%d", State, *Name, bDidOpen);
		return bDidOpen;
	}
