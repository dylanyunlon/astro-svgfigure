...
void FVisualizeTexture::DisplayPoolResourceListToLog(FVisualizeTexture::ESortBy SortBy)
{
	...
	for (int32 Index = 0; Index < SortedLines.Num(); Index++)
	{
		...
		UE_LOGF(LogConsoleResponse, Log, "   %3d = %ls", Entry.PoolIndex + 1, *Entry.Line);
	}

	UE_LOGF(LogConsoleResponse, Log, "");
	...
	UE_LOGF(LogConsoleResponse, Log, "Pool: %d/%d MB (referenced/allocated)", (UsedInKB + 1023) / 1024, (WholePoolInKB + 1023) / 1024);
...
void FVisualizeTexture::DisplayResourceListToLog(const TOptional<FWildcardString>& Wildcard)
{
	...
	UE_LOGF(LogConsoleResponse, Log, "RDGResourceName (what was rendered this frame, use <RDGResourceName>@<Version> to get intermediate versions):");
	...
	for (int32 RowId = 0; RowId < ColumnHeight; RowId++)
	{
		...
		UE_LOGF(LogConsoleResponse, Log, "   %ls", *Line);
	}
...
void FVisualizeTexture::DisplayViewListToLog()
{
	...
	UE_LOGF(LogConsoleResponse, Log, "Visualize Texture available views:");

	for (FSetElementId ElementId : Entries)
	{
		UE_LOGF(LogConsoleResponse, Log, "   %d  %ls", ViewDescriptionMap.Get(ElementId).Key, *ViewDescriptionMap.Get(ElementId).Value);
	}
}
	...
	if (EnumHasAnyFlags(Config.Flags, EFlags::SaveBitmap | EFlags::SaveBitmapAsStencil))
	{
			...
			[OutputTexture, Extent, ReadDataFlags, DebugName](FRHICommandListImmediate& RHICmdList)
		{
			...
			if (Bitmap.Num())
			{
				...
				UE_LOGF(LogRendererCore, Display, "Content was saved to \"%ls\"", *FPaths::ScreenShotDir());
			}
			else
			{
				UE_LOGF(LogRendererCore, Error, "Failed to save BMP for VisualizeTexture, format or texture type is not supported");
			}
		});
	}
}
...
void FVisualizeTexture::BeginViewRenderThread(ERHIFeatureLevel::Type InFeatureLevel, int32 UniqueId, const TCHAR* Description, bool bIsSceneCapture, bool bIsReflectionCapture)
{
	// Only support visualization for views with a unique ID, or reflection scene captures
	if (State == EState::Inactive || (!UniqueId && !(bIsSceneCapture && bIsReflectionCapture)))
	{
		return;
	}
	...
	if (!Requested.ViewName.IsEmpty() ? VisualizeTextureViewNameMatches(Requested.ViewName, Description) : (UniqueId && Requested.ViewUniqueId == UniqueId))
	{
...
bool FVisualizeTexture::IsNamedView(const TCHAR* Name) const
{
#if SUPPORTS_VISUALIZE_TEXTURE
	return VisualizeTextureViewNameMatches(Requested.ViewName, Name);
#else
	return false;
#endif
}
...
#if SUPPORTS_VISUALIZE_TEXTURE
{
	...
	if (bUseConfig)
	{
		// This function is called to visualize an external resource that isn't written by RDG, so we only want to add a visualization pass the first time
		// it's encountered, rather than each time, as we don't expect it to change.  So if it's already in VersionCountMap, return.  Also skip if this
		// isn't the active view.
		if (!GVisualizeTexture.IsRequestedView() || GVisualizeTexture.VersionCountMap.Find(InputTexture->Name))
		{
			return InputTexture;
		}

		// Add it to the map, so it shows up in the list of textures that can be visualized (even if it's not the requested texture).
		GVisualizeTexture.VersionCountMap.Add(InputTexture->Name) = 1;

		// If not the requested texture, just return.
		if (InputTexture->Name != GVisualizeTexture.Requested.Name)
		{
			return InputTexture;
		}
	}
	...
	return AddVisualizeTexturePass(GraphBuilder, ShaderMap, InputTexture, bUseConfig ? GVisualizeTexture.Config : VisualizeConfig, InputValueMapping, /* CaptureId = */ 0);
}
#else
{
	return InputTexture;
