...
std::atomic<bool> GRDGAllowRHIAccess{false};
std::atomic<bool> GRDGAllowRHIAccessAsync{false};
...
FAutoConsoleVariableRef CVarRDGParallelMobile(
	TEXT("r.RDG.ParallelMobile"), GRDGParallelMobile,
...
std::atomic_int32_t GRDGStatTransitionBatchCount{ 0 };
...
bool IsParallelExecuteEnabled(EShaderPlatform ShaderPlatform)
{
	return GRDGParallelExecute > 0
		...
		&& !GRDGDebugFlushGPU
		&& !GRDGTransitionLog
		&& (!IsMobilePlatform(ShaderPlatform) || IsParallelMobileEnabled())
...
FRDGScopeState::FState::FState(bool bInImmediate, bool bInParallelExecute)
	...
	, ScopeMode([]
	{
		...
		if (FRDGBuilder::IsDumpingFrame() || UE::RHI::GPUProfiler::IsProfiling())
		{
