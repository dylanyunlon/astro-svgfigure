...
class FRDGByteBufferShader : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<FUint16>, DstStructuredBuffer16x)
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<FUint12>, DstStructuredBuffer12x)
...
class FRDGMemcpyCS : public FRDGByteBufferShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FUint16>, SrcStructuredBuffer16x)
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FUint12>, SrcStructuredBuffer12x)
...
class FRDGScatterCopyCS : public FRDGByteBufferShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FUint16>, UploadStructuredBuffer16x)
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FUint12>, UploadStructuredBuffer12x)
...
EPixelFormat GetBufferPixelFormat(FRDGBuffer* Buffer)
{
	return EnumHasAnyFlags(Buffer->Desc.Usage, EBufferUsageFlags::ByteAddressBuffer | EBufferUsageFlags::StructuredBuffer) ? PF_Unknown : PF_A32B32G32R32F;
}

void MemsetResource(FRDGBuilder& GraphBuilder, FRDGBuffer* DstResource, const FMemsetResourceParams& Params)
{
	MemsetResource(GraphBuilder, GraphBuilder.CreateUAV(DstResource, GetBufferPixelFormat(DstResource), ERDGUnorderedAccessViewFlags::SkipBarrier), Params);
}
...
{
	MemcpyResource(GraphBuilder, GraphBuilder.CreateUAV(DstResource, GetBufferPixelFormat(DstResource), ERDGUnorderedAccessViewFlags::SkipBarrier), GraphBuilder.CreateSRV(SrcResource, GetBufferPixelFormat(SrcResource)), Params);
}

static EByteBufferStructuredSize GetStructuredBufferElementSize(FRDGBufferUAV * RDGBufferUAV)
{
	const int32 BytesPerElement = RDGBufferUAV->Desc.Buffer->Desc.BytesPerElement;
	switch (BytesPerElement)
	{
	case 4:  return EByteBufferStructuredSize::Uint1;
	case 8:  return EByteBufferStructuredSize::Uint2;
	case 16: return EByteBufferStructuredSize::Uint4;
	case 32: return EByteBufferStructuredSize::Uint8;
	case 48: return EByteBufferStructuredSize::Uint12;
	case 64: return EByteBufferStructuredSize::Uint16;
	default:
		checkf(false, TEXT("Unsupported structured buffer BytesPerElement size (%d) for buffer '%s' (supported sizes are 4, 8, 16, 32, 48, 64)."), BytesPerElement, RDGBufferUAV->Name);
		return EByteBufferStructuredSize::Uint4;
	}
}
...
void MemsetResource(FRDGBuilder& GraphBuilder, FRDGUnorderedAccessView* UAV, const FMemsetResourceParams& Params)
{
	...
	case EResourceType::STRUCTURED_BUFFER:
	{
		...
		case EByteBufferStructuredSize::Uint8:
			...
			break;
		case EByteBufferStructuredSize::Uint12:
			PassParameters->DstStructuredBuffer12x = RDGBufferUAV;
			break;
		case EByteBufferStructuredSize::Uint16:
			PassParameters->DstStructuredBuffer16x = RDGBufferUAV;
			break;
		default:
			break;
		};
	...
	TShaderMapRef<FRDGMemsetBufferCS> ComputeShader(GetGlobalShaderMap(GMaxRHIShaderPlatform), PermutationVector);
	...
	FComputeShaderUtils::AddPass(
		...
		ComputeShader,
		PassParameters,
		FComputeShaderUtils::GetGroupCountWrapped(FMath::DivideAndRoundUp(Params.Count, Divisor), 64));
}
...

	{
		...
		case EResourceType::STRUCTURED_BUFFER:
		{
			...
			case EByteBufferStructuredSize::Uint8:
				...
				break;
			case EByteBufferStructuredSize::Uint12:
				PassParameters->SrcStructuredBuffer12x = GetAs<FRDGBufferSRV>(SRV);
				PassParameters->Common.DstStructuredBuffer12x = RDGBufferUAV;
				break;
			case EByteBufferStructuredSize::Uint16:
				PassParameters->SrcStructuredBuffer16x = GetAs<FRDGBufferSRV>(SRV);
				PassParameters->Common.DstStructuredBuffer16x = RDGBufferUAV;
				break;
			default:
				break;
			};
		...
		TShaderMapRef<FRDGMemcpyCS> ComputeShader(GetGlobalShaderMap(GMaxRHIShaderPlatform), PermutationVector);
...
FRDGBuffer* ResizeBufferIfNeeded(FRDGBuilder& GraphBuilder, TRefCountPtr<FRDGPooledBuffer>& ExternalBuffer, const FRDGBufferDesc& BufferDesc, const TCHAR* Name)
{
	...
	{
		RDG_EVENT_SCOPE(GraphBuilder, "ResizeIfNeeded::MemcpyResource %d->%d", BufferSizeOld, BufferSizeNew);
	...
	FRDGScatterCopyCS::FPermutationDomain& OutPermutationVector)
{
	...
	else if (DstResourceType == EResourceType::STRUCTURED_BUFFER)
	{
		...
		case EByteBufferStructuredSize::Uint8:
			...
			break;
		case EByteBufferStructuredSize::Uint12:
			OutParameters.UploadStructuredBuffer12x = UploadBufferSRV;
			OutParameters.Common.DstStructuredBuffer12x = RDGBufferUAV;
			break;
		case EByteBufferStructuredSize::Uint16:
			OutParameters.UploadStructuredBuffer16x = UploadBufferSRV;
			OutParameters.Common.DstStructuredBuffer16x = RDGBufferUAV;
			break;
		default:
			break;
		};
	...
	else if (DstResourceType == EResourceType::BUFFER)
	{
		...
		OutParameters.UploadStructuredBuffer4x = UploadBufferSRV;
		OutParameters.ScatterStructuredBuffer = ScatterBufferSRV;
		OutParameters.Common.DstBuffer = GraphBuilder.CreateUAV(GetAsBuffer(DstResource), PF_A32B32G32R32F, ERDGUnorderedAccessViewFlags::SkipBarrier);
	}
...
void ScatterCopyResource(FRDGBuilder& GraphBuilder, FRDGViewableResource* DstResource, FRDGBufferSRV* ScatterBufferSRV, FRDGBufferSRV* UploadBufferSRV, const FScatterCopyParams &Params)
{
	...
	TShaderMapRef<FRDGScatterCopyCS> ComputeShader(GetGlobalShaderMap(GMaxRHIShaderPlatform), PermutationVector);
...
void FRDGScatterUploader::Lock(FRHICommandListBase& RHICmdList, uint32 ActualMaxScatters)
{
	check(State == EState::Empty);
	check(ActualMaxScatters <= MaxScatters);
	State = EState::Locked;

	ScatterBytes = ActualMaxScatters * sizeof(uint32);
	UploadBytes = ActualMaxScatters * NumBytesPerElement;
	MaxScatters = ActualMaxScatters;
...
void FRDGAsyncScatterUploadBuffer::End(FRDGBuilder& GraphBuilder, FRDGScatterUploader* Uploader)
{
	...
	// NumScatters is filled inside the dispatch group lambda.
	FRDGScatterCopyCS::FParameters Parameters;
	FRDGScatterCopyCS::FPermutationDomain PermutationVector;
	GetScatterCopyParamsAndPermutation(GraphBuilder, DstResource, ScatterBufferSRV, UploadBufferSRV, Config, /*NumScatters=*/ 0u, Parameters, PermutationVector);
...
UE::Tasks::FTask FRDGScatterUploadBuilder::Execute(FRDGBuilder& GraphBuilder)
{
	...
	if (!Passes.IsEmpty() || !CommandListFunctions.IsEmpty())
	{
		const bool bUseAsyncTask = MaxBytes > 32 * 1024 || !CommandListFunctions.IsEmpty();

		Task = GraphBuilder.AddCommandListSetupTask([Passes = Passes, CommandListFunctions = MoveTemp(CommandListFunctions)] (FRHICommandList& RHICmdList)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(FRDGScatterUploadBuilder);
			for (auto& Pass : Passes)
			{
				if (Pass.GetMaxScattersFunction)
				{
					Pass.Uploader->Lock(RHICmdList, Pass.GetMaxScattersFunction());
				}
				else
				{
			...
			for (auto& CommandListFunction : CommandListFunctions)
			{
				CommandListFunction(RHICmdList);
			}

		}, PrerequisiteTasks, UE::Tasks::ETaskPriority::Normal, bUseAsyncTask);
	}
	...
	Passes.Empty();
	CommandListFunctions.Empty();
	PrerequisiteTasks.Empty();
	return Task;
}
...
void MemsetResource(FRHICommandList& RHICmdList, const ResourceType& DstBuffer, const FMemsetResourceParams& Params)
{
	...
	auto ShaderMap = GetGlobalShaderMap(GMaxRHIShaderPlatform);

	auto ComputeShader = GetGlobalShaderMap(GMaxRHIShaderPlatform)->GetShader<FMemsetBufferCS>(PermutationVector);
...
void MemcpyResource(FRHICommandList& RHICmdList, const ResourceType& DstBuffer, const ResourceType& SrcBuffer, const FMemcpyResourceParams& Params, bool bAlreadyInUAVOverlap)
{
...

	{
		...
		auto ComputeShader = GetGlobalShaderMap(GMaxRHIShaderPlatform)->GetShader<FMemcpyCS >(PermutationVector);
...
template<typename ResourceType>
void FScatterUploadBuffer::ResourceUploadTo(FRHICommandList& RHICmdList, const ResourceType& DstBuffer)
{
	...
	auto ComputeShader = GetGlobalShaderMap(GMaxRHIShaderPlatform)->GetShader<FScatterCopyCS>(PermutationVector);
