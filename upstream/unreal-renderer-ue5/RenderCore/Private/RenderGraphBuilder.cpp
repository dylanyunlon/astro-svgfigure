	...
	PassParameters.EnumerateTextures([&](FRDGParameter Parameter)
	{
		...
		case UBMT_RDG_TEXTURE:
			if (FRDGTextureRef Texture = Parameter.GetAsTexture())
			{
				...
				if (GRHIGlobals.SupportsDedicatedSRVGraphicsNonPixelAccess)
				{
					EnumRemoveFlags(CurrentSRVAccess, ERHIAccess::SRVGraphicsNonPixel);
				}
				AccessFunction(nullptr, Texture, CurrentSRVAccess, NoneFlags, Texture->GetSubresourceRangeSRV());
			}
		break;
...
#endif
{
	...
	if (GRDGAllowRHIAccessAsync.load(std::memory_order_relaxed) != bParallelExecuteAllowedAsync)
	{
		WaitForAsyncExecuteTask();
		GRDGAllowRHIAccessAsync.store(bParallelExecuteAllowedAsync, std::memory_order_relaxed);
	}
#endif
}
...
FRHIUniformBuffer* FRDGBuilder::ConvertToExternalUniformBuffer(FRDGUniformBufferRef UniformBuffer)
{
	if (!UniformBuffer->bExternal)
	{
		UniformBuffer->GetParameters().Enumerate([this](const FRDGParameter& Param)
		{
			...
			case UBMT_RDG_UNIFORM_BLOCK:
			{
				if (FRDGBufferSRVRef SRV = Param.GetAsBufferSRV())
				{
					ConvertBuffer(this, SRV->GetParent());
				}
			}
			break;
	...
	if (!UniformBuffer->bExternal)
	{
		...
		if (!UniformBuffer->GetRHIUnchecked())
		{
			...
			IF_RDG_ENABLE_DEBUG(GRDGAllowRHIAccess.store(false, std::memory_order_relaxed));
		}
	}
...
void FRDGBuilder::Execute()
{
	...
	IF_RDG_ENABLE_DEBUG(GRDGAllowRHIAccess.store(ParallelExecute.IsEnabled(), std::memory_order_relaxed));
...
DECLARE_GPU_STAT(SubmitBufferUploads);
void FRDGBuilder::SubmitBufferUploads(FRHICommandList& RHICmdListUpload, UE::Tasks::FTaskEvent* AllocateUploadBuffersTask)
{
	...
	RHI_BREADCRUMB_EVENT_STAT(RHICmdListUpload, SubmitBufferUploads, "FRDGBuilder::SubmitBufferUploads");
...
void FRDGBuilder::SetupDispatchPassExecute()
{
	if (!DispatchPasses.IsEmpty())
	{
		...
		for (FRDGDispatchPass* DispatchPass : DispatchPasses)
		{
			...
			IF_RENDER_TRACING_ENABLED(RenderTracing::BeginExecuteDispatchPass(*DispatchPass));

			DispatchPass->LaunchDispatchPassTasks(DispatchPassBuilder);
			DispatchPassBuilder.Finish();

			for (const UE::Tasks::FTask& Task : DispatchPassBuilder.Prerequisites)
			{
				if (ParallelExecute.TasksAsync)
				{
					ParallelExecute.TasksAsync->AddPrerequisites(Task);
				}
				else if (ParallelExecute.TasksAwait)
				{
					ParallelExecute.TasksAwait->AddPrerequisites(Task);
				}
				else
...
{
	IF_RENDER_TRACING_ENABLED(RenderTracing::BeginExecuteRDGPass(*Pass, RHICmdListPass));
	...
	IF_RENDER_TRACING_ENABLED(RenderTracing::EndExecuteRDGPass(*Pass));
}
