...
{
	extern RENDERCORE_API const FName AnalysisArtifactsName = "ShaderStatTags.AnalysisArtifacts";
}
...
public:
	...
	void Initialize()
	{
		...
		if (!Modules.Num())
		{
			UE_LOGF(LogShaders, Error, "No target shader formats found!");
		}
	...
	{
		if (ShaderPlatform == EShaderPlatform::SP_InvalidPlatform)
		{
...
{
	return FShaderPlatformConfig::GetBindlessConfiguration(ShaderPlatform);
}

// deprecated
ERHIBindlessConfiguration UE::ShaderCompiler::GetBindlessResourcesConfiguration(FName ShaderFormat)
{
	return FShaderPlatformConfig::GetBindlessConfiguration(ShaderFormatToLegacyShaderPlatform(ShaderFormat));
}
...
bool UE::ShaderCompiler::ShouldCompileWithBindlessEnabled(EShaderPlatform ShaderPlatform, const FShaderCompilerInput& Input)
{
	...
	{
		check(FShaderPlatformConfig::GetBindlessConfiguration(ShaderPlatform) != ERHIBindlessConfiguration::Disabled);
		return true;
	}

	const ERHIBindlessConfiguration BindlessConfig = FShaderPlatformConfig::GetBindlessConfiguration(ShaderPlatform);
...
bool ShouldForceOptimizeRayTracingShaders(FName ShaderFormat)
{
	static const FShaderSymbolSettingHelper ForceOptimizeRT(TEXT("r.Shaders.ForceOptimizeRaytracing"));
	return ForceOptimizeRT.IsEnabled(ShaderFormat);
}
...
TOptional<FParameterAllocation> FShaderParameterMap::FindParameterAllocation(FStringView ParameterName) const
{
	if (const FParameterAllocation* Allocation = ParameterMap.FindByHash(GetTypeHash(ParameterName), ParameterName))
	{
		if (Allocation->bBound)
		{
			...
			//UE_LOGF(LogShaders, Warning, "Parameter %ls was bound multiple times. Code error?", ParameterName);
		}
...
void FShaderCompilerEnvironment::SerializeEverythingButFiles(FArchive& Ar)
{
	...
	Ar << SpecializationConstantValues;
...
void FShaderCompilerOutput::CompressOutput(FName ShaderCompressionFormat, FOodleDataCompression::ECompressor OodleCompressor, FOodleDataCompression::ECompressionLevel OodleLevel)
{
	...
	checkf(OutputHash != FShaderHash(), TEXT("Output hash must be generated before compressing the shader code."));
...
static void ReportVirtualShaderFilePathError(TArray<FShaderCompilerError>* CompileErrors, FString ErrorString)
{
	...
	{
		UE_LOGF(LogShaders, Error, "%ls", *ErrorString);
	}
}
...
public:
	...
	static bool CombineCodeOutputs(const IShaderFormat* Compiler, FShaderCompileJob& Job)
	{
		...
		// Combine symbol files from both outputs; we save them off before resetting the shader code struct below,
		// then re-set them on the new combined code prior to finalizing.
		TArray<FShaderSymbolFile> CombinedSymbolFiles(Job.Output.ShaderCode.GetSymbolFiles());
		CombinedSymbolFiles.Append(Job.SecondaryOutput->ShaderCode.GetSymbolFiles());
		...
		Job.Output.ShaderCode.SetSymbolFiles(MoveTemp(CombinedSymbolFiles));
		...
		{
			Job.Output.AddErrorf(TEXT("Mismatch in UsedAttributes between primary and secondary compilation; these must match since later stages may not have a secondary compile and so require both versions of the earlier stage to output the same set of attributes."));
			return false;
		}
		return true;
	}

	static void InvokeCompile(const IShaderFormat* Compiler, const FShaderCompilerInput& CompileInput, FShaderCompileJob& Job, FString& OutExceptionMsg, FString& OutExceptionCallstack)
	{
#if PLATFORM_WINDOWS
		__try
#endif
		{
			...
			if (Job.SecondaryOutput.IsValid())
			{
				...
				Compiler->CompilePreprocessedShader(CompileInput, Job.PreprocessOutput, *Job.SecondaryPreprocessOutput, Job.Output, *Job.SecondaryOutput);
			}
			else
			{
				Compiler->CompilePreprocessedShader(CompileInput, Job.PreprocessOutput, Job.Output);
			}
		}
#if PLATFORM_WINDOWS
	...
	static void FinalizeShaderCompileJob(FShaderCompilerOutput& Output, const FShaderCompilerInput& Input)
	{
		Output.Target = Input.Target;
		// also set the job input hash on the output struct for validation purposes
		Output.ValidateInputHash = Input.Hash;
		Output.ShaderCode.FinalizeShaderCode();
	}

	static void CompileShaderInternal(const IShaderFormat* Compiler, FShaderCompileJob& Job, FString& OutExceptionMsg, FString& OutExceptionCallstack, int32* CompileCount)
	{
		...
		// Pass a locally-merged input to the platform compiler instead of mutating Job.Input;
		// CompileShaderInternal can be re-entered (pipeline interpolator-removal retry, SCW fallback).
		const FShaderCompilerInput* CompileInput = &Job.Input;
		FShaderCompilerInput MergedInput;
		if (IsValidRef(Job.Input.SharedEnvironment))
		{
			MergedInput = Job.Input;
			MergedInput.Environment.Merge(*Job.Input.SharedEnvironment);
			CompileInput = &MergedInput;
		}

		InvokeCompile(Compiler, *CompileInput, Job, OutExceptionMsg, OutExceptionCallstack);
		if (Job.SecondaryOutput.IsValid())
		{
			...
			FinalizeShaderCompileJob(*Job.SecondaryOutput, Job.Input);
		...
		FinalizeShaderCompileJob(Job.Output, Job.Input);
...
static void CompileShader(const TArray<const IShaderFormat*>& ShaderFormats, FShaderCompilerInput& Input, FShaderCompilerOutput& Output, int32* CompileCount)
{
	FShaderCompileJob TempJob;
	TempJob.Input = Input;
	
	CompileShader(ShaderFormats, TempJob, CompileCount);
	Output = TempJob.Output;
}

void CompileShader(const TArray<const IShaderFormat*>& ShaderFormats, FShaderCompileJob& Job, int32* CompileCount)
{
	...
	if (!Compiler)
	{
		UE_LOGF(LogShaders, Fatal, "Can't compile shaders for format %ls, couldn't load compiler dll", *Job.Input.ShaderFormat.ToString());
	}

	FString ExceptionMsg;
	FString ExceptionCallstack;
	FInternalShaderCompilerFunctions::CompileShaderInternal(Compiler, Job, ExceptionMsg, ExceptionCallstack, CompileCount);
	if (!Job.Output.bSucceeded && (!ExceptionMsg.IsEmpty() || !ExceptionCallstack.IsEmpty()))
	{
		Job.Output.AddErrorf(
...
// Compiles all shader pipeline stage jobs and returns true if all succeeded.
static bool CompileShaderPipelineStages(const TArray<const IShaderFormat*>& ShaderFormats, FShaderPipelineCompileJob* PipelineJob, int32* CompileCount, bool* bOutInterpolatorRemovalFailed = nullptr)
{
	...
	for (int32 Index = 0; Index < PipelineJob->StageJobs.Num(); ++Index)
	{
		...
		if (bOutInterpolatorRemovalFailed && !bFirstJob && PreviousJob->Output.bSupportsQueryingUsedAttributes)
		{
		...
		CompileShader(ShaderFormats, *CurrentJob, CompileCount);
		...
		if (!CurrentJob->Output.bSucceeded)
		{
			...
			for (int32 SkipIndex = Index + 1; SkipIndex < PipelineJob->StageJobs.Num(); SkipIndex++)
			{
				...
				FInternalShaderCompilerFunctions::FinalizeShaderCompileJob(Job.Output, Job.Input);
				if (Job.SecondaryOutput.IsValid())
				{
					FInternalShaderCompilerFunctions::FinalizeShaderCompileJob(*Job.SecondaryOutput, Job.Input);
				}
			}
			return false;
		}
		else if (CurrentJob->Output.bInterpolatorRemovalFailed && bOutInterpolatorRemovalFailed)
		{
			*bOutInterpolatorRemovalFailed = true;
		}
	}
...
// Returns false if the specified pipeline job contains any shader stage other than Vertex or Pixel.
static bool HasOnlyVertexOrPixelShaderStages(const FShaderPipelineCompileJob& PipelineJob)
{
	for (const TRefCountPtr<FShaderCompileJob>& StageJob : PipelineJob.StageJobs)
	{
		const EShaderFrequency Stage = StageJob->Input.Target.GetFrequency();
...
// Resets the specified shader compiler output, but retains all errors so they can be reported as warnings.
static void ResetShaderCompilerOutputWithRetainedErrors(FShaderCompilerOutput& Output)
{
	TArray<FShaderCompilerError> RetainedErrors = MoveTemp(Output.Errors);
	bool bInterpolatorRemovalFailed = Output.bInterpolatorRemovalFailed;

	Output = FShaderCompilerOutput();

	Output.Errors = MoveTemp(RetainedErrors);
	Output.bInterpolatorRemovalFailed = bInterpolatorRemovalFailed;
}

void CompileShaderPipeline(const TArray<const IShaderFormat*>& ShaderFormats, FShaderPipelineCompileJob* PipelineJob, int32* CompileCount)
{
	...
	const bool bEnableRemovingUnused = HasOnlyVertexOrPixelShaderStages(*PipelineJob);
	if (bEnableRemovingUnused)
	{
		bool bInterpolatorRemovalFailed = false;
		if (!CompileShaderPipelineStages(ShaderFormats, PipelineJob, CompileCount, &bInterpolatorRemovalFailed))
		{
			return;
		}

		// If interpolator removal failed in any shader stage, compile pipeline again with interpolator removal disabled.
		// Otherwise, pipeline stages might end up with mismatching sets of interpolators.
		// If the pipeline consists of only a single stage (for DepthOnly pipelines for instance), there's no need to compile again, just take the successful result.
		if (bInterpolatorRemovalFailed && PipelineJob->StageJobs.Num() > 1)
		{
			// Reset compile states and strip interpolator removal option from each pipeline stage job
			PipelineJob->ForEachSingleShaderJob(
				[](FShaderCompileJob& StageJob)
				{
					// Reset information about used outputs
					StageJob.Input.bIncludeUsedOutputs = false;
					StageJob.Input.UsedOutputs.Empty();

					// Strip interpolator removal option from input, so our next compilation attempt avoids this optimization for the entire pipeline
					StageJob.Input.Environment.CompilerFlags.Remove(CFLAG_ForceRemoveUnusedInterpolators);

					// Reset job output before compiling again but retain errors so they can be reported as warnings
					ResetShaderCompilerOutputWithRetainedErrors(StageJob.Output);
				}
			);

			if (!CompileShaderPipelineStages(ShaderFormats, PipelineJob, CompileCount))
			{
				return;
			}
		}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AddShaderSourceFileEntry);
...
void GetAllVirtualShaderSourcePaths(TArray<FString>& OutVirtualFilePaths, EShaderPlatform ShaderPlatform, const FName* ShaderPlatformName)
{
	...
	AddShaderSourceFileEntry(OutVirtualFilePaths, FString("/Engine/Public/ShaderVersion.ush"), ShaderPlatform, ShaderPlatformName);
	AddShaderSourceFileEntry(OutVirtualFilePaths, FString("/Engine/Private/MaterialTemplate.ush"), ShaderPlatform, ShaderPlatformName);
	AddShaderSourceFileEntry(OutVirtualFilePaths, FString("/Engine/Private/Common.ush"), ShaderPlatform, ShaderPlatformName);
	AddShaderSourceFileEntry(OutVirtualFilePaths, FString("/Engine/Private/Definitions.usf"), ShaderPlatform, ShaderPlatformName);
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(VerifyShaderSourceFiles);
...
static void LogShaderSourceDirectoryMappings()
{
	...
	{
		UE_LOGF(LogShaders, Log, "Shader directory mapping %ls -> %ls", *Iter.Key, *Iter.Value);
	}
}
...
bool ReplaceVirtualFilePathForShaderAutogen(FString& InOutVirtualFilePath, EShaderPlatform ShaderPlatform, const FName* InShaderPlatformName)
{
	...
	{
		checkf(FDataDrivenShaderPlatformInfo::IsValid(ShaderPlatform) || InShaderPlatformName != nullptr,
			TEXT("ShaderPlatform %d (%s) is not valid in DataDrivenShaderPlatformInfo and no InShaderPlatformName was provided. Path='%s'"),
			(int32)ShaderPlatform, *LexToString(ShaderPlatform, false), *InOutVirtualFilePath);
...
void LoadShaderSourceFileChecked(const TCHAR* VirtualFilePath, EShaderPlatform ShaderPlatform, FString& OutFileContents, const FName* ShaderPlatformName)
{
	...
	{
		UE_LOGF(LogShaders, Fatal, "%ls", *FormatErrorCantFindSourceFile(VirtualFilePath));
	}
}
...
static void InternalGetShaderIncludes(const TCHAR* EntryPointVirtualFilePath, const TCHAR* VirtualFilePath, FStringView FileContents, TArray<FString>& IncludeVirtualFilePaths, EShaderPlatform ShaderPlatform, uint32 DepthLimit, bool AddToIncludeFile, const FName* ShaderPlatformName, FShaderPreprocessDependencies* OutDependencies)
{
	...
	if (FileContents.Len() > 0)
	{
		...
		{
			UE_LOGF(LogShaders, Warning, "GetShaderIncludes parsing terminated early to avoid infinite looping!\n Entrypoint \'%ls\' CurrentInclude \'%ls\' SearchCount %u Depth %u", 
	...
	EShaderPlatform ShaderPlatform, FString* OutErrorMessage)
{
	...
	{
		UE_LOGF(LogShaders, Verbose, "Generating hash of file %ls, ", VirtualFilePath);
	}
#endif
	for (int32 IncludeIndex = 0; IncludeIndex < IncludeVirtualFilePaths.Num(); IncludeIndex++)
	{
		...
		if (UE_LOG_ACTIVE(LogShaders, Verbose))
		{
			...
			UE_LOGF(LogShaders, Verbose, "Processing include file for %ls, %ls, %ls", VirtualFilePath, *IncludeVirtualFilePaths[IncludeIndex], *BytesToHex(IncrementalHash.Hash, 20));
		}
#endif
	}
...
const FSHAHash& GetShaderFileHash(const TCHAR* VirtualFilePath, EShaderPlatform ShaderPlatform)
{
	...
	if (!Hash)
	{
		UE_LOGF(LogShaders, Fatal, "%ls", *ErrorMessage);
...
#if WITH_EDITOR &&  !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
		UE_LOGF(LogShaders, Verbose, "Final hash for file %ls, %ls", VirtualFilePath,*BytesToHex(&NewHash.Hash[0], 20));
#endif
...
namespace PlatformInvariantShaderFileHash
{
	// Shader source files are treated as immutable after startup (matches GShaderHashCache), so
	// no invalidation path is provided.
	static FRWLock                 GCacheLock;
	static TMap<FString, FSHAHash> GCache;
} // namespace PlatformInvariantShaderFileHash

FSHAHash GetShaderFileHashPlatformInvariant(const TCHAR* VirtualFilePath)
{
	using namespace PlatformInvariantShaderFileHash;
	...
	// Walk the include closure with SP_InvalidPlatform so platform-aware helpers short-circuit.
	TArray<FString> AllFiles;
	AllFiles.Add(VirtualFilePath);

	FShaderSharedStringPtr EntryContents;
	if (LoadCachedShaderSourceFile(VirtualFilePath, EShaderPlatform::SP_InvalidPlatform, &EntryContents, nullptr)
		&& EntryContents.IsValid())
	{
		InternalGetShaderIncludes(VirtualFilePath, VirtualFilePath, *EntryContents, AllFiles,
			EShaderPlatform::SP_InvalidPlatform, /*DepthLimit=*/100, /*AddToIncludeFile=*/false,
			/*ShaderPlatformName=*/nullptr, /*OutDependencies=*/nullptr);
	}
	...
	{
		FShaderSharedStringPtr FileContents;
		if (LoadCachedShaderSourceFile(*FilePath, EShaderPlatform::SP_InvalidPlatform, &FileContents, nullptr)
...
void PrimeShaderFileHashCache(TArrayView<const FString> VirtualFilePaths, EShaderPlatform ShaderPlatform)
{
	STAT(double HashTime = 0);
	{
		SCOPE_SECONDS_COUNTER(HashTime);

		// Single read-lock pass: collect all paths not yet present in the cache.
		TArray<const FString*> UncachedPaths;
		UncachedPaths.Reserve(VirtualFilePaths.Num());
		{
			FRWScopeLock ShaderHashAccessLock(GShaderHashAccessRWLock, SLT_ReadOnly);
			for (const FString& VirtualFilePath : VirtualFilePaths)
			{
				if (!GShaderHashCache.FindHash(ShaderPlatform, VirtualFilePath))
				{
					UncachedPaths.Add(&VirtualFilePath);
				}
			}
		}
		...
		// Hold the calculation guard for the entire batch so no other thread redundantly
		// computes any of the same hashes while we are working through the list.
		FScopeLock FileHashCalculationAccessLock(&GShaderFileHashCalculationGuard);

		// Pre-allocate one slot per path so parallel tasks write to disjoint indices with no contention.
		// FilePath == nullptr serves as the sentinel for "skipped or failed".
		struct FComputedHash
		{
			const FString* FilePath = nullptr;
			FSHAHash Hash;
		};
		TArray<FComputedHash> ComputedHashes;
		ComputedHashes.SetNum(UncachedPaths.Num());

		// Compute hashes in parallel; each task owns its slot in ComputedHashes.
		// FindHash reads below are safe without RWLock: GShaderFileHashCalculationGuard
		// ensures no other thread writes new cache entries during this section.
		ParallelFor(UncachedPaths.Num(), [&](int32 Index)
		{
			const FString* VirtualFilePath = UncachedPaths[Index];

			// Double-check: a thread that held the guard before us may have already populated this entry.
			if (GShaderHashCache.FindHash(ShaderPlatform, *VirtualFilePath))
			{
				return;
			}
			...
			{
				HashState.Final();
				ComputedHashes[Index].FilePath = VirtualFilePath;
				HashState.GetHash(&ComputedHashes[Index].Hash.Hash[0]);
			}
			else
			{
				UE_LOGF(LogShaders, Warning, "PrimeShaderFileHashCache: failed to hash '%ls': %ls", **VirtualFilePath, *ErrorMessage);
			}
		});

		// Single write-lock pass to insert all newly computed hashes at once.
		FRWScopeLock ShaderHashAccessLock(GShaderHashAccessRWLock, SLT_Write);
		for (const FComputedHash& Entry : ComputedHashes)
		{
			if (Entry.FilePath)
			{
				FSHAHash& NewHash = GShaderHashCache.AddHash(ShaderPlatform, *Entry.FilePath);
				NewHash = Entry.Hash;
#if WITH_EDITOR && !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
				UE_LOGF(LogShaders, Verbose, "Final hash for file %ls, %ls", *(*Entry.FilePath), *BytesToHex(&NewHash.Hash[0], 20));
#endif
			}
		}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(InitializeShaderHashCache);
	FRWScopeLock ShaderHashAccessLock(GShaderHashAccessRWLock, SLT_Write);
...
{
	UE_LOGF(LogShaders, Log, "InitializeShaderTypes() begin");
	...
	UE_LOGF(LogShaders, Log, "InitializeShaderTypes() end");
}
...
{
	UE_LOGF(LogShaders, Log, "FlushShaderFileCache() begin");
	...
	UE_LOGF(LogShaders, Log, "FlushShaderFileCache() end");
}
	...
	bool bIncludeSourceHashes)
{
	...
	for (const FShaderTypeDependency& ShaderTypeDependency : ShaderTypeDependencies)
	{
		...
		if (ShaderType == nullptr)
		{
			...
			UE_LOGF(
				LogShaders, Display, "Failed to find FShaderType for dependency %s (total in the NameToTypeMap: %d)",
	...
	for (const FShaderPipelineTypeDependency& Dependency : ShaderPipelineTypeDependencies)
	{
		...
		if (ShaderPipelineType == nullptr)
		{
			...
			// This will generate a new key and invalidate the asset, causing the asset to be re-cooked.
			UE_LOGF(
				LogShaders, Display, "Failed to find FShaderPipelineType for dependency %s (total in the NameToTypeMap: %d)",
...
FString FShaderCompilerInput::GetOrCreateShaderDebugInfoPath() const
{
	...
	if (!IFileManager::Get().DirectoryExists(*OutDumpDebugInfoPath))
	{
		if (!IFileManager::Get().MakeDirectory(*OutDumpDebugInfoPath, true))
		{
			...
			UE_LOGF(LogShaders, Warning, "Last Error %u: Failed to create directory for shader debug info '%ls'. Try enabling large file paths or r.DumpShaderDebugShortNames.", ErrorCode, *OutDumpDebugInfoPath);
			return FString();
		}
	}
...
void AddShaderSourceDirectoryMapping(const FString& VirtualShaderDirectory, const FString& RealShaderDirectory)
{
	...
	checkf(
		...
		!VirtualShaderDirectory.Contains(FString(".")),
	...
	if (!bDirectoryExists)
	{
		UE_LOGF(LogShaders, Log, "Directory %ls", *RealShaderDirectory);
	}
	...
	UE_LOGF(LogShaders, Log, "Mapping virtual shader directory %ls to %ls",
...
void FShaderCode::Compress(FName ShaderCompressionFormat, FOodleDataCompression::ECompressor InOodleCompressor, FOodleDataCompression::ECompressionLevel InOodleLevel)
{
	...
	if (bCompressed && CompressedSize < Code.Num())
	{
		...
		checkf(ShaderCodeSize >= 0, TEXT("Unable to determine ShaderCodeSize from uncompressed code"));
...
const FShaderCodeResource& FShaderCode::GetFinalizedResource(EShaderFrequency Frequency, FShaderHash OutputHash) const
{
	...
	// Log the actual values on failure to distinguish a logic bug (plausible OptionalDataSize/Frequency)
	// from memory corruption (e.g. 0xcdcdcdcd / out-of-range Frequency).
	checkf(OptionalDataSize == -1,
		TEXT("FShaderCode not finalized when GetFinalizedResource was called. OptionalDataSize=%d (0x%08x), Frequency=%s, OutputHash=%s, HeaderPopulated=%d, CodeSize=%llu, UncompressedSize=%d"),
		OptionalDataSize, (uint32)OptionalDataSize,
		GetShaderFrequencyString(Frequency), *OutputHash.ToString(),
		ShaderCodeResource.Header ? 1 : 0,
		ShaderCodeResource.Code.GetSize(),
		UncompressedSize);
	...
	{
		// sanity check
		const EShaderFrequency HeaderFrequency = ShaderCodeResource.GetFrequency();
		checkf(HeaderFrequency == Frequency,
			TEXT("FShaderCodeResource frequency mismatch. Header.Frequency=%s (%d), expected Frequency=%s (%d), OutputHash=%s, Header.UncompressedSize=%d, Header.ShaderCodeSize=%d, CodeSize=%llu, OptionalDataSize=%d"),
			GetShaderFrequencyString(HeaderFrequency), (int32)HeaderFrequency,
			GetShaderFrequencyString(Frequency), (int32)Frequency,
			*OutputHash.ToString(),
			ShaderCodeResource.GetUncompressedSize(),
			ShaderCodeResource.GetShaderCodeSize(),
			ShaderCodeResource.Code.GetSize(),
			OptionalDataSize);
	...
	if (ShaderCompressionFormat != NAME_None)
	{
		...
		if (ShaderCompressionFormat != GetCompressionFormat())
		{
			...
			{
				UE_LOGF(LogShaders, Fatal, "Shader %ls is expected to be compressed with %ls, but it is compressed with %ls instead.",
					*OutputHash.ToString(),
			...
			UE_LOGF(LogShaders, Verbose, "Shader %ls is expected to be compressed with %ls, but it arrived uncompressed (size=%d). Assuming compressing made it longer and storing uncompressed.",
				*OutputHash.ToString(),
				*ShaderCompressionFormat.ToString(),
		...
		else if (ShaderCompressionFormat == NAME_Oodle)
		{
			...
			{
				UE_LOGF(LogShaders, Fatal, "Shader %ls is expected to be compressed with Oodle compressor %d level %d, but it is compressed with compressor %d level %d instead.",
					*OutputHash.ToString(),
...
{
	// SymbolFiles intentionally not serialized here: this operator only runs on the non-cache (cooked) path
	// via FShaderMapResourceCode::Serialize. Symbol files travel exclusively through the cache context's
	// SerializeCode / ReadFromRecord path and must never appear in cooked data.
	Ar << Resource.Header;
	Ar << Resource.Code;
	return Ar;
}
...
FArchive& operator<<(FArchive& Ar, FShaderCode& Output)
{
	...
	Ar << Output.ShaderCodeResource.SymbolFiles;
...
FArchive& operator<<(FArchive& Ar, FShaderCompilerInput& Input)
{
	...
	// VSDisallowedUniformBufferNames and NonVSDisallowedUniformBufferNames are not used in FShaderCompileJob::GetInputHash because these do not influence
	// the result of the shader compilation and are only there to perform validation wrt what material uniform buffer is used
	Ar << Input.VSDisallowedUniformBufferNames;
	Ar << Input.NonVSDisallowedUniformBufferNames;
...
const TCHAR* LexToString(EShaderCompileJobStatus Status)
{
	...
	case EShaderCompileJobStatus::Pending: return TEXT("Pending");
	case EShaderCompileJobStatus::Completed: return TEXT("Completed");
	case EShaderCompileJobStatus::Released: return TEXT("Released");
	default: return TEXT("(unknown)");
	}
}

const TCHAR* LexToString(EShaderCompileJobCompletionMethod Method)
{
	switch (Method)
	{
	case EShaderCompileJobCompletionMethod::Unset: return TEXT("Unset");
	case EShaderCompileJobCompletionMethod::CacheHit: return TEXT("CacheHit");
	case EShaderCompileJobCompletionMethod::DdcHit: return TEXT("DdcHit");
	case EShaderCompileJobCompletionMethod::Duplicate: return TEXT("Duplicate");
	case EShaderCompileJobCompletionMethod::LocalExecution: return TEXT("LocalExecution");
	case EShaderCompileJobCompletionMethod::DistributedExecution: return TEXT("DistributedExecution");
	default: return TEXT("(unknown)");
	}
}
...
void FShaderCompileJob::SerializeOutput(FShaderCacheSerializeContext& Ctx, int32 CodeIndex)
{
	...
	// Snapshot for the inconsistency check below: if Output.Target.Frequency drifts between the
	// Ar << Output write (which serializes Output.Target into ShaderObjectData) and the
	// GetFinalizedCodeResource call (which stamps the Header from current Output.Target.Frequency),
	// the cache record ends up internally inconsistent and trips the GetFrequency() == Frequency
	// assert on a future load. Hunt for the producer.
	const EShaderFrequency PreSaveFrequency = bIsSaving ? Output.Target.GetFrequency() : SF_NumFrequencies;
	...
	if (bIsSaving)
	{
		const EShaderFrequency HeaderFrequency = CodeResource.GetFrequency();
		const EShaderFrequency CurrentFrequency = Output.Target.GetFrequency();
		const EShaderFrequency InputFrequency = Input.Target.GetFrequency();
		checkf(PreSaveFrequency == HeaderFrequency && PreSaveFrequency == CurrentFrequency && PreSaveFrequency == InputFrequency,
			TEXT("Inconsistent shader code cache record being written. PreSave=%s Header=%s Current=%s Input=%s, InputHash=%s, OutputHash=%s"),
			GetShaderFrequencyString(PreSaveFrequency), GetShaderFrequencyString(HeaderFrequency),
			GetShaderFrequencyString(CurrentFrequency), GetShaderFrequencyString(InputFrequency),
			*LexToString(InputHash), *Output.OutputHash.ToString());
	}
	...
	checkf(!Output.bSucceeded || Output.OutputHash != FShaderHash(), TEXT("Successful compile job does not have an OutputHash generated."));
...
void FShaderCompileJob::OnComplete(FShaderDebugDataContext& Ctx)
{
	...
	if (Input.DumpDebugInfoEnabled() || bHasSecondary)
	{
...
#if WITH_EDITOR
	TConstArrayView<FShaderSymbolFile> SymbolFiles = Output.bSucceeded ? Output.GetFinalizedCodeResource().GetSymbolFiles() : TConstArrayView<FShaderSymbolFile>();
	if (SymbolFiles.Num() > 0)
	{
		FShaderSymbolExport::GetOrCreate(Input.ShaderFormat).ExportSymbols(SymbolFiles, Input.GenerateDebugInfo());
	}
#endif // WITH_EDITOR
}
...
void FShaderCompileJob::SerializeWorkerOutputInner(FArchive& Ar, bool bSerializeForArtifact)
{
	...
	if (!bSerializeForArtifact)
	{
		if (!ensureMsgf(Output.ValidateInputHash == Input.Hash, TEXT("Output.ValidateInputHash does not match Input.Hash; incorrect results associated with job?")))
		{
			...
			UE_LOGF(LogShaders, Fatal, "Cannot continue serializing shader compile jobs after input hash mismatch in primary output");
		}
	}
	...
	if (bSecondaryOutput)
	{
		...
		if (!bSerializeForArtifact)
		{
			if (!ensureMsgf(SecondaryOutput->ValidateInputHash == Input.Hash, TEXT("SecondaryOutput.ValidateInputHash does not match Input.Hash; incorrect results associated with job?")))
			{
				...
				UE_LOGF(LogShaders, Fatal, "Cannot continue serializing shader compile jobs after input hash mismatch in secondary output");
			}
		}
		// empty validation hash after running validation to avoid impacting output deduplication
	...
	if (!bSerializeForArtifact)
	{
		if (!ensureMsgf(Output.Target == Input.Target, TEXT("Output FShaderTarget does not match the input struct; incorrect results associated with job?")))
		{
			...
			UE_LOGF(LogShaders, Fatal, "Cannot continue serializing shader compile jobs after target mismatch in primary output");
		}
		if (bSecondaryOutput)
		{
			if (!ensureMsgf(SecondaryOutput->Target == Input.Target, TEXT("SecondaryOutput FShaderTarget does not match the input struct; incorrect results associated with job?")))
			{
				...
				UE_LOGF(LogShaders, Fatal, "Cannot continue serializing shader compile jobs after target mismatch in secondary output");
			}
		}
	}
}
...
void FShaderCompileJob::SerializeWorkerInputNoSource(FArchive& Ar)
{
	...
	// we still want to serialize the decompressed size so when using source file virtualization via UBA we can just pass the source binary blob
	// as-is as a virtual input file.
	PreprocessOutput.PreprocessedSource.DecompressedCharCount = SourceTemp.DecompressedCharCount;
	if (bHasSecondary)
	{
		SecondaryPreprocessOutput->PreprocessedSource.DecompressedCharCount = SecondarySourceTemp.DecompressedCharCount;
	}
	...
	Key(InKey)
{
	...
	{
		const FShaderCompileJobKey StageKey(ShaderType, InKey.Platform, InKey.VFType, InKey.PermutationId);
...
void FShaderPipelineCompileJob::OnComplete(FShaderDebugDataContext& Ctx)
{
	...
	if (bShouldDump)
	{
		...
		else
		{
			...
			UE_LOGF(LogShaders, Warning, "Cannot dump debug compile input for pipeline job; the following stage job(s) failed to dump source files: \n%ls", *MissingStagesStr);
		}
	}
}
