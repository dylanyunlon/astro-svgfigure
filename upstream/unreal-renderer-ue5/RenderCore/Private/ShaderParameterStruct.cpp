		...
		uint32 GeneralByteOffset)
	{
		...
		for (const FShaderParametersMetadata::FMember& Member : StructMembers)
		{
			...
			if (BaseType == UBMT_RENDER_TARGET_BINDING_SLOTS)
			{
				...
				{
					UE_LOGF(LogShaders, Fatal, "Render target binding slots collision: %ls & %ls",
			...
			const bool bIsRHIResource = (
				...
				BaseType == UBMT_DESCRIPTOR_RANGE ||
			...
			for (uint32 ArrayElementId = 0; ArrayElementId < (bIsResourceArray ? ArraySize : 1u); ArrayElementId++)
			{
				...
				{
					UE_LOGF(LogShaders, Fatal, "%ls can't bind shader parameter %ls, because it has already be bound by %ls.", *CppName, *ElementShaderBindingName, **ShaderGlobalScopeBindings.Find(ShaderBindingName));
				}
				...
				if (bIsVariableNativeType)
				{
					...
					{
						UE_LOGF(LogShaders, Fatal, "The size required to bind shader %ls's (Permutation Id %d) struct %ls parameter %ls is %i bytes, smaller than %ls's %i bytes.",
				...
				{
					if (IsParameterBindless(ParameterAllocation->Type) || ParameterAllocation->Type == EShaderParameterType::DescriptorRange)
					{
					...
					else
					{
						...
						{
							UE_LOGF(LogShaders, Fatal,
								"Error with shader %ls's (Permutation Id %d) parameter %ls is %i bytes, cpp name = %ls." "The shader compiler should give precisely which elements of an array did not get compiled out, " "for optimal automatic render graph pass dependency with ClearUnusedGraphResources().",
...
void FShaderParameterBindings::BindForRootShaderParameters(const FShader* Shader, int32 PermutationId, const FShaderParameterMap& ParametersMap)
{
	...
	if (BindingContext.ShaderGlobalScopeBindings.Num() != AllParameterNames.Num())
	{
		...
		UE_LOGF(LogShaders, Fatal, "%ls", *ErrorString);
	}
}
...
void EmitNullShaderParameterFatalError(const TShaderRef<FShader>& Shader, const FShaderParametersMetadata* ParametersMetadata, uint16 MemberOffset)
{
	...
	UE_LOGF(LogShaders, Fatal,
		"%ls's required shader parameter %ls::%ls was not set.",
		ShaderClassName,
		ParametersMetadata->GetStructTypeName(),
		*MemberName);
}

void EmitIncompatibleShaderParameterFatalError(const TShaderRef<FShader>& Shader, const FShaderParametersMetadata* ParametersMetadata, uint16 MemberOffset)
{
	const FString MemberName = ParametersMetadata->GetFullMemberCodeName(MemberOffset);

	const TCHAR* ShaderClassName = Shader.GetType()->GetName();

	UE_LOG(LogShaders, Fatal,
		TEXT("%s's required shader parameter %s::%s is not compatible with shader stage."),
		ShaderClassName,
		ParametersMetadata->GetStructTypeName(),
		*MemberName);
}
...
void ValidateShaderParameters(const TShaderRef<FShader>& Shader, const FShaderParametersMetadata* ParametersMetadata, const void* ParametersData, const FResourceExclusionArrayView ExclusionList)
{
	...
	for (const FShaderParameterBindings::FResourceParameter& Parameter : Bindings.ResourceParameters)
	{
		...
		switch (BaseType)
		{
			...
			case UBMT_RESOURCE_COLLECTION:
			{
				...
				if (!Resource && !ExclusionList.Contains(Parameter.BaseIndex))
				{
			...
			case UBMT_RDG_TEXTURE:
			{
				...
				if (!GraphTexture && !ExclusionList.Contains(Parameter.BaseIndex))
				{
					EmitNullShaderParameterFatalError(Shader, ParametersMetadata, Parameter.ByteOffset);
				}
				break;
			...
			case UBMT_RDG_TEXTURE_SRV:
			{
				...
				if (GRHIGlobals.SupportsDedicatedSRVGraphicsNonPixelAccess && IsValidPreRasterizationFrequency(Shader->GetFrequency()))
				{
					EmitIncompatibleShaderParameterFatalError(Shader, ParametersMetadata, Parameter.ByteOffset);
				}

				if (!GraphResource && !ExclusionList.Contains(Parameter.BaseIndex))
				{
					EmitNullShaderParameterFatalError(Shader, ParametersMetadata, Parameter.ByteOffset);
				}
				break;
...
void ValidateShaderParameterResourcesRHI(const void* Contents, const FRHIUniformBufferLayout& Layout)
{
	for (int32 Index = 0, Count = Layout.Resources.Num(); Index < Count; ++Index)
	{
		...
		const bool bSRVorUAV =
			Parameter.MemberType == UBMT_SRV ||
			Parameter.MemberType == UBMT_UAV ||
			Parameter.MemberType == UBMT_RDG_TEXTURE_SRV ||
			Parameter.MemberType == UBMT_RDG_TEXTURE_UAV ||
			Parameter.MemberType == UBMT_RDG_TEXTURE_NON_PIXEL_SRV ||
			Parameter.MemberType == UBMT_RDG_BUFFER_SRV ||
			Parameter.MemberType == UBMT_RDG_BUFFER_UAV ||
			Parameter.MemberType == UBMT_RDG_UNIFORM_BLOCK ||
			Parameter.MemberType == UBMT_RESOURCE_COLLECTION;

		// Allow null SRV's in uniform buffers for feature levels that don't support SRV's in shaders
		// Mobile does not set some UAVs to not break RDG pass auto-merging
		if (GMaxRHIFeatureLevel <= ERHIFeatureLevel::ES3_1 && bSRVorUAV)
		{
			continue;
		}
...
FResourceExclusionArray GetSpecializationResourceExclusions(const TShaderRef<FShader>& Shader)
{
#if PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS
	FRHIShader* RHIShader = Shader.GetRHIShaderBase(Shader->GetFrequency());
	if (RHIShader->HasSpecializationConstants())
	{
		FResourceExclusionArray ResourceExclusions;

		const uint32 SpecializationHash = GetTypeHash(Shader.GetSpecializationConstants());
		const FShaderParameterBindings& Bindings = Shader->Bindings;
		// Only one of the two following loops will fill ResourceExclusions (depending on if bindless is enabled)
		for (const FShaderParameterBindings::FResourceParameter& Parameter : Bindings.ResourceParameters)
		{
			// Check if this resource should be excluded from this combination of specialization constants
			if (RHIShader->IsExcludedFromSpecialization(Parameter.BaseIndex, SpecializationHash))
			{
				ResourceExclusions.Add(Parameter.BaseIndex);
			}
		}
		for (const FShaderParameterBindings::FBindlessResourceParameter& Parameter : Bindings.BindlessResourceParameters)
		{
			// Check if this resource should be excluded from this combination of specialization constants
			if (RHIShader->IsExcludedFromSpecialization(Parameter.GlobalConstantOffset, SpecializationHash))
			{
				ResourceExclusions.Add(Parameter.GlobalConstantOffset);
			}
		}
	...
	return FResourceExclusionArray();
}
...
FRHIShaderParameterResource ExtractShaderParameterResource(FShaderParameterReader Reader, const BindingParameterType& Parameter)
{
	...
	case UBMT_DESCRIPTOR_RANGE:
	{
		FRHIDescriptorRange* DescriptorRange = Reader.Read<FRHIDescriptorRange*>(Parameter);
		checkSlow(DescriptorRange);
		return FRHIShaderParameterResource(DescriptorRange, GetParameterIndex(Parameter));
	}
	case UBMT_RESOURCE_COLLECTION:
	{
		FRHIResourceCollection* Collection = Reader.Read<FRHIResourceCollection*>(Parameter);
		checkSlow(Collection);
		return FRHIShaderParameterResource(Collection, GetParameterIndex(Parameter));
	}
	case UBMT_RDG_TEXTURE:
	{
...
static void ExtractShaderParameterResources(
	...
	TConstArrayView<uint8> ParametersData,
	EUniformBufferBindingFlags UniformBufferBindingFlags,
	const FResourceExclusionArrayView ResourceExclusions)
{
	const FShaderParameterReader Reader(ParametersData);

#if PLATFORM_SUPPORTS_BINDLESS_RENDERING
	if (Bindings.BindlessResourceParameters.Num())
	{
		for (const FShaderParameterBindings::FBindlessResourceParameter& Parameter : Bindings.BindlessResourceParameters)
		{
			if (ResourceExclusions.Contains(Parameter.GlobalConstantOffset))
			{
				continue;
			}
	...
	if (NumBindings)
	{
		for (const FShaderParameterBindings::FResourceParameter& Parameter : Bindings.ResourceParameters)
		{
			if (ResourceExclusions.Contains(Parameter.BaseIndex))
			{
				continue;
			}
	...
	const FResourceExclusionArrayView ResourceExclusions)
{
	...
	ExtractShaderParameterResources(BatchedParameters, Bindings, FullParametersData, EUniformBufferBindingFlags::Shader, ResourceExclusions);
}
	...
	const FResourceExclusionArrayView ResourceExclusions)
{
	...
	SetShaderParameters(ShaderParameters, Bindings, ParametersMetadata, InParametersData, ResourceExclusions);
...
void SetShaderParameters(
	...
	const FResourceExclusionArrayView ResourceExclusions)
{
	SetShaderParametersInternal(RHICmdList, ShaderRHI, Bindings, ParametersMetadata, ParametersData, ResourceExclusions);
}
	...
	const void* InParametersData)
{
	...
	const FResourceExclusionArrayView NoExclusions;  // Specialization constants not supported in ray tracing yet
	ExtractShaderParameterResources(BatchedParameters, Bindings, FullParametersData, EUniformBufferBindingFlags::StaticAndShader, NoExclusions);
