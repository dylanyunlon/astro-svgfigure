...
// This is a per-project main switch for Virtual Shadow Maps support
// Disabling this can remove some shader permutations if virtual shadow maps are not needed
int32 GVirtualShadowMapsProjectEnabled = 1;
FAutoConsoleVariableRef CVarAllowVirtualShadowMaps(
	TEXT("r.Shadow.Virtual.ProjectEnabled"),
	GVirtualShadowMapsProjectEnabled,
	TEXT("This setting allows you to disable Virtual Shadow Maps on platforms that support it to reduce the number of shaders.\n"),
	ECVF_ReadOnly | ECVF_RenderThreadSafe
);

// This is a per-project main switch for Non-Nanite support in Virtual Shadow Maps.
// Disabling this can remove shader permutations and slightly improve performance, but Non-Nanite objects
// will not cast shadows when Virtual Shadow Maps are enabled.
int32 GVirtualShadowMapNonNaniteProjectEnabled = 1;
FAutoConsoleVariableRef CVarVirtualShadowMapNonNaniteProjectEnabled(
	TEXT("r.Shadow.Virtual.NonNanite.ProjectEnabled"),
	GVirtualShadowMapNonNaniteProjectEnabled,
	TEXT("This can be used to disable non-nanite rendering into Virtual Shadow Maps and related shader permutations.\n")
	TEXT("Only use this if you do not need any non-nanite geometry to cast shadows in VSMs."),
	ECVF_RenderThreadSafe | ECVF_ReadOnly
);

// This is a per-project main switch for supporting the classic shadow map techniques in Unreal for Non-Nanite geometry
// This includes cascaded shadow maps, (classic, non-VSM) spot light shadow maps and (classic, non-VSM) point light shadows
// Disabling this can remove shader permutations if these techniques are not needed
int32 GClassicShadowMapsProjectEnabled = 1;
FAutoConsoleVariableRef CVarClassicShadowMapsProjectEnabled(
	TEXT("r.Shadow.Classic.ProjectEnabled"),
	GClassicShadowMapsProjectEnabled,
	TEXT("This can be used to disable classic shadow maps (cascaded shadow maps, classic spot/point/rect light shadow maps).\n")
	TEXT("This will remove a significant number of shader permutations if these techniques are not needed."),
	ECVF_RenderThreadSafe | ECVF_ReadOnly
);
...
RENDERCORE_API bool MobileUsesExtendedGBuffer(FStaticShaderPlatform ShaderPlatform)
{
	// For a single pass deferred shading (r.Mobile.AllowFramebufferFetch = 1)
	// Android GLES: uses PLS for deferred shading and limited to 128 bits
	// Vulkan requires:
		// maxDescriptorSetInputAttachments > 4
		// maxColorAttachments > 4
	// iOS: A8+
	static FShaderPlatformCachedIniValue<int32> MobileExtendedGBufferIniValue(TEXT("r.Mobile.ExtendedGBuffer"));
	return MobileExtendedGBufferIniValue.Get(ShaderPlatform) != 0;
}
...
RENDERCORE_API bool MobileUsesGBufferCustomData(FStaticShaderPlatform ShaderPlatform)
{
	...
	return MobileUsesExtendedGBuffer(ShaderPlatform) || !IsStaticLightingAllowed();
}
...
RENDERCORE_API void RenderUtilsInit()
{
	...
	FShaderPlatformConfig::Initialize();
			...
			if (TargetPlatformSettings->UsesRayTracing())
			{
				...
				for (FName FormatName : PlatformRayTracingShaderFormats)
				{
					...
					for (EShaderPlatform ShaderPlatform : PossiblePreviewPlatformsAndMainPlatform)
					{
						...
						GRayTracingShadersPlatformMask[ShaderPlatformIndex] = bRayTracingShadersEnabled;
					}
				}
			}
		}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	const ERayTracingMode DesiredRayTracingMode = RayTracingCVar && RayTracingCVar->GetInt() ? ERayTracingMode::Enabled : ERayTracingMode::Disabled;

	if (DesiredRayTracingMode >= ERayTracingMode::Enabled)
	{
		...
		if (GRHISupportsRayTracing && bRayTracingAllowedOnCurrentPlatform)
		{
			...
			UE_LOGF(LogRendererCore, Log, "Ray tracing is %ls.", GetRayTracingModeName(GRayTracingMode));
			...
			if (bUseRayTracing && SkinCacheCompileShadersCVar->GetInt() <= 0)
			{
				...
				UE_LOGF(LogRendererCore, Fatal, "Ray tracing requires skin cache to be enabled. Set r.SkinCache.CompileShaders>=1.");
			}
			...
			{
				UE_LOGF(LogRendererCore, Log, "Ray tracing shaders are enabled.");
			}
			else
			{
				UE_LOGF(LogRendererCore, Log, "Ray tracing shaders are disabled.");
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogRendererCore, Log, "Ray tracing is disabled. Reason: not supported by current RHI.");
			}
			else
			{
				UE_LOGF(LogRendererCore, Log, "Ray tracing is disabled. Reason: disabled on current platform.");
			}
		}
	}
	else
	{
		UE_LOGF(LogRendererCore, Log, "Ray tracing is disabled. Reason: disabled through project setting (r.RayTracing=0).");
	}
...
{
	// A lot of rendering features assume that buffer size is dividable by 8
	const uint32 RenderingDividableBy = 8;

	static_assert(RenderingDividableBy % SUBSTRATE_TILE_SIZE == 0, "Ensure sizes are dividable by SUBSTRATE_TILE_SIZE");

	const uint32 DividableBy = FMath::Max(RenderingDividableBy, SuggestedDivisor);

	checkf(DividableBy % RenderingDividableBy == 0, TEXT("A lot of rendering features assume that buffer size is dividable by %u"), RenderingDividableBy);
...
{
	// Use FTargetPlatformCachedIniValue because the setting affects texture builds and must match across the target's shader platforms.
	static FTargetPlatformCachedIniValue<int32> PerTargetCVar(TEXT("r.VirtualTextures"));
	return PerTargetCVar.Get(InShaderPlatform) != 0;
}
...
RENDERCORE_API bool UseVirtualTexturing(const ITargetPlatformSettings* InTargetPlatform)
{
	...
	{	
		static FTargetPlatformCachedIniValue<int32> PerTargetCVar(TEXT("r.VirtualTextures"));
		return PerTargetCVar.Get(FName(InTargetPlatform->IniPlatformName())) != 0;
	}
	return UseVirtualTexturing(GMaxRHIShaderPlatform);
}
...
{
	return InTargetPlatform != nullptr ? UseVirtualTexturing(InTargetPlatform) : UseVirtualTexturing(InShaderPlatform);
}

RENDERCORE_API bool UseVirtualTextureLightmap(const FStaticShaderPlatform InShaderPlatform, const ITargetPlatformSettings* InTargetPlatform)
{
	static FTargetPlatformCachedIniValue<int32> PerTargetCVar(TEXT("r.VirtualTexturedLightmaps"));

	if (InTargetPlatform != nullptr)
	{
		return UseVirtualTexturing(InTargetPlatform) && PerTargetCVar.Get(FName(InTargetPlatform->IniPlatformName())) != 0;
	}

	return UseVirtualTexturing(InShaderPlatform) && PerTargetCVar.Get(InShaderPlatform) != 0;
}
...
{
	// "Hair Complex Transmittance" project settings
	static FShaderPlatformCachedIniValue<int32> PerPlatformCVar(TEXT("r.Material.HairComplexTransmittance"));
...
bool DoesPlatformSupportNanite(EShaderPlatform Platform, bool bCheckForProjectSetting)
{
	...
	const bool bFullCheck = bValidPlatform && bSupportNanite;
	return bFullCheck;
}
...
bool NaniteMeshShadersSupported(EShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<bool> PerPlatformCVar(TEXT("r.Nanite.AllowMeshShaders"));
	return PerPlatformCVar.Get(Platform) != 0;
}
...
bool NaniteTranslucencySupported()
{
	static const auto AllowTranslucency = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Nanite.AllowTranslucency"));
	static const bool bAllowTranslucency = (AllowTranslucency && AllowTranslucency->GetValueOnAnyThread() != 0);
	return bAllowTranslucency;
}
...
bool NaniteCurvesSupported()
{
	static const auto AllowCurves = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Nanite.AllowCurves"));
	static const bool bAllowCurves = (AllowCurves && AllowCurves->GetValueOnAnyThread() != 0);
	return bAllowCurves;
}

static bool BindlessConfigSupportsNanite(EShaderPlatform Platform)
{
	const ERHIBindlessConfiguration BindlessConfiguration = FShaderPlatformConfig::GetBindlessConfiguration(Platform);
	return IsBindlessEnabledForAnyGraphics(BindlessConfiguration);
}

bool UseNaniteBindlessShading(EShaderPlatform Platform)
{
	return BindlessConfigSupportsNanite(Platform) && FShaderPlatformConfig::GetEnableNaniteBindlessShading(Platform);
}

bool UseNaniteBindlessRasterization(EShaderPlatform Platform)
{
	return BindlessConfigSupportsNanite(Platform) && FShaderPlatformConfig::GetEnableNaniteBindlessRasterization(Platform);
}
...
bool UseNaniteTessellation()
{
	...
	return bTessellation && bProgrammable && bComputeRaster;
}
...
bool DoesMaterialQualitySupportNanite(EShaderPlatform ShaderPlatform, int32 MaterialQualityLevel)
{
	// Fix ordering of EMaterialQualityLevel Medium/High.
	...
	static FShaderPlatformCachedIniValue<int32> CVarPlatform(TEXT("r.Nanite.MinMaterialQuality"));
	const int32 MinNaniteMaterialQuality = CVarPlatform.Get(ShaderPlatform);
	return MaterialQualityLevel >= MinNaniteMaterialQuality;
}

bool DoesMaterialQualitySupportNanite(EShaderPlatform ShaderPlatform)
{
	static const auto MaterialQualityLevelCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.MaterialQualityLevel"));
	const int32 MaterialQualityLevel = MaterialQualityLevelCVar ? MaterialQualityLevelCVar->GetInt() : 4/**EMaterialQualityLevel::Num*/;
	return DoesMaterialQualitySupportNanite(ShaderPlatform, MaterialQualityLevel);
}
...
bool UseNanite(EShaderPlatform ShaderPlatform, bool bCheckForAtomicSupport /*= true*/, bool bCheckForProjectSetting /*= true*/)
{
	...
	return bNaniteEnabled && DoesMaterialQualitySupportNanite(ShaderPlatform) && DoesRuntimeSupportNanite(ShaderPlatform, bCheckForAtomicSupport, bCheckForProjectSetting);
}
...
bool DoesPlatformSupportClassicShadowMaps(EShaderPlatform Platform)
{
	// Unconditionally supported unless disabled by the project setting
	static FShaderPlatformCachedIniValue<int32> CVarPlatformClassicShadowMapsProjectEnabled(TEXT("r.Shadow.Classic.ProjectEnabled"));
	bool bIsProjectEnable = CVarPlatformClassicShadowMapsProjectEnabled.Get(Platform) != 0;
	return bIsProjectEnable;
}
...
bool DoesPlatformSupportVirtualShadowMaps(EShaderPlatform Platform)
{
	...
	return bIsProjectEnable && DoesPlatformSupportNanite(Platform);
}

bool DoesPlatformSupportNonNaniteVirtualShadowMaps(EShaderPlatform ShaderPlatform)
{
	static FShaderPlatformCachedIniValue<int32> CVarPlatformNonNaniteVSMProjectEnabled(TEXT("r.Shadow.Virtual.NonNanite.ProjectEnabled"));
	bool bIsProjectEnable = CVarPlatformNonNaniteVSMProjectEnabled.Get(ShaderPlatform) != 0;

	return bIsProjectEnable && DoesPlatformSupportVirtualShadowMaps(ShaderPlatform);
}

bool UseNonNaniteVirtualShadowMaps(EShaderPlatform ShaderPlatform, const FStaticFeatureLevel FeatureLevel)
{
	return DoesPlatformSupportNonNaniteVirtualShadowMaps(ShaderPlatform) && UseVirtualShadowMaps(ShaderPlatform, FeatureLevel);
}
...
bool UseVirtualShadowMaps(EShaderPlatform ShaderPlatform)
{
	...
	return bVirtualShadowMapsEnabled &&
		DoesPlatformSupportVirtualShadowMaps(ShaderPlatform) &&
		DoesRuntimeSupportNanite(ShaderPlatform, true /* check for atomics */, true /* check project setting */);
}
...
bool IsUsingSingleLayerWaterFallback(FStaticShaderPlatform Platform)
{
	static const auto CVarWaterSingleLayerMobileFallback = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Water.SingleLayer.MobileFallback"));
	...
	// Only multi-pass deferred supports SLW rendering
	// Disabled for GL, as there is not enough samplers
	if (IsMobileDeferredShadingEnabled(Platform) 
		&& !FReadOnlyCVARCache::MobileAllowFramebufferFetch(Platform)
		&& !IsOpenGLPlatform(Platform))
	{
		return (!CVarWaterSingleLayerMobileFallback || CVarWaterSingleLayerMobileFallback->GetInt() != 0);
	}
...
RENDERCORE_API bool AreInlineSkinningShadersEnabled(EShaderPlatform Platform)
{
	...
	// This results in meshes with more than 64k vertices having skin cache disabled at runtime.
	// So for mobile we must always support inline skinning shaders so that we have something to fall back to.
	if (IsMobilePlatform(Platform))
	{
		return true;
	}

	static FShaderPlatformCachedIniValue<bool> PerPlatformAllowCVar(TEXT("r.SkinCache.Allow"));
	static FShaderPlatformCachedIniValue<int32> PerPlatformCompileCVar(TEXT("r.SkinCache.CompileShaders"));
	return PerPlatformAllowCVar.Get(Platform) == 0 || PerPlatformCompileCVar.Get(Platform) != 2;
}

RENDERCORE_API bool AreSkinCacheShadersEnabled(EShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<int32> PerPlatformCVar(TEXT("r.SkinCache.CompileShaders"));
	return (PerPlatformCVar.Get(Platform) != 0);
}
...
RENDERCORE_API bool IsGPUSkinCacheAllowed(EShaderPlatform Platform)
{
	...
	static FShaderPlatformCachedIniValue<bool> PerPlatformMobileCVar(TEXT("r.Mobile.SkinCache.Allow"));
	return IsMobilePlatform(Platform) ? PerPlatformMobileCVar.Get(Platform) : PerPlatformCVar.Get(Platform);
}
...
{
	// Note that we rely on the IMeshDeformerProvider being loaded before any calls to this function.
	// OptimusSettings/ComputeFramework are PostConfigInit for that reason.
	static IMeshDeformerProvider* MeshDeformerProvider = IMeshDeformerProvider::Get();
	return MeshDeformerProvider && MeshDeformerProvider->IsSupported(Platform);
}
...
RENDERCORE_API bool IsGPUSkinPassThroughSupported(EShaderPlatform Platform)
{
	...
	return IsGPUSkinCacheAvailable(Platform) || AreMeshDeformersAvailable(Platform);
}
...
bool GetMaterialSimulateMobileForLowQuality()
{
	static const TConsoleVariableData<bool>* const CVarMaterialSimulateMobileForLowQuality = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("r.Material.SimulateMobileForLowQuality"));
	return CVarMaterialSimulateMobileForLowQuality && CVarMaterialSimulateMobileForLowQuality->GetValueOnAnyThread();
}

// This is a deprecated version of the function in 5.8 and should not be used.
...
{
	return !IsMobilePlatform(Platform) && (GVelocityEncodeDepthPlatformMask[(int)Platform]);
}
...
bool IsRayTracingEnabled()
{
	...
	const bool bRayTracingEnabled = RayTracingEnableCVar && RayTracingEnableCVar->GetValueOnAnyThread() != 0;
...
static TAutoConsoleVariable<FString> CVarSubstrateMaterialQualityLevelsClosuresPerPixel(
	TEXT("r.Substrate.MaterialQualityLevelsClosuresPerPixel"),
	TEXT(""),	// Empty => invalid so ignored.
	TEXT("Define the maximum number of closures evaluated per pixel for a given material quality level. Empty by default: no impact. Otherwise L1-M1-H1-E1 to set closure count for each of the material quality level (Low, Medium, High and Epic respecively)."),
...
static TAutoConsoleVariable<int32> CVarMaterialHairComplexTransmittance(
	TEXT("r.Material.HairComplexTransmittance"),
	0,
	TEXT("Enable hair complex transmittance (e.g,dual scattering) to all hair surfaces/pixel. When disabled, only groom geometries use hair complex transmittance.")
	TEXT("Disabled by default. Toggling this setting invalidates shaders and requires an editor restart."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);
...
static TAutoConsoleVariable<int32> CVarSubstrateSimpleTiledLightingPasses(
	TEXT("r.Substrate.SimpleTiledLightingPasses"),
	0,
	TEXT("Enabling simple tiled lighting passes when greater than 0, can help performance on low end platforms not supporting Rect primitives. Only stencil marking will be tiled. And tiled passes will use larger tiles. Not compatible with Lumen nor MegaLight."),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarSubstrateExperimentalToonReflectionQuantizationEnabled(
	TEXT("r.Substrate.Experimental.ToonReflectionQuantizationEnabled"),
	0,
	TEXT("Enable experimental toon reflection quantization."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarSubstrateExperimentalToonUnifiedDiffuse(
	TEXT("r.Substrate.Experimental.ToonUnifiedDiffuse"),
	0,
	TEXT("Enable experimental toon unified diffuse evaluation."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);
...
namespace Substrate
{
	...
	{
		return GetClosurePerPixel(InPlatform, 0);
	}

	struct FSubstratePerMaterialLevelQualityClosureCount
	{
		bool bIsValid = false;	// Only true if the cvar has been specified explicitely, i.e. not empty.
		uint32 LowMaterialQualityMaxClosureCount	= 1;
		uint32 MediumMaterialQualityMaxClosureCount	= 1;
		uint32 HighMaterialQualityMaxClosureCount	= 1;
		uint32 EpicMaterialQualityMaxClosureCount	= 1;
	};

	const FSubstratePerMaterialLevelQualityClosureCount& GetPerMaterialLevelQualityClosureCount(EShaderPlatform InPlatform)
	{
		// Typically there is only one platform. In editor many can be evaluated for multiple preview in a session. So a Map should be enough.
		static TMap<EShaderPlatform, FSubstratePerMaterialLevelQualityClosureCount> CachedPlatformResults;

		static FCriticalSection CS;
		FScopeLock Lock(&CS);

		FSubstratePerMaterialLevelQualityClosureCount* FoundResult = CachedPlatformResults.Find(InPlatform);
		if (!FoundResult)
		{
			FSubstratePerMaterialLevelQualityClosureCount& Result = CachedPlatformResults.Add(InPlatform);

			static FShaderPlatformCachedIniValue<FString> CVarMaterialQualityLevelsClosuresPerPixel(TEXT("r.Substrate.MaterialQualityLevelsClosuresPerPixel"));
			FString CVARString = CVarMaterialQualityLevelsClosuresPerPixel.Get(InPlatform);
			if (!CVARString.IsEmpty())
			{
				// The format must be in style "L1-M1-H1-E1"
				int32 ValidRead =
					...
					sscanf(
#endif
					TCHAR_TO_ANSI(*CVARString), "L%u-M%u-H%u-E%u",
					&Result.LowMaterialQualityMaxClosureCount,
					&Result.MediumMaterialQualityMaxClosureCount,
					&Result.HighMaterialQualityMaxClosureCount,
					&Result.EpicMaterialQualityMaxClosureCount);
				Result.bIsValid = true;

				if (ValidRead != 4) // not all input have been read.
				{
					Result.bIsValid = false;
					Result.LowMaterialQualityMaxClosureCount = 1;
					Result.MediumMaterialQualityMaxClosureCount = 1;
					Result.HighMaterialQualityMaxClosureCount = 1;
					Result.EpicMaterialQualityMaxClosureCount = 1;
				}

				Result.LowMaterialQualityMaxClosureCount = FMath::Max(1u, Result.LowMaterialQualityMaxClosureCount);
				Result.MediumMaterialQualityMaxClosureCount = FMath::Max(1u, Result.MediumMaterialQualityMaxClosureCount);
				Result.HighMaterialQualityMaxClosureCount = FMath::Max(1u, Result.HighMaterialQualityMaxClosureCount);
				Result.EpicMaterialQualityMaxClosureCount = FMath::Max(1u, Result.EpicMaterialQualityMaxClosureCount);
			}
			return Result;
		}
		return *FoundResult;
	...
	uint32 GetClosurePerPixel(EShaderPlatform InPlatform, uint8 InMaterialQualityLevel)
	{
		if (Substrate::IsSubstrateBlendableGBufferEnabled(InPlatform))
		{
			return 1u; // When using the blendable GBuffer, we enforce a single closure so that a single slab can be read to export.
		}

		uint32 OutClosurePerPixel = FShaderPlatformConfig::GetSubstrateMaxClosuresPerPixel(InPlatform);
		...
		const FSubstratePerMaterialLevelQualityClosureCount& PerMaterialLevelQualityClosureCount = GetPerMaterialLevelQualityClosureCount(InPlatform);
		if (PerMaterialLevelQualityClosureCount.bIsValid && InMaterialQualityLevel != IgnoreMaterialQualityLevelsForClosurePerPixel)
		{
			check(0 <= InMaterialQualityLevel && InMaterialQualityLevel <= 3);
			uint32 SelectedMaterialQualityMaxClosureCount = PerMaterialLevelQualityClosureCount.LowMaterialQualityMaxClosureCount;
			switch (InMaterialQualityLevel)
			{
				//	case 0: // Low
			case 2: // Medium
				SelectedMaterialQualityMaxClosureCount = PerMaterialLevelQualityClosureCount.MediumMaterialQualityMaxClosureCount;
				break;
			case 1: // High
				SelectedMaterialQualityMaxClosureCount = PerMaterialLevelQualityClosureCount.HighMaterialQualityMaxClosureCount;
				break;
			case 3: // Epic
				SelectedMaterialQualityMaxClosureCount = PerMaterialLevelQualityClosureCount.EpicMaterialQualityMaxClosureCount;
				break;
			}
			// The per material quality level value can only restricts the platform configured capacity.
			OutClosurePerPixel = FMath::Min(SelectedMaterialQualityMaxClosureCount, OutClosurePerPixel);
		}
		OutClosurePerPixel = FMath::Min<uint32>(ProjectClosurePerPixel, OutClosurePerPixel);
		...
		return FMath::Max<uint32>(1, OutClosurePerPixel);
	}
	...
	bool IsToonProfileEnabled()
	{
		return IsSubstrateEnabled();
	}
	...
	bool IsOpaqueRoughRefractionEnabled(EShaderPlatform InPlatform)
	{
		...
		const bool bIsSubstrateBlendableGBufferEnabled = IsSubstrateBlendableGBufferEnabled(InPlatform);

		// By default sample closures from cvars using highest material quality level. 
		// If even the highest level does not have many closures then we assume lower levels won't neither.
		bool bManyClosures = GetClosurePerPixel(InPlatform, Substrate::IgnoreMaterialQualityLevelsForClosurePerPixel) > 1;

		// If we have per material quality level closure count constraints, we should check them all and assume bManyClosures==true only when any of them is >1 since material/global shaders are all compile for only one case.
		const FSubstratePerMaterialLevelQualityClosureCount& PerMaterialLevelQualityClosureCount = GetPerMaterialLevelQualityClosureCount(InPlatform);
		if (bManyClosures && !bIsSubstrateBlendableGBufferEnabled && PerMaterialLevelQualityClosureCount.bIsValid)
		{
			if (PerMaterialLevelQualityClosureCount.LowMaterialQualityMaxClosureCount > 1
				|| PerMaterialLevelQualityClosureCount.MediumMaterialQualityMaxClosureCount > 1
				|| PerMaterialLevelQualityClosureCount.HighMaterialQualityMaxClosureCount > 1
				|| PerMaterialLevelQualityClosureCount.EpicMaterialQualityMaxClosureCount > 1)
			{
				bManyClosures = true;
			}
			else
			{
				// All material quality level will constrain the closure count to 1 so set bManyClosures to false.
				bManyClosures = false;
			}
		}

		return IsSubstrateEnabled() && CVar.Get(InPlatform) > 0 && !bIsSubstrateBlendableGBufferEnabled && bManyClosures && !IsExperimentalToonUnifiedDiffuseEnabled();
	}
	...
	{
		return CVarSubstrateSimpleTiledLightingPasses.GetValueOnAnyThread() <= 0;
	}
...
bool PlatformSupportsSeparatedSubsurfaceDiffuse(const FStaticShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<int32> PerPlatformCVar(TEXT("r.SSS.SeparatedDiffuse.Supported"));
	return (PerPlatformCVar.Get(Platform) != 0) && !Substrate::IsOpaqueRoughRefractionEnabled(Platform);
}
...
bool ShouldCompileMegaLightsShaders(EShaderPlatform ShaderPlatform)
{
	...
	static const TConsoleVariableData<bool>* CVar = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("r.MegaLights.Supported"));
	const bool bMegaLightsSupported = CVar && CVar->GetValueOnAnyThread();
...
bool SupportsSkyLightCaptureAffectsHeightFogInScattering(const FStaticShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<int32> CVar(TEXT("r.SupportsSkyLightCaptureAffectsHeightFogInScattering"));
	return CVar.Get(Platform) > 0;
}
...
bool ShouldUseParallelRendererTasks()
{
	return FApp::ShouldUseThreadingForPerformance() && GIsThreadedRendering;
}
