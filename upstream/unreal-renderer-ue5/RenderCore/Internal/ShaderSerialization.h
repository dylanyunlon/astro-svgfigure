...
{
	RENDERCORE_API FShaderCacheSerializeContext();

	RENDERCORE_API virtual ~FShaderCacheSerializeContext();
	...
	/* View on array of symbol file arrays, one per shader/stage. Each inner array holds the named
	 * symbol files for that shader (typically 1-2 files). May or may not point to OwnedShaderSymbolFiles.
	 */
	TArrayView<TArray<FShaderSymbolFile>> ShaderSymbolFiles;
	...
	/* Array of symbol file arrays actually owned by this context object; it is valid for this to be empty in cases
	 * where the array of ShaderSymbolFiles is stored externally.
	 */
	TArray<TArray<FShaderSymbolFile>> OwnedShaderSymbolFiles;
	...
	RENDERCORE_API void MoveCode(TArray<FCompositeBuffer>& TargetCode, TArray<TArray<FShaderSymbolFile>>& TargetSymbolFiles);
...
struct FShaderCacheSaveContext : public FShaderCacheSerializeContext
{
	...
	RENDERCORE_API virtual ~FShaderCacheSaveContext();
...
struct FShaderCacheLoadContext : public FShaderCacheSerializeContext
{
	...
	RENDERCORE_API FShaderCacheLoadContext(FSharedBuffer ShaderObjectData, TArrayView<FCompositeBuffer> CodeBuffers, TArrayView<TArray<FShaderSymbolFile>> SymbolFiles);

	RENDERCORE_API virtual ~FShaderCacheLoadContext();

	/* Resets internal state to point to the given buffers and recreates the owned memoryreader */
	RENDERCORE_API void Reset(FSharedBuffer ShaderObjectData, TArrayView<FCompositeBuffer> CodeBuffers, TArrayView<TArray<FShaderSymbolFile>> SymbolFiles);
...
#if WITH_EDITOR
	/* Populates this load context from a DDC record: extracts the main object buffer, per-shader code buffers
	 * and (when present) per-shader symbol files, and prepares the internal archive for subsequent Serialize() calls. */
	RENDERCORE_API void ReadFromRecord(const UE::DerivedData::FCacheRecord& CacheRecord);
#endif
