...
#define DDPI_NUM_STATIC_SHADER_PLATFORMS 16
	...
	SP_METAL_SM6_IOS 	  = SP_StaticPlatform_Last + 7,
	...
	/** Sentinel value used by callers that have no meaningful target shader platform; never use as an index. */
	SP_InvalidPlatform = (1 << SP_NumBits) - 1,
};
static_assert(SP_NumPlatforms <= (1 << SP_NumBits), "SP_NumPlatforms will not fit on SP_NumBits");
static_assert(SP_NumPlatforms < SP_InvalidPlatform, "SP_InvalidPlatform must be greater than every real shader platform value");
