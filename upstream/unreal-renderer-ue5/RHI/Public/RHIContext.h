...
public:
	...
	*/
	...
	virtual void RHISetUniformBlockOffset(FRHIShaderResourceView* SRV, uint32 Offset)
	{
	...
	UE_DEPRECATED(5.8, "RHIGetNativeCommandBuffer has been deprecated. Use RHI platform-specific interfaces to retrieve the native command buffer from a bottom-of-pipe RHI command list in a typesafe way.")
...
struct FRayTracingSceneBuildParams
{
	...
	// Buffer of actual number of instances used for indirect acceleration structure build (if supported)
	FRHIBuffer* InstanceCountBuffer = nullptr;
	uint32 InstanceCountBufferOffset = 0;
	...
	// Unique list of buffers referenced by instances in this scene.
	// Any referenced buffer is kept alive while the scene is alive.
	TConstArrayView<FRHIBuffer*> ReferencedBuffers;
