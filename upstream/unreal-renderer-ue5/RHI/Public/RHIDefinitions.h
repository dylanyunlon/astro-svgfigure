...
#define RHI_USE_SYNC_POINT_DEBUG_NAME	0
#else
...
#ifdef RHI_NEW_GPU_PROFILER
#undef RHI_NEW_GPU_PROFILER
#endif
#define RHI_NEW_GPU_PROFILER UE_DEPRECATED_MACRO(5.8, "The legacy GPU profiler has been removed. RHI_NEW_GPU_PROFILER is deprecated, and always defined to 1. Remove any remaining uses of this macro.") 1

#ifndef HAS_GPU_STATS
#define HAS_GPU_STATS ((STATS || CSV_PROFILER_STATS) && (!UE_BUILD_SHIPPING))
#endif


/**
 * The type used to pass the specialization values around
 */
using FSpecializationContainerType = TArray<int32>;
...
enum class UE_DEPRECATED(5.7, "ERHIBindlessSupport is not used anymore, it is now a boolean") ERHIBindlessSupport : uint8
{
	Unsupported,
	RayTracingOnly,
	AllShaderTypes,
...
inline bool IsValidPreRasterizationFrequency(EShaderFrequency InShaderFrequency)
{
	// Any graphics frequency that isn't pixel
	return IsValidGraphicsFrequency(InShaderFrequency) && (InShaderFrequency != SF_Pixel);
}
...
enum class UE_DEPRECATED(5.8, "ERHIZBuffer is being removed as the assumption of using an Inverted Z buffer is so ingrained that it could not reasonably be changed.") ERHIZBuffer
{
	FarPlane = 0,
	NearPlane = 1,
	IsInverted = 1,
...
enum class EShaderCodeResourceBindingType : uint8
{
	...
	// Texture2DMultiView can represent a Texture2D or Texture2DArray depending on whether shaders were compiled to use mobile multi-view (VK_KHR_multiview)
	Texture2DMultiView,
	RWTexture2DMultiView,
...
inline bool IsResourceBindingTypeSRV(EShaderCodeResourceBindingType Type)
{
	...
	case EShaderCodeResourceBindingType::Texture2DMultiView:
	...
	case EShaderCodeResourceBindingType::RWTexture2DMultiView:
	...
	case EShaderCodeResourceBindingType::DescriptorRange:
		return false;
...
enum EUniformBufferBaseType : uint8
{
	...
	UBMT_DESCRIPTOR_RANGE,

	// RDG-tracked view into a buffer bound as a uniform buffer object (UBO) on platforms that support it.
	UBMT_RDG_UNIFORM_BLOCK,
...
enum class ERHIUniformBufferFlags : uint8
{
	...
	/** When bindless is enabled, make sure this uniform buffer can be accessed bindlessly. */
	BindlessAccessible      = 1 << 4,
};
...
enum ERHIResourceType : uint8
{
	...
	RRT_Buffer,
	RRT_Texture,
	RRT_Texture2D      UE_DEPRECATED(5.7,      "RRT_Texture2D is unused. Remove any remaining uses of it. There is a single RHI texture type which always uses RRT_Texture. RHI texture references use RRT_TextureReference."),
	RRT_Texture2DArray UE_DEPRECATED(5.7, "RRT_Texture2DArray is unused. Remove any remaining uses of it. There is a single RHI texture type which always uses RRT_Texture. RHI texture references use RRT_TextureReference."),
	RRT_Texture3D      UE_DEPRECATED(5.7,      "RRT_Texture3D is unused. Remove any remaining uses of it. There is a single RHI texture type which always uses RRT_Texture. RHI texture references use RRT_TextureReference."),
	RRT_TextureCube    UE_DEPRECATED(5.7,    "RRT_TextureCube is unused. Remove any remaining uses of it. There is a single RHI texture type which always uses RRT_Texture. RHI texture references use RRT_TextureReference."),
	RRT_TextureReference,
	RRT_TimestampCalibrationQuery UE_DEPRECATED(5.8, "RRT_TimestampCalibrationQuery is unused. Remove any remaining uses of it."),
	RRT_GPUFence,
	RRT_RenderQuery,
	RRT_RenderQueryPool,
	RRT_Viewport,
	...
	RRT_RayTracingAccelerationStructure UE_DEPRECATED(5.7, "RRT_RayTracingAccelerationStructure is deprecated. Use RRT_RayTracingGeometry or RRT_RayTracingScene instead."),
	RRT_RayTracingGeometry,
	RRT_RayTracingScene,
...
inline constexpr ERHIDescriptorTypeMask RHIDescriptorTypeMaskFromType(ERHIDescriptorType InType)
{
	if (InType == ERHIDescriptorType::Invalid)
	{
		return ERHIDescriptorTypeMask::None;
	}
	return static_cast<ERHIDescriptorTypeMask>(1 << static_cast<int32>(InType));
}
...
struct FRHIDescriptorHandle
{
	...
	{
		return RHIDescriptorTypeMaskFromType(GetType());
	}

	bool IsValid() const
	{
		return Index != MAX_uint32 && Type != ERHIDescriptorType::Invalid;
	}
...
struct FRHIBindlessDescriptorAllocationInfo
{
	ERHIDescriptorTypeMask Types{};
	uint64 Count = 0;
	uint64 Capacity = 0;
...
enum class ERHIShaderBundleMethod
{
	// Dispatch shader bundles using the emulation path.
	Emulation = 0,

	// Dispatch shader bundles using native platform features.
	Native = 1,
};
...
inline bool IsRDGBufferReferenceShaderParameterType(EUniformBufferBaseType BaseType)
{
	return
		...
		BaseType == UBMT_RDG_UNIFORM_BLOCK;
}
...
inline bool IsShaderParameterTypeForUniformBufferLayout(EUniformBufferBaseType BaseType)
{
	return
		...
		// Uniform block occupies a UB binding slot.
		BaseType == UBMT_RDG_UNIFORM_BLOCK;
}
...
UE_DEPRECATED(5.7, "GetRHIResourceType(ETextureDimension Dimension) is deprecated. There is a single RHI texture type which always uses RRT_Texture now, which makes this function pointless. Remove any remaining uses of this function.")
inline ERHIResourceType GetRHIResourceType(ETextureDimension /*Dimension*/)
{
	return ERHIResourceType::RRT_Texture;
}
...
struct FRHIFlipDetails
{
	...
	FRHIFlipDetails(uint64 InPresentIndex, double InFlipTimeInSeconds, double InVBlankTimeInSeconds, uint64 InVBlankTimeInCycles, float InHz)
