...
extern RHI_API std::atomic<uint64> GInputLatencyTime;
...
public:
	...
	{
		FRHICommandListBase::EnqueueLambda(EThreadFence::Disabled, LambdaName, Forward<LAMBDA>(Lambda));
	}
...
public:
	...
	inline void UpdateUniformBuffer(FRHIUniformBuffer* UniformBufferRHI, const void* Contents)
	{
		...
		UE::RHI::UniformBufferContentMap::Update(UniformBufferRHI, Contents);
#endif
	}
	...
	inline void UpdateStreamSourceSlot(FRHIStreamSourceSlot* StreamSourceSlotRHI, FRHIBuffer* BufferRHI)
	{
		...
		{
			EnqueueLambda(EThreadFence::Enabled, TEXT("UpdateStreamSourceSlot"),
				[this, StreamSourceSlotRHI, BufferRHI](FRHICommandListBase&)
				{
					StreamSourceSlotRHI->Buffer = BufferRHI;
				});
		}
	...
	FDescriptorRangeRHIRef CreateDescriptorRange(const FRHIDescriptorRangeDesc& Desc, TConstArrayView<FRHIDescriptorResource> InitialResources)
	{
		return GDynamicRHI->RHICreateDescriptorRange(*this, Desc, InitialResources);
	}

	void UpdateDescriptorRange(FRHIDescriptorRange* DescriptorRange, uint32 FirstResourceIndex, TConstArrayView<FRHIDescriptorResource> NewResources)
	{
		GDynamicRHI->RHIUpdateDescriptorRange(*this, DescriptorRange, FirstResourceIndex, NewResources);
	}
	...
	/*
	* Writes the GPU addresses of the provided resources into a GPU buffer that can be accessed in shaders.
	* Addresses should be queried immediately before use, as certain RHI operations such as lock/update/defragment may move the resources in memory.
	*/
	void WriteResourceGPUAddresses(TConstArrayView<FRHIResource*> Resources, FRHIBuffer* OutputBuffer)
	{
		if (!Bypass())
		{
			Resources = AllocArray(Resources);
		}

		for (uint32 GPUIndex : GetGPUMask())
		{
			uint64* AddressesPtr = (uint64*)LockBufferMGPU(
				OutputBuffer,
				GPUIndex,
				0,
				Resources.Num() * sizeof(uint64), RLM_WriteOnly);

			GDynamicRHI->RHIWriteResourceGPUAddresses(*this, GPUIndex, Resources, AddressesPtr);

			UnlockBufferMGPU(OutputBuffer, GPUIndex);
		}
	}
...
protected:
	...
	inline void ValidateBoundShader(const FRHIComputeShader* ShaderRHI)
	{
		checkSlow(PersistentState.BoundComputeShaderRHI == ShaderRHI);
	}

	inline void ValidateBoundShader(const FRHIGraphicsShader* ShaderRHI)
	{
#if DO_GUARD_SLOW
		switch (ShaderRHI->GetFrequency())
		{
		case SF_Vertex:        checkSlow(PersistentState.BoundShaderInput.GetVertexShader()        == ShaderRHI); break;
		...
		case SF_Pixel:         checkSlow(PersistentState.BoundShaderInput.GetPixelShader()         == ShaderRHI); break;
		case SF_Geometry:      checkSlow(PersistentState.BoundShaderInput.GetGeometryShader()      == ShaderRHI); break;
		default: checkfSlow(false, TEXT("Unexpected shader type %d"), ShaderRHI->GetFrequency());
		}
#endif // DO_GUARD_SLOW
	}
...
#define FRHICOMMAND_MACRO_TPL(TemplateType, CommandName)			\
	...
	template<typename TemplateType>									\
	struct CommandName final : public FRHICommand<CommandName<TemplateType>, UE_JOIN(CommandName##String, __LINE__)>
...
FRHICOMMAND_MACRO(FRHICommandDispatchComputeShaderBundle)
{
	...
	inline FRHICommandDispatchComputeShaderBundle(
		...
		ERHIShaderBundleMethod InMethod
	)
...
FRHICOMMAND_MACRO(FRHICommandSetUniformBlockOffset)
{
	FRHIShaderResourceView* SRV;
	uint32 Offset;

	inline FRHICommandSetUniformBlockOffset(FRHIShaderResourceView* InSRV, uint32 InOffset)
		: SRV(InSRV)
		, Offset(InOffset)
	{
	}
...
extern RHI_API FComputePipelineState*		FindComputePipelineState(const FComputePipelineStateInitializer& Initializer, bool bVerifyUse = true);
extern RHI_API FComputePipelineState*		GetComputePipelineState(FRHIComputeCommandList& RHICmdList, const FComputePipelineStateInitializer& Initializer, bool bVerifyUse = true);
...
//UE_DEPRECATED(5.8, "Identifying compute pipelines specifying only a shader is deprecated, use a FComputePipelineStateInitializer.")
inline FComputePipelineState* FindComputePipelineState(FRHIComputeShader* ComputeShader, bool bVerifyUse = true)
{
	return FindComputePipelineState(FComputePipelineStateInitializer(ComputeShader), bVerifyUse);
}

//UE_DEPRECATED(5.8, "Identifying compute pipelines specifying only a shader is deprecated, use a FComputePipelineStateInitializer.")
inline FComputePipelineState* GetComputePipelineState(FRHIComputeCommandList& RHICmdList, FRHIComputeShader* ComputeShader, bool bVerifyUse = true)
{
	return GetComputePipelineState(RHICmdList, FComputePipelineStateInitializer(ComputeShader), bVerifyUse);
}
...
public:
	...
	inline void EnqueueLambda(const TCHAR* LambdaName, LAMBDA&& Lambda)
	{
		FRHIComputeCommandList::EnqueueLambda(EThreadFence::Disabled, LambdaName, Forward<LAMBDA>(Lambda));
	}
	...
	inline void SetUniformBlockOffset(FRHIShaderResourceView* SRV, uint32 Offset)
	{
		if (Bypass())
		{
			GetContext().RHISetUniformBlockOffset(SRV, Offset);
			return;
		}
		ALLOC_COMMAND(FRHICommandSetUniformBlockOffset)(SRV, Offset);
	}
	...
	)
	{
		...
		if (Bypass())
		{
			GetContext().RHIDispatchComputeShaderBundle(ShaderBundle, RecordArgBuffer, SharedBindlessParameters, Dispatches, Method);
			return;
		}
		ValidateShaderBundleComputeDispatch(Dispatches);
		ALLOC_COMMAND(FRHICommandDispatchComputeShaderBundle)(ShaderBundle, RecordArgBuffer, SharedBindlessParameters, Dispatches, Method);
	}
	...
	)
	{
		...
		if (Bypass())
		{
			GetContext().RHIDispatchGraphicsShaderBundle(ShaderBundle, RecordArgBuffer, BundleState, SharedBindlessParameters, Dispatches, Method);
			return;
		}
		ALLOC_COMMAND(FRHICommandDispatchGraphicsShaderBundle)(ShaderBundle, RecordArgBuffer, BundleState, SharedBindlessParameters, Dispatches, Method);
	}
	...
	inline FRHIBreadcrumbAllocator& GetBreadcrumbAllocator()
	{
		if (!BreadcrumbAllocator.IsValid())
		{
			...
			BreadcrumbAllocatorRefs.Add(BreadcrumbAllocator.ToSharedRef()); // Not AddUnique because we know this is a new instance.
		}
	...
	UE_DEPRECATED(5.8, "SubmitCommandsHint is deprecated, and has no effect if called on a non-immediate RHI command list. Consider calling ImmediateFlush(EImmediateFlushType::DispatchToRHIThread) on the immediate command list instead.")
	...
	*/
	...
	inline void BuildAccelerationStructures(TConstArrayView<FRayTracingSceneBuildParams> Params)
	{
		...
		{
			// Copy the params themselves as well their ReferencedGeometries/ReferencedBuffers, if there are any.
			// AllocArray() can't be used here directly, as we have to modify the params after copy.
			...
			for (int32 Index = 0; Index < Params.Num(); ++Index)
			{
				...
				if (Params[Index].ReferencedBuffers.Num())
				{
					InlineParams[Index].ReferencedBuffers = AllocArray(Params[Index].ReferencedBuffers);
				}
			}
...
public:
	...
	inline void EnqueueLambda(const TCHAR* LambdaName, LAMBDA&& Lambda)
	{
		FRHICommandList::EnqueueLambda(EThreadFence::Disabled, LambdaName, Forward<LAMBDA>(Lambda));
	}
	...
	UE_DEPRECATED(5.8, "ClearShaderBindingTable is deprecated and does nothing. Remove any remaining calls to this function.")
...
enum class ERHISubmitFlags
{
	...
	// When combined with EndFrame, triggers a GPU profile on the following frame.
	ProfileNextFrame = 1 << 4,
...
#if WITH_RHI_BREADCRUMBS
	EnableDrawEvents  = 1 << 7,
	DisableDrawEvents = 1 << 8
#endif
};
...
public:
	...
	UE_DEPRECATED(5.7, "FRHICommandListImmediate::EndDrawingViewport(FRHIViewport* Viewport, bool bPresent, bool bLockToVsync) is deprecated. Use FRHIPresentArgs to pass the parameters needed.")
	inline void EndDrawingViewport(FRHIViewport* Viewport, bool bPresent, bool bLockToVsync)
	{
		EndDrawingViewport(Viewport, FRHIPresentArgs(0, bPresent, bLockToVsync));
	}
	RHI_API void EndDrawingViewport(FRHIViewport* Viewport, const FRHIPresentArgs& InPresentArgs);
	...
	inline void EnqueueLambda(const TCHAR* LambdaName, LAMBDA&& Lambda)
	{
		FRHICommandListImmediate::EnqueueLambda(EThreadFence::Disabled, TEXT("TRHILambdaCommand"), Forward<LAMBDA>(Lambda));
	}
	...
	inline void* LockStagingBuffer(FRHIStagingBuffer* StagingBuffer, uint32 Offset, uint32 SizeRHI)
	{
		return LockStagingBuffer(StagingBuffer, nullptr, Offset, SizeRHI);
	}
	...
	RHI_API FTextureRHIRef AsyncReallocateTexture2D(FRHITexture* Texture2D, int32 NewMipCount, int32 NewSizeX, int32 NewSizeY, FThreadSafeCounter* RequestStatus);
	...
	inline void SubmitAndBlockUntilGPUIdle(const bool bDeleteResources = false)
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_RHIMETHOD_SubmitAndBlockUntilGPUIdle_Flush);

		const EImmediateFlushType::Type FlushType = bDeleteResources ? EImmediateFlushType::FlushRHIThreadFlushResources : EImmediateFlushType::FlushRHIThread;
		
		// Ensure all prior work is submitted down to the GPU, and the RHI thread is idle.
		ImmediateFlush(FlushType);

		// Block the calling thread (the render thread) until the GPU completes all work.
		GDynamicRHI->RHIBlockUntilGPUIdle();
	}
	
	UE_DEPRECATED(5.9, "BlockUntilGPUIdle() is deprecated. Call SubmitAndBlockUntilGPUIdle() instead.")
	inline void BlockUntilGPUIdle()
	{
		this->SubmitAndBlockUntilGPUIdle();
	}

	UE_DEPRECATED(5.9, "SubmitCommandsAndFlushGPU() is deprecated. Call SubmitAndBlockUntilGPUIdle() instead.")
	...
	UE_DEPRECATED(5.8, "GetNativePhysicalDevice is deprecated. Use GetIVulkanDynamicRHI()->RHIGetVkPhysicalDevice().")
	...
	UE_DEPRECATED(5.8, "GetNativeGraphicsQueue is deprecated. Use platform-specific RHI interfaces instead.")
	...
	UE_DEPRECATED(5.8, "GetNativeComputeQueue is deprecated. Use platform-specific RHI interfaces instead.")
	...
	UE_DEPRECATED(5.8, "GetNativeInstance is deprecated. The only useful implementation was in Vulkan RHI. Use GetIVulkanDynamicRHI()->RHIGetVkInstance() instead.")
	...
	UE_DEPRECATED(5.8,
		"GetNativeCommandBuffer is dangerous, and has been deprecated. "
		"Command buffers are owned by RHI contexts, and can therefore only be retrieved by bottom-of-pipe threads (the RHI thread, or parallel translate tasks). "
		"Use RHI platform-specific interfaces to retrieve the native command buffer from a bottom-of-pipe RHI command list in a typesafe way.")
	inline void* GetNativeCommandBuffer()
	{
		if (ensure(IsBottomOfPipe()))
		{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
			return GetContext().RHIGetNativeCommandBuffer();
	...
	UE_DEPRECATED(5.8, "SubmitCommandsHint is deprecated. Consider calling ImmediateFlush(EImmediateFlushType::DispatchToRHIThread) instead.")
	inline void SubmitCommandsHint()
	{
...
#if WITH_MGPU
	#define SCOPED_GPU_MASK(RHICmdList, GPUMask) FRHIScopedGPUMask UE_JOIN(ScopedGPUMask, __LINE__){ RHICmdList, GPUMask }
#else
...
#define SCOPED_UNIFORM_BUFFER_STATIC_BINDINGS(RHICmdList, UniformBuffers) FScopedUniformBufferStaticBindings UE_JOIN(UniformBuffers, __LINE__){ RHICmdList, UniformBuffers }
...
private:
	...
	public:
		struct ITask;
		using FFunc = TFunction<void(ITask&)>;

		struct ITask
		{
			virtual void ExtendTask(FGraphEventArray&& NewPrereqs, FFunc&& NewLambda) = 0;
		};

		// Enqueues the given lambda to run on the named thread.
		void Enqueue(ENamedThreads::Type NamedThread, FGraphEventArray&& Prereqs, FFunc&& Lambda);
	...
	struct FTranslateState
	{
		...
		FDynamicRHI::FRHICloseTranslateChainResult FinalizedCommandLists;
		...
		std::atomic<FTranslateState*> Next { nullptr };

		// True when parallel translate is enabled globally
		const bool bAllowParallelTranslate;

		// True when this particular chain is a parallel translate chain
		const bool bParallel;

		const bool bUsingSubCmdLists;

		bool bShouldFinalize = true;

		FTaskPipe* EnqueueTranslateTask(FGraphEventArray&& Prereqs, FTaskPipe::FFunc&& Lambda);

		void Translate_ExecuteCommandList(FRHICommandListBase* CmdList);
		void Translate_CloseChain();

		FTranslateState(const FSubmitState& SubmitState, FRHICommandListBase* FirstCmdList);
	};
	...
	struct FSubmitState
	{
		...
		struct
		{
			TArray<TUniquePtr<FTranslateState>> All;
			FGraphEventArray Events;

			FTranslateState* First = nullptr;
			FTranslateState* Current = nullptr;
			FTranslateState* Previous = nullptr;
		} TranslateJobs;
		...
		void Dispatch_ProcessCommandList(FRHICommandListBase* CmdList);
		void Dispatch_PrepareBreadcrumbs(FRHICommandListBase* CmdList);
		FGraphEventRef Dispatch_CloseTranslateChain(bool bLastChainInSubmission, uint32 RTSubmissionID);
		void Dispatch_CloseSubmission(TOptional<int64> LocalStatsFrame, uint32 RTSubmissionID);

		void Submit_SubmitToGPU(FTranslateState* First, FTaskPipe::ITask& Task, uint64 TracingSubmissionID);
		void Submit_Close(const FCloseArgs& Args);

		FSubmitState(bool bBatchSubmissions)
			: bBatchSubmissions(bBatchSubmissions)
		{}

	} *SubmitState = nullptr;

	FTaskPipe DispatchPipe;
	FTaskPipe SingleTranslatePipe;
	FTaskPipe SubmitPipe;

	FGraphEventRef LastMutate;
	FGraphEventRef LastClose;
	FGraphEventRef CompletionEvent;

	void EnqueueDispatchTask(FGraphEventArray&& Prereqs, FTaskPipe::FFunc&& Lambda);
	void EnqueueSubmitTask  (FGraphEventArray&& Prereqs, FTaskPipe::FFunc&& Lambda);
...
UE_DEPRECATED(5.8, "RHICreateTexture with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIAsyncReallocateTexture2D with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHILockTexture2DArray with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIUnlockTexture2DArray with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHILockTextureCubeFace with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIUnlockTextureCubeFace with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIUpdateTexture2D with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIBeginUpdateTexture3D with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIEndUpdateTexture3D with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIEndMultiUpdateTexture3D with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIUpdateTexture3D with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.9, "RHIFlushResources has been deprecated, use ImmediateFlush or SubmitAndBlockUntilGPUIdle(true) instead to block")
inline void RHIFlushResources()
{
	FRHICommandListExecutor::GetImmediateCommandList().ImmediateFlush(EImmediateFlushType::FlushRHIThreadFlushResources);
}

UE_DEPRECATED(5.8, "RHIVirtualTextureSetFirstMipInMemory with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIVirtualTextureSetFirstMipVisible with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIGetNativePhysicalDevice is deprecated. Use GetIVulkanDynamicRHI()->RHIGetVkPhysicalDevice().")
...
UE_DEPRECATED(5.8, "RHIGetNativeGraphicsQueue is deprecated. Use platform-specific RHI interfaces instead.")
...
UE_DEPRECATED(5.8, "RHIGetNativeComputeQueue is deprecated. Use platform-specific RHI interfaces instead.")
...
UE_DEPRECATED(5.8, "RHIGetNativeInstance is deprecated. The only useful implementation was in Vulkan RHI. Use GetIVulkanDynamicRHI()->RHIGetVkInstance() instead.")
...
UE_DEPRECATED(5.8, "RHILockStagingBuffer with an implied immediate command list is deprecated")
...
UE_DEPRECATED(5.8, "RHIUnlockStagingBuffer with an implied immediate command list is deprecated")
