...
void FRHICommandDispatchComputeShaderBundle::Execute(FRHICommandListBase& CmdList)
{
	...
	for (int32 DispatchIndex = 0; DispatchIndex < Dispatches.Num(); ++DispatchIndex)
	{
		...
		{
			// Must run on fallback-PSO too: pairs Verify_DecUse with GetComputePipelineState's Verify_IncUse.
			Dispatch.RHIPipeline = ExecuteSetComputePipelineState(Dispatch.PipelineState);
		}
	}
	INTERNAL_DECORATOR_COMPUTE(RHIDispatchComputeShaderBundle)(ShaderBundle, RecordArgBuffer, SharedBindlessParameters, Dispatches, Method);
}
...
void FRHICommandDispatchGraphicsShaderBundle::Execute(FRHICommandListBase& CmdList)
{
	...
	INTERNAL_DECORATOR_COMPUTE(RHIDispatchGraphicsShaderBundle)(ShaderBundle, RecordArgBuffer, BundleState, SharedBindlessParameters, Dispatches, Method);
}
...
void FRHICommandSetUniformBlockOffset::Execute(FRHICommandListBase& CmdList)
{
	RHISTAT(SetUniformBlockOffset);
	INTERNAL_DECORATOR(RHISetUniformBlockOffset)(SRV, Offset);
}
