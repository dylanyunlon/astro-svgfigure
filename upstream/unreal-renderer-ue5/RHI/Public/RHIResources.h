...
struct FShaderResourceTable
{
	...
	/** Mapping of bound uniform blocks to their location in resource tables. The bind target stored here is the cbuffer's BufferIndex. */
	TArray<uint32> UniformBlockMap;
	...
	friend bool operator == (const FShaderResourceTable& A, const FShaderResourceTable& B)
	{
		...
		bEqual &= (A.UniformBlockMap          .Num() == B.UniformBlockMap          .Num());
		...
		bEqual &= (FMemory::Memcmp(A.UniformBlockMap          .GetData(), B.UniformBlockMap          .GetData(), A.UniformBlockMap          .GetTypeSize() * A.UniformBlockMap          .Num()) == 0);
		return bEqual;
	}
...
struct FRHICreateShaderDesc
{
	...
	// Optional Owner of the memory backing the View
	TSharedPtr<void> CodeOwner;
	// Optional pointer to the shader library this shader is being created from
	class FRHIShaderLibrary* Library;

	FRHICreateShaderDesc(TConstArrayView<uint8> InCode, class FRHIShaderLibrary* InLibrary = nullptr)
	: Code(InCode)
	, Library(InLibrary)
	{
	}

	template<typename DeleterType, typename = std::enable_if_t<!std::is_pointer_v<std::decay_t<DeleterType>>>>
	FRHICreateShaderDesc(TConstArrayView<uint8> InCode, DeleterType&& InDeleter, class FRHIShaderLibrary* InLibrary = nullptr)
	 : Code(InCode)
	...
	// - Make the Owner valid to differentiate between data that stay valid as long as we keep a reference to it versus temporary data
	 , CodeOwner(MakeShareable((void*)InCode.GetData(), Forward<DeleterType>(InDeleter)))
	 , Library(InLibrary)
	{
	}
};
...
{
public:
	void SetHash(const FShaderHash& InHash) { Hash = InHash; }
	const FShaderHash& GetHash() const { return Hash; }
...
#if PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS
		, bHasSpecializationConstants(false)
#endif
	{
	}
...
#if PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS
	inline void SetSpecializationConstants(bool bValue)
	{
		bHasSpecializationConstants = bValue;
	}

	inline bool HasSpecializationConstants() const
	{
		return bHasSpecializationConstants;
	}

	const TMap<uint32, TArray<uint32>>& GetSpecializationExclusions() const
	{
		return SpecializationExclusions;
	}

	void SetSpecializationExclusions(TMap<uint32, TArray<uint32>>&& Exclusions)
	{
		SpecializationExclusions = Exclusions;
	}

	bool IsExcludedFromSpecialization(uint32 BindPoint, uint32 SpecializationHash) const
	{
		if (bHasSpecializationConstants)
		{
			if (const TArray<uint32>* Array = SpecializationExclusions.Find(BindPoint))
			{
				return Array->Find(SpecializationHash) != INDEX_NONE;
			}
		}
		return false;
	}
#else
	constexpr bool HasSpecializationConstants() const { return false; }
#endif // PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS

private:
#if PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS
	TMap<uint32, TArray<uint32>> SpecializationExclusions;
#endif // PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS
	FShaderHash Hash;
...
#if PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS
	uint8 bHasSpecializationConstants : 1;
#endif // PLATFORM_SUPPORTS_SPECIALIZATION_CONSTANTS
#if PLATFORM_WINDOWS
...
protected:
	...
	RHI_API virtual ~FRHIUniformBuffer();
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
namespace UE::RHI::UniformBufferContentMap
{
	RHI_API void Update(const FRHIUniformBuffer* InUniformBuffer, const void* Contents);
	RHI_API void Remove(const FRHIUniformBuffer* InUniformBuffer);
	RHI_API const void* Get(const FRHIUniformBuffer* InUniformBuffer);
}
#endif
...
public:
	...
	static constexpr double EmptyLastRenderTime = -FLT_MAX;
...
private:
	...
	std::atomic<double> LastRenderTime = EmptyLastRenderTime;
};
...
public:
	...
	virtual ~FRHIRenderQueryPool() = default;
...
struct FRHIPresentArgs
{
	FRHIPresentArgs(uint64 InFrameCounter, bool bInPresent, bool bInLockToVsync):
		...
		bPresent((uint8)bInPresent),
		bLockToVsync((uint8)bInLockToVsync)
	{ }


	/** Frame index that is going to Presented. */
	uint64 FrameCounter;

	/** true if the Engine should call Present the frame to the display */
	uint8 bPresent:1;

	/** true if the Engine should sync the frame presentation to the display vsync*/
	uint8 bLockToVsync:1;
};
...
public:
	...
	/**
	 * Creates a platform-level native slate rendering layer for handling rendering order with platform-native views.
	 * This means that everything on top of the specified layer will be rendered to a different platform-level view than 
	 * everything on the bottom of the specified layer.
	 * Right now this is only implemented in MetalRHI for macOS NSView interop with Slate.
	 */
	virtual void CreateNativeLayer(int32 NewNativeLayer, void* NativeViewHandle, const TArray<TInterval<int32>>& LayerRanges) {}
	
	/**
	 * Deletes a platform-level native layer created by CreateNativeLayer.
	 * Right now this is only implemented in MetalRHI for macOS NSView interop with Slate.
	 */
	virtual void DeleteNativeLayer(int32 OldNativeLayer) {}
	
	/**
	 * Sets the default native layer for items to render into. If the WITH_MULTI_VIEW_SLATE_WINDOW_SUPPORT
	 * compile flag is false this function will have no effect. 
	 */
	virtual void SetDefaultNativeLayer(int32 NativeLayer) {}

	/**
	* Some platforms can specify a Crop rectangle for the Viewport
	*/
	virtual void SetWindowCropSize( const FIntRect& NewCropWindow ) {}
	 ...
	 */
	 UE_DEPRECATED(5.8, "FRHIViewport::Tick is deprecated and unused.")
	...
	void SetPresentLatencyCallbacks(TFunction<void()>&& InBeginCallback, TFunction<void()>&& InEndCallback)
	{
		PresentBeginLatencyCallback = MoveTemp(InBeginCallback);
		PresentEndLatencyCallback = MoveTemp(InEndCallback);
	}

protected:
	TFunction<void()> PresentBeginLatencyCallback;
	TFunction<void()> PresentEndLatencyCallback;
};
...
enum ERayTracingGeometryType
{
	...
	// Linear Swept Sphere (LSS) is a ray tracing primitive that represents the volume traced by a sphere moving
	// linearly between two endpoints, optionally with varying radii. It efficiently models strand-like geometry
	// such as hair or fur without tessellating into triangles, offering smoother appearance, lower memory use,
	// and hardware-accelerated ray intersections.
...
public:
	...
	// Stride in bytes of the index buffer (2 for 16-bit indices, 4 for 32-bit).
	...
	uint32 GetIndexBufferStride() const
	{
		...
		{
			ensureMsgf(false, 
				TEXT("FRayTracingGeometryInitializer::IndexBufferStride not set; falling back to IndexBuffer->GetStride(). ")
				TEXT("Set IndexBufferStride when building the initializer to avoid a data race with index buffer streaming."));
			return IndexBuffer->GetStride();
		}
...
inline bool operator==(const FRayTracingGeometryInitializer& LHS, const FRayTracingGeometryInitializer& RHS)
{
	...
	if (LHS.IndexBuffer != RHS.IndexBuffer
		...
		|| LHS.IndexBufferStride != RHS.IndexBufferStride
...
protected:
	FRHIRayTracingAccelerationStructure(bool bIsRayTracingScene)
		: FRHIResource(bIsRayTracingScene ? RRT_RayTracingScene : RRT_RayTracingGeometry)
	{}

	FRayTracingAccelerationStructureSize SizeInfo = {};
};

using FRayTracingAccelerationStructureAddress UE_DEPRECATED(5.7, "FRayTracingAccelerationStructureAddress is deprecated and should no longer be used.") = uint64;
...
public:
	FRHIRayTracingGeometry()
		: FRHIRayTracingAccelerationStructure(false)
	{}
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(5.7, "Use RHICmdList.WriteResourceGPUAddresses(...) instead.")
...
protected:
	...
	void ValidateIndexBufferStride() const
	{
#if DO_CHECK
		if (Initializer.IndexBuffer && Initializer.IndexBufferStride != 0)
		{
			ensureMsgf(Initializer.IndexBufferStride == Initializer.IndexBuffer->GetStride(),
				TEXT("FRayTracingGeometryInitializer::IndexBufferStride (%u) does not match the live index buffer ")
				TEXT("stride (%u) at build time - the captured stride is stale or was set incorrectly by the producer."),
				Initializer.IndexBufferStride, Initializer.IndexBuffer->GetStride());
		}
#endif
	}
...
public:
	FRHIRayTracingScene()
		: FRHIRayTracingAccelerationStructure(true)
	{}

	virtual const FRayTracingSceneInitializer& GetInitializer() const = 0;

protected:
	RHI_API void SetReferencedResources(TConstArrayView<FRHIRayTracingGeometry*> InReferencedGeometries, TConstArrayView<FRHIBuffer*> InReferencedBuffers);
	...
	// Unique list of buffers referenced by all instances in this scene.
	// Any referenced buffer is kept alive while the scene is alive.
	TArray<TRefCountPtr<FRHIBuffer>> ReferencedBuffers;
};
...
public:
	...
	virtual ~FRHIStagingBuffer() = default;
...
public:
	...
	~FGenericRHIStagingBuffer() = default;
...
public:
	...
	virtual ~FRHICustomPresent() = default; // should release any references to D3D resources.
	...
	virtual bool Present(FRHIViewport* Viewport, IRHICommandContext& RHICmdContext, int32& InOutSyncInterval) { check(false); return true; };
	...
	// true, then InOutSyncInterval could be modified to switch between VSync/NoVSync for the normal 
	// Present.  Must match value previously returned by NeedsNativePresent for this frame.
	virtual bool PresentOnSubmissionThread(int32& InOutSyncInterval) { check(false); return true; };

	// Called after native Present has been called
...
struct FBoundShaderStateInput
{
	...
	FSpecializationContainerType MeshShaderSpecializationConstants;
	FSpecializationContainerType AmplificationShaderSpecializationConstants;
#else
	...
	FSpecializationContainerType GeometryShaderSpecializationConstants;
#else
	...
	FSpecializationContainerType VertexShaderSpecializationConstants;
	FSpecializationContainerType PixelShaderSpecializationConstants;
private:
#if PLATFORM_SUPPORTS_MESH_SHADERS
...
public:
	...
	FComputePipelineStateInitializer(FRHIComputeShader* InComputeShader)
		: ComputeShader(InComputeShader)
		, Flags(0)
	{
		checkf(!InComputeShader || !InComputeShader->HasSpecializationConstants(), TEXT("Shader requires specialization constants to be passed in using FComputePipelineStateInitializer."));
	}

	FComputePipelineStateInitializer(
		FRHIComputeShader* InComputeShader,
		const FSpecializationContainerType& InSpecializationConstants)
		: ComputeShader(InComputeShader)
		, SpecializationConstants(InSpecializationConstants)
		, Flags(0)
	{
	}
	...
	FSpecializationContainerType SpecializationConstants;
...
inline uint64 ComputeShaderTableHash(const TArrayView<TShaderType*>& ShaderTable, uint64 InitialHash = 5699878132332235837ull)
{
	...
	for (FRHIShader* ShaderRHI : ShaderTable)
	{
		...
		if (ShaderRHI)
		{
			ShaderHash = ShaderRHI->GetHash().Hash;
		}
...
public:
	...
	{
		uint32 Hash = GetTypeHash(Initializer.NameHash);
		Hash = HashCombine(Hash, GetTypeHash(Initializer.NameTableHash));
		Hash = HashCombine(Hash, GetTypeHash(Initializer.ShaderTableHash));
		Hash = HashCombine(Hash, GetTypeHash(Initializer.GraphicsPSOTableHash));
		return Hash;
	}
...
public:
	...
	bool bAllowOpacityMicromaps = false;

	// NOTE: GetTypeHash(const FRayTracingPipelineStateSignature& Initializer) should also be updated when changing this function
	bool operator==(const FRayTracingPipelineStateSignature& rhs) const
	{
		return MaxAttributeSizeInBytes == rhs.MaxAttributeSizeInBytes
			...
			&& bAllowOpacityMicromaps == rhs.bAllowOpacityMicromaps
	...
	// NOTE: operator==(const FRayTracingPipelineStateSignature& rhs) should also be updated when changing this function
	friend uint32 GetTypeHash(const FRayTracingPipelineStateSignature& Initializer)
	{
		check(Initializer.MaxAttributeSizeInBytes < UINT8_MAX);
		check(Initializer.MaxPayloadSizeInBytes < UINT16_MAX);
		const uint32 PackedFields = Initializer.MaxAttributeSizeInBytes | (Initializer.MaxPayloadSizeInBytes << 8) | (Initializer.bAllowOpacityMicromaps ? (1u << 31) : 0u);
		
		uint32 Hash = GetTypeHash(PackedFields);
		Hash = HashCombine(Hash, GetTypeHash(Initializer.GetRayGenHash()));
		Hash = HashCombine(Hash, GetTypeHash(Initializer.GetRayMissHash()));
		Hash = HashCombine(Hash, GetTypeHash(Initializer.GetHitGroupHash()));
		Hash = HashCombine(Hash, GetTypeHash(Initializer.GetCallableHash()));
		return Hash;
	}
