...
public:
	...
	virtual ~FValidationComputeContext() = default;
	...
	virtual void SetExecutingCommandList(FRHICommandListBase* InCmdList) override final
	{
		...
		Tracker->SetExecutingCmdList(InCmdList);
	}
	...
	virtual void RHIDispatchComputeShaderBundle(
		...
		ERHIShaderBundleMethod Method) final override
	{
		if (!GRHIGlobals.ShaderBundles.RequiresSharedBindlessParameters)
		{
			RHI_VALIDATION_CONTEXT_CHECK(SharedBindlessParameters.Num() == 0, this, TEXT("SharedBindlessParameters should not be set on this platform and configuration"));
		}
		
		RHI_VALIDATION_CONTEXT_CHECK(Dispatches.Num() > 0, this, TEXT("A shader bundle must be dispatched with at least one record."));
		...
		for (const FRHIShaderBundleComputeDispatch& Dispatch : Dispatches)
		{
			...
			if (Method == ERHIShaderBundleMethod::Emulation)
			{
		...
		RHIContext->RHIDispatchComputeShaderBundle(ShaderBundleRHI, RecordArgBuffer, SharedBindlessParameters, Dispatches, Method);
	}
		...
		ERHIShaderBundleMethod Method) final override
	{
		...
		Tracker->Assert(RecordArgBuffer->GetWholeResourceIdentity(), ERHIAccess::IndirectArgs);
#endif
		RHIContext->RHIDispatchGraphicsShaderBundle(ShaderBundleRHI, RecordArgBuffer, BundleState, SharedBindlessParameters, Dispatches, Method);
	}
	...
	virtual void RHISetShaderParameters(FRHIComputeShader* Shader, TConstArrayView<uint8> InParametersData, TConstArrayView<FRHIShaderParameter> InParameters, TConstArrayView<FRHIShaderParameterResource> InResourceParameters, TConstArrayView<FRHIShaderParameterResource> InBindlessParameters) final override
	{
		...
		{
			RHI_VALIDATION_CONTEXT_CHECK(false, this, TEXT("A compute PSO has to be set to set resources into a shader!"));
			return;
		}

		if (Shader != State.BoundShader)
		{
			RHI_VALIDATION_CONTEXT_CHECK(false, this, *FString::Printf(TEXT("Invalid attempt to set parameters for compute shader '%s' while the currently bound shader is '%s'"), Shader->GetShaderName(), State.BoundShader->GetShaderName()));
			return;
		}
...
public:
		...
		ERHIShaderBundleMethod Method) final override
	{
		...
		// TODO
#if 0
		RHI_VALIDATION_CONTEXT_CHECK(Dispatches.Num() > 0, this, TEXT("SharedBindlessParameters should not be set on this platform and configuration"));
	...
	PRAGMA_SUPPRESS_NEXT_DEPRECATION_WARNING virtual void* RHIGetNativeCommandBuffer() override final
	{
	...
	virtual void RHISetShaderParameters(FRHIGraphicsShader* Shader, TConstArrayView<uint8> InParametersData, TConstArrayView<FRHIShaderParameter> InParameters, TConstArrayView<FRHIShaderParameterResource> InResourceParameters, TConstArrayView<FRHIShaderParameterResource> InBindlessParameters) final override
	{
		if (!State.bGfxPSOSet)
		{
			RHI_VALIDATION_CONTEXT_CHECK(false, this, TEXT("A graphics PSO has to be set to set resources into a shader!"));
			return;
		}
	...
	virtual void RHISetShaderParameters(FRHIComputeShader* Shader, TConstArrayView<uint8> InParametersData, TConstArrayView<FRHIShaderParameter> InParameters, TConstArrayView<FRHIShaderParameterResource> InResourceParameters, TConstArrayView<FRHIShaderParameterResource> InBindlessParameters) final override
	{
		...
		{
			RHI_VALIDATION_CONTEXT_CHECK(false, this, *FString::Printf(TEXT("Invalid attempt to set parameters for compute shader '%s' while the currently bound shader is '%s'"), Shader->GetShaderName(), State.BoundShaders[SF_Compute]->GetShaderName()));
			return;
		}
	...
	virtual void RHISetUniformBlockOffset(FRHIShaderResourceView* SRV, uint32 Offset) override final
	{
		RHIContext->RHISetUniformBlockOffset(SRV, Offset);
	}
	...
	*/
	...
	void ValidateBeginRenderPass(const FRHIRenderPassInfo& InInfo, const TCHAR* InName)
	{
	...
	{
		ValidateBeginRenderPass(InInfo, InName);
