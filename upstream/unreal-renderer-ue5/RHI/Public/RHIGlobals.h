...
struct FRHIGlobals
{
	...
	/** Whether RHI supports anisotropic filtering for 3D textures. */
	bool SupportsTexture3DAnisotropicFiltering = true;
	...
	struct FRayTracing
	{
		...
		*/
		...
		/** Whether or not the RHI supports indirect building of ray tracing top level acceleration structures. */
		bool SupportsBuildTopLevelAccelerationStructureIndirect = false;
		...
		/** Whether or not the RHI supports a separate hit group buffer when building the acceleration structure */
		bool RequiresSeparateHitGroupContributionsBuffer = false;

		/** Whether or not the RHI supports linear swept sphere primitives **/
		bool SupportsLinearSweptSpheres = false;

		/** Whether or not the RHI supports opacity micromaps **/
		bool SupportsOpacityMicromaps = false;
	...
	/** Whether or not the current device has a Unified Memory Architecture (UMA) - Memory Shared across GPU and CPU but separate Virtual Addresses*/
	bool DeviceIsUMASeparateVA = false;

	/** Whether or not the current device UMA is Cache Coherent between the GPU and the CPU */
	bool DeviceIsCacheCoherentUMA = false;

	/** Whether or not the current device supports GPU Uploads Heaps (ReBar) */
	bool DeviceSupportsGPUUploadHeaps = false;
	...
	***/
	...
	/** 
	  * If true, RHI submit tasks (those that call RHISubmitCommandLists) are allowed to run concurrently with subsequent RHI translation tasks.
	  * This is only possible with RHIs that correctly pipeline viewport presentation, and don't implicitly submit GPU work from within RHI translate tasks.
	  * 
	  * If false, future RHI translate tasks cannot start until all prior RHI submit tasks have completed. Submit tasks run in isolation on the RHI thread.
	  */
	bool SupportsConcurrentTranslateAndSubmit = false;
	...
	struct
	{
		/** Enables support for platforms that use render passes to split command list translation across multiple threads. For platforms that require
		   ...
		   parent, should use ParentWaitForChildTranslate and/or ChildWaitForParentBegin */
		bool Supported = false;

		/** When true, signals to the renderer that the platform RHI requires a call to RHIGetParallelCommandContext to retrieve RHI contexts for use with parallel render passes.
		    Otherwise, regular graphics contexts are used. */
		bool UsesCustomContexts = false;

		/** When true, indicates that the platform RHI requires that the child command list translations in a parallel render pass are completed before the remainder of the parent command list can be translated. */
		bool ParentWaitForChildTranslate = false;

		/** When true, indicates that the platform RHI requires that child command list translations in a parallel render pass do not start until the parent command list translation has reached the BeginParallelRenderPass command. */
		bool ChildWaitForParentBegin = false;

	} ParallelRenderPasses;
	...
	struct FVariableRateShading
	{
		...
		/** Whether or not the RHI can support offsets for image-based Variable Rate Shading. Currently requires VK_EXT_fragment_density_map_offset to be supported and enabled. */
		bool SupportsImageOffsets = false;
		
		/** Current offsets of the shading rate image, in pixels. NOTE: This MUST be quantized by the amount specified in ImageOffsetGranularity below! */
		FIntPoint ImageOffsets[2];
		
		/** The granularity of the offset, in pixels, required by the RHI. This is queried at runtime, and must be used to enforce granularity for the ImageOffsets above. */
		FIntPoint ImageOffsetGranularity;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	/** Whether dynamic (bindless) resources are supported. Can be set even if bindless is not enabled. */
	bool bSupportsBindless = false;

	/** Whether bindless descriptor ranges are supported. Can be set even if bindless is not enabled. */
	bool bSupportsDescriptorRange = false;

	/** Array of descriptor type masks showing which descriptor types can be allocated together. */
	TArray<ERHIDescriptorTypeMask> DescriptorTypeGroups;
	...
	struct FReservedResources
	{
		...
		*/
		...
		/** Current size of the virtual address space allocated for all reserved buffers and textures */
		volatile int64 VirtualSize = 0;
	...
	/** True if RHI expects RDG to produce dedicated SRVGraphicsPixel/SRVGraphicsNonPixel access for texture srv parameters (instead of using the SRVGraphics mask everywhere). */
	bool SupportsDedicatedSRVGraphicsNonPixelAccess = false;

	/** True if the platform supports split barriers. */
	bool SupportsSplitBarriers = false;
...
#define GRHIDeviceIsUMASeparateVA                              GRHIGlobals.DeviceIsUMASeparateVA
#define GRHIDeviceIsCacheCoherentUMA                           GRHIGlobals.DeviceIsCacheCoherentUMA
#define GRHIDeviceSupportsGPUUploadHeaps                       GRHIGlobals.DeviceSupportsGPUUploadHeaps
...
#define GRHISupportsVariableRateShadingImageOffsets			   GRHIGlobals.VariableRateShading.SupportsImageOffsets
#define GRHIVariableRateShadingImageOffsets					   GRHIGlobals.VariableRateShading.ImageOffsets
