...
inline void EnumerateUniformBlocks(const FRHIUniformBuffer* RESTRICT Buffer, int32 BufferIndex, const uint32* RESTRICT ResourceMap, TCallback && Callback)
{
	const TRefCountPtr<FRHIResource>* RESTRICT Resources = Buffer->GetResourceTable().GetData();
	...
	if (BufferOffset > 0)
	{
		...
		do
		{
			...
			FRHIShaderResourceView* SRV = static_cast<FRHIShaderResourceView*>(Resources[ResourceIndex].GetReference());

			Callback(SRV, BindIndex);
...
// Helper to call the respective setters for each FRHIResource contained inside the provided uniform buffer.
template<typename TBinder>
void SetAllUniformBufferResourcesForBindless(TBinder&& Binder, const FRHIUniformBuffer* Buffer)
{
	double CurrentTimeForTextureTimes = FAppTime::GetApproximateCurrentTime();

	for (const TRefCountPtr<FRHIResource>& Resource : Buffer->GetResourceTable())
	{
		if (Resource)
		{
			const ERHIResourceType ResourceType = Resource->GetType();
			switch (ResourceType)
			{
			case RRT_Texture:
				static_cast<FRHITexture*>(Resource.GetReference())->SetLastRenderTime(CurrentTimeForTextureTimes);
				Binder.SetTexture(static_cast<FRHITexture*>(Resource.GetReference()), 0);
				break;
			case RRT_ShaderResourceView:
				Binder.SetSRV(static_cast<FRHIShaderResourceView*>(Resource.GetReference()), 0);
				break;
			case RRT_UnorderedAccessView:
				Binder.SetUAV(static_cast<FRHIUnorderedAccessView*>(Resource.GetReference()), 0);
				break;
			case RRT_ResourceCollection:
				Binder.SetResourceCollection(static_cast<FRHIResourceCollection*>(Resource.GetReference()), 0);
				break;
			case RRT_SamplerState:
				Binder.SetSampler(static_cast<FRHISamplerState*>(Resource.GetReference()), 0);
				break;
			}
		}
	}
                                         ...
                                         )
{
	...
	while (DirtyBits)
	{
		...
		if (!Buffer)
		{
			UE_LOGF(LogRHI, Fatal, "Shader expected a uniform buffer at slot %u but got null instead (Shader='%ls' UB='%ls'). Rendering code needs to set a valid uniform buffer for this slot."
			       , BufferIndex
		...
		else if (Buffer->GetLayout().GetHash() != SRT.ResourceTableLayoutHashes[BufferIndex])
		{
			...
			UE_LOGF(LogRHI, Fatal,
			       "Uniform buffer bound to slot %u is not what the shader expected:\n" "\tBound                : Uniform Buffer[%ls] with Hash[0x%08x]\n" "\tExpected             : Uniform Buffer[%ls] with Hash[0x%08x]\n" "\tShader Name          : %ls\n" "\tLayout CB Size       : %d\n" "\tLayout Num Resources : %d\n" "\tResource Types       : %ls\n"
			       , BufferIndex
		...
		// Uniform Block bindings (UBO)
		EnumerateUniformBlocks(Buffer, BufferIndex, SRT.UniformBlockMap.GetData(),
			[&](FRHIShaderResourceView* SRV, uint8 BindIndex)
			{
				if constexpr (requires { Binder.SetUniformBlock(SRV, BindIndex); })
				{
					Binder.SetUniformBlock(SRV, BindIndex);
				}
			});
	}
