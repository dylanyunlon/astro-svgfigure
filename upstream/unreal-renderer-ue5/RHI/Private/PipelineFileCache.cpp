...
static FString JOURNAL_FILE_EXTENSION(".jnl");
...
enum class EPipelineCacheFileFormatVersions : uint32
{
	...
	PSOPrecacheResult = 31,
	ChangedMaxVertexElementCount = 32,
};
...
const RHI_API uint32 FPipelineCacheFileFormatCurrentVersion = (uint32)EPipelineCacheFileFormatVersions::ChangedMaxVertexElementCount;
...
static FAutoConsoleVariableRef CVarPSOFileCacheExcludePrecachePSO(
														   ...
														   TEXT("1 excludes saving runtime-precached graphics & compute PSOs in the file cache which were successfully precached, 0 includes all of them (default). Excluding precached PSOs requires PSO precaching to be enabled."),
														   ECVF_ReadOnly
														   );

static int32 GPSOIncludeAllPrecachedPSOsInFileCache = 0;
static FAutoConsoleVariableRef CVarPSOFileCacheIncludeAllPrecachedPSOs(
														   TEXT("r.ShaderPipelineCache.IncludeAllPrecachedPSOs"),
														   GPSOIncludeAllPrecachedPSOsInFileCache,
														   TEXT("1 includes all successfully precached PSOs in the pipeline file cache even if they were never needed for rendering.\n")
														   TEXT("Logged entries carry EPSOPrecacheResult::Active; the result is upgraded to the actual runtime outcome if the PSO is later used at draw time.\n")
														   TEXT("Requires PSO precaching to be enabled. Default: 0 (disabled)."),
														   ECVF_ReadOnly
														   );
...
static FAutoConsoleVariableRef CVarPSOFileCachePrintNewPSODescriptors(
														   ...
														   TEXT("1 prints descriptions for all new PSO entries to the log/console while 0 does not. 2 prints additional details, such as the PSO type (graphics, compute, or raytracing), usage mask and bind count. Defaults to 0."),
														   ECVF_Default
														   );
...
TMap<uint32, FPipelineCacheFileFormatPSO> FPipelineFileCacheManager::NewPSOs;
...
FRHIShader::~FRHIShader()
{
	...
	{
		UE_LOGF(LogRHI, Fatal, "FRHIShader with hash: %ls and Frequency: %d still in use by PSO compilation when being destroyed", *Hash.ToString(), Frequency);
	}
}
	...
	void AddShaders(const FPipelineCacheFileFormatPSO& NewEntry)
	{
		switch (NewEntry.Type)
		{
			...
			case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
			{
				...
				if (NewEntry.GraphicsDesc.VertexShader != FShaderHash())
					Shaders.Add(NewEntry.GraphicsDesc.VertexShader);

				if (NewEntry.GraphicsDesc.FragmentShader != FShaderHash())
					Shaders.Add(NewEntry.GraphicsDesc.FragmentShader);

				if (NewEntry.GraphicsDesc.GeometryShader != FShaderHash())
					Shaders.Add(NewEntry.GraphicsDesc.GeometryShader);

				if (NewEntry.GraphicsDesc.MeshShader != FShaderHash())
					Shaders.Add(NewEntry.GraphicsDesc.MeshShader);

				if (NewEntry.GraphicsDesc.AmplificationShader != FShaderHash())
...
{
	if (VertexShader != FShaderHash())
	{
		OutBuilder << TEXT(" VS:");
		OutBuilder << VertexShader;
	}
	if (MeshShader != FShaderHash())
	{
		OutBuilder << TEXT(" MS:");
		OutBuilder << MeshShader;
	}
	if (GeometryShader != FShaderHash())
	{
		OutBuilder << TEXT(" GS:");
		OutBuilder << GeometryShader;
	}
	if (AmplificationShader != FShaderHash())
	{
		OutBuilder << TEXT(" AS:");
		OutBuilder << AmplificationShader;
	}
	if (FragmentShader != FShaderHash())
	{
		OutBuilder << TEXT(" PS:");
...
{
	return FString("VertexShader,FragmentShader,GeometryShader,MeshShader,AmplificationShader");
}
...
FString FPipelineCacheFileFormatPSO::GraphicsDescriptor::StateToString() const
{
	...
	Result += FString::Printf(TEXT("%d,%d,%lld,")
		...
		, uint64{EnumToUnderlyingType(DepthStencilFlags)}
	);
	...
	for (int32 Index = 0; Index < MaxSimultaneousRenderTargets; Index++)
	{
		Result += FString::Printf(TEXT("%d,%lld,%d,%d,")
			...
			, uint64{EnumToUnderlyingType(RenderTargetFlags[Index])}
			, 0/*Load*/
			, 0/*Store*/
		);
	}
	...
	Result += FString::Printf(TEXT("%d,%d,")
		, uint32(bAllowVariableRateShading)
		, uint32(ShadingRate)
	);
...
void FPipelineCacheFileFormatPSO::GraphicsDescriptor::AddStateToReadableString(TReadableStringBuilder& OutBuilder) const
{
	...
	OutBuilder << TEXT(" AllowVRS:");
	OutBuilder << bAllowVariableRateShading;
	OutBuilder << TEXT(" SR:");
	OutBuilder << uint32(ShadingRate);
	OutBuilder << TEXT("\n");
...
bool FPipelineCacheFileFormatPSO::GraphicsDescriptor::StateFromString(const FStringView& Src)
{
	...
	// parse variable rate shading information
	{
		uint32 LocalAllowVariableRateShading = 0;
		uint32 LocalShadingRate = 0;
		check(PartEnd - PartIt >= 2);
		LexFromString(LocalAllowVariableRateShading, *PartIt++);
		LexFromString(LocalShadingRate, *PartIt++);
		bAllowVariableRateShading = (bool)LocalAllowVariableRateShading;
		ShadingRate = (EVRSShadingRate)LocalShadingRate;
	}
	...
	SortVertexElements(VertexDescriptor);
...
FString FPipelineCacheFileFormatPSO::ToStringReadable() const
{
	...
	Builder << TEXT(" PSOPrecacheResult ");
	Builder << LexToString(PSOPrecacheResult);
#if PSO_COOKONLY_DATA
	Builder << TEXT(" mask ");
	Builder << UsageMask;
	Builder << TEXT(" bindc ");
...
bool FPipelineCacheFileFormatPSO::Verify() const
{
	...
	{
		return ComputeDesc.ComputeShader != FShaderHash();
	}
	else if(Type == DescriptorType::Graphics)
	{
		if (GraphicsDesc.VertexShader == FShaderHash() && GraphicsDesc.MeshShader == FShaderHash())
		{
		...
		if (GraphicsDesc.MeshShader != FShaderHash())
		{
			...
			if (GraphicsDesc.VertexShader != FShaderHash())
			{
	...
	{
		return RayTracingDesc.ShaderHash != FShaderHash() &&
...
/*friend*/ uint32 GetTypeHash(const FPipelineCacheFileFormatPSO &Key)
{
	...
	switch(Key.Type)
	{
		...
		case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
		{
			...
			KeyHash = FCrc::MemCrc32(&Key.GraphicsDesc.bAllowVariableRateShading, sizeof(Key.GraphicsDesc.bAllowVariableRateShading), KeyHash);
			KeyHash = FCrc::MemCrc32(&Key.GraphicsDesc.ShadingRate, sizeof(Key.GraphicsDesc.ShadingRate), KeyHash);
			...
			KeyHash = FCrc::MemCrc32(&Key.GraphicsDesc.RasterizerState.DepthClipMode, sizeof(Key.GraphicsDesc.RasterizerState.DepthClipMode), KeyHash);
...
{
	Ar << Info.Type;
	if (Ar.GameNetVer() >= (uint32)EPipelineCacheFileFormatVersions::PSOPrecacheResult)
	{
		Ar << Info.PSOPrecacheResult;
	}
#if PSO_COOKONLY_DATA
	...
	switch (Info.Type)
	{
		...
		case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
		{
			...
			{
				FShaderHash HullShader;
				Ar << HullShader;
				FShaderHash DomainShader;
				Ar << DomainShader;
			}
			...
			if (Ar.GameNetVer() < (uint32)EPipelineCacheFileFormatVersions::SortedVertexDesc)
			{
				...
				Ar << Elements;
				FPipelineCacheFileFormatPSO::SortVertexElements(Elements);
			...
			if (Ar.GameNetVer() >= (uint32)EPipelineCacheFileFormatVersions::VariableRateShading)
			{
				Ar << Info.GraphicsDesc.bAllowVariableRateShading;
				uint32 ShadingRate = (uint32)Info.GraphicsDesc.ShadingRate;
				Ar << ShadingRate;
				Info.GraphicsDesc.ShadingRate = (EVRSShadingRate)ShadingRate;
			}
...
/*static*/ bool FPipelineCacheFileFormatPSO::Init(FPipelineCacheFileFormatPSO& PSO, FComputePipelineStateInitializer const& Init, EPSOPrecacheResult PSOPrecacheResult)
{
	check(Init.ComputeShader != nullptr);

	PSO.Type = DescriptorType::Compute;
	PSO.PSOPrecacheResult = PSOPrecacheResult;
	...
	PSO.ComputeDesc.ComputeShader = Init.ComputeShader->GetHash();
...
/*static*/ bool FPipelineCacheFileFormatPSO::Init(FPipelineCacheFileFormatPSO& PSO, FGraphicsPipelineStateInitializer const& Init, EPSOPrecacheResult PSOPrecacheResult)
{
	bool bOK = true;
...
#endif
	{
		...
		check (Init.BoundShaderState.VertexDeclarationRHI->IsValid());
		{
			...
			SortVertexElements(PSO.GraphicsDesc.VertexDescriptor);
		}
	...
	PSO.GraphicsDesc.bAllowVariableRateShading = Init.bAllowVariableRateShading;
	PSO.GraphicsDesc.ShadingRate = Init.ShadingRate;
...
{
	VertexElements.Sort([](FVertexElement const& A, FVertexElement const& B)
	{
...
bool FPipelineCacheFileFormatPSO::operator==(const FPipelineCacheFileFormatPSO& Other) const
{
	...
	if (this != &Other)
	{
		...
		// ignore pso precache result for equals op
			...
			switch(Type)
			{
				...
				case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
				{
					...
					bSame &=
						...
						GraphicsDesc.bAllowVariableRateShading == Other.GraphicsDesc.bAllowVariableRateShading && GraphicsDesc.ShadingRate == Other.GraphicsDesc.ShadingRate &&
...
: Type(Other.Type), PSOPrecacheResult(Other.PSOPrecacheResult)
#if PSO_COOKONLY_DATA
...
FPipelineCacheFileFormatPSO& FPipelineCacheFileFormatPSO::operator=(const FPipelineCacheFileFormatPSO& Other)
{
	...
	{
		Type = Other.Type;
		PSOPrecacheResult = Other.PSOPrecacheResult;
#if PSO_COOKONLY_DATA
		UsageMask = Other.UsageMask;
		BindCount = Other.BindCount;
#endif
...
struct FPipelineCacheFileFormatTOC
{
	...
	void DumpToLog() const
	{
		...
		UE_LOGF(LogRHI, VeryVerbose, "Total PSOs %d", MetaData.Num());
	}
...
static bool ShouldDeleteExistingUserCache()
{
	...
	if (!bOnce)
	{
		...
		UE_CLOGF(bCmdLineForce, LogRHI, Warning, "****************************** Deleting user-writable PSO cache as requested on command line");
	}
...
public:
	...
	static bool OpenPipelineFileCache(const FString& FilePath, EShaderPlatform ShaderPlatform, FGuid& Guid, TSharedPtr<IAsyncReadFileHandle, ESPMode::ThreadSafe>& Handle, FPipelineCacheFileFormatTOC& Content, uint64& TOCOffsetOUT)
	{
		...
		if (FileReader)
		{
			...
			*FileReader << Header;
			if (Header.Magic == FPipelineCacheFileFormatMagic && Header.Version == FPipelineCacheFileFormatCurrentVersion && Header.GameVersion == GameVersion && Header.Platform == ShaderPlatform)
			{
				...
				UE_LOGF(LogRHI, Log, "FPipelineCacheFile Header Game Version: %d", Header.GameVersion);
				UE_LOGF(LogRHI, Log, "FPipelineCacheFile Header Engine Data Version: %d", Header.Version);
				UE_LOGF(LogRHI, Log, "FPipelineCacheFile Header TOC Offset: %llu", Header.TableOffset);
				UE_LOGF(LogRHI, Log, "FPipelineCacheFile File Size: %lld Bytes", FileReader->TotalSize());
				...
				if(!bSuccess)
				{
					UE_LOGF(LogRHI, Log, "FPipelineCacheFile: %ls is corrupt reading TOC", *FilePath);
				}
			}
			else
			{
				...
				UE_LOGF(LogRHI, Log, "FPipelineCacheFile: skipping %ls (different %ls%ls%ls%ls)", *FilePath,
			...
			if(bSuccess)
			{
				...
				if(Handle.IsValid())
				{
					UE_LOGF(LogRHI, Log, "Opened FPipelineCacheFile: %ls (GUID: %ls) with %d entries.", *FilePath, *Header.Guid.ToString(), Content.MetaData.Num());
				...
				{
					UE_LOGF(LogRHI, Log, "Failed to create async read file handle to FPipelineCacheFile: %ls (GUID: %ls)", *FilePath, *Header.Guid.ToString());
					bSuccess = false;
				}
			}
		}
		...
		{
			UE_LOGF(LogRHI, Log, "Could not open FPipelineCacheFile: %ls", *FilePath);
		}
	...
	{
		UE_LOGF(LogRHI, Log, "FPipelineCacheFile: GarbageCollectUserCache() Begin");
		ON_SCOPE_EXIT{ UE_LOGF(LogRHI, Log, "FPipelineCacheFile: GarbageCollectUserCache() End"); };

		int32 GCPeriodInDays = CVarPSOFileCacheUserCacheUnusedElementCheckPeriod.GetValueOnAnyThread();
		if (GCPeriodInDays < 0)
		{
			UE_LOGF(LogRHI, Log, "User cache GC is disabled");
			return;
		}
		...
		if (!FileReader)
		{
			UE_LOGF(LogRHI, Log, "No user cache file found");
			return;
		}
		...
		*FileReader << Header;
		...
		if (!(Header.Magic == FPipelineCacheFileFormatMagic && Header.Version == FPipelineCacheFileFormatCurrentVersion && Header.GameVersion == GameVersion && Header.Platform == ShaderPlatform))
		{
			UE_LOGF(LogRHI, Error, "File has invalid or out of date header");
			return;
		}
		...
		if (UnixTime < NextGCTime)
		{
			...
			UE_LOGF(LogRHI, Log, "Next GC on user cache is in %0.3f days.", DaysToNextGC);
			return;
		}
		...
		if (Header.TableOffset < (uint64)FileReader->TotalSize())
		{
			...
			*FileReader << Content;
			...
			{
				UE_LOGF(LogRHI, Log, "Failed to read TOC");
				return;
			}
		}
		...
		{
			UE_LOGF(LogRHI, Log, "No out of date entries.");
			return;
		}
		...
		if (NumOutOfDateEntries == Content.MetaData.Num())
		{
			...
			UE_LOGF(LogRHI, Log, "All entries are out of date, recreating cache.");
			return;
		}

		UE_LOGF(LogRHI, Log, "%d/%d elements are out of date, performing GC.", NumOutOfDateEntries, Content.MetaData.Num());
		...
		UE_LOGF(LogRHI, Log, "Deleting existing cache file");
		...
		if (!FileWriter)
		{
			UE_LOGF(LogRHI, Log, "Unable to open new cache file for writing");
			return;
		}
		...
		FileWriter->Close();
		delete FileWriter;
		UE_LOGF(LogRHI, Log, "Rewrote cache file. Old Size %lld, new size %lld (%lld byte reduction)", OriginalSize, NewSize, OriginalSize - NewSize);
	}
	...
	bool OpenPipelineFileCache(FString const& NameIn, EShaderPlatform Platform, FGuid& OutGameFileGuid)
	{
		...
		{
			UE_LOGF(LogRHI, Log, "Lazy loading from the shader code library is enabled.");
        ...
        for (auto const& Entry : TOC.MetaData)
        {
			...
			UE_CLOGF(!!FPipelineFileCacheManager::NewPSOUsage.Find(Entry.Key), LogRHI, Warning, "loaded PSOFC %ls contains entry (%u) previously marked as new ", *Name, (Entry.Key));
        ...
        {
        	UE_LOGF(LogRHI, Warning, "Found %d / %d PSO entries marked as invalid.", InvalidEntryCount, TOC.MetaData.Num());
        }
#endif
		
		INC_MEMORY_STAT_BY(STAT_FileCacheMemory, TOC.MetaData.GetAllocatedSize());

		UE_LOGF(LogRHI, VeryVerbose, "-- opened bundled %ls cache:", *NameIn);
		TOC.DumpToLog();
		UE_LOGF(LogRHI, VeryVerbose, "-- end of dump (%ls)", *NameIn);
	...
	bool OpenUserPipelineFileCache(FString const& CacheNameIn, EShaderPlatform Platform)
	{
		...
		if (CacheStatus == EStatus::Unknown)
		{
			...
			UE_LOGF(LogRHI, Log, "Base name for record PSOs is %ls", *RecordingFilename);
			...
			{
				UE_LOGF(LogRHI, Log, "Deleting FPipelineCacheFile: %ls", *FilePath);
	...
	bool SavePipelineFileCache(FPipelineFileCacheManager::SaveMode Mode, TMap<uint32, FPipelineStateStats*> const& Stats, TMap<uint32, FPipelineCacheFileFormatPSO>& NewEntries, FPipelineFileCacheManager::PSOOrder Order, TMap<uint32, FPSOUsageData>& NewPSOUsage)
	{
						...
						if(PipelineFile)
						{
							...
							if (FileSize > 0)
							{
								...
								else
								{
									...
									UE_CLOGF(!bReadOK, LogRHI, Warning, "Failed to read %lld bytes from %ls while re-saving the PipelineFileCache!", FileSize, *PipelineFile->FilePath);
								}
				...
				if (FileWriter)
				{
                    ...
                    switch (Mode)
                    {
                        ...
                        case FPipelineFileCacheManager::SaveMode::Incremental:
                        {
                            ...
                            {
								FPipelineCacheFileFormatPSO& NewEntry = It->Value;

								uint32 PSOHash = It->Key;
								...
								if (bFound)
								{
									...
									UE_LOGF(LogRHI, Display, "Incrementally saving new PSOs but entry (%u), is already cached..", PSOHash);
								...
								if (Stat && Stat->TotalBindCount > 0)
								{
									...
									It.RemoveCurrent();
									UE_LOGF(LogRHI, VeryVerbose, "Incremental save is appending new PSOs (%u)", PSOHash);
								}
                            }
                        ...
                        case FPipelineFileCacheManager::SaveMode::BoundPSOsOnly:
                        {
                            ...
                            {
                                FPipelineCacheFileFormatPSOMetaData Meta;
                                Meta.Stats.PSOHash = Entry.Key;
                                Meta.FileOffset = 0;
                                Meta.FileSize = 0;
                                Meta.FileGuid = Header.Guid;
								Meta.AddShaders(Entry.Value);
								check(Meta.FileGuid != FGuid());

                                TempTOC.MetaData.Add(Meta.Stats.PSOHash, Meta);
                                PSOs.Add(Entry.Key, Entry.Value);
                            }
					...
					*FileWriter << Header;
					...
					if (bFileWriteSuccess && bUseMemoryWriter)
					{
						...
						else
						{
							...
							UE_LOGF(LogRHI, Log, "FPipelineFileCacheManager skipping saving empty .upipelinecache (took %6.2fms): %ls.", ThisTimeMS, *SaveFilePath);
							return false;
						}
					}
					...
					if (bFileWriteSuccess)
                    {
						...
						if ((SaveFilePath == TempPath) || IFileManager::Get().Move(*SaveFilePath, *TempPath, true, true, true, true))
						{
							...
							UE_LOGF(LogRHI, Log, "FPipelineFileCacheManager %ls saved %u total, %u new, %u removed, %u cons .upipelinecache (took %6.2fms): %ls.", ModeName, TotalEntries, NumNewEntries, RemovedEntries, ConsolidatedEntries, ThisTimeMS, *SaveFilePath);
						...
						{
							float ThisTimeMS = float(FPlatformTime::Seconds() - StartTime) * 1000.0f;
							UE_LOGF(LogRHI, Error, "Failed to move .upipelinecache from %ls to %ls (took %6.2fms).", *TempPath, *SaveFilePath, ThisTimeMS);
						}
					}
                    else
                    {
						...
						float ThisTimeMS = float(FPlatformTime::Seconds() - StartTime) * 1000.0f;
						UE_LOGF(LogRHI, Error, "Failed to write .upipelinecache, (took %6.2fms): %ls.", ThisTimeMS, *SaveFilePath);
                    }
		 
				}
                else
                {
                    UE_LOGF(LogRHI, Error, "Failed to open .upipelinecache for write: %ls.", *SaveFilePath);
                }
			}
		}
	...
	void FetchPSODescriptors(TDoubleLinkedList<FPipelineCacheFileFormatPSORead*>& Batch)
	{
		for (TDoubleLinkedList<FPipelineCacheFileFormatPSORead*>::TIterator It(Batch.GetHead()); It; ++It)
		{
			...
			if((Meta.EngineFlags & FPipelineCacheFlagInvalidPSO) != 0)
			{
				...
				UE_LOGF(LogRHI, Verbose, "Encountered a PSO entry %u marked invalid - ignoring", Entry->Hash);
				Entry->bValid = false;
				continue;
			}
			...
			if (Meta.FileGuid == FileGuid)
			{
				...
				{
					UE_LOGF(LogRHI, Verbose, "Encountered a PSO entry %u that has been removed from the cache file: %ls ", Entry->Hash, *Meta.FileGuid.ToString());
					Entry->bValid = false;
					continue;
				}
			}
            ...
            {
                UE_LOGF(LogRHI, Warning, "Encountered a PSO entry %u that references unknown file ID: %ls", Entry->Hash, *Meta.FileGuid.ToString());
                Entry->bValid = false;
                continue;
            }
...
bool FPipelineFileCacheManager::IsPipelineFileCacheEnabled()
{
	...
	if (!bOnce)
	{
		...
		UE_CLOGF(bCmdLineForce, LogRHI, Warning, "****************************** Forcing PSO cache from command line");
	}
...
bool FPipelineFileCacheManager::LogPSOtoFileCache()
{
	...
	if (!bOnce)
	{
		...
		UE_CLOGF(bCmdLineForce, LogRHI, Warning, "****************************** Forcing logging of PSOs from command line");
	}
...
bool FPipelineFileCacheManager::ReportNewPSOs()
{
    ...
    if (!bOnce)
    {
        ...
        UE_CLOGF(bCmdLineForce, LogRHI, Warning, "****************************** Forcing reporting of new PSOs from command line");
    }
...
{
	UE_LOGF(LogTemp, Display, "Clearing the OS Cache");
	...
	if (CVarClearOSPSOFileCache.GetValueOnAnyThread() > 0 && !bCmdLineSkip)
	{
		...
		if(stat(TCHAR_TO_UTF8(*ExecutablePath), &FileInfo) != -1)
		{
			...
			if (stat(TCHAR_TO_UTF8(*Result), &FileInfo) != -1)
			{
				...
				{
					UE_LOGF(LogTemp, Display, "Clearing functions.data");
			...
			if (stat(TCHAR_TO_UTF8(*Result), &FileInfo) != -1)
			{
				...
				{
					UE_LOGF(LogTemp, Display, "Clearing functions.maps");
					unlink(TCHAR_TO_UTF8(*Result));
				}
			}
		}
...
bool FPipelineFileCacheManager::OpenPipelineFileCache(const FString& Key, const FString& CacheName, EShaderPlatform Platform, FGuid& OutGameFileGuid)
{
	...
	if(IsPipelineFileCacheEnabled())
	{
		...
		if(!bFound)
		{
			...
			{
				UE_LOGF(LogRHI, Display, "FPipelineCacheFile[%ls] opened %ls, filename %ls, guid %ls. ", *Key, *CacheName, *NewFileCache->GetCacheFilename(), *NewFileCache->GetFileGuid().ToString());
...
bool FPipelineFileCacheManager::OpenUserPipelineFileCache(const FString& Key, const FString& CacheName, EShaderPlatform Platform, FGuid& OutGameFileGuid)
{
	...
	if (IsPipelineFileCacheEnabled())
	{
		...
		UE_LOGF(LogRHI, Display, "FPipelineCacheFile User cache [key:%ls] opened '%ls'=%d, filename %ls, guid %ls. ", *UserCacheKey, *CacheName, (int)bUserFileOpened, *FileCache->GetCacheFilename(), *FileCache->GetFileGuid().ToString());
...
void FPipelineFileCacheManager::LogNewGraphicsPSOToConsoleAndCSV(FPipelineCacheFileFormatPSO& PSO, uint32 PSOHash, EPSOPrecacheResult PSOPrecacheResult)
{
	...
	// Only print if not completed by PSO precaching
	if (PSOPrecacheResult != EPSOPrecacheResult::Complete)
	{
		CSV_EVENT(PSO, TEXT("Encountered new graphics PSO"));
		UE_LOGF(LogRHI, Display, "Encountered a new graphics PSO: %u (PSOPrecacheState: %ls)", PSOHash, LexToString(PSOPrecacheResult));
		int32 LogDetailLevel = LogPSODetails() ? 2 : GPSOFileCachePrintNewPSODescriptors;
		if (LogDetailLevel > 0)
		{
			UE_LOGF(LogRHI, Display, "New Graphics PSO (%u)", PSOHash);
			if (LogDetailLevel > 1)
			{
				UE_LOGF(LogRHI, Display, "%ls", *PSO.ToStringReadable());
			}
		}
	...
	{
		UE_LOGF(LogRHI, Verbose, "Encountered a new graphics PSO for the file cache but it was already precached at runtime: %u", PSOHash);
	}
}

void FPipelineFileCacheManager::LogNewComputePSOToConsoleAndCSV(FPipelineCacheFileFormatPSO& PSO, uint32 PSOHash, EPSOPrecacheResult PSOPrecacheResult)
{
	if (!LogNewPSOsToConsoleAndCSV)
	{
		return;
	...
	if (PSOPrecacheResult != EPSOPrecacheResult::Complete)
	{
		...
		UE_LOGF(LogRHI, Display, "Encountered a new compute PSO: %u (PSOPrecacheState: %ls)", PSOHash, LexToString(PSOPrecacheResult));
		if (GPSOFileCachePrintNewPSODescriptors > 0)
		{
			UE_LOGF(LogRHI, Display, "New compute PSO (%u) Description: %ls", PSOHash, *PSO.ComputeDesc.ComputeShader.ToString());
		}
	}
	else
	{
		UE_LOGF(LogRHI, Verbose, "Encountered a new compute PSO for the file cache but it was already precached at runtime: %u", PSOHash);
	}
}
...
void FPipelineFileCacheManager::LogNewRaytracingPSOToConsole(FPipelineCacheFileFormatPSO& PSO, uint32 PSOHash, bool bIsNonBlockingPSO)
{
	...
	UE_LOGF(LogRHI, Display, "Encountered a new ray tracing PSO: %u", PSOHash);
	if (GPSOFileCachePrintNewPSODescriptors > 0)
	{
		UE_LOGF(LogRHI, Display, "New ray tracing PSO (%u) Description: %ls", PSOHash, *PSO.RayTracingDesc.ToString());
	}
}
...
void FPipelineFileCacheManager::CacheGraphicsPSO(uint32 RunTimeHash, FGraphicsPipelineStateInitializer const& Initializer, EPSOPrecacheResult PSOPrecacheResult, FPipelineStateStats** OutStats)
{
	if(IsPipelineFileCacheEnabled() && (LogPSOtoFileCache() || ReportNewPSOs()))
	{
		...
		if(PSOUsage == nullptr || !IsReferenceMaskSet(FPipelineFileCacheManager::GameUsageMask, PSOUsage->UsageMask))
		{
			...
			if(PSOUsage == nullptr)
			{
				...
				bool bOK = FPipelineCacheFileFormatPSO::Init(NewEntry, Initializer, PSOPrecacheResult);
				check(bOK);
				...
				{
					bool bActuallyNewPSO = !NewPSOs.Contains(PSOHash);
					if (Initializer.bFromPSOFileCache)
					{
						// FIXME: this is a workaround. Needs proper investigation
						UE_LOGF(LogRHI, Warning, "PSO from the cache was not found in the cache! PSOHash: %u", PSOHash);
					...
					if (bActuallyNewPSO)
					{
						// Only add PSOs which are not fully precached yet if GPSOExcludePrecachePSOsInFileCache is set (includes miss, too late, unknown, NotSupported)
						if (PSOPrecacheResult == EPSOPrecacheResult::Complete)
						{
							bActuallyNewPSO = !GPSOExcludePrecachePSOsInFileCache;
						}
						// Only include precache-only PSOs if r.ShaderPipelineCache.IncludeAllPrecachedPSOs is enabled
						if (PSOPrecacheResult == EPSOPrecacheResult::Active)
						{
							bActuallyNewPSO = GPSOIncludeAllPrecachedPSOsInFileCache != 0;
						}
					}

					if (bActuallyNewPSO) 
					{						
						LogNewGraphicsPSOToConsoleAndCSV(NewEntry, PSOHash, PSOPrecacheResult);

						if (LogPSOtoFileCache())
						{
							NewPSOs.Add(PSOHash, NewEntry);
			...
			else if(!IsReferenceMaskSet(FPipelineFileCacheManager::GameUsageMask, PSOUsage->UsageMask))
			{
				...
				// If this PSO was previously logged as precache-only, upgrade its stored result to the
				// real runtime outcome. The Active state itself guarantees the CVar was set when logged.
				FPipelineCacheFileFormatPSO* Entry = NewPSOs.Find(PSOUsage->PSOHash);
				if (Entry && Entry->PSOPrecacheResult == EPSOPrecacheResult::Active)
				{
					Entry->PSOPrecacheResult = PSOPrecacheResult;
				}
			}
		}
...
void FPipelineFileCacheManager::CacheComputePSO(uint32 RunTimeHash, FComputePipelineStateInitializer const& Initializer, EPSOPrecacheResult PSOPrecacheResult, FPipelineStateStats** OutStats)
{
	if(IsPipelineFileCacheEnabled() && (LogPSOtoFileCache() || ReportNewPSOs()))
	{
		FRWScopeLock Lock(FileCacheLock, SLT_ReadOnly);
			...
			if(PSOUsage == nullptr || !IsReferenceMaskSet(FPipelineFileCacheManager::GameUsageMask, PSOUsage->UsageMask))
			{
				...
				if(PSOUsage == nullptr)
				{
					...
					if (!FPipelineFileCacheManager::IsPSOEntryCached(NewEntry, &CurrentUsageData))
					{
						...
						if (bActuallyNewPSO)
						{
							LogNewComputePSOToConsoleAndCSV(NewEntry, PSOHash, PSOPrecacheResult);
	...
	if(IsPipelineFileCacheEnabled() && (LogPSOtoFileCache() || ReportNewPSOs()) && Initializer.bFromPSOFileCache)
	{
		...
		if(PSOUsage == nullptr || !IsReferenceMaskSet(FPipelineCacheFlagInvalidPSO, PSOUsage->EngineFlags))
		{
			...
			if(PSOUsage == nullptr)
			{
				...
				bool bOK = FPipelineCacheFileFormatPSO::Init(ShouldBeExistingEntry, Initializer, EPSOPrecacheResult::Untracked);
				check(bOK);
				...
				check(bCached);	//bFromPSOFileCache was set but not in the cache something has gone wrong
				{
					...
					UE_LOGF(LogRHI, Warning, "Graphics PSO (%u) compile failure registering to File Cache", PSOHash);
				}
			}
			else if(!IsReferenceMaskSet(FPipelineCacheFlagInvalidPSO, PSOUsage->EngineFlags))
			{
				...
				UE_LOGF(LogRHI, Warning, "Graphics PSO (%u) compile failure registering to File Cache", PSOUsage->PSOHash);
			}
		}
	}
}
...
FPipelineStateStats* FPipelineFileCacheManager::RegisterPSOStatsInternal(FRWScopeLock& Lock, uint32 RunTimeHash)
{
	...
	if (!PSOUsage)
	{
		UE_LOGF(LogRHI, Display, "PSO (%u) missing from cache, please instead use the OutStats parameter in CacheGraphicsPSO/CacheComputePSO", RunTimeHash);
		return nullptr;
	}
...
struct FPipelineCacheFileData
{
	...
	static FPipelineCacheFileData Open(FString const& FilePath)
	{
			...
			*FileAReader << Data.Header;
			if (Data.Header.Magic == FPipelineCacheFileFormatMagic && Data.Header.Version >= (uint32)EPipelineCacheFileFormatVersions::FirstWorking)
			{
				...
				*FileAReader << Data.TOC;
				...
				if (FileAReader->IsError())
				{
                    UE_LOGF(LogRHI, Error, "Failed to read: %ls.", *FilePath);
					Data.Header.Magic = 0;
				}
				else
				{
						...
						for (auto& Entry : Data.TOC.MetaData)
						{
							...
							switch(PSO.Type)
							{
								...
								case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
									...
									if (PSO.GraphicsDesc.FragmentShader != FShaderHash())
									{
										Entry.Value.Shaders.Add(PSO.GraphicsDesc.FragmentShader);
									}
									
									if (PSO.GraphicsDesc.GeometryShader != FShaderHash())
									{
									...
									if (PSO.GraphicsDesc.AmplificationShader != FShaderHash())
									{
        ...
        {
            UE_LOGF(LogRHI, Error, "Failed to open: %ls.", *FilePath);
        }
		return Data;
	}
};
...
uint32 FPipelineFileCacheManager::NumPSOsLogged()
{
	...
	if(IsPipelineFileCacheEnabled() && LogPSOtoFileCache())
	{
		...
		if(Result == 0 && NumNewPSOs > 0)
		{
			...
			for (auto& PSOPair : NewPSOs)
			{
				FPipelineStateStats const* Stat = Stats.FindRef(PSOPair.Key);
				if (Stat && Stat->TotalBindCount > 0)
				{
					Result++;
				}
	...
	for (const FPipelineCacheFileFormatPSO& Item : PSOs)
	{
		...
		switch (Item.Type)
		{
			...
			case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
			{
				...
				if (Item.GraphicsDesc.VertexShader != FShaderHash())
					Meta.Shaders.Add(Item.GraphicsDesc.VertexShader);

				if (Item.GraphicsDesc.FragmentShader != FShaderHash())
					Meta.Shaders.Add(Item.GraphicsDesc.FragmentShader);

				if (Item.GraphicsDesc.GeometryShader != FShaderHash())
					Meta.Shaders.Add(Item.GraphicsDesc.GeometryShader);

				if (Item.GraphicsDesc.MeshShader != FShaderHash())
					Meta.Shaders.Add(Item.GraphicsDesc.MeshShader);

				if (Item.GraphicsDesc.AmplificationShader != FShaderHash())
	...
	if (A.Header.Magic == FPipelineCacheFileFormatMagic && B.Header.Magic == FPipelineCacheFileFormatMagic && A.Header.GameVersion == B.Header.GameVersion && A.Header.Platform == B.Header.Platform && A.Header.Version == FPipelineCacheFileFormatCurrentVersion && B.Header.Version == FPipelineCacheFileFormatCurrentVersion)
	{
		...
		if (FileWriter)
		{
			...
			*FileWriter << Output.TOC;
            ...
            UE_CLOGF(!bOK, LogRHI, Error, "Failed to write output file: %ls.", *OutputPath);
        ...
        {
            UE_LOGF(LogRHI, Error, "Failed to open output file: %ls.", *OutputPath);
        }
	}
    else if (A.Header.GameVersion != B.Header.GameVersion)
    {
        UE_LOGF(LogRHI, Error, "Incompatible game versions: %u vs. %u.", A.Header.GameVersion, B.Header.GameVersion);
    }
    else if (A.Header.Platform != B.Header.Platform)
    {
        UE_LOGF(LogRHI, Error, "Incompatible shader platforms: %ls vs. %ls.", *LegacyShaderPlatformToShaderFormat(A.Header.Platform).ToString(), *LegacyShaderPlatformToShaderFormat(B.Header.Platform).ToString());
    }
    else if (A.Header.Version != B.Header.Version)
    {
        UE_LOGF(LogRHI, Error, "Incompatible file versions: %u vs. %u.", A.Header.Version, B.Header.Version);
    }
    else
    {
...
{
	FString Result("RayTracingShader,DeprecatedMaxPayloadSizeInBytes,Frequency,ShaderBindingLayout-Hash,ShaderBindingLayout-Flags,ShaderBindingLayout-NumUBEntries");
...
FString FPipelineCacheFileFormatPSO::FPipelineFileCacheRayTracingDesc::ToString() const
{
	FString Result = FString::Printf(TEXT("%s,%d,%d,%d,%d,%d")
		...
		, uint32{EnumToUnderlyingType(ShaderBindingLayout.GetFlags())}
