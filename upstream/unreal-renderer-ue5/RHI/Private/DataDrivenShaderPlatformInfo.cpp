...
struct FDataDrivenShaderPlatformInfoEditorOnly
{
	...
	EShaderPlatform RuntimePlatform = SP_NumPlatforms;
...
#define GET_SECTION_STATIC_SHADER_BINDING_LAYOUT_SUPPORT_HELPER(SettingName) \
	...
	GET_SECTION_BOOL_HELPER(bSupportsRayTracingLinearSweptSpheres);
	...
	GET_SECTION_INT_HELPER(ThreadGroupSizeMode);
	...
	GET_SECTION_BOOL_HELPER(bStripNaniteVertexShaders);
	...
	GET_SECTION_BOOL_HELPER(bSupportsDescriptorRange);
	...
	GET_SECTION_BOOL_HELPER(bSupportsSpecializationConstants);
	...
	for (int32 Index = 0; Index < NumDDInfoFiles; Index++)
	{
		...
		for (const TPair<FString, FConfigSection>& Section : AsConst(IniFile))
		{
...
#endif
					UE_CLOGF(bIsEnabled, LogRHI, Warning, "Found an unknown shader platform %ls in a DataDriven ini file", *SectionName.Mid(15));
					continue;
				}
			...
			else if (Section.Key.StartsWith(TEXT("PreviewShaderPlatform ")))
			{
				...
				DataDrivenShaderPlatformInfoEditorOnlyInfos[ShaderPlatform].PreviewShaderPlatformParent = ShaderPlatform;
	...
	for (int32 PlatformIndex=0; PlatformIndex < SP_NumPlatforms; PlatformIndex++)
	{
		...
		if (IsValid(PreviewPlatform) && GetIsPreviewPlatform(PreviewPlatform))
		{
			...
			if (RuntimePlatform < SP_NumPlatforms)
			{
				...
				FDataDrivenShaderPlatformInfoEditorOnly& PreviewEditorInfo = DataDrivenShaderPlatformInfoEditorOnlyInfos[PreviewPlatform];
				PreviewEditorInfo.RuntimePlatform = RuntimePlatform;
...
#define PREVIEW_FORCE_DISABLE(SettingName) \
	PREVIEW_FORCE_SETTING(SettingName, false)
				...
				PREVIEW_DISABLE_IF_RUNTIME_UNSUPPORTED(bStripNaniteVertexShaders);
				PREVIEW_DISABLE_IF_RUNTIME_UNSUPPORTED(bSupportsMobileMultiView);
				PREVIEW_DISABLE_IF_RUNTIME_UNSUPPORTED(SupportsBarycentricsIntrinsics);
				PREVIEW_DISABLE_IF_RUNTIME_UNSUPPORTED(SupportsBarycentricsSemantic);

				// Settings that need to match the runtime
				PREVIEW_USE_RUNTIME_VALUE(bSupportsSpecializationConstants);
				PREVIEW_USE_RUNTIME_VALUE(MaxMeshShaderThreadGroupSize);
				PREVIEW_USE_RUNTIME_VALUE(ThreadGroupSizeMode);
				...
				PREVIEW_USE_RUNTIME_VALUE(EnablesHLSL2021ByDefault);
				...
				PREVIEW_USE_RUNTIME_VALUE(bSupportsDescriptorRange);
				PREVIEW_USE_RUNTIME_VALUE(StaticShaderBindingLayoutSupport);

				PREVIEW_USE_RUNTIME_VALUE(bSupportsRayTracingClusterOps);
				...
				// Metals bindless and bSupportsSceneDataCompressedTransforms need to match the max shader platform
				if (IsMetalPlatform(GMaxRHIShaderPlatform))
				{
					PREVIEW_FORCE_SETTING(bSupportsBindless, Infos[GMaxRHIShaderPlatform].bSupportsBindless);
					PREVIEW_FORCE_SETTING(bSupportsSceneDataCompressedTransforms, Infos[GMaxRHIShaderPlatform].bSupportsSceneDataCompressedTransforms);
				}
...
FText FGenericDataDrivenShaderPlatformInfo::GetFriendlyName(EShaderPlatform Platform, FName DeviceProfileName)
{
...
const EShaderPlatform FGenericDataDrivenShaderPlatformInfo::GetPreviewShaderPlatformParent(EShaderPlatform Platform)
{
	check(IsValid(Platform));
	return DataDrivenShaderPlatformInfoEditorOnlyInfos[Platform].PreviewShaderPlatformParent;
}

const EShaderPlatform FGenericDataDrivenShaderPlatformInfo::GetPreviewRuntimePlatform(EShaderPlatform Platform)
{
	check(IsValid(Platform));
	check(GetIsPreviewPlatform(Platform));
	return DataDrivenShaderPlatformInfoEditorOnlyInfos[Platform].RuntimePlatform;
}

const bool FGenericDataDrivenShaderPlatformInfo::CanUseForMaterialValidation(EShaderPlatform Platform)
{
	check(IsValid(Platform));
...
FName FGenericDataDrivenShaderPlatformInfo::GetPreferredShaderCodeFormat(EShaderPlatform Platform)
{
