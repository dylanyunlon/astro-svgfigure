...
UE_TRACE_CHANNEL_DEFINE(RHICommandsChannel, "RHI command list execution and translation.");
...
TAutoConsoleVariable<bool> CVarRHICmdParallelTranslateBatchSubmissions(
	TEXT("r.RHICmd.ParallelTranslate.BatchSubmissions"),
	false,
	TEXT("When true, generated platform command lists are not submitted to the RHI until the immediate command list is dispatched with the SubmitToGPU flag. ")
	TEXT("When false, platform command lists are handed to the RHI immediately after finalizing (but still in submission order), reducing pipeline latency. "));
...
std::atomic<uint64> GInputLatencyTime = 0;
	...
	, PersistentState(InPersistentState)
{
	...
	CommandLink = &Root;
	IF_RENDER_TRACING_ENABLED(RenderTracing::CommandListCreated(*this, InPersistentState.bImmediate));
}
...
FRHICommandListBase::~FRHICommandListBase()
{
	...
	IF_RENDER_TRACING_ENABLED(RenderTracing::CommandListDestroyed(*this));
}
...
void FRHICommandListBase::InsertParallelRenderPass_Base(TSharedPtr<FRHIParallelRenderPassInfo> const& InInfo, TArray<FRHISubCommandList*>&& SubCommandLists)
{
	...
	bool bRequiresWait = GRHIGlobals.ParallelRenderPasses.ParentWaitForChildTranslate || GRHIGlobals.ParallelRenderPasses.ChildWaitForParentBegin;
	...
	// Split the RHICmdList by moving the current commands into a new instance on the heap, and reconstructing 'this'.
	{
		...
		IF_RENDER_TRACING_ENABLED(RenderTracing::CommandListDetach(*this, *HeapCmdList, false));
...
void FRHICommandListBase::FinishRecording()
{
	...
	IF_RENDER_TRACING_ENABLED(RenderTracing::CommandListFinishRecording(*this, bUsesLockFence));
	...
	if (PendingBufferUploads.Num() > 0)
	{
		...
		UE_LOGF(LogRHI, Fatal, "Detected pending buffer uploads on RHICmdList submission: %ls", BufferList.ToString());
	}
	if (PendingTextureUploads.Num() > 0)
	{
		TStringBuilder<0> TextureList;
		...
		UE_LOGF(LogRHI, Fatal, "Detected pending texture uploads on RHICmdList submission: %ls", TextureList.ToString());
	}
...
void FRHICommandListBase::AttachBreadcrumbSubTree(FRHIBreadcrumbAllocator& Allocator, FRHIBreadcrumbList& Nodes)
{
	...
	{
		check(!BreadcrumbAllocator.IsValid() || BreadcrumbAllocatorRefs.Contains(BreadcrumbAllocator.Get()));
...
void FRHICommandListBase::ActivatePipelines(ERHIPipeline Pipelines)
{
	...
	](FRHICommandListBase& ExecutingCmdList)
	{
		...
		IF_RENDER_TRACING_ENABLED(RenderTracing::ActivatePipelines(ExecutingCmdList, static_cast<uint8>(NewPipelines)));
		...
		for (ERHIPipeline Pipeline : MakeFlagsRange(NewPipelines))
		{
			...
			case ERHIPipeline::Graphics:
			{
				if (!Context)
				{
					...
					{
						if (GRHIGlobals.ParallelRenderPasses.UsesCustomContexts)
						{
							Context = GDynamicRHI->RHIGetParallelCommandContext(*ExecutingCmdList.SubRenderPassInfo);
						}
						else
						{
							// Platform does not implement RHIGetParallelCommandContext. Use a regular context and begin/end the render pass directly.
							IRHICommandContext* GraphicsContext = static_cast<IRHICommandContext*>(GDynamicRHI->RHIGetCommandContext(ERHIPipeline::Graphics));
							GraphicsContext->RHIBeginRenderPass(*ExecutingCmdList.SubRenderPassInfo, ExecutingCmdList.SubRenderPassInfo->PassName);
					...
					else
					{
						...
						Context = ExecutingCmdList.AllowParallelTranslate()
							? GDynamicRHI->RHIGetCommandContext(Pipeline)
							: GDynamicRHI->RHIGetDefaultContext();
					}
					IF_RENDER_TRACING_ENABLED(RenderTracing::SetTranslateContext(ExecutingCmdList, Context, static_cast<uint8>(Pipeline)));
				}
			...
			case ERHIPipeline::AsyncCompute:
			{
				...
				if (!Context)
				{
					Context = GDynamicRHI->RHIGetCommandContext(Pipeline);
					check(Context);
					IF_RENDER_TRACING_ENABLED(RenderTracing::SetTranslateContext(ExecutingCmdList, Context, static_cast<uint8>(Pipeline)));
				}
...
struct FRHICommandListExecutor::FTaskPipe::FTask final : public ITask
{
	FFunc Lambda;
	...
	FTask(ENamedThreads::Type NamedThread, FGraphEventArray&& Prereqs, FFunc&& Lambda)
	...
	void ExtendTask(FGraphEventArray&& NewPrereqs, FFunc&& NewLambda) override
	{
		check(!bExtended);
		Prereqs = MoveTemp(NewPrereqs);
		Lambda = MoveTemp(NewLambda);
...
void FRHICommandListExecutor::FTaskPipe::Enqueue(ENamedThreads::Type NamedThread, FGraphEventArray&& Prereqs, FTaskPipe::FFunc&& Lambda)
{
...
void FRHICommandListExecutor::EnqueueDispatchTask(FGraphEventArray&& Prereqs, FTaskPipe::FFunc&& Lambda)
{
	check(IsInRenderingThread());
...
FRHICommandListExecutor::FTaskPipe* FRHICommandListExecutor::FTranslateState::EnqueueTranslateTask(FGraphEventArray&& Prereqs, FTaskPipe::FFunc&& Lambda)
{
	// This is called on the dispatch pipe
	...
	if (!GRHICommandList.AllowParallel())
	{
		...
		Pipe = bAllowParallelTranslate
			? &GRHICommandList.SingleTranslatePipe
			: &GRHICommandList.SubmitPipe;
	}
	else if (!bParallel)
	{
...
void FRHICommandListExecutor::EnqueueSubmitTask(FGraphEventArray&& Prereqs, FTaskPipe::FFunc&& Lambda)
{
	// This is called on the dispatch pipe
	...
	SubmitPipe.Enqueue(NamedThread, MoveTemp(Prereqs), MoveTemp(Lambda));
}
...
FRHICommandListExecutor::FTranslateState::FTranslateState(const FSubmitState& SubmitState, FRHICommandListBase* FirstCmdList)
	: bAllowParallelTranslate(SubmitState.bAllowParallelTranslate)
	, bParallel              (FirstCmdList->AllowParallelTranslate())
	, bUsingSubCmdLists      (FirstCmdList->IsSubCommandList())
{
}

void FRHICommandListExecutor::FSubmitState::Dispatch_ProcessCommandList(FRHICommandListBase* CmdList)
{
	SCOPED_NAMED_EVENT(RHI_Dispatch, FColor::Yellow);

	check(CmdList->DispatchEvent->IsComplete());
	Dispatch_PrepareBreadcrumbs(CmdList);

	CmdList->bAllowParallelTranslate = bAllowParallelTranslate;

	//
	// Determine if the current translate chain should be closed, i.e.:
	//    - We've exceeded the threshold number of RHI commands.
	//    - The next command list requires single-threaded execution, but we're in a parallel chain.
	//
	if (TranslateJobs.Current)
	{
		bool bShouldSplitParallel = bAllowSingleParallelCombine
				? TranslateJobs.Current->bParallel && !CmdList->AllowParallelTranslate()  // Only start a new translate job if we were parallel, but the new command list requires single thread.
				: TranslateJobs.Current->bParallel !=  CmdList->AllowParallelTranslate(); // Always start a new translate job if bParallel is different. Avoids batching parallel work into single thread translates.
	
		bool bShouldSplitForThreshold = MaxCommandsPerTranslate  < 0 ? true  :
										MaxCommandsPerTranslate == 0 ? false :
										(TranslateJobs.Current->NumCommands + CmdList->NumCommands) > uint32(MaxCommandsPerTranslate);
	
		bool bShouldSplitForParentChild = CmdList->IsSubCommandList() != TranslateJobs.Current->bUsingSubCmdLists;
	
		if (bShouldSplitParallel || bShouldSplitForThreshold || bShouldSplitForParentChild)
		{
			//
			// Close the current translate chain
			//

			IF_RENDER_TRACING_ENABLED(RenderTracing::SplitTranslateJob(TranslateJobs.Current, *CmdList, bShouldSplitParallel, bShouldSplitForThreshold, bShouldSplitForParentChild));

			bool bAddChildWait  =  TranslateJobs.Current->bUsingSubCmdLists && GRHIGlobals.ParallelRenderPasses.ParentWaitForChildTranslate;
			bool bAddParentWait = !TranslateJobs.Current->bUsingSubCmdLists && CmdList->IsSubCommandList() && GRHIGlobals.ParallelRenderPasses.ChildWaitForParentBegin;

			FGraphEventRef Event = Dispatch_CloseTranslateChain(false, 0);
			if (bAddChildWait)
			{
	...
	//
	// We may have closed the current chain above, or this is the first submission.
	// Start a new chain if we don't have one.
	//
	if (!TranslateJobs.Current)
	{
		TranslateJobs.Current = TranslateJobs.All.Emplace_GetRef(MakeUnique<FTranslateState>(*this, CmdList)).Get();
	}

	check(TranslateJobs.Current->bShouldFinalize);
	TranslateJobs.Current->bShouldFinalize = !(GRHIGlobals.ParallelRenderPasses.ParentWaitForChildTranslate && CmdList->ParallelRenderPassBegin.IsValid());
	TranslateJobs.Current->NumCommands += CmdList->NumCommands;

	// Create the translation task for this command list.
	FGraphEventArray Prereqs;
	if (!CmdList->AllowParallelTranslate())
	{
		// Wait for all previous translate jobs to complete
		Prereqs = MoveTemp(TranslateJobs.Events);
	}
		
	// Handle adding prerequisites for Parent end waiting on children
	if (CmdList->ParallelRenderPassEnd.IsValid())
	{
		if (GRHIGlobals.ParallelRenderPasses.ParentWaitForChildTranslate)
		{
			Prereqs = TranslateJobs.Events;
	...
	else if (CmdList->IsSubCommandList() && GRHIGlobals.ParallelRenderPasses.ChildWaitForParentBegin)
	{
	...
	if (!GRHIGlobals.SupportsConcurrentTranslateAndSubmit)
	{
		//
		// Don't start new translations until all prior submissions have been closed.
		...
		//
		Prereqs.Add(GRHICommandList.LastClose);
	}

	IF_RENDER_TRACING_ENABLED(RenderTracing::DispatchCommandList(TranslateJobs.Current, *CmdList));

	TranslateJobs.Current->EnqueueTranslateTask(MoveTemp(Prereqs),
		[TranslateJob = TranslateJobs.Current, CmdList](FTaskPipe::ITask&)
		{
			TranslateJob->Translate_ExecuteCommandList(CmdList);
		});
}

FGraphEventRef FRHICommandListExecutor::FSubmitState::Dispatch_CloseTranslateChain(bool bLastChainInSubmission, uint32 RTSubmissionID)
{
	// Tell the current chain there will be no more command lists. This finalizes the RHI contexts.
	FTaskPipe* TranslatePipe = TranslateJobs.Current->EnqueueTranslateTask({},
		[Job = TranslateJobs.Current](FTaskPipe::ITask&)
		{
			Job->Translate_CloseChain();
		});

	FGraphEventRef TranslateEvent {};
	if (TranslatePipe == &GRHICommandList.SubmitPipe)
	{
		// Don't close the pipe if we got scheduled on the submit pipe, to avoid splitting tasks.
		// Since the pipes guarantee FIFO order, we don't need an event for the submission to await.
	}
	else
	{
		// Close the translate pipe. This gives us a TaskGraph event that is signalled when the
		// translation has completed, which submission will await.
		TranslateEvent = TranslatePipe->Close();

		TranslateJobs.Current->TranslateEvent = TranslateEvent;
		TranslateJobs.Events.Add(TranslateEvent);
	}

	// Link the completed translate chain into the submission queue
	// (the linked list of jobs following FTranslateState::Next).
	bool bStartTask = false;
	if (!TranslateJobs.First)
	{
		check(!TranslateJobs.Previous);
		TranslateJobs.First = TranslateJobs.Current;
		bStartTask = true;
	}
	else
	{
		...
		// Attempt to atomically append this translate job to the end of the submission queue.
		FTranslateState* Expected = nullptr;	
		if (TranslateJobs.Previous->Next.compare_exchange_strong(Expected, TranslateJobs.Current))
		{
			// Succesfully appended to the queue before the previous task finished.
			check(Expected == nullptr);
		}
		else
		{
			// Failed to append. The previous submission task has already run, and won't process the current chain.
			// We'll need to start a new task.

			// Submit_SubmitToGPU uses the job's 'this' pointer as a sentinel to indicate the closure of the queue.
			check(Expected == TranslateJobs.Previous);
	...
	// Pointers won't have the bottom bit set, so we use this to distinguish between submissions instigated by the render
	// thread via ERHISubmitFlags::SubmitToGPU and eager submissions caused by a translate chain being closed. In the latter
	// case we want to link the submission to the last translate job in the chain.
	uint64 TracingSubmissionID = !bLastChainInSubmission
		? static_cast<uint64>(reinterpret_cast<uintptr_t>(TranslateJobs.Current))
		: (static_cast<uint64>(RTSubmissionID) << 32) | 0x01;
#else
	uint64 TracingSubmissionID = 0;
#endif

	TranslateJobs.Previous = TranslateJobs.Current;
	TranslateJobs.Current = nullptr;

	//
	// When batching submissions, the submit task only starts when we've closed the last translate chain (bLastChainInSubmission == true)
	// 
	// Otherwise, we submit work to the GPU immediately. In this case, we may need to start a new task (bStartTask == true) if:
	//         1) there wasn't already one running
	//     OR  2) we failed to append this translate chain to the queue
	//
	if (bBatchSubmissions ? bLastChainInSubmission : bStartTask)
	{
		check(TranslateJobs.First);

		// Start a new task to process the submission queue from 'First'
		GRHICommandList.EnqueueSubmitTask({ TranslateEvent },
			[this, FirstJob = TranslateJobs.First, TracingSubmissionID](FTaskPipe::ITask& Task)
			{
				Submit_SubmitToGPU(FirstJob, Task, TracingSubmissionID);
			});
	}
...
void FRHICommandListExecutor::FSubmitState::Dispatch_CloseSubmission(TOptional<int64> LocalStatsFrame, uint32 RTSubmissionID)
{
	SCOPED_NAMED_EVENT(RHI_Dispatch_CloseSubmission, FColor::Purple);

	// We still have a translate chain open. Close it.
	Dispatch_CloseTranslateChain(true, RTSubmissionID);

	// Forward the close command to the submit pipe.
	FSubmitState::FCloseArgs CloseArgs;
#if WITH_RHI_BREADCRUMBS
	for (ERHIPipeline Pipeline : MakeFlagsRange(ERHIPipeline::All))
	{
		CloseArgs.GPUBreadcrumbs[Pipeline] = GRHICommandList.Breadcrumbs.GPU[Pipeline].Current;
	}
#endif
#if STATS
	CloseArgs.StatsFrame = LocalStatsFrame;
#endif

	GRHICommandList.LastClose = CompletionEvent;

	GRHICommandList.EnqueueSubmitTask({},
		[this, CloseArgs](FTaskPipe::ITask&) mutable
		{
			Submit_Close(CloseArgs);
		});
}

void FRHICommandListExecutor::FSubmitState::Dispatch_PrepareBreadcrumbs(FRHICommandListBase* CmdList)
{
#if WITH_RHI_BREADCRUMBS
...
void FRHICommandListExecutor::FTranslateState::Translate_ExecuteCommandList(FRHICommandListBase* CmdList)
{
	SCOPED_NAMED_EVENT(RHI_Translate, FColor::Orange);

	IF_RENDER_TRACING_ENABLED(RenderTracing::BeginTranslateCommandList(this, *CmdList));
	...
	else
	{
		...
		{
			CmdList->UploadContext = UploadContext;
		}
	}
	...
	UploadContext = CmdList->UploadContext;
	...
	IF_RENDER_TRACING_ENABLED(RenderTracing::EndTranslateCommandList(*CmdList));

	delete CmdList;
}

void FRHICommandListExecutor::FTranslateState::Translate_CloseChain()
{
	SCOPED_NAMED_EVENT(RHI_CloseChain, FColor::Magenta);

	FDynamicRHI::FRHICloseTranslateChainArgs CloseChainArgs;
	CloseChainArgs.UploadContext = UploadContext;
	CloseChainArgs.bShouldFinalize = bShouldFinalize;

	for (ERHIPipeline Pipeline : MakeFlagsRange(ERHIPipeline::All))
	{
		FPipelineState& State = PipelineStates[Pipeline];
		CloseChainArgs.Contexts[Pipeline] = State.Context;

		if (bUsingSubCmdLists && Pipeline == ERHIPipeline::Graphics && !GRHIGlobals.ParallelRenderPasses.UsesCustomContexts)
		{
			check(bShouldFinalize);
			static_cast<IRHICommandContext*>(State.Context)->RHIEndRenderPass();
		}
	}

	if (GDynamicRHI)
	{
		FinalizedCommandLists = GDynamicRHI->RHICloseTranslateChain(MoveTemp(CloseChainArgs));

		IF_RENDER_TRACING_ENABLED(RenderTracing::FinalizeTranslateJob(this, bParallel, bUsingSubCmdLists));
	}
}
...
void FRHICommandListExecutor::FSubmitState::Submit_SubmitToGPU(FTranslateState* First, FTaskPipe::ITask& Task, uint64 TracingSubmissionID)
{
	SCOPED_NAMED_EVENT(RHI_SubmitToGPU, FColor::Purple);

	// Coalesce finalized platform command lists into a single array,
	// taking as many from the submission queue as we can.
	TArray<IRHIPlatformCommandList*> CommandLists;

#if UE_RENDER_TRACING_ENABLED
	TArray<void*, TInlineAllocator<32>> TranslateJobIDs;
#endif

	FTranslateState* Current = First;
	while (true)
	{
		if (Current->TranslateEvent && !Current->TranslateEvent->IsComplete())
		{
			// Translation has not completed on this job yet, so we stop processing the queue at this point.
			// Extend this task to pick this back up when translation has completed later.
			Task.ExtendTask({ Current->TranslateEvent },[this, First = Current](FTaskPipe::ITask& Task)
			{
				SCOPED_NAMED_EVENT(RHI_ExtendedSubmit, FColor::Orange);
				// We can compute this value even when tracing is disabled since it's free.
				uint64 TracingSubmissionID = static_cast<uint64>(reinterpret_cast<uintptr_t>(First));
				Submit_SubmitToGPU(First, Task, TracingSubmissionID);
			});
			break;
		}

		// Insert the upload command list before any of the pipeline command lists
		if (Current->FinalizedCommandLists.UploadCommandList)
		{
			CommandLists.Add(Current->FinalizedCommandLists.UploadCommandList);
		}

		// Gather the completed platform command lists for every RHI pipeline.
		for (ERHIPipeline Pipeline : MakeFlagsRange(ERHIPipeline::All))
		{
			if (IRHIPlatformCommandList* CommandList = Current->FinalizedCommandLists.CommandLists[Pipeline])
			{
			#if WITH_RHI_BREADCRUMBS
				FRHIBreadcrumbRange& BreadcrumbRange = Current->PipelineStates[Pipeline].Range;
				CommandList->BreadcrumbAllocators = Current->BreadcrumbAllocatorRefs;

				auto& Allocators = CommandList->BreadcrumbAllocators;

				auto& GlobalLast = GRHICommandList.Breadcrumbs.GPU[Pipeline].Last;
				// Link ranges of breadcrumbs together into depth-first list
				if (GlobalLast)
				{
					FRHIBreadcrumbNode*& GlobalNext = GlobalLast->GetNextPtr(Pipeline);
					check(!GlobalNext);
					GlobalNext = BreadcrumbRange.First;
				}

				FRHIBreadcrumbRange Range{};
				Range.First = GlobalLast ? GlobalLast.Get() : BreadcrumbRange.First;
				if (Range.First) { Allocators.AddUnique(Range.First->Allocator); }

				// Advance the global last breadcrumb forward
				if (BreadcrumbRange.Last)
				{
					GlobalLast = BreadcrumbRange.Last;
				}

				Range.Last = GlobalLast ? GlobalLast.Get() : Range.First;
				if (Range.Last) { Allocators.AddUnique(Range.Last->Allocator); }

				CommandList->BreadcrumbRange = Range;
			#endif
...
#if UE_RENDER_TRACING_ENABLED
		TranslateJobIDs.Add(Current);
#endif

		// Try to close the queue. Use the current job's 'this' pointer as a sentinel
		// to indicate to the dispatch pipe that the queue was atomically closed.
		FTranslateState* Expected = nullptr;
		if (Current->Next.compare_exchange_strong(Expected, Current))
		{
			// Queue closed, no more chains to process.
			break;
		}
...
#if UE_RENDER_TRACING_ENABLED
	if (!TranslateJobIDs.IsEmpty())
	{
		RenderTracing::SubmitTranslateJobs(TranslateJobIDs, TracingSubmissionID);
	}
#endif

	// Submit everything we gathered to the GPU.
	// Call this even with empty command lists as some RHIs use this to perform non-commandlist housekeeping.
	// This can be called before the RHI is initialized.
	if (GDynamicRHI)
	{
		GDynamicRHI->RHISubmitCommandLists({ MoveTemp(CommandLists) });
	}
}

void FRHICommandListExecutor::FSubmitState::Submit_Close(const FCloseArgs& Args)
{
	SCOPED_NAMED_EVENT(RHI_SubmitClose, FColor::Red);

	{
		SCOPED_NAMED_EVENT(DeleteRHIResources, FColor::Red);
		while (true)
		{
	...
	if (EnumHasAnyFlags(SubmitFlags, ERHISubmitFlags::EndFrame))
	{
		...
		#endif
			.bProfileNextFrame = EnumHasAnyFlags(SubmitFlags, ERHISubmitFlags::ProfileNextFrame)
		};
...
RHI_API FGraphEventRef FRHICommandListExecutor::Submit(TConstArrayView<FRHICommandListBase*> AdditionalCommandLists, ERHISubmitFlags SubmitFlags)
{
...
#if UE_RENDER_TRACING_ENABLED
	const uint32 RTSubmissionID = RenderTracing::SubmitCommandLists(AllCmdLists, SubmitFlags);
#else
	const uint32 RTSubmissionID = 0;
#endif
	...
	// by a task in FTranslateState::Translate_CloseChain() that runs after the batch of translates have completed for that context. Multiple translates for 
	...
	// 
	// Once all finalized tasks have completed, FSubmitState::Submit_SubmitToGPU() called which hands the platform GPU command lists down to the RHI,
	// or if submission batching is disabled, this hand-off happens as soon as possible after each chain is closed, in submission order.
	// 
	...
	if (!SubmitState)
	{
		// RHIs that do not support parallel translate must always batch submissions to enforce the ordering of translate -> submit -> translate.
		bool bBatchSubmissions = (!GRHISupportsParallelRHIExecute) || CVarRHICmdParallelTranslateBatchSubmissions.GetValueOnRenderThread();

		SubmitState = new FSubmitState(bBatchSubmissions);
		...
		// Prevent use of parallel contexts if unsupported by the RHI, or while disabled by the cvar.
		if (GRHISupportsParallelRHIExecute && CVarRHICmdParallelTranslateEnable.GetValueOnRenderThread())
		{
	...
	if (EnumHasAnyFlags(SubmitFlags, ERHISubmitFlags::SubmitToGPU))
	{
		...
		EnqueueDispatchTask({},
			[this, State = SubmitState, LocalStatsFrame, RTSubmissionID](FTaskPipe::ITask&)
			{
				SCOPED_NAMED_EVENT(RHI_FinalizeSubmission, FColor::Yellow);
				State->Dispatch_CloseSubmission(LocalStatsFrame, RTSubmissionID);
			});
...
void FRHICommandListExecutor::WaitOnRHIThreadFence(FGraphEventRef& Fence)
{
	...
	if (Fence.GetReference() && !Fence->IsComplete())
	{
		...
		if (FTaskGraphInterface::Get().IsThreadProcessingTasks(RenderThread_Local))
		{
			...
			UE_LOGF(LogRHI, Fatal, "Deadlock in WaitOnRHIThreadFence.");
		}
...
void FRHICommandListImmediate::EndDrawingViewport(FRHIViewport* Viewport, const FRHIPresentArgs& InPresentArgs)
{
	RHI_BREADCRUMB_EVENT(*this, "EndDrawingViewport");
	
	GDynamicRHI->RHIEndDrawingViewport(*this, Viewport, InPresentArgs);

	// Dispatch to ensure any swapchain present commands enqueued by the platform RHI above are executed in a timely manner.
...
FTextureRHIRef FRHICommandListImmediate::AsyncReallocateTexture2D(FRHITexture* OldTexture, int32 NewMipCount, int32 NewSizeX, int32 NewSizeY, FThreadSafeCounter* RequestStatus)
{
	LLM_SCOPE(ELLMTag::Textures);
	RHI_BREADCRUMB_EVENT_F(*this, "AsyncReallocateTexture2D", "AsyncReallocateTexture2D - %s", RHI_BREADCRUMB_FIELD("Texture Name", OldTexture->GetName()));

	FRHIScopedGPUMask MaskScope(*this, FRHIGPUMask::All());

	ERHIPipeline OriginalPipeline = GetPipeline();
	if (OriginalPipeline != ERHIPipeline::Graphics)
	{
		SwitchPipeline(ERHIPipeline::Graphics);
	}

	FRHITextureDesc NewDesc = OldTexture->GetDesc();
	NewDesc.Extent = FIntPoint(NewSizeX, NewSizeY);
	NewDesc.NumMips = NewMipCount;

	FString DebugName = OldTexture->GetName().ToString();
	...
	FTextureRHIRef NewTexture = CreateTexture(CreateDesc);

	// Ensure the old texture is in CopySrc state (the new texture was already created in CopyDest state).
	FRHITransitionInfo TransitionsBefore[] =
	{
		FRHITransitionInfo(OldTexture, ERHIAccess::Unknown, ERHIAccess::CopySrc)
	};
	Transition(MakeArrayView(TransitionsBefore, UE_ARRAY_COUNT(TransitionsBefore)));

	UE::RHI::CopySharedMips(*this, OldTexture, NewTexture);

	FRHITransitionInfo TransitionsAfter[] =
	{
		FRHITransitionInfo(OldTexture, ERHIAccess::CopySrc, ERHIAccess::SRVMask),
		FRHITransitionInfo(NewTexture, ERHIAccess::CopyDest, RHIGetDefaultResourceState(NewDesc.Flags, false))
	};
	Transition(MakeArrayView(TransitionsAfter, UE_ARRAY_COUNT(TransitionsAfter)));

	if (OriginalPipeline != ERHIPipeline::Graphics)
	{
		SwitchPipeline(OriginalPipeline);
	}

	EnqueueLambda([RequestStatus](FRHICommandListImmediate&)
	{
...
void FRHICommandListExecutor::CleanupGraphEvents()
{
	...
	LastClose.SafeRelease();
	CompletionEvent.SafeRelease();

	DispatchPipe.CleanupGraphEvents();
	SingleTranslatePipe.CleanupGraphEvents();
	SubmitPipe.CleanupGraphEvents();
}

void FRHICommandListBase::InvalidBufferFatalError(const FRHIBufferCreateDesc& CreateDesc)
{
	UE_LOGF(LogRHI, Fatal, "Attempt to create zero-sized buffer '%ls', owner '%ls', usage 0x%x, stride %u.",
