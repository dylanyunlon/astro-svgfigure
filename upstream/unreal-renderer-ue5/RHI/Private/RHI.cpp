...
#if ENABLE_ABTEST
// Core is loaded before RHI, so FABTest's delegates exist by the time this runs.
struct FABTestGPUCounterBinder
{
	FABTestGPUCounterBinder()
	{
		FABTest::SetGPUCounter.BindLambda([](FName WatchedName, FABTest::FGPUSink Sink)
		{
			UE::RHI::GPUProfiler::FGPUStat::SetGlobalObserver(WatchedName, MoveTemp(Sink));
		});
	}
};
static const FABTestGPUCounterBinder GABTestGPUCounterBinder;
#endif // ENABLE_ABTEST
...
static FAutoConsoleCommandWithWorldAndArgs CVarRHISetGPUCaptureOptions(
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateStatic([](const TArray<FString>& Args, UWorld* World)
	{
		...
		{
			UE_LOGF(LogRHI, Display, "Usage: r.RHISetGPUCaptureOptions 0 or r.RHISetGPUCaptureOptions 1");
		}
	})
);
...
const FClearValueBinding FClearValueBinding::DepthNear(1.0f, 0);
const FClearValueBinding FClearValueBinding::DepthFar(0.0f, 0);
...
const FClearValueBinding FClearValueBinding::DefaultNormal8Bit(FLinearColor(128.0f / 255.0f, 128.0f / 255.0f, 128.0f / 255.0f, 1.0f));
	...
	void FRHIDrawStatsCategory::FManager::AccumulateFrameStats(FDrawCounts const& NewCounts)
	{
		// Update the current counts using the maximum over the update period
		for (int32 GPUIndex = 0; GPUIndex < CurrentCounts.Num(); ++GPUIndex)
		{
			FDrawCountsPerGPU      & Dst = CurrentCounts[GPUIndex];
			FDrawCountsPerGPU const& Src = NewCounts[GPUIndex];

			for (int32 CategoryIndex = 0; CategoryIndex < Src.Num(); ++CategoryIndex)
			{
				Dst[CategoryIndex] = FMath::Max(Dst[CategoryIndex], Src[CategoryIndex]);
			}
		}
		...
		if (CurrentTime - LastTime > TimeoutSeconds)
		{
			...
			{
				FScopeLock Lock(&DisplayCountsCS);
	...
	FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateStatic([](const TArray<FString>& Args, UWorld*, FOutputDevice& OutputDevice)
{
	...
	FName CategoryName("RHIResources");
...
namespace RHIInternal
{
	...
	struct FResourceFlags
	{
		...
		FString GetString()
		{
			...
			if (bReserved)
			{
				FlagsString += bHasFlag ? " | Reserved" : "Reserved";
				bHasFlag = true;
			}
			return FlagsString;
		}
	...
	FResourceFlags GetResourceFlagsInternal(const FResourceEntry& Resource)
	{
		...
		if (Resource.ResourceInfo.Type == RRT_Texture)
		{
			...
			Flags.bReserved = EnumHasAnyFlags(Texture->GetFlags(), TexCreate_ReservedResource);
		}
		else if (Resource.ResourceInfo.Type == RRT_Buffer)
		{
			...
			Flags.bReserved = EnumHasAnyFlags((EBufferUsageFlags)Buffer->GetUsage(), BUF_ReservedResource);
		}

		Flags.bHasFlags = Flags.bResident || Flags.bMarkedForDelete || Flags.bTransient || Flags.bStreaming || Flags.bRT || Flags.bDS || Flags.bUAV || Flags.bRTAS || Flags.bReserved;
		return Flags;
	}
}
...
void RHIDumpResourceMemoryToLog(int32 NumberOfResourcesToShow)
{
	...
	if (Output)
	{
		...
		FBufferedOutputDevice BufferedOutput;
		RHIDumpResourceMemory(NameFilter, TypeFilter, TransientFilter, NumberOfResourcesToShow, bUseCSVOutput, bSummaryOutput, bOutputToCSVFile, BufferedOutput);
		BufferedOutput.RedirectTo(*Output);
	}
}
...
bool IsRHIDeviceQualcomm()
{
	...
	return GRHIVendorId == 0x5143 || GRHIVendorId == 0x4D4F4351;
}
...
EGpuVendorId GetRHIDeviceVendorId()
{
	if (IsRHIDeviceAMD())
	{
	...
	if (IsRHIDeviceQualcomm())
	{
		return EGpuVendorId::Qualcomm;
	}
	if (IsRHIDeviceApple())
	{
		return EGpuVendorId::Apple;
	}
