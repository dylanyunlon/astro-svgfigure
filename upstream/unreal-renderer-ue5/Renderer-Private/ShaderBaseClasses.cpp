...
#endif // WITH_EDITORONLY_DATA
{
	...
	MaterialUniformBufferVS.Bind(Initializer.ParameterMap,TEXT("MaterialVS"));
...
{
	UE_LOGF(LogShaders, Warning, "Layout %ls, Hash %08x", *Layout.GetDebugName(), Layout.GetHash());
	...
	UE_LOGF(LogShaders, Warning, "Layout CB Size %d %d Resources: %ls", Layout.ConstantBufferSize, Layout.Resources.Num(), *ResourcesString);
};
...
void FMaterialShader::VerifyExpressionAndShaderMaps(const FMaterialRenderProxy* MaterialRenderProxy, const FMaterial& Material, const FUniformExpressionCache* UniformExpressionCache) const
{
	...
	if (!bUniformExpressionSetMismatch)
	{
		...
		{
			UE_LOGF(LogShaders, Warning, "Material Expression UB mismatch!");
...
static void DumpRegisteredMaterialParameterCollections(const FMaterial& Material, const FGuid& ParameterCollectionGuid)
{
	...
	UE_LOGF(LogRenderer, Fatal, "Failed to find parameter collection buffer with GUID '%ls' for %ls.\n"
		"Currently %i listed default instances: %ls",
	...
	const FSceneInterface* Scene)
{
	...
	if (UniformExpressionCache->UniformBufferVS)
	{
		SetUniformBufferParameter(BatchedParameters, MaterialUniformBufferVS, UniformExpressionCache->UniformBufferVS);
	}
...

	{
		...
		if (ParameterCollectionUniformBuffers.Num() < ParameterCollectionsNum)
		{
			UE_LOGF(LogRenderer, Warning,
				"ParameterCollectionUniformBuffers.Num() [%u] < ParameterCollectionsNum [%u], this would crash below on SetUniformBufferParameter.\n" "RenderProxy=%ls Material=%ls",
	...
	FMeshDrawSingleShaderBindings& ShaderBindings) const
{
	...
	if (UniformExpressionCache.UniformBufferVS)
	{
		ShaderBindings.Add(MaterialUniformBufferVS, UniformExpressionCache.UniformBufferVS);
	}
...
bool FMaterialShader::Serialize(FArchive& Ar)
{
	...
	static FName DebugUniformExpressionUB("DebugUniformExpressionUB");
	...
	FVertexInputStreamArray& VertexStreams) const
{
	...
	if (VertexFactory->SupportsGPUScene(FeatureLevel))
	{
