...
DEFINE_LOG_CATEGORY(LogSkyAtmosphereRendering)
...
static TAutoConsoleVariable<bool> CVarSkyAtmosphereStateVersioning(
	TEXT("r.SkyAtmosphere.StateVersioning"), true,
	TEXT("SkyAtmosphere will only update the transmittance and multiple scattering LUT when its state changes in order to save some GPU time.\n"),
	ECVF_RenderThreadSafe);
...
static TAutoConsoleVariable<int32> CVarSkyAtmosphereAsyncCompute(
	...
	TEXT("SkyAtmosphere on async compute (default: false). When running on the async pipe, SkyAtmosphere LUT generation will overlap with the occlusion pass.\n When enabled, opaque shadow map sampling on atmosphere will be disabled to avoid race conditions because these textures can be created in an overlaping parallel thread."),
	ECVF_RenderThreadSafe);
...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT(FSkyAtmosphereRealTimeReflectionLUTParameters, )
	SHADER_PARAMETER(FMatrix44f, SkyViewLutReferential)
	SHADER_PARAMETER(FVector4f, SkyPlanetTranslatedWorldCenterAndViewHeight)
	SHADER_PARAMETER(FVector3f, SkyCameraTranslatedWorldOrigin)
	SHADER_PARAMETER(float, RealTimeReflectionCapture)
END_GLOBAL_SHADER_PARAMETER_STRUCT()
...
{
	FSkyAtmosphereRealTimeReflectionLUTParameters Parameters;
	Parameters.SkyViewLutReferential = FMatrix44f::Identity;
	Parameters.SkyPlanetTranslatedWorldCenterAndViewHeight = FVector4f::Zero();
	Parameters.SkyCameraTranslatedWorldOrigin = FVector3f::Zero();
	Parameters.RealTimeReflectionCapture = 0.0f;
	return TUniformBufferRef<FSkyAtmosphereRealTimeReflectionLUTParameters>::CreateUniformBufferImmediate(Parameters, UniformBuffer_SingleDraw);
}
...
static bool GetSkyAtmosphereAsyncComputeEnabled()
{
	return CVarSkyAtmosphereAsyncCompute.GetValueOnRenderThread() != 0;
}

ESkyAtmospherePassLocation GetSkyAtmospherePassLocation()
{
	if (GetSkyAtmosphereAsyncComputeEnabled())
	{
...
bool ShouldSkySampleAtmosphereLightsOpaqueShadow(const FScene& Scene, const FVisibleLightInfoArray& VisibleLightInfos, SkyAtmosphereLightShadowData& LightShadowData, FDynamicShadowsTaskData* FilterDynamicShadowTaskData)
{
	...
	bool bCastShadowOnAtmosphereLight0 = LightShadowData.LightVolumetricShadowSceneinfo0 && LightShadowData.LightVolumetricShadowSceneinfo0->Proxy && LightShadowData.LightVolumetricShadowSceneinfo0->Proxy->GetCastShadowsOnAtmosphere();
	bool bCastShadowOnAtmosphereLight1 = LightShadowData.LightVolumetricShadowSceneinfo1 && LightShadowData.LightVolumetricShadowSceneinfo1->Proxy && LightShadowData.LightVolumetricShadowSceneinfo1->Proxy->GetCastShadowsOnAtmosphere();
	
	if (FilterDynamicShadowTaskData != nullptr && (bCastShadowOnAtmosphereLight0 || bCastShadowOnAtmosphereLight1))
	{
		// FilterDynamicShadows runs on a parallel task and writes = VisibleLightInfo.ShadowsToProject concurrently
		// Wait before accessing = VisibleLightInfo.ShadowsToProject in GetFirstWholeSceneShadowMap.
		extern void WaitForFilterDynamicShadowsTask(FDynamicShadowsTaskData* TaskData);
		WaitForFilterDynamicShadowsTask(FilterDynamicShadowTaskData);
	}
	
	if (bCastShadowOnAtmosphereLight0)
	{
		...
		if (Light0.VirtualShadowMapClipmaps.Num() > 0)
		{
			LightShadowData.VirtualShadowMapId0 = Light0.VirtualShadowMapClipmaps[0]->GetVirtualShadowMapId();
		}
		else
		{
			LightShadowData.VirtualShadowMapId0 = INDEX_NONE;
		}
	}
	if (bCastShadowOnAtmosphereLight1)
	{
		...
		if (Light1.VirtualShadowMapClipmaps.Num() > 0)
		{
			LightShadowData.VirtualShadowMapId1 = Light1.VirtualShadowMapClipmaps[0]->GetVirtualShadowMapId();
		}
		else
		{
			LightShadowData.VirtualShadowMapId1 = INDEX_NONE;
		}
	}
...
)
{
	...
	bool bSampleOpaqueShadow = bShouldSampleOpaqueShadow;
	if (GetSkyAtmosphereAsyncComputeEnabled())
	{
		// When enabling async compute, we can run RenderSkyAtmosphereLookupTables in parallel with FilterDynamicShadows, which allocates the depth buffers.
		// So when running sky atmosphere LUT generation on the async compute pipe, we disable opaque shadow sampling.
		bSampleOpaqueShadow = false;
	}

	if (bSampleOpaqueShadow && LightShadowData.LightVolumetricShadowSceneinfo0)
	{
	...
	if (bSampleOpaqueShadow && LightShadowData.LightVolumetricShadowSceneinfo1)
	{
...
void SkyAtmosphereOnRenderBegin(FScene& Scene, const FEngineShowFlags& EngineShowFlags)
{
	if (ShouldRenderSkyAtmosphere(&Scene, EngineShowFlags))
	{
...
static bool ShouldApplyAerialPerspective(EShaderPlatform ShaderPlatform)
{
	extern bool PlatformUsesBasicFogFeatures(EShaderPlatform ShaderPlatform);
	return PlatformUsesBasicFogFeatures(ShaderPlatform) == false;
}
	...
	:SkyAtmosphereSceneProxy(SkyAtmosphereSceneProxyIn)
{
	...
	// FSkyAtmosphereRenderSceneInfo must be created on the render thread to be able to create a multiframe uniform buffer. (see FScene::AddSkyAtmosphere)
	check(IsInRenderingThread());
	AtmosphereUniformBuffer = TUniformBufferRef<FAtmosphereUniformShaderParameters>::CreateUniformBufferImmediate(AtmosphereUniformShaderParameters, UniformBuffer_MultiFrame);
}
...
void FScene::RemoveSkyAtmosphere(FSkyAtmosphereSceneProxy* SkyAtmosphereSceneProxy)
{
	...
	ENQUEUE_RENDER_COMMAND(FRemoveSkyAtmosphereCommand)(
		[Scene, SkyAtmosphereSceneProxy](FRHICommandListImmediate& RHICmdList)
		{
			...
			SkyAtmosphereSceneProxy->RenderSceneInfo = nullptr;
...
class FRealTimeReflectionLUT : SHADER_PERMUTATION_BOOL("PERMUTATION_REALTIME_REFLECTION_LUT");
...
class FRenderSkyViewLutCS : public FGlobalShader
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FSampleCloudSkyAO, FSecondAtmosphereLight, FSampleOpaqueShadow, FSampleCloudShadow, FRealTimeReflectionLUT>;
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_STRUCT_REF(FSkyAtmosphereInternalCommonParameters, SkyAtmosphere)
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, ViewUniformBuffer)
		SHADER_PARAMETER_STRUCT_REF(FSkyAtmosphereRealTimeReflectionLUTParameters, SkyAtmosphereRealTimeReflectionLUTUniformBuffer)
...
class FRenderCameraAerialPerspectiveVolumeCS : public FGlobalShader
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FSampleCloudSkyAO, FSecondAtmosphereLight, FSampleOpaqueShadow, FSampleCloudShadow, FSeparateMieAndRayleighScattering, FRealTimeReflectionLUT>;
...
void InitSkyAtmosphereForScene(FRHICommandListImmediate& RHICmdList, FRDGBuilder& GraphBuilder, FScene* Scene)
{
	if (Scene)
	{
		...
		bool bCachedLUTsResourcesAreTheSame = true;
		...
		if (CVarSkyAtmosphereTransmittanceLUT.GetValueOnAnyThread() > 0)
		{
			...
			bCachedLUTsResourcesAreTheSame = GRenderTargetPool.FindFreeElement(RHICmdList, Desc, TransmittanceLutTexture, TEXT("SkyAtmosphere.TransmittanceLut")) && bCachedLUTsResourcesAreTheSame;
		}
		else
		{
		...
		bCachedLUTsResourcesAreTheSame = GRenderTargetPool.FindFreeElement(RHICmdList, Desc, MultiScatteredLuminanceLutTexture, TEXT("SkyAtmosphere.MultiScatteredLuminanceLut")) && bCachedLUTsResourcesAreTheSame;

		// If the resources have changed, we must recache them.
		if (!bCachedLUTsResourcesAreTheSame)
		{
			Scene->SetCachedTransmittanceAndMultiScatteringLUTsVersion(0);
		}
	...
	const FSkyAtmosphereRenderSceneInfo& SkyInfo)
{
	...
	InternalCommonParameters.BlueNoise = GetBlueNoiseGlobalParameters();
...
void FSceneRenderer::RenderSkyAtmosphereLookUpTables(FRDGBuilder& GraphBuilder, class FSkyAtmospherePendingRDGResources& PendingRDGResources, FDynamicShadowsTaskData* FilterDynamicShadowTaskData)
{
	...
	bool bEvaluateTransmittanceAndMultiScatteringLUTs = false;
	const FAtmosphereSetup& AtmosphereSetup = SkyAtmosphereSceneProxy.GetAtmosphereSetup();
	const uint32 IncomingTransmittanceAndMultiScatteringLUTsVersion = AtmosphereSetup.GetTransmittanceAndMultiScatteringLUTsVersion();
	uint32 CachedTransmittanceAndMultiScatteringLUTsVersion = Scene->GetCachedTransmittanceAndMultiScatteringLUTsVersion();
	...
	// Transmittance LUT
	FGlobalShaderMap* GlobalShaderMap = GetGlobalShaderMap(Scene->GetShaderPlatform());
	...
	// Multi-Scattering LUT
	if(bEvaluateTransmittanceAndMultiScatteringLUTs)
	{
		RDG_EVENT_SCOPE(GraphBuilder, "SkyAtmosphere::MultiScatteringLut");
	...
	if(CVarSkyAtmosphereDistantSkyLightLUT.GetValueOnRenderThread() > 0)
	{
		RDG_EVENT_SCOPE(GraphBuilder, "SkyAtmosphere::DistantSkyLightLut");
	...
	const bool bShouldSampleOpaqueShadow = ShouldSkySampleAtmosphereLightsOpaqueShadow(*Scene, VisibleLightInfos, LightShadowData, FilterDynamicShadowTaskData);
	...
	{
		FViewInfo& ViewForReflection = Views[0];
		const float AerialPerspectiveStartDepthInCm = GetValidAerialPerspectiveStartDepthInCm(ViewForReflection, SkyAtmosphereSceneProxy);
		...
		AtmosphereSetup.ComputeViewData(
			Scene->SkyLight->CapturePosition, ViewForReflection.ViewMatrices.GetPreViewTranslation(), SkyViewLutReferentialForward, SkyViewLutReferentialRight,
			SkyCameraTranslatedWorldOrigin, TempSkyPlanetData, SkyViewLutReferential);

		// Here, it would be convenient to create a new ViewUB using Scene->SkyLight->CapturePosition,
		// while setting up correct view parameters such as SkyPlanetTranslatedWorldCenterAndViewHeight, SkyViewLutReferential, etc.
		// But this is expenssive and can increase the cost of that function by 3 on low end platform.
		// As an optimisation to avoid creating a new View UB, shaders will fetch from constants what they need.
		FSkyAtmosphereRealTimeReflectionLUTParameters Parameters;
		Parameters.SkyViewLutReferential = SkyViewLutReferential;
		Parameters.SkyPlanetTranslatedWorldCenterAndViewHeight = TempSkyPlanetData;
		Parameters.SkyCameraTranslatedWorldOrigin = SkyCameraTranslatedWorldOrigin;
		Parameters.RealTimeReflectionCapture = 0.0f;
		TUniformBufferRef<FSkyAtmosphereRealTimeReflectionLUTParameters> SkyAtmosphereRealTimeReflectionLUTUB = TUniformBufferRef<FSkyAtmosphereRealTimeReflectionLUTParameters>::CreateUniformBufferImmediate(Parameters, UniformBuffer_SingleFrame);

		FVolumetricCloudRenderSceneInfo* CloudInfo = Scene->GetVolumetricCloudSceneInfo();
		FCloudShadowAOData CloudShadowAOData;
		GetCloudShadowAOData(CloudInfo, ViewForReflection, GraphBuilder, CloudShadowAOData);
		...
		{
			RDG_EVENT_SCOPE(GraphBuilder, "SkyAtmosphere::RealTimeCaptureSkyViewLut");
			FIntVector SkyViewLutSize = ViewForReflection.SkyAtmosphereViewLutTexture->GetDesc().GetSize();
			...
			PermutationVector.Set<FRealTimeReflectionLUT>(true);
			TShaderMapRef<FRenderSkyViewLutCS> ComputeShader(ViewForReflection.ShaderMap, PermutationVector);

			FRenderSkyViewLutCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FRenderSkyViewLutCS::FParameters>();
			PassParameters->Atmosphere = Scene->GetSkyAtmosphereSceneInfo()->GetAtmosphereUniformBuffer();
			PassParameters->SkyAtmosphere = SkyInfo.GetInternalCommonParametersUniformBuffer();
			PassParameters->ViewUniformBuffer = ViewForReflection.ViewUniformBuffer;
			PassParameters->SkyAtmosphereRealTimeReflectionLUTUniformBuffer = SkyAtmosphereRealTimeReflectionLUTUB;
			PassParameters->TransmittanceLutTextureSampler = SamplerLinearClamp;
			PassParameters->MultiScatteredLuminanceLutTextureSampler = SamplerLinearClamp;
		...
		{
			RDG_EVENT_SCOPE(GraphBuilder, "SkyAtmosphere::RealTimeCaptureCamera360VolumeLut");
			FIntVector CameraAPLutSize = ViewForReflection.SkyAtmosphereCameraAerialPerspectiveVolume->GetDesc().GetSize();
			EPixelFormat TextureAerialLUTFormat = (CVarSkyAtmosphereLUT32.GetValueOnAnyThread() != 0) ? PF_A32B32G32R32F : PF_FloatRGBA;
			...
			PermutationVector.Set<FRealTimeReflectionLUT>(true);
			TShaderMapRef<FRenderCameraAerialPerspectiveVolumeCS> ComputeShader(ViewForReflection.ShaderMap, PermutationVector);
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		// Sky View LUT
		{
			RDG_EVENT_SCOPE(GraphBuilder, "SkyAtmosphere::SkyViewLut");
			FRenderSkyViewLutCS::FPermutationDomain PermutationVector;
			PermutationVector.Set<FSampleCloudSkyAO>(CloudShadowAOData.bShouldSampleCloudSkyAO);
			PermutationVector.Set<FSecondAtmosphereLight>(bSecondAtmosphereLightEnabled);
			PermutationVector.Set<FSampleOpaqueShadow>(bShouldSampleOpaqueShadow);
			PermutationVector.Set<FSampleCloudShadow>(bShouldSampleCloudShadow);
			PermutationVector.Set<FRealTimeReflectionLUT>(false);
			...
			PassParameters->SkyAtmosphereRealTimeReflectionLUTUniformBuffer = GetDefaultSkyAtmosphereRealTimeReflectionLUTUniformBuffer();
			PassParameters->TransmittanceLutTextureSampler = SamplerLinearClamp;
			PassParameters->MultiScatteredLuminanceLutTextureSampler = SamplerLinearClamp;
			PassParameters->VolumetricCloudShadowMapTexture0Sampler = SamplerLinearClamp;
			PassParameters->VolumetricCloudShadowMapTexture1Sampler = SamplerLinearClamp;
		...
		{
			RDG_EVENT_SCOPE(GraphBuilder, "SkyAtmosphere::CameraVolumeLut");
			FRenderCameraAerialPerspectiveVolumeCS::FPermutationDomain PermutationVector;
			PermutationVector.Set<FSampleCloudSkyAO>(CloudShadowAOData.bShouldSampleCloudSkyAO);
			PermutationVector.Set<FSecondAtmosphereLight>(bSecondAtmosphereLightEnabled);
			PermutationVector.Set<FSampleOpaqueShadow>(bShouldSampleOpaqueShadow);
...
void FSkyAtmospherePendingRDGResources::CommitToSceneAndViewUniformBuffers(FRDGBuilder& GraphBuilder, FRDGExternalAccessQueue& ExternalAccessQueue) const
{
	...
	// We always need to run the conversion even if TransmittanceLut has not been written to, as long as RegisterExternalTexture has been called on it. This in order to finish any asynchronous resource transition.
	...
	FSkyAtmosphereRenderContext& SkyRC)
{
	...
	// Render the sky, and optionally the atmosphere aerial perspective, on the scene luminance buffer
	{
		...
		TShaderMapRef<FRenderSkyAtmospherePS> PixelShader(GetGlobalShaderMap(SkyRC.ShaderPlatform), PsPermutationVector);

		FRenderSkyAtmosphereVS::FPermutationDomain VsPermutationVector;
		TShaderMapRef<FRenderSkyAtmosphereVS> VertexShader(GetGlobalShaderMap(SkyRC.ShaderPlatform), VsPermutationVector);
		...
		{
			const FMatrix ProjectionMatrix = ViewMatrices.GetViewToClip();
...
void FSceneRenderer::RenderSkyAtmosphere(FRDGBuilder& GraphBuilder, const FMinimalSceneTextures& SceneTextures)
{
	...
	check(ShouldRenderSkyAtmosphere(Scene, ViewFamily.EngineShowFlags)); // This should not be called if we should not render SkyAtmosphere
	check(ViewFamily.EngineShowFlags.DeferredAtmospherePass);
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		SkyRC.ShaderPlatform = View.GetShaderPlatform();
	...
	if (CVarSkyAtmosphereFastSkyLUT.GetValueOnAnyThread() == 0 && CVarSkyAtmosphereAerialPerspectiveApplyOnOpaque.GetValueOnAnyThread() > 0)
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
				...
				[&View](FCanvas& Canvas)
			{
				...
				FString Text = "You are using a FastAerialPespective without FastSky, visuals might look wrong.";
