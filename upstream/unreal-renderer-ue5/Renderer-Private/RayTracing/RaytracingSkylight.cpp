...
bool ShouldRenderRayTracingSkyLight(const FSkyLightSceneProxy* SkyLightSceneProxy, EShaderPlatform ShaderPlatform)
{
	...
	return bRayTracingSkyEnabled && ShouldRenderRayTracingEffect(true, ERayTracingPipelineCompatibilityFlags::FullPipeline, ShaderPlatform) && (GetSkyLightSamplesPerPixel(SkyLightSceneProxy) > 0);
}
	...
	FRDGTextureRef& OutHitDistanceTexture)
{
	...
	for (FViewInfo& View : Views)
	{
		...
		TShaderMapRef<FRayTracingSkyLightRGS> RayGenerationShader(GetGlobalShaderMap(Scene->GetShaderPlatform()), PermutationVector);
