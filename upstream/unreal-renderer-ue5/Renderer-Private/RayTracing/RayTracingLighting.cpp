	...
	bool bBuildLightGrid)
{
	...
	if (bBuildLightGrid)
	{
		...
		for (const FLightSceneInfoCompact& Light : Scene->Lights)
		{
			...
			const FVector2D FadeParams = GetDirectionalLightDistanceFadeParameters(*Light.LightSceneInfo, View.GetFeatureLevel(), false, View.MaxShadowCascades);
	...
	)
{
	...
	{
		MissShaderPipelineIndex = FindRayTracingMissShaderIndex(View.MaterialRayTracingData.PipelineState, GetRayTracingDefaultMissShader(View.ShaderMap), true);
	}
