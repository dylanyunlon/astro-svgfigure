...
static TAutoConsoleVariable<bool> CVarRayTracingVisualizeOpaqueOnly(
	TEXT("r.RayTracing.Visualize.OpaqueOnly"),
	true,
	TEXT("Whether to only show opaque instances in ray tracing debug visualizations."),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<bool> CVarRayTracingVisualizeFarField(
	TEXT("r.RayTracing.Visualize.FarField"),
	false,
	TEXT("Whether to show far field geometry instead of near field in ray tracing visualizations."),
	ECVF_RenderThreadSafe
);
...
bool IsRayTracingVisualizeOpaqueOnly()
{
	return CVarRayTracingVisualizeOpaqueOnly.GetValueOnRenderThread();
}
...
class FRayTracingDebugRGS : public FGlobalShader
{
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		if (bUseNvAPITimestamp && 
			(IsVulkanPlatform(Parameters.Platform) || IsMetalPlatform(Parameters.Platform)))
		{
			return false;
		}
...
static FRDGBufferRef RayTracingPerformPicking(
	...
	const FViewInfo& View,
	TConstArrayView<FRayTracingShaderBindingData> VisibleRayTracingShaderBindings,
	FRayTracingPickingFeedback& PickingFeedback, ERayTracingSceneLayer LayerToTrace, bool bInlineRayTracing)
{
	...
	SharedParameters.InstancesExtraData = GraphBuilder.CreateSRV(RayTracingScene.GetInstanceExtraDataBuffer(LayerToTrace, View.GetRayTracingSceneViewHandle()));
	SharedParameters.InstancesDebugData = GraphBuilder.CreateSRV(RayTracingScene.GetInstanceDebugBuffer(LayerToTrace));
	SharedParameters.TLAS = RayTracingScene.GetLayerView(LayerToTrace, View.GetRayTracingSceneViewHandle());
	SharedParameters.OpaqueOnly = IsRayTracingVisualizeOpaqueOnly() ? 1 : 0;
	SharedParameters.InstanceBuffer = GraphBuilder.CreateSRV(RayTracingScene.GetInstanceBuffer(LayerToTrace, View.GetRayTracingSceneViewHandle()));
...
static FRDGBufferRef RayTracingPerformHitStatsPerPrimitive(
	FRDGBuilder& GraphBuilder,
	const FScene& Scene,
	const FViewInfo& View,
	TConstArrayView<FRayTracingShaderBindingData> VisibleRayTracingShaderBindings,
	ERayTracingSceneLayer LayerToTrace)
{
	const FRayTracingScene& RayTracingScene = Scene.RayTracingScene;
	...
	const uint32 NumInstancesInTLAS = FMath::Max(RayTracingScene.GetRHIRayTracingSceneChecked(LayerToTrace, View.GetRayTracingSceneViewHandle())->GetInitializer().MaxNumInstances, (uint32)CVarRayTracingVisualizeHitCountTopKHits.GetValueOnRenderThread());
	...
	RayGenParameters->TLAS = RayTracingScene.GetLayerView(LayerToTrace, View.GetRayTracingSceneViewHandle());
	RayGenParameters->OpaqueOnly = IsRayTracingVisualizeOpaqueOnly() ? 1 : 0;
...
static void DrawInstanceOverlap(FRDGBuilder& GraphBuilder, const FScene& Scene, const FViewInfo& View, FRDGTextureRef SceneColorTexture, FRDGTextureRef InputDepthTexture, ERayTracingSceneLayer LayerToTrace)
{
	...
	RayTracingDrawInstances(
		...
		SceneDepthTexture,
		Scene.RayTracingScene.GetInstanceExtraDataBuffer(LayerToTrace, View.GetRayTracingSceneViewHandle()),
		Scene.RayTracingScene.GetNumNativeInstances(LayerToTrace, View.GetRayTracingSceneViewHandle()),
		false);
...
extern void RenderRayTracingBarycentrics(FRDGBuilder& GraphBuilder, const FScene& Scene, const FViewInfo& View, FRDGTextureRef SceneColor, ERayTracingSceneLayer LayerToTrace, bool bVisualizeProceduralPrimitives, bool bOutputTiming);
...
extern void RenderRayTracingPrimaryRaysView(
	...
	ERayTracingSceneLayer LayerToTrace, int32 SamplePerPixel, int32 HeightFog, float ResolutionFraction, ERayTracingPrimaryRaysFlag Flags);
...
void RenderRayTracingDebug(FRDGBuilder& GraphBuilder, const FScene& Scene, const FViewInfo& View, FSceneTextures& SceneTextures, TConstArrayView<FRayTracingShaderBindingData> VisibleRayTracingShaderBindings, FRayTracingPickingFeedback& PickingFeedback)
{
	...
	const ERayTracingSceneLayer LayerToTrace = CVarRayTracingVisualizeFarField.GetValueOnRenderThread() ? ERayTracingSceneLayer::FarField : ERayTracingSceneLayer::Base;
	...
	{
		return RenderRayTracingBarycentrics(GraphBuilder, Scene, View, SceneColorTexture, LayerToTrace, (bool)GVisualizeProceduralPrimitives, /*bOutputTiming*/ DebugVisualizationMode == RAY_TRACING_DEBUG_VIZ_TIMING_TRAVERSAL);
	}
	...
	if (IsRayTracingDebugTraversalMode(DebugVisualizationMode) && ShouldRenderRayTracingEffect(true, ERayTracingPipelineCompatibilityFlags::Inline, View))
	{
		...
		PassParameters->TLAS = RayTracingScene.GetLayerView(LayerToTrace, View.GetRayTracingSceneViewHandle());
	...
	if (DebugVisualizationMode == RAY_TRACING_DEBUG_VIZ_PRIMARY_RAYS)
	{
		...
		RenderRayTracingPrimaryRaysView(
			...
			&OutputColor, &HitDistanceTexture, LayerToTrace, 1, 1, 1, ERayTracingPrimaryRaysFlag::PrimaryView);
	...
	if (IsRayTracingPickingEnabled(DebugVisualizationMode)
		&& RayTracingScene.GetInstanceExtraDataBuffer(LayerToTrace, View.GetRayTracingSceneViewHandle()) != nullptr
		&& RayTracingScene.GetInstanceDebugBuffer(LayerToTrace) != nullptr)
	{
		PickingBuffer = RayTracingPerformPicking(GraphBuilder, Scene, View, VisibleRayTracingShaderBindings, PickingFeedback, LayerToTrace, bInlineRayTracing);
	}
	else
	{
	...
	{
		StatsBuffer = RayTracingPerformHitStatsPerPrimitive(GraphBuilder, Scene, View, VisibleRayTracingShaderBindings, LayerToTrace);
	}
	else 
	{
	...
	FRDGBufferRef InstanceExtraDataBuffer = RayTracingScene.GetInstanceExtraDataBuffer(LayerToTrace, View.GetRayTracingSceneViewHandle());
	...
	FRDGBufferRef InstanceDebugBuffer = RayTracingScene.GetInstanceDebugBuffer(LayerToTrace);
	...
	FRDGBufferRef InstanceBuffer = RayTracingScene.GetInstanceBuffer(LayerToTrace, View.GetRayTracingSceneViewHandle());
	...
	const uint32 NumInstances = RayTracingScene.GetNumNativeInstances(LayerToTrace, View.GetRayTracingSceneViewHandle());
	...
	{
		DrawInstanceOverlap(GraphBuilder, Scene, View, SceneColorTexture, OutputDepthTexture, LayerToTrace);
	}
...
void RayTracingDebugDisplayOnScreenMessages(FScreenMessageWriter& Writer, const FViewInfo& View, bool bViewHasFarFieldInstances)
{
	const uint32 DebugVisualizationMode = GetRaytracingDebugViewModeID(View);
	...
	if (CVarRayTracingVisualizeFarField.GetValueOnRenderThread())
	{
		if (!View.RayTracingCullingParameters.bIsRayTracingFarField)
		{
			static const FText Message = NSLOCTEXT("Renderer", "RayTracingDebugVizFarFieldNotEnabled", "Ray Tracing Far Field is automatically disabled as there are no features using it.");
			Writer.DrawLine(Message);
		}
		else if (!bViewHasFarFieldInstances)
		{
			static const FText Message = NSLOCTEXT("Renderer", "RayTracingDebugVizFarFieldEmpty", "Ray Tracing Far Field doesn't have any instances.");
			Writer.DrawLine(Message);
		}
	}
}
...
void FDeferredShadingSceneRenderer::RayTracingDisplayPicking(const FRayTracingPickingFeedback& PickingFeedback, FScreenMessageWriter& Writer)
{
	...
	for (const FRayTracingGeometryInstance& Instance : Scene->RayTracingScene.GetInstances(LayerToTrace))
	{
	...
	if (PickingFeedback.Mask & RAY_TRACING_MASK_OPAQUE_REFLECTION)
	{
		Writer.DrawLine(FText::FromString(TEXT("   RAY_TRACING_MASK_OPAQUE_REFLECTION")), 10, FColor::Yellow);
	}
