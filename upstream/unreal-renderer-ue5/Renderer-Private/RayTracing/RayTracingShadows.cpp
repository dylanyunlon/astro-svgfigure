...
class FOcclusionCS : public FGlobalShader
{
	...
	{
		// Disable precaching for occlusion CS - should use project setting if it's used?
...
#if RHI_RAYTRACING
{
	...
	// Ray generation pass for shadow occlusion.
	{
		...
		else
		{
			...
			TShaderMapRef<FOcclusionRGS> RayGenerationShader(GetGlobalShaderMap(Scene->GetShaderPlatform()), PermutationVector);
