...
// Workaround for UE-300339
static TAutoConsoleVariable<int32> CVarShadowStencilCullingArtifactWorkaround(
	TEXT("r.Shadow.StencilCullingArtifactWorkaround"),
	1,
	TEXT("Workaround for a seemingly platform specific artifact (tile shaped artifacts) when both Substrate and r.Shadow.StencilCulling are enabled. The workaround forces a dummy pixel shader with late-Z in the stencil masking pass, which seems to fix the bug."),
	ECVF_RenderThreadSafe);
	...
	bool bMobileModulatedProjections)
{
	...
	{
		// See also GetShadowTermsBase from DeferredLightingCommon.ush.
...
class FLateZDummyPS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FLateZDummyPS);
	SHADER_USE_PARAMETER_STRUCT(FLateZDummyPS, FGlobalShader)
	...
	const FInstanceCullingDrawParams& InstanceCullingDrawParams) const
{
	...
	if (bPreShadow || bSelfShadowOnly)
	{
		...
		//       This means doing 2x renders, but letting the scissor rect kill the undesired half. Drawing the full mask once is easy, but since the outer
		...
		if (bIsInstancedStereo && ProjectionStencilingPasses.IsValidIndex(View->PrimaryViewIndex))
		{
	...
	else if (IsWholeSceneDirectionalShadow())
	{
		...
		const FVector4 Near = View->ViewMatrices.GetViewToClip().TransformFVector4(FVector4(0, 0, CascadeSettings.SplitNear));
		const FVector4 Far = View->ViewMatrices.GetViewToClip().TransformFVector4(FVector4(0, 0, CascadeSettings.SplitFar));
	...
	else
	{
		...
		TShaderRef<FLateZDummyPS> PixelShader;

		GraphicsPSOInit.BoundShaderState.VertexDeclarationRHI = GetVertexDeclarationFVector4();
		GraphicsPSOInit.BoundShaderState.VertexShaderRHI = VertexShader.GetVertexShader();
		if (CVarShadowStencilCullingArtifactWorkaround.GetValueOnRenderThread() != 0 && IsRHIDeviceNVIDIA())
		{
			PixelShader = View->ShaderMap->GetShader<FLateZDummyPS>();
			GraphicsPSOInit.BoundShaderState.PixelShaderRHI = PixelShader.GetPixelShader();
		}
		else
		{
		...
		// TODO: Support instanced stereo properly in the projection stenciling pass.
		const bool bIsInstancedStereo = View->IsInstancedStereo();
		const uint32 NumberOfInstances = bIsInstancedStereo ? 1 : View->GetStereoPassInstanceFactor();
	...
	bool bSubPixelShadow) const
{
	...
	FRDGTextureRef OutputTexture0 = CommonPassParameters.RenderTargets[0].GetTexture();
	const ERDGPassFlags PassFlags = OutputTexture0 == nullptr ? ERDGPassFlags::SkipRenderPass : ERDGPassFlags::None;
	...
	*PassParameters = CommonPassParameters;
	...
	// LoadAction from CommonPassParameters have to be adjusted depending on whether this is a first write to a target
	PassParameters->RenderTargets[0].SetLoadAction(GetLoadActionIfProduced(OutputTexture0, ERenderTargetLoadAction::EClear));
	...
	else if (bIsInstancedStereo && (bPreShadow || bSelfShadowOnly))
	{
	...
	GraphBuilder.AddPass(
		RDG_EVENT_NAME("ShadowProjection %s", *EventName),
		PassParameters,
	...
	FRHIUniformBuffer* HairStrandsUniformBuffer) const
{
	...
	const bool bDepthBoundsTestEnabled = IsWholeSceneDirectionalShadow() && GSupportsDepthBoundsTest && CVarCSMDepthBoundsTest.GetValueOnRenderThread() != 0;// && !bSubPixelSupport;
	const bool bSubPixelSupport = !bDepthBoundsTestEnabled && HairStrandsUniformBuffer != nullptr;// HairStrands::HasViewHairStrandsData(*View);
	...
	{
		// When CSM depth reuse splits cascades into static and dynamic regions, the dynamic-only
		// region must not use the fade plane blend mode. The fade plane blends between cascades
		// using an overwrite mode, which would incorrectly discard the static region's contribution
		// that was rendered into the same atlas from a previous frame.
		const int32 CascadeMask = CVarCSMShadowDepthReuseDynamicCascadeMask.GetValueOnRenderThread();
		bool bIsSplitDynamicCascade = (CVarCSMShadowDepthReuseSeparateDynamic.GetValueOnRenderThread() != 0)
			&& bIsDynamicOnly
			&& CSMShadowDepthReuseIndex >= 0
			&& (CascadeMask == -1 || (CascadeMask & (1 << CascadeSettings.ShadowSplitIndex)));

		GraphicsPSOInit.BlendState = FProjectedShadowInfo::GetBlendStateForProjection(
			GetLightSceneInfo().GetDynamicShadowMapChannel(),
			IsWholeSceneDirectionalShadow(),
			CascadeSettings.FadePlaneLength > 0 && !bRayTracedDistanceField && !bIsSplitDynamicCascade,
			bProjectingForForwardShading,
			bMobileModulatedProjections);
	}
...

	{
		...
		if (bDepthBoundsTestEnabled)
		{
			SetDepthBoundsTest(RHICmdList, CascadeSettings.SplitNear, CascadeSettings.SplitFar, View->ViewMatrices.GetViewToClip());
		}
	}
...
void FProjectedShadowInfo::RenderFrustumWireframe(FPrimitiveDrawInterface* PDI) const
{
	...
	{
		Color = FLinearColor::MakeFromHSV8(( ( SubjectPrimitiveId + LightSceneInfo->Id.GetIndex() ) * 31 ) & 255, 0, 255).ToFColor(true);
	}
...
FMatrix FProjectedShadowInfo::GetScreenToShadowMatrix(const FSceneView& View, uint32 TileOffsetX, uint32 TileOffsetY, uint32 TileResolutionX, uint32 TileResolutionY) const
{
	...
	FMatrix ViewDependentTransform =
		...
		View.ViewMatrices.GetClipToTranslatedWorld() *
	...
	if (View.UsesMobileMultiViewShaders())
	{
...
bool FSceneRenderer::CheckForProjectedShadows( const FLightSceneInfo* LightSceneInfo ) const
{
	...
	const ELightOcclusionType LightOcclusionType = GetLightOcclusionType(*LightSceneInfo->Proxy, ViewFamily);
	if (LightOcclusionType == ELightOcclusionType::Raytraced)
		return true;
	...
	bool bProjectingForForwardShading)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		const FViewInfo& View = Views[ViewIndex];
		const bool bIsInstancedStereo = View.IsInstancedStereo();
		if ((!bIsInstancedStereo && !View.ShouldRenderView()) || (bSubPixelShadow && !HairStrands::HasViewHairStrandsData(View)))
		{
			continue;
		}
		...
		CommonPassParameters.RenderTargets[0] = FRenderTargetBinding(OutputTexture, GetLoadActionIfProduced(OutputTexture, ERenderTargetLoadAction::EClear));
				...
				for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
				{
					...
					if (View.IsPrimarySceneView() && LightSceneInfo->ShouldRenderLight(View))
					{
						...
						if (DependentView == nullptr || DependentView == &View)
						{
							...
							Collector.AddLightForInjection(View, ViewIndex, LightGridViewData, VisibleLightInfos, *LightSceneInfo, ProjectedShadowInfo);
	...
	if (!bInjectedShadowedLight)
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			...
			if (View.IsPrimarySceneView() && LightSceneInfo->ShouldRenderLight(View))
			{
				...
				Collector.AddLightForInjection(View, ViewIndex, LightGridViewData, VisibleLightInfos, *LightSceneInfo);
			}
		}
	}
}
...
void FMobileSceneRenderer::ClearMobileScreenSpaceShadows(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FLightSceneProxy* LightSceneProxy, FRDGTextureRef ScreenShadowMaskTexture)
{
	FRenderTargetParameters* ClearPassParameters = GraphBuilder.AllocParameters<FRenderTargetParameters>();
	ClearPassParameters->RenderTargets[0] = FRenderTargetBinding(ScreenShadowMaskTexture, ERenderTargetLoadAction::ENoAction);

	GraphBuilder.AddPass(
		RDG_EVENT_NAME("ScreenSpaceShadowMaskClearQuad"),
		ClearPassParameters,
		ERDGPassFlags::Raster,
		[&View, LightSceneProxy](FRHICommandList& RHICmdList)
		{
			FIntRect ScissorRect;
			if (!LightSceneProxy->GetScissorRect(ScissorRect, View, View.ViewRect))
			{
...
void FMobileSceneRenderer::RenderMobileScreenSpaceShadows(FRDGBuilder& GraphBuilder, FIntPoint Extent, FRDGTextureRef ScreenShadowMaskTexture)
{
	static auto* ContactShadowsCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.ContactShadows"));
	if (ScreenShadowMaskTexture == nullptr
		|| IsOpenGLPlatform(ShaderPlatform) // FIXME: GL does not support writing to rg32f images
		|| (ContactShadowsCVar && ContactShadowsCVar->GetValueOnRenderThread() == 0))
	{
		return;
	}
		
	RDG_RHI_EVENT_SCOPE(GraphBuilder, RenderMobileScreenSpaceShadows);
...
FRDGTextureRef FMobileSceneRenderer::RenderMobileShadowProjections(FRDGBuilder& GraphBuilder, FIntPoint Extent, bool bIsMobileMultiView)
{
	RDG_RHI_EVENT_SCOPE(GraphBuilder, RenderMobileShadowProjections);
	
	FRDGTextureDesc TargetDesc = FRDGTextureDesc::CreateRenderTargetTextureDesc(Extent, PF_B8G8R8A8, FClearValueBinding::White, TexCreate_RenderTargetable | TexCreate_ShaderResource, bIsMobileMultiView);
	FRDGTextureRef ScreenShadowMaskTexture = GraphBuilder.CreateTexture(TargetDesc, TEXT("ScreenSpaceShadowMaskTextureMobile"));

	ScreenShadowMaskTexture->EncloseVisualizeExtent(FamilySize);
	...
	for (const FLightSceneInfoCompact& LightSceneInfoCompact : Scene->Lights)
	{
		...
		const bool bProjectingForForwardShading = !(IsMobileDeferredShadingEnabled(ShaderPlatform) && LightSceneProxy->GetLightType() == LightType_Point);

		// Other than point light, local light shadows don't render to shadow mask texture on mobile deferred
		if (LightSceneProxy->GetLightType() == LightType_Directional || LightSceneProxy->GetLightType() == LightType_Point || !IsMobileDeferredShadingEnabled(ShaderPlatform))
		{
	...
	return ScreenShadowMaskTexture;
}
