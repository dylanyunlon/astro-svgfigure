...
static bool PlatformSupportsRectLightAtlas(FStaticShaderPlatform Platform)
{
	// Mobile platforms currently do not suppport rect light source texture
	// See USE_SOURCE_TEXTURE in RectLight.ush
...
static bool SupportWaveQuadPassing(EShaderPlatform In)
{
	return GRHISupportsWaveOperations && RHISupportsWaveOperations(In);
}
...
public:
		static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters) { return PlatformSupportsRectLightAtlas(Parameters.Platform); }
...
public:
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters) { return ShaderPrint::IsSupported(Parameters.Platform) && PlatformSupportsRectLightAtlas(Parameters.Platform); }
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		FGlobalShader::ModifyCompilationEnvironment(Parameters, OutEnvironment);
...
class FRectAtlasAddTexturePS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, OutAtlasTextureMip1)
	...
	FRDGTextureRef& OutAtlas)
{
	...
	// If compiler support quad pass, use it to generate first MIP during atlas copy
	const bool bUseQuadPassing = SupportWaveQuadPassing(ShaderMap->GetPlatform());
	...
	for (uint32 PassIt = 0; PassIt < PassCount; ++PassIt)
	{
		...
		if (bUseQuadPassing)
		{
			Parameters->OutAtlasTextureMip1 = GraphBuilder.CreateUAV(FRDGTextureUAVDesc(OutAtlas, 1));
		}
...
class FRectAtlasFilterTexturePS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, bWriteMIP1)
	...
	FRDGTextureRef& InAtlas)
{
	...
	// If QuadPass is available generate two MIPs at a time when using box filtering
	const uint32 FilteringQuality = FMath::Clamp(CVarRectLighFilterQuality.GetValueOnRenderThread(), 0, 1);
	const bool bUseQuadPassing = SupportWaveQuadPassing(ShaderMap->GetPlatform()) && FilteringQuality == 0;

	FRDGTextureRef DummyTexture = GraphBuilder.CreateTexture(FRDGTextureDesc::Create2D(FIntPoint(1, 1), InAtlas->Desc.Format, FClearValueBinding::Black, TexCreate_UAV), TEXT("RectLight.DummyAtlasMIP"));
	FRDGTextureUAVRef DummyTextureUAV = GraphBuilder.CreateUAV(DummyTexture);

	const uint32 MipCount = InAtlas->Desc.NumMips; // FMath::Log2(FMath::Min(InAtlas->Desc.Extent.X, InAtlas->Desc.Extent.Y));
	for (uint32 DstMip = (bUseQuadPassing ? 2u : 1u); DstMip < MipCount; DstMip += (bUseQuadPassing ? 2u : 1u))
	{
		...
		if (SlotCount > 0)
		{
			...
			Parameters->FilterQuality		= FilteringQuality;
			...
			Parameters->VS.ScaleOffsetBuffer= DummyScaleOffsetBuffer;
			Parameters->VS.bHasScaleOffset  = false;
			Parameters->bWriteMIP1			= (bUseQuadPassing && DstMip+1 < InAtlas->Desc.NumMips) ? 1u : 0u;
			Parameters->OutAtlasTextureMip1 = bUseQuadPassing && Parameters->bWriteMIP1 ? GraphBuilder.CreateUAV(FRDGTextureUAVDesc(InAtlas, DstMip+1)) : DummyTextureUAV;
...
uint32 AddTexture(FStaticShaderPlatform Platform, UTexture* In, const FVector4f& ScaleOffset)
{
	check(IsInRenderingThread());

	uint32 SlotIndex = InvalidSlotIndex;
	if (In && PlatformSupportsRectLightAtlas(Platform))
	{
...
{
	return AddTexture(GMaxRHIShaderPlatform, In, ScaleOffset);
}
...
{
	if (GRectLightTextureManager.bLock || !PlatformSupportsRectLightAtlas(GetFeatureLevelShaderPlatform(FeatureLevel)))
	{
		return;
	}
	...
	static int32 CachedFilteringMethod = CVarRectLighFilterQuality.GetValueOnRenderThread();
	const EPixelFormat AtlasFormat = GetRectLightAtlasFormat();
	const bool bForceUpdate = CVarRectLighForceUpdate.GetValueOnRenderThread() > 0 || CachedMaxAtlasResolution != CVarRectLightTextureResolution.GetValueOnRenderThread() || CachedAtlasFormat != AtlasFormat || CachedFilteringMethod != CVarRectLighFilterQuality.GetValueOnRenderThread();
	...
	auto ResetAtlas = [&]()
	{
		...
		CachedFilteringMethod = CVarRectLighFilterQuality.GetValueOnRenderThread();
		GRectLightTextureManager.bHasPendingAdds = true;
	...
	if (FRDGTexture* AtlasTexture = GraphBuilder.FindExternalTexture(GRectLightTextureManager.AtlasTexture))
	{
...
void AddDebugPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureRef OutputTexture)
{
	if (CVarRectLighTextureDebug.GetValueOnRenderThread() > 0 
		&& ShaderPrint::IsSupported(View.Family->GetShaderPlatform()) 
		&& PlatformSupportsRectLightAtlas(View.Family->GetShaderPlatform()))
	{
