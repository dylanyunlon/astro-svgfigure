	...
	switch (CullingMethod)
	{
		...
		default:
		{
			if (CullingMethod >= 1 && CullingMethod <= 3)
			{
				...
				FMatrix Receiver(ProjectedShadowInfo->InvReceiverInnerMatrix);
				DrawFrustumWireframe(&ShadowFrustumPDI, Receiver * FTranslationMatrix(-ProjectedShadowInfo->PreShadowTranslation), FColor::Cyan, 0);
			}
		}
		break;
	}
...
FDynamicShadowsTaskData* FMobileSceneRenderer::InitDynamicShadows(FRDGBuilder& GraphBuilder, FInstanceCullingManager& InstanceCullingManager)
{
	...
	for (FLightSceneInfo* MobileDirectionalLightSceneInfo : Scene->MobileDirectionalLights)
	{
		...
		if (bShouldRecordShadowSubjectsForMobile)
		{
			...
			for (auto& View : Views)
			{
				// Mirrors logic from FScene::GetMobileDirectionalLightForView, see comment there for more info
				if (MobileDirectionalLightSceneInfo->Proxy->GetViewLightingChannelMask() & View.ViewLightingChannelMask)
				{
...
// Build a single FProjectedShadowInfo whose bounds encompass the entire CSM receiver range
// (near + far cascades), used for the "is this primitive a CSM receiver?" test.
bool BuildSingleCascadeShadowInfo(FViewInfo &View, FVisibleLightInfoArray& VisibleLightInfos, FLightSceneInfo* LightSceneInfo, FProjectedShadowInfo& OUTSingleCascadeInfo)
{
	const FLightSceneProxy* Proxy = LightSceneInfo->Proxy;
	const bool bPrecomputedLightingIsValid = LightSceneInfo->IsPrecomputedLightingValid();

	const float SplitNear = View.NearClippingDistance;
	const float SplitFar = GetMaxShadowReceiverDistance(*LightSceneInfo, View, bPrecomputedLightingIsValid);
	...
	FShadowCascadeSettings& Cascade = WholeSceneInitializer.CascadeSettings;

	FSphere Bounds = GetShadowSplitBoundsDepthRange(*LightSceneInfo, View, View.ViewMatrices.GetViewOrigin(), SplitNear, SplitFar, &Cascade);
	...
	WholeSceneInitializer.PreShadowTranslation      = -Bounds.Center;
	WholeSceneInitializer.WorldToLight              = FInverseRotationMatrix(Proxy->GetDirection().GetSafeNormal().Rotation());
	WholeSceneInitializer.Scales                    = FVector2D(1.0f / Bounds.W, 1.0f / Bounds.W);
	WholeSceneInitializer.SubjectBounds             = FBoxSphereBounds(FVector::ZeroVector, SubjectBounds.BoxExtent, SubjectBounds.SphereRadius);
	WholeSceneInitializer.WAxis                     = FVector4(0, 0, 0, 1);
	WholeSceneInitializer.MinLightW                 = FMath::Min<float>(-UE_OLD_HALF_WORLD_MAX, -SubjectBounds.SphereRadius);
	WholeSceneInitializer.MaxDistanceToCastInLightW = SubjectBounds.SphereRadius - WholeSceneInitializer.MinLightW;
	WholeSceneInitializer.bRayTracedDistanceField   = false;

	Cascade.SplitNear         = SplitNear;
	Cascade.SplitFar          = SplitFar;
	Cascade.FadePlaneOffset   = SplitFar;
	Cascade.FadePlaneLength   = 0.0f;
	Cascade.ShadowSplitIndex  = 0;
	Cascade.bFarShadowCascade = false;

	const FProjectedShadowInfo* Existing = VisibleLightInfo.AllProjectedShadows[0];
	OUTSingleCascadeInfo.SetupWholeSceneProjection(
		LightSceneInfo,
		&View,
		WholeSceneInitializer,
		Existing->ResolutionX,
		Existing->ResolutionY,
		Existing->ResolutionX,
		Existing->ResolutionY,
		Existing->BorderSize
	);

	// SetupWholeSceneProjection re-queries ShadowBounds from the proxy using cascade 0,
	// which loses the full near+far CSM range. Restore the sphere we actually built from.
	OUTSingleCascadeInfo.ShadowBounds = FSphere(Bounds.Center, Bounds.W);
	return true;
}
...
void FMobileSceneRenderer::BuildCSMVisibilityState(FLightSceneInfo* LightSceneInfo)
{
		...
		)
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			// Mirrors logic from FScene::GetMobileDirectionalLightForView, see comment there for more info
			if (!(LightSceneInfo->Proxy->GetViewLightingChannelMask() & View.ViewLightingChannelMask))
			{
				continue;
			}
...
void FMobileSceneRenderer::GatherShadowPointLights(
	const FViewInfo& View,
	const FSortedLightSetSceneInfo& SortedLightSet,
	TArray<int32, TInlineAllocator<8>>& OutShadowPointLightIndexes)
{
	if (!FReadOnlyCVARCache::EnablePointLightShadows(ShaderPlatform))
	{
		return;
	}

	int32 NumLights = SortedLightSet.SortedLights.Num();
	for (int32 LightIdx = SortedLightSet.UnbatchedLightStart; LightIdx < NumLights; ++LightIdx)
	{
		...
		{
			OutShadowPointLightIndexes.Add(LightIdx);
		}
	}
}
