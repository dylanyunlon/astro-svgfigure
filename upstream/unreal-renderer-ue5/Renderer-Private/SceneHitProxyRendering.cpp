...
static bool RequiresEditorPermutations(const FMeshMaterialShaderPermutationParameters& Parameters)
{
	...
	return bSupportEditor || bSupportCookedEditor;
}

class FHitProxyShaderElementData : public FMeshMaterialShaderElementData
{
public:
	FHitProxyShaderElementData(FHitProxyId InBatchHitProxyId, bool bInEditorSelectionPass = false)
		: BatchHitProxyId(InBatchHitProxyId)
		, bEditorSelectionPass(bInEditorSelectionPass)
	{
	}

	FHitProxyId BatchHitProxyId;
	bool bEditorSelectionPass; // When true, BatchHitProxyId is treated as OverlayColor
};
...
public:
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		...
		// Only compile the hit proxy vertex shader on desktop platforms
		return RequiresEditorPermutations(Parameters)
...
public:
	...
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		// Only compile the hit proxy vertex shader on editor platforms
		return RequiresEditorPermutations(Parameters)
		...
		FMeshMaterialShader(Initializer)
	{
		...
		WriteInstanceHitProxyId.Bind(Initializer.ParameterMap, TEXT("WriteInstanceHitProxyId"), SPF_Optional);
	}
		...
		FMeshDrawSingleShaderBindings& ShaderBindings) const
	{
		...
		// In the editor-selection pass, HitProxyId is actually the primitive's OverlayColor and must pass through untouched.
		if (!ShaderElementData.bEditorSelectionPass)
		{
#if WITH_EDITOR
		...
		// This prevents the per-instance hit proxy ID from being added to the output when we're writing overlay color
		ShaderBindings.Add(WriteInstanceHitProxyId, ShaderElementData.bEditorSelectionPass ? 0.0f : 1.0f);
	}

private:
	LAYOUT_FIELD(FShaderParameter, HitProxyId)
	LAYOUT_FIELD(FShaderParameter, WriteInstanceHitProxyId)
};
...
{
	SceneRenderer->Scene->AddGPUSkinCacheAsyncComputeWait(GraphBuilder);
...

	{
			...
			[&Views, HitProxyTextureExtent](FRDGAsyncTask, FRHICommandList& RHICmdList)
		{
			...
			for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
			{
				...
				DrawClearQuad(RHICmdList, false, FLinearColor(), true, 0.f, true, 0, HitProxyTextureExtent, FIntRect());
			}
		});
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		PassParameters->SceneTextures = CreateSceneTextureShaderParameters(GraphBuilder, View, ESceneTextureSetupMode::None);
			...
			[SceneRenderer, &View, LocalScene, FeatureLevel, PassParameters](FRHICommandList& RHICmdList)
		{
			...
			View.BatchedViewElements.Draw(RHICmdList, DrawRenderState, View, true);
			...
			View.TopBatchedViewElements.Draw(RHICmdList, DrawRenderState, View, true);
...
void FDeferredShadingSceneRenderer::RenderHitProxies(FRDGBuilder& GraphBuilder, const FSceneRenderUpdateInputs* SceneUpdateInputs)
{
	...
	if (bNaniteEnabled)
	{
		...
		SharedContext.Pipeline = Nanite::ERasterPipeline::HitProxy;

		FIntRect HitProxyTextureRect(0, 0, HitProxyTextureSize.X, HitProxyTextureSize.Y);

		const Nanite::FRasterContextInitParams RasterInitParams { .TextureSize = HitProxyTextureSize, .TextureRect = HitProxyTextureRect };
	...
	ERasterizerCullMode MeshCullMode)
{
	...
	FHitProxyShaderElementData ShaderElementData(OverlayColor, /*bEditorSelectionPass =*/ true);
	ShaderElementData.InitializeMeshMaterialData(ViewIfDynamicMeshCommand, PrimitiveSceneProxy, MeshBatch, StaticMeshId, false);
...
int32 FEditorSelectionMeshProcessor::GetStencilValue(const FSceneView* View, const FPrimitiveSceneProxy* PrimitiveSceneProxy)
{
	...
	// Keep Selection Outline color index (0) just for selection.
	// It is handled differently by EncodeSelectionStencilValue lambda below.
	// Using 0 index for other colors might have the function generate the same stencil value for different elements,
	// preventing the shader from distinguishing them as separate objects.
	static constexpr int SelectionOutlineColorIndex = 0; // See FSceneView::SelectionOutlineColor
	static constexpr int SubduedSelectionOutlineColorIndex = 1; // See FSceneView::SubduedSelectionOutlineColor
	static constexpr int PreselectionOutlineColorIndex = SubduedSelectionOutlineColorIndex;
	...
	else if (PrimitiveSceneProxy->IsParentSelected())
	{
		...
		if (bActorSelectionColorIsSubdued && (Color == SelectionOutlineColorIndex))
		{
			Color = SubduedSelectionOutlineColorIndex;
		}
	...
	else if (PrimitiveSceneProxy->IsHovered())
	{
		const int UniqueId = ActorNameToStencilIndex.Num();
		StencilValue = EncodeSelectionStencilValue(PreselectionOutlineColorIndex, UniqueId);
		ActorNameToStencilIndex.Add(PrimitiveSceneProxy->GetOwnerName(), StencilValue);
	}
