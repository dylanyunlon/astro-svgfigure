...
#include "Runtime/Renderer/Public/SystemTextures.h"
...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT(FShadowDepthPassUniformParameters,)
	...
	SHADER_PARAMETER(uint32, ShadowDepthType)
...
public:
	...
	/** Whether this shadow represents the dynamic-only region of a split cascade. */
	uint32 bIsDynamicOnly : 1;

	/** For split cascades, stores the densely-packed index into CSMDepthReuseData for interval calculations. */
	int32 CSMShadowDepthReuseIndex;
...
public:
	...
	void SetupWholeSceneProjection(
		...
		uint32 InBorderSize,
		int32 InSelectedViewIndex = INDEX_NONE
		);
	...
	*/
	...
	FIntPoint GetShadowBufferResolution() const
	{
		...
		{
			return FIntPoint(VSM_VIRTUAL_MAX_RESOLUTION_XY, VSM_VIRTUAL_MAX_RESOLUTION_XY);
		}
	...
	EShadowDepthType GetShadowDepthType() const 
	{
		...
		{
			ShadowDepthType |= EShadowDepthType::Spot;
		}
	...
	bool HasVirtualShadowMap() const { return VirtualShadowMapPerLightCacheEntry.IsValid(); }
	...
	FCullingVolume GetCullingVolume() const;

	/**
	 * Create a FSceneCullingRendererHierarchicalCullingQuery and initialize the SceneCullingRendererCullingQuery pointer to drive hierarchical CPU culling for the projected shadow.
	 * The SceneCullingRendererCullingQuery may be null if the system is not enabled (or there are no primitives with hierarchical instance culling set up).
	 */
	void SetupCullingQuery(FSceneRenderer& SceneRenderer);

	/**
	 * Finds a relevant view for a shadow
	 */
	const FViewInfo* FindViewForShadow(TConstArrayView<FViewInfo> SceneRendererViews) const;

	/** Get the densely-packed CSMShadowDepthReuse array index for this shadow. */
	FORCEINLINE int32 GetCSMShadowDepthReuseIndex() const
	{
		if (CSMShadowDepthReuseIndex >= 0)
		{
			return CSMShadowDepthReuseIndex;
		}
		return CascadeSettings.ShadowSplitIndex;
	}

	/** Determine if a CSM cascade should call RenderDepth this frame based on heuristic. */
	bool ShouldCSMRenderDepthThisFrame(const FScene* Scene, const FCSMDepthReuse* CSMDepthReuse) const;
...
private:
	...
	FSceneCullingRendererHierarchicalCullingQuery* SceneCullingRendererCullingQuery = nullptr;
	...
	/** The scene renderer primary view selected for this whole-scene shadow (i.e, local light shadow), not used for directional or per object shadows.*/
	int32 SelectedViewIndex = INDEX_NONE;
...
public:
	...
	void Set(FRHIBatchedShaderParameters& BatchedParameters, const FSceneView& View, const FProjectedShadowInfo* ShadowInfo, bool bModulatedShadows, bool bUseFadePlane, bool SubPixelShadow)
	{
		...
		if (SubPixelShadow)
		{
			...
			{
				FVector4 Near = View.ViewMatrices.GetViewToClip().TransformFVector4(FVector4(0, 0, ShadowInfo->CascadeSettings.SplitNear));
				FVector4 Far = View.ViewMatrices.GetViewToClip().TransformFVector4(FVector4(0, 0, ShadowInfo->CascadeSettings.SplitFar));
...
public:
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		return IsFeatureLevelSupported(Parameters.Platform, ERHIFeatureLevel::SM5)
			|| (bUseTransmission == 0 && SubPixelShadow == 0); // Mobile uses only those
	}
	...
	FORCEINLINE bool operator()( const FProjectedShadowInfo& A, const FProjectedShadowInfo& B ) const
	{
		if (A.IsWholeSceneDirectionalShadow())
		{
			if (B.IsWholeSceneDirectionalShadow())
			{
				...
				if (B.CascadeSettings.ShadowSplitIndex != A.CascadeSettings.ShadowSplitIndex)
				{
					return (B.CascadeSettings.ShadowSplitIndex < A.CascadeSettings.ShadowSplitIndex);
				}

				// Same cascade index: render static then dynamic
				return (A.bIsDynamicOnly < B.bIsDynamicOnly);
			}
