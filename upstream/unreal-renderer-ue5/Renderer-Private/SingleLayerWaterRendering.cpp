...
// Deprecated: use r.Water.SingleLayer.VelocityOutputPass instead
static TAutoConsoleVariable<int32> CVarWaterSingleLayerForceVelocity(
	TEXT("r.Water.SingleLayer.ForceVelocity"), -1,
	TEXT("Deprecated. Use r.Water.SingleLayer.VelocityOutputPass instead."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarWaterSingleLayerVelocityOutputPass(
	TEXT("r.Water.SingleLayer.VelocityOutputPass"), 1,
	TEXT("When to write velocity for Single Layer Water.\n")
	TEXT("  0: None - no velocity written for SLW.\n")
	TEXT("  1: Depth Prepass - Write during SLW depth prepass (default, supports Motion Vector World Offset)\n")
	TEXT("  2: Base Pass - Write during SLW base pass."),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);
...
static TAutoConsoleVariable<int32> CVarWaterSingleLayerMobileFallback(
	TEXT("r.Water.SingleLayer.MobileFallback"), 1,
	TEXT("Whether to use a translucent rendering fallback for a single layer water on mobile platforms"),
...
{
	const int32 RefractionDownsampleFactor = FMath::Clamp(CVarWaterSingleLayerRefractionDownsampleFactor.GetValueOnRenderThread(), 1, 2);
	return RefractionDownsampleFactor;
}


ESingleLayerWaterVelocityOutputPass GetSingleLayerWaterVelocityOutputPass(bool bIsGameThread)
{
	const int32 VelocityOutputPass = !bIsGameThread 
		? CVarWaterSingleLayerVelocityOutputPass.GetValueOnRenderThread() 
		: CVarWaterSingleLayerVelocityOutputPass.GetValueOnGameThread();

	return static_cast<ESingleLayerWaterVelocityOutputPass>(
		FMath::Clamp(VelocityOutputPass, 
			static_cast<int32>(ESingleLayerWaterVelocityOutputPass::None), 
			static_cast<int32>(ESingleLayerWaterVelocityOutputPass::BasePass)));
}

bool CanSingleLayerWaterDepthPrepassOutputVelocity(EShaderPlatform ShaderPlatform, ERHIFeatureLevel::Type FeatureLevel, bool bIsGameThread)
{
	return GetSingleLayerWaterVelocityOutputPass(bIsGameThread) == ESingleLayerWaterVelocityOutputPass::DepthPrepass
		&& IsSingleLayerWaterDepthPrepassEnabled(ShaderPlatform, FeatureLevel)
		&& !IsForwardShadingEnabled(ShaderPlatform);
}

static EGBufferLayout GetSingleLayerWaterGBufferLayout(EShaderPlatform ShaderPlatform, ERHIFeatureLevel::Type FeatureLevel, bool bIsGameThread = false)
{
	const ESingleLayerWaterVelocityOutputPass VelocityOutputPass = GetSingleLayerWaterVelocityOutputPass(bIsGameThread);

	if (VelocityOutputPass == ESingleLayerWaterVelocityOutputPass::None)
	{
		return GBL_Default;
	}

	if (CanSingleLayerWaterDepthPrepassOutputVelocity(ShaderPlatform, FeatureLevel, bIsGameThread))
	{
		return GBL_Default;
	}

	// BasePass mode, DepthPrepass mode with prepass disabled (fallback), or Forward Renderer (velocity RT does not match multrisampled depth)
...
FSingleLayerWaterPassUniformParameters* CreateSingleLayerWaterPassUniformParameters(FRDGBuilder& GraphBuilder, const FScene* Scene, const FViewInfo& View, const FSceneTextures& SceneTextures, FRDGTextureRef SceneDepthWithoutWater, FRDGTextureRef SceneColorWithoutWater, FRDGTextureRef RefractionMaskTexture)
{
...
static FSingleLayerWaterDepthPassParameters* GetSingleLayerWaterDepthPassParameters(FRDGBuilder& GraphBuilder, const FScene* Scene, const FViewInfo& View, const FSceneTextures& SceneTextures, FRDGTextureRef SceneDepthWithoutWater, 
	FRDGTextureRef DepthTexture, FRDGTextureRef VelocityTexture = nullptr, FRDGTextureRef MotionVectorWorldOffsetTexture = nullptr)
{
	...
	if (VelocityTexture)
	{
		// If the depth pass is the first to write to the texture, clear it instead of loading.
		const ERenderTargetLoadAction VelocityLoadAction = HasBeenProduced(VelocityTexture)
		...
		{
			PassParameters->RenderTargets[1] = FRenderTargetBinding(MotionVectorWorldOffsetTexture, ERenderTargetLoadAction::ELoad);
		}
	}

	FSingleLayerWaterPassUniformParameters* SLWUniformParameters = CreateSingleLayerWaterPassUniformParameters(GraphBuilder, Scene, View, SceneTextures, SceneDepthWithoutWater, nullptr, nullptr);
...
static FSingleLayerWaterTileClassification ClassifyTiles(FRDGBuilder& GraphBuilder, const FViewInfo &View, const FSceneTextures& SceneTextures, FRDGTextureRef DepthPrepassTexture, const Froxel::FViewData* OutFroxelViewData, EReflectionsMethod ReflectionsMethod)
{
	...
	if (bRunTiled)
	{
		...
		{
			bool bUsePrepassStencil = DepthPrepassTexture != nullptr;

			FWaterTileCategorisationMarkCS::FPermutationDomain PermutationVector;
			PermutationVector.Set<FWaterTileCategorisationMarkCS::FUsePrepassStencil>(bUsePrepassStencil);
			...
			PassParameters->View = View.GetShaderParameters();
			PassParameters->Substrate = !bUsePrepassStencil ? Substrate::BindSubstrateGlobalUniformParameters(View) : nullptr;
...
)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		// Under ISR the secondary view is in Views but skipped by ShouldRenderView() above.
		// Pick it up here so the copy runs once per eye with that eye's own viewport, view UB
		// and tile list. The tile VS computes NDC using View.ViewSizeAndInvSize (single eye),
		// so a single draw covering the union of both eye rects would stretch each tile 2x.
		TArray<TPair<int32, const FViewInfo*>, TInlineAllocator<2>> ViewsToDraw;
		ViewsToDraw.Emplace(ViewIndex, &View);
		if (View.IsInstancedStereo())
		{
			if (const FViewInfo* InstancedView = View.GetInstancedView())
			{
				const int32 InstancedViewIndex = Views.IndexOfByPredicate(
					[InstancedView](const FViewInfo& V) { return &V == InstancedView; });
				if (ensure(InstancedViewIndex != INDEX_NONE))
				{
					ViewsToDraw.Emplace(InstancedViewIndex, InstancedView);
				}
			}
		}

		for (const TPair<int32, const FViewInfo*>& DrawViewEntry : ViewsToDraw)
		{
			const int32 DrawViewIndex = DrawViewEntry.Key;
			const FViewInfo& DrawView = *DrawViewEntry.Value;
			...
			if (!DrawView.Family->bMultiGPUForkAndJoin)
			{
			...
			auto PixelShader = DrawView.ShaderMap->GetShader<FWaterRefractionCopyPS>(PermutationVector);

			const FIntRect RefractionViewRect = FIntRect(FIntPoint::DivideAndRoundDown(DrawView.ViewRect.Min, RefractionDownsampleFactor), FIntPoint::DivideAndRoundDown(DrawView.ViewRect.Max, RefractionDownsampleFactor));
			if (bDoTiledCopy)
			{
				const FTiledReflection* TiledReflection = &SingleLayerWaterPrepassResult->ViewTileClassification[DrawViewIndex].TiledReflection;
				SingleLayerWaterAddTiledFullscreenPass(GraphBuilder, DrawView.ShaderMap, RDG_EVENT_NAME("SLW::Copy"), PixelShader, PassParameters, DrawView.ViewUniformBuffer, RefractionViewRect, TiledReflection);
			}
			else
			{
				FPixelShaderUtils::AddFullscreenPass(GraphBuilder, DrawView.ShaderMap, RDG_EVENT_NAME("SLW::Copy"), PixelShader, &PassParameters->PS, RefractionViewRect);
			}
		}
	}
}
...
FSingleLayerWaterPrePassResult* FDeferredShadingSceneRenderer::RenderSingleLayerWaterDepthPrepass(FRDGBuilder& GraphBuilder, TArrayView<FViewInfo> InViews, const FSceneTextures& SceneTextures, ESingleLayerWaterPrepassLocation Location, TConstArrayView<Nanite::FRasterResults> NaniteRasterResults)
{
...
#if !UE_BUILD_SHIPPING
	{
		...
		if (!bValidated)
		{
			const int32 LegacyValue = CVarWaterSingleLayerForceVelocity.GetValueOnRenderThread();
			ensureMsgf(LegacyValue == -1,
				TEXT("r.Water.SingleLayer.ForceVelocity is deprecated and ignored. ")
				TEXT("Use r.Water.SingleLayer.VelocityOutputPass instead (0=Disabled, 1=DepthPrepass, 2=BasePass)."));
	...
	// Create an identical copy of the main depth buffer
	{
		...
		else
		{
			...
			FGlobalShaderMap* ShaderMap = GetGlobalShaderMap(Scene->GetShaderPlatform());
	...
	// Check if any view has SLW materials using motion vector world offset
	bool bAnySLWUsesMVWO = false;
	for (int32 ViewIndex = 0; ViewIndex < InViews.Num(); ++ViewIndex)
	{
		bAnySLWUsesMVWO |= InViews[ViewIndex].bSLWUsesMotionVectorWorldOffset;
	}

	// Conditionally create velocity and MVWO textures when needed.
	FRDGTextureRef VelocityPrepassTexture = nullptr;
	FRDGTextureRef MotionVectorWorldOffsetPrepassTexture = nullptr;
	if (CanSingleLayerWaterDepthPrepassOutputVelocity(ShaderPlatform, FeatureLevel))
	{
		const bool bPlatformSupportsMVWO = SupportsPixelShaderMotionVectorWorldOffset(ShaderPlatform);
		if (bAnySLWUsesMVWO && bPlatformSupportsMVWO)
		{
			const FRDGTextureDesc VelocityDesc = FVelocityRendering::GetRenderTargetDesc(ShaderPlatform, SceneTextures.Config.Extent);
			VelocityPrepassTexture = GraphBuilder.CreateTexture(VelocityDesc, TEXT("SLW.VelocityPrepassTexture"));
			MotionVectorWorldOffsetPrepassTexture = GraphBuilder.CreateTexture(VelocityDesc, TEXT("SLW.MotionVectorWorldOffsetPrepassTexture"));
			AddClearRenderTargetPass(GraphBuilder, VelocityPrepassTexture);
			AddClearRenderTargetPass(GraphBuilder, MotionVectorWorldOffsetPrepassTexture);
		}
		else if (bPlatformSupportsMVWO)
		{
			VelocityPrepassTexture = SceneTextures.Velocity;
			// No MVWO materials, still need to allocate as velocity shaders still declare SV_Target1 on this platform.
			const FRDGTextureDesc VelocityDesc = FVelocityRendering::GetRenderTargetDesc(ShaderPlatform, SceneTextures.Config.Extent);
			MotionVectorWorldOffsetPrepassTexture = GraphBuilder.CreateTexture(VelocityDesc, TEXT("SLW.MotionVectorWorldOffsetDummy"));
		}
		else
		{
			// No MVWO: write velocity directly to scene velocity textures
			VelocityPrepassTexture = SceneTextures.Velocity;
		}
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < InViews.Num(); ++ViewIndex)
	{
		...
		FSingleLayerWaterDepthPassParameters* PassParameters = GetSingleLayerWaterDepthPassParameters(GraphBuilder, Scene, View, SceneTextures, Result->SceneDepthWithoutWater, OutDepthPrepassTexture.Target, VelocityPrepassTexture, MotionVectorWorldOffsetPrepassTexture);
	...
	if (bAnySLWUsesMVWO && MotionVectorWorldOffsetPrepassTexture)
	{
		// Save them into intermediate texture so we can resolve later after SLW pass.
		Result->VelocityPrepassTexture = VelocityPrepassTexture;
		Result->MotionVectorWorldOffsetPrepassTexture = MotionVectorWorldOffsetPrepassTexture;
	}
	...
	const FSingleLayerWaterPrePassResult* SingleLayerWaterPrePassResult)
{
	...
	// Compose hair pixels below water into the main scene color in order to be affected by water transmittance
	if (SingleLayerWaterPrePassResult && HairStrands::HasHairStrandsVisible(Views))
	{
		RenderHairCompositionForSLW(
			GraphBuilder,
			Views,
			SingleLayerWaterPrePassResult->DepthPrepassTexture.Target,
			SceneColorTexture,
			SceneDepthTexture);
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		const float PixelSafeGuardBand = 0.55f;
	...
	for (int32 ViewIndex = 0; ViewIndex < InViews.Num(); ViewIndex++)
	{
		...
		if (bWaterVSMFiltering || bWaterDistanceFieldShadow)
		{
			...
			if (bWaterVSMFiltering && SelectedForwardDirectionalLightProxy)
			{
				...
				FLightSceneId LightId = SelectedForwardDirectionalLightProxy->GetLightSceneInfo()->Id;
	...
	bool bIsCameraUnderWater)
{
	...
	if (SingleLayerWaterPrePassResult &&
		SingleLayerWaterPrePassResult->MotionVectorWorldOffsetPrepassTexture)
	{
		// SceneTextures.Velocity is not produced and needs to be cleared before resolve.
		if (!HasBeenProduced(SceneTextures.Velocity))
		{
			AddClearRenderTargetPass(GraphBuilder, SceneTextures.Velocity);
		}

		ResolveSLWMotionVectorWorldOffset(
			GraphBuilder,
			InViews,
			SingleLayerWaterPrePassResult->DepthPrepassTexture.Resolve,
			SingleLayerWaterPrePassResult->VelocityPrepassTexture,
			SingleLayerWaterPrePassResult->MotionVectorWorldOffsetPrepassTexture,
	...
	const FSingleLayerWaterPrePassResult* SingleLayerWaterPrePassResult)
{
	...
	const EGBufferLayout GBufferLayout = GetSingleLayerWaterGBufferLayout(Scene->GetShaderPlatform(),FeatureLevel);
	...
	const bool bEnableSkylight = Scene->ShouldRenderSkylightInBasePass(/*bIsTranslucent*/true);
	...
	for (int32 ViewIndex = 0; ViewIndex < InViews.Num(); ++ViewIndex)
	{
		...
		FSingleLayerWaterPassUniformParameters* SLWUniformParameters = CreateSingleLayerWaterPassUniformParameters(GraphBuilder, Scene, View, SceneTextures, SceneWithoutWaterTextures.DepthTexture, SceneWithoutWaterTextures.ColorTexture, RefractionMaskTexture);

		FSingleLayerWaterPassParameters* PassParameters = GraphBuilder.AllocParameters<FSingleLayerWaterPassParameters>();
		PassParameters->View = View.GetShaderParameters();
		PassParameters->ReflectionCapture = View.ReflectionCaptureUniformBuffer;
		PassParameters->BasePass = CreateOpaqueBasePassUniformBuffer(GraphBuilder, View, ViewIndex, {}, {}, bEnableSkylight);
	...
	if (bHasDepthPrepass)
	{
		AddClearStencilPass(GraphBuilder, SingleLayerWaterPrePassResult->DepthPrepassTexture.Target);
	}
	...
	ERasterizerCullMode MeshCullMode)
{
	...
	if (!GetBasePassShaders<LightMapPolicyType>(
		...
		false, // 128bit
		false, // bIsDebug
		GetSingleLayerWaterGBufferLayout(GetFeatureLevelShaderPlatform(FeatureLevel), FeatureLevel),
		nullptr, // Mesh Shader
...
void FSingleLayerWaterPassMeshProcessor::CollectPSOInitializers(const FSceneTexturesConfig& SceneTexturesConfig, const FMaterial& Material, const FPSOPrecacheVertexFactoryData& VertexFactoryData, const FPSOPrecacheParams& PreCacheParams, TArray<FPSOPrecacheData>& PSOInitializers)
{
	if (Material.GetShadingModels().HasShadingModel(MSM_SingleLayerWater))
	{
		...
		const EGBufferLayout GBufferLayout = GetSingleLayerWaterGBufferLayout(GetFeatureLevelShaderPlatform(FeatureLevel), FeatureLevel, true /*bIsGameThread*/);
		...
		RenderTargetsInfo.DepthStencilAccess = bHasDepthPrepass ? FExclusiveDepthStencil::DepthRead_StencilRead : FExclusiveDepthStencil::DepthWrite_StencilNop;
...
FMeshPassProcessor* CreateSingleLayerWaterPassProcessor(ERHIFeatureLevel::Type FeatureLevel, const FScene* Scene, const FSceneView* InViewIfDynamicMeshCommand, FMeshPassDrawListContext* InDrawListContext)
{
	...
	FExclusiveDepthStencil::Type BasePassDepthStencilAccess_DepthWrite = FExclusiveDepthStencil::Type(bHasDepthPrepass ? FExclusiveDepthStencil::DepthRead_StencilRead : SceneBasePassDepthStencilAccess | FExclusiveDepthStencil::DepthWrite_StencilNop);
	...
	const FMaterial& Material)
{
	if (Material.GetShadingModels().HasShadingModel(MSM_SingleLayerWater))
	{
		...
		// When SLW velocity is enabled, all materials route through velocity shaders.
		// When disabled, only material using MVWO routes through velocity shaders instead of depth-only.
		const EShaderPlatform PrepassShaderPlatform = GetFeatureLevelShaderPlatform(FeatureLevel);
		const bool bSLWPrepassVelocity = CanSingleLayerWaterDepthPrepassOutputVelocity(PrepassShaderPlatform, FeatureLevel);
		if (bSLWPrepassVelocity)
		{
			const bool bMaterialNeedsVelocity =
				!Material.WritesEveryPixel(false, bVFTypeSupportsNullPixelShader)
				|| bModifiesMeshPosition
				|| (Material.MaterialUsesMotionVectorWorldOffset_RenderThread() && SupportsPixelShaderMotionVectorWorldOffset(PrepassShaderPlatform))
				|| (Material.MaterialUsesTemporalResponsiveness_RenderThread() && SupportsTemporalResponsiveness(PrepassShaderPlatform));

			if (bMaterialNeedsVelocity)
			{
				return ProcessVelocity(MeshBatch, BatchElementMask, PrimitiveSceneProxy, StaticMeshId, MaterialRenderProxy, Material, MeshFillMode, MeshCullMode);
			}
		}
	...
	ERasterizerCullMode MeshCullMode)
{
	...
	FMeshDrawCommandSortKey SortKey = CalculateDepthPassMeshStaticSortKey(bIsMasked, DepthPassShaders.VertexShader, DepthPassShaders.PixelShader);
...
bool FSingleLayerWaterDepthPrepassMeshProcessor::ProcessVelocity(
	const FMeshBatch& RESTRICT MeshBatch, 
	uint64 BatchElementMask, 
	const FPrimitiveSceneProxy* RESTRICT PrimitiveSceneProxy, 
	int32 StaticMeshId, const FMaterialRenderProxy& RESTRICT MaterialRenderProxy, 
	const FMaterial& RESTRICT MaterialResource, 
	ERasterizerFillMode MeshFillMode, 
	ERasterizerCullMode MeshCullMode)
{
	TMeshProcessorShaders<FVelocityVS, FVelocityPS> VelocityPassShaders;

	if (!GetVelocityPassShaders(
		MaterialResource,
		MeshBatch.VertexFactory->GetType(),
		FeatureLevel,
		EVelocityPassMode::Velocity_Standard,
	...
	// Override blend state to use the default: the depth prepass uses CW_NONE, but MVWO velocity
	// shaders need to write to RT0 (velocity) and RT1 (MVWO offset).
	FMeshPassProcessorRenderState VelocityDrawRenderState(PassDrawRenderState);
	VelocityDrawRenderState.SetBlendState(TStaticBlendState<>::GetRHI());
...
void FSingleLayerWaterDepthPrepassMeshProcessor::CollectPSOInitializers(const FSceneTexturesConfig& SceneTexturesConfig, const FMaterial& Material, const FPSOPrecacheVertexFactoryData& VertexFactoryData, const FPSOPrecacheParams& PreCacheParams, TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	if (Material.GetShadingModels().HasShadingModel(MSM_SingleLayerWater) && IsSingleLayerWaterDepthPrepassEnabled(ShaderPlatform, FeatureLevel))
	{
		const bool bSLWPrepassVelocity = CanSingleLayerWaterDepthPrepassOutputVelocity(ShaderPlatform, FeatureLevel, /*bIsGameThread*/true);
		...
		if (bSLWPrepassVelocity)
		{
			...
			const bool bMaterialNeedsVelocity = 
				...
				|| (Material.MaterialUsesMotionVectorWorldOffset_GameThread() && SupportsPixelShaderMotionVectorWorldOffset(ShaderPlatform))
				|| (Material.MaterialUsesTemporalResponsiveness_GameThread() && SupportsTemporalResponsiveness(ShaderPlatform));
			...
			if (bMaterialNeedsVelocity)
			{
					...
					VelocityPassShaders.PixelShader))
				{
					...
					// Two velocity render targets (velocity + MVWO offset)
					...
					// Must match ProcessVelocity's blend state override — depth prepass
					// has CW_NONE but MVWO velocity shaders need to write to RT0/RT1.
					FMeshPassProcessorRenderState VelocityDrawRenderState(PassDrawRenderState);
					VelocityDrawRenderState.SetBlendState(TStaticBlendState<>::GetRHI());
