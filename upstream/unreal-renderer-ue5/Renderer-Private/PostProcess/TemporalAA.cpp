...
// LargeGroup keeps the 8x8 threadgroup but each thread processes an
// (AA_PIXELS_PER_THREAD_X, AA_PIXELS_PER_THREAD_Y) quad of output pixels.
const int32 GTemporalAALargeGroupOutputTileSizeX = 16;
const int32 GTemporalAALargeGroupOutputTileSizeY = 8;
const int32 GTemporalAALargeGroupPixelsPerThreadX = 2;
const int32 GTemporalAALargeGroupPixelsPerThreadY = 1;
...
TAutoConsoleVariable<bool> CVarTemporalAAVGPROptSupported(
	TEXT("r.TemporalAA.VGPROpt.Supported"),
	false,
	TEXT("Whether the TAA VGPR-reduction permutation is supported on this platform. Read-only.\n")
	TEXT("Set to true in the platform's engine ini [ConsoleVariables] to cook the optimized permutation.\n"),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);

static bool IsTemporalAAVGPROptSupportedForPlatform(EShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<bool> CachedValue(TEXT("r.TemporalAA.VGPROpt.Supported"));
	return CachedValue.Get(Platform);
}

TAutoConsoleVariable<int32> CVarTemporalAAVGPROpt(
	TEXT("r.TemporalAA.VGPROpt"),
	0,
	TEXT("Runtime toggle for the TAA VGPR-reduction permutation (AA_VGPR_OPT).\n")
	TEXT(" 0: stock shader (default);\n")
	TEXT(" 1: optimized shader; only effective when r.TemporalAA.VGPROpt.Supported=true for this platform.\n"),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<bool> CVarTemporalAALargeGroupSupported(
	TEXT("r.TemporalAA.LargeGroup.Supported"),
	false,
	TEXT("Whether the larger-threadgroup TAA permutation is supported on this platform. Read-only.\n")
	TEXT("Set to true in the platform's engine ini [ConsoleVariables] to cook the larger-group permutation.\n"),
	ECVF_ReadOnly | ECVF_RenderThreadSafe);

static bool IsTemporalAALargeGroupSupportedForPlatform(EShaderPlatform Platform)
{
	static FShaderPlatformCachedIniValue<bool> CachedValue(TEXT("r.TemporalAA.LargeGroup.Supported"));
	return CachedValue.Get(Platform);
}

TAutoConsoleVariable<int32> CVarTemporalAALargeGroup(
	TEXT("r.TemporalAA.LargeGroup"),
	0,
	TEXT("Runtime toggle for the larger-threadgroup TAA permutation. Bigger groups amortize\n")
	TEXT("the 3x3 neighborhood halo over more output pixels, reducing redundant input reads at\n")
	TEXT("threadgroup boundaries. Targets bandwidth-bound platforms.\n")
	TEXT(" 0: 8x8 threadgroup (default);\n")
	TEXT(" 1: larger threadgroup; only effective when r.TemporalAA.LargeGroup.Supported=true for this platform.\n"),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarTemporalAALargeGroupMaxInputScale(
	TEXT("r.TemporalAA.LargeGroup.MaxInputScale"),
	90,
	TEXT("Maximum input-to-output ratio (as a percentage) at which LargeGroup is allowed.\n")
	TEXT("LargeGroup wins on cache locality at lower input scales but loses to extra LDS-fill work\n")
	TEXT("when the input approaches output resolution. Disable LargeGroup when SrcRect / DestRect >= this %.\n")
	TEXT("On platforms that opt into both LargeGroup and VGPROpt, VGPROpt follows via the coupling at dispatch.\n"),
	ECVF_RenderThreadSafe);
...
class FTemporalAACS : public FTemporalAA
{
	...
	class FTAAVgprOptDim : SHADER_PERMUTATION_BOOL("AA_VGPR_OPT");
	class FTAALargeGroupDim : SHADER_PERMUTATION_BOOL("AA_LARGE_GROUP");
	...
	// Output tile size per dispatched group. Differs from the [numthreads(...)] for LargeGroup,
	// where each thread processes a horizontal pair of output pixels.
	static FIntPoint GetOutputTileSize(const FPermutationDomain& PermutationVector)
	{
		if (PermutationVector.Get<FTAALargeGroupDim>())
		{
			return FIntPoint(GTemporalAALargeGroupOutputTileSizeX, GTemporalAALargeGroupOutputTileSizeY);
		}
		return FIntPoint(GTemporalAATileSizeX, GTemporalAATileSizeY);
	}
	...
	static FPermutationDomain RemapPermutation(FPermutationDomain PermutationVector)
	{
		...
		{
			const ETAAPassConfig PassConfig = PermutationVector.Get<FTAAPassConfigDim>();
			if (PassConfig != ETAAPassConfig::Main &&
				PassConfig != ETAAPassConfig::MainUpsampling &&
				PassConfig != ETAAPassConfig::MainSuperSampling)
			{
				PermutationVector.Set<FTAAVgprOptDim>(false);
				PermutationVector.Set<FTAALargeGroupDim>(false);
			}
		}

		// LargeGroup loads the input-color cache once per group and reuses it across the per-pixel
		// loop, which requires the cache to survive a single load per group:
		//   - Non-upsample Main reuses one LDS array for depth then color within a kernel, so the
		//     depth phase running per-iteration would clobber the shared color cache.
		//   - Screen-percentage ranges 2/3 use AA_SAMPLE_CACHE_METHOD_VGPR_3X3 - a per-thread VGPR
		//     cache that doesn't outlive the once-per-group call's stack frame.
		{
			const ETAAPassConfig PassConfig = PermutationVector.Get<FTAAPassConfigDim>();
			const int32 ScreenPercentageRange = PermutationVector.Get<FTAAScreenPercentageDim>();
			const bool bLargeGroupCompatible =
				(PassConfig == ETAAPassConfig::MainUpsampling || PassConfig == ETAAPassConfig::MainSuperSampling) &&
				(ScreenPercentageRange == 0 || ScreenPercentageRange == 1);
			if (!bLargeGroupCompatible)
			{
				PermutationVector.Set<FTAALargeGroupDim>(false);
			}
		}

		// The heavier per-pixel work at High / MediumHigh quality lifts VGPR pressure past what the
		// LargeGroup amortization can pay back; skip cooking those permutations.
		const ETAAQuality Quality = PermutationVector.Get<FTemporalAA::FTAAQualityDim>();
		if (Quality == ETAAQuality::High || Quality == ETAAQuality::MediumHigh)
		{
			PermutationVector.Set<FTAALargeGroupDim>(false);
		}
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		if (PermutationVector.Get<FTAAVgprOptDim>() &&
			!IsTemporalAAVGPROptSupportedForPlatform(Parameters.Platform))
		{
			return false;
		}

		// On platforms that opt into both LargeGroup and VGPROpt, VGPROpt is empirically only
		// a win paired with LargeGroup. Skip cooking the standalone VGPROpt variants.
		if (PermutationVector.Get<FTAAVgprOptDim>() &&
			!PermutationVector.Get<FTAALargeGroupDim>() &&
			IsTemporalAALargeGroupSupportedForPlatform(Parameters.Platform))
		{
			return false;
		}
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		OutEnvironment.SetDefine(TEXT("AA_PIXELS_PER_THREAD_X"),
			PermutationVector.Get<FTAALargeGroupDim>() ? GTemporalAALargeGroupPixelsPerThreadX : 1);
		OutEnvironment.SetDefine(TEXT("AA_PIXELS_PER_THREAD_Y"),
			PermutationVector.Get<FTAALargeGroupDim>() ? GTemporalAALargeGroupPixelsPerThreadY : 1);
	}
}; // class FTemporalAACS
...
{
	static FShaderPlatformCachedIniValue<int32> TemporalAAMobileUseCompute(TEXT("r.TemporalAA.Mobile.UseCompute"));
	return !IsMobilePlatform(Platform) || (TemporalAAMobileUseCompute.Get(Platform) > 0);
}
	...
	FTemporalAAHistory* OutputHistory)
{
...

	{
		...
		if (Inputs.bDownsample)
		{
			...
			if (Inputs.DownsampleRenderTarget)
			{
				Outputs.DownsampledSceneColor = Inputs.DownsampleRenderTarget;
			}
			else
			{
	...
	auto SetupTemporalAACommonPassParameters = [&](FTemporalAA::FParameters* PassParameters)
	{
		...
		FRDGTextureRef StencilSource = Inputs.StencilDepthTexture ? Inputs.StencilDepthTexture : Inputs.SceneDepthTexture;
		PassParameters->StencilTexture = GraphBuilder.CreateSRV(FRDGTextureSRVDesc::CreateWithPixelFormat(StencilSource, PF_X24_G8));
		...
		// Input buffer shader parameters
		{
			...
			PassParameters->InputSceneColor = GraphBuilder.CreateSRV(FRDGTextureSRVDesc::CreateForMipLevel(Inputs.SceneColorInput, 0));
	...
	if (bIsComputePass)
	{
		...
		PermutationVector.Set<FTemporalAACS::FTAAVgprOptDim>(
			IsTemporalAAVGPROptSupportedForPlatform(View.GetShaderPlatform()) &&
			CVarTemporalAAVGPROpt.GetValueOnRenderThread() != 0);
		const int32 LargeGroupMaxInputScale = CVarTemporalAALargeGroupMaxInputScale.GetValueOnRenderThread();
		const bool bAtUsableScaleForLargeGroup =
			SrcRect.Width()  * 100 < LargeGroupMaxInputScale * DestRect.Width()  &&
			SrcRect.Height() * 100 < LargeGroupMaxInputScale * DestRect.Height();
		PermutationVector.Set<FTemporalAACS::FTAALargeGroupDim>(
			IsTemporalAALargeGroupSupportedForPlatform(View.GetShaderPlatform()) &&
			CVarTemporalAALargeGroup.GetValueOnRenderThread() != 0 &&
			bAtUsableScaleForLargeGroup);
		...
		// On platforms that opt into both LargeGroup and VGPROpt, VGPROpt only pays back when
		// paired with LargeGroup. Honor that coupling post-Remap so the LargeGroup gates above
		// flow through to VGPROpt without re-checking each gate's condition here.
		if (IsTemporalAALargeGroupSupportedForPlatform(View.GetShaderPlatform()) &&
			!PermutationVector.Get<FTemporalAACS::FTAALargeGroupDim>())
		{
			PermutationVector.Set<FTemporalAACS::FTAAVgprOptDim>(false);
		}
		...
		// UAVs
		{
			...
			if (Outputs.DownsampledSceneColor)
			{
				if (Inputs.DownsampleRenderTarget)
				{
					// Use the specified render target and mip level
					check(Inputs.DownsampleRenderTarget->Desc.NumMips > Inputs.DownsampleRenderTargetMipLevel);
					PassParameters->OutComputeTexDownsampled = GraphBuilder.CreateUAV(FRDGTextureUAVDesc(Inputs.DownsampleRenderTarget, Inputs.DownsampleRenderTargetMipLevel));
				}
				else
				{
		...
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("TAA(%s Quality=%s%s%s%s) %dx%d -> %dx%d",
				PassName,
				...
				PermutationVector.Get<FTemporalAACS::FTAAVgprOptDim>() ? TEXT(" VGPROpt") : TEXT(""),
				PermutationVector.Get<FTemporalAACS::FTAALargeGroupDim>() ? TEXT(" LargeGroup") : TEXT(""),
				PracticableSrcRect.Width(), PracticableSrcRect.Height(),
				PracticableDestRect.Width(), PracticableDestRect.Height()),
			ComputeShader,
			PassParameters,
			FComputeShaderUtils::GetGroupCount(PracticableDestRect.Size(), FTemporalAACS::GetOutputTileSize(PermutationVector)));
	}
	else
	{
...
UE::Renderer::Private::IMegaFilters::FOutputs AddMegaFiltersPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const TCHAR* Name,const UE::Renderer::Private::IMegaFilters::FInputs& Inputs)
{
	using UE::Renderer::Private::IMegaFilters;

	const IMegaFilters* MegaFiltersToUse = View.Family->GetMegaFiltersInterface();
	IMegaFilters::FOutputs MegaFilterOutputs;

	if (MegaFiltersToUse)
	{
		const TCHAR* FilterName = MegaFiltersToUse->GetDebugName();

		IMegaFilters::FInputs MegaFilterInputs = Inputs;
		checkf(MegaFilterInputs.PrevHistory == nullptr,TEXT("History is set internally"));
		{
			// History can be accessed/updated in the same frame at different location for the same MegaFiltersPassType.
			TRefCountPtr<IMegaFilters::IHistory> PrevHistory = 
				(View.ViewState && View.ViewState->PrevFrameViewInfo.MegaFiltersHistory) ? 
				View.ViewState->PrevFrameViewInfo.MegaFiltersHistory : View.PrevViewInfo.MegaFiltersHistory;

			if (PrevHistory && PrevHistory->GetDebugName() == FilterName)
			{
				MegaFilterInputs.PrevHistory = PrevHistory;
			}
		}

		checkf(MegaFilterInputs.OutputViewRects.Num() > 0, TEXT("At least define one output view rect in MegaFilterInputs.OutputViewRects"));

		RDG_EVENT_SCOPE(
			GraphBuilder,
			"MegaFilters %s %dx%d -> %dx%d",
			Name,
			View.ViewRect.Width(), View.ViewRect.Height(),
			MegaFilterInputs.OutputViewRects[0].Width(), MegaFilterInputs.OutputViewRects[0].Height());

		MegaFilterOutputs = MegaFiltersToUse->AddPasses(
			GraphBuilder,
			View,
			MegaFilterInputs);
		...
		{
			View.ViewState->PrevFrameViewInfo.MegaFiltersHistory = MegaFilterOutputs.NewHistory;
		}
	}

	return MegaFilterOutputs;
}

FRDGTextureRef AddMegaFiltersTextureCopy(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureRef SourceTexture, UE::Renderer::Private::EMegaFiltersPassType PassType, const TCHAR* TextureName)
{
    const UE::Renderer::Private::IMegaFilters* MegaFiltersToUse = View.Family->GetMegaFiltersInterface();

    if (MegaFiltersToUse && SourceTexture)
    {
        FRDGTextureRef Copy = GraphBuilder.CreateTexture(SourceTexture->Desc, TextureName);
        AddCopyTexturePass(GraphBuilder, SourceTexture, Copy);
        return Copy;
    }
