...
{
	if (SplitScreenDebugViewState != nullptr)
	{
		SplitScreenDebugViewState->Destroy();
		SplitScreenDebugViewState = nullptr;
	}
...
static void PrepareDirectionalLightShadowOcclusionQuery(uint32& BaseVertexIndex, TRHIBufferInitializer<FVector3f>& DestinationBuffer, const FViewInfo& View, const FProjectedShadowInfo& ProjectedShadowInfo)
{
	...
	const FMatrix& ViewMatrix = ViewMatrices.GetWorldToView();
...

	{
			...
			[Primitives = MoveTemp(Primitives), BoundsCenterTexture, BoundsExtentTexture](FRHICommandListImmediate& RHICmdList)
		{
			...
			for (uint32 i = 0; i < NumPrimitives; i += BlockSize * BlockSize)
			{
				...
				RHICmdList.UpdateTexture2D((FRHITexture*)BoundsCenterTexture->GetRHI(), 0, Region, BlockStride, (uint8*)CenterBuffer);
				RHICmdList.UpdateTexture2D((FRHITexture*)BoundsExtentTexture->GetRHI(), 0, Region, BlockStride, (uint8*)ExtentBuffer);
			}
		});
	}
...
void AllocateOcclusionTests(const FScene* Scene, FViewOcclusionQueriesPerView& QueriesPerView, TArrayView<FViewInfo> Views, const FVisibleLightInfoArray& VisibleLightInfos)
{
	...
	FGraphicsPipelineStateInitializer* InGraphicsPSOInit = nullptr)
{
	...
	if (View.IsInstancedStereo())
	{
		...
		RHICmdList.SetStereoViewport(
			ViewRect.Min.X,
			SecondaryViewRect.Min.X,
			ViewRect.Min.Y,
			SecondaryViewRect.Min.Y,
			0.0f, ViewRect.Max.X,
			SecondaryViewRect.Max.X,
			ViewRect.Max.Y,
			SecondaryViewRect.Max.Y,
			1.0f);
	}
	else
	{
...
void FSceneRenderer::WaitOcclusionTests(FRHICommandListImmediate& RHICmdList)
{
	if (IsRunningRHIInSeparateThread())
	{
		...
		// Wait for async execution tasks from the previous render frame, namely the FenceOcclusionTasks pass.
