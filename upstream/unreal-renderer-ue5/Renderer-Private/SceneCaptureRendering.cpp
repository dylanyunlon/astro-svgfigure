...
extern TAutoConsoleVariable<bool> CVarMobileDiffuseFromCapture;

static TAutoConsoleVariable<bool> CVarDiffuseIrradianceReference(
	TEXT("r.SceneCapture.DiffuseIrradianceReference"),
	true,
	TEXT("Compute diffuse irradiance using reference implementation instead of from Spherical Harmonics.  Reference version looks\n")
	TEXT("much better, and is currently many times faster at the default 8x8 source mip resolution, because the original irradiance\n")
	TEXT("computation (designed for offline reflection capture builds) is totally unoptimized, and involves many passes.\n"),
...
public:
	...
	static FPermutationDomain GetPermutationVector(ESceneCaptureSource CaptureSource, bool bUse128BitRT, bool bForwardShadingEnabled, bool bIsReflectionCapture)
	{
		...
		case SCS_SceneColorHDRNoAlpha:
			SourceMode = bIsReflectionCapture ? ESourceMode::ColorOneAlpha : ESourceMode::ColorNoAlpha;
			break;
		...
		if (bForwardShadingEnabled && (SourceMode == ESourceMode::Normal || SourceMode == ESourceMode::BaseColor))
		{
			SourceMode = ESourceMode::ColorAndOpacity;
		}
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		static const TCHAR* ShaderSourceModeDefineName[] =
		{
			...
			TEXT("SOURCE_MODE_SCENE_COLOR_ONE_ALPHA"),
		};
	...
	const TArray<const FViewInfo*>& Views)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < NumViews; ViewIndex++)
	{
		...
		const FSceneCapturePS::FPermutationDomain PixelPermutationVector = FSceneCapturePS::GetPermutationVector(SceneCaptureSource, bUse128BitRT, bForwardShadingEnabled, View.IsRuntimeReflectionCapture());
		...
		{
			// CreatePublicGlobalUniformBuffer handles View.SubstrateViewData.SceneData==null.  Use null SceneData for depth captures, which won't
			// have produced Substrate data, to avoid an RDG validation error (even though technically the permutation doesn't use it).
			PassParameters->SubstratePublic = Substrate::CreatePublicGlobalUniformBuffer(GraphBuilder,
				SceneCaptureSource == SCS_SceneDepth || SceneCaptureSource == SCS_DeviceDepth ? nullptr : View.SubstrateViewData.SceneData);
		}
		...
		else
		{
			...
			TargetSize = View.bIsSceneCaptureCube ? ViewFamilyTexture->Desc.Extent : View.UnconstrainedViewRect.Size();
		}
			...
			[PassParameters, GraphicsPSOInit, VertexShader, PixelShader, &View, ViewIndex, TargetSize, SetViewportLambda = MoveTemp(SetViewportLambda)] (FRDGAsyncTask, FRHICommandList& RHICmdList)
		{
			...
			if (View.bIsSceneCaptureCube)
			{
				// When using timeslicing, cube capture faces are rendered over multiple frames.  In such cases, ViewRect is pushed to the origin,
				// while UnscaledViewRect is the actual offset where the output face should be written.  We don't want to use UnscaledViewRect in
				// the general case, as that will be incorrect if DRS has been inherited from the main view, but cube captures don't support that.
				DrawRectangle(
					RHICmdList,
	...
	bool bOrthographicCamera)
{
	...
	// TargetTexture (a.k.a. RenderTarget) is what gets rendered to, while OutputTexture is the final output.  For 2D scene captures, these
	// textures are the same.  For cube captures, OutputTexture will be a cube map, while TargetTexture will be a 2D render target containing
	// either one face of the cube map (when GSceneCaptureCubeSinglePass=0) or the six faces of the cube map tiled in a split screen configuration.
	...
	if (CopyInfos.Num() && !CopyInfos[0].Size.IsZero() && !OutputTexture->Desc.IsTextureCube())
	{
	...
	UE_CLOGF(FSceneCaptureLogUtils::bEnableSceneCaptureLogging, LogSceneCapture, Log, "Running UpdateSceneCaptureContent_RenderThread.");
...
static void BuildOrthoMatrix(FIntPoint InRenderTargetSize, float InOrthoWidth, float InNearPlane, float InFarPlane, int32 InTileID, int32 InNumXTiles, int32 InNumYTiles, FMatrix& OutProjectionMatrix)
{
	...
	const float NearPlane = InNearPlane;
	const float FarPlane = InFarPlane;
...
void BuildProjectionMatrix(FIntPoint InRenderTargetSize, float InFOV, float InNearClippingPlane, float InFarClippingPlane, FMatrix& OutProjectionMatrix)
{
	float const XAxisMultiplier = 1.0f;
	float const YAxisMultiplier = InRenderTargetSize.X / float(InRenderTargetSize.Y);
...
void GetShowOnlyAndHiddenComponents(USceneCaptureComponent* SceneCaptureComponent, TSet<FPrimitiveComponentId>& HiddenPrimitives, TOptional<TSet<FPrimitiveComponentId>>& ShowOnlyPrimitives)
{
	...
	else if (SceneCaptureComponent->ShowOnlyComponents.Num() > 0 || SceneCaptureComponent->ShowOnlyActors.Num() > 0)
	{
		...
		if (!bWarned)
		{
			UE_LOGF(LogRenderer, Log, "Scene Capture has ShowOnlyComponents or ShowOnlyActors ignored by the PrimitiveRenderMode setting! %ls", *SceneCaptureComponent->GetPathName());
			bWarned = true;
		}
	}
}
	...
	int32 CubemapFaceOffset)
{
	...
	check(CubemapFaceIndex == INDEX_NONE || Views.Num() == 1 || CubemapFaceIndex == CubeFace_MAX);

	// Initialize frame number
	ViewFamily.FrameNumber = ViewFamily.Scene->GetFrameNumber();
	ViewFamily.FrameCounter = GFrameCounter;

	ViewFamily.bOverrideVirtualTextureThrottle = EnumHasAnyFlags(Flags, ESetupViewFamilyFlags::OverrideVirtualTextureThrottle);
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		ViewInitOptions.SkylightScale = SceneCaptureComponent ? SceneCaptureComponent->SkylightScale : FLinearColor::White;
		ViewInitOptions.bIsSceneCapture = true;
		ViewInitOptions.bIsPlanarReflection = EnumHasAnyFlags(Flags, ESetupViewFamilyFlags::IsPlanarReflection);
		ViewInitOptions.bIsReflectionCapture = EnumHasAnyFlags(Flags, ESetupViewFamilyFlags::IsReflectionCapture);
		...
		if (EnumHasAnyFlags(Flags, ESetupViewFamilyFlags::CaptureSceneColor))
		{
		...
		if (SceneCaptureComponent)
		{
			...
			ViewInitOptions.SceneViewStateInterface = SceneCaptureComponent->GetViewState((uint32)CubemapFaceIndex < CubeFace_MAX ? CubemapFaceIndex : ViewIndex + CubemapFaceOffset);
...
static FSceneRenderer* CreateSceneRendererForSceneCapture(
	...
	float MaxViewDistance,
	float InFOV,
	ESceneRendererCreationFlags InFlags,
	FPostProcessSettings* PostProcessSettings,
	float PostProcessBlendWeight,
	const AActor* ViewActor,
	int32 CubemapFaceIndex = INDEX_NONE)
{
	...
	{
		// These flags are reserved for 2D captures :
		check(!EnumHasAnyFlags(InFlags, ESceneRendererCreationFlags::CameraCut2D | ESceneRendererCreationFlags::CopyMainViewTemporalSettings2D));
	}
	...
	FSceneViewFamilyContext ViewFamily(FSceneViewFamily::ConstructionValues(
		...
		.SetResolveScene(!EnumHasAnyFlags(InFlags, ESceneRendererCreationFlags::CaptureSceneColor))
	...
	ESetupViewFamilyFlags SetupViewFamilyFlags = ESetupViewFamilyFlags::None;
	if (EnumHasAnyFlags(InFlags, ESceneRendererCreationFlags::CaptureSceneColor))
	{
		SetupViewFamilyFlags |= ESetupViewFamilyFlags::CaptureSceneColor;
	}

	if (EnumHasAnyFlags(InFlags, ESceneRendererCreationFlags::OverrideVirtualTextureThrottle))
	{
		SetupViewFamilyFlags |= ESetupViewFamilyFlags::OverrideVirtualTextureThrottle;
	}
	...
	if (IsValid(SceneCaptureComponent2D))
	{
		...
		Views[0]->bCameraCut = EnumHasAnyFlags(InFlags, ESceneRendererCreationFlags::CameraCut2D);

		if (EnumHasAnyFlags(InFlags, ESceneRendererCreationFlags::CopyMainViewTemporalSettings2D))
		{
			const FSceneViewFamily* MainViewFamily = SceneCaptureComponent2D->MainViewFamily;
...
public:
		...
		, bAutoGenerateMips(InRenderTarget->bAutoGenerateMips)
	{
		...
		UserData->bExcludeFromSceneTextureExtents = CaptureComponent->bExcludeFromSceneTextureExtents;
	...
	virtual void OnPreRender(FRDGBuilder& GraphBuilder) override
	{
		...
		// If a Custom Render Pass output was accessed by Niagara, it will have converted it to external access, so switch it back to internal access.
		// If it's already in internal access mode (the common expected case), this is a nop.
...
{
	using namespace UE::Renderer::SceneCapture::Private;
	...
	FTextureRenderTargetResource* TextureRenderTargetResource = TextureRenderTarget ? TextureRenderTarget->GameThread_GetRenderTargetResource() : nullptr;
	...
	if (TextureRenderTargetResource)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogRenderer, Warning, "SceneCapture - Orthographic and tiling with CaptureSource not using SceneColor (i.e FinalColor) not compatible. SceneCapture render will not be tiled");
			}
		}
		...
		if (CaptureComponent->ShouldRenderWithMainViewCamera() && CaptureComponent->MainViewFamily)
		{
			...
			ViewRotationMatrix = MainView->ViewMatrices.GetWorldToView().RemoveTranslation();
			ProjectionMatrix = MainView->ViewMatrices.GetViewToClip();
		}
		else
		{
			...
			else
			{
				...
				{
					const float NearClip = (CaptureComponent->bOverride_CustomNearClippingPlane) ? CaptureComponent->CustomNearClippingPlane : GNearClippingPlane;
					const float FarClip = CaptureComponent->MaxViewDistanceOverride > 0.0f && CaptureComponent->bFiniteFarPlane ?
						FMath::Max(CaptureComponent->MaxViewDistanceOverride, NearClip) : NearClip;

					BuildProjectionMatrix(CaptureSize, FOV, NearClip, FarClip, ProjectionMatrix);
				}
				else
				{
					// Support custom near plane and far plane while keeping the old defaults.
					const float OrthoNearPlane = (CaptureComponent->bOverride_CustomNearClippingPlane) ? CaptureComponent->CustomNearClippingPlane : 0.0f;
					const float OrthoFarPlane = CaptureComponent->MaxViewDistanceOverride > 0.0f ?
						FMath::Max(CaptureComponent->MaxViewDistanceOverride, OrthoNearPlane + UE_KINDA_SMALL_NUMBER) : UE_FLOAT_HUGE_DISTANCE / 4.0f;

					if (bEnableOrthographicTiling)
					{
						BuildOrthoMatrix(CaptureSize, CaptureComponent->OrthoWidth, OrthoNearPlane, OrthoFarPlane, CaptureComponent->TileID, NumXTiles, NumYTiles, ProjectionMatrix);
						CaptureSize /= FIntPoint(NumXTiles, NumYTiles);
					}
					else
					{
						BuildOrthoMatrix(CaptureSize, CaptureComponent->OrthoWidth, OrthoNearPlane, OrthoFarPlane, -1, 0, 0, ProjectionMatrix);
					}
				}
			}
		}
		...
		ESceneRendererCreationFlags SceneRendererCreationFlags = ESceneRendererCreationFlags::None;
		if (bUseSceneColorTexture)
		{
			SceneRendererCreationFlags |= ESceneRendererCreationFlags::CaptureSceneColor;
		}

		// Copy temporal AA related settings for main view camera scene capture, to match jitter.  Don't match if resolution differs from main view,
		...
		// been computed yet.
		if ((CaptureComponent->MainViewFamily &&
			CaptureComponent->ShouldRenderWithMainViewCamera() &&
			CaptureComponent->ShouldRenderWithMainViewResolution() &&
		...
		{
			SceneRendererCreationFlags |= ESceneRendererCreationFlags::CopyMainViewTemporalSettings2D;
			if (CaptureComponent->MainViewFamily->Views[0]->bCameraCut)
			{
				SceneRendererCreationFlags |= ESceneRendererCreationFlags::CameraCut2D;
			}
		}
		else
		{
			if (CaptureComponent->bCameraCutThisFrame)
			{
				SceneRendererCreationFlags |= ESceneRendererCreationFlags::CameraCut2D;
			}
		}

		if (CaptureComponent->bOverrideVirtualTextureThrottle)
		{
			SceneRendererCreationFlags |= ESceneRendererCreationFlags::OverrideVirtualTextureThrottle;
		}
		...
		SceneRenderer->Views[0].bSceneCaptureMainViewJitter = EnumHasAnyFlags(SceneRendererCreationFlags, ESceneRendererCreationFlags::CopyMainViewTemporalSettings2D);
			...
			] (FRDGBuilder& GraphBuilder, const FSceneRenderFunctionInputs& Inputs)
		{
			...
			FRDGTextureRef OutputTexture = RegisterExternalTexture(GraphBuilder, TextureRenderTargetResource->TextureRHI, TEXT("SceneCaptureTexture"));
			...
			if (const FSceneViewState* ViewState = Inputs.Renderer->Views[0].ViewState)
			{
				if (ViewState)
				{
					...
					if (bLogSizes)
					{
						UE_LOGF(LogRenderer, Log, "LogSizes\tSceneCapture\t%ls\t%ls\t%dx%d", Inputs.FullPath, *TargetName.ToString(), TextureRenderTargetResource->GetSizeX(), TextureRenderTargetResource->GetSizeY());
					}
...
void FScene::UpdateSceneCaptureContents(USceneCaptureComponentCube* CaptureComponent, ISceneRenderBuilder& SceneRenderBuilder)
{
	...
	bool bIsReflectionCapture = CaptureComponent->ReflectionComponent != nullptr;

	UTextureRenderTargetCube* TextureTarget = nullptr;
	FTextureRenderTargetCubeResource* TextureRenderTarget = nullptr;
	bool bGenerateMips;
	FGenerateMipsParams GenerateMipsParams;
	FIntPoint CaptureSize;

	if (bIsReflectionCapture)
	{
		uint32 ReflectionCaptureSize = UReflectionCaptureComponent::GetReflectionCaptureSize();
		bGenerateMips = true;
		CaptureSize = FIntPoint(ReflectionCaptureSize, ReflectionCaptureSize);
	}
	else if (CaptureComponent->TextureTarget)
	{
		TextureTarget = CaptureComponent->TextureTarget;
		TextureRenderTarget = static_cast<FTextureRenderTargetCubeResource*>(TextureTarget->GameThread_GetRenderTargetResource());
		bGenerateMips = TextureTarget->bAutoGenerateMips;
		GenerateMipsParams = FGenerateMipsParams{ TextureTarget->MipsSamplerFilter == TF_Nearest ? SF_Point : (TextureTarget->MipsSamplerFilter == TF_Trilinear ? SF_Trilinear : SF_Bilinear), AM_Clamp, AM_Clamp };
	...
	else
	{
		...
		if (bIsReflectionCapture)
		{
			// For reflection captures, use the reflection capture actor name (reflection captures use a singleton CaptureComponent)
			Owner = CaptureComponent->ReflectionComponent->GetOwner();
		}
		else
		{
			Owner = CaptureComponent->GetOwner();
		}

		if (Owner)
		{
			// The label might be non-unique, so include the actor name as well
			EventName = Owner->GetActorNameOrLabel();

			FName ActorName = Owner->GetFName();
			if (ActorName != EventName)
			{
				EventName.Appendf(TEXT(" (%s)"), *ActorName.ToString());
			}
	...
	auto ComputeProjectionMatrix = [CaptureComponent, Transform, CaptureSize, FOVInRadians](ECubeFace TargetFace, FMatrix& OutViewRotationMatrix, FMatrix& OutProjectionMatrix)
	{
		...
		const float FarClip = CaptureComponent->MaxViewDistanceOverride > 0.0f && CaptureComponent->bFiniteFarPlane ?
			FMath::Max(CaptureComponent->MaxViewDistanceOverride, GNearClippingPlane) : GNearClippingPlane;

		BuildProjectionMatrix(CaptureSize, FOVInRadians, GNearClippingPlane, FarClip, OutProjectionMatrix);
	};
	...
	if (bIsReflectionCapture)
	{
		SceneRendererCreationFlags |= ESceneRendererCreationFlags::IsReflectionCapture;
	}

	SCENE_RENDER_GROUP_SCOPE(SceneRenderBuilder, MoveTemp(EventName), SceneRenderGroupFlags);

	// Reflection captures only support the single pass render case below, which uses a separately allocated render target for the six faces.  The reflection
	// environment cube map array isn't configured to be used as a render target, and direct individual face rendering would require that.
	if (GSceneCaptureCubeSinglePass == false && !bIsReflectionCapture)
	{
		for (int32 FaceIndex = 0; FaceIndex < (int32)ECubeFace::CubeFace_MAX; FaceIndex++)
		{
				...
				] (FRDGBuilder& GraphBuilder, const FSceneRenderFunctionInputs& Inputs)
			{
				...
				FRDGTextureRef OutputTexture = RegisterExternalTexture(GraphBuilder, TextureRenderTarget->TextureRHI, TEXT("SceneCaptureTexture"));
				UpdateSceneCaptureContent_RenderThread(
					GraphBuilder,
					Inputs.Renderer,
					Inputs.SceneUpdateInputs,
				...
				if (const FSceneViewState* ViewState = Inputs.Renderer->Views[0].ViewState)
				{
					...
					if (bLogSizes)
					{
						UE_LOGF(LogRenderer, Log, "LogSizes\tSceneCaptureCube[%d]\t%ls\t%dx%d", TargetFace, Inputs.FullPath, TextureRenderTarget->GetSizeX(), TextureRenderTarget->GetSizeY());
	...
	{
		int32 TimeSliceFirst = 0;
		int32 TimeSliceMask = (1 << CubeFace_MAX) - 1;		// All faces rendered
		if (bIsReflectionCapture)
		{
			int32 TimeSliceCount = FMath::Clamp(CaptureComponent->ReflectionTimeSliceCount, 1, CubeFace_MAX);
			TimeSliceFirst = FMath::Clamp(CaptureComponent->ReflectionTimeSliceFirst, 0, CubeFace_MAX - 1);
			TimeSliceMask &= ((1 << TimeSliceCount) - 1) << TimeSliceFirst;
		}

		TArray<FSceneCaptureViewInfo, TFixedAllocator<ECubeFace::CubeFace_MAX>> SceneCaptureViewInfos;
		for (int32 faceidx = 0; faceidx < (int32)ECubeFace::CubeFace_MAX; faceidx++)
		{
			if (!(TimeSliceMask & (1 << faceidx)))
			{
				continue;
			}
			...
			FSceneCaptureViewInfo& SceneCaptureViewInfo = SceneCaptureViewInfos.AddDefaulted_GetRef();
			SceneCaptureViewInfo.ViewRotationMatrix = ViewRotationMatrix;
			SceneCaptureViewInfo.ViewOrigin = ViewLocation;
			SceneCaptureViewInfo.ProjectionMatrix = ProjectionMatrix;
			SceneCaptureViewInfo.StereoPass = EStereoscopicPass::eSSP_FULL;
			SceneCaptureViewInfo.StereoViewIndex = INDEX_NONE;
			SceneCaptureViewInfo.ViewRect = FIntRect(ViewportOffset.X, ViewportOffset.Y, ViewportOffset.X + CaptureSize.X, ViewportOffset.Y + CaptureSize.Y);
			SceneCaptureViewInfo.FOV = 90.f;
		}
		...
		{
		public:
			FCubeFaceRenderTarget(FTextureRenderTargetCubeResource* InTextureRenderTarget, TRefCountPtr<IPooledRenderTarget>* InExternalTarget, FIntPoint CaptureSize)
			{
				...
				ExternalTarget = InExternalTarget;
				...
				FIntPoint Size;
				Size.X = CaptureSize.X * (GCubeFaceViewportOffsets[(int32)ECubeFace::CubeFace_MAX - 1][0] + 1);
				Size.Y = CaptureSize.Y * (GCubeFaceViewportOffsets[(int32)ECubeFace::CubeFace_MAX - 1][1] + 1);
			...
			{
				// Set the format now that it's available.  If no render target was specified, we are rendering a reflection capture, which is 16-bit float RGBA.
				CubeFaceDesc.Format = TextureRenderTarget ? TextureRenderTarget->GetRenderTargetTexture()->GetFormat() : PF_FloatRGBA;

				// Reallocate the cached external target if the dimensions or format change.
				const bool bCanReuseExternalTarget = ExternalTarget
					&& ExternalTarget->IsValid()
					&& (*ExternalTarget)->GetDesc().Extent == CubeFaceDesc.Extent
					&& (*ExternalTarget)->GetDesc().Format == CubeFaceDesc.Format;

				if (bCanReuseExternalTarget)
				{
		...
		private:
			...
			TRefCountPtr<IPooledRenderTarget>* ExternalTarget;
			FPooledRenderTargetDesc CubeFaceDesc;
			TRefCountPtr<IPooledRenderTarget> RenderTarget;
			FTextureRHIRef RenderTargetTexture;
		};

		TUniquePtr<FCubeFaceRenderTarget> CubeFaceTarget = MakeUnique<FCubeFaceRenderTarget>(TextureRenderTarget, bIsReflectionCapture ? &ReflectionSceneData.RuntimeCaptureTarget : nullptr, CaptureSize);
		...
		FSceneViewFamilyContext ViewFamily(FSceneViewFamily::ConstructionValues(
			...
			.SetResolveScene(!EnumHasAnyFlags(SceneRendererCreationFlags, ESceneRendererCreationFlags::CaptureSceneColor))
		...
		ESetupViewFamilyFlags SetupViewFamilyFlags = ESetupViewFamilyFlags::None;
		if (EnumHasAnyFlags(SceneRendererCreationFlags, ESceneRendererCreationFlags::CaptureSceneColor))
		{
			SetupViewFamilyFlags |= ESetupViewFamilyFlags::CaptureSceneColor;
		}

		if (EnumHasAnyFlags(SceneRendererCreationFlags, ESceneRendererCreationFlags::IsReflectionCapture))
		{
			SetupViewFamilyFlags |= ESetupViewFamilyFlags::IsReflectionCapture;
		}
		...
		SceneRenderBuilder.AddRenderer(SceneRenderer,
			[
				...
				, bGenerateMips
				, GenerateMipsParams
				, ReflectionComponent = CaptureComponent->ReflectionComponent
				, TimeSliceMask
				, TimeSliceFirst
			] (FRDGBuilder& GraphBuilder, const FSceneRenderFunctionInputs& Inputs)
		{
			FReflectionEnvironmentSceneData& ReflectionSceneData = Inputs.Renderer->Scene->ReflectionSceneData;
			int32 ReflectionCaptureIndex = INDEX_NONE;
			if (ReflectionComponent)
			{
				// Assume allocation of the reflection state may have failed, and skip rendering
				FCaptureComponentSceneState* CaptureSceneState = ReflectionSceneData.AllocatedReflectionCaptureState.Find(ReflectionComponent);
				if (!CaptureSceneState)
				{
					return false;
				}
				ReflectionCaptureIndex = CaptureSceneState->CubemapIndex;
			}

			CubeFaceTarget->InitRHI(GraphBuilder.RHICmdList);

			const bool bIncludesLastFace = TimeSliceMask & (1 << (CubeFace_MAX - 1));

			TArray<FRHICopyTextureInfo, TFixedAllocator<ECubeFace::CubeFace_MAX>> CopyInfos;

			if (bIncludesLastFace)
			{
				for (int32 FaceIndex = 0; FaceIndex < (int32)ECubeFace::CubeFace_MAX; FaceIndex++)
				{
					FRHICopyTextureInfo& CopyInfo = CopyInfos.AddDefaulted_GetRef();
					CopyInfo.Size.X = CaptureSize.X;
					CopyInfo.Size.Y = CaptureSize.Y;
					CopyInfo.SourcePosition.X = GCubeFaceViewportOffsets[FaceIndex][0] * CaptureSize.X;
					CopyInfo.SourcePosition.Y = GCubeFaceViewportOffsets[FaceIndex][1] * CaptureSize.Y;
					CopyInfo.DestSliceIndex = FaceIndex;
				}
			}

			const bool bClearRenderTarget = (TimeSliceFirst == 0);
			const bool bOrthographicCamera = false;

			FRDGTextureRef OutputTexture;
			if (TextureRenderTarget)
			{
				OutputTexture = RegisterExternalTexture(GraphBuilder, TextureRenderTarget->TextureRHI, TEXT("SceneCaptureTexture"));
			}
			else
			{
				OutputTexture = GraphBuilder.CreateTexture(FRDGTextureDesc::CreateCube(
					CaptureSize.X,
					PF_FloatRGBA,
					FClearValueBinding::None,
					ETextureCreateFlags::ShaderResource | ETextureCreateFlags::UAV,
					ReflectionSceneData.CubemapArray.GetRenderTarget()->GetDesc().NumMips), TEXT("SceneCaptureTexture"));
			}
			...
			UpdateSceneCaptureContent_RenderThread(
				...
				OutputTexture,
				CopyInfos,
				bGenerateMips && bIncludesLastFace,
				GenerateMipsParams,
				bClearRenderTarget,
				bOrthographicCamera);

			if (ReflectionCaptureIndex >= 0)
			{
				// If we failed to properly render shadows due to the timesliced shadow atlas going invalid (typically due to an edit operation
				// causing the light render proxy to be recreated), skip copying it back to the reflection cube map array, to avoid a glitch.
				// jhoerner_todo:  Communicate this back to the game thread to indicate that it should retry the capture immediately, rather than
				// wait on its next timeslice.
				if (bIncludesLastFace && !Inputs.Renderer->Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFailed)
				{
					// Only generate diffuse irradiance mips if the rendering feature is enabled for the reflection pass.
					if (CVarMobileDiffuseFromCapture.GetValueOnAnyThread())
					{
						if (CVarDiffuseIrradianceReference.GetValueOnRenderThread())
						{
							// Uses exhaustive convolution to 8x8 cube map mips and below
							ComputeDiffuseIrradianceReference(GraphBuilder, Inputs.Renderer->FeatureLevel, Inputs.Renderer->Views[0].ShaderMap, OutputTexture, OutputTexture);
						}
						else
						{
							// Generates Spherical Harmonics from 32x32 mip, then evaluates that for the 8x8 cube map mips and below
							ComputeDiffuseIrradiance(GraphBuilder, Inputs.Renderer->Views[0].ShaderMap, OutputTexture, nullptr);
						}
					}

					FRHICopyTextureInfo ReflectionCopyInfo;
					ReflectionCopyInfo.DestSliceIndex = ReflectionCaptureIndex * 6;
					ReflectionCopyInfo.NumSlices = 6;
					ReflectionCopyInfo.NumMips = ReflectionSceneData.CubemapArray.GetRenderTarget()->GetDesc().NumMips;
					AddCopyTexturePass(GraphBuilder, OutputTexture, GraphBuilder.RegisterExternalTexture(ReflectionSceneData.CubemapArray.GetRenderTarget()), ReflectionCopyInfo);

					// The cubemap slot now holds freshly-rendered content, so the capture may be used for shading.  Re-find the state here
					// rather than reusing the earlier pointer, since the cache may have been mutated (and its TMap storage relocated) during
					// scene rendering above.  Mark that captures have changed, so SortedCaptures get rebuilt.
					if (FCaptureComponentSceneState* StateForShading = ReflectionSceneData.AllocatedReflectionCaptureState.Find(ReflectionComponent))
					{
						if (!StateForShading->bRenderedForShading)
						{
							StateForShading->bRenderedForShading = true;
							ReflectionSceneData.AllocatedReflectionCaptureStateHasChanged = true;
						}
					}
				}
			}
			...
			if (Inputs.Renderer->Views[0].ViewState)
			{
				...
				if (bLogSizes)
				{
					UE_LOGF(LogRenderer, Log, "LogSizes\tSceneCaptureCube\t%ls\t%dx%d", Inputs.FullPath, CubeFaceTarget->GetSizeXY().X, CubeFaceTarget->GetSizeXY().Y);
					for (int32 FaceIdx = 0; FaceIdx < Inputs.Renderer->Views.Num(); FaceIdx++)
					{
