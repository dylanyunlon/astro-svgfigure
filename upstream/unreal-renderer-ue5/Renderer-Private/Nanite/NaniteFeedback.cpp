...
FFeedbackManager::FFeedbackManager()
{
		...
		[this](GPUMessage::FReader Message)
	{
		...
		const uint32 MaxVisibleAssemblyParts	= Nanite::FGlobalResources::GetMaxVisibleAssemblyParts();
		...
		const uint32 PeakVisibleAssemblyParts	= Message.Read<uint32>(0);

		if (NodeState.Update(PeakNodes, MaxNodes))
		{
			UE_LOGF(LogRenderer, Warning, "Nanite node buffer overflow detected. New high-water mark is %d / %d. "
		...
		{
			UE_LOGF(LogRenderer, Warning, "Nanite candidate cluster buffer overflow detected. New high-water mark is %d / %d. "
		...
		{
			UE_LOGF(LogRenderer, Warning, "Nanite visible cluster buffer overflow detected. New high-water mark is %d / %d. "
		...
		if (VisibleAssemblyPartState.Update(PeakVisibleAssemblyParts, MaxVisibleAssemblyParts))
		{
			UE_LOGF(LogRenderer, Warning, "Nanite visible assembly part buffer overflow detected. New high-water mark is %d / %d. "
												"Increase r.Nanite.MaxVisibleAssemblyParts to prevent potential visual artifacts.", VisibleAssemblyPartState.HighWaterMark, MaxVisibleAssemblyParts);
		}
	});
		...
		[this](TMultiMap<FCoreDelegates::EOnScreenMessageSeverity, FText>& OutMessages)
	{
		...
		if (CurrentTime - VisibleAssemblyPartState.LatestOverflowTime < ShowWarningSeconds)
		{
			OutMessages.Add(FCoreDelegates::EOnScreenMessageSeverity::Warning,
				FText::Format(LOCTEXT("NaniteVisibleAssemblyPartOverflow",
					"Nanite visible assembly part buffer overflow detected: {0} / {1}. High-water mark is {2}. "
					"Increase r.Nanite.MaxVisibleAssemblyParts to prevent potential visual artifacts."),
					VisibleAssemblyPartState.LatestOverflowPeak, MaxVisibleAssemblyParts, VisibleAssemblyPartState.HighWaterMark));
		}
...
void FFeedbackManager::ReportMaterialPerformanceWarning(const FString &MaterialName)
{
	...
	if (bShouldLogNow)
	{
		UE_LOGF(LogRenderer, Log, "Performance Warning: Programmable Nanite material uses PDO or is Masked, %ls", *MaterialName);
	}
}
