...
#include "Nanite/NaniteFeedback.h"
...
static const FLinearColor NaniteRasterClearColor = FLinearColor::Transparent;
...
static TAutoConsoleVariable<int32> CVarNaniteVSMInvalidateOnLODDelta(
	...
	TEXT("  NOTE: May cause a large increase in invalidations in cases where the streamer has difficulty keeping up; see r.Shadow.Virtual.DeferredInvalidationBudget to throttle."),
	ECVF_RenderThreadSafe
);
...
static TAutoConsoleVariable<int32> CVarNaniteFastVisBufferClear(
	...
	1,
	TEXT("Whether the fast clear optimization is enabled. Set to 2 for tile clear. Set to 3 for metadata clear."),
	ECVF_RenderThreadSafe
);
...
static TAutoConsoleVariable<int32> CVarNaniteCullingSkinnedNodeBounds(
	TEXT("r.Nanite.Culling.SkinnedNodeBounds"),
	1,
	TEXT("Set to 0 to test disabling skinned node bounds, falling back to instance bounds."),
	ECVF_RenderThreadSafe
);
...
class FNaniteCurveClusterParameters;
...
class FInstanceCull_CS : public FNaniteGlobalShader
{
	...
	static EShaderPermutationPrecacheRequest ShouldPrecachePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		FPermutationDomain PermutationVector(Parameters.PermutationId);
		/*
		// Debug permutation is also used at runtime without debug rendering enabled, so needs to be precached because otherwise
		// severe runtime hitches are triggered
...
class FRasterBinBuild_CS : public FNaniteGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<FNaniteRasterGroupMeta>,	OutRasterGroupMeta)
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWBuffer<uint>,								OutRasterGroupArgsSWHW)
	...
	{
		FNaniteGlobalShader::ModifyCompilationEnvironment(Parameters, OutEnvironment);
		const bool bForceBatching = !FDataDrivenShaderPlatformInfo::GetSupportsPrimitiveShaders(Parameters.Platform);
...
class FRasterGroupInit_CS : public FNaniteGlobalShader
{
	DECLARE_GLOBAL_SHADER(FRasterGroupInit_CS);
	SHADER_USE_PARAMETER_STRUCT(FRasterGroupInit_CS, FNaniteGlobalShader);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, RasterGroupCount)
	END_SHADER_PARAMETER_STRUCT()
...
IMPLEMENT_GLOBAL_SHADER(FRasterGroupInit_CS, "/Engine/Private/Nanite/NaniteRasterBinning.usf", "RasterGroupInit", SF_Compute);

class FRasterGroupReserve_CS : public FNaniteGlobalShader
{
	DECLARE_GLOBAL_SHADER(FRasterGroupReserve_CS);
	SHADER_USE_PARAMETER_STRUCT(FRasterGroupReserve_CS, FNaniteGlobalShader);
...
IMPLEMENT_GLOBAL_SHADER(FRasterGroupReserve_CS, "/Engine/Private/Nanite/NaniteRasterBinning.usf", "RasterGroupReserve", SF_Compute);

class FRasterGroupDepthBlock_CS : public FNaniteGlobalShader
{
	DECLARE_GLOBAL_SHADER(FRasterGroupDepthBlock_CS);
	SHADER_USE_PARAMETER_STRUCT(FRasterGroupDepthBlock_CS, FNaniteGlobalShader);
...
IMPLEMENT_GLOBAL_SHADER(FRasterGroupDepthBlock_CS, "/Engine/Private/Nanite/NaniteRasterBinning.usf", "RasterGroupDepthBlock", SF_Compute);
...
IMPLEMENT_GLOBAL_SHADER(FRasterBinFinalize_CS, "/Engine/Private/Nanite/NaniteRasterBinning.usf", "RasterGroupFinalize", SF_Compute);
	...
	bool bMaterialUsesDisplacement)
{
	...
	Flags.bTranslucent			= RasterPipeline.bTranslucent;
...
{
	const bool bProgrammableVertexUVs = RasterMaterial.MaterialUsesVertexInterpolator_GameThread() || RasterMaterial.GetNumCustomizedUVs() > 0;
...
{
	const bool bProgrammableVertexUVs = RasterMaterial.MaterialUsesVertexInterpolator_RenderThread() || RasterMaterial.GetNumCustomizedUVs() > 0;
...
class FMicropolyRasterizeCS : public FNaniteMaterialShader
{
	...
	class FCurvesDim : SHADER_PERMUTATION_BOOL("NANITE_CURVES");
	class FSplineDeformDim : SHADER_PERMUTATION_BOOL("USE_SPLINEDEFORM");
	class FSkinningDim : SHADER_PERMUTATION_BOOL("USE_SKINNING");
	...
	static bool ShouldCompilePermutation(const FMaterialShaderPermutationParameters& Parameters)
	{
		...
		{
			if (!NaniteVoxelsSupported() || !Parameters.MaterialParameters.bIsDefaultMaterial || PermutationVector.Get<FTwoSidedDim>() || PermutationVector.Get<FSplineDeformDim>() || PermutationVector.Get<FCurvesDim>())
			{
				return false;
			}
		}

		if (PermutationVector.Get<FCurvesDim>())
		{
			if (!NaniteCurvesSupported() || !Parameters.MaterialParameters.bIsDefaultMaterial || PermutationVector.Get<FTwoSidedDim>() || PermutationVector.Get<FSplineDeformDim>() )
			{
				return false;
			}
		}
	...
	static void ModifyCompilationEnvironment(const FMaterialShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		{
			// Using *AndCompileArgument because there's some validation happening in the shader compiler to make sure we're using the expected uniform buffer
			OutEnvironment.SetDefineAndCompileArgument(TEXT("SHADER_USES_ONLY_VERTEX_DATA"), true);
		}
...
class FHWRasterizeVS : public FNaniteMaterialShader
{
	...
	static bool ShouldCompilePermutation(const FMaterialShaderPermutationParameters& Parameters)
	{
		...
		if (!PermutationVector.Get<FPrimShaderDim>() && FDataDrivenShaderPlatformInfo::GetStripNaniteVertexShaders(Parameters.Platform))
		{
			// HW rasterization vertex shaders have been removed from the build
			return false;
		}
...
class FHWRasterizeMS : public FNaniteMaterialShader
{
	...
	static bool ShouldCompilePermutation(const FMaterialShaderPermutationParameters& Parameters)
	{
		...
		if (!NaniteMeshShadersSupported(Parameters.Platform))
		{
			// Skip compilation if mesh shaders are stripped from the build.
			return false;
		}
...
public:
	...
	static void ModifyCompilationEnvironment(const FMaterialShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		const bool bForceBatching = !FDataDrivenShaderPlatformInfo::GetSupportsPrimitiveShaders(Parameters.Platform);
		if (bForceBatching || (PermutationVector.Get<FPrimShaderDim>() && PermutationVector.Get<FVertexProgrammableDim>()))
		{
			OutEnvironment.SetDefine(TEXT("NANITE_VERT_REUSE_BATCH"), 1);
		}
...
struct FRasterizerPass
{
	...
	inline uint32 CalcSortKey() const
	{
		...
		const bool bDepthTest = bPixelProgrammable || RasterPipeline.bVoxel || RasterPipeline.bCurve;
		if (bDepthTest)
		{
			// Place voxel, curve, and pixel programmable rasterizers last as they do depth rejection.
			// Assume pixel programmable shaders are likely closer than voxels, so draw them first.
			SortKey |= (RasterPipeline.bVoxel || RasterPipeline.bCurve) ? (1u << 31) : 0u;
	...
	{
		return (RasterBin & FGlobalResources::GetFixedFunctionBinMask()) == RasterBin;
	}
};


struct FRasterizerPassGroupKey
{
	FShaderHash		PixelShaderHash;
	FShaderHash		VertexShaderHash;
	FShaderHash		MeshShaderHash;
	FShaderHash		ClusterComputeShaderHash;
	FShaderHash		PatchComputeShaderHash;

	TArray<uint8>	BindingsData;
	uint32			MaterialFlags = 0u;
	uint32			CachedHash = 0u;

	FRasterizerPassGroupKey() = default;
	FRasterizerPassGroupKey(const FRasterizerPass& RasterizerPass, uint32 InMaterialFlags,
		const FScene* Scene,
		const ERHIFeatureLevel::Type FeatureLevel)
	{
		// TODO: Cache/Optimize how we get the bindings?
		BindingsData.Reserve(1024);

		if (RasterizerPass.RasterPixelShader.IsValid())
		{
			PixelShaderHash = RasterizerPass.RasterPixelShader.GetPixelShader()->GetHash();
			
			const FMeshDrawShaderBindingsLayout Layout(TShaderRef<FShader>(RasterizerPass.RasterPixelShader));
			const uint32 Offset = BindingsData.Num();
			BindingsData.AddZeroed(Layout.GetDataSizeBytes());
			FMeshDrawSingleShaderBindings ShaderBindings(Layout, BindingsData.GetData() + Offset);
			RasterizerPass.RasterPixelShader->GetShaderBindings(Scene, FeatureLevel, *RasterizerPass.PixelMaterialProxy, *RasterizerPass.PixelMaterial, ShaderBindings);
		}
		
		if (RasterizerPass.RasterVertexShader.IsValid() && RasterizerPass.RasterVertexShader->GetFrequency() == SF_Vertex)
		{
			VertexShaderHash = RasterizerPass.RasterVertexShader.GetVertexShader()->GetHash();
			
			const FMeshDrawShaderBindingsLayout Layout(TShaderRef<FShader>(RasterizerPass.RasterVertexShader));
			const uint32 Offset = BindingsData.Num();
			BindingsData.AddZeroed(Layout.GetDataSizeBytes());
			FMeshDrawSingleShaderBindings ShaderBindings(Layout, BindingsData.GetData() + Offset);
			RasterizerPass.RasterVertexShader->GetShaderBindings(Scene, FeatureLevel, *RasterizerPass.VertexMaterialProxy, *RasterizerPass.VertexMaterial, ShaderBindings);
		}

		if (RasterizerPass.RasterMeshShader.IsValid() && RasterizerPass.RasterMeshShader->GetFrequency() == SF_Mesh)
		{
			MeshShaderHash = RasterizerPass.RasterMeshShader.GetMeshShader()->GetHash();
			
			const FMeshDrawShaderBindingsLayout Layout(TShaderRef<FShader>(RasterizerPass.RasterMeshShader));
			const uint32 Offset = BindingsData.Num();
			BindingsData.AddZeroed(Layout.GetDataSizeBytes());
			FMeshDrawSingleShaderBindings ShaderBindings(Layout, BindingsData.GetData() + Offset);
			RasterizerPass.RasterMeshShader->GetShaderBindings(Scene, FeatureLevel, *RasterizerPass.VertexMaterialProxy, *RasterizerPass.VertexMaterial, ShaderBindings);
		}

		if (RasterizerPass.ClusterComputeShader.IsValid())
		{
			if (RasterizerPass.ClusterComputeShader->GetFrequency() == SF_Compute || RasterizerPass.ClusterComputeShader->GetFrequency() == SF_WorkGraphComputeNode)
			{
				if (RasterizerPass.ClusterComputeShader->GetFrequency() == SF_Compute)
				{
					ClusterComputeShaderHash = RasterizerPass.ClusterComputeShader.GetComputeShader()->GetHash();
				}
				else // RasterizerPass.ClusterComputeShader->GetFrequency() == SF_WorkGraphComputeNode
				{
					ClusterComputeShaderHash = RasterizerPass.ClusterComputeShader.GetWorkGraphShader()->GetHash();
				}

				const FMeshDrawShaderBindingsLayout Layout(TShaderRef<FShader>(RasterizerPass.ClusterComputeShader));
				const uint32 Offset = BindingsData.Num();
				BindingsData.AddZeroed(Layout.GetDataSizeBytes());
				FMeshDrawSingleShaderBindings ShaderBindings(Layout, BindingsData.GetData() + Offset);
				RasterizerPass.ClusterComputeShader->GetShaderBindings(Scene, FeatureLevel, *RasterizerPass.ComputeMaterialProxy, *RasterizerPass.ComputeMaterial, ShaderBindings);
			}
			else
			{
				check(false);
		...
		if (RasterizerPass.PatchComputeShader.IsValid())
		{
			if (RasterizerPass.PatchComputeShader->GetFrequency() == SF_Compute || RasterizerPass.PatchComputeShader->GetFrequency() == SF_WorkGraphComputeNode)
			{
				if (RasterizerPass.PatchComputeShader->GetFrequency() == SF_Compute)
				{
					PatchComputeShaderHash = RasterizerPass.PatchComputeShader.GetComputeShader()->GetHash();
				}
				else // RasterizerPass.PatchComputeShader->GetFrequency() == SF_WorkGraphComputeNode
				{
					PatchComputeShaderHash = RasterizerPass.PatchComputeShader.GetWorkGraphShader()->GetHash();
				}

				const FMeshDrawShaderBindingsLayout Layout(TShaderRef<FShader>(RasterizerPass.PatchComputeShader));
				const uint32 Offset = BindingsData.Num();
				BindingsData.AddZeroed(Layout.GetDataSizeBytes());
				FMeshDrawSingleShaderBindings ComputeShaderBindings(Layout, BindingsData.GetData() + Offset);
				RasterizerPass.PatchComputeShader->GetShaderBindings(Scene, FeatureLevel, *RasterizerPass.ComputeMaterialProxy, *RasterizerPass.ComputeMaterial, ComputeShaderBindings);
			}
			else
			{
				check(false);
	...
	{
		uint32 Hash = 0;
		Hash = HashCombine(Hash, GetTypeHash(PixelShaderHash));
		Hash = HashCombine(Hash, GetTypeHash(VertexShaderHash));
		Hash = HashCombine(Hash, GetTypeHash(MeshShaderHash));
		Hash = HashCombine(Hash, GetTypeHash(ClusterComputeShaderHash));
		Hash = HashCombine(Hash, GetTypeHash(PatchComputeShaderHash));
		Hash = HashCombine(Hash, GetTypeHash(FXxHash64::HashBuffer(BindingsData)));
		Hash = HashCombine(Hash, MaterialFlags);
		CachedHash = Hash;
	}

	friend bool operator==(const FRasterizerPassGroupKey& A, const FRasterizerPassGroupKey& B) = default;
};

FORCEINLINE uint32 GetTypeHash(const FRasterizerPassGroupKey& K)
{
	return K.CachedHash;
}

struct FRasterizerPassGroup
{
	TArray<uint32, TInlineAllocator<4>> RasterizerPasses;
...
static FORCEINLINE const FString& GetRasterMaterialName(const FRasterizerPass& InRasterPass)
{
	...
	const bool bFixedFunctionBin = (InRasterPass.RasterBin & FGlobalResources::GetFixedFunctionBinMask()) == InRasterPass.RasterBin;
	if (bFixedFunctionBin)
	{
		static const FString Bin0	= "Fixed Function";

		static const FString Bin1	= "Fixed Function (TwoSided)";
		static const FString Bin2	= "Fixed Function (Spline)";
		static const FString Bin4	= "Fixed Function (Skinned)";
		static const FString Bin8	= "Fixed Function (CastShadow)";
		static const FString Bin16	= "Fixed Function (Voxel)";
		static const FString Bin32	= "Fixed Function (Curve)";

		// Note: Spline and Skinned are mutually exclusive

		static const FString Bin9	= "Fixed Function (TwoSided | CastShadow)";

		static const FString Bin3	= "Fixed Function (Spline | TwoSided)";
		static const FString Bin10	= "Fixed Function (Spline | CastShadow)";
		static const FString Bin11	= "Fixed Function (Spline | TwoSided | CastShadow)";

		static const FString Bin5	= "Fixed Function (Skinned | TwoSided)";
		static const FString Bin12	= "Fixed Function (Skinned | CastShadow)";
		static const FString Bin13	= "Fixed Function (Skinned | TwoSided | CastShadow)";

		static const FString Bin20	= "Fixed Function (Voxel | Skinned)";
		static const FString Bin24	= "Fixed Function (Voxel | CastShadow)";
		static const FString Bin28	= "Fixed Function (Voxel | CastShadow | Skinned)";

		static const FString Bin36	= "Fixed Function (Curve | Skinned)";
		static const FString Bin40	= "Fixed Function (Curve | CastShadow)";
		static const FString Bin44	= "Fixed Function (Curve | CastShadow | Skinned)";
		...
		case NANITE_FIXED_FUNCTION_BIN_CURVE:
			return Bin32;
		...
		case (NANITE_FIXED_FUNCTION_BIN_CURVE | NANITE_FIXED_FUNCTION_BIN_SKINNED):
			return Bin36;

		case (NANITE_FIXED_FUNCTION_BIN_CURVE | NANITE_FIXED_FUNCTION_BIN_CAST_SHADOW):
			return Bin40;

		case (NANITE_FIXED_FUNCTION_BIN_CURVE | NANITE_FIXED_FUNCTION_BIN_CAST_SHADOW | NANITE_FIXED_FUNCTION_BIN_SKINNED):
			return Bin44;
		}
	}
...
{
	const bool bTriangle = !(bVoxel || bCurve);
	...
	check(bTriangle || !(bSplineMesh || bIsTwoSided));
	...
	PermutationVectorCS_Cluster.Set<FMicropolyRasterizeCS::FCurvesDim>(bCurve);
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	const bool bPSORequired = UPSOPrecacheSettingsManager::GetSettings().bNaniteProgrammableRasterRequired;
	...
	{		
		// Graphics PSO setup
		{
			...
			FGraphicsPipelineStateInitializer GraphicsPSOInit = MinimalPipelineStateInitializer.AsGraphicsPipelineStateInitializer(false);
			if (MinimalPipelineStateInitializer.BoundShaderState.IsShaderAllLoaded())
			{
				...
				PSOPrecacheData.bRequired = bPSORequired;
			#if PSO_PRECACHING_VALIDATE
		...
		// Cluster CS PSO Setup
		{
			...
			if (ClusterShaders->TryGetComputeShader(&ClusterCS))
			{
				...
				if (ComputePSOPrecacheData.SetComputeShader(ClusterCS))
				{
					ComputePSOPrecacheData.bRequired = bPSORequired;
				#if PSO_PRECACHING_VALIDATE
		...
		if (bDisplacement)
		{
			...
			if (PatchShader.TryGetComputeShader(&PatchCS))
			{
				FPSOPrecacheData ComputePSOPrecacheData;
				ComputePSOPrecacheData.Type = FPSOPrecacheData::EType::Compute;
				if (ComputePSOPrecacheData.SetComputeShader(PatchCS))
				{
					ComputePSOPrecacheData.bRequired = bPSORequired;
				#if PSO_PRECACHING_VALIDATE
					ComputePSOPrecacheData.ResourceName = Material.GetAssetName();
							...
							for (uint32 FixedDisplacementFallbackMesh = 0; FixedDisplacementFallbackMesh < 2; ++FixedDisplacementFallbackMesh)
							{
								...
								for (uint32 Voxel = 0; Voxel < 2; ++Voxel)
								{
									...
									for (uint32 Curve = 0; Curve < 2; ++Curve)
									{
										...
										if (bCurve && (bVoxel || bIsTwoSided || bSplineMesh))
											continue;
										...
										CollectRasterPSOInitializersForPermutation(Material, ShaderPlatform, HardwarePath, bVertexProgrammable, bPixelProgrammable, bIsTwoSided, bSplineMesh, bSkinnedMesh,
											bDisplacement, bFixedDisplacementFallback, bVoxel, bCurve, PermutationVectorVS, PermutationVectorMS, PermutationVectorPS, PermutationVectorCluster, PermutationVectorPatch, PSOCollectorIndex, PSOInitializers);
									}
								}
							}
						}
...
static bool IsNanitePrimeHZBEnabled()
{
	static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Nanite.PrimeHZB"));
	return CVar && CVar->GetValueOnAnyThread() != 0;
}

// Describes one raster configuration the precache needs to enumerate. 
struct FRasterPrecacheVariant
{
...
// Returns the set of raster variants the precache should enumerate for the given platform.
// Note: Lumen, HitProxy and MaterialCache pipelines are not enumerated here today;
static TArray<FRasterPrecacheVariant, TInlineAllocator<3>> GetRasterPrecacheVariants(EShaderPlatform ShaderPlatform, ERHIFeatureLevel::Type FeatureLevel)
{
	TArray<FRasterPrecacheVariant, TInlineAllocator<3>> Variants;

	// Primary: VisBuffer, no VSM.
	Variants.Add({ ERasterPipeline::Primary, EOutputBufferMode::VisBuffer, /* bHasVSM */ false, /* bIsMaterialCache */ false });

	// Shadows: depth-only, VSM if supported on this platform.
	Variants.Add({ ERasterPipeline::Shadows, EOutputBufferMode::DepthOnly, UseVirtualShadowMaps(ShaderPlatform, FeatureLevel), /* bIsMaterialCache */ false });

	// Optional Nanite::PrimeHZB pass: Primary pipeline rasterized depth-only without VSM. This produces
	// a (DepthOnly=true, VirtualShadowMap=false) permutation that neither Primary nor Shadows covers.
	if (IsNanitePrimeHZBEnabled())
	{
		Variants.Add({ ERasterPipeline::Primary, EOutputBufferMode::DepthOnly, /* bHasVSM */ false, /* bIsMaterialCache */ false });
	}
...
void CollectRasterPSOInitializersForPipeline(
	...
	FNaniteRasterPipeline RasterPipeline,
	int32 PSOCollectorIndex,
	const FRasterPrecacheVariant& Variant,
	bool bDefaultMaterial,
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	EShaderPlatform ShaderPlatform = GetFeatureLevelShaderPlatform(FeatureLevel);
	const ERasterHardwarePath HardwarePath = GetRasterHardwarePath(ShaderPlatform, Variant.Pipeline);
	...
	SetupPermutationVectors(
		...
		HardwarePath,
		bVisualizeActive,
		Variant.bHasVirtualShadowMapArray,
		Variant.bIsMaterialCache,
		PermutationVectorVS,
		PermutationVectorMS,
		PermutationVectorPS,
		PermutationVectorCS_Cluster,
	...
	{
		const auto AddPSOInitializers = [&](FNaniteRasterPipeline& RasterPipeline)
		{
			...
			CollectRasterPSOInitializersForPermutation(RasterMaterial, ShaderPlatform, HardwarePath, bVertexProgrammable, bPixelProgrammable, bIsTwoSided, bSplineMesh, bSkinnedMesh, bDisplacement, bFixedDisplacementFallback,
				/* bVoxel */ false, /* bCurve */ false, PermutationVectorVS, PermutationVectorMS, PermutationVectorPS, PermutationVectorCS_Cluster, PermutationVectorCS_Patch, PSOCollectorIndex, PSOInitializers);
		};		

		AddPSOInitializers(RasterPipeline);
		
		FNaniteRasterPipeline FallbackPipeline;
		if (RasterPipeline.GetFallbackPipeline(FallbackPipeline))
		{
			AddPSOInitializers(FallbackPipeline);
		}
	}
}
...
void CollectRasterPSOInitializers(
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	// Only enable pixel eval on masked if not marked as ignored
	bool bIsMasked = RasterMaterial.IsMasked() && !PreCacheParams.bIgnoreMaskMaterial;

	// Set up a RasterPipeline that enables the feature set we're collecting for
	FNaniteRasterPipeline RasterPipeline;
	RasterPipeline.bCastShadow = PreCacheParams.bCastShadow;
	RasterPipeline.bWPOEnabled = RasterMaterial.MaterialUsesWorldPositionOffset_GameThread();
	RasterPipeline.bDisplacementEnabled = RasterMaterial.MaterialUsesDisplacement_GameThread();
	RasterPipeline.bPerPixelEval = bIsMasked || RasterMaterial.MaterialUsesPixelDepthOffset_GameThread();
	...
	RasterPipeline.bHasDisplacementFadeOut = RasterMaterial.IsDisplacementFadeEnabled();
	RasterPipeline.bHasPixelDistance = PreCacheParams.bHasPixelDistance;
	RasterPipeline.bHasWPODistance = PreCacheParams.bHasWPODistance;

	for (const FRasterPrecacheVariant& Variant : GetRasterPrecacheVariants(ShaderPlatform, SceneTexturesConfig.FeatureLevel))
	{
		CollectRasterPSOInitializersForPipeline(SceneTexturesConfig.FeatureLevel, RasterMaterial, RasterPipeline, PSOCollectorIndex, Variant, PreCacheParams.bDefaultMaterial, PSOInitializers);
	}
}
...
{
	static const int32 PSOCollectorIndex = FPSOCollectorCreateManager::GetIndex(EShadingPath::Deferred, TEXT("NaniteRaster"));
	
	const FMaterial& RasterMaterial = RasterPipeline.RasterMaterial->GetIncompleteMaterialWithFallback(FeatureLevel);
	const bool bDefaultMaterial = false;

	const EShaderPlatform ShaderPlatform = GetFeatureLevelShaderPlatform(FeatureLevel);
	for (const FRasterPrecacheVariant& Variant : GetRasterPrecacheVariants(ShaderPlatform, FeatureLevel))
	{
		CollectRasterPSOInitializersForPipeline(FeatureLevel, RasterMaterial, RasterPipeline, PSOCollectorIndex, Variant, bDefaultMaterial, OutPSOPrecacheData);
	}
}
...
inline void SetHWBundleParameters(
	...
	const FMaterial& Material,
	const FRasterizerPassGroup& Group,
	const TConstArrayView<FRasterizerPass> RasterizerPasses,
	ERHIFeatureLevel::Type FeatureLevel,
	bool bPixelShader,
	bool bBindless)
{
	...
	if(bBindless)
	{
		for (const uint32 PassIndex : Group.RasterizerPasses)
		{
			FRHIUniformBuffer* MaterialUB = bPixelShader ? 
				RasterizerPasses[PassIndex].PixelMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBuffer :
				RasterizerPasses[PassIndex].VertexMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBuffer;
...
struct FCurveResources
{
	uint32 FrameIndex = 0;
	uint32 GroupIndex = ~uint32(0u); // TODO_CURVE: build a list of groups (fixed, skinned, ...)
	...
	// Tiles
	uint32 TileSegmentMaxCount = 0;
	FRDGBufferRef TileCount = nullptr;
	FRDGBufferRef TileOffset = nullptr;
	FRDGBufferRef TileSegment = nullptr;
	FRDGBufferRef TileMinMaxZ = nullptr;
	FRDGTextureRef TileHeat = nullptr;

	// Fine raster
	FRDGTextureRef OutCoverage = nullptr;
	FRDGTextureRef OutVisibility = nullptr;
	FRDGTextureRef OutVisibilityW = nullptr;

	// Active tiles
	FRDGBufferRef ActiveTileCount = nullptr;
	FRDGBufferRef ActiveTile = nullptr;
	FRDGBufferRef ActiveTileIndirectArgs = nullptr;

	bool IsValid() const { return TileCount != nullptr; }
};

static void CreateCurveBinningResources(FRDGBuilder& GraphBuilder, const FIntPoint& ViewResolution, uint32 FrameIndex, bool bIsShadowView, FCurveResources& Out);
static bool UsesCurveTiledRasterization(bool bIsVirtualShadowView);
...
private:
	...
	using FRasterGroupMetaArray = TArray<FNaniteRasterGroupMeta, SceneRenderingAllocator>;

	struct FDispatchContext
	{
		struct FDispatchList
		{
			TArray<int32, SceneRenderingAllocator> Indirections;	// GroupIndices
		};
		...
		TArray<FRasterizerPassGroup, SceneRenderingAllocator> RasterizerPassGroups;

		FNaniteTranslucencyContext TranslucencyContext;

		FRasterBinMetaArray		BinMetaBufferData;
		FRDGBufferRef			BinMetaBuffer = nullptr;
		FRasterGroupMetaArray	GroupMetaBufferData;
		FRDGBufferRef			GroupMetaBuffer = nullptr;

		FCurveResources CurveResources;
		...
		void Reserve(int32 BinCount, int32 GroupCount)
		{
			RasterizerPasses.Reserve(BinCount);
			RasterizerPassGroups.Reserve(GroupCount);

			Dispatches_HW_Triangles.Indirections.Reserve(GroupCount);
			Dispatches_SW_Triangles.Indirections.Reserve(GroupCount);
			Dispatches_SW_Tessellated.Indirections.Reserve(GroupCount);
		}
			...
			if (DispatchList.Indirections.Num() > 0)
			{
				...
				if (HWShaderBundle != nullptr)
				{
					auto RecordDispatches = [&](FRHICommandDispatchGraphicsShaderBundle& Command)
					{
						...
						for (const int32 GroupIndex : DispatchList.Indirections)
						{
							const FRasterizerPassGroup& Group = RasterizerPassGroups[GroupIndex];
							const FRasterizerPass& FirstRasterizerPass = RasterizerPasses[Group.RasterizerPasses[0]];

							Parameters.PassData = FUintVector4(GroupIndex, 0u, 0u, 0u);

							FRHIShaderBundleGraphicsDispatch& Dispatch = Command.Dispatches[GroupIndex];
							Dispatch.RecordIndex = GroupIndex;
							...
							const bool bCullModeNone = FirstRasterizerPass.RasterPipeline.bIsTwoSided || (InRenderFlags & NANITE_RENDER_FLAG_IS_MATERIAL_CACHE);
							GraphicsPSOInit.RasterizerState = GetStaticRasterizerState<false>(FM_Solid, bCullModeNone ? CM_None : CM_CW);

							BindShadersToPSOInit(FirstRasterizerPass);

						#if PSO_PRECACHING_VALIDATE
							if (PSOCollectorStats::IsFullPrecachingValidationEnabled())
							{
								PSOCollectorStats::CheckFullPipelineStateInCache(GraphicsPSOInit, EPSOPrecacheResult::Unknown, FirstRasterizerPass.RasterPipeline.RasterMaterial, &FNaniteVertexFactory::StaticType, nullptr, PSOCollectorIndex);
							}
						#endif

							if (IsMeshShaderRasterPath(HardwarePath))
							{
								SetHWBundleParameters(Dispatch.Parameters_MSVS, ScratchAllocator, FirstRasterizerPass.RasterMeshShader, Parameters, bUsingSharedParameters, ViewInfo, FirstRasterizerPass.VertexMaterialProxy, *FirstRasterizerPass.VertexMaterial, Group, MakeArrayView(RasterizerPasses), FeatureLevel, false, bBindless);
							}
							else
							{
								SetHWBundleParameters(Dispatch.Parameters_MSVS, ScratchAllocator, FirstRasterizerPass.RasterVertexShader, Parameters, bUsingSharedParameters, ViewInfo, FirstRasterizerPass.VertexMaterialProxy, *FirstRasterizerPass.VertexMaterial, Group, MakeArrayView(RasterizerPasses), FeatureLevel, false, bBindless);
							}

							SetHWBundleParameters(Dispatch.Parameters_PS, ScratchAllocator, FirstRasterizerPass.RasterPixelShader, Parameters, bUsingSharedParameters, ViewInfo, FirstRasterizerPass.PixelMaterialProxy, *FirstRasterizerPass.PixelMaterial, Group, MakeArrayView(RasterizerPasses), FeatureLevel, true, bBindless);

							Dispatch.PipelineInitializer = GraphicsPSOInit;

						#if PLATFORM_USE_FALLBACK_PSO
							Dispatch.PipelineState = nullptr; // only PipelineInitializer is needed
						#else // PLATFORM_USE_FALLBACK_PSO
							Dispatch.PipelineState = FirstRasterizerPass.bUseWorkGraphHW ? nullptr : FindGraphicsPipelineState(Dispatch.PipelineInitializer);
							if (Dispatch.PipelineState == nullptr && !(FirstRasterizerPass.bUseWorkGraphHW && GraphicsPSOInit.BoundShaderState.GetWorkGraphShader()))
							{
						...
						#endif // PLATFORM_USE_FALLBACK_PSO
						}
					};
				...
				else
				{
					for (const int32 GroupIndex : DispatchList.Indirections)
					{
					...
					#if WANTS_DRAW_MESH_EVENTS
						SCOPED_CONDITIONAL_DRAW_EVENTF(RHICmdList, HWRaster, bShowDrawEvents != 0, TEXT("%s"), GetRasterMaterialName(FirstRasterizerPass));
					#endif
						...
						auto BindShaderParameters = [HardwarePath, &RHICmdList, &ViewInfo, &Parameters, FeatureLevel, this](const FRasterizerPassGroup& Group, const FRasterizerPass& PassToBind)
						{
							// Vertex/Mesh shader
							{
								const bool bMeshShader = IsMeshShaderRasterPath(HardwarePath);
								FRHIBatchedShaderParameters& BatchedParameters = RHICmdList.GetScratchShaderParameters();

								if (bMeshShader)
								{
									SetBatchedShaderParametersMixed(BatchedParameters, PassToBind.RasterMeshShader, Parameters, ViewInfo, PassToBind.VertexMaterialProxy, *PassToBind.VertexMaterial);
								}
								else
								{
									SetBatchedShaderParametersMixed(BatchedParameters, PassToBind.RasterVertexShader, Parameters, ViewInfo, PassToBind.VertexMaterialProxy, *PassToBind.VertexMaterial);
								}
								...
								{								
									for (const uint32 PassIndex : Group.RasterizerPasses)
									{
										FRHIUniformBuffer* MaterialUB = RasterizerPasses[PassIndex].VertexMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBuffer;
										if (MaterialUB->GetBindlessHandle().IsValid())
										{
											BatchedParameters.SetBindlessShaderUniformBuffer(MaterialUB);
										}
								...
								RHICmdList.SetBatchedShaderParameters(bMeshShader ? PassToBind.RasterMeshShader.GetGraphicsShader() : PassToBind.RasterVertexShader.GetGraphicsShader(), BatchedParameters);
							}

							// Pixel shader
							FRHIBatchedShaderParameters& BatchedParameters = RHICmdList.GetScratchShaderParameters();
							SetBatchedShaderParametersMixed(BatchedParameters, PassToBind.RasterPixelShader, Parameters, ViewInfo, PassToBind.PixelMaterialProxy, *PassToBind.PixelMaterial);

							if (bBindless)
							{
								for (const uint32 PassIndex : Group.RasterizerPasses)
								{
									FRHIUniformBuffer* MaterialUB = RasterizerPasses[PassIndex].PixelMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBuffer;
									if (MaterialUB->GetBindlessHandle().IsValid())
									{
										BatchedParameters.SetBindlessShaderUniformBuffer(MaterialUB);
									}
							...
							RHICmdList.SetBatchedShaderParameters(PassToBind.RasterPixelShader.GetPixelShader(), BatchedParameters);
						};
						...
						BindShaderParameters(Group, FirstRasterizerPass);
			...
			if (DispatchList.Indirections.Num() > 0)
			{
				...
				if (ShaderBundleToUse != nullptr)
				{
					auto RecordDispatches = [&](FRHICommandDispatchComputeShaderBundle& Command)
					{
						...
						for (const int32 GroupIndex : DispatchList.Indirections)
						{
							...
							FRHIShaderBundleComputeDispatch& Dispatch = Command.Dispatches[GroupIndex];
							Dispatch.RecordIndex = GroupIndex;
							Dispatch.Constants = Parameters.PassData;

							const TShaderRef<FMicropolyRasterizeCS>* Shader = bPatches ? &FirstRasterizerPass.PatchComputeShader : &FirstRasterizerPass.ClusterComputeShader;
							...
							SetShaderBundleParameters(
								...
								*Shader,
								...
								ShaderFrequency,
								ViewInfo,
								FirstRasterizerPass.ComputeMaterialProxy,
								*FirstRasterizerPass.ComputeMaterial
							);
							...
							if (bBindless)
							{
								for (const uint32 PassIndex : Group.RasterizerPasses)
								{
									FRHIUniformBuffer* MaterialUB = RasterizerPasses[PassIndex].ComputeMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBuffer;
									if (MaterialUB->GetBindlessHandle().IsValid())
									{
										Dispatch.Parameters->SetBindlessShaderUniformBuffer(MaterialUB);
									}
								}
							}
							...
							if (Dispatch.Shader != nullptr)
							{
								...
								PSOCollectorStats::CheckComputePipelineStateInCache(*Dispatch.Shader, PSOPrecacheResult, FirstRasterizerPass.ComputeMaterialProxy, PSOCollectorIndex);
							}
						#endif

						#if PLATFORM_USE_FALLBACK_PSO
							Dispatch.PipelineState = nullptr; // only Shader is needed
						#else // PLATFORM_USE_FALLBACK_PSO
							...
							if (Dispatch.Shader != nullptr && Dispatch.PipelineState == nullptr)
							{
								...
								// * If tiled-curve resources exists, skip drawing, as actual drawing is done later in separate passes
								const bool bSkipDraw = !PipelineStateCache::IsPSOPrecachingEnabled() || (FirstRasterizerPass.RasterPipeline.bCurve && CurveResources.IsValid());
				...
				else
				{
					for (const int32 GroupIndex : DispatchList.Indirections)
					{
						...
						// * If tiled-curve resources exists, skip drawing, as actual drawing is done later in separate passes
						const bool bSkipDraw = (FirstRasterizerPass.RasterPipeline.bCurve && CurveResources.IsValid());
						if (bSkipDraw)
						{
							continue;
						}
					...
					#if WANTS_DRAW_MESH_EVENTS
						SCOPED_CONDITIONAL_DRAW_EVENTF(RHICmdList, SWRaster, bShowDrawEvents, TEXT("%s"), GetRasterMaterialName(FirstRasterizerPass));
					#endif

						Parameters.PassData = FUintVector4(GroupIndex, 0u, 0u, 0u);

						const TShaderRef<FMicropolyRasterizeCS>* ComputeShader = bPatches ? &FirstRasterizerPass.PatchComputeShader : &FirstRasterizerPass.ClusterComputeShader;
						...
						FComputeShaderUtils::ValidateIndirectArgsBuffer(IndirectArgsBuffer->GetSize(), Group.IndirectOffset);

						SetComputePipelineState(RHICmdList, ShaderRHI);

					#if PSO_PRECACHING_VALIDATE
						EPSOPrecacheResult PSOPrecacheResult = PipelineStateCache::CheckPipelineStateInCache(ShaderRHI);
						PSOCollectorStats::CheckComputePipelineStateInCache(*ShaderRHI, PSOPrecacheResult, FirstRasterizerPass.ComputeMaterialProxy, PSOCollectorIndex);
					#endif
						...
						FRHIBatchedShaderParameters& BatchedParameters = RHICmdList.GetScratchShaderParameters();
						SetBatchedShaderParametersMixed(BatchedParameters, *ComputeShader, Parameters, ViewInfo, FirstRasterizerPass.ComputeMaterialProxy, *FirstRasterizerPass.ComputeMaterial);
						...
						RHICmdList.DispatchIndirectComputeShader(IndirectArgsBuffer, Group.IndirectOffset);
...
private:
	...
	FIntRect		ViewRectUnion; // Area == 0 is interpreted as Unknown
	...
	FRDGBufferRef	RasterBinDataBuffer			= nullptr;
	FRDGBufferRef	RasterBinArgsBuffer			= nullptr;
	FRDGBufferRef	RasterGroupMetaBuffer		= nullptr;
	...
	void 			AddPass_CurveTiledRasterize(
		const FDispatchContext& DispatchContext,
	...
	void SetCurveClusterParameters(
		const FBinningData& BinningData,
		FRDGBufferRef ClusterOffsetSWHW,
		FRDGBufferRef IndirectArgs,
	...
	, HZBBuildViewRect( InViewRect )
{
	...
	// TODO: Exclude from shipping builds
	{
		...
		if (CVarNaniteCullingSkinnedNodeBounds.GetValueOnRenderThread() == 0)
		{
			DebugFlags |= NANITE_DEBUG_FLAG_DISABLE_SKINNED_NODE_BOUNDS;
		}
...
void FRenderer::AddPass_PrimitiveFilter()
{
	...
	if (CVarNaniteFilterPrimitives.GetValueOnRenderThread() != 0 && (bAnyPrimitiveFilter || bAnyFilterFlags))
	{
		...
		if (ShowOnlyPrimitiveCount > 0)
		{
			...
			ShowOnlyPrimitiveIds.Reserve(ShowOnlyPrimitiveCount);
		...
		if (HiddenPrimitiveCount > 0)
		{
			...
			HiddenPrimitiveIds.Reserve(HiddenPrimitiveCount);
		...
		PassParameters->NumHiddenPrimitives = HiddenPrimitiveCount;
		PassParameters->NumShowOnlyPrimitives = ShowOnlyPrimitiveCount;
...
static uint32 GetMaxClusterIndirections()
{
	...
	return MaxClusterIndirections;
}

static uint32 GetRasterGroupIndirectOffset(uint32 InGroupIndex)
{
	// Note: The indirect args offset is in bytes
	return InGroupIndex * NANITE_RASTERIZER_ARG_COUNT * 4u;
}
...
)
{
	...
	BinningData.BinCount = DispatchContext.BinMetaBufferData.Num();
	BinningData.GroupCount = DispatchContext.BinMetaBufferData.Num();	// TODO: Can we come up with a better bound?

	ENaniteMeshPass::Type MeshPass = GetMeshPass(Configuration);

	if (BinningData.BinCount > 0)
	{
		BinningData.BinMetaBuffer = GraphBuilder.CreateBuffer(DispatchContext.BinMetaBuffer->Desc, DispatchContext.BinMetaBuffer->Name);
		AddCopyBufferPass(GraphBuilder, BinningData.BinMetaBuffer, DispatchContext.BinMetaBuffer);

		BinningData.GroupMetaBuffer = GraphBuilder.CreateBuffer(DispatchContext.GroupMetaBuffer->Desc, DispatchContext.GroupMetaBuffer->Name);
		AddCopyBufferPass(GraphBuilder, BinningData.GroupMetaBuffer, DispatchContext.GroupMetaBuffer);

		// Initialize Group Ranges
		{
			FRasterGroupInit_CS::FParameters* InitPassParameters = GraphBuilder.AllocParameters<FRasterGroupInit_CS::FParameters>();
			InitPassParameters->OutRasterGroupMeta = GraphBuilder.CreateUAV(BinningData.GroupMetaBuffer);
			InitPassParameters->RasterGroupCount = BinningData.GroupCount;

			auto ComputeShader = SharedContext.ShaderMap->GetShader<FRasterGroupInit_CS>();
			ClearUnusedGraphResources(ComputeShader, InitPassParameters);

			GraphBuilder.AddPass(
				RDG_EVENT_NAME("RasterGroupInit"),
				InitPassParameters,
				PassFlags,
				[InitPassParameters, &DispatchContext, VisiblePatches, ComputeShader, GroupCount = BinningData.GroupCount](FRDGAsyncTask, FRHIComputeCommandList& RHICmdList)
				{
					FComputeShaderUtils::Dispatch(
						RHICmdList,
						ComputeShader,
						*InitPassParameters,
						FComputeShaderUtils::GetGroupCountWrapped(GroupCount, 64)
					);
				}
			);
		}

		BinningData.IndirectArgs = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateIndirectDesc(BinningData.GroupCount * NANITE_RASTERIZER_ARG_COUNT), TEXT("Nanite.RasterGroupIndirectArgs"));

		const uint32 MaxClusterIndirections = GetMaxClusterIndirections();
		...
		const bool bDepthBucketing = Nanite::FGlobalResources::UseExtendedClusterSize() &&
			...
			(NaniteVoxelsSupported() || NaniteCurvesSupported() || CVarNaniteDepthBucketPixelProgrammable.GetValueOnRenderThread() != 0);
		...
		CommonBinBuildParameters.OutRasterBinMeta			= GraphBuilder.CreateUAV(BinningData.BinMetaBuffer);
		CommonBinBuildParameters.OutRasterGroupMeta			= GraphBuilder.CreateUAV(BinningData.GroupMetaBuffer);
		...
		if (bDepthBucketing)
		{
			...
			DepthBucketsBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(uint32), BinningData.GroupCount * 2 * NANITE_NUM_DEPTH_BUCKETS_PER_BLOCK), TEXT("Nanite.DepthBuckets"));
		...
		CommonBinBuildParameters.MaxVisibleClusters = Nanite::FGlobalResources::GetMaxVisibleClusters();
		...
		// Reserve Group Ranges
		{
			...
			FRasterGroupReserve_CS::FParameters* ReservePassParameters = GraphBuilder.AllocParameters<FRasterGroupReserve_CS::FParameters>();
			ReservePassParameters->OutRasterGroupArgsSWHW = GraphBuilder.CreateUAV(BinningData.IndirectArgs);
			ReservePassParameters->OutRasterGroupMeta = GraphBuilder.CreateUAV(BinningData.GroupMetaBuffer);
			ReservePassParameters->OutRangeAllocator = GraphBuilder.CreateUAV(RangeAllocatorBuffer);
			ReservePassParameters->RasterGroupCount = BinningData.GroupCount;
			ReservePassParameters->RenderFlags = RenderFlags;

			auto ComputeShader = SharedContext.ShaderMap->GetShader<FRasterGroupReserve_CS>();
			ClearUnusedGraphResources(ComputeShader, ReservePassParameters);

			GraphBuilder.AddPass(
				RDG_EVENT_NAME("RasterGroupReserve"),
				ReservePassParameters,
				PassFlags,
				[ReservePassParameters, &DispatchContext, VisiblePatches, ComputeShader, GroupCount = BinningData.GroupCount](FRDGAsyncTask, FRHIComputeCommandList& RHICmdList)
				{
					if (VisiblePatches == nullptr || DispatchContext.HasTessellated())
					{
						FComputeShaderUtils::Dispatch(
		...
		if(bDepthBucketing)
		{
			FRasterGroupDepthBlock_CS::FParameters* DepthBlockPassParameters = GraphBuilder.AllocParameters<FRasterGroupDepthBlock_CS::FParameters>();
			DepthBlockPassParameters->OutDepthBuckets = GraphBuilder.CreateUAV(DepthBucketsBuffer);

			auto ComputeShader = SharedContext.ShaderMap->GetShader<FRasterGroupDepthBlock_CS>();
			ClearUnusedGraphResources(ComputeShader, DepthBlockPassParameters);

			GraphBuilder.AddPass(
				RDG_EVENT_NAME("RasterGroupDepthBlock"),
		...
		// Scatter SW & HW Clusters
		{
			...
			PassParameters->OutRasterGroupArgsSWHW = GraphBuilder.CreateUAV(BinningData.IndirectArgs);
			PassParameters->OutRasterBinMeta = GraphBuilder.CreateUAV(BinningData.BinMetaBuffer);
			PassParameters->OutRasterGroupMeta = GraphBuilder.CreateUAV(BinningData.GroupMetaBuffer);
		...
		if(VisiblePatches == nullptr)
		{
			...
			FinalizePassParameters->OutRasterGroupArgsSWHW = GraphBuilder.CreateUAV(BinningData.IndirectArgs);
			FinalizePassParameters->OutRasterGroupMeta = GraphBuilder.CreateUAV(BinningData.GroupMetaBuffer);
			FinalizePassParameters->RasterGroupCount = BinningData.GroupCount;
			...
			GraphBuilder.AddPass(
				...
				PassFlags,
				[FinalizePassParameters, &DispatchContext, VisiblePatches, ComputeShader, GroupCount = BinningData.GroupCount](FRDGAsyncTask, FRHIComputeCommandList& RHICmdList)
				{
					if (VisiblePatches == nullptr || DispatchContext.HasTessellated())
					{
						FComputeShaderUtils::Dispatch(
...
{
	const EShaderPlatform ShaderPlatform = GetFeatureLevelShaderPlatform(FeatureLevel);
	const bool bBindless = UseNaniteBindlessRasterization(ShaderPlatform);
	...
	const uint32 RasterGroupCount = RasterPipelines.GetBinCount();	//TODO: Can we come up with a tighter bound?

	static const FNaniteRasterBinMeta DefaultBinMeta = { .GroupIndex = NANITE_INVALID_RASTER_GROUP };
	Context.BinMetaBufferData.Init(DefaultBinMeta, RasterBinCount);
	Context.GroupMetaBufferData.SetNumZeroed(RasterGroupCount);
	
	Context.BundleMethod = ERHIShaderBundleMethod::Emulation;
	
	if ((!!GRHISupportsShaderBundleDispatch || CanUseShaderBundleWorkGraph(ShaderPlatform)) && !bBundleEmulation)
	{
		Context.BundleMethod = ERHIShaderBundleMethod::Native;
	}
	...
	if (RasterGroupCount > 0)
	{
		...
		*/
		const uint32 NumRecords = RasterGroupCount;
	...
	]
	{
		...
		const auto FillFixedMaterialShaders = [&](FRasterizerPass& RasterizerPass)
		{
			...
			const bool bFixedDisplacementFallback = RasterizerPass.RasterPipeline.bIsDisplacementFallback;
			const bool bIsTwoSided = RasterizerPass.RasterPipeline.bIsTwoSided || bIsMaterialCache;
			...
			PermutationVectorCS_Cluster.Set<FMicropolyRasterizeCS::FCurvesDim>(RasterizerPass.RasterPipeline.bCurve); 
		...
		{
			FNaniteRasterBinMeta& BinMeta = Context.BinMetaBufferData[RasterizerPass.RasterBin];
			...
			RasterizerPass.bTranslucent = MaterialBitFlags & NANITE_MATERIAL_FLAG_TRANSLUCENT;
			...
			else if (RasterizerPass.bVertexProgrammable || RasterizerPass.bPixelProgrammable)
			{
				...
				GetMaterialShaderTypes(
					...
					RasterizerPass.RasterPipeline.bCurve,
			...
			if(bBindless)
			{
				FRHIUniformBuffer* MaterialUB_VS = RasterizerPass.VertexMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBuffer;
				FRHIUniformBuffer* MaterialUB_CS = RasterizerPass.ComputeMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBuffer;
				FRHIUniformBuffer* MaterialUB_PS = RasterizerPass.PixelMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBuffer;

				FRHIDescriptorHandle HandleVS = MaterialUB_VS->GetBindlessHandle();
				FRHIDescriptorHandle HandleCS = MaterialUB_CS->GetBindlessHandle();
				FRHIDescriptorHandle HandlePS = MaterialUB_PS->GetBindlessHandle();
				check(HandleVS.IsValid());
				check(HandleCS.IsValid());
				check(HandlePS.IsValid());

				BinMeta.MaterialCBIndexVS			= HandleVS.GetIndex();
				BinMeta.MaterialCBIndexCS			= HandleCS.GetIndex();
				BinMeta.MaterialCBIndexPS			= HandlePS.GetIndex();

				BinMeta.MaterialCBIndexVS_Compact = MAX_uint32;
				FRHIUniformBuffer* MaterialUB_VS_Compact = RasterizerPass.VertexMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBufferVS;
				if (MaterialUB_VS_Compact)
				{
					FRHIDescriptorHandle HandleVS_Compact = MaterialUB_VS_Compact->GetBindlessHandle();
					if (HandleVS_Compact.IsValid())
					{
						BinMeta.MaterialCBIndexVS_Compact = HandleVS_Compact.GetIndex();
					}
				}

				BinMeta.MaterialCBIndexCS_Compact = MAX_uint32;
				FRHIUniformBuffer* MaterialUB_CS_Compact = RasterizerPass.ComputeMaterialProxy->UniformExpressionCache[FeatureLevel].UniformBufferVS;
				if (MaterialUB_CS_Compact)
				{
					FRHIDescriptorHandle HandleCS_Compact = MaterialUB_CS_Compact->GetBindlessHandle();
					if (HandleCS_Compact.IsValid())
					{
						BinMeta.MaterialCBIndexCS_Compact = HandleCS_Compact.GetIndex();
					}
				}
			}
			...
			BinMeta.MaterialFlags = MaterialBitFlags;
		};
		...
		Context.Reserve(RasterPipelines.GetBinCount(), RasterPipelines.GetBinCount());

		TMap<FRasterizerPassGroupKey, uint32> PassGroupKeyToIndex;	// TODO: Use better hashmap?
		PassGroupKeyToIndex.Reserve(Pipelines.Num());
		...
		for (const auto& RasterBin : Pipelines)
		{
			...
			// Any bins within the fixed function bin mask are special cased
			const bool bFixedFunctionBin = (RasterEntry.BinIndex & FGlobalResources::GetFixedFunctionBinMask()) == RasterEntry.BinIndex;
			if (bFixedFunctionBin)
			{
				...
				{
					if ((RasterEntry.BinIndex & (NANITE_FIXED_FUNCTION_BIN_TWOSIDED | NANITE_FIXED_FUNCTION_BIN_SPLINE | NANITE_FIXED_FUNCTION_BIN_CURVE)) != 0u)
					{
						continue;
					}
				}

				if ((RasterEntry.BinIndex & NANITE_FIXED_FUNCTION_BIN_CURVE) != 0u)
				{
			...
			const uint32 RasterizerPassIndex = Context.RasterizerPasses.Num();
			...
			if (bUseSetupCache)
			{
				...
				RasterMaterialCacheKey.bIsTranslucent = RasterizerPass.RasterPipeline.bTranslucent;
				...
				RasterMaterialCacheKey.bCurve = RasterEntry.RasterPipeline.bCurve;
				...
				RasterMaterialCacheKey.bIsDisplacementFallback = RasterEntry.RasterPipeline.bIsDisplacementFallback;
			...
			else if (bDisableProgrammable || RasterizerPass.bTranslucent)
			{
				...
				// Translucency will also just be skipped using the same mechanism.
				RasterizerPass.bHidden = true;
			}

			const bool bBuildGroup = (!RasterizerPass.bHidden) || RasterizerPass.bTranslucent;
			...
			{
				FNaniteRasterBinMeta& BinMeta = Context.BinMetaBufferData[RasterizerPass.RasterBin];
				BinMeta.MaterialFlags |= NANITE_MATERIAL_FLAG_USED;

				{
					// Groups
					uint32* GroupIndexPtr = nullptr;
					
					FRasterizerPassGroupKey Key;
					if (bBindless && !RasterizerPass.bTranslucent) // GW-TODO: Allow merging by resolving translucent shaders?
					{
						Key = FRasterizerPassGroupKey(RasterizerPass, BinMeta.MaterialFlags, Scene, FeatureLevel);
						GroupIndexPtr = PassGroupKeyToIndex.Find(Key);
					}
					...
					{
						// New Group
						PassGroupKeyToIndex.Add(Key, Context.RasterizerPassGroups.Num());
						GroupIndex = Context.RasterizerPassGroups.Num();

						Context.RasterizerPassGroups.AddDefaulted();

						FNaniteRasterGroupMeta& GroupMeta = Context.GroupMetaBufferData[GroupIndex];

						{
							const uint32 NumDepthBlocks =	(BinMeta.MaterialFlags & NANITE_MATERIAL_FLAG_VOXEL) ? 1 :
															(bDepthBucketPixelProgrammable && (BinMeta.MaterialFlags & NANITE_MATERIAL_PIXEL_PROGRAMMABLE_FLAGS)) ? 2 : 0;
							...
							check(BinMeta.MaterialFlags <= 0xFFFFu);
							GroupMeta.MaterialFlags_DepthBlock = BinMeta.MaterialFlags | (DepthBlockIndex << 16);
						}

						// Build dispatch list indirections
						if (RasterizerPass.bTranslucent)
						{
							// Intentionally skip indirections if translucent
						}
						else if (RasterizerPass.bDisplacement)
						{
							// Displaced meshes never run the HW path
							Context.Dispatches_SW_Tessellated.Indirections.Emplace(GroupIndex);
						}
						else
						{
							Context.Dispatches_SW_Triangles.Indirections.Emplace(GroupIndex);
							Context.Dispatches_HW_Triangles.Indirections.Emplace(GroupIndex);
						}
					}

					FRasterizerPassGroup& Group = Context.RasterizerPassGroups[GroupIndex];
					Group.RasterizerPasses.Add(RasterizerPassIndex);
					Group.IndirectOffset = GetRasterGroupIndirectOffset(GroupIndex);
					
					RasterizerPass.RasterGroup = GroupIndex;
					BinMeta.GroupIndex = GroupIndex;

					// Register the group index of the BinCurve
					if (bFixedFunctionBin && (RasterizerPass.RasterBin == NANITE_FIXED_FUNCTION_BIN_CURVE || RasterizerPass.RasterBin == (NANITE_FIXED_FUNCTION_BIN_CURVE | NANITE_FIXED_FUNCTION_BIN_CAST_SHADOW)))
					{
						Context.CurveResources.GroupIndex = GroupIndex;
					}
				}
			}

			if (RasterizerPass.bTranslucent && RasterizerPass.RasterPipeline.TranslucencyPassData.IsValid())
			{
				FNaniteTranslucencyBin& TranslucencyBin = Context.TranslucencyContext.TranslucencyBins.Emplace_GetRef();
				TranslucencyBin.RasterBin = RasterizerPass.RasterBin;
				TranslucencyBin.RasterGroup = RasterizerPass.RasterGroup;
				TranslucencyBin.RasterPipeline = RasterizerPass.RasterPipeline;

				// Note: The indirect args offset is in bytes
				TranslucencyBin.IndirectOffset = (TranslucencyBin.RasterGroup * NANITE_RASTERIZER_ARG_COUNT) * 4u;
			}
		}
		...
		if (CVarNaniteRasterSort.GetValueOnRenderThread())
		{
			auto SortIndirections = [&](FDispatchContext::FDispatchList& List)
			{
				...
				for (uint32 GroupIndex : List.Indirections)
				{
					FRasterizerPassGroup& Group = Context.RasterizerPassGroups[GroupIndex];
					const FRasterizerPass& FirstRasterizerPass = Context.RasterizerPasses[Group.RasterizerPasses[0]];
					SortList.Emplace(FirstRasterizerPass.CalcSortKey(), GroupIndex);
				}
			...
			// GW-TODO: Sort translucency bins by group and then rasterizer pass sort key
		}
	...
	if (RasterBinCount > 0)
	{
		Context.BinMetaBuffer = CreateStructuredBuffer(
			GraphBuilder,
			TEXT("Nanite.RasterBinMeta"),
			sizeof(FNaniteRasterBinMeta),
			FMath::RoundUpToPowerOfTwo(FMath::Max(RasterBinCount, 1u)),
			Context.BinMetaBufferData.GetData(),
			sizeof(FNaniteRasterBinMeta) * RasterBinCount,
			// The buffer data is allocated on the RDG timeline and gets filled by an RDG setup task.
	...
	if (RasterGroupCount > 0)
	{
		Context.GroupMetaBuffer = CreateStructuredBuffer(
			GraphBuilder,
			TEXT("Nanite.RasterGroupMeta"),
			sizeof(FNaniteRasterGroupMeta),
			FMath::RoundUpToPowerOfTwo(FMath::Max(RasterGroupCount, 1u)),
			Context.GroupMetaBufferData.GetData(),
			sizeof(FNaniteRasterGroupMeta) * RasterGroupCount,
			// The buffer data is allocated on the RDG timeline and gets filled by an RDG setup task.
			ERDGInitialDataFlags::NoCopy
		);
	}

	// Create raster data for curve rendering
	if (NaniteCurvesSupported() && UsesCurveTiledRasterization(IsUsingVirtualShadowMap()))
	{
		const uint32 FrameIndex = SceneView.ViewState ? SceneView.ViewState->FrameIndex : 0u;
		CreateCurveBinningResources(GraphBuilder, RasterContext.TextureSize, FrameIndex, bHasVirtualShadowMap, Context.CurveResources);
	}
}
	...
	bool bMainPass)
{
	...
	LLM_SCOPE_BYTAG(Nanite);
	RDG_EVENT_SCOPE(GraphBuilder, "Rasterize");
	...
	FRDGBufferRef DummyBufferRasterBinMeta = GSystemTextures.GetDefaultStructuredBuffer<FNaniteRasterBinMeta>(GraphBuilder);
	...
	FIntRect ScissorRect = ViewRect;
	if (ViewRectUnion.Area() > 0 && !IsUsingVirtualShadowMap())
	{
		ScissorRect = ViewRectUnion;
	}
	...
	const auto CreatePassParameters = [&](const FBinningData& BinningData, bool bPatches)
	{
		...
		RasterPassParameters->RasterBinMeta				= GraphBuilder.CreateSRV(BinningData.BinMetaBuffer);
		RasterPassParameters->RasterGroupMeta			= GraphBuilder.CreateSRV(BinningData.GroupMetaBuffer);
	...
	if (ClusterBinning.BinMetaBuffer == nullptr)
	{
		ClusterBinning.BinMetaBuffer = DummyBufferRasterBinMeta;
	}
	...
	if (bTessellationEnabled)
	{
		...
		FRDGPass* SWTessellatedPass = GraphBuilder.AddPass(
			...
			ERDGPassFlags::Compute,
			[ClusterPassParameters, &DispatchContext, &SceneView = SceneView, RenderFlags = RenderFlags, PSOCollectorIndex, FeatureLevel](FRDGAsyncTask, FRHIComputeCommandList& RHICmdList)
			{
	...
	FRDGPass* HWTrianglesPass = GraphBuilder.AddPass(
		...
		[ClusterPassParameters, &DispatchContext, ViewRect, &SceneView = SceneView, bMainPass, HardwarePath, PSOCollectorIndex, RenderFlags = RenderFlags, FeatureLevel, ScissorRect = ScissorRect](FRDGAsyncTask, FRHICommandList& RHICmdList)
		{
	...
	// Curve's tiled rasterizer needs to run before the tesselation passes as the tesselation AddPass_Binning() will reset the RasterGroupMeta, 
	// and culling data will be become invalid
	if (DispatchContext.CurveResources.IsValid() && bMainPass)
	{
		AddPass_CurveTiledRasterize(
			DispatchContext,
			ClusterBinning,
			bMainPass ? MainRasterizeArgsSWHW : PostRasterizeArgsSWHW,
			ClusterBinning.IndirectArgs,
			ERDGPassFlags::Compute);
	}
	...
	if (bTessellationEnabled)
	{
		...
		if (PatchBinning.BinMetaBuffer == nullptr)
		{
			PatchBinning.BinMetaBuffer = DummyBufferRasterBinMeta;
		}
		...
		FRDGPass* SWPatchesPass = GraphBuilder.AddPass(
			...
			PatchPassParameters,
			PatchPassFlags,
			[PatchPassParameters, &DispatchContext, &SceneView = SceneView, RenderFlags = RenderFlags, PSOCollectorIndex, FeatureLevel](FRDGAsyncTask, FRHIComputeCommandList& RHICmdList)
			{
				DispatchContext.DispatchSW(
					RHICmdList,
					DispatchContext.Dispatches_SW_Tessellated,
	...
	FRDGTextureRef ExternalDepthBuffer)
{
	...
	const int32 FastClearMode = CVarNaniteFastVisBufferClear.GetValueOnRenderThread();
	const bool bUseFastClear = FastClearMode != 0 && (RectMinMaxBufferSRV == nullptr && NumRects == 0 && ExternalDepthBuffer == nullptr);

	FRDGTextureRef MetadataClearTarget = (RasterContext.RasterMode == EOutputBufferMode::DepthOnly) ? RasterContext.DepthBuffer : RasterContext.VisBuffer64;
	const bool bUseMetadataClear =	bUseFastClear && FastClearMode == 3 && 
									EnumHasAnyFlags(MetadataClearTarget->Desc.Flags, TexCreate_RenderTargetable) &&
									MetadataClearTarget->Desc.ClearValue.ColorBinding == EClearBinding::EColorBound &&
									MetadataClearTarget->Desc.ClearValue.GetClearColor() == NaniteRasterClearColor;

	if (bUseMetadataClear)
	{
		AddClearRenderTargetPass(GraphBuilder, MetadataClearTarget, NaniteRasterClearColor);

		if (RasterContext.RasterMode != EOutputBufferMode::DepthOnly && RasterContext.VisualizeActive)
		{
	...
	else if (bUseFastClear)
	{
		...
		const bool bTiled = (FastClearMode == 2);
...
)
{
	...
	check( InitParams.ExternalDepthBuffer == nullptr || InitParams.ExternalDepthBuffer->Desc.Extent == InitParams.TextureSize );
	checkSlow(DoesPlatformSupportNanite(GMaxRHIShaderPlatform));
	...
	RasterContext.bCustomPass = InitParams.bCustomPass;
	RasterContext.VisualizeActive = InitParams.bVisualize;
	RasterContext.VisualizeModeOverdraw = InitParams.bVisualize && InitParams.bVisualizeOverdraw;
	RasterContext.bEnableAssemblyMeta = InitParams.bEnableAssemblyMeta;
	RasterContext.bAllowTessellation = InitParams.bAllowTessellation;
	RasterContext.TextureSize = InitParams.TextureSize;

	// Set rasterizer scheduling based on config and platform capabilities.
	bool bAsyncCompute = InitParams.bAsyncCompute;
	if (CVarNaniteComputeRasterization.GetValueOnRenderThread() != 0)
	{
		bAsyncCompute = bAsyncCompute
			...
			&& !(InitParams.bCustomPass && !UseAsyncComputeForCustomPass(ViewFamily));
	...
	RasterContext.RasterMode = InitParams.RasterMode;

	const bool bRenderTargetableClear = CVarNaniteFastVisBufferClear.GetValueOnRenderThread() == 3;
	const EPixelFormat PixelFormat64 = (bRenderTargetableClear || !GPixelFormats[PF_R64_UINT].Supported) ? PF_R32G32_UINT : PF_R64_UINT;

	const ETextureCreateFlags DepthBufferFlags = TexCreate_ShaderResource | TexCreate_UAV | TexCreate_AtomicCompatible | (bRenderTargetableClear ? TexCreate_RenderTargetable : TexCreate_None);
	const FClearValueBinding DepthBufferClearBinding = bRenderTargetableClear ? FClearValueBinding(NaniteRasterClearColor) : FClearValueBinding::None;

	RasterContext.DepthBuffer	= InitParams.ExternalDepthBuffer ? InitParams.ExternalDepthBuffer :
								  GraphBuilder.CreateTexture( FRDGTextureDesc::Create2D(RasterContext.TextureSize, PF_R32_UINT, DepthBufferClearBinding, DepthBufferFlags), TEXT("Nanite.DepthBuffer32") );
	
	const ETextureCreateFlags VisBuffer64Flags = TexCreate_ShaderResource | TexCreate_UAV | ETextureCreateFlags::Atomic64Compatible | (bRenderTargetableClear ? TexCreate_RenderTargetable : TexCreate_None);
	const FClearValueBinding VisBuffer64ClearBinding = bRenderTargetableClear ? FClearValueBinding(NaniteRasterClearColor) : FClearValueBinding::None;
	FRDGTextureDesc NaniteVisBuffer64Desc = FRDGTextureDesc::Create2D(RasterContext.TextureSize, PixelFormat64, VisBuffer64ClearBinding, VisBuffer64Flags);
	...
	if (RasterContext.RasterMode == EOutputBufferMode::DepthOnly)
	{
		...
		if (RasterContext.DepthBuffer->Desc.Dimension == ETextureDimension::Texture2DArray)
		{
			...
			check(!InitParams.bClearTarget); // Clearing is not required; this path is only used with VSMs.
		}
		else
		{
	...
	AddClearVisBufferPass(
		...
		PixelFormat64,
		RasterContext,
		InitParams.TextureRect,
		InitParams.bClearTarget,
		InitParams.RectMinMaxBufferSRV,
		InitParams.NumRects,
		InitParams.ExternalDepthBuffer
	);
	...
	const TConstArrayView<FInstanceDraw>* OptionalInstanceDraws)
{
	...
	{
		UE_LOGF(LogRenderer, Warning, "Nanite view overflow detected: %d / %d.", ViewArray.NumViews, NANITE_MAX_VIEWS_PER_CULL_RASTERIZE_PASS);
	}

	ViewRectUnion = FIntRect(MAX_int32, MAX_int32, MIN_int32, MIN_int32);
	for (uint32 ViewIdx = 0; ViewIdx < ViewArray.NumViews; ++ViewIdx)
	{
		const FIntVector4& VR = ViewArray.GetViews()[ViewIdx].ViewRect;
		ViewRectUnion.Union(FIntRect(VR.X, VR.Y, VR.Z, VR.W));
	}
...
)
{
	...
	const bool bTessellationEnabled = RasterContext.bAllowTessellation &&
		UseNaniteTessellation() &&
		(RenderFlags & NANITE_RENDER_FLAG_DISABLE_PROGRAMMABLE) == 0u;
	...
	if (DispatchContext.CurveResources.IsValid() && !VirtualShadowMapArray && IsNaniteCurveDebugEnabled())
	{
		AddPass_CurveDebug(DispatchContext);
	}
	...
	// Clear ViewRectUnion for next call to DrawGeometry
	ViewRectUnion = FIntRect();

	// GW-TODO: Cleanup
	//RasterBinMetaBuffer = DispatchContext.BinMetaBuffer;
	RasterBinMetaBuffer = MainPassBinning.BinMetaBuffer;
	RasterBinDataBuffer = /*PostPassBinning.DataBuffer ? PostPassBinning.DataBuffer :*/ MainPassBinning.DataBuffer;
	RasterBinArgsBuffer = /*PostPassBinning.IndirectArgs ? PostPassBinning.IndirectArgs :*/ MainPassBinning.IndirectArgs;
	RasterGroupMetaBuffer = MainPassBinning.GroupMetaBuffer;

	TranslucencyContext = DispatchContext.TranslucencyContext;
...
void FRenderer::ExtractResults( FRasterResults& RasterResults )
{
	...
	RasterResults.RasterBinData			= RasterBinDataBuffer;
	RasterResults.RasterBinArgs			= RasterBinArgsBuffer;
	RasterResults.RasterGroupMeta		= RasterGroupMetaBuffer;

	RasterResults.TranslucencyContext	= TranslucencyContext;
...
class FCalculateRasterStatsCS : public FNaniteGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters,)
		...
		SHADER_PARAMETER(uint32, NumMainPassRasterGroups)
		SHADER_PARAMETER(uint32, NumPostPassRasterGroups)
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FNaniteRasterGroupMeta>, MainPassRasterGroupMeta)
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FNaniteRasterGroupMeta>, PostPassRasterGroupMeta)
	END_SHADER_PARAMETER_STRUCT()
};
	...
	{
		{
			...
			PassParameters->NumMainPassRasterGroups		= MainPassBinning.GroupCount;
			PassParameters->MainPassRasterBinMeta		= GraphBuilder.CreateSRV(MainPassBinning.BinMetaBuffer);
			PassParameters->MainPassRasterGroupMeta		= GraphBuilder.CreateSRV(MainPassBinning.GroupMetaBuffer);

			if (Configuration.bTwoPassOcclusion)
			{
				check(PostPassBinning.BinMetaBuffer);
				check(PostPassBinning.BinCount == PostPassBinning.BinCount);
				check(PostPassBinning.GroupCount == PostPassBinning.GroupCount);

				PassParameters->NumPostPassRasterGroups = PostPassBinning.GroupCount;
				PassParameters->PostPassRasterGroupMeta = GraphBuilder.CreateSRV(PostPassBinning.GroupMetaBuffer);
			}
			else
			{
				PassParameters->NumPostPassRasterGroups = 0;
			}
...
{
	// Double buffer because the post pass buffer is used as output to in the main pass instance cull (and then in the post pass hierarchy cull) so both must exist at the same time
