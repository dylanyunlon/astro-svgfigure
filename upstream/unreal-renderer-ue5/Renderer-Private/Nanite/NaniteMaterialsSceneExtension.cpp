...
FMaterialsSceneExtension::~FMaterialsSceneExtension()
{
	// Mark all pending requests as invalid and don't process those request anymore
	for (auto Iter : PrimitivesWithPendingRequestStaticMeshUpdate)
	{
		*(Iter.Value.Get()) = false;
	}
}
...
TSharedPtr<bool> FMaterialsSceneExtension::AddPendingRequestStaticMeshUpdate(const FPrimitiveSceneInfo* PrimitiveSceneInfo)
{
	FScopeLock ScopeLock(&PrimitivesWithPendingRequestStaticMeshUpdateCS);

	// Create shared bool which can be used to check if the primitive is still valid when pending task is executed
	TSharedPtr<bool> RequestValid = MakeShared<bool>(true);
	check(!PrimitivesWithPendingRequestStaticMeshUpdate.Contains(PrimitiveSceneInfo));
	PrimitivesWithPendingRequestStaticMeshUpdate.Add(PrimitiveSceneInfo, RequestValid);
	return RequestValid;
}

void FMaterialsSceneExtension::ClearPendingRequestStaticMeshUpdate(const FPrimitiveSceneInfo* PrimitiveSceneInfo)
{
	FScopeLock ScopeLock(&PrimitivesWithPendingRequestStaticMeshUpdateCS);

	TSharedPtr<bool>* RequestValid = PrimitivesWithPendingRequestStaticMeshUpdate.Find(PrimitiveSceneInfo);
	if (RequestValid)
	{
		// Mark request invalid 
		*(RequestValid->Get()) = false;
		PrimitivesWithPendingRequestStaticMeshUpdate.Remove(PrimitiveSceneInfo);
	}	
}

bool FMaterialsSceneExtension::HasPendingRequestStaticMeshUpdate(const FPrimitiveSceneInfo* PrimitiveSceneInfo)
{
	FScopeLock ScopeLock(&PrimitivesWithPendingRequestStaticMeshUpdateCS);
	return PrimitivesWithPendingRequestStaticMeshUpdate.Find(PrimitiveSceneInfo) != nullptr;
}
	...
	TaskHandles[UpdateDebugViewModeTask] = GraphBuilder.AddSetupTask(
		[this, BuildDependency]
		{
			...
			for (const FNaniteShadingCommand& ShadingCommand : ShadingCommands.Commands)
			{
				...
				{
					for (uint32 ShadingBin : ShadingCommand.PipelineGroup->PipelineShadingBins)
					{
						FNaniteMaterialDebugViewInfo& DebugData = DebugViewData[ShadingBin];
				...
				{
					FNaniteMaterialDebugViewInfo& DebugData = DebugViewData[ShadingCommand.ArgsIndex];
		...
		SceneData->TaskHandles[InitPrimitiveDataTask] = GraphBuilder.AddSetupTask(
			[this]
			{
				for (auto PrimitiveSceneInfo : AddedList)
				{
					...
					// Error case: all Nanite meshes must have at least one material (check for errors from scene proxy creation).
					// We handle it gracefully by enforcing one material slot in the buffer, default initialized, causing binning
					// of *any* material index to be skipped (prevents corruption, GPU hangs/crashes)
					const int32 NumMaterials = FMath::Max(NaniteProxy->GetMaterialMaxIndex() + 1, 1);
					
					FPrimitiveData NewData;
					NewData.PrimitiveSceneInfo = PrimitiveSceneInfo;
					NewData.NumMaterials = NumMaterials;
	...
	const TConstArrayView<FPrimitiveSceneInfo*>& SceneInfosWithStaticDrawListUpdate)
{
	...
	auto UploadMaterialData = [this](const FPrimitiveData& Data)
	{
		...
		for (auto& PassMaterialSlots : Data.PrimitiveSceneInfo->NaniteMaterialSlots)
		{
			...
			{
				uint32 SectionIndex = 0;
				uint64 UninitializedSlots = Data.NumMaterials >= 64 ? ~0ull : (1ull << uint64(Data.NumMaterials)) - 1ull;
