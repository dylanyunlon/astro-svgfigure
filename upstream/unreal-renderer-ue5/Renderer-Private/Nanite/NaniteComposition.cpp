...
class FEmitSceneDepthPS : public FNaniteGlobalShader
{
	...
	class FCurveSkinningDim : SHADER_PERMUTATION_BOOL("USE_CURVE_SKINNING");
	using FPermutationDomain = TShaderPermutationDomain<FVelocityExportDim, FShadingMaskExportDim, FSkinningDim, FCurveSkinningDim>;
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		if (PermutationVector.Get<FCurveSkinningDim>())
		{
			if (!NaniteCurvesSupported())
			{
				return false;
			}
			...
			if (!PermutationVector.Get<FVelocityExportDim>())
			{
				// Curve Skinning is only needed when velocity will be exported
				return false;
			}
		}
...
)
{
	...
	if (UseComputeDepthExport())
	{
		...
		const uint32 PlatformConfig = RHIGetHTilePlatformConfig(SceneDepth->Desc);
		...
		PermutationVectorCS.Set<FDepthExportCS::FCurveSkinningDim>(bEmitVelocity && NaniteCurvesSupported());
	...
	else
	{
		...
		// Emit scene depth buffer, mask and velocity
		{
			...
			PermutationVectorPS.Set<FEmitSceneDepthPS::FCurveSkinningDim>(bEmitVelocity && NaniteCurvesSupported());
		...
		// Emit scene stencil
		{
			...
			FPixelShaderUtils::AddFullscreenPass(
				...
				TStaticBlendState<>::GetRHI(),
				TStaticRasterizerState<>::GetRHI(),
				TStaticDepthStencilState<false, CF_Always, true, CF_Always, SO_Keep, SO_Keep, SO_Replace>::GetRHI(),
				StencilDecalMask | GET_STENCIL_BIT_MASK(RAY_TRACING_REPRESENTATION, 1)
			);
		}
	...
	if (FirstStageDepthBuffer)
	{
		...
		PermutationVectorPS.Set<FEmitSceneDepthPS::FCurveSkinningDim>(false);
		auto  PixelShader = View.ShaderMap->GetShader<FEmitSceneDepthPS>(PermutationVectorPS);
...
)
{
	...
	if (CustomDepthContext.bComputeExport)
	{
		...
		// Export depth
		{
			...
			const FIntVector DispatchDim = FComputeShaderUtils::GetGroupCount(ViewRect.Size(), kHTileSize);
			const uint32 PlatformConfig = RHIGetHTilePlatformConfig(CustomDepth->Desc);
			...
			PermutationVectorCS.Set<FDepthExportCS::FCurveSkinningDim>(false);
			auto ComputeShader = View.ShaderMap->GetShader<FDepthExportCS>(PermutationVectorCS);
...
)
{
	...
	PassParameters->PS.ViewRect = FIntVector4(0, 0, ViewportSize.X, ViewportSize.Y);
