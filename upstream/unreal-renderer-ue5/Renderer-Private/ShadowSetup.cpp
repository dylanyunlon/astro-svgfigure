...
#include "Components/DirectionalLightSceneProxy.h"
...
TAutoConsoleVariable<bool> CVarCSMShadowDepthReuse(
	TEXT("r.Shadow.CSMShadowDepthReuse"),
	false,
	TEXT("0: Call CSM RenderDepth every frame. (default)\n")
	TEXT("1: Enable CSM ShadowDepth Reuse. This is orthogonal to scrolling."),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarCSMShadowDepthReuseInterval(
	TEXT("r.Shadow.CSMShadowDepthReuse.Interval"),
	1,
	TEXT("Interval for any given cascade to call RenderDepth if reuse is on."),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarCSMShadowDepthReuseSeparateDynamic(
	TEXT("r.Shadow.CSMShadowDepthReuse.SeparateDynamic"),
	0,
	TEXT("0: Single atlas region per cascade (default)\n")
	TEXT("1: Separate static and dynamic atlas regions per cascade"),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarCSMShadowDepthReuseDynamicInterval(
	TEXT("r.Shadow.CSMShadowDepthReuse.DynamicInterval"),
	1,
	TEXT("Interval for any dynamic-only cascade to call RenderDepth if reuse is on."),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarCSMShadowDepthReuseFarInterval(
	TEXT("r.Shadow.CSMShadowDepthReuse.FarInterval"),
	-1,
	TEXT("Override for far cascades interval to call RenderDepth if reuse is on. Only affects the static/combined far pass. A dynamic-only far pass still uses DynamicInterval\n")
	TEXT("-1: Inherit r.Shadow.CSMShadowDepthReuse.Interval (default - far cascades behave exactly as before)\n")
	TEXT(">=1: Use this interval to set and override the value set by via r.Shadow.CSMShadowDepthReuse.Interval\n"),
	ECVF_RenderThreadSafe);

TAutoConsoleVariable<int32> CVarCSMShadowDepthReuseDynamicCascadeMask(
	TEXT("r.Shadow.CSMShadowDepthReuse.DynamicCascadeMask"),
	0,
	TEXT("If splitting cascades for static and dynamic meshes, this is a bitmask of which cascades require split static and dynamic cascades.\n")
	TEXT("0x0: None (default - combined static and dynamic)\n")
	TEXT("0x1: Cascade 0 only\n")
	TEXT("0x2: Cascades 1 only\n")
	TEXT("etc. 0x(2^N) is cascade N only\n")
	TEXT("-1: All cascades\n"),
	ECVF_RenderThreadSafe);
...
/**
 * Determines whether a given CSM cascade should be split into separate static and dynamic depth passes
 * when CSM depth reuse is enabled with SeparateDynamic mode.
 */
static bool ShouldSplitCascadeStaticDynamic(int32 CascadeIndex)
{
	int32 CascadeMask = CVarCSMShadowDepthReuseDynamicCascadeMask.GetValueOnRenderThread();
	return (CascadeMask == -1) || ((CascadeIndex >= 0) && (CascadeMask & (1 << CascadeIndex)));
}

/**
 * Computes the densely-packed base index into the CSMDepthReuseData array for a given cascade,
 * accounting for prior cascades that may each consume 1 or 2 slots depending on whether they are split.
 */
static int32 ComputeBaseDepthReuseIndex(int32 CascadeIndex)
{
	const int32 CascadeMask = CVarCSMShadowDepthReuseDynamicCascadeMask.GetValueOnRenderThread();
	const bool bSeparateDynamic = CVarCSMShadowDepthReuseSeparateDynamic.GetValueOnRenderThread() != 0;
	int32 BaseIndex = 0;
	for (int32 i = 0; i < CascadeIndex; i++)
	{
		const bool bPriorCascadeHasSeparateDynamicPass = bSeparateDynamic && ((CascadeMask == -1) || (CascadeMask & (1 << i)));
		BaseIndex += bPriorCascadeHasSeparateDynamicPass ? 2 : 1;
	}
	return BaseIndex;
}

/**
 * Invalidates all cached shadow data for a light when the dynamic cascade mask changes at runtime.
 */
static void InvalidateCachedShadowsOnMaskChange(FCSMSceneExtension& CSMExt, FLightSceneId LightId, int32 CurrentMask)
{
	int32* PreviousMask = CSMExt.GetPreviousDynamicCascadeMask(LightId);

	if (PreviousMask != nullptr && *PreviousMask != CurrentMask)
	{
		TArray<FCSMDepthReuse>* CSMDepthReuseDatas = CSMExt.GetCSMDepthReuseData(LightId);
		if (CSMDepthReuseDatas != nullptr)
		{
			for (FCSMDepthReuse& DepthReuse : *CSMDepthReuseDatas)
			{
				DepthReuse.PreviousAtlas.Release();
			}
		}

		TArray<FCachedShadowMapData>* CachedShadowMaps = CSMExt.GetCachedShadowMapDatas(LightId);
		if (CachedShadowMaps != nullptr)
		{
			for (FCachedShadowMapData& CachedData : *CachedShadowMaps)
			{
				CachedData.InvalidateCachedShadow();
			}
		}
	}

	CSMExt.SetPreviousDynamicCascadeMask(LightId, CurrentMask);
}
...
// For now, this feature is only enabled for the mobile renderer.  It could theoretically work for the deferred renderer as well, but the implementation
// currently isn't compatible with Virtual Shadow Maps, which are usually enabled by default for the deferred renderer.  In practice, it's only important
// for the mobile renderer, as it enables timesliced rendering of shadow cascades for mobile specific runtime reflection captures.
static TAutoConsoleVariable<bool> CVarSceneCaptureCubeMobileShareCascades(
	TEXT("r.SceneCapture.CubeMobileShareCascades"),
	true,
	TEXT("When set to true, mobile cube scene captures render cascade shadow depths once and share that single shadow map across all cube faces."),
	ECVF_RenderThreadSafe);

// Note that a mirror of this function is present in the Engine module's DirectionalLightComponent.cpp.  If this implementation changes, that also needs to be updated!
bool IsSceneCaptureCubeShareCascadesEnabled(EShaderPlatform Platform)
{
	return CVarSceneCaptureCubeMobileShareCascades.GetValueOnAnyThread() && IsMobilePlatform(Platform);
}
...
static float GetEffectiveWholeSceneDynamicShadowRadius(const FDirectionalLightSceneProxy& Proxy, bool bPrecomputedLightingIsValid)
{
	return bPrecomputedLightingIsValid ? Proxy.WholeSceneDynamicShadowRadius : CVarUnbuiltWholeSceneDynamicShadowRadius.GetValueOnAnyThread();
}

static float GetCSMMaxDistance(const FDirectionalLightSceneProxy& Proxy, bool bPrecomputedLightingIsValid, int32 MaxShadowCascades)
{
	...
	float Distance = GetEffectiveWholeSceneDynamicShadowRadius(Proxy, bPrecomputedLightingIsValid) * Scale;
	return Distance;
}

static uint32 GetNumShadowMappedCascades(const FDirectionalLightSceneProxy& Proxy, uint32 MaxShadowCascades, bool bPrecomputedLightingIsValid)
{
	int32 EffectiveNumDynamicShadowCascades = Proxy.DynamicShadowCascades;
	if (!bPrecomputedLightingIsValid)
	{
		...
		static const auto CVarUnbuiltPreviewShadowsInGameLocal = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Shadow.UnbuiltPreviewInGame"));
		bool bUnbuiltPreviewShadowsInGame = CVarUnbuiltPreviewShadowsInGameLocal->GetInt() != 0;

		if (!bUnbuiltPreviewShadowsInGame && !Proxy.GetSceneInterface()->IsEditorScene())
		{
			EffectiveNumDynamicShadowCascades = 0;
		}
	}

	const int32 NumCascades = GetCSMMaxDistance(Proxy, bPrecomputedLightingIsValid, MaxShadowCascades) > 0.0f ? EffectiveNumDynamicShadowCascades : 0;
	return FMath::Min<int32>(NumCascades, MaxShadowCascades);
}

static float GetDistanceFieldShadowDistance(const FDirectionalLightSceneProxy& Proxy)
{
	...
	return Proxy.DistanceFieldShadowDistance * FMath::Clamp(CVarRTDFDistanceScale.GetValueOnRenderThread(), 0.0001f, 10000.0f);
}

static float GetShadowTransitionScale()
{
...
static float GetEffectiveCascadeDistributionExponent(const FDirectionalLightSceneProxy& Proxy, bool bPrecomputedLightingIsValid)
{
	return bPrecomputedLightingIsValid ? Proxy.CascadeDistributionExponent : 4;
}

static bool ShouldCreateRayTracedCascade(const FDirectionalLightSceneProxy& Proxy, ERHIFeatureLevel::Type InFeatureLevel, bool bPrecomputedLightingIsValid, int32 MaxNearCascades);

// @param SplitIndex 0: near, 1:end of 1st cascade, ...
static float GetSplitDistance(const FLightSceneInfo& LightSceneInfo, const FDirectionalLightSceneProxy& Proxy, const FSceneView& View, uint32 SplitIndex, bool bPrecomputedLightingIsValid, bool bDistanceFieldShadows)
{
	// near cascade means non far and non distance field cascade
	uint32 NumNearCascades = GetNumShadowMappedCascades(Proxy, View.MaxShadowCascades, bPrecomputedLightingIsValid);

	float CascadeDistanceWithoutFar = GetCSMMaxDistance(Proxy, bPrecomputedLightingIsValid, View.MaxShadowCascades);
	const FShadowDepthCaptureView* CapturedView = LightSceneInfo.Scene->GetExtension<FCSMSceneExtension>().GetCapturedView(LightSceneInfo.Id);
	float ShadowNear = CapturedView != nullptr ? CapturedView->NearClippingDistance : View.NearClippingDistance;
	const float EffectiveCascadeDistributionExponent = GetEffectiveCascadeDistributionExponent(Proxy, bPrecomputedLightingIsValid);
	...
	if (SplitIndex > NumNearCascades)
	{
		if (bDistanceFieldShadows)
		{
			...
			return GetDistanceFieldShadowDistance(Proxy);
		}
		else
		{
			// the far cascades start at the after the near cascades
			uint32 ClampedFarShadowCascadeCount = FMath::Min((uint32)CVarMaxNumFarShadowCascades.GetValueOnAnyThread(), Proxy.GetFarShadowCascadeCount());

			return CascadeDistanceWithoutFar + ComputeAccumulatedScale(EffectiveCascadeDistributionExponent, SplitIndex - NumNearCascades, ClampedFarShadowCascadeCount) * ((CVarFarShadowDistanceOverride.GetValueOnAnyThread() > 0.0f ? CVarFarShadowDistanceOverride.GetValueOnAnyThread() : Proxy.GetFarShadowDistance()) - CascadeDistanceWithoutFar);
		}
	}
	else
	{
...
uint32 GetNumViewDependentWholeSceneShadows(const FLightSceneInfo& LightSceneInfo, const FSceneView& View, bool bPrecomputedLightingIsValid)
{
	check(LightSceneInfo.Proxy->GetLightType() == LightType_Directional);
	const FDirectionalLightSceneProxy& Proxy = static_cast<const FDirectionalLightSceneProxy&>(*LightSceneInfo.Proxy);

	uint32 ClampedFarShadowCascadeCount = FMath::Min((uint32)CVarMaxNumFarShadowCascades.GetValueOnAnyThread(), Proxy.GetFarShadowCascadeCount());

	uint32 TotalCascades = GetNumShadowMappedCascades(Proxy, View.MaxShadowCascades, bPrecomputedLightingIsValid) + ClampedFarShadowCascadeCount;
...
static bool ShouldCreateRayTracedCascade(const FDirectionalLightSceneProxy& Proxy, ERHIFeatureLevel::Type InFeatureLevel, bool bPrecomputedLightingIsValid, int32 MaxNearCascades)
{
	static auto CVarDistanceFieldShadowingLocal = IConsoleManager::Get().FindConsoleVariable(TEXT("r.DistanceFieldShadowing"));
	static IConsoleVariable* CVarHFShadowing = IConsoleManager::Get().FindConsoleVariable(TEXT("r.HeightFieldShadowing"));

	if (CVarDistanceFieldShadowingLocal != nullptr && CVarDistanceFieldShadowingLocal->GetInt() == 0 && (!CVarHFShadowing || !CVarHFShadowing->GetInt()))
	{
		return false;
	}

	const uint32 NumCascades = GetNumShadowMappedCascades(Proxy, MaxNearCascades, bPrecomputedLightingIsValid);
	const float RayTracedShadowDistance = GetDistanceFieldShadowDistance(Proxy);
	const bool bCreateWithCSM = NumCascades > 0 && RayTracedShadowDistance > GetCSMMaxDistance(Proxy, bPrecomputedLightingIsValid, MaxNearCascades);
...
FVector2D GetDirectionalLightDistanceFadeParameters(const FLightSceneInfo& LightSceneInfo, ERHIFeatureLevel::Type InFeatureLevel, bool bPrecomputedLightingIsValid, int32 MaxNearCascades)
{
	...
	if (Proxy.UseVirtualShadowMaps())
	{
		return FVector2D::ZeroVector;
	}

	float FarDistance = GetCSMMaxDistance(Proxy, bPrecomputedLightingIsValid, MaxNearCascades);
	{
		if (ShouldCreateRayTracedCascade(Proxy, InFeatureLevel, bPrecomputedLightingIsValid, MaxNearCascades))
		{
			FarDistance = GetDistanceFieldShadowDistance(Proxy);
		}
		FarDistance = FMath::Max(FarDistance, CVarFarShadowDistanceOverride.GetValueOnAnyThread() > 0.0f ? CVarFarShadowDistanceOverride.GetValueOnAnyThread() : Proxy.GetFarShadowDistance());
	}
	...
	const float NearDistance = FarDistance - FarDistance * (Proxy.ShadowDistanceFadeoutFraction * CVarCSMShadowDistanceFadeoutMultiplier.GetValueOnAnyThread());
	return FVector2D(NearDistance, 1.0f / FMath::Max<float>(FarDistance - NearDistance, UE_KINDA_SMALL_NUMBER));
}

float GetMaxShadowReceiverDistance(const FLightSceneInfo& LightSceneInfo, const FSceneView& View, bool bPrecomputedLightingIsValid)
{
	check(LightSceneInfo.Proxy->GetLightType() == LightType_Directional);
	const FDirectionalLightSceneProxy& Proxy = static_cast<const FDirectionalLightSceneProxy&>(*LightSceneInfo.Proxy);

	const float CSMMax = GetCSMMaxDistance(Proxy, bPrecomputedLightingIsValid, View.MaxShadowCascades);
	const uint32 ClampedFarCount = FMath::Min((uint32)CVarMaxNumFarShadowCascades.GetValueOnAnyThread(), Proxy.GetFarShadowCascadeCount());
	if (ClampedFarCount > 0)
	{
		const float FarOverride = CVarFarShadowDistanceOverride.GetValueOnAnyThread();
		const float FarEnd = FarOverride > 0.0f ? FarOverride : Proxy.GetFarShadowDistance();
		return FMath::Max(CSMMax, FarEnd);
	}
	return CSMMax;
}

FSphere GetShadowSplitBoundsDepthRange(const FLightSceneInfo& LightSceneInfo, const FSceneView& View, FVector ViewOrigin, float SplitNear, float SplitFar, FShadowCascadeSettings* OutCascadeSettings)
{
	check(LightSceneInfo.Proxy->GetLightType() == LightType_Directional);
	const FDirectionalLightSceneProxy& Proxy = static_cast<const FDirectionalLightSceneProxy&>(*LightSceneInfo.Proxy);

	const FVector LightDirection = -Proxy.GetDirection();

	const FShadowDepthCaptureView* CapturedView = LightSceneInfo.Scene ? LightSceneInfo.Scene->GetExtension<FCSMSceneExtension>().GetCapturedView(LightSceneInfo.Id) : nullptr;

	if (CapturedView != nullptr)
	{
		ViewOrigin = CapturedView->ViewOrigin;
	}

	if (View.bIsSceneCaptureCube && IsSceneCaptureCubeShareCascadesEnabled(View.GetShaderPlatform()))
	{
		if (OutCascadeSettings)
		{
			// Get the 8 corners of the cascade's camera frustum, in world space
			FVector CascadeFrustumVerts[8];
			CascadeFrustumVerts[0] = ViewOrigin + FVector( SplitFar,  SplitFar, -SplitFar);    // 0 Near Top    Right
			CascadeFrustumVerts[1] = ViewOrigin + FVector( SplitFar, -SplitFar, -SplitFar);    // 1 Near Bottom Right
			CascadeFrustumVerts[2] = ViewOrigin + FVector(-SplitFar,  SplitFar, -SplitFar);    // 2 Near Top    Left
			CascadeFrustumVerts[3] = ViewOrigin + FVector(-SplitFar, -SplitFar, -SplitFar);    // 3 Near Bottom Left
			CascadeFrustumVerts[4] = ViewOrigin + FVector( SplitFar,  SplitFar,  SplitFar);    // 4 Far  Top    Right
			CascadeFrustumVerts[5] = ViewOrigin + FVector( SplitFar, -SplitFar,  SplitFar);    // 5 Far  Bottom Right
			CascadeFrustumVerts[6] = ViewOrigin + FVector(-SplitFar,  SplitFar,  SplitFar);    // 6 Far  Top    Left
			CascadeFrustumVerts[7] = ViewOrigin + FVector(-SplitFar, -SplitFar,  SplitFar);    // 7 Far  Bottom Left
		...
		// Set a sphere around the entire box bounding volume of all six cube map faces
		const float Sqrt3 = 1.73205080756887729353f;
		return FSphere(ViewOrigin, Sqrt3 * SplitFar);
	}

	// When a shadow-depth capture is pending, run cascade fitting against the captured pose's matrices instead of the camera's
	const FViewMatrices& ViewMatrices = View.GetShadowViewMatrices();
	const FMatrix& ViewMatrix       = CapturedView != nullptr ? CapturedView->WorldToView : ViewMatrices.GetWorldToView();
	const FMatrix& ProjectionMatrix = CapturedView != nullptr ? CapturedView->ViewToClip  : ViewMatrices.GetViewToClipNoAA();
	...
	bool bIsPerspectiveProjection = CapturedView != nullptr ? true : ViewMatrices.IsPerspectiveProjection();
...
FSphere GetShadowSplitBounds(const FLightSceneInfo& LightSceneInfo, const FSceneView& View, int32 InCascadeIndex, bool bPrecomputedLightingIsValid, FShadowCascadeSettings* OutCascadeSettings)
{
	check(LightSceneInfo.Proxy->GetLightType() == LightType_Directional);
	const FDirectionalLightSceneProxy& Proxy = static_cast<const FDirectionalLightSceneProxy&>(*LightSceneInfo.Proxy);
	...
	const bool bHasRayTracedCascade = ShouldCreateRayTracedCascade(Proxy, View.GetFeatureLevel(), bPrecomputedLightingIsValid, View.MaxShadowCascades);

	// this checks for WholeSceneDynamicShadowRadius and DynamicShadowCascades
	uint32 NumNearAndFarCascades = GetNumViewDependentWholeSceneShadows(LightSceneInfo, View, bPrecomputedLightingIsValid);
	...
	float SplitNear = GetSplitDistance(LightSceneInfo, Proxy, View, ShadowSplitIndex, bPrecomputedLightingIsValid, bIsRayTracedCascade);
	float SplitFar = GetSplitDistance(LightSceneInfo, Proxy, View, ShadowSplitIndex + 1, bPrecomputedLightingIsValid, bIsRayTracedCascade);

	float FadePlane = SplitFar;

	float LocalCascadeTransitionFraction = Proxy.CascadeTransitionFraction * GetShadowTransitionScale();
	...
	else if (!(bPrecomputedLightingIsValid && Proxy.HasStaticShadowing()))
	{
	...
	if (OutCascadeSettings)
	{
		...
		if (ShadowSplitIndex >= 1)
		{
			...
			float BeforeSplitNear = GetSplitDistance(LightSceneInfo, Proxy, View, ShadowSplitIndex - 1, bPrecomputedLightingIsValid, bIsRayTracedCascade);
			float BeforeSplitFar = GetSplitDistance(LightSceneInfo, Proxy, View, ShadowSplitIndex, bPrecomputedLightingIsValid, bIsRayTracedCascade);
		...
		OutCascadeSettings->CascadeBiasDistribution = Proxy.ShadowCascadeBiasDistribution;
	...
	bool bUseNearOverride = (Proxy.bCastShadowsOnClouds || Proxy.bCastShadowsOnAtmosphere)
	...
	const float BoundsCalcNear = bUseNearOverride ? GetSplitDistance(LightSceneInfo, Proxy, View, 0, bPrecomputedLightingIsValid, bIsRayTracedCascade) : SplitNear;
	const FSphere CascadeSphere = GetShadowSplitBoundsDepthRange(LightSceneInfo, View, View.ViewMatrices.GetViewOrigin(), BoundsCalcNear, SplitFar, OutCascadeSettings);

	return CascadeSphere;
}

bool GetViewDependentWholeSceneProjectedShadowInitializer(const FLightSceneInfo& LightSceneInfo, const FSceneView& View, int32 InCascadeIndex, bool bPrecomputedLightingIsValid, FWholeSceneProjectedShadowInitializer& OutInitializer)
{
	check(LightSceneInfo.Proxy->GetLightType() == LightType_Directional);
	const FDirectionalLightSceneProxy& Proxy = static_cast<const FDirectionalLightSceneProxy&>(*LightSceneInfo.Proxy);
	...
	FSphere Bounds = GetShadowSplitBounds(LightSceneInfo, View, InCascadeIndex, bPrecomputedLightingIsValid, &OutInitializer.CascadeSettings);
	...
	OutInitializer.WorldToLight = FInverseRotationMatrix(Proxy.GetDirection().GetSafeNormal().Rotation());
...
FProjectedShadowInfo::FProjectedShadowInfo()
	...
	, bIsDynamicOnly(false)
	, CSMShadowDepthReuseIndex(-1)
...
{	
	...
	bVolumetricShadow = InLightSceneInfo->Proxy->CastsVolumetricShadow();
	BorderSize = InBorderSize;
	SelectedViewIndex = InSelectedViewIndex;
	...
	if (CascadeSettings.ShadowSplitIndex >= 0 && bDirectionalLight)
	{
		...
		ShadowBounds = ::GetShadowSplitBounds(
			*InLightSceneInfo,
			*InDependentView,
...
void FProjectedShadowInfo::SetupClipmapProjection(FLightSceneInfo* InLightSceneInfo, FViewInfo* InDependentView, const TSharedPtr<FVirtualShadowMapClipmap>& InVirtualShadowMapClipmap, float InMaxNonFarCascadeDistance)
{
	...
	else
	{
		...
		ShadowBounds = ::GetShadowSplitBoundsDepthRange(*InLightSceneInfo, *DependentView, DependentView->ViewMatrices.GetViewOrigin(), 10.0f, VirtualShadowMapClipmap->GetMaxRadius(), &CascadeSettings);
	}
	...
	int32& NumMeshCommandBuildRequestElements)
{
	...
	if (bUseCachedMeshCommand)
	{
		...
		if (StaticMeshCommandInfoIndex >= 0)
		{
			...
			const FMeshDrawCommand& MeshDrawCommand = Scene->CachedMeshDrawCommandStateBuckets[PassType].GetByElementId(CachedMeshDrawCommand.StateBucketId).Key;
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	FSceneRendererLightBitArray OnePassShadowUnsupportedLights;
#endif
};
...
struct FDynamicShadowsTaskData
{
	...
	float MinScreenBoundsRadiusSquared;			// For screen size culling
		...
		, bMultithreadedGDME(bMultithreadedCreateAndFilterShadows && GetNumShadowDynamicMeshElementTasks() > 0)
	{
		...
		// We can just pass the first view to this function, as the view is only used to check flags to see if the current scene renderer is
		// a runtime reflection capture, which use a different more aggressive screen culling size.  All views in such a scene renderer will
		// have the same flags set.
		MinScreenBoundsRadiusSquared = ScreenVisibilityGetMinScreenBoundsRadiusSquared(Views[0]);
	}
...
FLODMask FProjectedShadowInfo::CalcAndUpdateLODToRender(FViewInfo& CurrentView, const FBoxSphereBounds& Bounds, const FPrimitiveSceneInfo* PrimitiveSceneInfo, int32 ForcedLOD) const
{
	...
	const bool bNeedsIndependentLODMask = ShadowLODDistanceFactor != 1.0f
		// Must always trigger the culling calculations for this case as they compute both the LOD mask and culling result.
		|| (SceneCullingRendererCullingQuery && SceneCullingRendererCullingQuery->HasCullingInfo(PrimitiveId));

	FLODMask ShadowLODToRender = CurrentView.PrimitivesLODMask[PrimitiveId];
	// calculate it it's not set OR if LOD is overridden
	if (ForcedLOD > -1 || !ShadowLODToRender.IsValid() || bNeedsIndependentLODMask)
	{
		...
		ShadowLODToRender = ComputePrimitiveMeshLODs(CurrentView, ForcedLOD, LODScale, PrimitiveSceneInfo->Proxy, PrimitiveSceneInfo, PrimitiveId, Bounds, SceneCullingRendererCullingQuery);

		// TODO: support caching when ShadowLODDistanceFactorEnabled (cascades of the same type (regular/far) could reuse results)
		if (!bNeedsIndependentLODMask)
		{
...
bool FProjectedShadowInfo::AddSubjectPrimitive(FPrimitiveSceneInfo* PrimitiveSceneInfo, TArrayView<FViewInfo> ViewArray, bool bShouldRecordShadowSubjectsForMobileShading, bool bInvertViewShadowRelevance)
{
		...
		&& TestPrimitiveFarCascadeConditions(PrimitiveSceneInfo->Proxy->CastsFarShadow(), PrimitiveSceneInfo->Proxy->GetBounds()))
	{
		...
		else if (SelectedViewIndex != INDEX_NONE)
		{
			Views = TArrayView<FViewInfo>(&ViewArray[SelectedViewIndex], 1);
		}
		else
		{
		...
		PrimitiveId = PrimitiveSceneInfo->GetIndex();
		...
		if (bOpaque && bShadowRelevance)
		{
			...
			// EShadowMeshSelection::All is intentional because AllVertexFactoriesSupportGPUScene being false may indicate that some VFs (e.g., some LODs) support GPUScene while others don't
			// thus we have to leave the final decision until the mesh batches are produced.
			else if (bDynamicRelevance && EnumHasAnyFlags(MeshSelectionMask, Proxy->AllVertexFactoriesSupportGPUScene() ? EShadowMeshSelection::VSM : EShadowMeshSelection::All))
			{
...
bool FProjectedShadowInfo::TestPrimitiveFarCascadeConditions(bool bPrimitiveCastsFarShadow, const FBoxSphereBounds& Bounds) const
{
	...
	if (bVSM && IsWholeSceneDirectionalShadow() && MaxNonFarCascadeDistance > 0.0f)
	{
		check(DependentView);
	...
	FAddSubjectPrimitiveOverflowedIndices& OverflowBuffer) const
{
		...
		&& TestPrimitiveFarCascadeConditions(PrimitiveSceneInfoCompact.Proxy->CastsFarShadow(), FBoxSphereBounds(PrimitiveSceneInfoCompact.Bounds)))
	{
		...
		{
			CurrentView = &ViewArray[SelectedViewIndex];
		}
		else
		{
			checkf(ViewArray.Num(),
		...
		if (bOpaque)
		{
			...
			// EShadowMeshSelection::All is intentional because AllVertexFactoriesSupportGPUScene being false may indicate that some VFs (e.g., some LODs) support GPUScene while others don't
			// thus we have to leave the final decision until the mesh batches are produced.
			if (!bDrawingStaticMeshes && bDynamicRelevance && EnumHasAnyFlags(MeshSelectionMask, PrimitiveSceneInfoCompact.PrimitiveFlagsCompact.bAllVertexFactoriesSupportGPUScene ? EShadowMeshSelection::VSM : EShadowMeshSelection::All))
			{
	...
	FFinalizeAddSubjectPrimitiveContext& Context)
{
	...
	if (CacheMode != SDCM_Uncached && IsWholeSceneDirectionalShadow())
	{
		...
		FCachedShadowMapData& CachedShadowMapData = PrimitiveSceneInfo->Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDataRef(LightSceneInfo->Id, CascadeSettings.ShadowSplitIndex);
	...
	if (Result.bCopyCachedMeshDrawCommand)
	{
		...
		for (int32 Idx = 0; Idx < NumMDCs; ++Idx)
		{
			...
			const FMeshDrawCommand& CachedCmd = Scene->CachedMeshDrawCommandStateBuckets[MeshPassType].GetByElementId(CmdInfo.StateBucketId).Key;
...
FCullingVolume FProjectedShadowInfo::GetCullingVolume() const
{
	FCullingVolume CullingVolume;
	CullingVolume.WorldToVolumeTranslation = PreShadowTranslation;

	if (VirtualShadowMapClipmap)
	{
		const bool bIsCached = VirtualShadowMapClipmap->GetCacheEntry() && !VirtualShadowMapClipmap->GetCacheEntry()->IsUncached();
		...
		if (!bIsCached && !CascadeSettings.ShadowBoundsAccurate.Planes.IsEmpty())
		{
			CullingVolume.ConvexVolume = CascadeSettings.ShadowBoundsAccurate;
			// ShadowBoundsAccurate is in world-space
			CullingVolume.WorldToVolumeTranslation = FVector3d::ZeroVector;
		}
		else
		{
			CullingVolume.Sphere = VirtualShadowMapClipmap->GetBoundingSphere();
			CullingVolume.ConvexVolume = VirtualShadowMapClipmap->GetViewFrustumBounds();
		}
	}
	else if (!CascadeSettings.ShadowBoundsAccurate.Planes.IsEmpty())
	{
		CullingVolume.ConvexVolume = CascadeSettings.ShadowBoundsAccurate;
		// ShadowBoundsAccurate is in world-space
		CullingVolume.WorldToVolumeTranslation = FVector3d::ZeroVector;
	...
	{
		CullingVolume.Sphere = GetLightSceneInfo().Proxy->GetBoundingSphere();
		CullingVolume.ConvexVolume = CasterOuterFrustum;
	}
	return CullingVolume;
}

void FProjectedShadowInfo::SetupCullingQuery(FSceneRenderer& SceneRenderer)
{
	if (FSceneCullingRenderer* SceneCullingRenderer = SceneRenderer.GetSceneExtensionsRenderers().GetRendererPtr<FSceneCullingRenderer>())
	{
		if (SceneCullingRenderer->UseHierarchicalCPUCulling())
		{
			check(DependentView || SelectedViewIndex != INDEX_NONE);
			const FViewInfo& View = DependentView ? *DependentView : SceneRenderer.Views[SelectedViewIndex];
			SceneCullingRendererCullingQuery = SceneCullingRenderer->CreateHierarchicalCullingQuery(GetCullingVolume());
		}
	}
}								
...
void FProjectedShadowInfo::SetupMeshDrawCommandsForShadowDepth(FSceneRenderer& Renderer, FInstanceCullingManager& InstanceCullingManager)
{
	...
	if (!bPerObjectOpaqueShadow && 
		((!HasVirtualShadowMap() && CacheMode != SDCM_StaticPrimitivesOnly) 
	...
	// Note: Needed to support legacy, non-GPU-Scene culled, primitives, this is merely used to allocate enough space for CPU-side replication.
	...
	ShadowDepthPass.DispatchPassSetup(
		Renderer.Scene,
		*ShadowDepthView,
		FInstanceCullingContext(TEXT("ShadowDepth"), Renderer.ShaderPlatform, &InstanceCullingManager, ViewIds, nullptr, EInstanceCullingMode::Normal, EInstanceCullingFlags::NoInstanceOrderPreservation, SingleInstanceProcessingMode, SceneCullingRendererCullingQuery),
		EMeshPass::Num,
...
{
	SceneCullingRendererCullingQuery = nullptr;
	NumDynamicSubjectMeshElements = 0;
	NumSubjectMeshCommandBuildRequestElements = 0;
...
void FProjectedShadowInfo::ComputeScissorRectOptim()
{
	...
	// Skip the scissor-rect optimization for frozen / captured shadows — clipped depth would void the cached atlas.
	FScene* Scene = GetLightSceneInfo().Scene;
	check(Scene);
	if (Scene->GetExtension<FCSMSceneExtension>().GetCapturedView(GetLightSceneInfo().Id) != nullptr)
	{
		ScissorRectOptim.Min = FIntPoint(BorderSize, BorderSize);
		ScissorRectOptim.Max = FIntPoint(ResolutionX + BorderSize, ResolutionY + BorderSize);
		return;
	}

	// get the far plane corners in world space
	const FMatrix& ViewMatrix = DependentView->ViewMatrices.GetWorldToView();
	const FMatrix& ProjectionMatrix = DependentView->ViewMatrices.GetViewToClip();
	const FVector CameraDirection = ViewMatrix.GetColumn(2);
...
bool FProjectedShadowInfo::ShouldCSMRenderDepthThisFrame(
	const FScene* Scene,
	const FCSMDepthReuse* CSMDepthReuse
) const
{
	check(Scene != nullptr);

	if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() == 0 || CSMDepthReuse == nullptr)
	{
		return true;
	}

	// Explicit-view freeze takes priority over the reuse-interval heuristic: while frozen with a captured atlas, return false so the caller's restore-and-return path keeps reusing it.
	if (CSMDepthReuse->bHasEverRendered && Scene->GetExtension<FCSMSceneExtension>().IsShadowDepthFrozen(GetLightSceneInfo().Id))
	{
		return false;
	}

	// Select the re-render interval for this cascade. There are 3 possible cases:
	//   - When depth reuse is on and r.Shadow.CSMShadowDepthReuse.SeparateDynamic is set, dynamic passes (both CSM and Far cascades) will use DynamicInterval
	//   - When r.Shadow.CSMShadowDepthReuse.FarInterval is set, far-shadow static/combined cascades will use FarInterval when it is set to >=0
	//   - Every other case will use the shared near Interval
	int32 IntervalValue;
	if (bIsDynamicOnly)
	{
		IntervalValue = CVarCSMShadowDepthReuseDynamicInterval.GetValueOnRenderThread();
	}
	else if (CascadeSettings.bFarShadowCascade && CVarCSMShadowDepthReuseFarInterval.GetValueOnRenderThread() >= 0)
	{
		IntervalValue = CVarCSMShadowDepthReuseFarInterval.GetValueOnRenderThread();
	}
	else
	{
		IntervalValue = CVarCSMShadowDepthReuseInterval.GetValueOnRenderThread();
	}
	const uint32 Interval = static_cast<uint32>(FMath::Max<int32>(IntervalValue, 1));

	uint32 CurrentFrame = Scene->GetFrameNumberRenderThread();
	uint32 PreviousFrame = CSMDepthReuse->PreviousRenderDepthFrameNumber;

	if (CascadeSettings.ShadowSplitIndex < 0)
	{
		return false;
	}
	uint32 CascadeOffset = static_cast<uint32>(CascadeSettings.ShadowSplitIndex) % Interval;
	if (!CSMDepthReuse->bHasEverRendered)
	{
		return (CurrentFrame % Interval) == CascadeOffset; 
	}

	const uint32 FrameGap = CurrentFrame - PreviousFrame;
	bool bShouldRender = (FrameGap >= Interval) && ((CurrentFrame % Interval) == CascadeOffset);
...
FSceneViewState::FProjectedShadowKey::FProjectedShadowKey(const FProjectedShadowInfo& ProjectedShadowInfo)
	...
	, LightComponentId(ProjectedShadowInfo.GetLightSceneInfo().Proxy->GetLightComponentId())
...
void FSceneRenderer::UpdatePreshadowCache()
{
	if (ShouldUseCachePreshadows() && !Views[0].bIsSceneCapture)
	{
		...
		for (int32 CachedShadowIndex = Scene->CachedPreshadows.Num() - 1; CachedShadowIndex >= 0; CachedShadowIndex--)
		{
			...
			{
				bShadowBeingRenderedThisFrame = VisibleLightInfo.ProjectedPreShadows.Find(CachedShadow) != INDEX_NONE;
		...
		for (const FVisibleLightInfo& VisibleLightInfo : VisibleLightInfos)
		{
			...
			{
				TRefCountPtr<FProjectedShadowInfo> CurrentShadow = VisibleLightInfo.ProjectedPreShadows[ShadowIndex];
...
void FSceneRenderer::SetupInteractionShadows(
	...
	TArray<FProjectedShadowInfo*,SceneRenderingAllocator>& PreShadows)
{
	check(!UseVirtualShadowMaps(ShaderPlatform));
	...
	if (!bShadowHandledByParent)
	{
		...
		bool bCreateOpaqueObjectShadow = Interaction->HasInsetObjectShadow();
		if (bCreateOpaqueObjectShadow && TaskData.FeatureLevel < ERHIFeatureLevel::SM5)
		{
			bCreateOpaqueObjectShadow = (CVarMobileInsetShadows.GetValueOnRenderThread() && Interaction->HasInsetObjectShadow())
				|| (IsCapsuleDirectShadowsEnabled(TaskData.ShaderPlatform) && PrimitiveSceneInfo->Proxy->CastsCapsuleDirectShadow());
		}
		...
		if (Interaction->HasShadow()
			...
			&& (bCreateTranslucentObjectShadow || bCreateOpaqueObjectShadow || bCreateObjectShadowForStationaryLight))
		{
			// Create projected shadow infos
			CreatePerObjectProjectedShadow(TaskData, Interaction, bCreateTranslucentObjectShadow, bCreateOpaqueObjectShadow || bCreateObjectShadowForStationaryLight, ViewDependentWholeSceneShadows, PreShadows);
		}
	}
}
	...
	TArray<FProjectedShadowInfo*, SceneRenderingAllocator>& OutPreShadows)
{
	...
	bool bCanUseShadowDepthAtlas = FProjectedShadowInfo::SupportsShadowStencilCulling(ShaderPlatform);
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		const FSceneViewState::FProjectedShadowKey OpaqueKey(PrimitiveSceneInfo->PrimitiveComponentId, LightSceneInfo->Proxy->GetLightComponentId(), INDEX_NONE, false);
		...
		const FSceneViewState::FProjectedShadowKey TranslucentKey(PrimitiveSceneInfo->PrimitiveComponentId, LightSceneInfo->Proxy->GetLightComponentId(), INDEX_NONE, true);
		...
		&& LightSceneInfo->Proxy->GetPerObjectProjectedShadowInitializer(Bounds, ShadowInitializer))
	{
		...
		if (CVarAllowPerObjectShadows.GetValueOnRenderThread() && MaxFadeAlpha > 1.0f / 256.0f)
		{
			...
			int32 SizeX = bCanUseShadowDepthAtlas	? (1 << (FMath::CeilLogTwo(MaxDesiredResolution) - 1))
													: (FMath::RoundUpToPowerOfTwo(MaxDesiredResolution) - SHADOW_BORDER * 2); // see AllocateAtlasedShadowDepthTargets, we round up target size there

			SizeX = FMath::Min<int32>(SizeX, MaxShadowResolution);
			...
			&& bOpaque)
		{
			...
			for (int32 i = 0; i < ViewDependentWholeSceneShadows.Num(); i++)
			{
				...
				const FVector2D DistanceFadeValues = GetDirectionalLightDistanceFadeParameters(WholeSceneShadow->GetLightSceneInfo(), Scene->GetFeatureLevel(), WholeSceneShadow->GetLightSceneInfo().IsPrecomputedLightingValid(), WholeSceneShadow->DependentView->MaxShadowCascades);
	...
	EShadowDepthCacheMode* OutCacheModes)
{
	...
	FCSMSceneExtension& CSMShadowScene = Scene->GetExtension<FCSMSceneExtension>();
	...
	{
		TArray<FCachedShadowMapData>* CachedShadowMapDatas = CSMShadowScene.GetCachedShadowMapDatas(LightSceneInfo->Id);
		...
		if (CachedShadowMapDatas)
		{
			...
			if (InOutProjectedShadowInitializer.IsCachedShadowValid(CachedShadowMapData->Initializer) 
				&& CachedShadowMapData->bCachedShadowMapHasNaniteGeometry)
			{
		...
		else
		{
			...
			[&]()
			{
				CSMShadowScene.GetAllCachedShadowMaps().Add(LightSceneInfo->Id).Add(FCachedShadowMapData(InOutProjectedShadowInitializer, RealTime));
			},
			[&]()
			{
			});
	...
	else
	{
		OutNumShadowMaps = 1;
		OutCacheModes[0] = SDCM_Uncached;
		CSMShadowScene.GetAllCachedShadowMaps().Remove(LightSceneInfo->Id);
	}
	...
	if (CVarCSMCaching.GetValueOnAnyThread() == 1 && !UseVirtualShadowMaps(Scene->GetShaderPlatform(), Scene->GetFeatureLevel()))
	{
		...
		else
		{
			...
			[&]()
			{
				...
				{
					CachedShadowMapDatas = &CSMShadowScene.GetAllCachedShadowMaps().Add(LightSceneInfo->Id);
				}
	...
	uint32& InOutNumSpotShadowCachesUpdatedThisFrame)
{
	...
	if (LightSceneInfo->Proxy->GetWholeSceneProjectedShadowInitializer(ViewFamily, ProjectedShadowInitializers))
	{
		...
		FadeAlphas.SetNumZeroed(ViewCount);
		...
		for(int32 ViewIndex = 0; ViewIndex < ViewCount; ++ViewIndex)
		{
...

			{
				...
				FadeAlphas[ViewIndex] = FadeAlpha;
			}
		...
		if (MaxFadeAlpha > 1.0f / 256.0f)
		{
			for (int32 ShadowIndex = 0, ShadowCount = ProjectedShadowInitializers.Num(); ShadowIndex < ShadowCount; ShadowIndex++)
			{
				...
				// NOTE: Allow lights that explicitly opt in to distance field shadows to take precidence
				// We may want to change this directly in the proxy, but for now this maintains existing behavior
				if (LightSceneInfo->Proxy->UseVirtualShadowMaps() && !ProjectedShadowInitializer.bRayTracedDistanceField)
				{
					// VSMs should be enabled globally and proxy should never ask for more than one
					if (ensure(VirtualShadowMapArray.IsEnabled() &&	ProjectedShadowInitializers.Num() == 1))
					{
						...
						{
							ProjectedShadowInfo->SetupCullingQuery(*this);
				...
				else // No VSM, fall back to classic shadow map path
				{
					// Round down to the nearest power of two so that resolution changes are always doubling or halving the resolution, which increases filtering stability
					// Use the max resolution if the desired resolution is larger than that
					...
					for (int32 CacheModeIndex = 0; CacheModeIndex < NumShadowMaps; CacheModeIndex++)
					{
						...
						// Initialize depth reuse data structures upfront for CSM
						if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0
							&& LightSceneInfo->Proxy->GetLightType() == LightType_Directional
							&& ProjectedShadowInitializer.CascadeSettings.ShadowSplitIndex >= 0)
						{
							const FCSMSceneExtension& CSMExt = TaskData.Scene->GetExtension<FCSMSceneExtension>();
							int32 CascadeIndex = ProjectedShadowInitializer.CascadeSettings.ShadowSplitIndex;

							// Ensure array exists and is sized for this cascade
							TArray<FCSMDepthReuse>& CSMDepthReuseDatas = CSMExt.GetAllCSMDepthReuseData().FindOrAdd(LightSceneInfo->Id);
							const bool bWillBeSplit = CVarCSMShadowDepthReuseSeparateDynamic.GetValueOnRenderThread() != 0
								&& ShouldSplitCascadeStaticDynamic(CascadeIndex);
							const int32 PackedBaseIndex = ComputeBaseDepthReuseIndex(CascadeIndex);
							const int32 MaxSlotNeeded = PackedBaseIndex + (bWillBeSplit ? 2 : 1) - 1;
							if (CSMDepthReuseDatas.Num() <= MaxSlotNeeded)
							{
								CSMDepthReuseDatas.SetNum(MaxSlotNeeded + 1);
							}
						}
						...
						if (CacheMode[CacheModeIndex] == SDCM_StaticPrimitivesOnly)
						{
							...
							FCachedShadowMapData& CachedShadowMapData = Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDataRef(ProjectedShadowInfo->GetLightSceneInfo().Id);
...
void FSceneRenderer::InitProjectedShadowVisibility(FDynamicShadowsTaskData& TaskData)
{
	...
	for(auto LightIt = Scene->Lights.CreateConstIterator(); LightIt; ++LightIt)
	{
		...
		for( int32 ShadowIndex=0; ShadowIndex<VisibleLightInfo.AllProjectedShadows.Num(); ShadowIndex++ )
		{
			...
			for(int32 ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
			{
				...
				{
					// The view dependent projected shadow is valid for this view if it's the right eye and the projected shadow
					// is being rendered for the left eye, or if it's a cascade shared across scene capture cube faces.
					const bool bIsValidForView =
						((IStereoRendering::IsASecondaryView(View) && IStereoRendering::IsAPrimaryView(*ProjectedShadowInfo.DependentView)) ||
						 (View.bIsSceneCaptureCube && IsSceneCaptureCubeShareCascadesEnabled(View.GetShaderPlatform())))
						&& ProjectedShadowInfo.FadeAlphas.IsValidIndex(ViewIndex)
						&& ProjectedShadowInfo.FadeAlphas[ViewIndex] != 0.0f;
	...
	if(GDumpShadowSetup)
	{
		...
		UE_LOGF(LogRenderer, Display, "Dump Shadow Setup:");

		for (int32 ViewIndex = 0;ViewIndex < Views.Num(); ++ViewIndex)
		{
			FViewInfo& View = Views[ViewIndex];

			UE_LOGF(LogRenderer, Display, " View  %d/%d", ViewIndex, Views.Num());

			uint32 LightIndex = 0;
			for (const FLightSceneInfoCompact& LightSceneInfoCompact : Scene->Lights)
			{
				FLightSceneId LightId = LightSceneInfoCompact.LightSceneInfo->GetPersistentIndex();
				FVisibleLightInfo& VisibleLightInfo = VisibleLightInfos[LightId];
				FVisibleLightViewInfo& VisibleLightViewInfo = View.VisibleLightInfos[LightId];

				UE_LOGF(LogRenderer, Display, "  Light %d/%d:", LightId.GetIndex(), Scene->Lights.Num());
				...
				for( int32 ShadowIndex = 0, ShadowCount = VisibleLightInfo.AllProjectedShadows.Num(); ShadowIndex < ShadowCount; ShadowIndex++ )
				{
					...
					if(VisibleLightViewInfo.bInViewFrustum)
					{
						UE_LOGF(LogRenderer, Display, "   Shadow %d/%d: ShadowId=%d",  ShadowIndex, ShadowCount, ProjectedShadowInfo.ShadowId);
						UE_LOGF(LogRenderer, Display, "    WholeSceneDir=%d SplitIndex=%d near=%f far=%f",
						...
						UE_LOGF(LogRenderer, Display, "    bDistField=%d bFarShadows=%d Bounds=%f,%f,%f,%f",
						...
						UE_LOGF(LogRenderer, Display, "    SplitFadeRegion=%f .. %f FadePlaneOffset=%f FadePlaneLength=%f",
...
void FSceneRenderer::DrawDebugShadowFrustum(FViewInfo& View, FProjectedShadowInfo& ProjectedShadowInfo)
{
	...
	if (ProjectedShadowInfo.IsWholeSceneDirectionalShadow())
	{
		...
		const FMatrix ViewMatrix = View.ViewMatrices.GetWorldToView();
...
struct FGatherShadowPrimitivesPacket
{
	...
	bool FilterForHigherPriorityShadow(FDynamicShadowsTaskData& TaskData, const FPrimitiveSceneInfo* PrimitiveSceneInfo, const FPrimitiveSceneProxy* PrimitiveProxy) const
	{
		if (UseVirtualShadowMaps(TaskData.ShaderPlatform, TaskData.FeatureLevel))
		{
			return false;
		}

		bool bCastsHigherPriorityShadow = false;

		// If light attachment root is valid, we're in a group and need to get the flag from the root.
		// If not in mobile, it is sufficient to only check inset shadow because Inset Shadow = Inset Shadow || CapsuleDirectShadow
		if (PrimitiveSceneInfo->LightingAttachmentRoot.IsValid())
		{
			...
			if (AttachmentGroup.ParentSceneInfo)
			{
				bCastsHigherPriorityShadow = AttachmentGroup.ParentSceneInfo->Proxy->CastsInsetShadow();
			}
		}
		else
		{
			bCastsHigherPriorityShadow = PrimitiveProxy->CastsInsetShadow();
		}

		// if we are on mobile check if inset/capsule shadows are actually cast
		if (bCastsHigherPriorityShadow && TaskData.FeatureLevel < ERHIFeatureLevel::SM5)
		{
			const bool MobileAllow = MobileUsesFullDepthPrepass(TaskData.ShaderPlatform) || (IsMobileDeferredShadingEnabled(TaskData.ShaderPlatform) && !MobileAllowFramebufferFetch(TaskData.ShaderPlatform));
			const bool bSupportsMobileInsetShadows = CVarMobileInsetShadows.GetValueOnRenderThread() && MobileAllow;
			const bool bSupportsMobileCapsuleDirectShadows = IsCapsuleDirectShadowsEnabled(TaskData.ShaderPlatform) && MobileAllow;

			// primitive casts inset shadow and inset shadow is turned on
			const bool bCastsMobileInsetShadow = bCastsHigherPriorityShadow && bSupportsMobileInsetShadows;

			// Primitive casts capsule direct shadow and capsule direct shadow is turned on
			// We may need to check against light attachment root
			bool bCastsMobileCapsuleDirectShadow = false;
			if (bSupportsMobileCapsuleDirectShadows)
			{
				if (PrimitiveSceneInfo->LightingAttachmentRoot.IsValid())
				{
					const FAttachmentGroupSceneInfo& AttachmentGroup = PrimitiveSceneInfo->Scene->AttachmentGroups.FindChecked(PrimitiveSceneInfo->LightingAttachmentRoot);
					if (AttachmentGroup.ParentSceneInfo)
					{
						bCastsMobileCapsuleDirectShadow = AttachmentGroup.ParentSceneInfo->Proxy->CastsCapsuleDirectShadow();
					}
				}
				else
				{
					bCastsMobileCapsuleDirectShadow = PrimitiveProxy->CastsCapsuleDirectShadow();
				}
			}

			bCastsHigherPriorityShadow = bCastsMobileInsetShadow || bCastsMobileCapsuleDirectShadow;
		}

		return bCastsHigherPriorityShadow;
	}
	...
	void FilterPrimitiveForShadows(FDynamicShadowsTaskData& TaskData, const FPrimitiveSceneInfoCompact& PrimitiveSceneInfoCompact)
	{
		...
		const FCSMSceneExtension& CSMShadowScene = TaskData.Scene->GetExtension<FCSMSceneExtension>();
				...
				&& ProjectedShadowInfo->CascadeSettings.ShadowBoundsAccurate.IntersectBox(PrimitiveBounds.Origin, PrimitiveBounds.BoxExtent))
			{
...

				{
					...
					bScreenSpaceSizeCulled = (FMath::Square(PrimitiveBounds.SphereRadius) < FMath::Square(MinScreenRadiusForShadowCaster) * DistanceSquared * LODScaleSquared) ||
						IsScreenSizeCulled(*ProjectedShadowInfo->DependentView, PrimitiveBounds, TaskData.MinScreenBoundsRadiusSquared);
				}
				...
				{
					const FCachedShadowMapData& CachedShadowMapData = CSMShadowScene.GetCachedShadowMapDataRef(LightSceneInfo.Id, ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex);
				...
				else if (CVarCSMShadowDepthReuseSeparateDynamic.GetValueOnRenderThread() != 0 && ProjectedShadowInfo->bDirectionalLight)
				{
					// Filter subjects based on mobility when this cascade is split into static/dynamic passes
					if (ShouldSplitCascadeStaticDynamic(ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex))
					{
						if (ProjectedShadowInfo->bIsDynamicOnly)
						{
							bShadowRelevance = PrimitiveProxy->IsMovable();
						}
						else
						{
							bShadowRelevance = !PrimitiveProxy->IsMovable();
				...
				// Filter out when inset or capsule shadows are drawn
				bool bCastsHigherPriorityShadow = FilterForHigherPriorityShadow(TaskData, PrimitiveSceneInfo, PrimitiveProxy);
	...
	void AnyThreadFinalize(FDynamicShadowsTaskData& TaskData)
	{
		...
		for (int32 ShadowIndex = 0; ShadowIndex < ViewDependentWholeSceneShadowSubjectPrimitives.Num(); ShadowIndex++)
		{
			...
			{
				FCachedShadowMapData& CachedShadowMapData = TaskData.Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDataRef(ProjectedShadowInfo->GetLightSceneInfo().Id, ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex);
...
void FSceneRenderer::FinishGatherShadowPrimitives(FDynamicShadowsTaskData* TaskData)
{
	...
	if (!TaskData->ShadowArrays.OnePassShadowUnsupportedLights.IsEmpty())
	{
		OnGetOnScreenMessages.AddLambda([Scene = Scene, OnePassShadowUnsupportedLights = MoveTemp(TaskData->ShadowArrays.OnePassShadowUnsupportedLights)](FScreenMessageWriter& ScreenMessageWriter)->void
		{
			...
			for (FSceneRendererLightBitArray::FIterator It(OnePassShadowUnsupportedLights); It; ++It)
			{
				FLightSceneId LightId = It.GetIndex();
...
static bool UseSceneCaptureCubeSharedCascade(const FSceneRenderer& Renderer, const FLightSceneInfo& LightSceneInfo)
{
	// If the CVar is changed during a time sliced render of a reflection capture, and the shadow atlas was already rendered,
	// keep the shared cascade feature enabled for the remaining time sliced faces.
	return Renderer.Views[0].bIsSceneCaptureCube && LightSceneInfo.Type == LightType_Directional &&
		(IsSceneCaptureCubeShareCascadesEnabled(Renderer.ShaderPlatform) || (Renderer.Views[0].bIsReflectionCapture && Renderer.Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame));
}
	...
	uint32& NumCSMCachesUpdatedThisFrame)
{
	...
	// Cascade math below static_casts Proxy to FDirectionalLightSceneProxy unconditionally; this gate
	// is the canonical light-type guard for the whole function.
	if (LightSceneInfo.Proxy->GetLightType() != LightType_Directional)
	{
		return;
	}

	const bool bNeedsVirtualShadowMap = LightSceneInfo.Proxy->UseVirtualShadowMaps();
	const bool bCubeSharedCascade = UseSceneCaptureCubeSharedCascade(*this, LightSceneInfo);
	const bool bTimesliced = bCubeSharedCascade && Views[0].bIsReflectionCapture && Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame;
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		TArray<float, TInlineAllocator<2> > FadeAlphas;
		if (bCubeSharedCascade)
		{
			// All views sharing the cascade get the same shadow amount
			FadeAlphas.Init(LightShadowAmount, Views.Num());
		}
		else
		{
		...
		// Similarly, for the cube capture shared cascade, we only render shadows with the first view.
		if (IStereoRendering::IsAPrimaryView(View) && (!bCubeSharedCascade || ViewIndex == 0))
		{
			const FDirectionalLightSceneProxy& DirProxy = static_cast<const FDirectionalLightSceneProxy&>(*LightSceneInfo.Proxy);
			const bool bExtraDistanceFieldCascade = LightSceneInfo.ShouldRenderLightViewIndependent()
				&& ShouldCreateRayTracedCascade(DirProxy, View.GetFeatureLevel(), LightSceneInfo.IsPrecomputedLightingValid(), View.MaxShadowCascades);

			const int32 ProjectionCount = GetNumViewDependentWholeSceneShadows(LightSceneInfo, View, LightSceneInfo.IsPrecomputedLightingValid()) + (bExtraDistanceFieldCascade?1:0);
			...
			if (!bNeedsVirtualShadowMap || bVsmUseFarShadowRules)
			{
				for (int32 Index = 0; Index < ProjectionCount; Index++)
				{
					...
					if (GetViewDependentWholeSceneProjectedShadowInitializer(LightSceneInfo, View, LocalIndex, LightSceneInfo.IsPrecomputedLightingValid(), ProjectedShadowInitializer))
					{
						...
						{
							// VSM disables CSM, but DF shadows can still be enabled
						...
						// When depth reuse with separate dynamic is enabled, each cascade may be split
						// into a static-only pass and a dynamic-only pass, allowing different reuse intervals.
						const int32 CascadeIndex = ProjectedShadowInitializer.CascadeSettings.ShadowSplitIndex;
						const bool bSplitStaticAndDynamic = CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0
															&& CVarCSMShadowDepthReuseSeparateDynamic.GetValueOnRenderThread() != 0
															&& ShouldSplitCascadeStaticDynamic(CascadeIndex);
						const int32 BaseDepthReuseIndex = ComputeBaseDepthReuseIndex(CascadeIndex);
						const int32 NumDepthReuseSplits = bSplitStaticAndDynamic ? 2 : 1;

						for (int32 SplitIndex = 0; SplitIndex < NumDepthReuseSplits; SplitIndex++)
						{
							for (int32 CacheModeIndex = 0; CacheModeIndex < NumShadowMaps; CacheModeIndex++)
							{
								// Create the projected shadow info.
								...
								ProjectedShadowInfo->bIsDynamicOnly = (SplitIndex == 1);
								ProjectedShadowInfo->CSMShadowDepthReuseIndex = BaseDepthReuseIndex + SplitIndex;

								// Initialize depth reuse data structures upfront for CSM
								if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0
									&& ProjectedShadowInfo->bDirectionalLight
									&& ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex >= 0)
								{
									FCSMSceneExtension& CSMExt = Scene->GetExtension<FCSMSceneExtension>();
									int32 ShadowDepthReuseIndex = ProjectedShadowInfo->GetCSMShadowDepthReuseIndex();

									// Ensure array exists and is sized for this cascade
									TArray<FCSMDepthReuse>& CSMDepthReuseDatas = CSMExt.GetAllCSMDepthReuseData().FindOrAdd(LightSceneInfo.Id);
									if (CSMDepthReuseDatas.Num() <= ShadowDepthReuseIndex)
									{
										CSMDepthReuseDatas.SetNum(ShadowDepthReuseIndex + 1);
									}
								}
								...
								if (!ProjectedShadowInfo->bRayTracedDistanceField)
								{
									// If shadows are timesliced, we are reusing results from a previous frame, and shouldn't kick off expensive primitive culling work
									if (!bTimesliced)
									{
										ProjectedShadowInfo->SetupCullingQuery(*this);
									...
									{
										FCachedShadowMapData& CachedShadowMapData = Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDataRef(LightSceneInfo.Id, ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex);
						...
						{
							FCSMSceneExtension& CSMExt = Scene->GetExtension<FCSMSceneExtension>();
							InvalidateCachedShadowsOnMaskChange(CSMExt, LightSceneInfo.Id, CVarCSMShadowDepthReuseDynamicCascadeMask.GetValueOnRenderThread());
						}
					}
				}
			}
			...
			if (bNeedsVirtualShadowMap && bDirectionalLight)
			{
				if (ensure(VirtualShadowMapArray.IsEnabled()))
				{
					FShadowSceneRenderer& ShadowSceneRenderer = GetSceneExtensionsRenderers().GetRenderer<FShadowSceneRenderer>();
...
void FSceneRenderer::AllocateShadowDepthTargets(FDynamicShadowsTaskData& TaskData)
{
	...
	SET_MEMORY_STAT(STAT_CachedShadowmapMemory, Scene->GetExtension<FCSMSceneExtension>().GetCachedWholeSceneShadowMapsSize());
	...
	TArray<FSortedShadowMapAtlas,SceneRenderingAllocator>& OutAtlases)
{
	...
	for (int32 ShadowIndex = 0; ShadowIndex < Shadows.Num(); ShadowIndex++)
	{
		...
		{
			const FCachedShadowMapData& CachedShadowMapData = Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDataRef(ProjectedShadowInfo->GetLightSceneInfo().Id);
...
void FSceneRenderer::AllocateCachedShadowDepthTargets(FRHICommandListBase& RHICmdList, TConstArrayView<FProjectedShadowInfo*> CachedShadows)
{
	...
	for (int32 ShadowIndex = 0; ShadowIndex < CachedShadows.Num(); ShadowIndex++)
	{
		...
		FCachedShadowMapData& CachedShadowMapData = CSMShadowScene.GetCachedShadowMapDataRef(ProjectedShadowInfo->GetLightSceneInfo().Id, FMath::Max(ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex, 0));
	...
	)
{
	if (WholeSceneDirectionalShadows.Num() > 0)
	{
		...
		for (int32 LayoutIndex = 0; LayoutIndex < Layouts.Num(); LayoutIndex++)
		{
			...
			// Try to reuse last frame's atlas if amortization is enabled
			bool bReusedAtlas = false;
			if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0 && CurrentLayout.Shadows.Num() > 0)
			{
				FCSMSceneExtension& CSMExt = Scene->GetExtension<FCSMSceneExtension>();
				for (FProjectedShadowInfo* ProjectedShadowInfo : CurrentLayout.Shadows)
				{
					...
					{
						int32 ShadowDepthReuseIndex = ProjectedShadowInfo->GetCSMShadowDepthReuseIndex();
						const FCSMDepthReuse* CSMDepthReuse = CSMExt.GetCSMDepthReuse(ProjectedShadowInfo->GetLightSceneInfo().Id, ShadowDepthReuseIndex);

						if (CSMDepthReuse != nullptr && CSMDepthReuse->PreviousAtlas.IsValid()
							&& CSMDepthReuse->PreviousAtlas.DepthTarget != nullptr && CSMDepthReuse->PreviousAtlas.DepthTarget->GetDesc().Extent == WholeSceneAtlasSize)
						{
							ShadowMapAtlas.RenderTargets = CSMDepthReuse->PreviousAtlas;
							bReusedAtlas = true;
							break;
						}
					}
...
void FSceneRenderer::AllocateOnePassPointLightDepthTargets(FRHICommandListBase& RHICmdList, TConstArrayView<FProjectedShadowInfo*> WholeScenePointShadows)
{
	if (FeatureLevel >= ERHIFeatureLevel::SM5 || DoesRuntimeSupportOnePassPointLightShadows(ShaderPlatform))
	{
		...
		for (int32 ShadowIndex = 0; ShadowIndex < WholeScenePointShadows.Num(); ShadowIndex++)
		{
			...
			if (ProjectedShadowInfo->CacheMode == SDCM_MovablePrimitivesOnly && !ProjectedShadowInfo->HasSubjectPrims())
			{
				const FCachedShadowMapData& CachedShadowMapData = CSMShadowScene.GetCachedShadowMapDataRef(ProjectedShadowInfo->GetLightSceneInfo().Id);
				ProjectedShadowInfo->X = ProjectedShadowInfo->Y = 0;
				ProjectedShadowInfo->bAllocated = true;
				// Skip the shadow depth pass since there are no movable primitives to composite, project from the cached shadowmap directly which contains static primitive depths
			...
			else
			{
				...
				{
					FCachedShadowMapData& CachedShadowMapData = CSMShadowScene.GetCachedShadowMapDataRef(ProjectedShadowInfo->GetLightSceneInfo().Id);
...
void FSceneRenderer::CreateDynamicShadows(FDynamicShadowsTaskData& TaskData)
{
	...
	const bool bMobileInsetShadows = CVarMobileInsetShadows.GetValueOnRenderThread();
	const bool bCapsuleDirectShadows = IsCapsuleDirectShadowsEnabled(ShaderPlatform);
...

	{
		...
		if (GetShadowQuality() > 0)
		{
			...
			const auto GetCachedShadowMapsSize = [&]
			{
				if (CachedShadowMapsSize < 0)
				{
					...
					CachedShadowMapsSize = Scene->GetExtension<FCSMSceneExtension>().GetCachedWholeSceneShadowMapsSize();
				}
			...
			for (const FLightSceneInfoCompact& LightSceneInfoCompact : Scene->Lights)
			{
				...
				const ELightOcclusionType OcclusionType = GetLightOcclusionType(LightSceneInfoCompact, ViewFamily);

				// Register visible lights for allowing hair strands to cast shadow (non-directional light)
				if (bHairStrands && OcclusionType == ELightOcclusionType::Raytraced && LightSceneInfo->Proxy->GetLightType() != LightType_Directional)
				{
				...
				if (OcclusionType != ELightOcclusionType::Shadowmap &&
					OcclusionType != ELightOcclusionType::MegaLightsVSM)
				{
					continue;
				}
				...
				if (LightSceneInfoCompact.bCastStaticShadow || LightSceneInfoCompact.bCastDynamicShadow)
				{
					...
					if (bIsVisibleInAnyView)
					{
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
						bUsedPointLightWholeSceneShadow = bUsedPointLightWholeSceneShadow || ((bShouldCreateShadowForMovableLight || bShouldCreateShadowForOverflowStaticShadowing || bShouldCreateShadowToPreviewStaticLight) && bPointLightShadow);
#endif
						...
						if ((!LightSceneInfo->Proxy->HasStaticLighting() && LightSceneInfoCompact.bCastDynamicShadow) || bCreateShadowToPreviewStaticLight)
						{
							...
							if (!bMobile || bMobileInsetShadows || bCapsuleDirectShadows || (LightSceneInfo->Proxy->CastsModulatedShadows() && !LightSceneInfo->Proxy->UseCSMForDynamicObjects() && LightSceneInfo->Proxy->HasStaticShadowing()))
							{
								if (!LightSceneInfo->Proxy->UseVirtualShadowMaps())
								{
		...
		Scene->GetExtension<FCSMSceneExtension>().RemoveExpiredCacheEntries(ViewFamily.Time.GetRealTimeSeconds());
...
void WaitForFilterDynamicShadowsTask(FDynamicShadowsTaskData* TaskData)
{
	if (TaskData)
	{
		TaskData->FilterDynamicShadowsTask.Wait();
...
void FSceneRenderer::FilterDynamicShadows(FDynamicShadowsTaskData& TaskData)
{
	...
	// Whether to merge spot light shadows into directional light atlas
	const bool bMergeMobileDynamicSpotlightShadows = bMobile && !IsMobileDeferredShadingEnabled(ShaderPlatform);
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	auto MarkUnsupportedOnePassShadow = [&ShadowArrays](FLightSceneId LightId)
	{
		ShadowArrays.OnePassShadowUnsupportedLights.PadToId(LightId, false);
		ShadowArrays.OnePassShadowUnsupportedLights[LightId] = true;
	};
#else 
	auto MarkUnsupportedOnePassShadow = [](FLightSceneId LightId) {};
#endif
	...
	for (const FLightSceneInfoCompact& LightSceneInfoCompact : Scene->Lights)		
	{
		...
		for (int32 ShadowIndex = 0; ShadowIndex < VisibleLightInfo.AllProjectedShadows.Num(); ShadowIndex++)
		{
			...
			{
				const FCachedShadowMapData& CachedShadowMapData = Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDataRef(ProjectedShadowInfo->GetLightSceneInfo().Id, FMath::Max(ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex, 0));
			...
			// Shadows must be disabled if timesliced shadows are enabled, but no timeslice is active for the given shadow map.  This
			// generally happens in the editor where a shadow caster is modified in a way that forces the render proxy to be recreated
			// or destroyed.
			if (IsMobilePlatform(ShaderPlatform) &&
				Views[0].IsRuntimeReflectionCapture() &&
				ProjectedShadowInfo->IsWholeSceneDirectionalShadow() &&
				Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame &&
				Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame != LightSceneInfo->Proxy->TimesliceAtlasFrame)
			{
				bShadowIsVisible = false;
				Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFailed = true;
			}
			...
			if (bShadowIsVisible)
			{
				...
				if (bNeedsShadowmapSetup)
				{
					...
					else if (bMergeMobileDynamicSpotlightShadows && ProjectedShadowInfo->bWholeSceneShadow)
					{
...
void FSceneRenderer::AllocateMobileCSMAndSpotLightShadowDepthTargets(FRHICommandListBase& RHICmdList, TConstArrayView<FProjectedShadowInfo*> MobileCSMAndSpotLightShadows)
{
	...
	{
		// Decide if we want to generate or reuse a timesliced shadow atlas layout.  There is no need to timeslice if all 6 views are being rendered at once.
		bool bRuntimeReflectionTimeslice = false;
		bool bRuntimeReflectionTimesliceFirst = false;
		if (Views[0].IsRuntimeReflectionCapture() && Views.Num() < 6)
		{
			// Check if this the first render of a timeslice.  It should include the first view, and either be a render of multiple views or a render
			// with FaceCount == 0, indicating it's a standalone shadow render occurring before the faces are rendered.
			if (Views[0].UnscaledViewRect.Min.X == 0 && Views[0].UnscaledViewRect.Min.Y == 0 && (Views.Num() > 1 || Scene->ReflectionSceneData.RuntimeCaptureTimesliceFaceCount_RT == 0))
			{
				// Check the CVar for the first view.  Track the atlas frame if so.
				bRuntimeReflectionTimeslice = IsSceneCaptureCubeShareCascadesEnabled(ShaderPlatform);
				if (bRuntimeReflectionTimeslice)
				{
					bRuntimeReflectionTimesliceFirst = true;
				}
			}
			else
			{
				// For subsequent views, check whether the timesliced atlas was generated with the first view.  TimesliceAtlasFrame is cleared to
				// zero before a new runtime reflection capture is kicked off.
				bRuntimeReflectionTimeslice = Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame != 0;
			}
		}
		...
		for (int32 ShadowIndex = 0; ShadowIndex < MobileCSMAndSpotLightShadows.Num(); ShadowIndex++)
		{
			...
			FLightSceneProxy* LightSceneProxy = ProjectedShadowInfo->GetLightSceneInfo().Proxy;

			if (bRuntimeReflectionTimeslice && !bRuntimeReflectionTimesliceFirst)
			{
				// We should have filtered out the shadow as non-renderable earlier if timeslicing is enabled, but no valid atlas location is available
				check(LightSceneProxy->TimesliceAtlasFrame == Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame);
			
				const FReflectionEnvironmentCachedShadow& CachedShadow = Scene->ReflectionSceneData.RuntimeCaptureShadows.Shadows[LightSceneProxy->TimesliceAtlasIndex[ProjectedShadowInfo->ProjectionIndex]];
				ProjectedShadowInfo->X = CachedShadow.X;
				ProjectedShadowInfo->Y = CachedShadow.Y;

				ProjectedShadowInfo->bAllocated = true;
				MobileCSMAndSpotLightShadowLayout.Shadows.Add(ProjectedShadowInfo);
			}
			else if (MobileCSMAndSpotLightShadowLayout.TextureLayout.AddElement(
				ProjectedShadowInfo->X,
				ProjectedShadowInfo->Y,
				ProjectedShadowInfo->ResolutionX + ProjectedShadowInfo->BorderSize * 2,
				ProjectedShadowInfo->ResolutionY + ProjectedShadowInfo->BorderSize * 2)
				)
			{
				...
				// Save off the cached atlas location and frame
				if (bRuntimeReflectionTimeslice)
				{
					int32 TimesliceAtlasIndex = Scene->ReflectionSceneData.RuntimeCaptureShadows.Shadows.AddDefaulted();
					LightSceneProxy->TimesliceAtlasIndex[ProjectedShadowInfo->ProjectionIndex] = TimesliceAtlasIndex;
					LightSceneProxy->TimesliceAtlasFrame = GFrameNumberRenderThread;

					FReflectionEnvironmentCachedShadow& CachedShadow = Scene->ReflectionSceneData.RuntimeCaptureShadows.Shadows[TimesliceAtlasIndex];
					CachedShadow.X = ProjectedShadowInfo->X;
					CachedShadow.Y = ProjectedShadowInfo->Y;
					CachedShadow.Position = LightSceneProxy->Position;
					CachedShadow.WorldToLight = LightSceneProxy->WorldToLight;
					CachedShadow.LightToWorld = LightSceneProxy->LightToWorld;
				}
			}
		}

		FIntPoint WholeSceneAtlasSize = (bRuntimeReflectionTimeslice && !bRuntimeReflectionTimesliceFirst) ?
			Scene->ReflectionSceneData.RuntimeCaptureShadows.AtlasSize :
			FIntPoint(MobileCSMAndSpotLightShadowLayout.TextureLayout.GetSizeX(), MobileCSMAndSpotLightShadowLayout.TextureLayout.GetSizeY());

		if (WholeSceneAtlasSize.X > 0)
		{
			SortedShadowsForShadowDepthPass.ShadowMapAtlases.AddDefaulted();
			...
			if (bRuntimeReflectionTimeslice && !bRuntimeReflectionTimesliceFirst)
			{
				// Reuse timesliced atlas rendered on a previous frame
				ShadowMapAtlas.RenderTargets.DepthTarget = Scene->ReflectionSceneData.RuntimeCaptureShadows.AtlasRenderTarget;
			}
			else
			{
				FPooledRenderTargetDesc WholeSceneShadowMapDesc2D(FPooledRenderTargetDesc::Create2DDesc(WholeSceneAtlasSize, PF_ShadowDepth, FClearValueBinding::DepthOne, TexCreate_None, TexCreate_DepthStencilTargetable | TexCreate_ShaderResource, false));
				...
				// Try to reuse last frame's atlas if amortization is enabled
				bool bReusedMobileAtlas = false;
				if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0 && MobileCSMAndSpotLightShadowLayout.Shadows.Num() > 0)
				{
					FCSMSceneExtension& CSMExt = Scene->GetExtension<FCSMSceneExtension>();
					for (FProjectedShadowInfo* ProjectedShadowInfo : MobileCSMAndSpotLightShadowLayout.Shadows)
					{
						if (ProjectedShadowInfo->bDirectionalLight)
						{
							int32 ShadowDepthReuseIndex = ProjectedShadowInfo->GetCSMShadowDepthReuseIndex();
				...
				if (!bReusedMobileAtlas)
				{
					GRenderTargetPool.FindFreeElement(RHICmdList, WholeSceneShadowMapDesc2D, ShadowMapAtlas.RenderTargets.DepthTarget,
						bRuntimeReflectionTimeslice ? TEXT("RuntimeCaptureShadowAtlas") : TEXT("MobileCSMAndSpotLightShadowmap"));
				}

				// Store off timesliced atlas information for subsequent frames
				if (bRuntimeReflectionTimeslice)
				{
					Scene->ReflectionSceneData.RuntimeCaptureShadows.AtlasRenderTarget = ShadowMapAtlas.RenderTargets.DepthTarget;
					Scene->ReflectionSceneData.RuntimeCaptureShadows.AtlasSize = WholeSceneAtlasSize;
					Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame = GFrameNumberRenderThread;
				}
			}
