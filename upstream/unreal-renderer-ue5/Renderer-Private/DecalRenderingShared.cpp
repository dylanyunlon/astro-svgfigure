...
{
	// Allocate decal visibility task data if any view needs decals
	for (FViewInfo* View : Views)
	{
		if (AreDecalsEnabled(*View) && !HasRayTracedOverlay(*View->Family))
		{
			return GraphBuilder.AllocObject<FDecalVisibilityTaskData>(Scene, Views);
		}
	}
	return nullptr;
}

FDecalVisibilityTaskData::FDecalVisibilityTaskData(const FScene& Scene, TConstArrayView<FViewInfo*> Views)
{
	...
	, View(InView)
{
	...
	EShaderPlatform ShaderPlatform = View.Family->GetShaderPlatform();
	const bool bDBufferEnabled = ::IsDBufferEnabled(*View.Family, ShaderPlatform);
	const bool bGBufferEnabled = IsUsingGBuffers(ShaderPlatform);
	...
	void SetParameters(FRHIBatchedShaderParameters& BatchedParameters, const FViewInfo& View, const FVisibleDecal& VisibleDecal, const FMaterialRenderProxy* MaterialProxy, const FMaterial* MaterialResource, const float FadeAlphaValue = 1.0f, const FScene* Scene = nullptr)
	{
		...
		if(SvPositionToDecal.IsBound())
		{
			...
			FMatrix44f SvPositionToDecalValue = FMatrix44f(										// LWC_TODO: Precision loss
				SvPositionToDecalBase * View.ViewMatrices.GetClipToWorld() * WorldToDecalMatrix);
			...
			if (RightEyeSvPositionToDecal.IsBound())
			{
				...
				if (InstancedView)
				{
					FMatrix44f RightEyeSvPositionToDecalValue = FMatrix44f(										// LWC_TODO: Precision loss
						SvPositionToDecalBase * InstancedView->ViewMatrices.GetClipToWorld() * WorldToDecalMatrix);
...
namespace DecalRendering
{
	...
	float CalculateDecalFadeAlpha(float DecalFadeScreenSize, const FMatrix& ComponentToWorldMatrix, const FViewInfo& View, float FadeMultiplier)
	{
		...
		float FOVFactor = ((2.0f / View.ViewMatrices.GetViewToClip().M[0][0]) / View.ViewRect.Width()) * FadeCoeffScale;
	...
	FMatrix ComputeComponentToClipMatrix(const FViewInfo& View, const FMatrix& DecalComponentToWorld)
	{
		...
		else
		{
			...
			return ComponentToWorldMatrixTrans * View.ViewMatrices.GetTranslatedWorldToClip();
		}
	}
		...
		TShaderRef<FShader>& OutPixelShader)
	{
		...
		TShaderMapRef<FDeferredDecalVS> VertexShader(GetGlobalShaderMap(Material.GetShaderPlatform()));
		...
		bool bRequired)
	{
		...
		FRHIPixelShader* PixelShaderRHI = PixelShader.GetPixelShader(bRequired);
		...
		OutBoundShaderState.VertexShaderRHI = VertexShaderRHI;
		OutBoundShaderState.PixelShaderRHI = PixelShaderRHI;
