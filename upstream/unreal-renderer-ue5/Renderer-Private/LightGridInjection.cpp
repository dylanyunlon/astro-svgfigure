...
#include "HairStrands/HairStrandsTransmittance.h"
...
static int32 GLightBufferMode = (int32)ELightBufferMode::VisibleLights;
...
// Experimental version of FLightGridInjectionCS that uses specialization constants
static TAutoConsoleVariable<int32> CVarLightGridInjectionUseSpecializationConstants(
	TEXT("r.LightGridInjection.UseSpecializationConstants"),
	1,
	TEXT("Experimental: Selects whether LightGridInjection should use specialization constant. (default: 1)"),
	ECVF_RenderThreadSafe);
...
void SetupDummyForwardLightUniformParameters(FRDGBuilder& GraphBuilder, FForwardLightUniformParameters& ForwardLightUniformParameters, EShaderPlatform ShaderPlatform)
{
	...
	ForwardLightUniformParameters.DirectionalLightLightFunctionAtlasLightIndex = 0;

	ForwardLightUniformParameters.DirectionalLightAffectsTranslucentLighting = 0;
	...
	FRDGBufferRef LightNamesBuffer = GSystemTextures.GetDefaultByteAddressBuffer(GraphBuilder, sizeof(uint32));
	ForwardLightUniformParameters.LightNamesBuffer = GraphBuilder.CreateSRV(LightNamesBuffer);
	ForwardLightUniformParameters.LightNameDwords = 0;
}

void SetupDummyForwardDirLightShadowParameters(FRDGBuilder& GraphBuilder, FForwardDirectionalLightShadowMapParameters& ShadowParameters)
{
	const FRDGSystemTextures& SystemTextures = FRDGSystemTextures::Get(GraphBuilder);
	ShadowParameters.DirectionalLightShadowmapAtlas = SystemTextures.Black;
	ShadowParameters.DirectionalLightStaticShadowmap = GBlackTexture->TextureRHI;
}
...
TRDGUniformBufferRef<FForwardDirectionalLightShadowMapParameters> CreateDummyForwardDirLightShadowUniformBuffer(FRDGBuilder& GraphBuilder)
{
	FForwardDirectionalLightShadowMapParameters* ShadowParameters = GraphBuilder.AllocParameters<FForwardDirectionalLightShadowMapParameters>();
	SetupDummyForwardDirLightShadowParameters(GraphBuilder, *ShadowParameters);
	return GraphBuilder.CreateUniformBuffer(ShadowParameters);
}
...
void SetDummyForwardLightUniformBufferOnViews(FRDGBuilder& GraphBuilder, EShaderPlatform ShaderPlatform, TArray<FViewInfo>& Views)
{
	...
	TRDGUniformBufferRef<FForwardDirectionalLightShadowMapParameters> ForwardDirLightShadowUniformBuffer = CreateDummyForwardDirLightShadowUniformBuffer(GraphBuilder);
	for (auto& View : Views)
	{
		View.ForwardLightingResources.SetUniformBuffer(ForwardLightUniformBuffer);
		View.ForwardLightingResources.SetShadowUniformBuffer(ForwardDirLightShadowUniformBuffer);
	}
}

void SetForwardLightUniformParametersFromView(FRDGBuilder& GraphBuilder, const FViewInfo& View, FForwardLightUniformParameters& ForwardLightUniformParameters)
{
	if (View.ForwardLightingResources.ForwardLightUniformParameters)
	{
		ForwardLightUniformParameters = *View.ForwardLightingResources.ForwardLightUniformParameters;
	}
	else
	{
		SetupDummyForwardLightUniformParameters(GraphBuilder, ForwardLightUniformParameters, View.GetShaderPlatform());
	}
}

void SetForwardDirLightShadowParametersFromView(FRDGBuilder& GraphBuilder, const FViewInfo& View, FForwardDirectionalLightShadowMapParameters& Parameters)
{
	if (View.ForwardLightingResources.ForwardDirLightShadowParameters)
	{
		Parameters = *View.ForwardLightingResources.ForwardDirLightShadowParameters;
	}
	else
	{
		SetupDummyForwardDirLightShadowParameters(GraphBuilder, Parameters);
	}
}
...
FForwardDirectionalLightShadowMapParameters::FForwardDirectionalLightShadowMapParameters()
{
	FMemory::Memzero(*this);
...
IMPLEMENT_SCENE_EXTENSION(FLightGridSceneExtension);

class FLightGridSceneExtensionUpdater : public ISceneExtensionUpdater
{
	DECLARE_SCENE_EXTENSION_UPDATER(FLightGridSceneExtensionUpdater, FLightGridSceneExtension);

public:
	FLightGridSceneExtensionUpdater(FLightGridSceneExtension& InSceneExtension) : SceneExtension(InSceneExtension) {}
...
private:
	FLightGridSceneExtension& SceneExtension;

	const FScenePostUpdateChangeSet* PostUpdateChangeSet = nullptr;
};

void FLightGridSceneExtensionUpdater::PreLightsUpdate(FRDGBuilder& GraphBuilder, const FLightSceneChangeSet& LightSceneChangeSet)
{
	if (LightSceneChangeSet.RemovedLightIds.Num() > 0)
	{
		for (FLightGridSceneExtension::FViewData& ViewData : SceneExtension.ViewDatas)
		{
			for (FLightSceneId Id : LightSceneChangeSet.RemovedLightIds)
			{
				if (ViewData.LightSceneIdToForwardLightIndex.IsValidIndex(Id.GetIndex()))
				{
					ViewData.LightSceneIdToForwardLightIndex[Id.GetIndex()] = INDEX_NONE;
				}
			}
		}
	}
...
void FLightGridSceneExtensionUpdater::PreSceneUpdate(FRDGBuilder& GraphBuilder, const FScenePreUpdateChangeSet& ChangeSet)
{
	if (ChangeSet.ViewUpdateChangeSet)
	{
		for (FPersistentViewId ViewId : ChangeSet.ViewUpdateChangeSet->RemovedViewIds)
		{
			check(SceneExtension.ViewDatas.IsAllocated(ViewId.Index));
			SceneExtension.ViewDatas.RemoveAt(ViewId.Index);
		}
	}
}

void FLightGridSceneExtensionUpdater::PostSceneUpdate(FRDGBuilder& GraphBuilder, const FScenePostUpdateChangeSet& ChangeSet)
{
	if (ChangeSet.ViewUpdateChangeSet)
	{
		for (FPersistentViewId ViewId : ChangeSet.ViewUpdateChangeSet->AddedViewIds)
		{
			if (!SceneExtension.ViewDatas.IsValidIndex(ViewId.Index))
			{
				SceneExtension.ViewDatas.EmplaceAt(ViewId.Index);
			}
		}
	}
}

ISceneExtensionUpdater* FLightGridSceneExtension::CreateUpdater()
{
	return new FLightGridSceneExtensionUpdater(*this);
}

FLightGridSceneExtension::FViewData* FLightGridSceneExtension::GetViewData(const FViewInfo& View)
{
	if (View.PersistentViewId.IsValid())
	{
		check(ViewDatas.IsAllocated(View.PersistentViewId.Index));
		return &ViewDatas[View.PersistentViewId.Index];
	}
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<float4>, LightViewSpaceWeightEstimate)
	...
	{
		// Do not precache if not in use
		if (UseSpecializationConstants(Parameters.Platform))
		{
...
// Experimental version of FLightGridInjectionCS that uses specialization constants
class FLightGridInjectionSpecConstCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FLightGridInjectionSpecConstCS);
	SHADER_USE_PARAMETER_STRUCT(FLightGridInjectionSpecConstCS, FGlobalShader)
public:
	class FUseLinkedList : SHADER_SPECIALIZATION_BOOL("USE_LINKED_CULL_LIST");
	class FRefineRectLightBounds : SHADER_SPECIALIZATION_BOOL("REFINE_RECTLIGHT_BOUNDS");
	class FUseHZBCull : SHADER_SPECIALIZATION_BOOL("USE_HZB_CULL");
	class FUseParentLightGrid : SHADER_SPECIALIZATION_BOOL("USE_PARENT_LIGHT_GRID");
	class FUseThreadGroupPerCell : SHADER_SPECIALIZATION_BOOL("USE_THREAD_GROUP_PER_CELL");
	class FUseThreadGroupSize32 : SHADER_PERMUTATION_BOOL("USE_THREAD_GROUP_SIZE_32");
	class FApplyIndirection : SHADER_SPECIALIZATION_BOOL("APPLY_INDIRECTION");
	using FPermutationDomain = TShaderPermutationDomain<FUseLinkedList, FRefineRectLightBounds, FUseHZBCull, FUseParentLightGrid, FUseThreadGroupPerCell, FUseThreadGroupSize32, FApplyIndirection>;

	using FParameters = FLightGridInjectionCS::FParameters;

	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		return FDataDrivenShaderPlatformInfo::GetSupportsSpecializationConstants(Parameters.Platform);
	}
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		const FIntVector GroupSize = GetGroupSize(PermutationVector);
...
IMPLEMENT_GLOBAL_SHADER(FLightGridInjectionSpecConstCS, "/Engine/Private/LightGridInjection.usf", "LightGridInjectionCS", SF_Compute);
	...
	const float RayEndBias)
{
	...
	const FVector2f LightColorPacked = PackLightColor(FVector3f(SimpleLightColor));
...
FLightGrid LightGridInjection(
	...
	FRDGBufferSRVRef LightViewSpaceWeightEstimateSRV,
	...
	uint32 ParentGridSizeFactor)
{
	...
	PassParameters->MinSampleWeightEstimate = MegaLights::GetMinSampleWeightEstimate();
	...
	PassParameters->LightViewSpaceWeightEstimate = LightViewSpaceWeightEstimateSRV;
	...
	if (UseSpecializationConstants(View.GetShaderPlatform()))
	{
		FLightGridInjectionSpecConstCS::FPermutationDomain PermutationVector;
		PermutationVector.Set<FLightGridInjectionSpecConstCS::FUseLinkedList>(GLightLinkedListCulling != 0);
		PermutationVector.Set<FLightGridInjectionSpecConstCS::FRefineRectLightBounds>(bRefineRectLightBounds);
		PermutationVector.Set<FLightGridInjectionSpecConstCS::FUseHZBCull>(GLightGridHZBCull != 0 && bIsHZBValid);
		PermutationVector.Set<FLightGridInjectionSpecConstCS::FUseParentLightGrid>(ParentNumCulledLightsGridSRV != nullptr && ParentCulledLightDataGridSRV != nullptr);
		PermutationVector.Set<FLightGridInjectionSpecConstCS::FUseThreadGroupPerCell>(bThreadGroupPerCell);
		PermutationVector.Set<FLightGridInjectionSpecConstCS::FUseThreadGroupSize32>(bThreadGroupSize32);
		PermutationVector.Set<FLightGridInjectionSpecConstCS::FApplyIndirection>(IndirectionIndicesSRV != nullptr);
		auto ComputeShader = View.ShaderMap->GetShader<FLightGridInjectionSpecConstCS>(PermutationVector);

		const FIntVector NumGroups = bThreadGroupPerCell ? GridSize : FComputeShaderUtils::GetGroupCount(GridSize, FLightGridInjectionSpecConstCS::GetGroupSize(PermutationVector));

		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("LightGridInject SpecConsts %s %s %s %s %s",
				PermutationVector.Get<FLightGridInjectionSpecConstCS::FUseLinkedList>() ? TEXT("LinkedList") : TEXT("NoLinkedList"),
				PermutationVector.Get<FLightGridInjectionSpecConstCS::FRefineRectLightBounds>() ? TEXT("RefineRectLightBounds") : TEXT(""),
				PermutationVector.Get<FLightGridInjectionSpecConstCS::FUseHZBCull>() ? TEXT("UseHZBCull") : TEXT(""),
				PermutationVector.Get<FLightGridInjectionSpecConstCS::FUseParentLightGrid>() ? TEXT("UseParentLightGrid") : TEXT(""),
				PermutationVector.Get<FLightGridInjectionSpecConstCS::FUseThreadGroupPerCell>() ? TEXT("ThreadGroup") : TEXT("SingleThread")),
			RDGPassFlags,
			ComputeShader,
			PassParameters,
			NumGroups);
	...
	else
	{
		...
		const FIntVector NumGroups = bThreadGroupPerCell ? GridSize : FComputeShaderUtils::GetGroupCount(GridSize, FLightGridInjectionCS::GetGroupSize(PermutationVector));

		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("LightGridInject %s %s %s %s %s",
				PermutationVector.Get<FLightGridInjectionCS::FUseLinkedList>() ? TEXT("LinkedList") : TEXT("NoLinkedList"),
				PermutationVector.Get<FLightGridInjectionCS::FRefineRectLightBounds>() ? TEXT("RefineRectLightBounds") : TEXT(""),
				PermutationVector.Get<FLightGridInjectionCS::FUseHZBCull>() ? TEXT("UseHZBCull") : TEXT(""),
				PermutationVector.Get<FLightGridInjectionCS::FUseParentLightGrid>() ? TEXT("UseParentLightGrid") : TEXT(""),
...
FLinearColor GetSimpleLightColor(const FViewInfo& View, const FSimpleLightEntry& SimpleLight)
{
	return SimpleLight.Color * FLightRenderParameters::GetLightExposureScale(View.GetLastEyeAdaptationExposure(), SimpleLight.InverseExposureBlend);
}

FComputeLightGridOutput FSceneRenderer::ComputeLightGrid(FRDGBuilder& GraphBuilder, bool bCullLightsToGrid, const FSortedLightSetSceneInfo& SortedLightSet, FComputeLightGridArray<FForwardLightUniformParameters*>& PerViewForwardLightUniformParameters, FComputeLightGridArray<FForwardDirectionalLightShadowMapParameters*>& PerViewForwardDirLightShadowParameters)
{
	...
	FLightGridSceneExtension& LightGridExtension = Scene->GetExtension<FLightGridSceneExtension>();

	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		FViewInfo& View = Views[ViewIndex];

		FLightGridSceneExtension::FViewData* LightGridViewData = LightGridExtension.GetViewData(View);
		...
		FForwardDirectionalLightShadowMapParameters* DirLightShadowParameters = PerViewForwardDirLightShadowParameters[ViewIndex];
		DirLightShadowParameters->DirectionalLightShadowmapAtlas = SystemTextures.Black;
		DirLightShadowParameters->DirectionalLightStaticShadowmap = GBlackTexture->TextureRHI;
		...
		TArray<FVector4f, SceneRenderingAllocator> ViewSpaceWeightEstimateData;

		TArray<int32, SceneRenderingAllocator> IndirectionIndices;

		TArray<uint8, SceneRenderingAllocator> LightNames;
		const bool bNeedsLightNames = MegaLights::IsUsingLightNames(View);
		const uint32 LightNameDwords = 16;
		const uint32 LightNameBytes = LightNameDwords * sizeof(uint32);
		ForwardLightUniformParameters->LightNameDwords = LightNameDwords;

		TArray<VisualizeLightShape::FSortKey> VisualizeLightShapeSortKeys;
		const bool bVisualizeLightShape = VisualizeLightShape::UpdateLastToggleTime(View);
		...
		uint8 LightGridLightingChannelAndMask = 0xFF;
		...
		if (bCullLightsToGrid)
		{
			if (GLightBufferMode == (int32)ELightBufferMode::VisibleLightsStableIndices)
			{
				...
				if (bNeedsLightNames)
				{
					LightNames.AddUninitialized(FMath::Max(1, MaxLightId) * LightNameBytes);
				}
			}
			...
			if (SimpleLightsEnd > 0)
			{
				...
				ViewSpaceWeightEstimateData.AddZeroed(SimpleLightsEnd);

				if (bVisualizeLightShape)
				{
					VisualizeLightShapeSortKeys.AddUninitialized(SimpleLightsEnd);
				}
			}
			...
			if (LightGridViewData)
			{
				NewSimpleLightIdToForwardLightIndex.Reserve(LightGridViewData->SimpleLightIdToForwardLightIndex.Num());
			}
			...
			for (int32 SortedIndex = SimpleLightsEnd; SortedIndex < SortedLights.Num(); ++SortedIndex)
			{
				...
				if (!SortedLightInfo.SortKey.Fields.bIsNotSimpleLight)
				{
					...
					if (SortedIndex == SortedLightSet.MegaLightsLightStart)
					{
						...
						ViewSpaceRectPlanesData.Reserve(ViewSpaceRectPlanesData.Num() + NumMegaSimpleLights * NUM_PLANES_PER_RECT_LIGHT);
						ViewSpaceWeightEstimateData.Reserve(ViewSpaceWeightEstimateData.Num() + NumMegaSimpleLights);
						IndirectionIndices.Reserve(IndirectionIndices.Num() + NumMegaSimpleLights);

						if (bVisualizeLightShape)
						{
							VisualizeLightShapeSortKeys.Reserve(VisualizeLightShapeSortKeys.Num() + NumMegaSimpleLights);
						}

						if (bNeedsLightNames)
						{
							LightNames.Reserve(LightNames.Num() + NumMegaSimpleLights * LightNameBytes);
						}
					}
					...
					if (LightGridViewData && SimpleLight.LightId.IsValid())
					{
						PrevForwardLightIndex = LightGridViewData->SimpleLightIdToForwardLightIndex.FindRef(SimpleLight.LightId, INDEX_NONE);
						NewSimpleLightIdToForwardLightIndex.Add(SimpleLight.LightId, IndexInBuffer);
					}

					const FLinearColor SimpleLightColor = GetSimpleLightColor(View, SimpleLight);
					PackLightData(LightData, View, SimpleLight, SimpleLightPerViewData, SimpleLightColor, PrevForwardLightIndex, true /*bHandledByMegaLights*/, GlobalRayEndBias);

					const FVector4f ViewSpacePosAndRadius(FVector4f(View.ViewMatrices.GetWorldToView().TransformPosition(SimpleLightPerViewData.Position)), SimpleLight.Radius);
					...
					FVector4f WeightEstimateData;
					WeightEstimateData.X = SimpleLightColor.GetLuminance() * FMath::Max(SimpleLight.DiffuseScale, SimpleLight.SpecularScale);
					WeightEstimateData.Y = SimpleLight.Exponent;
					WeightEstimateData.Z = 0.0f;
					WeightEstimateData.W = 0.0f;
					ViewSpaceWeightEstimateData.Add(WeightEstimateData);

					IndirectionIndices.Add(IndexInBuffer);

					if (bVisualizeLightShape)
					{
						VisualizeLightShape::FSortKey& SortKey = VisualizeLightShapeSortKeys[VisualizeLightShapeSortKeys.AddUninitialized()];
						SortKey.LightData.ForwardLightIndex = IndexInBuffer;
						SortKey.LightData.LightType = LightType_Point;
						SortKey.Distance = FVector3f(ViewSpacePosAndRadius).SizeSquared();
					}

					if (bNeedsLightNames)
					{
						const int32 FirstCharIndex = LightNames.AddUninitialized(LightNameBytes);
						uint8* Dest = &LightNames[FirstCharIndex];
						const FAnsiString LightName = FAnsiString::Printf("Simple_%d_%d", SimpleLight.LightId.SourcePrimitiveId, SimpleLight.LightId.LocalId);
						const int32 CharsToCopy = FMath::Min<int32>(LightName.Len(), LightNameBytes - 1);

						FMemory::Memcpy(Dest, *LightName, CharsToCopy);
						Dest[CharsToCopy] = 0;
					}
					...
					(AssociatedPrimaryView && LightSceneInfo->ShouldRenderLight(*AssociatedPrimaryView)))
				{
					...
					int32 IndexInBuffer = INDEX_NONE;
					...
					{
						IndexInBuffer = GLightBufferMode == (int32)ELightBufferMode::VisibleLightsStableIndices ? LightSceneInfo->Id.GetIndex() : ForwardLightData.AddUninitialized(1);
						FForwardLightData& LightData = ForwardLightData[IndexInBuffer];
						...
						if (LightGridViewData)
						{
							...
							{
								PrevForwardLightIndex = LightGridViewData->LightSceneIdToForwardLightIndex[LightSceneInfo->Id.GetIndex()];
							}
							else
							{
								PrevForwardLightIndex = INDEX_NONE;

								const int32 OldSize = LightGridViewData->LightSceneIdToForwardLightIndex.Num();
								LightGridViewData->LightSceneIdToForwardLightIndex.SetNumUninitialized(LightSceneInfo->Id.GetIndex() + 1);

								for (int32 Index = OldSize; Index < LightGridViewData->LightSceneIdToForwardLightIndex.Num(); ++Index)
								{
									LightGridViewData->LightSceneIdToForwardLightIndex[Index] = INDEX_NONE;
								}
							}
							LightGridViewData->LightSceneIdToForwardLightIndex[LightSceneInfo->Id.GetIndex()] = IndexInBuffer;
						}
						...
						if (bNeedsLightNames)
						{
							const int32 FirstCharIndex = GLightBufferMode == (int32)ELightBufferMode::VisibleLightsStableIndices ?
								LightSceneInfo->Id.GetIndex() * LightNameBytes :
								LightNames.AddUninitialized(LightNameBytes);
							uint8* Dest = &LightNames[FirstCharIndex];
							const FAnsiString LightName = FAnsiString::Printf("%s/%s",
								StringCast<ANSICHAR>(*LightProxy->GetOwnerNameOrLabel()).Get(),
								StringCast<ANSICHAR>(*LightProxy->GetComponentFName().ToString()).Get());
							const int32 CharsToCopy = FMath::Min<int32>(LightName.Len(), LightNameBytes - 1);
						...
						|| (SortedLightInfo.SortKey.Fields.LightType == LightType_Rect && ViewFamily.EngineShowFlags.RectLights))
					{
						...
						const FVector ViewSpaceBoundingSphereCenter(View.ViewMatrices.GetWorldToView().TransformPosition(BoundingSphere.Center));
						const float Distance = ViewSpaceBoundingSphereCenter.Z + BoundingSphere.W;
						FurthestLight = FMath::Max(FurthestLight, Distance);

						const FVector3f LightViewPosition = FVector4f(View.ViewMatrices.GetWorldToView().TransformPosition(LightParameters.WorldPosition)); // LWC_TODO: precision loss
						const FVector3f LightViewDirection = FVector4f(View.ViewMatrices.GetWorldToView().TransformVector((FVector)LightParameters.Direction)); // LWC_TODO: precision loss
						...
						if (bUseTightRectLightCulling)
						{
							...
							{
								const FPlane4f ViewPlane(Plane.TransformBy(LightProxy->GetLightToWorld() * View.ViewMatrices.GetWorldToView()));
						...
						FVector4f WeightEstimateData;
						WeightEstimateData.X = LightParameters.Color.GetLuminance() * FMath::Max(LightParameters.DiffuseScale, LightParameters.SpecularScale);
						if (bIsRectLight)
						{
							WeightEstimateData.X *= 2.0f * LightParameters.SourceRadius * 2.0f * LightParameters.SourceLength;
						}
						WeightEstimateData.Y = LightProxy->IsInverseSquared() ? 0.0f : LightParameters.FalloffExponent;
						WeightEstimateData.Z = 0.0f;
						WeightEstimateData.W = 0.0f;
						ViewSpaceWeightEstimateData.Add(WeightEstimateData);
						...
						if (bVisualizeLightShape)
						{
							VisualizeLightShape::FSortKey& SortKey = VisualizeLightShapeSortKeys[VisualizeLightShapeSortKeys.AddUninitialized()];
							SortKey.LightData.ForwardLightIndex = IndexInBuffer;
							SortKey.LightData.LightType = SortedLightInfo.SortKey.Fields.LightType;
							SortKey.Distance = (float)ViewSpaceBoundingSphereCenter.SizeSquared();
						}

						bHasRectLights |= bIsRectLight;
						bHasTexturedLights |= LightProxy->HasSourceTexture();
						LightGridLightingChannelAndMask &= LightProxy->GetLightingChannelMask();
					}
					else if (SortedLightInfo.SortKey.Fields.LightType == LightType_Directional && ViewFamily.EngineShowFlags.DirectionalLights)
					{
							...
							|| (LightForwardShadingPriority == SelectedForwardDirectionalLightPriority && LightIntensitySq > SelectedForwardDirectionalLightIntensitySq))
						{
							...
							View.ForwardLightingResources.SelectedForwardDirectionalLightIndex = IndexInBuffer;
							...
							ForwardLightUniformParameters->DirectionalLightLightFunctionAtlasLightIndex = LightParameters.LightFunctionAtlasLightIndex;
							ForwardLightUniformParameters->DirectionalLightAffectsTranslucentLighting = LightParameters.bAffectsTranslucentLighting ? 1 : 0;
							ForwardLightUniformParameters->DirectionalLightHandledByMegaLights = SortedLightInfo.SortKey.Fields.bHandledByMegaLights ? 1 : 0;

							const FVector2D FadeParams = GetDirectionalLightDistanceFadeParameters(*LightSceneInfo, View.GetFeatureLevel(), LightSceneInfo->IsPrecomputedLightingValid(), View.MaxShadowCascades);
							...
							if (bDynamicShadows)
							{
								...
								DirLightShadowParameters->NumDirectionalLightCascades = 0;
								// Unused cascades should compare > all scene depths
								DirLightShadowParameters->CascadeEndDepths = FVector4f(MAX_FLT, MAX_FLT, MAX_FLT, MAX_FLT);
								DirLightShadowParameters->NumFarShadowCascades = 0;
								DirLightShadowParameters->FarShadowEndDepths = FVector4f(MAX_FLT, MAX_FLT, MAX_FLT, MAX_FLT);
								...
								for (const FProjectedShadowInfo* ShadowInfo : DirectionalLightShadowInfos)
								{
									...
									if (ShadowInfo->CascadeSettings.bFarShadowCascade)
									{
								...
								for (const FProjectedShadowInfo* ShadowInfo : DirectionalLightShadowInfos)
								{
									...
									if (ShadowInfo->IsWholeSceneDirectionalShadow() && !ShadowInfo->HasVirtualShadowMap() && ShadowInfo->bAllocated && !ShadowInfo->CascadeSettings.bFarShadowCascade && CascadeIndex < GMaxForwardShadowCascades)
									{
										const FMatrix WorldToShadow = ShadowInfo->GetWorldToShadowMatrix(DirLightShadowParameters->DirectionalLightShadowmapMinMax[CascadeIndex]);
										const FMatrix44f TranslatedWorldToShadow = FMatrix44f(TranslatedWorldToWorld * WorldToShadow);

										DirLightShadowParameters->NumDirectionalLightCascades++;
										DirLightShadowParameters->DirectionalLightTranslatedWorldToShadowMatrix[CascadeIndex] = TranslatedWorldToShadow;
										float CascadeSplitFar = ShadowInfo->CascadeSettings.SplitFar;
										if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0)
										{
											const FCSMSceneExtension& CSMExt = Scene->GetExtension<FCSMSceneExtension>();
											const int32 DepthReuseIndex = ShadowInfo->GetCSMShadowDepthReuseIndex();
											const FCSMDepthReuse* CSMDepthReuse = CSMExt.GetCSMDepthReuse(ShadowInfo->GetLightSceneInfo().Id, DepthReuseIndex);
											if (CSMDepthReuse && CSMDepthReuse->bHasEverRendered
												&& !ShadowInfo->ShouldCSMRenderDepthThisFrame(Scene, CSMDepthReuse)
												&& !CSMDepthReuse->FrozenViewForward.IsNearlyZero())
											{
												const FVector CurOrigin   = FVector(View.ViewMatrices.GetViewOrigin());
												const float   DepthOffset = FVector::DotProduct(CurOrigin - CSMDepthReuse->FrozenViewOrigin, CSMDepthReuse->FrozenViewForward);
												CascadeSplitFar = FMath::Max(0.0f, CSMDepthReuse->FrozenSplitFar - DepthOffset);
											}
										}
										DirLightShadowParameters->CascadeEndDepths[CascadeIndex] = CascadeSplitFar;

										if (CascadeIndex == 0)
										{
											DirLightShadowParameters->DirectionalLightShadowmapAtlas = GraphBuilder.RegisterExternalTexture(ShadowInfo->RenderTargets.DepthTarget);
											DirLightShadowParameters->DirectionalLightDepthBias = ShadowInfo->GetShaderDepthBias();
											FVector2D AtlasSize = DirLightShadowParameters->DirectionalLightShadowmapAtlas->Desc.Extent;
											DirLightShadowParameters->DirectionalLightShadowmapAtlasBufferSize = FVector4f(AtlasSize.X, AtlasSize.Y, 1.0f / AtlasSize.X, 1.0f / AtlasSize.Y);
										}
									}
									else if (ShadowInfo->IsWholeSceneDirectionalShadow() && !ShadowInfo->HasVirtualShadowMap() && ShadowInfo->bAllocated && ShadowInfo->CascadeSettings.bFarShadowCascade)
									{
										// Far shadow cascade - fill per-slot end depth for the overlay visualisation.
										const int32 FarIdx = CascadeIndex - FarShadowSplitIndexBase;
										if (FarIdx >= 0 && FarIdx < GMaxForwardFarShadowCascades)
										{
											DirLightShadowParameters->FarShadowEndDepths[FarIdx] = ShadowInfo->CascadeSettings.SplitFar;
											DirLightShadowParameters->NumFarShadowCascades = FMath::Max<uint32>(DirLightShadowParameters->NumFarShadowCascades, FarIdx + 1);
										}
									}
								}
							}
							...
							DirLightShadowParameters->DirectionalLightUseStaticShadowing = bStaticallyShadowedValue;
							if (bStaticallyShadowedValue)
							{
								...
								DirLightShadowParameters->DirectionalLightStaticShadowBufferSize = FVector4f(StaticShadowDepthMap->Data->ShadowMapSizeX, StaticShadowDepthMap->Data->ShadowMapSizeY, 1.0f / StaticShadowDepthMap->Data->ShadowMapSizeX, 1.0f / StaticShadowDepthMap->Data->ShadowMapSizeY);
								DirLightShadowParameters->DirectionalLightTranslatedWorldToStaticShadow = TranslatedWorldToShadow;
								DirLightShadowParameters->DirectionalLightStaticShadowmap = StaticShadowDepthMap->TextureRHI;
							}
							else
							{
								DirLightShadowParameters->DirectionalLightStaticShadowBufferSize = FVector4f(0, 0, 0, 0);
								DirLightShadowParameters->DirectionalLightTranslatedWorldToStaticShadow = FMatrix44f::Identity;
								DirLightShadowParameters->DirectionalLightStaticShadowmap = GWhiteTexture->TextureRHI;
							}
						}
					}
				}
			...
			if (LightGridViewData)
			{
				LightGridViewData->SimpleLightIdToForwardLightIndex = MoveTemp(NewSimpleLightIdToForwardLightIndex);
			}
			...
			if (SimpleLightsEnd > 0)
			{
				...
				for (int32 SortedIndex = 0; SortedIndex < SimpleLightsEnd; ++SortedIndex)
				{
					...
					const FLinearColor SimpleLightColor = GetSimpleLightColor(View, SimpleLight);
					PackLightData(LightData, View, SimpleLight, SimpleLightPerViewData, SimpleLightColor, INDEX_NONE /*PrevLocalLightIndex*/, false /*bHandledByMegaLights*/, GlobalRayEndBias);
					
					FVector4f ViewSpacePosAndRadius(FVector4f(View.ViewMatrices.GetWorldToView().TransformPosition(SimpleLightPerViewData.Position)), SimpleLight.Radius);
					...
					if (bVisualizeLightShape)
					{
						VisualizeLightShape::FSortKey& SortKey = VisualizeLightShapeSortKeys[SortedIndex];
						SortKey.LightData.ForwardLightIndex = IndexInBuffer;
						SortKey.LightData.LightType = LightType_Point;
						SortKey.Distance = FVector3f(ViewSpacePosAndRadius).SizeSquared();
					}
		...
		View.LightGridLightingChannelAndMask = LightGridLightingChannelAndMask;
		...
		if (AssociatedPrimaryView)
		{
			...
			check(PerViewForwardLightUniformParameters[View.PrimaryViewIndex]->NumLocalLights == NumVisibleLocalLights);
			...
			ForwardLightUniformParameters->LightNamesBuffer = PerViewForwardLightUniformParameters[View.PrimaryViewIndex]->LightNamesBuffer;
		}
		else
		{
			...
			FRDGBufferRef LightNamesBuffer = CreateByteAddressBuffer(GraphBuilder, bNeedsLightNames ? TEXT("LightNamesBuffer") : TEXT("DummyLightNamesBuffer"), MoveTemp(LightNames));
			ForwardLightUniformParameters->LightNamesBuffer = GraphBuilder.CreateSRV(LightNamesBuffer);
		}
		...
		ForwardLightUniformParameters->DirectLightingShowFlag = (ViewFamily.EngineShowFlags.Lighting && ViewFamily.EngineShowFlags.DirectLighting) ? 1 : 0;
		...
		if ((uint64)ForwardLightData.Num() > NumIndexableLights)
		{
			...
			if (!bWarned)
			{
				UE_LOGF(LogRenderer, Warning, "Exceeded indexable light count, glitches will be visible (%u / %llu)", ForwardLightData.Num(), NumIndexableLights);
				bWarned = true;
			}
		}
		...
		check(ViewSpaceWeightEstimateData.Num() == NumVisibleLocalLights);
		check(IndirectionIndices.Num() == NumVisibleLocalLights);

		if (bVisualizeLightShape && VisualizeLightShapeSortKeys.Num() > 0)
		{
			FRDGUploadData<VisualizeLightShape::FLightInfo> VisualizeLightShapeLightInfo(GraphBuilder, VisualizeLightShapeSortKeys.Num());

			GraphBuilder.AddSetupTask([SortKeys = MoveTemp(VisualizeLightShapeSortKeys), VisualizeLightShapeLightInfo]() mutable
			{
				SCOPED_NAMED_EVENT(VisualizeLightShapeSortLights, FColor::Emerald);

				Algo::Sort(SortKeys, TGreater<>());

				for (int32 Index = 0; Index < SortKeys.Num(); ++Index)
				{
					VisualizeLightShapeLightInfo[Index] = SortKeys[Index].LightData;
				}
			});

			View.ForwardLightingResources.VisualizeLightShapeLightInfos = TConstArrayView<VisualizeLightShape::FLightInfo>(VisualizeLightShapeLightInfo.GetData(), VisualizeLightShapeLightInfo.Num());
			View.ForwardLightingResources.VisualizeLightShapeLightInfoBuffer = CreateUploadBuffer(GraphBuilder, TEXT("VisualizeLightShape.LightInfos"), VisualizeLightShapeLightInfo);
		}
		...
		FRDGBufferRef LightViewSpaceWeightEstimate = CreateStructuredBuffer(GraphBuilder, TEXT("ViewSpaceWeightEstimateData"), TConstArrayView<FVector4f>(ViewSpaceWeightEstimateData));
		...
		FRDGBufferSRVRef LightViewSpaceWeightEstimateSRV = GraphBuilder.CreateSRV(FRDGBufferSRVDesc(LightViewSpaceWeightEstimate));
		...
		if(CVarLightCullingTwoLevel.GetValueOnRenderThread() && (int32)ForwardLightUniformParameters->NumLocalLights > CVarLightCullingTwoLevelThreshold.GetValueOnRenderThread())
		{
			...
			ParentLightGrid = LightGridInjection(
				...
				LightViewSpaceWeightEstimateSRV,
				nullptr,
...
FComputeLightGridOutput FSceneRenderer::PrepareForwardLightData(FRDGBuilder& GraphBuilder, bool bCullLightsToGrid, const FSortedLightSetSceneInfo& SortedLightSet)
{
	...
	FComputeLightGridArray<FForwardLightUniformParameters*> PerViewForwardLightUniformParameters;
	PerViewForwardLightUniformParameters.Reserve(Views.Num());

	FComputeLightGridArray<FForwardDirectionalLightShadowMapParameters*> PerViewForwardDirLightShadowParameters;
	PerViewForwardDirLightShadowParameters.Reserve(Views.Num());

	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		PerViewForwardLightUniformParameters.Add(GraphBuilder.AllocParameters<FForwardLightUniformParameters>());
		PerViewForwardDirLightShadowParameters.Add(GraphBuilder.AllocParameters<FForwardDirectionalLightShadowMapParameters>());
	}
		...
		GraphBuilder.AddSetupTask([this, &View, &SortedLightSet, LightViewData, LightShaderParameterFlags, Exposure]
			{
				...
				for (int32 SortedIndex = SortedLightSet.SimpleLightsEnd; SortedIndex < SortedLightSet.SortedLights.Num(); ++SortedIndex)
				{
					...
					const FLightSceneId LightSceneId = LightSceneInfo->Id;

					if (!ensureMsgf(LightViewData.IsValidIndex(LightSceneId.GetIndex()), TEXT("Visible light is missing from GPU Scene.")))
					{
						continue;
					}

					if (!ensureMsgf(Scene->Lights.IsValidIndex(LightSceneId), TEXT("Visible light is missing from GPU Scene.")))
					{
						continue;
					}
					...
					FLightViewData& CurrentLightViewData = LightViewData[LightSceneId.GetIndex()];
	...
	FComputeLightGridOutput Result = ComputeLightGrid(GraphBuilder, bCullLightsToGrid, SortedLightSet, PerViewForwardLightUniformParameters, PerViewForwardDirLightShadowParameters);
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		View.ForwardLightingResources.SetShadowUniformBuffer(GraphBuilder.CreateUniformBuffer(PerViewForwardDirLightShadowParameters[ViewIndex]));
	}
	...
	StatusFeedbackSocket = GPUMessage::RegisterHandler(TEXT("LightGrid.StatusFeedback"),
		[this](GPUMessage::FReader Message)
		{
			...
			if (AllocatedEntries > MaxEntries)
			{
				...
				if (bWarn)
				{
					UE_LOGF(LogRenderer, Warning, "Building light grid exceeded number of available entries (%u / %u). "
			...
			if (AllocatedLinks > MaxLinks)
			{
				...
				if (bWarn)
				{
					UE_LOGF(LogRenderer, Warning, "Building light grid exceeded number of available links, glitches will be visible (%u / %u).", AllocatedLinks, MaxLinks);
					bWarn = false;
				}
			}
		});
