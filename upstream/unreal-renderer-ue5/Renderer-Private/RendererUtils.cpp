...
FDepthBounds::FDepthBoundsValues FDepthBounds::CalculateNearFarDepthExcludingSky()
{
	...
	{
		SmallestFloatAbove0 = 1.1754943508e-38f;				// 32bit float depth
	}
...
namespace Substrate
{
	// This version of the function is deprecated, as FScene no longer contains FSubstrateSceneData, and so is forced to return dummy data.
	RENDERER_API TRDGUniformBufferRef<FSubstratePublicGlobalUniformParameters> GetPublicGlobalUniformBuffer(FRDGBuilder& GraphBuilder, FScene& Scene)
	{
		...
		{
			return CreatePublicGlobalUniformBuffer(GraphBuilder, nullptr);
		}
		return nullptr;
	}

	TRDGUniformBufferRef<FSubstratePublicGlobalUniformParameters> GetPublicGlobalUniformBuffer(FRDGBuilder& GraphBuilder, const FSceneTextures& SceneTextures)
	{		
		if(::Substrate::IsSubstrateEnabled())
		{
			const FSubstrateSceneData* SubstrateScene = SceneTextures.SubstrateSceneData.Get();

			if (!SubstrateScene || SubstrateScene->SubstratePublicGlobalUniformParameters == nullptr)
			{
...
void FBufferScatterUploader::UploadTo(FRDGBuilder& GraphBuilder, FRDGBuffer *DestBuffer, FRDGBuffer *ScatterOffsets, FRDGBuffer *Values, uint32 NumScatters, uint32 NumBytesPerElement, int32 NumValuesPerScatter, EPixelFormat ValuePixelFormat)
{
	FScatterCopyParams ScatterCopyParams { NumScatters, NumBytesPerElement, NumValuesPerScatter };
	ScatterCopyResource(GraphBuilder, DestBuffer, GraphBuilder.CreateSRV(ScatterOffsets), GraphBuilder.CreateSRV(Values, ValuePixelFormat), ScatterCopyParams);
}
...
void AddClearIndirectDispatchArgsPass(FRDGBuilder& GraphBuilder, ERHIFeatureLevel::Type FeatureLevel, FRDGBufferRef IndirectArgsRDG, const FIntVector3 &DimClearValue, uint32 NumIndirectArgs, uint32 IndirectArgStride)
{
	...
	auto ComputeShader = GetGlobalShaderMap(GShaderPlatformForFeatureLevel[FeatureLevel])->GetShader<FClearIndirectDispatchArgsCS>();
