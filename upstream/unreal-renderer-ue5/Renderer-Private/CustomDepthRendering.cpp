...
FCustomDepthTextures FCustomDepthTextures::Create(FRDGBuilder& GraphBuilder, FIntPoint CustomDepthExtent, EShaderPlatform ShaderPlatform, bool bRequireMultiView, uint16 MobileMultiViewRenderTargetNumLayers)
{
	...
	CustomDepthDesc.AliasableFormats.Add(PF_X24_G8);
	...
	TConstArrayView<Nanite::FPackedView> PrimaryNaniteViews)
{
	...
	if (TotalNaniteInstances > 0)
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < NumViewsToRender; ++ViewIndex)
		{
			...
			const FIntRect HZBViewRect = bDrawSceneViewsInOneNanitePass ? FIntRect(0, 0, FamilySize.X, FamilySize.Y) : GetHZBViewRect(View);
...
{
	int32 CustomDepthPrecacheMode = UPSOPrecacheSettingsManager::GetSettings().CustomDepthMode;
	...
	{		
		CollectDefaultMaterialPSOInitializers(SceneTexturesConfig, Material, VertexFactoryData, (EPrimitiveType)PreCacheParams.PrimitiveType, PSOInitializers);
		return;
	}
	...
	{
		const FMaterial* EffectiveMaterial = &Material;
		if (bUseDefaultMaterial && ((!bSupportPositionOnlyStream && VertexFactoryData.CustomDefaultVertexDeclaration) || PreCacheParams.PrimitiveType != PT_TriangleList))
		{
		...
		if (!bUseDefaultMaterial && bPrecacheCustomDepth)
		{
			...
			CollectPSOInitializersInternal<false>(VertexFactoryData, *EffectiveMaterial, MeshFillMode, MeshCullMode, (EPrimitiveType)PreCacheParams.PrimitiveType, PSOInitializers);
		}
	}
}
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	// Collect PSOs for all possible default material combinations
	{
		...
		CollectPSOInitializersInternal<true>(VertexFactoryData, Material, MeshFillMode, MeshCullMode, PrimitiveType, PSOInitializers);
		CollectPSOInitializersInternal<false>(VertexFactoryData, Material, MeshFillMode, MeshCullMode, PrimitiveType, PSOInitializers);
	}
	{
