	...
	if (Material.IsDeferredDecal())
	{
		...
		if (Material.GetRenderingThreadShaderMap())
		{
			...
			const bool bShouldRender =
				...
				DecalRendering::GetRenderTargetMode(ShaderPlatform, DecalBlendDesc, PassDecalStage) == RenderTargetMode;
	...
	ERasterizerCullMode MeshCullMode)
{
	...
	SortKey.BasePass.VertexShaderHash = MeshDecalPassShaders.VertexShader.GetSortKey() & 0xFFFF;
	SortKey.BasePass.PixelShaderHash = MeshDecalPassShaders.PixelShader.GetSortKey();
...
{
	const EShaderPlatform ShaderPlatform = GetFeatureLevelShaderPlatform(FeatureLevel);
	EDecalRenderTargetMode LocalRenderTargetMode = DecalRendering::GetRenderTargetMode(ShaderPlatform, DecalBlendDesc, DecalRenderStage);
...
void FMeshDecalMeshProcessor::CollectPSOInitializers(
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	if (!Material.IsDeferredDecal() || !UPSOPrecacheSettingsManager::GetSettings().bDeferredDecals)
	{
		return;
	}
	...
	const EShaderPlatform ShaderPlatform = GetFeatureLevelShaderPlatform(FeatureLevel);
	const FDecalBlendDesc DecalBlendDesc = DecalRendering::ComputeDecalBlendDesc(ShaderPlatform, Material);
	const bool bPlatformUsesDBuffers = IsUsingDBuffers(ShaderPlatform);
	...
	for (uint32 DecalStageIter = 0; DecalStageIter < (uint32)EDecalRenderStage::Num; ++DecalStageIter)
	{
		...
		EDecalRenderTargetMode LocalRenderTargetMode = DecalRendering::GetRenderTargetMode(ShaderPlatform, DecalBlendDesc, LocalDecalRenderStage);
		if (LocalRenderTargetMode == EDecalRenderTargetMode::DBuffer && !bPlatformUsesDBuffers)
		{
			continue;
		}
...
FMeshPassProcessor* CreateMeshDecalSceneColorAndGBufferNoNormalMeshProcessor_Mobile(ERHIFeatureLevel::Type FeatureLevel, const FScene* Scene, const FSceneView* InViewIfDynamicMeshCommand, FMeshPassDrawListContext* InDrawListContext)
{
	return new FMeshDecalMeshProcessor(Scene, FeatureLevel, InViewIfDynamicMeshCommand, EDecalRenderTargetMode::SceneColorAndGBufferNoNormal, EShadingPath::Mobile, InDrawListContext);
}
...
REGISTER_MESHPASSPROCESSOR_AND_PSOCOLLECTOR(Mobile_MeshDecal_SceneColorAndGBufferNoNormal, CreateMeshDecalSceneColorAndGBufferNoNormalMeshProcessor_Mobile, EShadingPath::Mobile, EMeshPass::MeshDecal_SceneColorAndGBufferNoNormal, EMeshPassFlags::CachedMeshCommands | EMeshPassFlags::MainView);
	...
	if (auto* Pass = View.ParallelMeshDrawCommandPasses[DecalMeshPassType]; HasAnyDraw(Pass))
	{
		...
		if (bRenderInParallel)
		{
			...
			GraphBuilder.AddDispatchPass(
				RDG_EVENT_NAME("MeshDecals: %s", GetMeshPassName(DecalMeshPassType)),
				PassParameters,
