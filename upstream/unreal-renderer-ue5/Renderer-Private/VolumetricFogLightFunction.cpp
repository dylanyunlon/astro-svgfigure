	...
	FRDGTexture*& OutDirectionalLightFunctionTexture)
{
	...
	FIntPoint LightFunctionResolution;
	{
		...
		if (LiFuncResXU32 > MaxTextureResU32 || LiFuncResYU32 > MaxTextureResU32)
		{
			...
			UE_LOGF(LogRenderer, Error,
				"Invalid LightFunctionResolution %dx%d, View={ %ls}, LightDirection={ %ls }",
		...
		FSphere Bounds = GetShadowSplitBoundsDepthRange(*DirectionalLightSceneInfo, View, View.ViewMatrices.GetViewOrigin(), 0, VolumetricFogMaxDistance, nullptr);
		check(Bounds.W > 0);
