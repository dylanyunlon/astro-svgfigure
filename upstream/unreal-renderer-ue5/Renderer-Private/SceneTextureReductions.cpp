	...
	FExtraParameters ExtraParameters)
{
	...
	// Force compute when a VisBuffer is provided, since HZBBuildPS does not support sampling it.
	bool bUseCompute = bReduceClosestDepth || (VisBufferTexture != nullptr) || HZBBuildUseCompute(ShaderPlatform);
		...
		bool bOutputClosest, bool bOutputFurthest, bool bProduceFroxelData, bool bLevel0Copy)
	{
		...
		if (bUseCompute)
		{
			...
			TShaderMapRef<FHZBBuildCS> ComputeShader(GetGlobalShaderMap(ShaderPlatform), PermutationVector);
