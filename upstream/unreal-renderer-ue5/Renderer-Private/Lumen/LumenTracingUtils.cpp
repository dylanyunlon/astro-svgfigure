...
TAutoConsoleVariable<float> CVarLumenSkylightPrefilterMipBias(
	TEXT("r.Lumen.SkylightPrefilterMipBias"),
	16.0f,
	TEXT("Mip bias applied to skylight lookup. 0 = use the mip that matches the probe being traced, 1 = sample skylight at 2x higher resolution than probe being traced. Currently only used by World Radiance Cache. Skylight prefiltering is disabled by default to prevent leaking indoors."),
...
static TAutoConsoleVariable<int32> CVarLumenSampleHeightFog(
	TEXT("r.Lumen.HeightFog"),
	1,
	TEXT("Whether to apply height fog to the lighting from Lumen's traces."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarLumenSampleHeightFogOnGI(
	TEXT("r.Lumen.HeightFogOnGI"),
	0,
	TEXT("Whether to apply height fog to the lighting from Lumen's GI traces. This is usually less noticeable than fog on reflections and creates a mismatch with direct lighting in heavily fogged scenes (sky lighting is removed)."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<float> CVarLumenMinDistanceForFoggingSky(
	TEXT("r.Lumen.HeightFog.MinDistanceForFoggingSky"),
	100000.0f,
	TEXT("Provides a minimum distance for fogging ray misses (sky) so that MaxTraceDistance is orthogonal from fog amount"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
	...
	FLumenCardTracingParameters& TracingParameters)
{
	...
	TracingParameters.SkylightPrefilterMipBias = FMath::Clamp<float>(CVarLumenSkylightPrefilterMipBias.GetValueOnRenderThread(), 0.0f, 16.0f);
	...
	TracingParameters.SampleHeightFog = CVarLumenSampleHeightFog.GetValueOnRenderThread() > 0 ? 1u : 0u;
	TracingParameters.SampleHeightFogOnGI = CVarLumenSampleHeightFog.GetValueOnRenderThread() > 0 && CVarLumenSampleHeightFogOnGI.GetValueOnRenderThread() > 0 ? 1u : 0u;
	TracingParameters.MinDistanceForFoggingSky = CVarLumenMinDistanceForFoggingSky.GetValueOnRenderThread();
	...
	TracingParameters.RadiosityNumFramesAccumulatedAtlas = OrBlack2DIfNull(GraphBuilder, FrameTemporaries.RadiosityNumFramesAccumulatedAtlas);
	TracingParameters.DirectLightingAtlas = OrBlack2DIfNull(GraphBuilder, FrameTemporaries.DirectLightingAtlas);
	TracingParameters.IndirectLightingAtlas = OrBlack2DIfNull(GraphBuilder, FrameTemporaries.IndirectLightingAtlas);
	TracingParameters.FinalLightingAtlas = OrBlack2DIfNull(GraphBuilder, FrameTemporaries.FinalLightingAtlas);
	...
	TracingParameters.AlbedoAtlas = OrBlack2DIfNull(GraphBuilder, FrameTemporaries.AlbedoAtlas);
	TracingParameters.NormalAtlas = OrBlack2DIfNull(GraphBuilder, FrameTemporaries.NormalAtlas);
	TracingParameters.EmissiveAtlas = OrBlack2DIfNull(GraphBuilder, FrameTemporaries.EmissiveAtlas);
	TracingParameters.DepthAtlas = OrBlack2DIfNull(GraphBuilder, FrameTemporaries.DepthAtlas);
	...
	if (View.GlobalDistanceFieldInfo.PageObjectGridBuffer)
	{
		...
		TracingParameters.GlobalDistanceFieldPageObjectGridBufferSize = View.GlobalDistanceFieldInfo.PageObjectGridBuffer->Desc.NumElements;
	}
	else
	{
		...
		TracingParameters.GlobalDistanceFieldPageObjectGridBufferSize = 0;
	}
