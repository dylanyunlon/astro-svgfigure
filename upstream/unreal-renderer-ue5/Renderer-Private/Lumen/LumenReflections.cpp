...
static TAutoConsoleVariable<int32> CVarLumenReflectionBilateralFilterMaxDisocclusionFrames(
	TEXT("r.Lumen.Reflections.BilateralFilter.MaxDisocclusionFrames"),
	2,
...
namespace LumenReflections
{
	...
	int32 GetSpatialFilterMaxDisocclusionFrames()
	{
		return FMath::Clamp(CVarLumenReflectionBilateralFilterMaxDisocclusionFrames.GetValueOnRenderThread(), 0, GetMaxFramesAccumulated());
	}
...
class FLumenReflectionResolveCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float3>, RWFinalThroughput)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<UNORM float2>, RWBackgroundVisibility)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2DArray<float3>, TraceThroughput)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2DArray<float2>, TraceBackgroundVisibility)
	...
	class FFrontLayerTranslucency : SHADER_PERMUTATION_BOOL("FRONT_LAYER_TRANSLUCENCY");
	class FResolveTraceThroughput : SHADER_PERMUTATION_BOOL("RESOLVE_TRACE_THROUGHPUT");
	...
	using FPermutationDomain = TShaderPermutationDomain<FSpatialReconstruction, FFrontLayerTranslucency, FResolveTraceThroughput, FDownsampleFactorX, FDownsampleFactorY, FUseAnisotropy, FDebugMode>;
	...
	if (Scene)
	{
		return Lumen::IsLumenFeatureAllowedForView(Scene, View, bSkipTracingDataCheck, bSkipProjectCheck)
			...
			&& (ShouldRenderLumenDiffuseGI(Scene, View, bSkipTracingDataCheck, bSkipProjectCheck)
				// Hit Lighting is required for standalone Lumen Reflections, Lumen::LumenHardwareRayTracing::GetHitLightingMode forces hit lighting
				|| (bIncludeStandalone && Lumen::UseHardwareRayTracedReflections(*View.Family) && Lumen::IsHardwareRayTracedHitLightingSupported()))
			&& (bSkipTracingDataCheck || Lumen::UseHardwareRayTracedReflections(*View.Family) || Lumen::IsSoftwareRayTracingSupported(View.GetShaderPlatform()));
	}
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	const bool bFrontLayer = FrontLayerTranslucencyData && FrontLayerTranslucencyData->IsValid();
	...
	// Tile classification was already done
	FRDGTextureRef LumenTileBitmask = bFrontLayer ? FrontLayerTranslucencyData->StochasticLighting.LumenTileBitmask.GetRenderTarget() : FrameTemporaries.StochasticLighting.LumenTileBitmask.GetRenderTarget();
...
class FLumenReflectionDenoiserClearCS : public FGlobalShader
{
	...
	class FClearFinalRadianceAndThroughput : SHADER_PERMUTATION_BOOL("CLEAR_FINAL_RADIANCE_AND_THROUGHPUT");
	using FPermutationDomain = TShaderPermutationDomain<FClearFinalRadianceAndThroughput>;
...
class FLumenFinalizeBackgroundVisibilityCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FLumenFinalizeBackgroundVisibilityCS)
	SHADER_USE_PARAMETER_STRUCT(FLumenFinalizeBackgroundVisibilityCS, FGlobalShader)

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, View)
		SHADER_PARAMETER_RDG_UNIFORM_BUFFER(FSceneTextureUniformParameters, SceneTexturesStruct)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<UNORM float>, RWFinalBackgroundVisibility)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D<float2>, ResolvedBackgroundVisibilityTexture)
	END_SHADER_PARAMETER_STRUCT()
...
IMPLEMENT_GLOBAL_SHADER(FLumenFinalizeBackgroundVisibilityCS, "/Engine/Private/Lumen/LumenCompositeBackgroundVisibility.usf", "LumenFinalizeBackgroundVisibilityCS", SF_Compute);

class FLumenCopyFinalizedBackgroundVisibilityPS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FLumenCopyFinalizedBackgroundVisibilityPS)
	SHADER_USE_PARAMETER_STRUCT(FLumenCopyFinalizedBackgroundVisibilityPS, FGlobalShader)

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D<float>, FinalBackgroundVisibilityTexture)
...
IMPLEMENT_GLOBAL_SHADER(FLumenCopyFinalizedBackgroundVisibilityPS, "/Engine/Private/Lumen/LumenCompositeBackgroundVisibility.usf", "LumenCopyFinalizedBackgroundVisibilityPS", SF_Pixel);
...
class FLumenReflectionDenoiserSpatialCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D<float3>, FinalThroughputTexture)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D<float2>, BackgroundVisibilityTexture)
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	{
		check(ReflectionsConfig.FrontLayerTranslucencyData != nullptr);
		check(ReflectionsConfig.FrontLayerTranslucencyData->SceneDepth->Desc.Extent == SceneTextures.Config.Extent);
	}
	...
	const FFrontLayerTranslucencyGBufferParameters FrontLayerTranslucencyGBufferParameters = GetFrontLayerTranslucencyGBufferParameters(ReflectionsConfig.FrontLayerTranslucencyData ? *ReflectionsConfig.FrontLayerTranslucencyData : FFrontLayerTranslucencyData());
	...
	// Ensure that the downsampled depth is initialized to invalid depth value, as it is used during resolve pass to know if a pixel contains or not valid luminance data.
	// Downsampled depth is properly initialized for all pixels with ClosureIndex=0, and valid pixels with ClosureIndex>0. This clear addesses invalid pixels with ClosureIndex>0
	if (TracedClosureCount > 1)
	{
		AddClearUAVPass(GraphBuilder, GraphBuilder.CreateUAV(FRDGTextureUAVDesc(ReflectionTracingParameters.DownsampledDepth, 0, DownsampledDepthDesc.Format, 1, TracedClosureCount-1)), -1.f);
	}
	...
	{
		ReflectionTileParameters = ReflectionTileClassification(GraphBuilder, View, SceneTextures, FrameTemporaries, ReflectionTracingParameters, ReflectionsConfig.FrontLayerTranslucencyData, ComputePassFlags);
	}
...

	{
		...
		PassParameters->FrontLayerTranslucencyGBufferParameters = FrontLayerTranslucencyGBufferParameters;
		PassParameters->ReflectionTracingParameters = ReflectionTracingParameters;
		PassParameters->ReflectionTileParameters = ReflectionTileParameters;
	...
	auto ReflectionDenoiserClear = [&](uint32 ClosureIndex)
	{
		...
		PassParameters->RWFinalThroughput = nullptr;
		...
		PermutationVector.Set<FLumenReflectionDenoiserClearCS::FClearFinalRadianceAndThroughput>(false);
	...
	auto ResolveReflections = [&](uint32 ClosureIndex)
	{
		...
		PassParameters->RWFinalThroughput = nullptr;
		PassParameters->RWBackgroundVisibility = nullptr;
		PassParameters->TraceThroughput = nullptr;
		...
		PermutationVector.Set<FLumenReflectionResolveCS::FResolveTraceThroughput>(false);
	...
	if (bTemporal)
	{
		...
		DenoiserParameters.FrontLayerTranslucencyGBufferParameters = FrontLayerTranslucencyGBufferParameters;
		...
		if (View.ViewState)
		{
			...
			SceneDepthHistory = TryRegisterExternalTexture(GraphBuilder, View.ViewState->StochasticLighting.SceneDepthHistory);
		}
			...
			&& bTemporal)
		{
			...
			if (ReflectionPass == ELumenReflectionPass::FrontLayerTranslucency)
			{
				SceneDepthHistory = TryRegisterExternalTexture(GraphBuilder, View.ViewState->StochasticLighting.FrontLayerTranslucencyDepthHistory);
			}
		}
		...
		auto SpatialFilter = [&](uint32 ClosureIndex)
		{
			...
			PassParameters->FinalThroughputTexture = nullptr;
			...
			PassParameters->TemporalMaxFramesAccumulated = LumenReflections::GetMaxFramesAccumulated();
			PassParameters->SpatialFilterMaxDisocclusionFrames = LumenReflections::GetSpatialFilterMaxDisocclusionFrames();
...
void FDeferredShadingSceneRenderer::RenderRayTracedTranslucencyView(
	...
	FRDGTextureRef& InOutFinalThroughput)
{
	...
	const FFrontLayerTranslucencyGBufferParameters FrontLayerTranslucencyGBufferParameters = GetFrontLayerTranslucencyGBufferParameters(FrontLayerTranslucencyData);

	const FLumenReflectionTileParameters ReflectionTileParameters =
		ReflectionTileClassification(GraphBuilder, View, SceneTextures, FrameTemporaries, ReflectionTracingParameters, &FrontLayerTranslucencyData, ComputePassFlags);
	...
	FRDGTextureDesc TraceThroughputDesc(FRDGTextureDesc::Create2DArray(ReflectionTracingParameters.ReflectionTracingBufferSize, PF_FloatR11G11B10, FClearValueBinding::White, TexCreate_ShaderResource | TexCreate_UAV, ClosureCount));
	FRDGTextureRef TraceThroughputTexture = GraphBuilder.CreateTexture(TraceThroughputDesc, TEXT("Lumen.RTTranslucency.TraceThroughput"));
	ReflectionTracingParameters.RWTraceThroughput = GraphBuilder.CreateUAV(FRDGTextureUAVDesc(TraceThroughputTexture));

	FRDGTextureDesc TraceBackgroundVisibilityDesc(FRDGTextureDesc::Create2DArray(ReflectionTracingParameters.ReflectionTracingBufferSize, PF_R8G8, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV, ClosureCount));
	...
	FRDGTextureRef ResolvedBackgroundVisibility = GraphBuilder.CreateTexture(
		FRDGTextureDesc::Create2D(EffectiveTextureResolution, PF_R8G8, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV),
		TEXT("Lumen.RTTranslucency.ResolvedBackgroundVisibility"));
	...
	if (!InOutFinalThroughput)
	{
		InOutFinalThroughput = GraphBuilder.CreateTexture(
			...
			TEXT("Lumen.RTTranslucency.FinalThroughput"));
	}

	FRDGTextureUAVRef ResolvedSpecularUAV = GraphBuilder.CreateUAV(ResolvedSpecularIndirect, ERDGUnorderedAccessViewFlags::SkipBarrier);
	FRDGTextureUAVRef FinalRadianceUAV = GraphBuilder.CreateUAV(InOutFinalRadiance, ERDGUnorderedAccessViewFlags::SkipBarrier);
	FRDGTextureUAVRef FinalThroughputUAV = GraphBuilder.CreateUAV(InOutFinalThroughput, ERDGUnorderedAccessViewFlags::SkipBarrier);
	FRDGTextureUAVRef ResolvedBackgroundVisibilityUAV = GraphBuilder.CreateUAV(ResolvedBackgroundVisibility, ERDGUnorderedAccessViewFlags::SkipBarrier);
	...
	if (bDenoise)
	{
		...
		PassParameters->RWFinalThroughput = FinalThroughputUAV;
		PassParameters->RWBackgroundVisibility = ResolvedBackgroundVisibilityUAV;
		PassParameters->bClearToSceneColor = bCompositeBackToSceneColor;
		PassParameters->ClosureIndex = 0;
		PassParameters->Substrate = Substrate::BindSubstrateGlobalUniformParameters(View);

		FLumenReflectionDenoiserClearCS::FPermutationDomain PermutationVector;
		PermutationVector.Set<FLumenReflectionDenoiserClearCS::FClearFinalRadianceAndThroughput>(true);
	...
	// Resolve translucency
	{
		...
		PassParameters->RWFinalThroughput = FinalThroughputUAV;
		PassParameters->RWBackgroundVisibility = ResolvedBackgroundVisibilityUAV;
		PassParameters->TraceThroughput = TraceThroughputTexture;
		...
		PermutationVector.Set<FLumenReflectionResolveCS::FResolveTraceThroughput>(true);
	...
	// If not compositing back to scene color, then we need to copy the updated background visibility to scene color alpha
	if (!bCompositeBackToSceneColor && IsPrimitiveAlphaHoldoutEnabledForAnyView(Views))
	{
		FRDGTextureRef FinalBackgroundVisibility = GraphBuilder.CreateTexture(
			FRDGTextureDesc::Create2D(EffectiveTextureResolution, PF_G8, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV),
			TEXT("Lumen.RTTranslucency.FinalBackgroundVisibility"));

		{
			FLumenFinalizeBackgroundVisibilityCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FLumenFinalizeBackgroundVisibilityCS::FParameters>();
			PassParameters->View = View.ViewUniformBuffer;
			PassParameters->SceneTexturesStruct = SceneTextures.UniformBuffer;
			PassParameters->RWFinalBackgroundVisibility = GraphBuilder.CreateUAV(FinalBackgroundVisibility);
			PassParameters->ResolvedBackgroundVisibilityTexture = ResolvedBackgroundVisibility;

			auto ComputeShader = View.ShaderMap->GetShader<FLumenFinalizeBackgroundVisibilityCS>();

			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("FinalizeBackgroundVisibility"),
				ComputePassFlags,
				ComputeShader,
				PassParameters,
				FComputeShaderUtils::GetGroupCount(View.ViewRect.Size(), FLumenFinalizeBackgroundVisibilityCS::GetGroupSize()));
		}

		{
			FLumenCopyFinalizedBackgroundVisibilityPS::FParameters* PassParameters = GraphBuilder.AllocParameters<FLumenCopyFinalizedBackgroundVisibilityPS::FParameters>();
			PassParameters->FinalBackgroundVisibilityTexture = FinalBackgroundVisibility;
			PassParameters->RenderTargets[0] = FRenderTargetBinding(SceneTextures.Color.Target, ERenderTargetLoadAction::ELoad);

			auto PixelShader = View.ShaderMap->GetShader<FLumenCopyFinalizedBackgroundVisibilityPS>();
			...
			FPixelShaderUtils::AddFullscreenPass(
				GraphBuilder,
				View.ShaderMap,
				RDG_EVENT_NAME("CopyFinalizedBackgroundVisibility"),
				PixelShader,
				PassParameters,
				View.ViewRect,
				BlendState);
	...
	if (bDenoise)
	{
		...
		// Spatial filter
		{
			...
			PassParameters->SpecularLightingAndSecondMomentTexture = SpecularAndSecondMoment;
			PassParameters->FinalThroughputTexture = InOutFinalThroughput;
			PassParameters->BackgroundVisibilityTexture = ResolvedBackgroundVisibility;
			PassParameters->NumFramesAccumulatedTexture = NumFramesAccumulated;
			PassParameters->SpatialFilterDepthWeightScale = GLumenReflectionBilateralFilterDepthWeightScale;
			PassParameters->SpatialFilterKernelRadius = CVarLumenReflectionBilateralFilterKernelRadius.GetValueOnRenderThread();
			PassParameters->SpatialFilterNumSamples = FMath::Clamp(GLumenReflectionBilateralFilterNumSamples, 0, 1024);
	...
	const FFrontLayerTranslucencyData& FrontLayerTranslucencyData)
{
	...
	FRDGTextureRef FinalThroughputTexture = nullptr;
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		RenderRayTracedTranslucencyView(
			...
			FinalThroughputTexture);

		if (FinalRadianceTexture && FinalThroughputTexture && !bSceneColorChanged && bRenderDistortion)
		{
			...
			TranslucencyResources.ColorModulateTexture = FinalThroughputTexture;
		}
	}
