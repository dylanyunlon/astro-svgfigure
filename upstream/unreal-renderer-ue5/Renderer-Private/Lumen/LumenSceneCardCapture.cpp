...
{	
	...
	if (ComputePSOPrecacheData.SetComputeShader(LumenCardComputeShader))
	{
		ComputePSOPrecacheData.bRequired = UPSOPrecacheSettingsManager::GetSettings().bNaniteLumenCardRequired;
...
{
	check(!ShadingCommand.IsBindless());
	...
	ShadingCommand.PassData.X = ShadingCommand.ArgsIndex; // Active Shading Bin
...
bool LoadLumenCardPipeline(
	...
	FGraphEventArray& OutAsyncPSOCompileEvents
)
{
	...
	if (bLoaded)
	{
		// check if PSOs are compiled
		if (PipelineStateCache::IsPSOPrecachingEnabled())
		{
			FGraphEventRef CompletionEvent;		
			FPSOPrecacheRequestID PSOPrecacheRequestID;
			EPSOPrecacheResult PSOPrecacheResult = PipelineStateCache::CheckPipelineStateInCache(LumenCardComputeShader.GetComputeShader(), CompletionEvent, PSOPrecacheRequestID);
		...
		ShadingPipeline.bCurve				= false;
		ShadingPipeline.MaterialBitFlags	= PackMaterialBitFlags(*ShadingPipeline.Material, ShadingPipeline.BoundTargetMask, ShadingPipeline.bNoDerivativeOps, false, false);
...
void FCardPageRenderData::CacheProjectionMatrix()
{
	const FVector FaceLocalExtent(CardWorldOBB.Extent);
...
void FCardPageRenderData::GetViewMatrices(const FViewInfo& MainView, FViewMatrices& OutViewMatrices) const
{
	...
	Initializer.ProjectionMatrix = ProjectionMatrixUnadjustedForRHI;
	...
	OutViewMatrices = FViewMatrices(Initializer);
}
...
void FCardPageRenderData::PatchView(const FScene* Scene, FViewInfo* View) const
{
	...
	GetViewMatrices(*View, View->ViewMatrices);
			...
			&& WorldSpaceCardBox.Intersect(PrimitiveSceneInfo->Proxy->GetBounds().GetBox()))
		{
			...
			else
			{
				...
				for (int32 MeshIndex = 0; MeshIndex < PrimitiveSceneInfo->StaticMeshRelevances.Num(); MeshIndex++)
				{
					...
					bool bBuildMeshDrawCommands = (PrimitiveGroup.IsLandscape() ? StaticMeshRelevance.bUseForLumenSceneCapture : StaticMeshRelevance.bUseForMaterial) && StaticMeshRelevance.GetLODIndex() == LODToRender;
					...
					if (bBuildMeshDrawCommands)
					{
						...
						if (StaticMeshCommandInfoIndex >= 0)
						{
							...
							const FMeshDrawCommand& MeshDrawCommand = Scene->CachedMeshDrawCommandStateBuckets[MeshPass].GetByElementId(CachedMeshDrawCommand.StateBucketId).Key;
