...
namespace LumenRadianceCache
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FRadianceCacheTracingParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<float4>, ProbeWorldOffset)
		...
		SHADER_PARAMETER(uint32, SkylightLeaking)
...
class FLumenRadianceCacheHardwareRayTracing : public FLumenHardwareRayTracingShaderBase
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<uint>, RWTraceHitTexture)
	...
	using FPermutationDomain = TShaderPermutationDomain<FLumenHardwareRayTracingShaderBase::FBasePermutationDomain, FRayTracingPass, FUseShaderExecutionReordering, FFarFieldOcclusionOnly, FRadianceCacheSkyVisibility, FRadianceCacheBatchSize>;
...
void FDeferredShadingSceneRenderer::PrepareLumenHardwareRayTracingRadianceCache(const FViewInfo& View, TArray<FRHIRayTracingShader*>& OutRayGenShaders)
{
	...
	if (Lumen::UseHardwareRayTracedRadianceCache(*View.Family) && bUseHitLighting)
	{
		for (int32 BatchSize = 1; BatchSize <= LumenRadianceCache::MaxBatchSize; ++BatchSize)
		{
			...
			PermutationVector.Set<FLumenRadianceCacheHardwareRayTracingRGS::FUseShaderExecutionReordering>(LumenHardwareRayTracing::UseShaderExecutionReordering(View.GetShaderPlatform()));
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	for (int32 BaseRadianceCacheIndex = 0; BaseRadianceCacheIndex < InputArray.Num(); BaseRadianceCacheIndex += LumenRadianceCache::MaxBatchSize)
	{
		...
		for (uint32 IndexInBatch = 0; IndexInBatch < BatchSize; ++IndexInBatch)
		{
			...
			// Full update budget is only going to be used with probe downsampling so divide resolution by 2
			const int32 MinProbeTraceTileResolution = FMath::DivideAndRoundUp<uint32>(RadianceCacheInputs.RadianceProbeResolution, 2 * LumenRadianceCache::TRACE_TILE_SIZE_2D);
			const int32 TempAtlasNumTraceTilesPerProbe = MinProbeTraceTileResolution * MinProbeTraceTileResolution;
			TempAtlasNumTraceTiles += Setup.UpdateParameters.TempProbeAtlasSizeInProbes.X * Setup.UpdateParameters.TempProbeAtlasSizeInProbes.Y * TempAtlasNumTraceTilesPerProbe;
		}
		...
		BatchTracingParameters.TargetFormatQuantizationError = Lumen::GetLightingQuantizationError();
		...
		for (uint32 IndexInBatch = 0; IndexInBatch < BatchSize; ++IndexInBatch)
		{
			...
			RadianceCacheParameters.ProbeWorldOffset = GraphBuilder.CreateSRV(ProbeWorldOffsetArray[RadianceCacheIndex], Outputs.RadianceCacheState.ProbeWorldOffsetFormat);
			...
			RadianceCacheParameters.SkylightLeaking = Inputs.RadianceCacheInputs.CalculateIrradiance;
		...
		FRDGTextureRef TraceHitTexture = GraphBuilder.CreateTexture(
			FRDGTextureDesc::Create2D(TempTraceAtlasResolution, PF_R16_UINT, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV),
		...
		// Default tracing of near-field
		{
			...
			PermutationVector.Set<FLumenRadianceCacheHardwareRayTracing::FUseShaderExecutionReordering>(bUseHitLighting && LumenHardwareRayTracing::UseShaderExecutionReordering(View.GetShaderPlatform()));
