...
// Scalability control which can disable HitLighting
static TAutoConsoleVariable<int32> CVarLumenHardwareRayTracingHitLightingAllowed(
	TEXT("r.Lumen.HardwareRayTracing.HitLighting.Allowed"),
	1,
	TEXT("Whether to allow HitLighting"),
...
static TAutoConsoleVariable<float> CVarLumenHardwareRayTracingFarFieldBias(
	...
	500.0f,
	TEXT("Determines bias for the far field traces. Default = 500"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
LumenHardwareRayTracing::EHitLightingMode LumenHardwareRayTracing::GetHitLightingMode(const FViewInfo& View, EDiffuseIndirectMethod DiffuseIndirectMethod)
{
	...
	if (View.FinalPostProcessSettings.LumenRayLightingMode == ELumenRayLightingModeOverride::SurfaceCache 
		|| !Lumen::IsHardwareRayTracedHitLightingSupported())
	{
...
bool Lumen::IsHardwareRayTracedHitLightingSupported()
{
	return LumenHardwareRayTracing::IsRayGenSupported() && CVarLumenHardwareRayTracingHitLightingAllowed.GetValueOnAnyThread() != 0;
}

bool LumenHardwareRayTracing::UseShaderExecutionReordering(EShaderPlatform InPlatform)
{
	// If current hardware suports it and user asked for it to be enabled
	return GRHIGlobals.SupportsShaderExecutionReordering && CVarLumenHardwareRayTracingShaderExecutionReordering.GetValueOnAnyThread() && FDataDrivenShaderPlatformInfo::GetSupportsShaderExecutionReordering(InPlatform);
}
	...
	FLumenHardwareRayTracingShaderBase::FSharedParameters* SharedParameters)
{
	...
	SharedParameters->FarFieldBias = LumenHardwareRayTracing::GetFarFieldBias();
}
