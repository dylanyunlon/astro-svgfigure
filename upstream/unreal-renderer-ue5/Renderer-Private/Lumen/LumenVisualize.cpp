...
FAutoConsoleVariableRef CVarLumenVisualize(
	...
	TEXT("20 - Card tile shadow downsample factor\n")
	TEXT("21 - Card sharing ID\n")
	TEXT("22 - ScreenProbeGather fast update mode amount\n")
	TEXT("23 - ScreenProbeGather number of frames accumulated\n")
	TEXT("24 - Radiosity Num Frames Accumulated"),
...
FAutoConsoleVariableRef CVarLumenSceneDumpStats(
	...
	TEXT("2 - dump mesh DF\n")
	TEXT("3 - dump LumenScene primitive culling infos\n")
	...
	const FMinimalSceneTextures& SceneTextures)
{
	if (CVarVisualizeUseShaderPrintForTraces.GetValueOnRenderThread() != 0)
	{
		...
		if (PooledVisualizeTracesData.IsValid())
		{
			...
			PassParameters->TonemappingParameters = StochasticLightingVisualize::GetTonemappingParameters(GraphBuilder, View, /*bAllowTonemapping*/ CVarLumenVisualizeToneMap.GetValueOnRenderThread());
	...
	FLumenVisualizeSceneSoftwareRayTracingParameters& VisualizeParameters)
{
	...
	// FLumenVisualizeSceneParameters
	{
		...
		CommonParameters.TonemappingParameters = StochasticLightingVisualize::GetTonemappingParameters(GraphBuilder, View, /*bAllowTonemapping*/ CVarLumenVisualizeToneMap.GetValueOnRenderThread());
		...
		LumenReflections::SetupCompositeParameters(View, ReflectionsMethod, CommonParameters.ReflectionsCompositeParameters);
		...
		StochasticLightingVisualize::GetTileOutputView(ViewRect, VisualizeTileIndex, CommonParameters.OutputViewOffset, CommonParameters.OutputViewSize, NumOverviewTilesPerRow);
	}
...
void FDeferredShadingSceneRenderer::RenderLumenMiscVisualizations(FRDGBuilder& GraphBuilder, const FMinimalSceneTextures& SceneTextures, const FLumenSceneFrameTemporaries& FrameTemporaries)
{
	...
	if (GLumenSceneDumpStats)
	{
		...
		LumenSceneData.DumpStats(
			...
			/*bDumpPrimitiveCullingInfos*/ GLumenSceneDumpStats == 3,
			/*bDumpPrimitiveGroups*/ GLumenSceneDumpStats == 4);
...
LumenRadianceCache::FRadianceCacheInputs GetFinalGatherRadianceCacheInputsForVisualize(const FViewInfo& View)
{
	...
	{
		return LumenIrradianceFieldGather::SetupRadianceCacheInputs(View);
	}
	else
	{
	...
	EReflectionsMethod ReflectionsMethod)
{
	...
	FLumenVisualizeSceneSoftwareRayTracingParameters VisualizeParameters;
	SetupVisualizeParameters(GraphBuilder, View, Output.ViewRect, ReflectionsMethod, VisualizeMode, VisualizeTileIndex, NumOverviewTilesPerRow, VisualizeParameters);
	...
	FString LabelText;
	int32 LabelOffsetY = 20;
	{
		switch (VisualizeMode)
		{
			case VISUALIZE_MODE_GEOMETRY_NORMALS:
				LabelText = TEXT("Geometry Normals");
				break;

			case VISUALIZE_MODE_REFLECTION_VIEW:
				if (Lumen::UseHardwareRayTracing(*View.Family))
				{
					...
					switch (HitLightingMode)
					{
						case LumenHardwareRayTracing::EHitLightingMode::HitLighting:
							LabelText = TEXT("Reflection View, HWRT, Hit Lighting");
							break;

						case LumenHardwareRayTracing::EHitLightingMode::HitLightingForReflections:
							LabelText = TEXT("Reflection View, HWRT, Hit Lighting for Reflections");
							break;

						default:
							LabelText = TEXT("Reflection View, HWRT");
							break;
					}
				}
				else
				{
					LabelText = Lumen::UseMeshSDFTracing(View.Family->EngineShowFlags) ? TEXT("Reflection View, SWRT with detail tracing") : TEXT("Reflection View, SWRT");
				}
				break;

			case VISUALIZE_MODE_SURFACE_CACHE:
				LabelText = TEXT("Lumen Scene, Pink - missing Surface Cache coverage, Yellow - culled Surface Cache");
				break;

			case VISUALIZE_MODE_ALBEDO:
				LabelText = TEXT("Lumen Scene Albedo");
				break;

			case VISUALIZE_MODE_NORMALS:
				LabelText = TEXT("Lumen Scene Normals");
				break;

			case VISUALIZE_MODE_RADIOSITY_NUM_FRAMES_ACCUMULATED:
				LabelText = TEXT("Radiosity Num Frames Accumulated");
				break;

			case VISUALIZE_MODE_DEDICATED_REFLECTION_RAYS:
				LumenReflections::FCompositeParameters ReflectionCompositeParameters;
				LumenReflections::SetupCompositeParameters(View, ReflectionsMethod, ReflectionCompositeParameters);
	...
	if (!LabelText.IsEmpty())
	{
		AddDrawCanvasPass(GraphBuilder, RDG_EVENT_NAME("MegaLightsVisualizeLabel"), View, FScreenPassRenderTarget(Output, ERenderTargetLoadAction::ELoad),
			[&ViewRect = Output.ViewRect, VisualizeTileIndex, NumOverviewTilesPerRow, LabelText, LabelOffsetY](FCanvas& Canvas)
			{
				...
				FIntPoint LabelLocation(OutputViewOffset.X + 2 * StochasticLightingVisualize::OverviewTileMargin, OutputViewOffset.Y + OutputViewSize.Y - LabelOffsetY);
...
FScreenPassTexture AddVisualizeLumenScenePass(FRDGBuilder& GraphBuilder, const FViewInfo& View, EDiffuseIndirectMethod DiffuseIndirectMethod, EReflectionsMethod ReflectionsMethod, const FVisualizeLumenSceneInputs& Inputs, const FLumenSceneFrameTemporaries& FrameTemporaries)
{
	...
	if (Lumen::IsLumenFeatureAllowedForView(Scene, View) 
		&& (DiffuseIndirectMethod == EDiffuseIndirectMethod::Lumen || ReflectionsMethod == EReflectionsMethod::Lumen))
	{
		const bool bVisualizeScene = Lumen::ShouldVisualizeScene(ViewFamily.EngineShowFlags);

		if (bVisualizeScene && (Lumen::ShouldVisualizeHardwareRayTracing(ViewFamily) || Lumen::IsSoftwareRayTracingSupported(View.GetShaderPlatform())))
		{
			...
			{
				const int32 VisualizeTiles[] = { VISUALIZE_MODE_GEOMETRY_NORMALS, VISUALIZE_MODE_REFLECTION_VIEW, VISUALIZE_MODE_SURFACE_CACHE };
			...
			else if (VisualizeMode == VISUALIZE_MODE_PERFORMANCE_OVERVIEW)
			{
				...
				const int32 VisualizeTiles[] = {VISUALIZE_MODE_DEDICATED_REFLECTION_RAYS};
	...
	if (FrameTemporaries.DebugData != nullptr)
	{
		...
		FLumenVisualizeSceneSoftwareRayTracingParameters VisualizeParameters;
		SetupVisualizeParameters(GraphBuilder, View, Output.ViewRect, ReflectionsMethod, 0, 0, /*NumOverviewTilesPerRow*/ 1, VisualizeParameters);
...
void VisualizeRayTracingGroups(const FViewInfo& View, const FLumenSceneData& LumenSceneData, FViewElementPDI& ViewPDI)
{
	...
	GetViewFrustumBounds(ViewFrustum, View.ViewMatrices.GetWorldToClip(), true);
...
void VisualizeCardPlacement(FSceneRenderingBulkObjectAllocator& Allocator, const FViewInfo& View, const FLumenSceneData& LumenSceneData, FViewElementPDI& ViewPDI)
{
	...
	for (const FLumenPrimitiveGroup& PrimitiveGroup : LumenSceneData.PrimitiveGroups)
	{
		...
		case 4:
			bVisible = bVisible && PrimitiveGroup.IsFarField();
			break;
		}
			...
			&& ViewFrustum.IntersectBox((FVector)WorldSpaceBoundingBox.GetCenter(), (FVector)WorldSpaceBoundingBox.GetExtent()))
		{
			...
			for (uint32 CardIndex = MeshCardsEntry.FirstCardIndex; CardIndex < MeshCardsEntry.FirstCardIndex + MeshCardsEntry.NumCards; ++CardIndex)
			{
				...
				const FLumenCardCullingInfo& CardCullingInfo = LumenSceneData.CardCullingInfos[CardIndex];

				bVisible = CardCullingInfo.bVisible;

				if (GVisualizeLumenCardPlacementIndex >= 0 && int32(Card.IndexInMeshCards) != GVisualizeLumenCardPlacementIndex)
				{
					bVisible = false;
				}
				...
				if (bVisible)
				{
					const FLumenCardOBBf& CardLocalOBB = Card.GetLocalOBB();
					uint32 CardHash = HashCombine(GetTypeHash(CardLocalOBB.Origin), GetTypeHash(CardLocalOBB.Extent));
					CardHash = HashCombine(CardHash, GetTypeHash(CardLocalOBB.AxisZ));
					...
					const FLumenCardOBBd& CardWorldOBB = Card.GetWorldOBB();
					const FMatrix CardToWorld(CardWorldOBB.GetCardToLocal());
					const FBox LocalBounds(-CardWorldOBB.Extent, CardWorldOBB.Extent);
							...
							&& PrimitiveGroup.MeshCardsIndex >= 0)
						{
							...
							for (uint32 CardIndex = MeshCardsEntry.FirstCardIndex; CardIndex < MeshCardsEntry.FirstCardIndex + MeshCardsEntry.NumCards; ++CardIndex)
							{
								...
								if (CardCullingInfo.bVisible
									&& (int32(Card.IndexInMeshCards) == GVisualizeLumenCardPlacementIndex || GVisualizeLumenCardPlacementIndex < 0)
...
void FDeferredShadingSceneRenderer::LumenScenePDIVisualization(const FLumenSceneFrameTemporaries* FrameTemporaries)
{
	FLumenSceneData& LumenSceneData = *Scene->GetLumenSceneData(Views[0]);
