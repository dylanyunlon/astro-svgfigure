...
static TAutoConsoleVariable<int32> CVarLumenRadianceCacheTemporaryAtlasDownsampleFactor(
	TEXT("r.Lumen.RadianceCache.TemporaryAtlasDownsampleFactor"),
	6,
	TEXT("Downsample factor on the temporary buffer used for probes updates. Higher downsample factors save more transient allocator memory, but may cause overflow and artifacts."),
...
static TAutoConsoleVariable<bool> CVarRadianceCacheFilterProbes(
...
namespace LumenRadianceCache
{
	...
	{
		return CVarRadianceCacheFilterProbes.GetValueOnRenderThread();
	}

	FIntPoint GetProbeAtlasResolutionInProbes(uint32 RequestedResolution)
	{
		RequestedResolution = FMath::Clamp(RequestedResolution, 1, 1024);

		FIntPoint AtlasRes;

		// Align up width to next power of two, so that ModuloMask and DivideShift works correctly
		AtlasRes.X = FMath::RoundUpToPowerOfTwo(RequestedResolution);

		// Set Y to match requested resolution
		AtlasRes.Y = FMath::DivideAndRoundUp<uint32>(RequestedResolution * RequestedResolution, AtlasRes.X);

		return AtlasRes;
	}
};

bool SupportAdaptiveProbes(EShaderPlatform Platform)
{
	// AllocateUsedProbesCS uses more than 8 UAVs
...
public:
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		return DoesPlatformSupportLumenGI(Parameters.Platform) && (!PermutationVector.Get<FAdaptiveProbes>() || SupportAdaptiveProbes(Parameters.Platform));
	}
...
class FAllocateProbeTracesCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, FrameNumber)
		SHADER_PARAMETER(uint32, MaxNumProbes)
		SHADER_PARAMETER(uint32, MaxProbesToTrace)
		SHADER_PARAMETER_STRUCT_INCLUDE(LumenRadianceCache::FRadianceCacheInterpolationParameters, RadianceCacheParameters)
	END_SHADER_PARAMETER_STRUCT()
...
class FRadianceCacheTraceFromProbesCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<uint>, ProbeTraceTileAllocator)
		SHADER_PARAMETER_STRUCT_INCLUDE(LumenRadianceCache::FRadianceCacheInterpolationParameters, RadianceCacheParameters)
		SHADER_PARAMETER_STRUCT_INCLUDE(LumenRadianceCache::FRadianceCacheUpdateParameters, RadianceCacheUpdateParameters)
...
public:
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		return DoesPlatformSupportLumenGI(Parameters.Platform) && ShouldCompileDistanceFieldShaders(Parameters.Platform);
	}
...
class FCalculateProbeIrradianceCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_STRUCT_INCLUDE(LumenRadianceCache::FRadianceCacheUpdateParameters, RadianceCacheUpdateParameters)
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, View)
		SHADER_PARAMETER(uint32, bUseTempProbeAtlas)
...
class FPrepareProbeOcclusionCS : public FGlobalShader
{
	...
	class FProbeOcclusionHWRT : SHADER_PERMUTATION_BOOL("PROBE_OCCLUSION_HWRT");
	using FPermutationDomain = TShaderPermutationDomain<FProbeOcclusionHWRT>;
...
FRDGTextureRef CreateRadianceProbeIndirectionTexture(FRDGBuilder& GraphBuilder, const FIntVector& RadianceProbeIndirectionTextureSize)
{
	const FRDGTextureDesc ProbeIndirectionDesc = FRDGTextureDesc::Create3D(
		RadianceProbeIndirectionTextureSize,
		PF_R32_UINT,
		FClearValueBinding::None,
		TexCreate_ShaderResource | TexCreate_UAV | TexCreate_3DTiling);

	return GraphBuilder.CreateTexture(ProbeIndirectionDesc, TEXT("Lumen.RadianceCache.RadianceProbeIndirectionTexture"));
}

FRDGTextureRef CreateRadianceProbeIndirectionTexture(FRDGBuilder& GraphBuilder, const LumenRadianceCache::FRadianceCacheInputs& RadianceCacheInputs)
{
	...
	return CreateRadianceProbeIndirectionTexture(GraphBuilder, RadianceProbeIndirectionTextureSize);
}

void ClearRadianceProbeIndirectionTexture(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureRef RadianceProbeIndirectionTexture, int32 NumClipmaps, ERDGPassFlags ComputePassFlags)
{
	FClearProbeIndirectionCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FClearProbeIndirectionCS::FParameters>();
	PassParameters->RWRadianceProbeIndirectionTexture = GraphBuilder.CreateUAV(RadianceProbeIndirectionTexture);

	auto ComputeShader = View.ShaderMap->GetShader<FClearProbeIndirectionCS>();
	...
	FComputeShaderUtils::AddPass(
		GraphBuilder,
		RDG_EVENT_NAME("ClearProbeIndirectionCS Clipmaps=%u", NumClipmaps),
		ComputePassFlags,
		ComputeShader,
		PassParameters,
		GroupSize);
...
{
	const FRadianceCacheInputs RadianceCacheInputs = LumenScreenProbeGatherRadianceCache::SetupRadianceCacheInputs(View);

	FRadianceCacheState RadianceCacheState;
	UpdateRadianceCacheState(GraphBuilder, View, RadianceCacheInputs, RadianceCacheState);

	FRDGTextureRef RadianceProbeIndirectionTexture = CreateRadianceProbeIndirectionTexture(GraphBuilder, RadianceCacheInputs);

	ClearRadianceProbeIndirectionTexture(GraphBuilder, View, RadianceProbeIndirectionTexture, RadianceCacheInputs.NumRadianceProbeClipmaps, ERDGPassFlags::Compute);

	FRDGTextureUAVRef RadianceProbeIndirectionTextureUAV = GraphBuilder.CreateUAV(RadianceProbeIndirectionTexture);
	const FRadianceCacheMarkParameters RadianceCacheMarkParameters = GetMarkParameters(RadianceProbeIndirectionTextureUAV, RadianceCacheState, RadianceCacheInputs);
	...
	return RadianceProbeIndirectionTexture;
}

static bool IsR8BufferSupported()
{
	...
	return bR8BufferSupported;
}

FRDGBufferSRVRef GetProbeValidSRV(FRDGBuilder& GraphBuilder, FRDGBufferRef InProbeValid)
{
	return GraphBuilder.CreateSRV(FRDGBufferSRVDesc(InProbeValid, IsR8BufferSupported() ? PF_R8_UINT : PF_R32_UINT));
}
	...
	ERDGPassFlags ComputePassFlags)
{
	if (GRadianceCacheUpdate != 0)
	{
		...
		const bool bFilterProbes = LumenRadianceCache::ShouldFilterProbes();
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			SetupOutputs.bAdaptiveProbes = RadianceCacheInputs.CalculateIrradiance && RadianceCacheInputs.UseAdaptiveProbes != 0 && SupportAdaptiveProbes(View.GetShaderPlatform());
			...
			if (RadianceCacheInputs.CalculateIrradiance)
			{
				...
				if (RadianceCacheState.FinalIrradianceAtlas.IsValid()
					...
					&& RadianceCacheState.ProbeInterpolationMisses.IsValid() == (SetupOutputs.bAdaptiveProbes))
				{
			...
			// Radiance cache update parameters
			{
				const int32 TempAtlastDownsampleFactor = FMath::Clamp(CVarLumenRadianceCacheTemporaryAtlasDownsampleFactor.GetValueOnRenderThread(), 1, 1024);

				// Default temporary atlas size is a fraction of the entire probe atlas
				int32 MaxTempProbes = FMath::DivideAndRoundUp(RadianceCacheInputs.ProbeAtlasResolutionInProbes.X * RadianceCacheInputs.ProbeAtlasResolutionInProbes.Y, TempAtlastDownsampleFactor);

				// Need to update at least the requested amount assuming that all probes were downsampled
				MaxTempProbes = FMath::Max<int32>(MaxTempProbes, 4 * RadianceCacheInputs.NumProbesToTraceBudget);

				FIntPoint TempAtlasSizeInProbes = LumenRadianceCache::GetProbeAtlasResolutionInProbes(FMath::FloorToInt(FMath::Sqrt((float)MaxTempProbes)));

				// Can't update more than fits in the atlas
				TempAtlasSizeInProbes.X = FMath::Min(TempAtlasSizeInProbes.X, RadianceCacheInputs.ProbeAtlasResolutionInProbes.X);
				TempAtlasSizeInProbes.Y = FMath::Min(TempAtlasSizeInProbes.Y, RadianceCacheInputs.ProbeAtlasResolutionInProbes.Y);

				// Make sure we update everything on a full update
				if (bResizedHistoryState || bPropagateGlobalLightingChange || GRadianceCacheForceFullUpdate)
				{
					TempAtlasSizeInProbes = RadianceCacheInputs.ProbeAtlasResolutionInProbes;
				}

				SetupOutputs.UpdateParameters.TempProbeAtlasSizeInProbes = TempAtlasSizeInProbes;
				SetupOutputs.UpdateParameters.TempProbeAtlasResolutionModuloMask = (1u << FMath::FloorLog2(TempAtlasSizeInProbes.X)) - 1;
				SetupOutputs.UpdateParameters.TempProbeAtlasResolutionDivideShift = FMath::FloorLog2(TempAtlasSizeInProbes.X);
				SetupOutputs.UpdateParameters.FinalRadianceProbeBorder = 1u << RadianceCacheInputs.FinalRadianceAtlasMaxMip;
			}

			SetupOutputs.TempRadianceAtlas = GraphBuilder.CreateTexture(
				FRDGTextureDesc::Create2D(SetupOutputs.UpdateParameters.TempProbeAtlasSizeInProbes * RadianceCacheInputs.RadianceProbeResolution, Lumen::GetLightingDataFormat(), FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV),
				TEXT("Lumen.RadianceCache.TempRadianceProbeAtlas"));

			if (Inputs.Configuration.bSkyVisibility)
			{
				SetupOutputs.TempSkyVisibilityAtlas = GraphBuilder.CreateTexture(
					FRDGTextureDesc::Create2D(SetupOutputs.UpdateParameters.TempProbeAtlasSizeInProbes * RadianceCacheInputs.RadianceProbeResolution, PF_G8, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV),
					TEXT("Lumen.RadianceCache.TempSkyVisibilityProbeAtlas"));
			}
			...
			// Don't throttle on one-time updates
			if (bResizedHistoryState || bPropagateGlobalLightingChange || GRadianceCacheForceFullUpdate)
			{
			...
			if (Inputs.RadianceProbeIndirectionTexture)
			{
				check(Inputs.RadianceProbeIndirectionTexture->Desc.GetSize() == RadianceProbeIndirectionTextureSize);

				RadianceCacheParameters.RadianceProbeIndirectionTexture = Inputs.RadianceProbeIndirectionTexture;
			}
			else
			{
				RadianceCacheParameters.RadianceProbeIndirectionTexture = CreateRadianceProbeIndirectionTexture(GraphBuilder, RadianceProbeIndirectionTextureSize);
			}
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			if (!Inputs.RadianceProbeIndirectionTexture)
			{
				...
				ClearRadianceProbeIndirectionTexture(GraphBuilder, Inputs.View, Outputs.RadianceCacheParameters.RadianceProbeIndirectionTexture, Inputs.RadianceCacheInputs.NumRadianceProbeClipmaps, ClearPassFlags);
			}
		}
		...
		const bool bR8BufferSupported = IsR8BufferSupported();
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			FRDGBufferUAVRef ProbeInterpolationMissesUAV = Setup.bAdaptiveProbes ? GraphBuilder.CreateUAV(FRDGBufferUAVDesc(ProbeInterpolationMisses[RadianceCacheIndex], PF_R32_UINT)) : nullptr;
			...
			if (!Setup.bPersistentCache || !IsValidRef(RadianceCacheState.ProbeFreeListAllocator))
			{
				...
				PermutationVector.Set<FClearProbeFreeList::FAdaptiveProbes>(Setup.bAdaptiveProbes);
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			{
				AddClearUAVPass(GraphBuilder, ProbeAllocatorUAV, 0, ComputePassFlags);
			}
		}
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			if (Setup.bPersistentCache)
			{
...

				{
					...
					FRDGBufferUAVRef ProbeFreeListUAV = GraphBuilder.CreateUAV(FRDGBufferUAVDesc(ProbeFreeList[RadianceCacheIndex], PF_R32_UINT));
					FRDGBufferUAVRef ProbeLastUsedFrameUAV = GraphBuilder.CreateUAV(FRDGBufferUAVDesc(ProbeLastUsedFrame[RadianceCacheIndex], PF_R32_UINT));
					FRDGBufferUAVRef ProbeAdaptiveIndicesUAV = Setup.bAdaptiveProbes ? GraphBuilder.CreateUAV(FRDGBufferUAVDesc(ProbeAdaptiveIndices[RadianceCacheIndex], PF_R32_UINT)) : nullptr;
					...
					PermutationVector.Set<FUpdateCacheForUsedProbesCS::FAdaptiveProbes>(Setup.bAdaptiveProbes);
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			{	
				...
				FRDGBufferUAVRef ProbeAdaptiveIndicesUAV = Setup.bAdaptiveProbes ? GraphBuilder.CreateUAV(FRDGBufferUAVDesc(ProbeAdaptiveIndices[RadianceCacheIndex], PF_R32_UINT)) : nullptr;
				FRDGBufferUAVRef ProbeWorldOffsetUAV = GraphBuilder.CreateUAV(FRDGBufferUAVDesc(ProbeWorldOffset[RadianceCacheIndex], RadianceCacheState.ProbeWorldOffsetFormat));
				FRDGBufferSRVRef LastFrameProbeInterpolationMissesSRV = Setup.bAdaptiveProbes ? GraphBuilder.CreateSRV(FRDGBufferSRVDesc(ProbeInterpolationMisses[RadianceCacheIndex], PF_R32_UINT)) : nullptr;
				...
				PermutationVector.Set<FAllocateUsedProbesCS::FAdaptiveProbes>(Setup.bAdaptiveProbes);
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			if (Setup.bAdaptiveProbes)
			{
				...
				AddClearUAVPass(GraphBuilder, ProbeInterpolationMissesUAV, 0, ComputePassFlags);
			}
		}
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			// Trace probes up to selected priority bucket
			{
				...
				FRDGBufferUAVRef ProbeTraceAllocatorUAV = GraphBuilder.CreateUAV(FRDGBufferUAVDesc(ProbeTraceAllocator[RadianceCacheIndex], PF_R32_UINT));
				const int32 MaxNumProbes = RadianceCacheInputs.ProbeAtlasResolutionInProbes.X * RadianceCacheInputs.ProbeAtlasResolutionInProbes.Y;
				FRDGBufferSRVRef ProbeAdaptiveIndicesSRV = Setup.bAdaptiveProbes ? GraphBuilder.CreateSRV(FRDGBufferSRVDesc(ProbeAdaptiveIndices[RadianceCacheIndex], PF_R32_UINT)) : nullptr;
				...
				PassParameters->FrameNumber = View.ViewState ? View.ViewState->GetFrameIndex() : View.Family->FrameNumber;
				PassParameters->MaxNumProbes = MaxNumProbes;
				PassParameters->MaxProbesToTrace = Setup.UpdateParameters.TempProbeAtlasSizeInProbes.X * Setup.UpdateParameters.TempProbeAtlasSizeInProbes.Y;
				PassParameters->RadianceCacheParameters = RadianceCacheParameters;

				FAllocateProbeTracesCS::FPermutationDomain PermutationVector;
				PermutationVector.Set<FAllocateProbeTracesCS::FAdaptiveProbes>(Setup.bAdaptiveProbes);
		...
		const int32 NumTraceTileTypes = 2;
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			GenerateProbeTraceTilesIndirectArgs[RadianceCacheIndex] = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateIndirectDesc<FRHIDispatchIndirectParameters>(NumTraceTileTypes), TEXT("Lumen.RadianceCache.GenerateProbeTraceTilesIndirectArgs"));
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			if (RadianceCacheInputs.CalculateIrradiance 
				&& RadianceCacheState.ProbeOffsetMode != 0
				&& UE::PixelFormat::HasCapabilities(RadianceCacheState.ProbeWorldOffsetFormat, EPixelFormatCapabilities::TypedUAVLoad) 
				&& UE::PixelFormat::HasCapabilities(RadianceCacheState.ProbeWorldOffsetFormat, EPixelFormatCapabilities::TypedUAVStore))
			{
				FRDGBufferUAVRef ProbeWorldOffsetUAV = GraphBuilder.CreateUAV(FRDGBufferUAVDesc(ProbeWorldOffset[RadianceCacheIndex], RadianceCacheState.ProbeWorldOffsetFormat));
				...
				const bool bOffsetUsingGBuffer = RadianceCacheState.ProbeOffsetMode == 1 || !Lumen::IsSoftwareRayTracingSupported(View.GetShaderPlatform());
			...
			{
				{
					...
					FComputeShaderUtils::AddPass(
						...
						PassParameters,
						PassParameters->IndirectArgs,
						1 * sizeof(FRHIDispatchIndirectParameters));
				}
			}
			else
			{
		...
		else
		{
			for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
			{
				...
				PassParameters->ProbeTraceTileAllocator = GraphBuilder.CreateSRV(FRDGBufferSRVDesc(ProbeTraceTileAllocator[RadianceCacheIndex], PF_R32_UINT));
				PassParameters->RadianceCacheParameters = RadianceCacheParameters;
				PassParameters->RadianceCacheUpdateParameters = Setup.UpdateParameters;
				...
				PermutationVector = FRadianceCacheTraceFromProbesCS::RemapPermutation(PermutationVector);
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			if (bFilterProbes)
			{
				...
				PassParameters->RWRadianceProbeAtlasTexture = GraphBuilder.CreateUAV(Setup.TempRadianceAtlas);
				PassParameters->RWSkyVisibilityProbeAtlasTexture = Setup.TempSkyVisibilityAtlas ? GraphBuilder.CreateUAV(Setup.TempSkyVisibilityAtlas) : nullptr;
				PassParameters->RadianceProbeAtlasTexture = Setup.RadianceProbeAtlasTextureSource;
				PassParameters->SkyVisibilityProbeAtlasTexture = Setup.SkyVisibilityProbeAtlasTextureSource;
				PassParameters->DepthProbeAtlasTexture = Setup.DepthProbeAtlasTexture;
				PassParameters->ProbeTraceData = GraphBuilder.CreateSRV(FRDGBufferSRVDesc(ProbeTraceData[RadianceCacheIndex], PF_A32B32G32R32F));
				PassParameters->View = View.ViewUniformBuffer;
				PassParameters->BlueNoise = CreateUniformBufferImmediate(GetBlueNoiseGlobalParameters(), UniformBuffer_SingleDraw);
				PassParameters->RadianceCacheParameters = RadianceCacheParameters;
				PassParameters->RadianceCacheUpdateParameters = Setup.UpdateParameters;
		...
		for (int32 RadianceCacheIndex = 0; RadianceCacheIndex < InputArray.Num(); RadianceCacheIndex++)
		{
			...
			const bool bTempProbeAtlas = bFilterProbes;
			FRDGTextureRef RadianceProbeAtlasTexture = bTempProbeAtlas ? Setup.TempRadianceAtlas : Setup.RadianceProbeAtlasTextureSource;
			FRDGTextureRef SkyVisibilityProbeAtlasTexture = bTempProbeAtlas ? Setup.TempSkyVisibilityAtlas : Setup.SkyVisibilityProbeAtlasTextureSource;
			...
			{
				{
					...
					PassParameters->RadianceCacheUpdateParameters = Setup.UpdateParameters;
					PassParameters->View = View.ViewUniformBuffer;
					PassParameters->bUseTempProbeAtlas = bTempProbeAtlas ? 1 : 0;
		...
		
			{
				...
				static_assert(LumenRadianceCache::MaxMipmaps == 3);

				{
					...
					PassParameters->RadianceProbeAtlasTexture = RadianceProbeAtlasTexture;
					PassParameters->SkyVisibilityProbeAtlasTexture = SkyVisibilityProbeAtlasTexture;
					PassParameters->ProbeTraceData = GraphBuilder.CreateSRV(FRDGBufferSRVDesc(ProbeTraceData[RadianceCacheIndex], PF_A32B32G32R32F));
					PassParameters->View = View.ViewUniformBuffer;
					PassParameters->RadianceCacheParameters = RadianceCacheParameters;
					PassParameters->RadianceCacheUpdateParameters = Setup.UpdateParameters;
