...
static TAutoConsoleVariable<float> CVarLocalFogVolumeGlobalStartDistance(
	TEXT("r.LocalFogVolume.GlobalStartDistance"), 1000.0f,
	TEXT("The start distance in centimeter from which local fog volumes starts to appear. Default to 10 meters. This will push away from the camera all the local fog volume visibility independenly of their own start distance."),
	ECVF_RenderThreadSafe);
...
{
	if (ShouldRenderLocalFogVolume(Scene, SceneViewFamily) 
		&& !SceneViewFamily.EngineShowFlags.VisualizeLocalFogVolumes)	// When this is set, the debug is rendered using the post process chain.
	{
		const FEngineShowFlags& EngineShowFlags = SceneViewFamily.EngineShowFlags;
...
float GetLocalFogVolumeViewStartDistance(const FViewInfo& View)
{
	const float FinalStartDistance = FMath::Max(GetLocalFogVolumeGlobalStartDistance(), View.LocalFogVolumeViewData.DepthBoundStartDistance);
	return FinalStartDistance;
}
...
void SetDummyLocalFogVolumeForView(FRDGBuilder& GraphBuilder, FViewInfo& View)
{
	...
	View.LocalFogVolumeViewData.DepthBoundStartDistance		= FLT_MAX;
...
void GetLocalFogVolumeViewSortingData(const FScene* Scene, const FViewInfo& View, FRDGBuilder& GraphBuilder, FLocalFogVolumeSortingData& Out)
{
	...
	Out.LocalFogVolumeStartDistanceAndUniformScale = (FVector2f*)GraphBuilder.Alloc(sizeof(FVector2f) * Out.LocalFogVolumeInstanceCount, 16);
	...
	for (FLocalFogVolumeSceneProxy* LHF : Scene->LocalFogVolumes)
	{
		...
		const float NormalizeTolerance = 1.e-32f;
		...
		LocalFogVolumeGPUInstanceDataIt->Data1[3] = AsUint32(LHF->FogStartDistance);
		...
		Out.LocalFogVolumeStartDistanceAndUniformScale[Out.LocalFogVolumeInstanceCountFinal]= FVector2f(LHF->FogStartDistance, LHF->FogUniformScale);
...
void CreateViewLocalFogVolumeBufferSRV(const FScene* Scene, FViewInfo& View, FRDGBuilder& GraphBuilder, FLocalFogVolumeSortingData& SortingData, bool bShouldRenderLocalFogVolumeInVolumetricFog, bool bUseHalfResLocalFogVolume)
{
	...
	float DepthBoundStartDistance = FLT_MAX;
	for (uint32 i = 0; i < SortingData.LocalFogVolumeInstanceCountFinal; ++i)
	{
		const uint64 Index = SortingData.LocalFogVolumeSortKeys[i].FogVolume.Index;
		const FVector FogCenterPos = SortingData.LocalFogVolumeCenterPos[Index];	// Recovered form the original array via index because the sorting of the previous view might have changed the order.
		const FVector2f FogStartDistanceAndUniformScale = SortingData.LocalFogVolumeStartDistanceAndUniformScale[Index];
		const float FogStartDistance = float(FogStartDistanceAndUniformScale.X);
		const float FogUniformScale = float(FogStartDistanceAndUniformScale.Y);

		const float DistanceToView = float((FogCenterPos - ViewOrigin).Size());
		SortingData.LocalFogVolumeSortKeys[i].FogVolume.Distance = BitCast<uint32>(DistanceToView);

		// For a volume, if the camera is inside, the start closest start distance is FogStartDistance.
		// Otherwise it is the distance to the edge of the volume.
		DepthBoundStartDistance = FMath::Min(DepthBoundStartDistance, FMath::Max(FogStartDistance, DistanceToView - FogUniformScale));
	}
	...
	for (uint32 i = 0; i < SortingData.LocalFogVolumeInstanceCountFinal; i++)
	{
		...
		const FVector& LFVPosition = SortingData.LocalFogVolumeCenterPos[LFVKey.FogVolume.Index];
	...
	View.LocalFogVolumeViewData.DepthBoundStartDistance		= DepthBoundStartDistance;
	...
	View.LocalFogVolumeViewData.GPUInstanceCullingDataBuffer = CreateVertexBuffer(
		...
		FRDGBufferDesc::CreateBufferDesc(sizeof(FVector4f), SortingData.LocalFogVolumeInstanceCountFinal),
...
void RenderLocalFogVolumeForView(
	const FViewInfo& View,
	FRDGBuilder& GraphBuilder,
	int32 LocalFogVolumeTileDebugMode,
...
{
	const bool bRenderLocalFogVolumes = View.LocalFogVolumeViewData.GPUInstanceCount == 0;
	if (bRenderLocalFogVolumes)
	{
		return;
	}

	RDG_EVENT_SCOPE_CONDITIONAL_STAT(GraphBuilder, bRenderLocalFogVolumes, LocalFogVolumeVolumes, "%s", bVisualizationPass ? TEXT("LocalFogVolume.Visualization") : TEXT("LocalFogVolume.Render"));

	// This distance is the start distance account for global and all local fog volume start distance from the camera.
	const float FinalStartDistance = GetLocalFogVolumeViewStartDistance(View);

	const FIntRect ViewRect = View.ViewRect;
	const FMatrix ViewProjectionMatrix = View.ViewMatrices.GetViewToClip();
	const FMatrix ViewInvProjectionMatrix = View.ViewMatrices.GetClipToView();
	FDepthBoundSetup DepthBoundSetup = GetDepthBoundSetup(FinalStartDistance, ViewProjectionMatrix, ViewInvProjectionMatrix);
	...
	PassParameters->PS.LocalFogVolumeTileDebug = LocalFogVolumeTileDebugMode;
	...
	if (bVisualizationPass)
	{
		const float LFVGlobalStartDistance = GetLocalFogVolumeGlobalStartDistance();
		const float LFVInstancesStartDistance = View.LocalFogVolumeViewData.DepthBoundStartDistance;
		// Now debug print
		AddDrawCanvasPass(GraphBuilder, {}, View, FScreenPassRenderTarget(SceneColorTexture, ERenderTargetLoadAction::ELoad),
			[&View, LFVGlobalStartDistance, LFVInstancesStartDistance, FinalStartDistance](FCanvas& Canvas)
			{
				FString Text;
				...
				Canvas.DrawShadowedString(50.0, 50.0, TEXT("LOCAL FOG VOLUME DEBUG VIEW"), GEngine->GetLargeFont(), FLinearColor::White);

				Canvas.DrawShadowedString(50.0, 70.0, TEXT("Global Start Distance"), GEngine->GetLargeFont(), FLinearColor::White);
				Text = FString::Printf(TEXT("= %.1f"), LFVGlobalStartDistance);
				Canvas.DrawShadowedString(200.0, 70.0, *Text, GEngine->GetLargeFont(), FLinearColor::White);

				Canvas.DrawShadowedString(50.0, 85.0, TEXT("Instances Start Distance"), GEngine->GetLargeFont(), FLinearColor::White);
				Text = FString::Printf(TEXT("= %.1f"), LFVInstancesStartDistance);
				Canvas.DrawShadowedString(200.0, 85.0, *Text, GEngine->GetLargeFont(), FLinearColor::White);

				Canvas.DrawShadowedString(50.0, 100.0, TEXT("Final Start Distance"), GEngine->GetLargeFont(), FLinearColor::White);
				Text = FString::Printf(TEXT("= %.1f"), FinalStartDistance);
				Canvas.DrawShadowedString(200.0, 100.0, *Text, GEngine->GetLargeFont(), FLinearColor::White);
			});
	}
}
	...
	bool bVisualizationPass)
{
	...
	if (LocalFogVolumeInstanceCount > 0 && ShouldRenderLocalFogVolume(Scene, SceneViewFamily))
	{
		...
		for (FViewInfo& View : Views)
		{
			...
			RenderLocalFogVolumeForView(
				View,
				GraphBuilder,
				GetLocalFogVolumeTileDebugMode(SceneViewFamily.EngineShowFlags),
				SceneColorTexture,
				SceneDepthTexture,
...
FScreenPassTexture RenderLocalFogVolumeVisualizationPostProcess(
	FRDGBuilder& GraphBuilder,
	const FViewInfo& View,
	const FScreenPassTexture& ScreenPassSceneColor,
	...
	const FScreenPassRenderTarget& OverrideOutput)
{
	...
	if (!OutputRT.IsValid())
	{
		OutputRT = FScreenPassRenderTarget::CreateFromInput(GraphBuilder, ScreenPassSceneColor, View.GetOverwriteLoadAction(), TEXT("VisualizeLocalFogVolumes"));
	}

	if (ScreenPassSceneColor.Texture != OutputRT.GetRenderTargetBinding().GetTexture())
	{
		// Copy the scene color because below we do not write and copy all the view pixel. We only stamp the atlas and its tiles on top.
		AddDrawTexturePass(
			GraphBuilder,
			View,
			ScreenPassSceneColor.Texture,
			OutputRT.GetRenderTargetBinding().GetTexture(),
			FIntPoint::ZeroValue,
			OutputRT.GetRenderTargetBinding().GetTexture()->Desc.Extent,
			FIntPoint::ZeroValue,
			OutputRT.GetRenderTargetBinding().GetTexture()->Desc.Extent);
	}
	...
	RenderLocalFogVolumeForView(
		View,
		GraphBuilder,
		GetLocalFogVolumeTileDebugMode(View.Family->EngineShowFlags),
		OutputRT.Texture,
		SceneDepth.Texture,
		LightShaftOcclusionTexture,
		View.GetSceneTextures().UniformBuffer,
		bVisualizationPass);
	...
	const FViewInfo& View)
{
	...
	FDepthBoundSetup DepthBoundSetup = GetDepthBoundSetup(FinalStartDistance, View.ViewMatrices.GetViewToClip(), View.ViewMatrices.GetClipToView());
	GraphicsPSOInit.bDepthBounds = DepthBoundSetup.bEnabled;
