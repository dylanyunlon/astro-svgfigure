...
static TAutoConsoleVariable<int32> CVarHeterogeneousCascadeShadowsForDirectionalLight(
	TEXT("r.HeterogeneousVolumes.Shadows.Cascades"),
	1,
	TEXT("Enables cascade shadows for the directional light (Default = 1)\n")
	TEXT("0: Disabled\n")
	TEXT("1: View-based frustums (Default)\n")
	TEXT("2: View-based clipmaps centered about the camera\n")
	TEXT("3: Auto-fit shadow maps based on bounding dimensions of contained volumes"),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarHeterogeneousVolumesCascadeShadowsQuantization(
	TEXT("r.HeterogeneousVolumes.Shadows.Cascades.PixelSnapping"),
	16,
	TEXT("Cascade quantization unit in pixel-space (Default = 16)"),
	ECVF_RenderThreadSafe
);
...
namespace HeterogeneousVolumes
{
	...
	{
		if (SupportsLightType(LightType_Directional) && SupportsShadowForLightType(LightType_Directional))
		{
			return CVarHeterogeneousCascadeShadowsForDirectionalLight.GetValueOnRenderThread() != 0;
		}
	...
	{
		return static_cast<ECascadeShadowMode>(CVarHeterogeneousCascadeShadowsForDirectionalLight.GetValueOnRenderThread());
	}

	int32 GetCascadeShadowsQuantizationUnit()
	{
		if (SupportsCascadeShadowsForDirectionalLight())
		{
			return CVarHeterogeneousVolumesCascadeShadowsQuantization.GetValueOnRenderThread();
		}
...
)
{
	...
	else
	{
		...
		const FGlobalShaderMap* GlobalShaderMap = GetGlobalShaderMap(Scene->GetShaderPlatform());
...
)
{
	...
	FMatrix WorldToView = View.ViewMatrices.GetWorldToView();
	FMatrix ViewToWorld = View.ViewMatrices.GetViewToWorld();
...
)
{
	...
	TArray<FLightSceneInfoCompact, TInlineAllocator<64>> LightSceneInfoCompact = HeterogeneousVolumes::GatherShadowLights(Scene, Views[0].ViewLightingChannelMask);
...
class FConvertVolumetricShadowMapToBeerLawShadowMapCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FConvertVolumetricShadowMapToBeerLawShadowMapCS);
	SHADER_USE_PARAMETER_STRUCT(FConvertVolumetricShadowMapToBeerLawShadowMapCS, FGlobalShader);
...
)
{
	...
	//FString ShadowMapString = FString::Printf(TEXT("HeterogeneousVolumes.%s.BeerShadowMapTexture"), *ShadowMapName);
	//BeerShadowMapTexture = GraphBuilder.CreateTexture(Desc, *ShadowMapString);
	...
	FConvertVolumetricShadowMapToBeerLawShadowMapCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FConvertVolumetricShadowMapToBeerLawShadowMapCS::FParameters>();
	{
		// Input
		...
		PassParameters->VolumetricShadowLinkedListBuffer0 = GraphBuilder.CreateSRV(VolumetricShadowLinkedListBuffer);

		// Output
		PassParameters->RWBeerShadowMapTexture = GraphBuilder.CreateUAV(BeerShadowMapTexture);
	}

	FConvertVolumetricShadowMapToBeerLawShadowMapCS::FPermutationDomain PermutationVector;
	TShaderRef<FConvertVolumetricShadowMapToBeerLawShadowMapCS> ComputeShader = View.ShaderMap->GetShader<FConvertVolumetricShadowMapToBeerLawShadowMapCS>(PermutationVector);
	FComputeShaderUtils::AddPass(
		GraphBuilder,
		// Add Light name..
...
)
{
	Parameters = GraphBuilder.AllocParameters<FAdaptiveVolumetricShadowMapUniformBufferParameters>();
	{
		...
		for (int32 i = 0; i < NumShadowMatrices; ++i)
		{
			...
			Parameters->TranslatedWorldOrigin[i] = TranslatedWorldOrigin[i];
			Parameters->TranslatedWorldPlane[i] = TranslatedWorldPlane[i];
			Parameters->SplitDepths[i] = SplitDepths[i];
		}
...
{
	Parameters = GraphBuilder.AllocParameters<FBeerShadowMapUniformBufferParameters>();
	{
		Parameters->NumShadowMatrices = NumShadowMatrices;
		for (int32 i = 0; i < NumShadowMatrices; ++i)
		{
		...
		Parameters->bIsEmpty = (VolumetricShadowMapResolution == FIntPoint::ZeroValue);
		Parameters->bIsDirectionalLight = bIsDirectionalLight;
		Parameters->BeerShadowMapTexture = GraphBuilder.CreateSRV(BeerShadowMapTexture);
		Parameters->RadianceTexture = GraphBuilder.CreateSRV(GSystemTextures.GetBlackDummy(GraphBuilder));
		Parameters->TextureSampler = TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
	}
}
...
{
	FBeerShadowMapUniformBufferParameters* UniformBufferParameters;
	...
	BeerShadowMapUniformBuffer = GraphBuilder.CreateUniformBuffer(UniformBufferParameters);
}
...
namespace HeterogeneousVolumes
{
	...
	)
	{
		FAdaptiveVolumetricShadowMapUniformBufferParameters* UniformBufferParameters = GraphBuilder.AllocParameters<FAdaptiveVolumetricShadowMapUniformBufferParameters>();
		{
			...
			for (int32 i = 0; i < 6; ++i)
			{
				...
				UniformBufferParameters->TranslatedWorldOrigin[i] = FVector3f::ZeroVector;
				UniformBufferParameters->TranslatedWorldPlane[i] = FVector4f::Zero();
				UniformBufferParameters->SplitDepths[i] = FVector4f::Zero();
			}
		...
		FSceneViewState* ViewState)
	{
		...
		else
		{
			...
			for (int32 i = 0; i < Parameters.NumShadowMatrices; ++i)
			{
				...
				Parameters.TranslatedWorldOrigin[i] = FVector3f::ZeroVector;
				Parameters.TranslatedWorldPlane[i] = FVector4f::Zero();
			}
	...
	// Beer Shadow Map Management Functions
	TRDGUniformBufferRef<FBeerShadowMapUniformBufferParameters> CreateEmptyBeerShadowMapUniformBuffer(
		FRDGBuilder& GraphBuilder
	)
	{
		FBeerShadowMapUniformBufferParameters* UniformBufferParameters = GraphBuilder.AllocParameters<FBeerShadowMapUniformBufferParameters>();
		{
			UniformBufferParameters->NumShadowMatrices = 1;
			for (int32 i = 0; i < 6; ++i)
			{
			...
			UniformBufferParameters->bIsEmpty = true;
			UniformBufferParameters->bIsDirectionalLight = false;
			UniformBufferParameters->BeerShadowMapTexture = GraphBuilder.CreateSRV(GSystemTextures.GetBlackDummy(GraphBuilder));
			UniformBufferParameters->RadianceTexture = GraphBuilder.CreateSRV(GSystemTextures.GetBlackDummy(GraphBuilder));
			UniformBufferParameters->TextureSampler = TStaticSamplerState<SF_Bilinear>::GetRHI();
		}
	...
	{
		BeerShadowMapUniformBuffer = nullptr;
	}
	...
	{
		bool bIsEnabled = HeterogeneousVolumes::GetShadowType() == HeterogeneousVolumes::EShadowType::BeerShadowMap;
		if (ViewState && LightSceneInfo && bIsEnabled)
		{
			TRDGUniformBufferRef<FBeerShadowMapUniformBufferParameters>* BeerShadowMapUniformBuffer = ViewState->BeerShadowMapUniformBufferMap.Find(LightSceneInfo->Id);
			if (BeerShadowMapUniformBuffer != nullptr)
			{
				return *BeerShadowMapUniformBuffer;
			}
		}

		return CreateEmptyBeerShadowMapUniformBuffer(GraphBuilder);
	}

	TRDGUniformBufferRef<FBeerShadowMapUniformBufferParameters> GetBeerShadowCameraMapUniformBuffer(
		FRDGBuilder& GraphBuilder,
		FSceneViewState* ViewState
	)
	{
		if (ViewState && ShouldCompositeHeterogeneousVolumesWithTranslucency())
		{
			if (ViewState->BeerShadowCameraMapUniformBuffer != nullptr)
			{
				return ViewState->BeerShadowCameraMapUniformBuffer;
			}
			else if (ViewState->BeerShadowCameraMapUniformBufferParameters != nullptr)
			{
				return GraphBuilder.CreateUniformBuffer(ViewState->BeerShadowCameraMapUniformBufferParameters);
			}
		}
...
)
{
	...
	VolumetricShadowLinkedListElementCount *= GroupCount.Z;
...
)
{
	...
	FRenderVolumetricShadowMapForLightWithVoxelGridCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FRenderVolumetricShadowMapForLightWithVoxelGridCS::FParameters>();
	{
		...
		TranslatedWorldOrigin[0] = PassParameters->TranslatedWorldOrigin;
...
)
{
	...
	// Light culling
	TArray<FLightSceneInfoCompact, TInlineAllocator<64>> LightSceneInfoCompact = HeterogeneousVolumes::GatherShadowLights(Scene, View.ViewLightingChannelMask);
...
class FVisualizeCascadesCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FVisualizeCascadesCS);
	SHADER_USE_PARAMETER_STRUCT(FVisualizeCascadesCS, FGlobalShader);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		// Output
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture3D<float>, RWVisualizationTexture)
	END_SHADER_PARAMETER_STRUCT()
...
{
	FRDGTextureDesc VisualizationTextureDesc = SceneTextures.Color.Target->Desc;
	VisualizationTextureDesc.Extent = HeterogeneousVolumes::GetScaledViewRect(View.ViewRect);
	VisualizationTexture = GraphBuilder.CreateTexture(VisualizationTextureDesc, TEXT("HeterogeneousVolumes.VisualizationTexture"));

	FVisualizeCascadesCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FVisualizeCascadesCS::FParameters>();
	{
		// Scene
		PassParameters->View = View.ViewUniformBuffer;
		PassParameters->SceneTextures = GetSceneTextureParameters(GraphBuilder, SceneTextures);
		...
		// Output
		PassParameters->RWVisualizationTexture = GraphBuilder.CreateUAV(VisualizationTexture);
	}

	FIntVector GroupCount = FComputeShaderUtils::GetGroupCount(VisualizationTextureDesc.Extent, FVisualizeCascadesCS::GetThreadGroupSize2D());

	TShaderRef<FVisualizeCascadesCS> ComputeShader = View.ShaderMap->GetShader<FVisualizeCascadesCS>();
	FComputeShaderUtils::AddPass(
		GraphBuilder,
		RDG_EVENT_NAME("FVisualizeCascadesCS"),
		ComputeShader,
		PassParameters,
		GroupCount
	);
