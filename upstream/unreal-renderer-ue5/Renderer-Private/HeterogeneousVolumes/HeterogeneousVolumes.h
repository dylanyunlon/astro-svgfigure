...
namespace HeterogeneousVolumes
{
	...
	// Helper functions to cull lights, used internally
...
BEGIN_UNIFORM_BUFFER_STRUCT(FPreshadingUniformBufferParameters, )
	// Object data
	SHADER_PARAMETER(FMatrix44f, LocalToWorld)
	SHADER_PARAMETER(FMatrix44f, WorldToLocal)
	SHADER_PARAMETER(FVector3f, LocalBoundsOrigin)
	...
	SHADER_PARAMETER(FIntVector, MajorantResolution)
	SHADER_PARAMETER_RDG_TEXTURE(Texture3D, MajorantTexture)
...
struct FHeterogeneousVolumesPrimitiveInstanceKey
{
	uint32 PersistentPrimitiveIndex;
	uint32 InstanceIndex;

	friend FORCEINLINE bool operator == (
		const FHeterogeneousVolumesPrimitiveInstanceKey& A,
		const FHeterogeneousVolumesPrimitiveInstanceKey& B
	)
	{
		return A.PersistentPrimitiveIndex == B.PersistentPrimitiveIndex && A.InstanceIndex == B.InstanceIndex;
	}

	friend FORCEINLINE uint32 GetTypeHash(const FHeterogeneousVolumesPrimitiveInstanceKey& Key)
	{
		return HashCombine(GetTypeHash(Key.PersistentPrimitiveIndex), GetTypeHash(Key.InstanceIndex));
	}
};

struct FPathTracingHeterogeneousVolumesCallableInfo
{
	// Preshading data
	TRefCountPtr<IPooledRenderTarget> MajorantRT;
	TRefCountPtr<IPooledRenderTarget> ExtinctionRT;
	TRefCountPtr<IPooledRenderTarget> EmissionRT;
	TRefCountPtr<IPooledRenderTarget> AlbedoRT;

	// Callable interface
	int32 IntersectShaderId;
	int32 GetDensityBoundsShaderId;
	int32 GetDensityShaderId;
	int32 SampleDistanceShaderId;
	int32 GetTransmittanceShaderId;
};
...
void ComputeHeterogeneousVolumeBakeMaterial(
	...
	bool bJitter,
	bool bLooseBounds,
	bool bEvaluateAlbedo,
	bool bEvaluateEmission,
	// Volume data
...
void ComputeHeterogeneousVolumeBakeMaterial(
	...
	FIntVector VolumeResolution,
	// Output
	TRefCountPtr<IPooledRenderTarget>& HeterogeneousVolumeExtinctionTexture,
	TRefCountPtr<IPooledRenderTarget>& HeterogeneousVolumeEmissionTexture,
	TRefCountPtr<IPooledRenderTarget>& HeterogeneousVolumeAlbedoTexture
);
