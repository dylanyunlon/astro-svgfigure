...
static TAutoConsoleVariable<int32> CVarHeterogeneousVolumes(
	...
	1,
	TEXT("Whether to use Heterogeneous volumes by default (Default = 1)"),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarHeterogeneousVolumesAllowed(
	TEXT("r.HeterogeneousVolumes.Allowed"),
	1,
	TEXT("Whether Heterogeneous Volumes is allowed by scalability and device profiles (Default = 1)"),
...
static bool IsHeterogeneousVolumesEnabled()
{
	return CVarHeterogeneousVolumes.GetValueOnRenderThread() != 0 &&
		CVarHeterogeneousVolumesAllowed.GetValueOnRenderThread() != 0;
}
...
bool ShouldBuildVoxelGrids(const FScene* Scene)
{
	...
	{
		for(const FLightSceneInfoCompact& LightCompactInfo : Scene->Lights)		
		{
			if (LightCompactInfo.LightSceneInfo->Proxy->CastsVolumetricShadow())
			{
				return true;
			}
		}
...
)
{
	...
	if (ShouldRenderHeterogeneousVolumesForAnyView(Views))
	{
		...
		if (IsDeferredPrimitiveAlphaHoldoutSupported())
		{
			Desc.Format = PF_R8;
			...
			else
			{
				...
				for (int32 VolumeIndex = 0; VolumeIndex < VolumeMeshes.Num(); ++VolumeIndex)
				{
					...
					else
					{
						...
						if (View.ViewState)
						{
							// Build a camera-space Beer Shadow Map uniform buffer for compositing.
							if (ShouldCompositeHeterogeneousVolumesWithTranslucency() &&
								HeterogeneousVolumes::GetTranslucencyCompositingMode() == HeterogeneousVolumes::EShadowType::BeerShadowMap)
							{
								// Resolution
								...
								CreateBeerShadowMapUniformBufferParameters(
									...
									bIsDirectionalLight,
									HeterogeneousVolumeBeerShadowMap,
									View.ViewState->BeerShadowCameraMapUniformBufferParameters
								);
							}

							// Append beauty image and build uniform buffer (AVSM path)
							...
							// Append beauty image and build uniform buffer (BSM path)
							if (ShouldCompositeHeterogeneousVolumesWithTranslucency() && View.ViewState->BeerShadowCameraMapUniformBufferParameters)
							{
								View.ViewState->BeerShadowCameraMapUniformBufferParameters->RadianceTexture = GraphBuilder.CreateSRV(HeterogeneousVolumeRadiance);
								View.ViewState->BeerShadowCameraMapUniformBufferParameters->TextureSampler = TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();

								View.ViewState->BeerShadowCameraMapUniformBuffer =
									GraphBuilder.CreateUniformBuffer(View.ViewState->BeerShadowCameraMapUniformBufferParameters);
							}
						}
					}
				}
...
class FHeterogeneousVolumesCompositeCS : public FGlobalShader
{
	...
	class FShadowMethodDim : SHADER_PERMUTATION_INT("SHADOW_METHOD", 3);
	using FPermutationDomain = TShaderPermutationDomain<FShadowMethodDim>;
...
)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		if (ShouldRenderHeterogeneousVolumesForView(Views, ViewIndex))
		{
			...
			// Composite
			{
				const bool bComposite = ShouldCompositeHeterogeneousVolumesWithTranslucency();
				const HeterogeneousVolumes::EShadowType CompositingMode = HeterogeneousVolumes::GetTranslucencyCompositingMode();
				const int32 ShadowMethod = bComposite
					? (CompositingMode == HeterogeneousVolumes::EShadowType::BeerShadowMap ? 2 : 1)
					: 0;
				...
				FHeterogeneousVolumesCompositeCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FHeterogeneousVolumesCompositeCS::FParameters>();
				{
					...
					// Transmittance structure: bind the relevant uniform buffer for the selected method, empty otherwise.
					PassParameters->AVSM = (ShadowMethod == 1)
						? HeterogeneousVolumes::GetAdaptiveVolumetricCameraMapUniformBuffer(GraphBuilder, View.ViewState)
						: HeterogeneousVolumes::CreateEmptyAdaptiveVolumetricShadowMapUniformBuffer(GraphBuilder);
					PassParameters->BeerShadowMap = (ShadowMethod == 2)
						? HeterogeneousVolumes::GetBeerShadowCameraMapUniformBuffer(GraphBuilder, View.ViewState)
						: HeterogeneousVolumes::CreateEmptyBeerShadowMapUniformBuffer(GraphBuilder);
					// Dispatch data
				...
				FHeterogeneousVolumesCompositeCS::FPermutationDomain PermutationVector;
				PermutationVector.Set<FHeterogeneousVolumesCompositeCS::FShadowMethodDim>(ShadowMethod);
				TShaderRef<FHeterogeneousVolumesCompositeCS> ComputeShader = View.ShaderMap->GetShader<FHeterogeneousVolumesCompositeCS>(PermutationVector);
				FComputeShaderUtils::AddPass(
					GraphBuilder,
...
namespace HeterogeneousVolumes
{
	TArray<FLightSceneInfoCompact, TInlineAllocator<64>> GatherLights(const FScene* Scene, const FViewInfo& View, const IHeterogeneousVolumeInterface* HeterogeneousVolumeInterface)
	{
		TArray<FLightSceneInfoCompact, TInlineAllocator<64>> LightSceneInfoCompact;
		for (const FLightSceneInfoCompact& LightCompactInfo : Scene->Lights)
		{
			if (SupportsLightType(LightCompactInfo.LightType) &&
				(LightCompactInfo.LightSceneInfo->Proxy->GetViewLightingChannelMask() & View.ViewLightingChannelMask) &&
				LightCompactInfo.AffectsPrimitive(HeterogeneousVolumeInterface->GetBounds(), HeterogeneousVolumeInterface->GetPrimitiveSceneProxy()))
			{
				LightSceneInfoCompact.Add(LightCompactInfo);
			}
		}
		return LightSceneInfoCompact;
	}

	TArray<FLightSceneInfoCompact, TInlineAllocator<64>> GatherShadowLights(const FScene* Scene, uint32 ViewLightingChannelMask)
	{
		TArray<FLightSceneInfoCompact, TInlineAllocator<64>> LightSceneInfoCompact;
		for (const FLightSceneInfoCompact& LightCompactInfo : Scene->Lights)
		{
			// TODO: Use global bounds information..
			//if (LightCompactInfo.AffectsPrimitive(...))
			// TODO: For view/light masking, should we check if the light is visible in any view,
			// instead of just the first view? For now, we assume all views share the same channel mask.
			if (SupportsShadowForLightType(LightCompactInfo.LightType) &&
				(LightCompactInfo.LightSceneInfo->Proxy->GetViewLightingChannelMask() & ViewLightingChannelMask))
			{
				LightSceneInfoCompact.Add(LightCompactInfo);
			}
		}
	...
	void PostRender(FScene& Scene, TArray<FViewInfo>& Views)
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			if (ViewState)
			{
				...
				ViewState->BeerShadowCameraMapUniformBufferParameters = nullptr;
				DestroyBeerShadowMapUniformBuffer(ViewState->BeerShadowCameraMapUniformBuffer);

				for (TPair<FLightSceneId, TRDGUniformBufferRef<FAdaptiveVolumetricShadowMapUniformBufferParameters>>& Pair : ViewState->AdaptiveVolumetricShadowMapUniformBufferMap)
				{
