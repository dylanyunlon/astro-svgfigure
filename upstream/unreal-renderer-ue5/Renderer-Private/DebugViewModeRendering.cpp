...
int32 GShaderComplexityBaselineMobileForwardVS = 134;
static FAutoConsoleVariableRef CVarShaderComplexityBaselineMobileForwardVS(
	TEXT("r.ShaderComplexity.Baseline.MobileForward.VS"),
	GShaderComplexityBaselineMobileForwardVS,
	TEXT("Minimum number of instructions for vertex shaders in Mobile forward shading (default=134)"),
	ECVF_Default
);

int32 GShaderComplexityBaselineMobileForwardPS = 143;
static FAutoConsoleVariableRef CVarShaderComplexityBaselineMobileForwardPS(
	TEXT("r.ShaderComplexity.Baseline.MobileForward.PS"),
	GShaderComplexityBaselineMobileForwardPS,
	TEXT("Minimum number of instructions for pixel shaders in Mobile forward shading (default=143)"),
	ECVF_Default
);

int32 GShaderComplexityBaselineMobileForwardUnlitPS = 6;
static FAutoConsoleVariableRef CVarShaderComplexityBaselineMobileForwardUnlitPS(
	TEXT("r.ShaderComplexity.Baseline.MobileForward.UnlitPS"),
	GShaderComplexityBaselineMobileForwardUnlitPS,
	TEXT("Minimum number of instructions for unlit material pixel shaders in Mobile forward shading (default=6)"),
	ECVF_Default
);

int32 GShaderComplexityBaselineMobileDeferredVS = 134;
static FAutoConsoleVariableRef CVarShaderComplexityBaselineMobileDeferredVS(
	TEXT("r.ShaderComplexity.Baseline.MobileDeferred.VS"),
	GShaderComplexityBaselineMobileDeferredVS,
	TEXT("Minimum number of instructions for vertex shaders in MObile deferred shading (default=134)"),
	ECVF_Default
);

int32 GShaderComplexityBaselineMobileDeferredPS = 50;
static FAutoConsoleVariableRef CVarShaderComplexityBaselineMobileDeferredPS(
	TEXT("r.ShaderComplexity.Baseline.MobileDeferred.PS"),
	GShaderComplexityBaselineMobileDeferredPS,
	TEXT("Minimum number of instructions for pixel shaders in Mobile deferred shading (default=50)"),
	ECVF_Default
);

int32 GShaderComplexityBaselineMobileDeferredUnlitPS = 9;
static FAutoConsoleVariableRef CVarShaderComplexityBaselineMobileDeferredUnlitPS(
	TEXT("r.ShaderComplexity.Baseline.MobileDeferred.UnlitPS"),
	GShaderComplexityBaselineMobileDeferredUnlitPS,
	TEXT("Minimum number of instructions for unlit material pixel shaders in Mobile deferred shading (default=9)"),
	ECVF_Default
);
...
void SetupDebugViewModePassUniformBufferConstants(const FViewInfo& ViewInfo, FDebugViewModeUniformParameters& Parameters)
{
	...
	// LOD / HLOD colors
	{
		...
		else if (ViewInfo.Family->EngineShowFlags.StreamingTextureDeficit
		      || ViewInfo.Family->EngineShowFlags.StreamingTextureResidency
		      || ViewInfo.Family->EngineShowFlags.StreamingMeshLODDeficit
		      || ViewInfo.Family->EngineShowFlags.StreamingMeshLODResidency)
		{
			Colors = &GEngine->StreamingDeficitColors;
		}
...
{
	if (ForceEnableDebugViewSupport(Parameters.Platform))
	{
		return true;
	}

	if (ForceDisableDebugViewSupport(Parameters.Platform))
	{
		return false;
	}

	if (Parameters.VertexFactoryType && Parameters.VertexFactoryType->SupportsNaniteRendering())
	{
		return false;
	}
...
void ClearDebugAux(FRDGBuilder& GraphBuilder, const FViewFamilyInfo& ViewFamily, FRDGTextureRef DebugAuxTexture)
{
	if (DebugAuxTexture)
	{
		...
		else if (DebugViewMode == DVSM_ShadowCasters || DebugViewMode == DVSM_FarShadowCasters)
		{
...
void RenderDebugViewMode(FRDGBuilder& GraphBuilder, TArrayView<FViewInfo> Views, FRDGTextureRef DebugAuxTexture, const FRenderTargetBinding& SceneColorRTBinding, const FDepthStencilBinding& SceneDepthBinding)
{
	...
	else if (DebugViewMode == EDebugViewShaderMode::DVSM_ShadowCasters || DebugViewMode == EDebugViewShaderMode::DVSM_FarShadowCasters)
	{
...
void FDebugViewModePS::ModifyCompilationEnvironment(const FMaterialShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	OutEnvironment.SetDefine(TEXT("NUM_LOD_COLORATION_COLORS"), (uint32)NumLODColorationColors);
...
/** Compute the worst-case texture streaming deficit across all textures in a material (uncached). */
int32 ComputeTextureStreamingDeficitForMaterial(const FMaterialRenderProxy& MaterialRenderProxy, const FMaterial& Material, bool bUseWantedMips)
{
	...
	for (EMaterialTextureParameterType TextureType : TextureTypes)
	{
		for (int32 ParameterIndex = 0; ParameterIndex < UniformExpressions.GetNumTextures(TextureType); ++ParameterIndex)
		{
			...
			if (!State.bSupportsStreaming || State.MaxNumLODs == 0)
			{
				continue;
			}
			...
			if (bUseWantedMips)
			{
				const uint8 WantedLODs = Texture->GetCachedNumWantedLODs();
				Deficit = (WantedLODs > 0) ? FMath::Max(0, (int32)WantedLODs - (int32)State.NumResidentLODs) : 0;
			}
			else
			{
				Deficit = FMath::Max(0, (int32)State.MaxNumLODs - (int32)State.NumResidentLODs);
			}

			MaxDeficit = FMath::Max(MaxDeficit, Deficit);
		}
	}
	return FMath::Clamp(MaxDeficit, 0, NumLODColorationColors - 1);
#else
	return 0;
#endif
}

/** Cached lookup: returns per-proxy texture streaming deficit, computing only on first access. */
static int32 ComputeTextureStreamingDeficit(const FMaterialRenderProxy& MaterialRenderProxy, const FMaterial& Material, bool bUseWantedMips, TMap<const FMaterialRenderProxy*, int32>& Cache)
{
#if !UE_BUILD_SHIPPING
	TRACE_CPUPROFILER_EVENT_SCOPE(ComputeTextureStreamingDeficit);

	// Cache by FMaterialRenderProxy
	const FMaterialRenderProxy* CacheKey = &MaterialRenderProxy;
	if (int32* Cached = Cache.Find(CacheKey))
	{
		return *Cached;
	}

	int32 Result = ComputeTextureStreamingDeficitForMaterial(MaterialRenderProxy, Material, bUseWantedMips);
...
/** Compute mesh LOD streaming deficit from the scene proxy. */
static int32 ComputeMeshStreamingDeficit(const FPrimitiveSceneProxy* PrimitiveSceneProxy, bool bUseWantedMips)
{
#if !UE_BUILD_SHIPPING
	TRACE_CPUPROFILER_EVENT_SCOPE(ComputeMeshStreamingDeficit);
	...
	if (bUseWantedMips)
	{
		uint8 Deficit = PrimitiveSceneProxy->GetStreamingLODWantedDeficit();
		return FMath::Clamp((int32)Deficit, 0, NumLODColorationColors - 1);
	}
	else
	{
		// Absolute residency: FirstLODIdx == number of missing LODs from the top
		uint8 FirstAvailableLOD = PrimitiveSceneProxy->GetCurrentFirstLODIdx_RenderThread();
		return FMath::Clamp((int32)FirstAvailableLOD, 0, NumLODColorationColors - 1);
	}
#else
	return 0;
#endif
...
void FDebugViewModeMeshProcessor::AddMeshBatch(const FMeshBatch& RESTRICT MeshBatch, uint64 BatchElementMask, const FPrimitiveSceneProxy* RESTRICT PrimitiveSceneProxy, int32 StaticMeshId)
{
	...
	const bool bDitheredLODTransition = MeshBatch.bDitheredLODTransition && MeshBatch.CachedLODDither != FMeshBatch::EDitherState::None;

	FMeshPassProcessorRenderState DrawRenderState;
	FDebugViewModeInterface::FRenderState InterfaceRenderState;
	DebugViewModeInterface->SetDrawRenderState(DebugViewMode, BatchMaterial->GetBlendMode(), InterfaceRenderState, Scene ? (Scene->GetShadingPath() == EShadingPath::Deferred && Scene->EarlyZPassMode != DDM_NonMaskedOnly) : false, bDitheredLODTransition);
	...
	int32 DebugLODIndex = MeshBatch.VisualizeLODIndex;
	if (ViewIfDynamicMeshCommand)
	{
		const FEngineShowFlags& ViewFlags = ViewIfDynamicMeshCommand->Family->EngineShowFlags;
		if (ViewFlags.StreamingTextureDeficit)
		{
			DebugLODIndex = ComputeTextureStreamingDeficit(*MaterialRenderProxy, *Material, /*bUseWantedMips=*/ true, TextureStreamingDeficitCache);
		}
		else if (ViewFlags.StreamingTextureResidency)
		{
			DebugLODIndex = ComputeTextureStreamingDeficit(*MaterialRenderProxy, *Material, /*bUseWantedMips=*/ false, TextureStreamingDeficitCache);
		}
		else if (ViewFlags.StreamingMeshLODDeficit)
		{
			DebugLODIndex = ComputeMeshStreamingDeficit(PrimitiveSceneProxy, /*bUseWantedMips=*/ true);
		}
		else if (ViewFlags.StreamingMeshLODResidency)
		{
			DebugLODIndex = ComputeMeshStreamingDeficit(PrimitiveSceneProxy, /*bUseWantedMips=*/ false);
		}
		else if (ViewFlags.HLODColoration)
		{
			DebugLODIndex = MeshBatch.VisualizeHLODIndex;
		}
	}
...
void FDebugViewModeMeshProcessor::UpdateInstructionCount(FDebugViewModeShaderElementData& OutShaderElementData, const FMaterial* InBatchMaterial, FVertexFactoryType* InVertexFactoryType)
{
	...
	if (Scene)
	{
		...
		if (Scene->GetShadingPath() == EShadingPath::Deferred)
		{
			...
			ShaderTypes.AddShaderType<TBasePassPS<TUniformLightMapPolicy<LMP_NO_LIGHTMAP>, GBL_Default>>();
		...
		else // EShadingPath::Mobile
		{
			...
			if (MobileBasePass::GetShaders(FUniformLightMapPolicy(LMP_NO_LIGHTMAP), EMobileLocalLightSetting::LOCAL_LIGHTS_DISABLED, *InBatchMaterial, InVertexFactoryType, MobileVS, MobilePS))
			{
			...
			if (IsMobileDeferredShadingEnabled(ShaderPlatform) == false)
			{
				OutShaderElementData.NumVSInstructions -= GShaderComplexityBaselineMobileForwardVS - GShaderComplexityBaselineMobileDeferredVS;
				OutShaderElementData.NumPSInstructions -= InBatchMaterial->GetShadingModels().IsLit() ?
					(GShaderComplexityBaselineMobileForwardPS - GShaderComplexityBaselineMobileDeferredPS) : 
					(GShaderComplexityBaselineMobileForwardUnlitPS - GShaderComplexityBaselineMobileDeferredUnlitPS);

				OutShaderElementData.NumVSInstructions = FMath::Max(OutShaderElementData.NumVSInstructions, 0);
				OutShaderElementData.NumPSInstructions = FMath::Max(OutShaderElementData.NumPSInstructions, 0);
			}
