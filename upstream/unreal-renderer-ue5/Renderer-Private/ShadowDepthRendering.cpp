...
static TAutoConsoleVariable<int32> CVarShouldBeginDeferredCullingAfterShadowRendering(
	...
	1,
	TEXT("If turned on (default), a new deferred scope of instance culling will be started directly after shadow depth rendering (iff instance occlusion culling is enabled).\n")\
	TEXT("  This has a mild performance benefit on some platforms."),
	ECVF_RenderThreadSafe);
	...
	FShadowDepthPassUniformParameters& ShadowDepthPassParameters)
{
	...
	ShadowDepthPassParameters.ShadowDepthType = (uint32)ShadowInfo->GetShadowDepthType();
...
enum class EShadowDepthShaderMode
{
	PerspectiveCorrect,
	OutputDepth,
	OnePassPointLight,
...
// Returns bitfield of shadow depth types supported by the given shader mode
// NOTE: By default any modes covered by the "Combined" mode will not be included as separate shader permutations,
// and instead that mode will be used for any passes requesting the individual modes.
static constexpr EShadowDepthType GetShadowDepthTypesForMode(EShadowDepthShaderMode Mode)
{
	switch (Mode)
	{
		case EShadowDepthShaderMode::PerspectiveCorrect: 	return EShadowDepthType::Spot;
		case EShadowDepthShaderMode::OutputDepth: 			return EShadowDepthType::Directional;
		case EShadowDepthShaderMode::OnePassPointLight: 	return EShadowDepthType::OnePassPoint;
		case EShadowDepthShaderMode::VirtualShadowMap: 		return EShadowDepthType::VSM;
		case EShadowDepthShaderMode::Combined: 				return EShadowDepthType::VSM | EShadowDepthType::OnePassPoint;
		default: return EShadowDepthType::None;
	}
}

static constexpr bool IsShadowDepthTypeInCombinedMode(EShadowDepthType Types)
{
	EShadowDepthType CombinedShadowDepthTypes = GetShadowDepthTypesForMode(EShadowDepthShaderMode::Combined);
	// NOTE: This is slightly messy, but the shadow depth types for mesh passes get set to "VSM | Spot" or similar
	// in the VSM case. In these cases we only need the VSM permutation, so using "Any" allows them to correctly
	// route down to the combined permutation.
	return EnumHasAnyFlags(CombinedShadowDepthTypes, Types);
}

static constexpr EShadowDepthType GetShadowDepthTypesThatRequirePS()
{
	return EShadowDepthType::VSM | EShadowDepthType::Spot;
}

// Returns a bitfield of shadow depth types supported on the given platform
// TODO: Cache result for current platform...?
EShadowDepthType GetPlatformSupportedShadowDepthTypes(EShaderPlatform Platform)
{
	...
	EShadowDepthType Types = EShadowDepthType::None;

	if (!IsFeatureLevelSupported(Platform, ERHIFeatureLevel::SM5))
	{
		// Mobile only needs OutputDepth, and optionally PerspectiveCorrect and OnePassPointLight
		Types |= EShadowDepthType::Directional
			| (MobileUsesPerspectiveCorrectShadowPermutation(Platform) ? EShadowDepthType::Spot : EShadowDepthType::None)
			| (FReadOnlyCVARCache::EnablePointLightShadows(Platform) && bShouldCompileVSLayer ? EShadowDepthType::OnePassPoint : EShadowDepthType::None);
	}
	else
	{
		Types |= EShadowDepthType::Directional
			| EShadowDepthType::Spot
			| (bShouldCompileVSLayer ? EShadowDepthType::OnePassPoint : EShadowDepthType::None)
			| (DoesPlatformSupportNonNaniteVirtualShadowMaps(Platform) ? EShadowDepthType::VSM : EShadowDepthType::None);
	}

	// If classic shadow maps are disabled at the platform/project level, remove those permutations
	if (!DoesPlatformSupportClassicShadowMaps(Platform))
	{
		Types &= ~(EShadowDepthType::Directional | EShadowDepthType::Spot | EShadowDepthType::OnePassPoint);
	}

	return Types;
}

// Get the "effective" supported shadow depth types for a given mode given both the mode and platform support.
EShadowDepthType GetEffectiveShadowDepthTypes(EShaderPlatform Platform, EShadowDepthShaderMode Mode)
{
	return GetShadowDepthTypesForMode(Mode) & GetPlatformSupportedShadowDepthTypes(Platform);
}

void SetEnvironmentDefinesForShadowDepthTypes(FShaderCompilerEnvironment& OutEnvironment, EShadowDepthType Types)
{
	OutEnvironment.SetDefine(TEXT("SUPPORT_SHADOW_DEPTH_VSM"), 			EnumHasAnyFlags(Types, EShadowDepthType::VSM));
	OutEnvironment.SetDefine(TEXT("SUPPORT_SHADOW_DEPTH_DIRECTIONAL"), 	EnumHasAnyFlags(Types, EShadowDepthType::Directional));
	OutEnvironment.SetDefine(TEXT("SUPPORT_SHADOW_DEPTH_SPOT"), 		EnumHasAnyFlags(Types, EShadowDepthType::Spot));
	OutEnvironment.SetDefine(TEXT("SUPPORT_SHADOW_DEPTH_POINT"), 		EnumHasAnyFlags(Types, EShadowDepthType::OnePassPoint));
}

/**
* A vertex shader for rendering the depth of a mesh.
*/
template <EShadowDepthShaderMode ShaderMode, bool bUsePositionOnlyStream>
...
public:
	...
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		EShadowDepthType ShadowDepthTypes = GetEffectiveShadowDepthTypes(Parameters.Platform, ShaderMode);
		if (ShadowDepthTypes == EShadowDepthType::None)
		{
			return false;
		}

		// Don't include this permutation if it is not the combined mode, but the depth types are covered by it
		if (ShaderMode != EShadowDepthShaderMode::Combined && IsShadowDepthTypeInCombinedMode(ShadowDepthTypes))
		{
			return false;
		}
	...
	static void ModifyCompilationEnvironment(const FMaterialShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		EShadowDepthType Types = GetEffectiveShadowDepthTypes(Parameters.Platform, ShaderMode);
		check(Types != EShadowDepthType::None);		// Should have been killed by ShouldCompilePermutation
		SetEnvironmentDefinesForShadowDepthTypes(OutEnvironment, Types);

		OutEnvironment.SetDefine(TEXT("POSITION_ONLY"), (uint32)bUsePositionOnlyStream);

		// TransformShadowDirectional in ShadowDepthVertexShader.usf uses a "precise" modifier on its final depth output to avoid
		// shadow-triangle clipping artifacts that we historically saw on D3D.  DXIL validation forbids "precise" instructions
		// from referring to min16float operands, and adding "precise" on an output forces the compiler to implicitly tag
		// all calculations feeding into it as precise as well.  Half precision math can come from using half precision tangents
		// or normals to compute WPO (which is a fairly common use case).  So FORCE_FLOATS needs to be set at the same time, to
		// guarantee no half precision math in the shader, which is why setting PLATFORM_NEEDS_PRECISE_SHADOW_DEPTH has been moved
		// here from D3DCommon.ush, so we can set both defines narrowly, only for the shadow VS permutations where it applies.
		if (IsD3DPlatform(Parameters.Platform) && EnumHasAnyFlags(Types, EShadowDepthType::Directional))
		{
			OutEnvironment.SetDefine(TEXT("PLATFORM_NEEDS_PRECISE_SHADOW_DEPTH"), 1);
			OutEnvironment.SetDefine(TEXT("FORCE_FLOATS"), 1);
		}

		if (EnumHasAnyFlags(Types, EShadowDepthType::VSM))
		{
			FVirtualShadowMapArray::SetShaderDefines(OutEnvironment);
		}

		if (EnumHasAnyFlags(Types, EShadowDepthType::OnePassPoint))
		{
			// TODO: This is possibly harmful for other shadow depth modes in the combined case
...
#define IMPLEMENT_SHADOW_DEPTH_SHADERMODE_SHADERS(ShaderMode) \
	typedef TShadowDepthVS<EShadowDepthShaderMode::ShaderMode, false> TShadowDepthVS##ShaderMode; \
	...
	typedef TShadowDepthVS<EShadowDepthShaderMode::ShaderMode, true> TShadowDepthVSPositionOnly##ShaderMode; \
	IMPLEMENT_MATERIAL_SHADER_TYPE(template<>, TShadowDepthVSPositionOnly##ShaderMode, TEXT("/Engine/Private/ShadowDepthVertexShader.usf"), TEXT("PositionOnlyMain"), SF_Vertex);

IMPLEMENT_SHADOW_DEPTH_SHADERMODE_SHADERS(PerspectiveCorrect);
IMPLEMENT_SHADOW_DEPTH_SHADERMODE_SHADERS(OutputDepth);
IMPLEMENT_SHADOW_DEPTH_SHADERMODE_SHADERS(OnePassPointLight);
IMPLEMENT_SHADOW_DEPTH_SHADERMODE_SHADERS(VirtualShadowMap);
IMPLEMENT_SHADOW_DEPTH_SHADERMODE_SHADERS(Combined);
...
template <EShadowDepthShaderMode ShaderMode>
...
public:
	...
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		...
		const bool bModeRequiresPS = EnumHasAnyFlags(GetShadowDepthTypesThatRequirePS(), ShadowDepthTypes);
...
#define IMPLEMENT_SHADOWDEPTHPASS_PIXELSHADER_TYPE(ShaderMode) \
	typedef TShadowDepthPS<EShadowDepthShaderMode::ShaderMode> TShadowDepthPS##ShaderMode; \
...
IMPLEMENT_SHADOWDEPTHPASS_PIXELSHADER_TYPE(PerspectiveCorrect);
IMPLEMENT_SHADOWDEPTHPASS_PIXELSHADER_TYPE(OutputDepth);
IMPLEMENT_SHADOWDEPTHPASS_PIXELSHADER_TYPE(OnePassPointLight);
IMPLEMENT_SHADOWDEPTHPASS_PIXELSHADER_TYPE(VirtualShadowMap);
IMPLEMENT_SHADOWDEPTHPASS_PIXELSHADER_TYPE(Combined);
	...
	TShaderRef<FShadowDepthBasePS>& PixelShader)
{
	...
	const bool bUseCombinedMode = IsShadowDepthTypeInCombinedMode(ShadowDepthType);
	...
	if (bUseCombinedMode)
	{
		if (bPositionOnlyVS)
		{
			ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::Combined, true>>();
		}
		else
		{
			ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::Combined, false>>();
		}
	}
	else if (EnumHasAnyFlags(ShadowDepthType, EShadowDepthType::VSM))
	{
		if (bPositionOnlyVS)
		{
			ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::VirtualShadowMap, true>>();
		}
		else
		{
			ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::VirtualShadowMap, false>>();
		}
	}
	else
	{
		// Classic shadow map techniques. If disabled, we bail as we can't render this batch.
		if (!DoesPlatformSupportClassicShadowMaps(Platform))
		{
			return false;
		}
		...
		{
			if (DoesRuntimeSupportOnePassPointLightShadows(Platform))
			{
				if (bPositionOnlyVS)
				{
					ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::OnePassPointLight, true>>();
				}
				else
				{
					ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::OnePassPointLight, false>>();
				}
			}
			else
			{
		...
		else if (EnumHasAnyFlags(ShadowDepthType, EShadowDepthType::Spot))
		{
			if (bPositionOnlyVS)
			{
				ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::PerspectiveCorrect, true>>();
			}
			else
			{
				ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::PerspectiveCorrect, false>>();
			}
		}
		else if (EnumHasAnyFlags(ShadowDepthType, EShadowDepthType::Directional))
		{
			if (bPositionOnlyVS)
			{
				ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::OutputDepth, true>>();
			}
			else
			{
				ShaderTypes.AddShaderType<TShadowDepthVS<EShadowDepthShaderMode::OutputDepth, false>>();
			}
		}
		else
		{
	...
	// Pixel shaders
	const bool bNullPixelShader = 
		bVFTypeSupportsNullPixelShader &&
		Material.WritesEveryPixel(true, bVFTypeSupportsNullPixelShader) &&
		!EnumHasAnyFlags(GetShadowDepthTypesThatRequirePS(), ShadowDepthType);

	if (!bNullPixelShader)
	{
		// TODO: Move this around to the same branch above, just respect NullPixelShader as appropriate
		if (bUseCombinedMode)
		{
			ShaderTypes.AddShaderType<TShadowDepthPS<EShadowDepthShaderMode::Combined>>();
		}
		else if (EnumHasAnyFlags(ShadowDepthType, EShadowDepthType::VSM))
		{
			ShaderTypes.AddShaderType<TShadowDepthPS<EShadowDepthShaderMode::VirtualShadowMap>>();
		}
		else if (EnumHasAnyFlags(ShadowDepthType, EShadowDepthType::OnePassPoint))
		{
			ShaderTypes.AddShaderType<TShadowDepthPS<EShadowDepthShaderMode::OnePassPointLight>>();
		}
		else if (EnumHasAnyFlags(ShadowDepthType, EShadowDepthType::Spot))
		{
			ShaderTypes.AddShaderType<TShadowDepthPS<EShadowDepthShaderMode::PerspectiveCorrect>>();
		}
		else
		{
			ShaderTypes.AddShaderType<TShadowDepthPS<EShadowDepthShaderMode::OutputDepth>>();
		}
	}
...
void FProjectedShadowInfo::SetStateForView(FRHICommandList& RHICmdList, bool bRequiresBorderClamp) const
{
	...
	else if (bRequiresBorderClamp)
	{
		...
		RHICmdList.SetScissorRect(
			...
			X + BorderSize,
			Y + BorderSize,
			X + ResolutionX + BorderSize,
			Y + ResolutionY + BorderSize
		);
	}
}
	...
	const FMeshPassProcessorRenderState& DrawRenderState)
{
	...
	const FCachedShadowMapData& CachedShadowMapData = SceneRenderer->Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDataRef(GetLightSceneInfo().Id, FMath::Max(CascadeSettings.ShadowSplitIndex, 0));
	...
	bool bDoCrossGPUCopy)
{
	...
	if (GetEmitDrawEvents())
	{
		...
		EventName += FString(" ") + FString::FromInt(ResolutionX) + TEXT("x") + FString::FromInt(ResolutionY);
	}
	...
	if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0 && bDirectionalLight && CascadeSettings.ShadowSplitIndex >= 0)
	{
		FCSMSceneExtension& CSMExt = Scene->GetExtension<FCSMSceneExtension>();
		int32 ShadowDepthReuseIndex = GetCSMShadowDepthReuseIndex();
		const FCSMDepthReuse* CSMDepthReuse = CSMExt.GetCSMDepthReuse(GetLightSceneInfo().Id, ShadowDepthReuseIndex);

		if (!ShouldCSMRenderDepthThisFrame(Scene, CSMDepthReuse))
		{
			if (CSMDepthReuse->bHasEverRendered)
			{
				// This cascade is reusing last frame's depth.
				// Restore previous transforms from last time renderdepth was successful and skip rendering.
				MaxSubjectZ = CSMDepthReuse->MaxSubjectZ;
				MinSubjectZ = CSMDepthReuse->MinSubjectZ;
				PreShadowTranslation = CSMDepthReuse->PreShadowTranslation;

				TranslatedWorldToClipInnerMatrix = CSMDepthReuse->TranslatedWorldToClipInnerMatrix;
				TranslatedWorldToClipOuterMatrix = CSMDepthReuse->TranslatedWorldToClipOuterMatrix;

				X = CSMDepthReuse->X;
				Y = CSMDepthReuse->Y;

				// Reproject the frozen cascade depth range into the current view to keep shadow coverage world-space-anchored to the frozen frustum
				if (DependentView != nullptr && !CSMDepthReuse->FrozenViewForward.IsNearlyZero())
				{
					const FVector CurOrigin   = FVector(DependentView->ViewMatrices.GetViewOrigin());
					const float   DepthOffset = FVector::DotProduct(CurOrigin - CSMDepthReuse->FrozenViewOrigin, CSMDepthReuse->FrozenViewForward);

					const float ProjectedNear = CSMDepthReuse->FrozenSplitNear - DepthOffset;
					const float ProjectedFar  = CSMDepthReuse->FrozenSplitFar  - DepthOffset;

					CascadeSettings.SplitNear = FMath::Max(0.0f, FMath::Min(ProjectedNear, ProjectedFar));
					CascadeSettings.SplitFar  = FMath::Max(CascadeSettings.SplitNear, FMath::Max(ProjectedNear, ProjectedFar));

					if (CSMDepthReuse->FrozenFadePlaneLength > 0.0f)
					{
						const float ProjectedFade = CSMDepthReuse->FrozenFadePlaneOffset - DepthOffset;
						CascadeSettings.FadePlaneOffset = FMath::Clamp(ProjectedFade, CascadeSettings.SplitNear, CascadeSettings.SplitFar);
						CascadeSettings.FadePlaneLength = CascadeSettings.SplitFar - CascadeSettings.FadePlaneOffset;
					}
					else
					{
						CascadeSettings.FadePlaneOffset = CascadeSettings.SplitFar;
						CascadeSettings.FadePlaneLength = 0.0f;
					}
				}
				else
				{
					CascadeSettings.SplitNear       = CSMDepthReuse->FrozenSplitNear;
					CascadeSettings.SplitFar        = CSMDepthReuse->FrozenSplitFar;
					CascadeSettings.FadePlaneOffset = CSMDepthReuse->FrozenFadePlaneOffset;
					CascadeSettings.FadePlaneLength = CSMDepthReuse->FrozenFadePlaneLength;
				}

				// For directional lights, InvMaxSubjectDepth is always 1.0f
	...
	bool bRequiresBorderClamp = !SupportsShadowStencilCulling(SceneRenderer->ShaderPlatform);
	...
	// Successful renderdepth! Store CSM unified atlas for reuse
	if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0 && bDirectionalLight && CascadeSettings.ShadowSplitIndex >= 0)
	{
		FShadowMapRenderTargetsRefCounted* AtlasRenderTarget = nullptr;
		for (const FSortedShadowMapAtlas& ShadowMapAtlas : SceneRenderer->SortedShadowsForShadowDepthPass.ShadowMapAtlases)
		{
			if (ShadowMapAtlas.Shadows.Contains(this))
			{
				AtlasRenderTarget = const_cast<FShadowMapRenderTargetsRefCounted*>(&ShadowMapAtlas.RenderTargets);
				break;
			}
		}

		if (AtlasRenderTarget != nullptr && AtlasRenderTarget->IsValid())
		{
			FCSMSceneExtension& CSMExt = Scene->GetExtension<FCSMSceneExtension>();
			int32 ShadowDepthReuseIndex = GetCSMShadowDepthReuseIndex();
			FCSMDepthReuse& CSMDepthReuseRef = CSMExt.GetCSMDepthReuseRef(GetLightSceneInfo().Id, ShadowDepthReuseIndex);

			CSMDepthReuseRef.PreviousAtlas = *AtlasRenderTarget;
			CSMDepthReuseRef.bHasEverRendered = true;
			CSMDepthReuseRef.PreviousRenderDepthFrameNumber = Scene->GetFrameNumberRenderThread();

			CSMDepthReuseRef.MaxSubjectZ = MaxSubjectZ;
			CSMDepthReuseRef.MinSubjectZ = MinSubjectZ;
			CSMDepthReuseRef.PreShadowTranslation = PreShadowTranslation;
			CSMDepthReuseRef.TranslatedWorldToClipInnerMatrix = TranslatedWorldToClipInnerMatrix;
			CSMDepthReuseRef.TranslatedWorldToClipOuterMatrix = TranslatedWorldToClipOuterMatrix;
			CSMDepthReuseRef.FrozenSplitFar       = CascadeSettings.SplitFar;
			CSMDepthReuseRef.FrozenSplitNear       = CascadeSettings.SplitNear;
			CSMDepthReuseRef.FrozenFadePlaneOffset = CascadeSettings.FadePlaneOffset;
			CSMDepthReuseRef.FrozenFadePlaneLength = CascadeSettings.FadePlaneLength;
			CSMDepthReuseRef.X = X;
			CSMDepthReuseRef.Y = Y;
			if (DependentView != nullptr)
			{
				CSMDepthReuseRef.FrozenViewOrigin = FVector(DependentView->ViewMatrices.GetViewOrigin());
				CSMDepthReuseRef.FrozenViewForward = DependentView->GetViewDirection();
			}

			CSMExt.NotifyCSMDepthRendered(GetLightSceneInfo().Id);
		}
	}
}
...
{
	float Result = GFarShadowLODDistanceFactor;
	if (CascadeSettings.bFarShadowCascade)
	{
		Result = GFarShadowLODDistanceFactor;
	}
	else
	{
		Result = GShadowLODDistanceFactor;
		// Make sure values are in a reasonable range (Local lights set SplitIndex to -1 and VSM to 1000)
		if (CascadeSettings.ShadowSplitIndex > 0 && CascadeSettings.ShadowSplitIndex < 16)
		{
			Result += CascadeSettings.ShadowSplitIndex * GShadowLODDistanceFactorCascadeScale;
		}
	}
	return Result;
}
...
const FViewInfo* FProjectedShadowInfo::FindViewForShadow(TConstArrayView<FViewInfo> SceneRendererViews) const
{
	// If the shadow view was pre-selected during setup, return that.
	if (SelectedViewIndex != INDEX_NONE)
	{
		check(!bDirectionalLight);
		check(!bPerObjectOpaqueShadow);
		check(bWholeSceneShadow);

		return &SceneRendererViews[SelectedViewIndex];
	}

	// Choose an arbitrary view where this shadow's subject is relevant.
	const FViewInfo* FoundView = nullptr;
	for (int32 ViewIndex = 0; ViewIndex < SceneRendererViews.Num(); ViewIndex++)
	{
		const FViewInfo* CheckView = &SceneRendererViews[ViewIndex];
...
{
	const FViewInfo* FoundView = FindViewForShadow(SceneRenderer->Views);
	check(FoundView && IsInParallelRenderingThread());
	FViewInfo* DepthPassView = FoundView->CreateSnapshot(EViewSnapshotType::Shadow);
...
void FProjectedShadowInfo::GetShadowTypeNameForDrawEvent(FString& TypeName) const
{
	...
	if (bWholeSceneShadow)
	{
		...
		{
			TypeName = FString("WholeScene split") + FString::FromInt(CascadeSettings.ShadowSplitIndex);
		}
		else
		{
			if (CacheMode == SDCM_MovablePrimitivesOnly)
			{
				TypeName = FString("WholeScene MovablePrimitives");
			}
			else if (CacheMode == SDCM_StaticPrimitivesOnly)
			{
				TypeName = FString("WholeScene StaticPrimitives");
			}
			else
			{
				TypeName = FString("WholeScene");
			}
		}
	}
	else if (bPreShadow)
	{
		TypeName = FString("PreShadow ") + ParentName.ToString();
	}
	else
	{
		TypeName = FString("PerObject ") + ParentName.ToString();
	}
}
...
/*
 *	True when this cascade is reusing last frame's CSM depth and should skip the depth raster
 *	this frame. Caller looks up CSMDepthReuse via FCSMSceneExtension::GetCSMDepthReuse and passes
 *	it in — two callsites need the pointer for other reasons (PreviousAtlas check) and would
 *	otherwise double-look up.
 */
static bool IsCSMCascadeDepthReused(
	const FScene* Scene,
	const FProjectedShadowInfo* ProjectedShadowInfo,
	const FCSMDepthReuse* CSMDepthReuse)
{
	return CSMDepthReuse != nullptr
		&& CSMDepthReuse->bHasEverRendered
		&& !ProjectedShadowInfo->ShouldCSMRenderDepthThisFrame(Scene, CSMDepthReuse);
}
	...
	const int32 AtlasIndex)
{
	...
	/*
	 *	Mirror the FProjectedShadowInfo::RenderDepth freeze when bCSMReuseEnabled. The non-Nanite path
	 *	honors ShouldCSMRenderDepthThisFrame and short-circuits to the restore-and-return path when
	 *	the cascade should reuse cached depth. Skip cascades that should reuse depth.
	 */
	const bool bCSMReuseEnabled = CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0;
	FCSMSceneExtension* CSMExt = bCSMReuseEnabled ? &Scene.GetExtension<FCSMSceneExtension>() : nullptr;
	...
	for (int32 ShadowIndex = 0; ShadowIndex < ShadowMapAtlas.Shadows.Num(); ShadowIndex++)
	{
		...
		/*
		 *	Skip Nanite shadow rasterization for cascades that the non-Nanite path reused.
		 */
		if (CSMExt && ProjectedShadowInfo->bDirectionalLight && ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex >= 0)
		{
			const FCSMDepthReuse* CSMDepthReuse = CSMExt->GetCSMDepthReuse(ProjectedShadowInfo->GetLightSceneInfo().Id, ProjectedShadowInfo->GetCSMShadowDepthReuseIndex());
			if (IsCSMCascadeDepthReused(&Scene, ProjectedShadowInfo, CSMDepthReuse))
			{
				continue;
			}
		}
	...
	auto DrawShadows = [&](const TArray<FProjectedShadowInfo*, SceneRenderingAllocator>& ShadowsToEmit, Nanite::EFilterFlags HiddenFilterFlags)
	{
		...
		{	
			Nanite::FPackedViewArray* PackedViews = Nanite::FPackedViewArray::CreateWithSetupTask(
				...
				[&ShadowsToEmit, AtlasSize, AtlasIndex] (Nanite::FPackedViewArray::ArrayType& PackedViews)
				{
					...
					for (FProjectedShadowInfo* ProjectedShadowInfo : ShadowsToEmit)
					{
						...
						Nanite::SetViewFrameIndex(ProjectedShadowInfo->ShadowDepthView, Initializer);
			...
			SharedContext.ShaderPlatform = Scene.GetShaderPlatform();
			...
			FRDGTextureRef PrevAtlasHZB = (bUseHZB && PrevAtlasHZBs[AtlasIndex]) ? GraphBuilder.RegisterExternalTexture(PrevAtlasHZBs[AtlasIndex]) : nullptr;
			...
			const Nanite::FRasterContextInitParams RasterInitParams =
			{
				...
				.TextureRect = FullAtlasViewRect,
				.RasterMode = Nanite::EOutputBufferMode::DepthOnly
			};
			Nanite::FRasterContext RasterContext = Nanite::InitRasterContext(GraphBuilder, SharedContext, ViewFamily, RasterInitParams);
			...
			for (FProjectedShadowInfo* ProjectedShadowInfo : ShadowsToEmit)
			{
				...
				Nanite::EmitShadowMap(
					...
					AtlasViewRect,
					AtlasViewRect.Min,
					ProjectedShadowInfo->GetShadowDepthRenderingViewMatrices().GetViewToClip(),
...
void FSceneRenderer::RenderShadowDepthMapAtlases(FRDGBuilder& GraphBuilder)
{
	...
	for (int32 AtlasIndex = 0; AtlasIndex < SortedShadowsForShadowDepthPass.ShadowMapAtlases.Num(); AtlasIndex++)
	{
		...
		// Check if we are using timesliced shadows for a runtime reflection capture
		if (Views[0].IsRuntimeReflectionCapture())
		{
			if (ShadowMapAtlas.Shadows.Num() && ShadowMapAtlas.Shadows[0]->GetLightSceneInfo().Proxy->TimesliceAtlasFrame == GFrameNumberRenderThread)
			{
				// We've started timeslicing on the current frame for the shadows in this atlas.  Mark the frame we are rendering those shadows.
				Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame = GFrameNumberRenderThread;
			}
			else if (Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame)
			{
				// Timeslicing is active from a previous frame, and we don't need to render.  Make readable because AtlasDepthTexture is not tracked via RDG yet.
		...
		bool bNeedsFullClear = true;
		if (CVarCSMShadowDepthReuse.GetValueOnRenderThread() != 0)
		{
			FCSMSceneExtension& CSMExt = Scene->GetExtension<FCSMSceneExtension>();

			// Check if any cascades are being reused this frame
			bool bNewAtlas = true;
			bool bHasReusedCascades = false;
			for (FProjectedShadowInfo* ProjectedShadowInfo : ShadowMapAtlas.Shadows)
			{
				if (ProjectedShadowInfo->bDirectionalLight)
				{
					...
					if (CSMDepthReuse != nullptr && CSMDepthReuse->PreviousAtlas.IsValid() && CSMDepthReuse->PreviousAtlas.DepthTarget == ShadowMapAtlas.RenderTargets.DepthTarget)
					{
						bNewAtlas = false;
					}
					...
					if (IsCSMCascadeDepthReused(Scene, ProjectedShadowInfo, CSMDepthReuse))
					{
						bHasReusedCascades = true;
					}
				}
			}

			if (!bNewAtlas && bHasReusedCascades)
			{
				bNeedsFullClear = false;
				...
				for (FProjectedShadowInfo* ProjectedShadowInfo : ShadowMapAtlas.Shadows)
				{
					if (ProjectedShadowInfo->bDirectionalLight)
					{
						...
						if (!IsCSMCascadeDepthReused(Scene, ProjectedShadowInfo, CSMDepthReuse))
						{
							AddClearShadowDepthPass(GraphBuilder, AtlasDepthTexture, ProjectedShadowInfo);
						}
					}
					else
					{
		...
		/*
		 *	Visualizer check point: when CSM reuse is enabled, no pass writes to the atlas on
		 *	frozen frames, so RDG's pass-tracker never spawns a
		 *	content-capture pass. Anchor a checkpoint here so `Vis WholeSceneShadowmap` keeps working
		 *	regardless of freeze state. Cheap when no Vis is active (ShouldCapture returns nullopt).
		 */
		GVisualizeTexture.SetCheckPoint(GraphBuilder, ShadowMapAtlas.RenderTargets.DepthTarget.GetReference());
...
void FSceneRenderer::RenderShadowDepthMaps(FRDGBuilder& GraphBuilder, FDynamicShadowsTaskData* DynamicShadowsTaskData, FInstanceCullingManager &InstanceCullingManager, FRDGExternalAccessQueue& ExternalAccessQueue)
{
	...
	for (int32 CubemapIndex = 0; CubemapIndex < SortedShadowsForShadowDepthPass.ShadowMapCubemaps.Num(); CubemapIndex++)
	{
		...
		if (ProjectedShadowInfo->CacheMode != SDCM_MovablePrimitivesOnly || !Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDataRef(ProjectedShadowInfo->GetLightSceneInfo().Id, FMath::Max(ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex, 0)).bCachedShadowMapHasPrimitives)
		{
		...
		if (bNaniteEnabled && CVarNaniteShadows.GetValueOnRenderThread())
		{
...

			{
				...
				for (int32 CubemapFaceIndex = 0; CubemapFaceIndex < 6; CubemapFaceIndex++)
				{
					...
					FRDGTextureRef PrevHZB = (bUseHZB && PrevShadowState && PrevShadowState->HZB) ? GraphBuilder.RegisterExternalTexture(PrevShadowState->HZB) : nullptr;
...

					{
						...
						Nanite::SetViewFrameIndex(&SceneView, Params);
	...
	if (ShouldRenderHeterogeneousVolumes(Scene) && ShouldHeterogeneousVolumesCastShadows() &&
		(HeterogeneousVolumes::GetShadowPipeline() == HeterogeneousVolumes::EShadowPipeline::LiveShading))
	{
	...
	// Begin another deferred batch for any instance culling after ShadowDepths. Orininally introduced to work around a bug (now fixed) 
	// that would cause subsequent passes enabling HZB for all passes. Left in place because it can under some circumstances be a small 
	// performance win, possibly by changing the batching allowing a different permutation to be picked.
	...
	ERasterizerCullMode MeshCullMode)
{
	...
	const bool bUseGpuSceneInstancing = VertexFactory->SupportsGPUScene(FeatureLevel);
	...
	const FMaterial& Material)
{
		...
		&& EnumHasAnyFlags(MeshSelectionMask, MeshBatch.VertexFactory->SupportsGPUScene(FeatureLevel) ? EShadowMeshSelection::VSM : EShadowMeshSelection::SM))
	{
		...
		const bool bModifiesMeshPosition = DoMaterialAndPrimitiveModifyMeshPosition(Material, PrimitiveSceneProxy, MeshBatch);
...
void FShadowDepthPassMeshProcessor::CollectPSOInitializers(const FSceneTexturesConfig& SceneTexturesConfig, const FMaterial& Material, const FPSOPrecacheVertexFactoryData& VertexFactoryData, const FPSOPrecacheParams& PreCacheParams, TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	if (!PreCacheParams.bDefaultMaterial && 
		...
		!ShouldIncludeMaterialInDefaultOpaquePass(Material) ||
		!EnumHasAnyFlags(MeshSelectionMask, VertexFactoryData.VertexFactoryType->SupportsGPUScene(SceneTexturesConfig.FeatureLevel) ? EShadowMeshSelection::VSM : EShadowMeshSelection::SM)))
	{
		return;
	}
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	{
		const EShadowDepthType LocalShadowDepthType = ShadowDepthVSMBit | EShadowDepthType::Spot;
		ERasterizerCullMode FinalCullMode = SetupShadowCullMode(FeatureLevel, LocalShadowDepthType, Material, MeshCullMode, bCastShadowAsTwoSided);
		CollectPSOInitializersForEachStreamSetup(VertexFactoryData, Material, LocalShadowDepthType, MeshFillMode, FinalCullMode, bRequired, PSOInitializers);
	}
}
