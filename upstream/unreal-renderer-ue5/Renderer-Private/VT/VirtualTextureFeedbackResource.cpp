...
void FVirtualTextureFeedbackBufferResource::Begin(FRDGBuilder& GraphBuilder, uint32 InFeedbackBufferSize, uint32 InExtendedDebugBufferSize, ERHIFeatureLevel::Type InFeatureLevel)
{
	...
	if (++BeginEndScopeCount != 1)
	{
		// Where overlapping scopes occur we will assume that it is OK to use the UAV from the outer scope.
		// This remains correct, but not ideal, if the inner scope requests a bigger buffer size then the outer scope.
		// In that case the feedback shader still respects the smaller size when writing, but we are more likely to fill the buffer and lose feedback.
		return;
	}
...
{
	if (--BeginEndScopeCount != 0)
	{
		if (!ensure(BeginEndScopeCount > 0))
		{ 
			// Calling End() more often than Begin() should never happen. Recover by resetting the count.
	...
	// If there are no allocated VTs then we can assume that no GPU feedback was generated.
	// So early out here to avoid paying any GPU cost compacting the empty feedback buffer.
	if (!FVirtualTextureSystem::Get().HasAllocatedVirtualTextures())
	{
		return;
	}
	...
	FGlobalShaderMap* GlobalShaderMap = GetGlobalShaderMap(GShaderPlatformForFeatureLevel[FeatureLevel]);
...
{
	if (BeginEndScopeCount <= 0 || ExtendedDebugBufferSize == 0)
	{
		return nullptr;
	}
...
{
	return BeginEndScopeCount > 0 ? FeedbackBufferSize : 0;
}

uint32 FVirtualTextureFeedbackBufferResource::GetExtendedDebugBufferSize() const
{
	return BeginEndScopeCount > 0 ? ExtendedDebugBufferSize : 0;
}

FRHIUnorderedAccessView* FVirtualTextureFeedbackBufferResource::GetUAV() const
{
	return UAV != nullptr && BeginEndScopeCount > 0 ? UAV : GEmptyStructuredBufferWithUAV->UnorderedAccessViewRHI.GetReference();
}
...
void GetFeedbackShaderParams(FFeedbackShaderParams& OutParams)
{
	...
	VirtualTexture::GetFeedbackShaderParams(GFrameNumberRenderThread, VirtualTextureFeedbackTileSize, OutParams);
}
