...
// This is now enabled by default since perfomance ReplayRun on a game has shown no performance regression.
static TAutoConsoleVariable<int32> CVarVolumetricCloudApplyFogLate(
	TEXT("r.VolumetricCloud.ApplyFogLate"), 1,
...
static TAutoConsoleVariable<int32> CVarVolumetricCloudApplyFogLateCloudOnTranslucent(
	TEXT("r.VolumetricCloud.ApplyFogLate.CloudOnTranslucent"), 0,
	TEXT("When r.VolumetricCloud.ApplyFogLate=1, fog is not into the volumetric cloud reconstructed buffer. So when applied on translucent, a visual discrepancy can become visible for translucent behind clouds. This enables the reconstruction step to also output an extra cloud buffer with fog included for application on translucent. It is more expenssive and use more memory so it is disabled by default."),
	ECVF_RenderThreadSafe);
...
{
	static FShaderPlatformCachedIniValue<int32> CVar(TEXT("r.VolumetricCloud.Support"));
	return CVar.Get(ShaderPlatform) > 0;
}
...
bool ShouldVolumetricCloudsApplyFogDuringUpsampling(const FViewInfo& ViewInfo)
{
	if (ViewInfo.ViewState)
	{
		FVolumetricRenderTargetViewStateData& VRT = ViewInfo.ViewState->VolumetricCloudRenderTarget;
...
bool ShouldVolumetricCloudsApplyFogDuringUpsamplingForCloudOnTranslucent(const FViewInfo& ViewInfo)
{
	extern bool ShouldRenderFog(const FSceneViewFamily & Family);

	if (ViewInfo.ViewState)
	{
		return ShouldVolumetricCloudsApplyFogDuringUpsampling(ViewInfo) && CVarVolumetricCloudApplyFogLateCloudOnTranslucent.GetValueOnRenderThread() > 0 
			&& ViewInfo.Family && (ViewInfo.Family->Scene->HasAnyExponentialHeightFog() || ViewInfo.LocalFogVolumeViewData.GPUInstanceCount > 0)
			&& ShouldRenderFog(*ViewInfo.Family);
	}
	return false;
}
...
void SetupDefaultRenderVolumetricCloudGlobalParameters(FRDGBuilder& GraphBuilder, FRenderVolumetricCloudGlobalParameters& VolumetricCloudParams, FVolumetricCloudRenderSceneInfo& CloudInfo, FViewInfo& ViewInfo, bool bForceNonLateFogOnCloud = false)
{
	...
	VolumetricCloudParams.SceneDepthMinAndMaxTexture = GSystemTextures.GetBlackDummy(GraphBuilder);
	...
	if (ShouldVolumetricCloudsApplyFogDuringUpsampling(ViewInfo) && !bForceNonLateFogOnCloud)
	{
...
static const TCHAR* VolumetricCloudRenderShadowPassName = TEXT("VolumetricCloudRenderShadow");
...
public:
	FVolumetricCloudRenderShadowMeshProcessor(const FScene* Scene, ERHIFeatureLevel::Type FeatureLevel, const FViewInfo* InViewIfDynamicMeshCommand, FMeshPassDrawListContext* InDrawListContext)
		: FMeshPassProcessor(VolumetricCloudRenderShadowPassName, Scene, FeatureLevel, InViewIfDynamicMeshCommand, InDrawListContext)
	{
		PassDrawRenderState.SetBlendState(TStaticBlendState<>::GetRHI());
		PassDrawRenderState.SetDepthStencilState(TStaticDepthStencilState<false, CF_Always>::GetRHI());
	}
...
private:
	...
	virtual void CollectPSOInitializers(const FSceneTexturesConfig& SceneTexturesConfig, const FMaterial& Material, const FPSOPrecacheVertexFactoryData& VertexFactoryData, const FPSOPrecacheParams& PreCacheParams, TArray<FPSOPrecacheData>& PSOInitializers) override
	{
		...
		FRDGTextureDesc CloudSkyOARDGTextureDesc = GetVolumetricCloudSkyAORDGTextureDesc(FIntPoint(0));

		FGraphicsPipelineRenderTargetsInfo RenderTargetsInfo;
		RenderTargetsInfo.NumSamples = 1;
		AddRenderTargetInfo(CloudSkyOARDGTextureDesc.Format, CloudSkyOARDGTextureDesc.Flags, RenderTargetsInfo);
...
{
	return new FVolumetricCloudRenderShadowMeshProcessor(nullptr, FeatureLevel, nullptr, nullptr);
}
FRegisterPSOCollectorCreateFunction RegisterPSOCollectorVolumetricCloudRenderShadow(&CreatePSOCollectorVolumetricCloudRenderShadow, EShadingPath::Deferred, VolumetricCloudRenderShadowPassName);
...
void FSceneRenderer::InitVolumetricCloudsForViews(FRDGBuilder& GraphBuilder, bool bShouldRenderVolumetricCloud, FInstanceCullingManager& InstanceCullingManager)
{
	...
	auto CleanUpCloudDataPass = [&Views = Views](FRDGBuilder& GraphBuilder)
	{
		...
		{
			// Make sure we always clear the texture on views to make sure no dangling texture pointers are ever used
			...
			if (CloudVolumeMaterialProxy->GetIncompleteMaterialWithFallback(ViewFamily.GetFeatureLevel()).GetMaterialDomain() == MD_Volume && !ViewFamily.EngineShowFlags.PathTracing)
			{
				...
				for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
				{
					...
					auto FilterTracedCloudTexture = [&](FRDGTextureRef* TracedCloudTextureOutput, FVector4f TracedTextureSizeInvSize, FVector4f CloudAOTextureTexelWorldSizeInvSize, bool bSkyAOPass)
					{
						...
						TShaderMapRef<FCloudShadowFilterCS> ComputeShader(GetGlobalShaderMap(ViewInfo.GetShaderPlatform()), Permutation);
					...
					if (ShouldRenderCloudSkyAO(SkyLight))
					{
						...
						const FRDGTextureDesc CloudSkyAOTextureDesc = GetVolumetricCloudSkyAORDGTextureDesc(FIntPoint(VolumetricCloudSkyAOResolution,VolumetricCloudSkyAOResolution));
						FRDGTextureRef CloudSkyAOTexture = GraphBuilder.CreateTexture(CloudSkyAOTextureDesc, TEXT("Cloud.SkyAOTexture"));
					...
					auto GenerateCloudTexture = [&](FLightSceneProxy* AtmosphericLight, int LightIndex)
					{
						...
						if (ShouldRenderCloudShadowmap(AtmosphericLight))
						{
							...
							if(CloudShadowTemporalEnabled && ViewInfo.ViewState)
							{
								...
								TShaderMapRef<FCloudShadowTemporalProcessCS> ComputeShader(GetGlobalShaderMap(ViewInfo.GetShaderPlatform()), Permutation);
...
static TRDGUniformBufferRef<FRenderVolumetricCloudGlobalParameters> CreateCloudPassUniformBuffer(FRDGBuilder& GraphBuilder, FCloudRenderContext& CloudRC)
{
	...
	{
		if (ShouldVolumetricCloudsApplyFogDuringUpsampling(MainView))
		{
			VolumetricCloudParams.FogStruct.ApplyVolumetricFog = 0;
			VolumetricCloudParams.EnableHeightFog = 0;
		}
...
void FSceneRenderer::RenderVolumetricCloudsInternal(FRDGBuilder& GraphBuilder, FCloudRenderContext& CloudRC, FInstanceCullingManager& InstanceCullingManager, const FIntPoint& CloudViewportSize)
{
	...
	if (ShouldUseComputeForCloudTracing(Scene->GetFeatureLevel()))
	{
		...
		PermutationVector.Set<typename FRenderVolumetricCloudRenderViewCS::FCloudLocalFogVolume>(MainView.LocalFogVolumeViewData.GPUInstanceCount > 0 && !CloudRC.bIsSkyRealTimeReflectionRendering && !ShouldVolumetricCloudsApplyFogDuringUpsampling(MainView)); // LFVs are not culled for real time capture views.
	...
	if (CloudSceneProxy.GetCloudVolumeMaterial())
	{
		...
		if (CloudVolumeMaterialProxy->GetIncompleteMaterialWithFallback(ViewFamily.GetFeatureLevel()).GetMaterialDomain() == MD_Volume)
		{
			...
			for (int32 ViewIndex = 0; ViewIndex < ViewsToProcess.Num(); ViewIndex++)
			{
				...
				{
					CloudRC.SceneDepthZ = GSystemTextures.GetBlackDummy(GraphBuilder);
				...
				if (bShouldUseHighQualityAerialPerspective)
				{
					...
					SkyRC.ShaderPlatform = ViewInfo.GetShaderPlatform();
				...
				if (DebugCloudShadowMap || DebugCloudSkyAO)
				{
					...
					auto DebugCloudTexture = [&](FDrawDebugCloudShadowCS::FParameters* Parameters)
					{
						if (ShaderPrint::IsEnabled(ViewInfo.ShaderPrintData))
						{
							...
							TShaderMapRef<FDrawDebugCloudShadowCS> ComputeShader(GetGlobalShaderMap(ViewInfo.GetShaderPlatform()), Permutation);
