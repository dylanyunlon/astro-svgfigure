...
#include "PostProcess/MobilePostProcessInputs.h"
...
DECLARE_GPU_STAT_NAMED(CapMobileSceneRender, TEXT("Mobile Scene Render (Cap)"));
...
static FMobileCustomDepthStencilUsage GetCustomDepthStencilUsage(const FViewInfo& View)
{
	...
	if ((View.bHasCustomDepthPrimitives || GetCustomDepthMode() == ECustomDepthMode::EnabledWithStencil))
	{
		...
		{
			CustomDepthStencilUsage.bUsesCustomDepthStencil |= Config.bMaskEnabled;
			CustomDepthStencilUsage.bSamplesCustomStencil |= Config.bMaskEnabled;
		}
	}
...
struct FRenderViewContext
{
	FViewInfo* ViewInfo;
	int32 ViewIndex;
	int32 DecalViewOffset;
	bool bIsFirstView;
	bool bIsLastView;
};
using FRenderViewContextArray = TArray<FRenderViewContext, TInlineAllocator<2, SceneRenderingAllocator>>;

static void GetRenderViews(TArrayView<FViewInfo> InViews, int32 DecalViewOffset, FRenderViewContextArray& RenderViews)
{
	for (int32 ViewIndex = 0; ViewIndex < InViews.Num(); ViewIndex++)
	{
		...
		if (View.ShouldRenderView())
		{
			...
			RenderView.DecalViewOffset = DecalViewOffset;
...
static bool IsShadowOnlyTimeslice_GT(const FSceneViewFamily* InViewFamily, FScene* Scene)
{
	check(IsInGameThread());
	if (!InViewFamily->Views.Num())
	{
		return false;
	}
	return InViewFamily->Views[0]->IsRuntimeReflectionCapture() && Scene->ReflectionSceneData.RuntimeCaptureTimesliceFaceCount_GT == 0;
}

static bool IsShadowOnlyTimeslice_RT(const FSceneViewFamily* InViewFamily, FScene* Scene)
{
	...
	return InViewFamily->Views[0]->IsRuntimeReflectionCapture() && Scene->ReflectionSceneData.RuntimeCaptureTimesliceFaceCount_RT == 0;
}
...
FMobileSceneRenderer::FMobileSceneRenderer(const FSceneViewFamily* InViewFamily, FHitProxyConsumer* HitProxyConsumer)
	...
	, bUseVirtualTexturing(UseVirtualTexturing(ShaderPlatform) && GetRendererOutput() != FSceneRenderer::ERendererOutput::DepthPrepassOnly && !IsShadowOnlyTimeslice_GT(InViewFamily, Scene))
	, bSupportsSimpleLights(bDeferredShading || MobileForwardEnableParticleLights(ShaderPlatform))
{
	...
	bShouldRenderVolumetricFog = false;
...
void FMobileSceneRenderer::SetupMobileBasePassAfterShadowInit(FExclusiveDepthStencil::Type BasePassDepthStencilAccess, TArrayView<FViewCommands> ViewCommandsPerView, FInstanceCullingManager& InstanceCullingManager)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < AllViews.Num(); ++ViewIndex)
	{
		...
		EInstanceCullingMode InstanceCullingMode = View.IsInstancedStereo() ? EInstanceCullingMode::Stereo : EInstanceCullingMode::Normal;
		...
		FSceneCullingRenderer* SceneCullingRenderer = GetSceneExtensionsRenderers().GetRendererPtr<FSceneCullingRenderer>();
		FSceneCullingRendererHierarchicalCullingQuery* HierarchicalCullingQuery = SceneCullingRenderer ? SceneCullingRenderer->GetPrimaryViewHierarchicalCullingQuery(View.SceneRendererPrimaryViewId) : nullptr;
		Pass.DispatchPassSetup(
			Scene,
			View,
			FInstanceCullingContext(GetMeshPassName(EMeshPass::BasePass), ShaderPlatform, &InstanceCullingManager, ViewIds, View.PrevViewInfo.HZB, InstanceCullingMode, EInstanceCullingFlags::None, EBatchProcessingMode::UnCulled, HierarchicalCullingQuery),
			EMeshPass::BasePass,
	...
	FInitViewTaskDatas& TaskDatas)
{
	...
	const bool bShadowOnlyTimeslice = IsShadowOnlyTimeslice_RT(&ViewFamily, Scene);
	const bool bRendererOutputFinalSceneColor = (GetRendererOutput() != ERendererOutput::DepthPrepassOnly) && !bShadowOnlyTimeslice;
	...
	// Volumetric fog requires a full depth before lighting
	bShouldRenderVolumetricFog = ShouldRenderVolumetricFog() && (bIsFullDepthPrepassEnabled || (bDeferredShading && !MobileAllowFramebufferFetch(ShaderPlatform)));
	if (bShouldRenderVolumetricFog && bRendererOutputFinalSceneColor)
	{
		SetupVolumetricFog();
	}
	...
	// Runtime reflection captures should support shadows.  Offline reflection captures do not.
	bRequiresShadowProjections = (MobileUsesShadowMaskTexture(ShaderPlatform) || bInsetShadows)
		&& ViewFamily.EngineShowFlags.Lighting
		&& (!Views[0].bIsReflectionCapture || Views[0].IsRuntimeReflectionCapture())
		&& !Views[0].bIsPlanarReflection
		&& !ViewFamily.EngineShowFlags.HitProxies
		&& !ViewFamily.EngineShowFlags.VisualizeLightCulling
		&& !ViewFamily.UseDebugViewPS()
		&& (bRendererOutputFinalSceneColor || bShadowOnlyTimeslice);
	...
	const bool bVisualizeCSMOverlay = Views.Num() > 0 && ShouldVisualizeCSMOverlay(Views[0]);
	...
	// Runtime reflection captures should support DFAO.  Offline reflection captures do not.
	...
	// Depth is needed for the mobile DebugView render pass which runs separately from the base pass.
	...
	if (bRendererOutputFinalSceneColor || bShadowOnlyTimeslice)
	{
	...
	if (bRendererOutputFinalSceneColor)
	{
		...
		if (ILCTaskData)
		{
			Scene->IndirectLightingCache.FinalizeCacheUpdates(RHICmdList, Scene, *this, *ILCTaskData);
		}
	}
	...
	if (!bShadowOnlyTimeslice)
	{
		DecalVisibilityTaskData = FDecalVisibilityTaskData::Launch(GraphBuilder, *Scene, AllViews);
	}
}
...
void FMobileSceneRenderer::RenderFullDepthPrepass(FRDGBuilder& GraphBuilder, TArrayView<FViewInfo> InViews, FSceneTextures& SceneTextures, FInstanceCullingManager& InstanceCullingManager, bool bIsSceneCaptureRenderPass)
{
	...
	GetRenderViews(InViews, GetDecalViewOffset(InViews), RenderViews);

	FRDGTextureRef DepthTargetTexture = SceneTextures.Depth.Target;

#if WITH_EDITOR
	if (ViewFamily.EngineShowFlags.Wireframe)
	{
		DepthTargetTexture = SceneTextures.EditorPrimitiveDepth;
	}
#endif
	...
	{
		AddDitheredStencilFillPass(GraphBuilder, InViews, DepthTargetTexture, DepthPass);
	}
	...
	for (FRenderViewContext& ViewContext : RenderViews)
	{
		...
		PassParameters->RenderTargets.DepthStencil = DepthPass.IsRasterStencilDitherEnabled() ?
			FDepthStencilBinding(DepthTargetTexture, ERenderTargetLoadAction::ELoad, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthWrite_StencilWrite):
			FDepthStencilBinding(DepthTargetTexture, ERenderTargetLoadAction::EClear, ERenderTargetLoadAction::EClear, FExclusiveDepthStencil::DepthWrite_StencilWrite);
	...
	{
		AddDitheredStencilClearPass(GraphBuilder, DepthTargetTexture, ShaderPlatform, false);
	}
	...
	if (bDoOcclusionQueries)
	{
		for (FRenderViewContext& ViewContext : RenderViews)
		{
			...
			PassParameters->RenderTargets.DepthStencil = FDepthStencilBinding(DepthTargetTexture, ERenderTargetLoadAction::ELoad, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthRead_StencilRead);
			PassParameters->View = View.GetShaderParameters();
...
void FMobileSceneRenderer::RenderCustomRenderPassBasePass(FRDGBuilder& GraphBuilder, TArrayView<FViewInfo> InViews, FRDGTextureRef ViewFamilyTexture, FSceneTextures& SceneTextures, FMobileBasePassTextures& BasePassTextures, FInstanceCullingManager& InstanceCullingManager, bool bIncludeTranslucent)
{
	...
	for (FRenderViewContext& ViewContext : RenderViews)
	{
		...
		struct FCustomPassParameterCollection
		{
			...
			FInstanceCullingDrawParams MeshDecalInstanceCullingDrawParams;
		};
		auto ParameterCollection = GraphBuilder.AllocParameters<FCustomPassParameterCollection>();

		EMobileSceneTextureSetupMode SetupMode = bIsFullDepthPrepassEnabled ? 
			(EMobileSceneTextureSetupMode::SceneDepth | EMobileSceneTextureSetupMode::SceneVelocity) : EMobileSceneTextureSetupMode::None;
		auto PassParameters = &ParameterCollection->PassParameters;
		PassParameters->View = View.GetShaderParameters();
		...
		if (NumAdditionalSubpasses > 0)
		{
			BuildMeshRenderingCommands(
				GraphBuilder,
				IsForwardShadingEnabled(View.Family->GetShaderPlatform()) ? EMeshPass::MeshDecal_SceneColor : EMeshPass::MeshDecal_SceneColorAndGBuffer,
				View, Scene->GPUScene, InstanceCullingManager, ParameterCollection->MeshDecalInstanceCullingDrawParams);
		}
		...
		EDecalRenderStage DecalStage = bRequiresDBufferDecals ? EDecalRenderStage::Emissive : EDecalRenderStage::Mobile;
		FDecalViewInfo DecalViewInfo = FDecalViewInfo(ViewContext.DecalViewOffset, ViewContext.ViewIndex,
													  DecalStage,
													  bDeferredShading ? EDecalRenderTargetMode::SceneColorAndGBuffer : EDecalRenderTargetMode::SceneColor,
													  GetRelevantDeferredDecals(ViewContext.DecalViewOffset, ViewContext.ViewIndex, DecalStage));

		GraphBuilder.AddPass(
			RDG_EVENT_NAME("RenderMobileBasePass"),
			PassParameters,
			ERDGPassFlags::Raster,
			[this, DecalViewInfo, ParameterCollection, ViewContext, NumAdditionalSubpasses](FRHICommandList& RHICmdList)
			{
				FViewInfo& View = *ViewContext.ViewInfo;
				auto PassParameters = &ParameterCollection->PassParameters;
				...
				for (int32 AddSubpass = 0; AddSubpass < NumAdditionalSubpasses; ++AddSubpass)
				{
					...
					if (AddSubpass == 0)
					{
						RenderDecals(RHICmdList, View, DecalViewInfo, &ParameterCollection->MeshDecalInstanceCullingDrawParams);
					}
				}
			});

		// Decals are rendered as the first subpass above -- if no subpasses, render them as a separate pass.
		if (NumAdditionalSubpasses == 0)
		{
			...
			RenderDeferredMultiPassDecals(GraphBuilder, View, ViewContext.DecalViewOffset, ViewContext.ViewIndex, SceneTextures, InstanceCullingManager);
		}
...
void FMobileSceneRenderer::Render(FRDGBuilder& GraphBuilder, const FSceneRenderUpdateInputs* SceneUpdateInputs)
{
	...
	const bool bShadowOnlyTimeslice = IsShadowOnlyTimeslice_RT(&ViewFamily, Scene);
	const bool bRendererOutputFinalSceneColor = (RendererOutput != ERendererOutput::DepthPrepassOnly) && !bShadowOnlyTimeslice;

	// Allow GPU cost of scene captures to be distinguished from main views, useful for profiling the amortized GPU cost of runtime reflection captures.
	RDG_RHI_EVENT_SCOPE_CONDITIONAL_STAT(GraphBuilder, MobileSceneRender,    MobileSceneRender,    !Views[0].bIsSceneCapture);
	RDG_RHI_EVENT_SCOPE_CONDITIONAL_STAT(GraphBuilder, CapMobileSceneRender, CapMobileSceneRender,  Views[0].bIsSceneCapture);

	// Shadow state swapping (to a transform cached in the scene, rather than the current transform) is necessary for timesliced shadows, in case the
	// shadow casting actor moved since timeslicing started (say there is a time of day progression, or it is moved in the editor).  Even small changes
	// to the transform will cause shadows to be radically incorrect, making this a necessity.
	bool bShadowStateSwapped = false;
	if (Views[0].IsRuntimeReflectionCapture() && Scene->ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame)
	{
		Scene->SwapMobileReflectionCaptureCachedShadowState();
		bShadowStateSwapped = true;
	}
	...
	if (SceneUpdateInputs)
	{
		...
		{
			// It's necessary to check all scene renderers when deciding whether to update distance field scene data, as depth
			// only scene captures and shadow only timeslices don't use distance fields, and would otherwise skip their update.
			// The first renderer in a batch is responsible for scene updates required by other renderers.
			for (const FSceneRenderer* Renderer : SceneUpdateInputs->Renderers)
			{
				if (Renderer->GetRendererOutput() != ERendererOutput::DepthPrepassOnly && !IsShadowOnlyTimeslice_RT(&Renderer->ViewFamily, Scene))
				{
					const FSceneViewFamily& Family = Renderer->ViewFamily;
					const FEngineShowFlags& EngineShowFlags = Family.EngineShowFlags;
					const FSceneView& View = *Family.Views[0];

					// Runtime reflection captures should support distance field shadows.  Offline reflection captures do not
					if (EngineShowFlags.Lighting && !EngineShowFlags.VisualizeLightCulling && !Family.UseDebugViewPS() && (!View.bIsReflectionCapture || View.IsRuntimeReflectionCapture()) && !View.bIsPlanarReflection)
					{
	...
	if (bUseVirtualTexturing)
	{
		...
		// Disable VT feedback for runtime reflection captures.  These are low resolution (usually) one-time
		// renders at a given location, so it's not worth the overhead.
		if (Views.Num() && Views[0].IsRuntimeReflectionCapture())
		{
	...
	if (bRendererOutputFinalSceneColor)
	{
        ...
        const bool bEnableVirtualShadowMaps = ViewFamily.EngineShowFlags.DynamicShadows;
	...
	if (bRendererOutputFinalSceneColor)
	{
		...
		const bool bVisualizeCSMOverlay = Views.Num() > 0 && ShouldVisualizeCSMOverlay(Views[0]);
		const bool bNeedsForwardLightData =	bDeferredShading || bEnableClusteredLocalLights || bEnableClusteredReflections || bVisualizeCSMOverlay;

		if (bNeedsForwardLightData)
		{
			...
			const bool bCullLightsToGrid = (((bEnableClusteredReflections || bDeferredShading) && NumReflectionCaptures > 0) || bEnableClusteredLocalLights	|| bVisualizeCSMOverlay);
		...
		InitLocalFogVolumesForViews(Scene, Views, ViewFamily, GraphBuilder, bShouldRenderVolumetricFog, bUseHalfResLocalFogVolume);
	...
	if (CustomRenderPassInfos.Num() > 0)
	{
		...
		for (int32 i = 0; i < CustomRenderPassInfos.Num(); ++i)
		{
			...
			bool bNeedGBufferC = !SceneCaptureUserData.UserSceneTextureBaseColor.IsNone() || CustomRenderPass->GetSceneCaptureSource() == SCS_BaseColor;
			bool bNeedGBufferA = !SceneCaptureUserData.UserSceneTextureNormal.IsNone() || CustomRenderPass->GetSceneCaptureSource() == SCS_Normal;

			// Initialize scene textures if owned by this Custom Render Pass (not reusing the main view family).
			...
			{
				// If we are reading back GBuffer textures, they can't be memoryless.
				for (uint32 Layout = 0; Layout < GBL_Num; ++Layout)
				{
					FGBufferBindings& Bindings = CustomRenderPassViewFamily.SceneTexturesConfig.GBufferBindings[Layout];
					if (bNeedGBufferC)
					{
			...
			EMobileSceneTextureSetupMode OriginalSceneTextureSetupMode = CustomRenderPassSceneTextures.MobileSetupMode;
			TRDGUniformBufferRef<FMobileSceneTextureUniformParameters> OriginalSceneTextureUniformBuffer = CustomRenderPassSceneTextures.MobileUniformBuffer;
...

			{
				...
				if (bIsFullDepthPrepassEnabled)
				{
					RenderFullDepthPrepass(GraphBuilder, CustomRenderPassViews, CustomRenderPassSceneTextures, InstanceCullingManager, true);
					if (!bRequiresSceneDepthAux)
					{
						AddResolveSceneDepthPass(GraphBuilder, CustomRenderPassViews, CustomRenderPassSceneTextures.Depth);
					}
				}
				...
				{
					FMobileBasePassTextures CustomRenderPassBasePassTextures{};

					if (CustomRenderPass->GetRenderMode() == FCustomRenderPassBase::ERenderMode::DepthAndBasePass && bRequiresDBufferDecals)
					{
						// If mobile multi-view is enabled, force separate DBuffer textures to be allocated.  Multi-view causes the DBuffer textures to be allocated as array
						// textures, and the concern is that could cause issues with Custom Render Passes.  We also need separate DBuffer textures if this view allocates its
						// own scene textures, as the scene texture resolution will be different than the main view in that case.
						if (CustomRenderPassViewFamily.IsSceneTexturesOwner() || bIsMobileMultiView)
						{
							CustomRenderPassBasePassTextures.DBufferTextures = CreateDBufferTextures(GraphBuilder, CustomRenderPassSceneTextures.Config.Extent, ShaderPlatform);
						}
						else
						{
							// Sharing main renderer DBuffer textures, allocate them if not yet allocated.  Note that scene texture extents for custom render passes are the
							// same as the main view(s), as upstream scene texture extent calculation logic ensures the extent is a union of all views rendered in the frame,
							// including Scene Captures and Custom Render Passes.  The exception is if bExcludeFromSceneTextureExtents is set, which excludes the given
							// Scene Capture (or Custom Render Pass) from extent calculations.  In that case, IsSceneTexturesOwner() will be true, and the code above runs.
							if (!BasePassTextures.DBufferTextures.DBufferA && !BasePassTextures.DBufferTextures.DBufferATexArray)
							{
							...
							CustomRenderPassBasePassTextures.DBufferTextures = BasePassTextures.DBufferTextures;
						}
						RenderDBuffer(GraphBuilder, CustomRenderPassViews, CustomRenderPassSceneTextures, CustomRenderPassBasePassTextures.DBufferTextures, InstanceCullingManager);
					}

					RenderCustomRenderPassBasePass(GraphBuilder, CustomRenderPassViews, ViewFamilyTexture, CustomRenderPassSceneTextures, CustomRenderPassBasePassTextures, InstanceCullingManager, CustomRenderPass->IsTranslucentIncluded());
				}

				CustomRenderPassSceneTextures.MobileSetupMode = EMobileSceneTextureSetupMode::SceneColor | EMobileSceneTextureSetupMode::SceneDepth | EMobileSceneTextureSetupMode::SceneDepthAux;

				if (!IsForwardShadingEnabled(ViewFamily.GetShaderPlatform()))
				{
					if (bNeedGBufferC)
					{
						CustomRenderPassSceneTextures.MobileSetupMode |= EMobileSceneTextureSetupMode::GBufferC;
					}
					if (bNeedGBufferA)
					{
						CustomRenderPassSceneTextures.MobileSetupMode |= EMobileSceneTextureSetupMode::GBufferA;
					}
				}

				CustomRenderPassSceneTextures.MobileUniformBuffer = CreateMobileSceneTextureUniformBuffer(GraphBuilder, &CustomRenderPassSceneTextures, CustomRenderPassSceneTextures.MobileSetupMode);
			...
			CustomRenderPassSceneTextures.MobileSetupMode = OriginalSceneTextureSetupMode;
			CustomRenderPassSceneTextures.MobileUniformBuffer = OriginalSceneTextureUniformBuffer;
			...
			// Call finish on decal visibility task data.  Note that this is handled by the FCompositionLighting destructor for Custom Render Passes
			// in the deferred renderer, but the mobile renderer doesn't use that class.
			int32 DecalViewOffset = GetDecalViewOffset(CustomRenderPassViews);
			if (DecalViewOffset != INDEX_NONE)
			{
				for (int32 ViewIndex = 0; ViewIndex < CustomRenderPassViews.Num(); ViewIndex++)
				{
					DecalVisibilityTaskData->Finish(DecalViewOffset, ViewIndex);
				}
			}
		}
	}

	if (bIsFullDepthPrepassEnabled && !bShadowOnlyTimeslice)
	{
		...
		SceneTextures.MobileSetupMode = EMobileSceneTextureSetupMode::SceneDepth | EMobileSceneTextureSetupMode::SceneVelocity;
		SceneTextures.MobileUniformBuffer = CreateMobileSceneTextureUniformBuffer(GraphBuilder, &SceneTextures, SceneTextures.MobileSetupMode);
	}
	...
	if (bIsFullDepthPrepassEnabled)
	{
		...
		// Render SSAO after base pass when possible, for GBuffer access. AsyncCompute is suspposed to run with a BasePass
		if (bRendererOutputFinalSceneColor && ((!bDeferredShading || !bRequiresMultiPass) || MobileSSAOUsesCompute()))
		{
		...
		if (bRendererOutputFinalSceneColor)
		{
			...
			{
				// DBufferTextures may already have been created for Custom Render Passes above
				if (!BasePassTextures.DBufferTextures.DBufferA && !BasePassTextures.DBufferTextures.DBufferATexArray)
				{
					BasePassTextures.DBufferTextures = CreateDBufferTextures(GraphBuilder, SceneTextures.Config.Extent, ShaderPlatform, bIsMobileMultiView);
				}
				RenderDBuffer(GraphBuilder, Views, SceneTextures, BasePassTextures.DBufferTextures, InstanceCullingManager);
			}
		}
	...
	if (bRendererOutputFinalSceneColor)
	{
		...
		if (ViewFamily.EngineShowFlags.Wireframe)
		{
			// Wireframe swaps the base-pass RT0 to EditorPrimitiveColor and depth to EditorPrimitiveDepth, leaving SceneTextures.Color, SceneTextures.Depth.Target unwritten; the editor-primitive composite still samples them, so clear it explicitly
			const FLinearColor SceneColorClearValue(MainView->BackgroundColor.R, MainView->BackgroundColor.G, MainView->BackgroundColor.B, 1.0f);
			AddClearRenderTargetPass(GraphBuilder, SceneTextures.Color.Target, SceneColorClearValue);
			AddClearDepthStencilPass(GraphBuilder, SceneTextures.Depth.Target, ERenderTargetLoadAction::EClear, ERenderTargetLoadAction::EClear);
		}
#endif
		...
		int32 DecalViewOffset = GetDecalViewOffset(Views);
		if (DecalViewOffset != INDEX_NONE)
		{
			for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
			{
		...
		if (!(bDeferredShading && bRequiresMultiPass))
		{
		...
		if (Views[0].IsRuntimeReflectionCapture())
		{
			// Needs to run before CopySceneCaptureComponentToTarget, as it disables the scene color copy that occurs there,
			// if a post pass has already handled copying to the final output.
			AddRuntimeReflectionCapturePostPass(GraphBuilder, ViewFamily, ViewFamilyTexture, ViewFamilyDepthTexture, Views);
		}
		...
		if (Views.Num() > 0 && ShouldVisualizeCSMOverlay(Views[0]))
		{
			RenderVisualizeCSMOverlay(GraphBuilder, Views, SceneTextures, Scene, VisibleLightInfos);
		}
		...
		if (ViewFamily.bResolveScene)
		{
			if (bRenderToSceneColor && !bTonemapSubpassInline)
			{
				// Finish rendering for each view, or the full stereo buffer if enabled
				{
					...
					if (ShouldVisualizeCSMDepthBuffers(*MainView))
					{
						PostProcessingInputs.CSMDepthBufferTiles = CollectVisualizeCSMDepthBufferTiles(SortedShadowsForShadowDepthPass.ShadowMapAtlases);
					}
	...
	else // !bRendererOutputFinalSceneColor
	{
		// We need to call the GPU sort manager to reset the indirect draw buffers, etc, that were used in the depth only pass
		if (FXSystem)
		{
			if (FGPUSortManager* GPUSortManager = FXSystem->GetGPUSortManager())
			{
...
void FMobileSceneRenderer::RenderStereoMotionVectors(FRDGBuilder& GraphBuilder, FViewInfo& View, const FSceneTextures& SceneTextures, FInstanceCullingManager& InstanceCullingManager)
{
	...
	if (View.ShouldRenderView())
	{
		...
		for (int32 PassIndex = 0; PassIndex < TotalMotionVectorPasses; ++PassIndex)
		{
			...
			MotionVectorRenderTargets.DepthStencil = FDepthStencilBinding(SceneTextures.StereoMotionVectorDepth, LoadAction, FExclusiveDepthStencil::DepthWrite_StencilWrite);
				...
				[this, PassParameters, MeshPass, &View, &SceneTextures](FRHICommandListImmediate& RHICmdList)
			{
				...
				if (auto* Pass = View.ParallelMeshDrawCommandPasses[MeshPass])
				{
					...
					RHICmdList.SetViewport(0.0f, 0.0f, 0.0f, VelocityBufferSize.X, VelocityBufferSize.Y, 1.0f);
...
FRenderTargetBindingSlots FMobileSceneRenderer::InitRenderTargetBindings_Forward(FRDGTextureRef ViewFamilyTexture, FSceneTextures& SceneTextures)
{
	...
	if (ViewFamily.EngineShowFlags.Wireframe)
	{
		BasePassRenderTargets[0].SetTexture(SceneTextures.EditorPrimitiveColor);
		BasePassRenderTargets[0].SetResolveTexture(nullptr);
		BasePassRenderTargets.DepthStencil.SetTexture(SceneTextures.EditorPrimitiveDepth);
		BasePassRenderTargets.DepthStencil.SetResolveTexture(nullptr);
	}
#endif
...
void FMobileSceneRenderer::RenderForward(FRDGBuilder& GraphBuilder, FRDGTextureRef ViewFamilyTexture, FSceneTextures& SceneTextures, FMobileBasePassTextures& BasePassTextures, FInstanceCullingManager& InstanceCullingManager)
{
	...
	FRenderViewContextArray RenderViews;
	GetRenderViews(Views, GetDecalViewOffset(Views), RenderViews);
	...
	for (FRenderViewContext& ViewContext : RenderViews)
	{
		...
		SCOPED_GPU_MASK(GraphBuilder.RHICmdList, !View.IsInstancedStereo() ? View.GPUMask : (View.GPUMask | View.GetInstancedView()->GPUMask));
...
void FMobileSceneRenderer::RenderForwardSinglePass(FRDGBuilder& GraphBuilder, FMobileRenderPassParameters* PassParameters, FRenderViewContext& ViewContext, FSceneTextures& SceneTextures, FInstanceCullingManager& InstanceCullingManager, FViewOcclusionQueries* ViewOcclusionQueries)
{
	...
	GraphBuilder.AddPass(
		...
		ERDGPassFlags::Raster | ERDGPassFlags::NeverMerge,
		[this, DecalViewInfo, PassParameters, ParameterCollection, ViewContext, bDoOcclusionInMainPass, &SceneTextures, ViewQueries = bDoOcclusionInMainPass ? MoveTemp(*ViewOcclusionQueries) : FViewOcclusionQueries()](FRHICommandList& RHICmdList)
	{
		FViewInfo& View = *ViewContext.ViewInfo;
		...
		RHICmdList.NextSubpass();
		RenderDecals(RHICmdList, View, DecalViewInfo, &ParameterCollection->MeshDecalSceneColorInstanceCullingDrawParams);
	...
	RenderMobileDebugViewMode(GraphBuilder, View, SceneTextures, InstanceCullingManager, *PassParameters);
...
void FMobileSceneRenderer::RenderForwardMultiPass(FRDGBuilder& GraphBuilder, FMobileRenderPassParameters* PassParameters, FRenderViewContext& ViewContext, FSceneTextures& SceneTextures, FInstanceCullingManager& InstanceCullingManager, FViewOcclusionQueries* ViewOcclusionQueries)
{
	...
	if (ViewOcclusionQueries != nullptr)
	{
		FMobileRenderPassParameters* OcclusionPassParams = GraphBuilder.AllocParameters<FMobileRenderPassParameters>();
		OcclusionPassParams->View = PassParameters->View;
		OcclusionPassParams->MobileBasePass = PassParameters->MobileBasePass;
		OcclusionPassParams->RenderTargets.DepthStencil = FDepthStencilBinding(SceneTextures.Depth.Resolve, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthRead_StencilNop);
		OcclusionPassParams->RenderTargets.MultiViewCount = PassParameters->RenderTargets.MultiViewCount;
		OcclusionPassParams->RenderTargets.NumOcclusionQueries = ComputeNumOcclusionQueriesToBatch();
	...
	*SecondPassParameters = *PassParameters;
	...
	GraphBuilder.AddPass(
		...
		ERDGPassFlags::Raster,
		[this, SecondParameterCollection, ViewContext, &SceneTextures, DecalViewInfo](FRHICommandList& RHICmdList)
	{
		FViewInfo& View = *ViewContext.ViewInfo;
		auto SecondPassParameters = &SecondParameterCollection->PassParameters;
			
		// scene depth is read only and can be fetched
		RenderDecals(RHICmdList, View, DecalViewInfo, &SecondParameterCollection->MeshDecalSceneColorInstanceCullingDrawParams);
		RenderModulatedShadowProjections(RHICmdList, ViewContext.ViewIndex, View);
		RenderFog(RHICmdList, View);
		// Draw translucency.
...
FColorTargets FMobileSceneRenderer::GetColorTargets_Deferred(FSceneTextures& SceneTextures)
{
	...
	else
	{
		...
		if (MobileUsesExtendedGBuffer(ShaderPlatform))
		{
...
void FMobileSceneRenderer::RenderDeferredSinglePass(FRDGBuilder& GraphBuilder, FSceneTextures& SceneTextures, const FSortedLightSetSceneInfo& SortedLightSet, FMobileBasePassTextures& BasePassTextures, FInstanceCullingManager& InstanceCullingManager)
{
	...
	bool bRenderDeferredLighting = ViewFamily.EngineShowFlags.Lighting && ViewFamily.EngineShowFlags.DeferredLighting;
	...
	for (FRenderViewContext& ViewContext : RenderViews)
	{
		...
		EDecalRenderStage DecalStage = bRequiresDBufferDecals ? EDecalRenderStage::Emissive : EDecalRenderStage::MobileBeforeLighting;
		FDecalViewInfo DecalViewInfo = FDecalViewInfo(ViewContext.DecalViewOffset, ViewContext.ViewIndex,
													  DecalStage,
		...
		GraphBuilder.AddPass(
			...
			ViewQueries = (bDoOcclusionQueries) ? MoveTemp(QueriesPerView[ViewContext.ViewIndex]) : FViewOcclusionQueries()](FRHICommandList& RHICmdList)
			{
				...
				RHICmdList.NextSubpass();
				RenderDecals(RHICmdList, View, DecalViewInfo, &ParameterCollection->MeshDecalSceneColorAndGBufferInstanceCullingDrawParams);
...
void FMobileSceneRenderer::RenderDeferredMultiPassDecals(FRDGBuilder& GraphBuilder, FViewInfo& View, int32 DecalViewOffset, uint32 ViewIndex, FSceneTextures& SceneTextures, FInstanceCullingManager& InstanceCullingManager)
{
	if (DecalViewOffset == INDEX_NONE ||
		(Scene->Decals.Num() == 0 && !HasAnyDraw(View.ParallelMeshDrawCommandPasses[EMeshPass::MeshDecal_SceneColorAndGBuffer])
								  && !HasAnyDraw(View.ParallelMeshDrawCommandPasses[EMeshPass::MeshDecal_SceneColorAndGBufferNoNormal])))
	{
	...
	const EDecalRenderTargetMode DecalRenderTargetModes[] = { EDecalRenderTargetMode::SceneColorAndGBuffer, EDecalRenderTargetMode::SceneColorAndGBufferNoNormal };
	const EDecalRenderStage DecalRenderStage = bRequiresDBufferDecals ? EDecalRenderStage::Emissive : EDecalRenderStage::MobileBeforeLighting;
	constexpr const TCHAR* PassNames[2] = { TEXT("SceneColorAndGBuffer"), TEXT("SceneColorAndGBufferNoNormal")};
	TConstArrayView<const FVisibleDecal*> RelevantDeferredDecals = GetRelevantDeferredDecals(DecalViewOffset, ViewIndex, DecalRenderStage);

	for (const EDecalRenderTargetMode DecalRenderTargetMode : DecalRenderTargetModes)
	{
		EMeshPass::Type MeshDecalsPass = DecalRendering::GetMeshPassType(DecalRenderTargetMode);

		const FVisibleDecal* const* FirstDecal = RelevantDeferredDecals.FindByPredicate( [&View, &DecalRenderTargetMode, &DecalRenderStage](const FVisibleDecal* Decal) { return DecalRenderTargetMode == DecalRendering::GetRenderTargetMode(View.GetShaderPlatform(), Decal->BlendDesc, DecalRenderStage); });
		if (FirstDecal == nullptr && !HasAnyDraw(View.ParallelMeshDrawCommandPasses[MeshDecalsPass]))
		{
			continue;
		}
		...
		RenderTargets[ColorTargetIndex++] = FRenderTargetBinding(SceneTextures.Color.Target, ERenderTargetLoadAction::ELoad);
		if (DecalRenderTargetMode != EDecalRenderTargetMode::SceneColorAndGBufferNoNormal)
		{
			RenderTargets[ColorTargetIndex++] = FRenderTargetBinding(SceneTextures.GBufferA, ERenderTargetLoadAction::ELoad);
		}
		RenderTargets[ColorTargetIndex++] = FRenderTargetBinding(SceneTextures.GBufferB, ERenderTargetLoadAction::ELoad);
		RenderTargets[ColorTargetIndex++] = FRenderTargetBinding(SceneTextures.GBufferC, ERenderTargetLoadAction::ELoad);
		...
		EMobileSceneTextureSetupMode TextureReadAccess = EMobileSceneTextureSetupMode::SceneDepth;
		if (DecalRenderTargetMode == EDecalRenderTargetMode::SceneColorAndGBufferNoNormal)
		{
			TextureReadAccess |= EMobileSceneTextureSetupMode::GBufferA;
		}
		PassParameters->MobileBasePass = CreateMobileBasePassUniformBuffer(GraphBuilder, View, EMobileBasePass::Opaque, TextureReadAccess);
		...
		FDecalViewInfo DecalViewInfo = FDecalViewInfo(DecalViewOffset, 
			ViewIndex,
			DecalRenderStage,
			DecalRenderTargetMode,
			RelevantDeferredDecals);

		GraphBuilder.AddPass(
			RDG_EVENT_NAME("Decals_%s", DecalRendering::GetRenderTargetModeName(DecalRenderTargetMode)),
			PassParameters,
			ERDGPassFlags::Raster,
			[this, DecalViewOffset, DecalViewInfo, PassParameters, &View](FRHICommandList& RHICmdList)
			{
				RenderDecals(RHICmdList, View, DecalViewInfo, &PassParameters->InstanceCullingDrawParams);
			});
	}
}
...
void FMobileSceneRenderer::RenderDeferredMultiPass(FRDGBuilder& GraphBuilder, FSceneTextures& SceneTextures, const FSortedLightSetSceneInfo& SortedLightSet, FMobileBasePassTextures& BasePassTextures, FInstanceCullingManager& InstanceCullingManager)
{
	...
	SceneTextures.MobileSetupMode = EMobileSceneTextureSetupMode::SceneDepth | EMobileSceneTextureSetupMode::GBuffers | EMobileSceneTextureSetupMode::SceneVelocity;
	SceneTextures.MobileUniformBuffer = CreateMobileSceneTextureUniformBuffer(GraphBuilder, &SceneTextures, SceneTextures.MobileSetupMode);
	...
	if (bShouldRenderVolumetricFog && !bIsFullDepthPrepassEnabled)
	{
		ComputeVolumetricFog(GraphBuilder, SceneTextures);
	}
	...
	if (!HasBeenProduced(BasePassTextures.AmbientOcclusionTexture))
	{
		BasePassTextures.AmbientOcclusionTexture = RenderAmbientOcclusion(GraphBuilder, SceneTextures);
	}
	...
	const bool bShouldRenderSingleLayerWater = ShouldRenderSingleLayerWater(Views);
	const bool bNeedsDepthCopy = bShouldRenderSingleLayerWater;
	...
	for (FRenderViewContext& ViewContext : RenderViews)
	{
		...
		EMobileSceneTextureSetupMode SetupMode = 
			...
			EMobileSceneTextureSetupMode::SceneVelocity |
		...
		const EMobileSSRQuality MobileSSRQuality = ActiveMobileSSRQuality(View, bShouldRenderVelocities);
		const bool bRenderDeferredLighting = ViewFamily.EngineShowFlags.Lighting && ViewFamily.EngineShowFlags.DeferredLighting;

		// Determine if a shadow point light is present.
		TArray<int32, TInlineAllocator<8>> ShadowPointLightIndexes;
		GatherShadowPointLights(View, SortedLightSet, ShadowPointLightIndexes);
		const bool bSeparateShadowPointLights = (ShadowPointLightIndexes.Num() > 0);

		{
			RDG_EVENT_SCOPE(GraphBuilder, "LightingAndFog", ViewIndex);

			if (bRenderDeferredLighting)
			{
				GraphBuilder.AddPass(
					RDG_EVENT_NAME("Lighting"),
					PassParameters,
					ERDGPassFlags::Raster,
					[this, PassParameters, ViewContext, MobileSSRQuality, &SortedLightSet](FRHICommandList& RHICmdList)
					{
						FViewInfo& View = *ViewContext.ViewInfo;
				...
				if (bSeparateShadowPointLights)
				{
					for (int32 Index = 0; Index < ShadowPointLightIndexes.Num(); ++Index)
					{
						const FSortedLightSceneInfo& SortedLight = SortedLightSet.SortedLights[ShadowPointLightIndexes[Index]];
						const FLightSceneInfo& LightSceneInfo = *SortedLight.LightSceneInfo;

						RDG_EVENT_SCOPE(GraphBuilder, "PointLightShadow_%s", LightSceneInfo.Proxy->GetOwnerNameOrLabel());
						{
							ClearMobileScreenSpaceShadows(GraphBuilder, View, LightSceneInfo.Proxy, BasePassTextures.ScreenSpaceShadowMaskTexture);
							RenderShadowProjections(GraphBuilder, SceneTextures, BasePassTextures.ScreenSpaceShadowMaskTexture, nullptr, &LightSceneInfo, false);

							GraphBuilder.AddPass(
								RDG_EVENT_NAME("PointLightShadowPass"),
								PassParameters,
								ERDGPassFlags::Raster,
								[this, &View, LightSceneInfo](FRHICommandList& RHICmdList)
								{
									MobileDeferredRenderLocalLight(RHICmdList, *Scene, View, LightSceneInfo, VisibleLightInfos);
								});
						}
					}
				}
		...
		if (IsMobileDistortionBeforeTranslucency())
		{
			RenderDistortion(GraphBuilder, View, SceneTextures.Color.Target, SceneTextures.Depth.Target);
		}
			
		const bool bRenderTranslucency = ShouldRenderTranslucency(StandardTranslucencyPass, Views) && ViewFamily.EngineShowFlags.Translucency && HasAnyDraw(View.ParallelMeshDrawCommandPasses[StandardTranslucencyMeshPass]); 
		const bool bRenderSingleLayerWater = bShouldRenderSingleLayerWater && View.bHasSingleLayerWaterMaterial && HasAnyDraw(View.ParallelMeshDrawCommandPasses[EMeshPass::SingleLayerWaterPass]);
		ETranslucencyView TranslucencyViewsToRender = bRenderTranslucency ? GetTranslucencyViews(Views) : ETranslucencyView::None;
		const bool bIsCameraUnderWater = EnumHasAnyFlags(TranslucencyViewsToRender, ETranslucencyView::UnderWater);
		
		// - Above water : render water then translucencies
		// - Under water : render translucencies then water
		auto AddTranslucencyPass = [&]()
		{
			check(bRenderTranslucency);

			// No need to copy SceneColor before translucencies when underwater
			if (View.TranslucentPrimCount.UseSceneColorCopy(StandardTranslucencyPass) && (!bRenderSingleLayerWater || !bIsCameraUnderWater))
			{
				FRDGTextureRef SceneColorCopyTexture = AddCopySceneColorPass(GraphBuilder, Views, SceneTextures.Color, false, false, true);
		...
		if (bIsCameraUnderWater && bRenderTranslucency)
		{
			AddTranslucencyPass();
		}
			
		if (bRenderSingleLayerWater)
		{
			...
			*SLWPassParameters = *PassParameters;
			SLWPassParameters->RenderTargets[0] = BasePassRenderTargets[0];
			SLWPassParameters->RenderTargets.DepthStencil = FDepthStencilBinding(BasePassRenderTargets.DepthStencil.GetTexture(), ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthWrite_StencilWrite);
			...
			FRDGTextureRef SceneColorCopyTexture = AddCopySceneColorPass(GraphBuilder, Views, SceneTextures.Color, false, false, false);
			if (SceneColorCopyTexture != nullptr)
			{
...
void FMobileSceneRenderer::RenderMobileDebugViewMode(
	FRDGBuilder& GraphBuilder,
	FViewInfo& View,
	FSceneTextures& SceneTextures,
	FInstanceCullingManager& InstanceCullingManager,
	const FMobileRenderPassParameters& BasePassParameters)
{
	...
	auto* Pass = View.ParallelMeshDrawCommandPasses[EMeshPass::DebugViewMode];
	if (!Pass)
	{
		return;
	}

	FRDGTextureRef SceneColorTexture = BasePassParameters.RenderTargets[0].GetTexture();
	if (!SceneColorTexture)
	{
		return;
	}
	FRDGTextureRef SceneColorResolveTexture = BasePassParameters.RenderTargets[0].GetResolveTexture();

	auto* PassParameters = GraphBuilder.AllocParameters<FMobileRenderPassParameters>();
	PassParameters->View = BasePassParameters.View;
	PassParameters->MobileBasePass = BasePassParameters.MobileBasePass;
	PassParameters->ReflectionCapture = BasePassParameters.ReflectionCapture;
	PassParameters->LocalFogVolumeInstances = BasePassParameters.LocalFogVolumeInstances;
	PassParameters->LocalFogVolumeTileDrawIndirectBuffer = BasePassParameters.LocalFogVolumeTileDrawIndirectBuffer;
	PassParameters->LocalFogVolumeTileDataTexture = BasePassParameters.LocalFogVolumeTileDataTexture;
	PassParameters->LocalFogVolumeTileDataBuffer = BasePassParameters.LocalFogVolumeTileDataBuffer;
	PassParameters->HalfResLocalFogVolumeViewSRV = BasePassParameters.HalfResLocalFogVolumeViewSRV;
	PassParameters->HalfResLocalFogVolumeDepthSRV = BasePassParameters.HalfResLocalFogVolumeDepthSRV;
	// Shader-complexity output is additive over black; folding the clear into the load action avoids
	// an extra render-pass scope on tile GPUs.
	PassParameters->RenderTargets[0] = FRenderTargetBinding(SceneColorTexture, SceneColorResolveTexture, ERenderTargetLoadAction::EClear);
	{
		const EDebugViewShaderMode DebugViewMode = ViewFamily.GetDebugViewShaderMode();
		const bool bWriteToSharedDebugAux =	DebugViewMode == DVSM_FarShadowCasters && SceneTextures.DebugAux != nullptr;

		if (bWriteToSharedDebugAux)
		{
			// Normally the mobile renderer doesn't contain the same DebugAuxTexture path as the deferred shading renderer
			PassParameters->RenderTargets[1] = FRenderTargetBinding(SceneTextures.DebugAux, ERenderTargetLoadAction::ELoad);
		}
		else
		{
			FRDGTextureDesc DummyDesc = SceneColorTexture->Desc;
			DummyDesc.Format = PF_R8_UINT;
			DummyDesc.Flags = TexCreate_ShaderResource | TexCreate_RenderTargetable | TexCreate_UAV;
			DummyDesc.ClearValue = FClearValueBinding::Black;
			FRDGTextureRef DummyDebugAux = GraphBuilder.CreateTexture(DummyDesc, TEXT("DummyDebugAux"));
			PassParameters->RenderTargets[1] = FRenderTargetBinding(DummyDebugAux, ERenderTargetLoadAction::ENoAction);
		}
	}
	...
	PassParameters->RenderTargets.MultiViewCount = BasePassParameters.RenderTargets.MultiViewCount;

	BuildMeshRenderingCommands(
			GraphBuilder, EMeshPass::DebugViewMode, View, Scene->GPUScene,
			InstanceCullingManager, PassParameters->InstanceCullingDrawParams);

	GraphBuilder.AddPass(
		RDG_EVENT_NAME("MobileDebugViewMode"),
		PassParameters,
		ERDGPassFlags::Raster,
...
void FMobileSceneRenderer::UpdateDirectionalLightUniformBuffers(FRDGBuilder& GraphBuilder, const FViewInfo& View)
{
	...
	{
		const bool bRenderDirectionalLights = ViewFamily.EngineShowFlags.Lighting && ViewFamily.EngineShowFlags.DirectLighting && ViewFamily.EngineShowFlags.DirectionalLights;
		const bool bDynamicShadows = ViewFamily.EngineShowFlags.DynamicShadows;
...
void FMobileSceneRenderer::PreTonemapMSAA(FRHICommandList& RHICmdList, const FMinimalSceneTextures& SceneTextures)
{
	...
	const auto ShaderMap = GetGlobalShaderMap(ViewFamily.GetShaderPlatform());
...
bool FMobileSceneRenderer::ShouldRenderHZB(TArrayView<FViewInfo> InViews)
{
	...
	bool bIsFeatureRequested =  RequiresMobileAmbientOcclusionPass(Scene, InViews[0]);
...
void FMobileSceneRenderer::RenderHZB(FRDGBuilder& GraphBuilder, FRDGTextureRef SceneDepthTexture)
{
	...
	if (!HasBeenProduced(SceneDepthTexture))
	{
		return;
	}
