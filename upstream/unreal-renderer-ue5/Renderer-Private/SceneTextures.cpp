					...
					if (bIsVRScene)
					{
						if (!bIsSceneCapture && !bIsReflectionCapture)
						{
							...
							if (DesiredFamilyExtent.X != LastStereoExtent.X || DesiredFamilyExtent.Y != LastStereoExtent.Y)
							{
								...
								UE_LOGF(LogRenderer, Warning, "Resizing VR buffer to %d by %d", DesiredFamilyExtent.X, DesiredFamilyExtent.Y);
							}
								...
								if (LastStereoExtent.X > 0 && bIsSceneCapture)
								{
									...
									if (!DisplayedCaptureSizeWarning)
									{
										...
										UE_LOGF(LogRenderer, Warning, 
											"Scene capture of %d by %d is larger than the current VR target. If this is deliberate for a capture that is being done for multiple frames, consider the performance and memory implications. "
											"To disable this warning and ensure optimal behavior with this path, set r.SceneRenderTargetResizeMethod to 2, and r.SceneRenderTargetResizeMethodForceOverride to 1.", 
											DesiredFamilyExtent.X, DesiredFamilyExtent.Y);
									}
								}
								SceneTargetsSizingMethod = RequestedSize;
							}
...
void FSceneTextures::InitializeViewFamily(FRDGBuilder& GraphBuilder, FViewFamilyInfo& ViewFamily, FIntPoint FamilySize)
{
	...
	if (Config.ShadingPath == EShadingPath::Deferred &&
		UseSeparatedSubsurfaceDiffuse(Config.ShaderPlatform))
	{
		FRDGTextureDesc Desc = FRDGTextureDesc::Create2D(Config.Extent, PF_R16F, FClearValueBinding::Black, TexCreate_RenderTargetable | TexCreate_ShaderResource | TexCreate_UAV);
		SceneTextures.SeparatedSubsurfaceDiffuse = GraphBuilder.CreateTexture(Desc, TEXT("SeparatedSubsurfaceDiffuse"));
	}
...
#if WITH_EDITOR
	{
		...
		FRDGTextureDesc DepthDesc(FRDGTextureDesc::Create2D(Config.Extent, PF_DepthStencil, FClearValueBinding::DepthFar, TexCreate_ShaderResource | TexCreate_DepthStencilTargetable | TexCreate_InputAttachmentRead, 1, Config.EditorPrimitiveNumSamples));
		DepthDesc.AliasableFormats.Add(PF_X24_G8);
		SceneTextures.EditorPrimitiveDepth = GraphBuilder.CreateTexture(DepthDesc, TEXT("Editor.PrimitivesDepth"));
	}
#endif

	SceneTextures.DBufferTextures = MakePimpl<FDBufferTextures, EPimplPtrMode::DeepCopy>();
	SceneTextures.SubstrateSceneData = MakePimpl<FSubstrateSceneData, EPimplPtrMode::DeepCopy>();
...
#if WITH_DEBUG_VIEW_MODES
	{
		...
		FClearValueBinding DebugAuxClearValue = FClearValueBinding::None;
		...
		else if (DebugViewMode == DVSM_FarShadowCasters	|| (DebugViewMode == DVSM_ShadowCasters && Config.ShadingPath != EShadingPath::Mobile))
		{
			// DVSM_ShadowCasters is a no-op on mobile (VSM unavailable) so don't allocate DebugAux texture
			DebugAuxTextureExtent = Config.Extent;
			DebugAuxClearValue = FClearValueBinding::Black;
		}

		const FRDGTextureDesc DebugAuxTextureDesc(FRDGTextureDesc::Create2D(DebugAuxTextureExtent, DebugAuxTextureFormat, DebugAuxClearValue, TexCreate_ShaderResource | TexCreate_RenderTargetable | TexCreate_UAV));
...
TArray<FRDGTextureRef> FSceneTextures::EnumerateSceneTextures() const
{
	...
	AddTextureIfNonNull(DitheredHalfResDepth, Results);
...
FRHITexture* FSceneTextureExtracts::GetDepthTexture() const
{
