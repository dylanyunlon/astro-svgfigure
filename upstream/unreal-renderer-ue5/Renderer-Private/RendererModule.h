...
class FInstanceCullingGlobalUniforms;
...
public:
	...
	virtual FSceneRemovedEvent::RegistrationType& GetSceneRemovedEvent() override;
	...
	virtual FSceneUniformBuffer* CreateSinglePrimitiveSceneUniformBuffer(FRDGBuilder& GraphBuilder, const FPrimitiveSceneProxy* PrimitiveSceneProxy, ERHIFeatureLevel::Type FeatureLevel, FMeshBatch& Mesh) override;
	...
	virtual TRDGUniformBufferRef<FInstanceCullingGlobalUniforms> CreateSinglePrimitiveInstanceCullingUniformBuffer(FRDGBuilder& GraphBuilder, FStaticShaderPlatform ShaderPlatform, FMeshBatch& Mesh) override;
	...
	virtual IMaterialCacheVirtualTextureAllocator* GetMaterialCacheVirtualTextureAllocator() override;
	...
	virtual uint64 GetVirtualTextureRequestRecordBuffer(TMap<uint64, uint32>& OutRequestData) override;
	virtual void PackVirtualTextureRequestsToStream(TMap<uint64, uint32> const& InPageRequests, TArray<uint32>& OutRequestStream) override;
	virtual void RequestVirtualTextureTiles(TArray<uint32>&& InRequestStream, FIntPoint const& InCaptureResolution) override;

	virtual void SetNaniteRequestRecordBuffer(uint64 Handle) override;
	virtual uint64 GetNaniteRequestRecordBuffer(TMap<uint64, uint32>& OutPageRequests) override; 	
	virtual void PackNaniteRequestsToStream(TMap<uint64, uint32> const& InPageRequests, TArray<uint32>& OutRequestStream) override;
	virtual void RequestNanitePages(TArray<uint32>&& InRequestStream) override;
	...
	virtual IScenePrimitiveRenderingContext* BeginScenePrimitiveRendering(FRDGBuilder& GraphBuilder, FSceneViewFamily* ViewFamily, EBeginScenePrimitiveRenderingFlags BeginScenePrimitiveRenderingFlags = EBeginScenePrimitiveRenderingFlags::All) override;
	virtual IScenePrimitiveRenderingContext* BeginScenePrimitiveRendering(FRDGBuilder& GraphBuilder, FSceneInterface& Scene, EBeginScenePrimitiveRenderingFlags BeginScenePrimitiveRenderingFlags = EBeginScenePrimitiveRenderingFlags::All) override;
	...
	virtual IStateStreamManager* GetStateStreamManager() override;
	virtual void DebugLogRenderProxies(FName Type, uint32 Num) override;
#endif
...
private:
	...
	FSceneRemovedEvent SceneRemovedEvent;
	FDelegateHandle StopRenderingThreadDelegate;

#if WITH_STATE_STREAM
	class FRendererStateStreamManager;
	FRendererStateStreamManager* StateStreamManager = nullptr;
#endif
};
