...
void FInstanceCullingMergedContext::MergeBatches()
{
	...
	UniquePrimitives.Empty(TotalUniquePrimitives);
	...
	// Mobile UBO path: byte offset into BatchedPrimitiveData (= absolute primitive slot * stride).
	uint32 PrimitiveDataByteOffset = 0U;
	...
	for (int32 BatchIndex = 0; BatchIndex < Batches.Num(); ++BatchIndex)
	{
		...
		UniquePrimitives.Append(InstanceCullingContext.UniquePrimitives);

		BatchInfo.ViewIdsOffset = ViewIds.Num();
		BatchInfo.NumViewIds_Flags = uint32(InstanceCullingContext.ViewIds.Num()) << INSTANCE_CULLING_FLAGS_BITS;
		if (InstanceCullingContext.PrevHZB.IsValid())
		{
			BatchInfo.NumViewIds_Flags |= INSTANCE_CULLING_FLAGS_ALLOW_OCCLUSION_CULLING;
		}
		if (EnumHasAnyFlags(InstanceCullingContext.Flags, EInstanceCullingFlags::DrawOnlySelected))
		{
			BatchInfo.NumViewIds_Flags |= INSTANCE_CULLING_FLAGS_DRAW_ONLY_SELECTED;
		}
		...
		Result.PrimitiveDataByteOffset = PrimitiveDataByteOffset;
		...
		// Mobile UBO path: byte offset advances by the sub-context's unique primitive count * stride.
		PrimitiveDataByteOffset = UniquePrimitives.Num() * BATCHED_PRIMITIVE_DATA_STRIDE;
	}
	TotalUniquePrimitives = UniquePrimitives.Num();
}
...
void FInstanceCullingMergedContext::AddBatchItem(const FBatchItem& BatchItem)
{
	...
	if (Context->HasCullingCommands())
	{
		...
		InstanceIdBufferElements += (Context->TotalInstances * Context->ViewIds.Num());
		BatchedInstanceDataBufferElements += (Context->TotalInstances * Context->ViewIds.Num() * BATCHED_INSTANCE_DATA_STRIDE_FLOAT4);
		// Mobile UBO path: one primitive record per unique primitive (per AddInstancesToDrawCommand call).
		BatchedPrimitiveDataBufferElements += (Context->TotalUniquePrimitives * Context->ViewIds.Num() * BATCHED_PRIMITIVE_DATA_STRIDE_FLOAT4);
