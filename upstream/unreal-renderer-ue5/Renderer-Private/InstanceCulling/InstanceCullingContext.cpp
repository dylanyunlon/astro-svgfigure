...
// FInstanceCullingGlobalUniformsContents is the byte-level mirror used to fill the dynamic layout below
static_assert(sizeof(FInstanceCullingGlobalUniformsContents) == sizeof(FInstanceCullingGlobalUniforms), "FInstanceCullingGlobalUniformsContents must mirror FInstanceCullingGlobalUniforms byte-for-byte.");
static_assert(alignof(FInstanceCullingGlobalUniformsContents) == alignof(FInstanceCullingGlobalUniforms), "FInstanceCullingGlobalUniformsContents alignment mismatch.");
static_assert(offsetof(FInstanceCullingGlobalUniformsContents, InstanceIdsBuffer) == offsetof(FInstanceCullingGlobalUniforms, InstanceIdsBuffer), "InstanceIdsBuffer offset mismatch.");
static_assert(offsetof(FInstanceCullingGlobalUniformsContents, PageInfoBuffer) == offsetof(FInstanceCullingGlobalUniforms, PageInfoBuffer), "PageInfoBuffer offset mismatch.");
static_assert(offsetof(FInstanceCullingGlobalUniformsContents, BatchedPrimitiveData) == offsetof(FInstanceCullingGlobalUniforms, BatchedPrimitiveData), "BatchedPrimitiveData offset mismatch.");
static_assert(offsetof(FInstanceCullingGlobalUniformsContents, BatchedInstanceData) == offsetof(FInstanceCullingGlobalUniforms, BatchedInstanceData), "BatchedInstanceData offset mismatch.");
static_assert(offsetof(FInstanceCullingGlobalUniformsContents, BufferCapacity) == offsetof(FInstanceCullingGlobalUniforms, BufferCapacity), "BufferCapacity offset mismatch.");

// Layout that hash-matches FInstanceCullingGlobalUniforms but accepts plain FRHIShaderResourceView* pointers in its resource slots.
// Used by DrawDynamicMeshPassPrivate to build per-pass instance-culling UBs at execute time outside of RDG.
class FDynamicInstanceCullingLayout : public FRenderResource
{
public:
	virtual void InitRHI(FRHICommandListBase&) override
	{
		const FShaderParametersMetadata* Metadata = FInstanceCullingGlobalUniforms::FTypeInfo::GetStructMetadata();
		const FRHIUniformBufferLayout& RegisteredLayout = Metadata->GetLayout();

		// Mirror the registered layout exactly so ComputeHash() matches
		FRHIUniformBufferLayoutInitializer Init(*RegisteredLayout.GetDebugName(), RegisteredLayout.ConstantBufferSize);
		Init.Flags = RegisteredLayout.Flags;
		Init.BindingFlags = RegisteredLayout.BindingFlags;
		Init.StaticSlot = RegisteredLayout.StaticSlot;
		for (const FRHIUniformBufferResource& R : RegisteredLayout.Resources)
		{
			Init.Resources.Add(FRHIUniformBufferResourceInitializer{ R.MemberOffset, R.MemberType });
		}
		Init.ComputeHash();

		// After hashing, swap RDG resource types for plain SRV so RHICreateUniformBuffer accepts FRHIShaderResourceView* pointers in the contents
		for (FRHIUniformBufferResourceInitializer& R : Init.Resources)
		{
			if (R.MemberType == UBMT_RDG_BUFFER_SRV || R.MemberType == UBMT_RDG_UNIFORM_BLOCK)
			{
				R.MemberType = UBMT_SRV;
			}
		}
		LayoutRHI = RHICreateUniformBufferLayout(Init);
	}
	virtual void ReleaseRHI() override { LayoutRHI = nullptr; }

	FUniformBufferLayoutRHIRef LayoutRHI;
};
static TGlobalResource<FDynamicInstanceCullingLayout> GDynamicInstanceCullingLayout;

const FRHIUniformBufferLayout* GetDynamicInstanceCullingLayout()
{
	return GDynamicInstanceCullingLayout.LayoutRHI.GetReference();
}
...
FMeshDrawCommandOverrideArgs GetMeshDrawCommandOverrideArgs(const FInstanceCullingDrawParams& InstanceCullingDrawParams)
{
	...
	Result.PrimitiveDataByteOffset = InstanceCullingDrawParams.PrimitiveDataByteOffset;
	...
	if (Result.InstanceCullingStaticUB)
	{
		//TODO: should be computed/extracted from FInstanceCullingGlobalUniforms layout		
		const int32 BatchedPrimitiveDataIdx = 2;
		const int32 BatchedInstanceDataIdx = 3;
		Result.BatchedPrimitiveDataSRV = static_cast<FRHIShaderResourceView*>(Result.InstanceCullingStaticUB->GetResourceTable()[BatchedPrimitiveDataIdx].GetReference());
		Result.BatchedInstanceDataSRV = static_cast<FRHIShaderResourceView*>(Result.InstanceCullingStaticUB->GetResourceTable()[BatchedInstanceDataIdx].GetReference());
	}
...
FUniformBufferStaticSlot FInstanceCullingContext::GetStaticUniformBufferSlot(EShaderPlatform ShaderPlatform)
{
	...
	return FUniformBufferStaticSlotRegistry::Get().FindSlotByName(InstanceCullingSlotName);
}
...
uint32 FInstanceCullingContext::GetInstanceIdBufferStride(EShaderPlatform ShaderPlatform)
{
	...
	{
		return BATCHED_INSTANCE_DATA_STRIDE;
	}
	else
	{
...
uint32 FInstanceCullingContext::StepInstanceDataOffsetBytes(uint32 NumStepDraws) const
{
	...
	{
		return TotalInstances * BATCHED_INSTANCE_DATA_STRIDE * ViewIds.Num();
	}
	else
	{
...
uint32 FInstanceCullingContext::GetInstanceIdNumElements() const
{
	if (bUsesUniformBufferView)
	{
		...
		return TotalInstances * BATCHED_INSTANCE_DATA_STRIDE_FLOAT4 * ViewIds.Num();
	}
	else
	{
...
FInstanceCullingContext::FInstanceCullingContext(
	...
	FSceneCullingRendererHierarchicalCullingQuery* InCullingQuery) :
	...
	InstanceCullingStaticSlot(MAX_UNIFORM_BUFFER_STATIC_SLOTS),
	CullingQuery(InCullingQuery)
{
	...
	{	
		// Mobile manually merges several mesh passes into one RDG pass (e.g. RenderForwardSinglePass), RDG only auto-binds one InstanceCulling UB at the outer pass. 
		// Need to explicitly re-bind InstanceCulling UB to a static slot for each mesh pass
		InstanceCullingStaticSlot = GetStaticUniformBufferSlot(InShaderPlatform);
	}
}
...
void FInstanceCullingContext::ResetCommands(int32 MaxNumCommands)
{
	...
	UniquePrimitives.Empty(bUsesUniformBufferView ? MaxNumCommands : 0);
	TotalInstances = 0U;
	TotalUniquePrimitives = 0U;
	bHasAnyIndirectDraw = false;
...
void FInstanceCullingContext::AddInstancesToDrawCommand(uint32 IndirectArgsOffset, int32 InstanceDataOffset, uint32 RunOffset, uint32 NumInstances, EInstanceFlags InstanceFlags, int32 PrimitiveSlotInDraw)
{
	...
	if (bPreserveInstanceOrder)
	{
		...
		PayloadData.Emplace(bDynamicInstanceDataOffset, IndirectArgsOffset, InstanceDataOffset, RunOffset, DrawCommandCompactionData.Num(), uint32(PrimitiveSlotInDraw));
	}
	else if (bUsesUniformBufferView)
	{
		// Mobile UBO compact layout
		//   bits 2-7  : PrimitiveSlotInDraw (6 bits, max 63 bounded by MaxInstanceBatchSize)
		//   bits 8-25 : IndirectArgsOffset (18 bits, max ~262K draws)
		Payload |= ((uint32(PrimitiveSlotInDraw) & 0x3Fu) << INSTANCE_CULLING_PAYLOAD_NUM_COMMON_BITS);
		Payload |= (IndirectArgsOffset << (INSTANCE_CULLING_PAYLOAD_NUM_COMMON_BITS + 6u));
	}
	else
	{
...
void FInstanceCullingContext::AddInstancesToDrawCommand(uint32 IndirectArgsOffset, int32 InstanceDataOffset, uint32 RunOffset, uint32 NumInstances, EInstanceFlags InstanceFlags, uint32 MaxBatchSize, int32 PrimitiveSlotInDraw)
{
	...
	{
		AddInstancesToDrawCommand(IndirectArgsOffset, InstanceDataOffset, RunOffset, NumInstances, InstanceFlags, PrimitiveSlotInDraw);
		return;
	}
	...
	if (RunOffset > 0 && NumInstances > 0)
	{
		...
		if (NumInstancesInBatch > 0)
		{
			...
			AddInstancesToDrawCommand(IndirectArgsOffset, InstanceDataOffset, RunOffset, NumInstancesInBatch, InstanceFlags, PrimitiveSlotInDraw);
...
void FInstanceCullingContext::AddInstanceRunsToDrawCommand(uint32 IndirectArgsOffset, int32 InstanceDataOffset, const uint32* Runs, uint32 NumRuns, EInstanceFlags InstanceFlags, uint32 MaxBatchSize, int32 PrimitiveSlotInDraw)
{
	...
	for (uint32 Index = 0; Index < NumRuns; ++Index)
	{
		...
		AddInstancesToDrawCommand(IndirectArgsOffset, InstanceDataOffset + RunStart, NumInstancesInRuns, NumInstances, InstanceFlags | EInstanceFlags::ForceInstanceCulling, MaxBatchSize, PrimitiveSlotInDraw);
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<float4>, BatchedInstanceDataOut)
	END_SHADER_PARAMETER_STRUCT()
};
...
public:
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		OutEnvironment.SetDefine(TEXT("PRIMITIVE_HAS_TILEOFFSET_DATA"), GPrimitiveHasTileOffsetData);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, bDrawOnlySelected)
	END_SHADER_PARAMETER_STRUCT()
};
...
// Mobile UBO path: writes the per-draw-command primitive record table
class FBuildBatchedPrimitiveDataCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FBuildBatchedPrimitiveDataCS);
	SHADER_USE_PARAMETER_STRUCT(FBuildBatchedPrimitiveDataCS, FGlobalShader)
...
public:
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		return PlatformGPUSceneUsesUniformBufferView(Parameters.Platform);
	}
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer< FInstanceCullingContext::FUniquePrimitiveInfo >, UniquePrimitives)
		SHADER_PARAMETER_RDG_BUFFER_UAV(RWStructuredBuffer<float4>, BatchedPrimitiveDataOut)

		SHADER_PARAMETER(uint32, NumUniquePrimitives)
		SHADER_PARAMETER(uint32, DynamicInstanceIdOffset)
	END_SHADER_PARAMETER_STRUCT()
};
IMPLEMENT_GLOBAL_SHADER(FBuildBatchedPrimitiveDataCS, "/Engine/Private/InstanceCulling/BuildBatchedPrimitiveData.usf", "BuildBatchedPrimitiveDataCS", SF_Compute);
...
const TRDGUniformBufferRef<FInstanceCullingGlobalUniforms> FInstanceCullingContext::CreateDummyInstanceCullingUniformBuffer(FRDGBuilder& GraphBuilder)
{
	...
	{
		FRDGBufferDesc UBODesc = FRDGBufferDesc::CreateStructuredDesc(16u, (PLATFORM_MAX_UNIFORM_BUFFER_RANGE / 16u));
		UBODesc.Usage |= EBufferUsageFlags::UniformBuffer | EBufferUsageFlags::ShaderResource;
		FRDGBufferSRVRef DummyUBOView = GraphBuilder.CreateSRV(GraphBuilder.CreateBuffer(UBODesc, TEXT("DummyUBO")), PF_A32B32G32R32F);
		InstanceCullingGlobalUniforms->BatchedPrimitiveData = DummyUBOView;
		InstanceCullingGlobalUniforms->BatchedInstanceData = DummyUBOView;
	}
...
{
	check(!SyncPrerequisitesFunc);  
	return bHasAnyIndirectDraw || TotalInstances > 0u;
}
	...
	FInstanceCullingDrawParams* InstanceCullingDrawParams)
{
	...
	FRDGBufferRef InstanceIdsBuffer = GSystemTextures.GetDefaultStructuredBuffer(GraphBuilder, 4, 0u);
	FRDGBufferUAVRef InstanceIdsBufferUAV = GraphBuilder.CreateUAV(InstanceIdsBuffer, ERDGUnorderedAccessViewFlags::SkipBarrier);
	// Only for UBO path
	FRDGBufferRef BatchedInstanceDataBuffer = GSystemTextures.GetDefaultBuffer(GraphBuilder, 16, 0u);
	FRDGBufferRef BatchedPrimitiveDataBuffer = BatchedInstanceDataBuffer;
	FRDGBufferUAVRef BatchedInstanceDataBufferUAV = GraphBuilder.CreateUAV(BatchedInstanceDataBuffer, ERDGUnorderedAccessViewFlags::SkipBarrier);
	FRDGBufferUAVRef BatchedPrimitiveDataBufferUAV = BatchedInstanceDataBufferUAV;
	
	if (bUsesUniformBufferView)
	{
		// An additional max range slack to a buffer size, so when binding last element we still have a full UBO range
		const int32 BufferSlack = (PLATFORM_MAX_UNIFORM_BUFFER_RANGE / 16u);
		const int32 InstanceBufferNumElements = TotalInstances * ViewIds.Num() * BATCHED_INSTANCE_DATA_STRIDE_FLOAT4;
		
		FRDGBufferDesc Desc = FRDGBufferDesc::CreateStructuredDesc(16u /*float4*/, InstanceBufferNumElements + BufferSlack);
		Desc.Usage |= EBufferUsageFlags::UniformBuffer | EBufferUsageFlags::ShaderResource;

		BatchedInstanceDataBuffer = GraphBuilder.CreateBuffer(Desc, TEXT("InstanceCulling.BatchedInstanceDataBuffer"));
		BatchedInstanceDataBufferUAV = GraphBuilder.CreateUAV(BatchedInstanceDataBuffer, ERDGUnorderedAccessViewFlags::SkipBarrier);
		
		const int32 PrimtiveBufferNumElements = FMath::Max<uint32>(1u, TotalUniquePrimitives) * ViewIds.Num() * BATCHED_PRIMITIVE_DATA_STRIDE_FLOAT4;
		Desc.NumElements = PrimtiveBufferNumElements + BufferSlack;
		BatchedPrimitiveDataBuffer = GraphBuilder.CreateBuffer(Desc, TEXT("InstanceCulling.BatchedPrimitiveDataBuffer"));
		BatchedPrimitiveDataBufferUAV = GraphBuilder.CreateUAV(BatchedPrimitiveDataBuffer, ERDGUnorderedAccessViewFlags::SkipBarrier);
	}
	else
	{
		const int32 InstanceIdBufferNumElements = TotalInstances * ViewIds.Num();
		InstanceIdsBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(uint32), InstanceIdBufferNumElements), TEXT("InstanceCulling.InstanceIdsBuffer"));
	...
	PassParametersTmp.bDrawOnlySelected = EnumHasAnyFlags(Flags, EInstanceCullingFlags::DrawOnlySelected);
	...
	PassParametersTmp.BatchedInstanceDataOut = BatchedInstanceDataBufferUAV;
	...
	// Mobile UBO path: write BatchedPrimitiveData in a dedicated dispatch (one thread per unique primitive)
	if (bUsesUniformBufferView && UniquePrimitives.Num() > 0)
	{
		FRDGBufferRef UniquePrimitivesRDG = CreateStructuredBuffer(GraphBuilder, TEXT("InstanceCulling.UniquePrimitives"), UniquePrimitives);

		FBuildBatchedPrimitiveDataCS::FParameters* PrimParams = GraphBuilder.AllocParameters<FBuildBatchedPrimitiveDataCS::FParameters>();
		PrimParams->Scene = PassParametersTmp.Scene;
		PrimParams->GPUSceneParameters = PassParametersTmp.GPUSceneParameters;
		PrimParams->UniquePrimitives = GraphBuilder.CreateSRV(UniquePrimitivesRDG);
		PrimParams->BatchedPrimitiveDataOut = BatchedPrimitiveDataBufferUAV;
		PrimParams->NumUniquePrimitives = UniquePrimitives.Num();
		PrimParams->DynamicInstanceIdOffset = DynamicInstanceIdOffset;

		auto PrimShader = ShaderMap->GetShader<FBuildBatchedPrimitiveDataCS>();
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("BuildBatchedPrimitiveData(%u)", UniquePrimitives.Num()),
			PrimShader,
			PrimParams,
			FComputeShaderUtils::GetGroupCountWrapped(UniquePrimitives.Num(), FBuildBatchedPrimitiveDataCS::NumThreadsPerGroup));
	}
	...
	if (bOrderPreservationEnabled && NumCompactionBlocks > 0)
	{
		...
		// Compaction phase two - write instances to compact final location
		{
			...
			PassParameters->BatchedInstanceDataOut = BatchedInstanceDataBufferUAV;			
	...
	// If there were no instances & thus the compute passes were skipped we need to clear the buffer to signal to RDG that it is ok to bind it later.
	if (TotalInstances == 0u)
	{
		AddClearUAVPass(GraphBuilder, InstanceIdsBufferUAV, 0u);
	}
	...
	UniformParameters->BufferCapacity = InstanceIdsBuffer->Desc.NumElements;
	UniformParameters->BatchedInstanceData = GraphBuilder.CreateSRV(BatchedInstanceDataBuffer, PF_A32B32G32R32F);
	UniformParameters->BatchedPrimitiveData = GraphBuilder.CreateSRV(BatchedPrimitiveDataBuffer, PF_A32B32G32R32F);
...
#define INST_CULL_CREATE_STRUCT_BUFF_ARGS_BIN_INDEX(ArrayName) \
	...
	FRDGBufferRef UniquePrimitivesRDG = CreateStructuredBuffer(INST_CULL_CREATE_STRUCT_BUFF_ARGS(UniquePrimitives));
	...
	if (PlatformGPUSceneUsesUniformBufferView(ShaderPlatform))
	{
		FRDGBufferDesc Desc = FRDGBufferDesc::CreateStructuredDesc(16u /*float4*/, 1);
		Desc.Usage |= EBufferUsageFlags::UniformBuffer | EBufferUsageFlags::ShaderResource;
		
		// Add an additional max range slack to a buffer size, so when binding last element we still have a full UBO range
		BatchedInstanceDataBuffer = GraphBuilder.CreateBuffer(Desc , TEXT("InstanceCulling.BatchedInstanceDataBuffer"),
			INST_CULL_CALLBACK(DeferredContext->BatchedInstanceDataBufferElements + (PLATFORM_MAX_UNIFORM_BUFFER_RANGE / 16u)));
		BatchedInstanceDataBufferUAV = GraphBuilder.CreateUAV(BatchedInstanceDataBuffer, ERDGUnorderedAccessViewFlags::SkipBarrier);

		BatchedPrimitiveDataBuffer = GraphBuilder.CreateBuffer(Desc, TEXT("InstanceCulling.BatchedPrimitiveDataBuffer"),
			INST_CULL_CALLBACK(DeferredContext->BatchedPrimitiveDataBufferElements + (PLATFORM_MAX_UNIFORM_BUFFER_RANGE / 16u)));
		BatchedPrimitiveDataBufferUAV = GraphBuilder.CreateUAV(BatchedPrimitiveDataBuffer, ERDGUnorderedAccessViewFlags::SkipBarrier);
	}
	else
	{
		InstanceIdsBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(uint32), 1), TEXT("InstanceCulling.InstanceIdsBuffer"), 
			INST_CULL_CALLBACK(DeferredContext->InstanceIdBufferElements));
		InstanceIdsBufferUAV = GraphBuilder.CreateUAV(InstanceIdsBuffer, ERDGUnorderedAccessViewFlags::SkipBarrier);
	}
	...
	// Mobile UBO path: dedicated dispatch that writes BatchedPrimitiveData.
	if (PlatformGPUSceneUsesUniformBufferView(ShaderPlatform))
	{
		FBuildBatchedPrimitiveDataCS::FParameters* PrimParams = GraphBuilder.AllocParameters<FBuildBatchedPrimitiveDataCS::FParameters>();
		PrimParams->Scene = PassParametersTmp.Scene;
		...
		PrimParams->UniquePrimitives = GraphBuilder.CreateSRV(UniquePrimitivesRDG);
		PrimParams->BatchedPrimitiveDataOut = BatchedPrimitiveDataBufferUAV;
		PrimParams->NumUniquePrimitives = 0u; // Will be set in a callback
		PrimParams->DynamicInstanceIdOffset = 0u; // Deferred contexts encode per-sub-context offset in payload entry.

		auto PrimShader = ShaderMap->GetShader<FBuildBatchedPrimitiveDataCS>();
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("BuildBatchedPrimitiveData"),
			PrimShader,
			PrimParams,
			[PassParameters, PrimParams, DeferredContext]()
			{
				DeferredContext->ProcessBatched(PassParameters);
				PrimParams->NumUniquePrimitives = DeferredContext->UniquePrimitives.Num();
				return FComputeShaderUtils::GetGroupCountWrapped(DeferredContext->UniquePrimitives.Num(), FBuildBatchedPrimitiveDataCS::NumThreadsPerGroup);
			});
	}
	...
	if (bEnableInstanceCompaction)
	{
		...
		// Compaction phase two - write instances to compact final location
		{
			...
			PassParameters2->BatchedInstanceDataOut = BatchedInstanceDataBufferUAV;
...
void FInstanceCullingContext::SetDeferredContextCullingParams(const FInstanceCullingDeferredContext& DeferredContext, FInstanceCullingDrawParams& Parameters)
{
	Parameters.DrawIndirectArgsBuffer = DeferredContext.DrawIndirectArgsBuffer;
	Parameters.InstanceIdOffsetBuffer = DeferredContext.InstanceDataBuffer;
	Parameters.InstanceCulling = DeferredContext.UniformBuffer;
}
...
static float UnpackScreenSizeHelper(uint32 PackedScreenSize, EMeshDrawCommandCullingPayloadFlags CurrentFlags, EMeshDrawCommandCullingPayloadFlags TestFlag, float DefaultValue)
{
	if (EnumHasAnyFlags(CurrentFlags, TestFlag))
	{
		return FMeshDrawCommandCullingPayload::UnpackScreenSize(PackedScreenSize);
	}
	...
	int32& NewPassVisibleMeshDrawCommandsNum)
{
	...
	FMeshDrawCommandCullingPayload CurrentCullingPayload;
	MaxInstances = 1;
	...
	const bool bOrderPreservationEnabled = IsInstanceOrderPreservationEnabled();
	const uint32 MaxInstanceBatchSize = bUsesUniformBufferView ? (PLATFORM_MAX_UNIFORM_BUFFER_RANGE / BATCHED_INSTANCE_DATA_STRIDE) : MAX_uint32;
	
	// Used to detect when we change between direct and indirect draws and break auto instancing there. This is a workaround for the fact that we need an indirect draw to support GPU culling and when MDCs are identical they could batch together otherwise.
	// In the future we should rework the below to instead decide the draw command at the end, but this is complicated by the mobile GPU scene batching.
	// Note: initial value is arbitrary because we will never use the result during the first iteration as CurrentStateBucketId is initialized to -1
	bool bPrevUseIndirectDraw = false;

	// Mobile UBO path: relative primitive slot within the current draw. Resets to 0 each time we start a new draw
	int32 CurrentDrawRelativeSlot = 0;
	...
	for (int32 DrawCommandIndex = 0; DrawCommandIndex < NumDrawCommandsIn; ++DrawCommandIndex)
	{
		...
		const bool bCompactIdenticalCommands = bInCompactIdenticalCommands && !(bUseIndirectDraw && bPreserveInstanceOrder) 
			&& (bUsesUniformBufferView ? (CurrentAutoInstanceCount < MaxInstanceBatchSize && !bUseIndirectDraw) : true);

		const bool bChangedDrawMode = bPrevUseIndirectDraw != bUseIndirectDraw;
		bPrevUseIndirectDraw = bUseIndirectDraw;

		bool bMeshDrawCommandMatch = VisibleMeshDrawCommand.StateBucketId == CurrentStateBucketId
									&& VisibleMeshDrawCommand.CullingPayloadFlags == CurrentCullingPayloadFlags
									&& VisibleMeshDrawCommand.CullingPayload.PackedData == CurrentCullingPayload.PackedData;

		if (bCompactIdenticalCommands && CurrentStateBucketId != -1 && bMeshDrawCommandMatch && !bChangedDrawMode)
		{
		...
		else
		{
			...
			// New draw begins, relative primitive slot in this draw restarts at 0.
			CurrentDrawRelativeSlot = 0;
			...
			//if (bSupportsGPUSceneInstancing)
			{
				...
				DrawCmd.PrimitiveDataByteOffset = TotalUniquePrimitives * BATCHED_PRIMITIVE_DATA_STRIDE;
			...
			CurrentCullingPayload = VisibleMeshDrawCommand.CullingPayload;
		...
		if (bSupportsGPUSceneInstancing)
		{
			...
			const int32 PrimitiveSlotInDraw = CurrentDrawRelativeSlot;
			if (bUsesUniformBufferView)
			{
				FUniquePrimitiveInfo& UniqueInfo = UniquePrimitives.AddDefaulted_GetRef();
				UniqueInfo.InstanceDataOffset = VisibleMeshDrawCommand.PrimitiveIdInfo.InstanceSceneDataOffset;
				UniqueInfo.MeshLODIndex_Flags = (VisibleMeshDrawCommand.CullingPayload.LodIndex & 0xF) | (VisibleMeshDrawCommand.PrimitiveIdInfo.bIsDynamicPrimitive ? FUniquePrimitiveInfo::UniquePrimDynamicBit : 0u);
				++TotalUniquePrimitives;
				++CurrentDrawRelativeSlot;
			}
			...
			{
				AddInstanceRunsToDrawCommand(CurrentIndirectArgsOffset, VisibleMeshDrawCommand.PrimitiveIdInfo.InstanceSceneDataOffset, VisibleMeshDrawCommand.RunArray, VisibleMeshDrawCommand.NumRuns, InstanceFlags, MaxInstanceBatchSize, PrimitiveSlotInDraw);
			}
			else if (bFetchInstanceCountFromScene)
			{
				...
				const FPrimitiveSceneInfo* RESTRICT PrimitiveSceneInfo = Scene->Primitives[VisibleMeshDrawCommand.PrimitiveIdInfo.ScenePrimitiveId];
				uint32 NumInstances = uint32(PrimitiveSceneInfo->GetNumInstanceSceneDataEntries());
				if (NumInstances > 0u)
				{
					if (const FSceneCulling::FPrimitiveCullingInfo* PrimitiveCullingData = CullingQuery ? CullingQuery->GetCullingInfo(PrimitiveSceneInfo->GetPersistentIndex()) : nullptr)
					{
						FVector2f MinMaxLODScreenSize(
							UnpackScreenSizeHelper(VisibleMeshDrawCommand.CullingPayload.MinScreenSize, CurrentCullingPayloadFlags, EMeshDrawCommandCullingPayloadFlags::MinScreenSizeCull, std::numeric_limits<float>::lowest()),
							UnpackScreenSizeHelper(VisibleMeshDrawCommand.CullingPayload.MaxScreenSize, CurrentCullingPayloadFlags, EMeshDrawCommandCullingPayloadFlags::MaxScreenSizeCull, std::numeric_limits<float>::max())
						);
						...
						for (int32 ItemIndex = 0; ItemIndex < PrimitiveCullingData->Items.Num(); ++ItemIndex)
						{
							...
							{
								AddInstancesToDrawCommand(CurrentIndirectArgsOffset, VisibleMeshDrawCommand.PrimitiveIdInfo.InstanceSceneDataOffset + FirstInstance, NumInstancesDrawn, Item.NumInstances, InstanceFlags, MaxInstanceBatchSize, PrimitiveSlotInDraw);
					...
					{
						AddInstancesToDrawCommand(CurrentIndirectArgsOffset, VisibleMeshDrawCommand.PrimitiveIdInfo.InstanceSceneDataOffset, 0, NumInstances, InstanceFlags, MaxInstanceBatchSize, PrimitiveSlotInDraw);
					}
				}
			}
			else
			{
				...
				AddInstancesToDrawCommand(CurrentIndirectArgsOffset, VisibleMeshDrawCommand.PrimitiveIdInfo.InstanceSceneDataOffset, 0, VisibleMeshDrawCommand.MeshDrawCommand->NumInstances, InstanceFlags, MaxInstanceBatchSize, PrimitiveSlotInDraw);
			}
	...
	FRHICommandList& RHICmdList) const
{
	...
	if (bUsesUniformBufferView)
	{
		SceneArgs.BatchedPrimitiveDataSRV = OverrideArgs.BatchedPrimitiveDataSRV;
		SceneArgs.BatchedInstanceDataSRV = OverrideArgs.BatchedInstanceDataSRV;
	}

	if (OverrideArgs.InstanceCullingStaticUB 
		&& IsUniformBufferStaticSlotValid(InstanceCullingStaticSlot))
	{
		// Mobile manually merges several mesh passes into one RDG pass (e.g. RenderForwardSinglePass), RDG only auto-binds one InstanceCulling UB at the outer pass. 
		// Explicitly re-bind InstanceCulling UB to a static slot for each mesh pass
	...
	for (int32 DrawCommandIndex = StartIndex; DrawCommandIndex < StartIndex + NumMeshDrawCommands; DrawCommandIndex++)
	{
		...
		SceneArgs.BatchedPrimitiveDataOffset = OverrideArgs.PrimitiveDataByteOffset + DrawCommandInfo.PrimitiveDataByteOffset;
		SceneArgs.BatchedInstanceDataOffset = OverrideArgs.InstanceDataByteOffset + DrawCommandInfo.InstanceDataByteOffset;

#if WANTS_DRAW_MESH_EVENTS
		RHI_BREADCRUMB_EVENT_CONDITIONAL_F(RHICmdList, GShowMaterialDrawEvents != 0, "MaterialDrawEvent", "%s %s (%u instances)"
			, VisibleMeshDrawCommand.MeshDrawCommand->GetDebugData().MaterialRenderProxyName
		...
		if (FMeshDrawCommand::SubmitDrawBegin(*VisibleMeshDrawCommand.MeshDrawCommand, GraphicsMinimalPipelineStateSet, SceneArgs, InstanceFactor, RHICmdList, StateCache, bAllowSkipDrawCommand))
		{
			...
			for (uint32 BatchIdx = 1; BatchIdx < DrawCommandInfo.NumBatches; ++BatchIdx)
			{
				...
				SceneArgs.BatchedInstanceDataOffset += DrawCommandInfo.BatchDataStride;
