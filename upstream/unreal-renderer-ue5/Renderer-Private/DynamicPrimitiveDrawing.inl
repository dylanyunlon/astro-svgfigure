	...
	)
{
	...
	bool bIsPerspective = (ViewInfo->ViewMatrices.GetViewToClip().M[3][3] < 1.0f) ? true : false;
	if( !bIsPerspective )
	{
...
inline int32 FViewElementPDI::DrawMesh(const FMeshBatch& Mesh)
{
	...
	if (ensure(MeshBatchHasPrimitives(Mesh)))
	{
...

		{
			...
			EShaderPlatform ShaderPlatform = ViewInfo->GetShaderPlatform();
			ENQUEUE_RENDER_COMMAND(FCopyDynamicPrimitiveShaderData)(
				[NewMesh, DynamicPrimitiveCollectorForRT = DynamicPrimitiveCollector, FeatureLevel, ShaderPlatform](FRHICommandListImmediate& RHICmdList)
				{
					const bool bPrimitiveShaderDataComesFromSceneBuffer = NewMesh->VertexFactory->SupportsGPUScene(FeatureLevel);
