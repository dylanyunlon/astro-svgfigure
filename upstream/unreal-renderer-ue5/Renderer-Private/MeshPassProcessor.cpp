	...
	FShaderBindingState& RESTRICT ShaderBindingState)
{
	...
	for (int32 UniformBufferIndex = 0; UniformBufferIndex < NumUniformBuffers; UniformBufferIndex++)
	{
		...
		check(Parameter.BaseIndex < FShaderBindingState::MAX_UNIFORM_BUFFERS_PER_STAGE);
	...
	ERayTracingLocalShaderBindingType BindingType) const
{
	...
	if (LooseParameterBuffers.Num())
	{
		...
		MaxUniformBufferIndex = FMath::Max((int32)LooseParameterBuffer.BaseIndex, MaxUniformBufferIndex);
...
void FMeshDrawShaderBindings::Initialize(const FMeshProcessorShaders& Shaders)
{
	const int32 NumShaderFrequencies = 
		...
		(Shaders.MeshShader.IsValid() ? 1 : 0) +
	...
	{
		ShaderLayouts.Add(FMeshDrawShaderBindingsLayout(Shaders.MeshShader));
		ShaderBindingDataSize += ShaderLayouts.Last().GetDataSizeBytes();
		check(ShaderFrequencyBits < (1 << SF_Mesh));
		ShaderFrequencyBits |= (1 << SF_Mesh);
	}
#endif
...
void FGraphicsMinimalPipelineStateInitializer::SetupBoundShaderState(FRHIVertexDeclaration* VertexDeclaration, const FMeshProcessorShaders& Shaders)
{
	...
	new (&BoundShaderState) FMinimalBoundShaderStateInput();
	BoundShaderState.VertexDeclarationRHI = VertexDeclaration;

#if PLATFORM_SUPPORTS_MESH_SHADERS
	checkf(Shaders.VertexShader.IsValid() || Shaders.MeshShader.IsValid(), TEXT("Can't render without a vertex or mesh shader"));
#else
	...
	if (Shaders.MeshShader.IsValid())
	{
		checkSlow(Shaders.MeshShader->GetFrequency() == SF_Mesh);
		BoundShaderState.MeshShaderResource = Shaders.MeshShader.GetResource();
		BoundShaderState.MeshShaderIndex = Shaders.MeshShader->GetResourceIndex();
		check(BoundShaderState.MeshShaderResource->IsValidShaderIndex(BoundShaderState.MeshShaderIndex));
	}
#endif
	if (Shaders.PixelShader.IsValid())
	{
	...
	const FMeshProcessorShaders* ShadersForDebugging)
{
	...
	// Note: we override the instance count to ensure we hash to the same state bucket for MDCs where we know it will be changed at draw time anyway.
	NumInstances = BatchElement.bFetchInstanceCountFromScene ? 1 : BatchElement.NumInstances;
	...
	// Store the num vertices for the MDC stats - with indirect args they are otherwise lost because not really needed for draw but handy for stat tracking
	// (can't be retrieved from draw data and indirect args)
	StatsData.NumVertices = BatchElement.MaxVertexIndex - BatchElement.MinVertexIndex + 1;
#endif
...
void FMeshDrawShaderBindings::SetOnCommandList(FRHICommandList& RHICmdList, const FBoundShaderStateInput& Shaders, FShaderBindingState* StateCacheShaderBindings) const
{
	...
	for (int32 ShaderBindingsIndex = 0; ShaderBindingsIndex < ShaderLayouts.Num(); ShaderBindingsIndex++)
	{
		...
		else if (Frequency == SF_Mesh)
		{
			FRHIBatchedShaderParameters& BatchedParameters = RHICmdList.GetScratchShaderParameters();
			FReadOnlyMeshDrawSingleShaderBindings::SetShaderBindings(BatchedParameters, SingleShaderBindings, ShaderBindingState);
			RHICmdList.SetBatchedShaderParameters(Shaders.GetMeshShader(), BatchedParameters);
		}
	#endif
	...
	bool bAllowSkipDrawCommand)
{
	...
	if (MeshDrawCommand.CachedPipelineId.GetId() != StateCache.PipelineId)
	{
		...
		{
			UE_LOGF(LogRenderer, Log, "Missing Preload shaders. Material Friendly: %ls Material Proxy: %ls Static: %ls", *MeshDrawCommand.DebugData.Material->GetFriendlyName(), *MeshDrawCommand.DebugData.MaterialRenderProxyName, *MeshDrawCommand.StatsData.CategoryName.ToString());
...
#if PSO_PRECACHING_VALIDATE
#if MESH_DRAW_COMMAND_DEBUG_DATA
		PSOCollectorStats::CheckFullPipelineStateInCache(GraphicsPSOInit, PSOPrecacheResult, MeshDrawCommand.DebugData.Material,
			MeshDrawCommand.DebugData.VertexFactoryType, nullptr /* PrimitiveSceneProxy */, MeshDrawCommand.DebugData.PSOCollectorIndex);
#else
		PSOCollectorStats::CheckFullPipelineStateInCache(GraphicsPSOInit, PSOPrecacheResult, (const FMaterial*)nullptr, nullptr, nullptr, INDEX_NONE);
#endif // MESH_DRAW_COMMAND_DEBUG_DATA
	...
	const int8 PrimitiveIdStreamIndex = (SceneArgs.BatchedPrimitiveDataSRV != nullptr ? -1 : MeshDrawCommand.PrimitiveIdStreamIndex);
...
void FMeshDrawCommand::SubmitDrawEnd(const FMeshDrawCommand& MeshDrawCommand, const FMeshDrawCommandSceneArgs& SceneArgs, uint32 InstanceFactor, FRHICommandList& RHICmdList)
{
	...
	if (SceneArgs.BatchedInstanceDataSRV)
	{
		RHICmdList.SetUniformBlockOffset(SceneArgs.BatchedPrimitiveDataSRV, SceneArgs.BatchedPrimitiveDataOffset);
		RHICmdList.SetUniformBlockOffset(SceneArgs.BatchedInstanceDataSRV, SceneArgs.BatchedInstanceDataOffset);
	}
	...
	FMeshDrawCommandStateCache& RESTRICT StateCache)
{
	...
	RHI_BREADCRUMB_EVENT_CONDITIONAL_F(RHICmdList, GShowMaterialDrawEvents != 0, "MaterialDrawEvent", "%s %s (%u instances)"
		, MeshDrawCommand.DebugData.MaterialRenderProxyName
...
void FMeshDrawCommand::SetDebugData(const FPrimitiveSceneProxy* PrimitiveSceneProxy, const FMaterial* Material, const FMaterialRenderProxy* MaterialRenderProxy, const FMeshProcessorShaders& UntypedShaders, const FVertexFactory* VertexFactory, const FMeshBatch& MeshBatch, int32 PSOCollectorIndex)
{
	...
	DebugData.MaterialRenderProxyName = MaterialRenderProxy ? MaterialRenderProxy->GetMaterialName() : TEXT(""); 
...
void FMeshDrawCommand::GetStatsData(FVisibleMeshDrawCommandStatsData& OutVisibleStatsData) const
{
	...
	OutVisibleStatsData.MaterialName = DebugData.MaterialRenderProxyName;
#endif
}
	...
	FRHICommandList& RHICmdList)
{
	...
	for (int32 DrawCommandIndex = StartIndex; DrawCommandIndex < StartIndex + NumMeshDrawCommands; DrawCommandIndex++)
	{
		...
		const uint32 PerDrawDelta = (bDynamicInstancing ? VisibleMeshDrawCommand.PrimitiveIdBufferOffset : DrawCommandIndex) * PrimitiveIdBufferStride;
		LocalSceneArgs.PrimitiveIdOffset = InSceneArgs.PrimitiveIdOffset + PerDrawDelta;
		LocalSceneArgs.BatchedPrimitiveDataOffset = InSceneArgs.BatchedPrimitiveDataOffset + PerDrawDelta;
		LocalSceneArgs.BatchedInstanceDataOffset = InSceneArgs.BatchedInstanceDataOffset + PerDrawDelta;
	...
	uint32 InstanceFactor)
{
	if (VisibleMeshDrawCommands.Num() > 0)
	{
		...
		FRHIBuffer* BatchedInstanceDataBufferForOverride = nullptr;
		SortAndMergeDynamicPassMeshDrawCommands(View, RHICmdList, VisibleMeshDrawCommands, DynamicMeshDrawCommandStorage, SceneArgs.PrimitiveIdsBuffer, BatchedInstanceDataBufferForOverride, InstanceFactor, &ViewInfo->DynamicPrimitiveCollector);

		if (PlatformGPUSceneUsesUniformBufferView(ShaderPlatform))
		{
			FShaderResourceViewRHIRef BatchedPrimitiveSRV = RHICmdList.CreateShaderResourceView(
				SceneArgs.PrimitiveIdsBuffer,
				FRHIViewDesc::CreateBufferSRV()
					.SetType(FRHIViewDesc::EBufferType::Typed)
					.SetFormat(PF_A32B32G32R32F));
			FShaderResourceViewRHIRef BatchedInstanceSRV = RHICmdList.CreateShaderResourceView(
				BatchedInstanceDataBufferForOverride,
				FRHIViewDesc::CreateBufferSRV()
					.SetType(FRHIViewDesc::EBufferType::Typed)
					.SetFormat(PF_A32B32G32R32F));

			// Build a per-pass instance-culling UB pointing BatchedPrimitiveData and BatchedInstanceData at the real SRVs.
			// RDG bound the dummy UB at the static slot during pass-execute setup, we override it here.
			FInstanceCullingGlobalUniformsContents UBContents{};
			UBContents.BatchedPrimitiveData = BatchedPrimitiveSRV.GetReference();
			UBContents.BatchedInstanceData = BatchedInstanceSRV.GetReference();

			FUniformBufferRHIRef PerPassUB = RHICreateUniformBuffer(&UBContents, GetDynamicInstanceCullingLayout(),	UniformBuffer_SingleFrame, EUniformBufferValidation::None);
			RHICmdList.SetStaticUniformBuffer(FInstanceCullingContext::GetStaticUniformBufferSlot(ShaderPlatform), PerPassUB);

			// Suppress vertex-stream binding
			SceneArgs.PrimitiveIdsBuffer = nullptr;
			SceneArgs.BatchedPrimitiveDataSRV = BatchedPrimitiveSRV.GetReference();
			SceneArgs.BatchedInstanceDataSRV = BatchedInstanceSRV.GetReference();
		}
...
{
	// No sorting for deprecated path. Slower performance but safe.
	FMeshDrawCommandSortKey SortKey;
	SortKey.Generic.VertexShaderHash = 0;
	SortKey.Generic.PixelShaderHash = 0;
...
{
	FMeshDrawCommandSortKey SortKey;
	SortKey.Generic.VertexShaderHash = VertexShader.GetSortKey();
	SortKey.Generic.PixelShaderHash = PixelShader.GetSortKey();

	return SortKey;
}

static const FScene* ResolveSceneFromViewIfNull(const FScene* InScene, const FSceneView* InViewIfDynamicMeshCommand)
{
	if (InScene == nullptr && InViewIfDynamicMeshCommand != nullptr && InViewIfDynamicMeshCommand->Family != nullptr)
	{
		if (const FSceneInterface* SceneInterface = InViewIfDynamicMeshCommand->Family->Scene)
		{
			return SceneInterface->GetRenderScene();
		}
	}
...
FMeshPassProcessor::FMeshPassProcessor(EMeshPass::Type InMeshPassType, const FScene* InScene, ERHIFeatureLevel::Type InFeatureLevel, const FSceneView* InViewIfDynamicMeshCommand, FMeshPassDrawListContext* InDrawListContext)
	...
	, Scene(ResolveSceneFromViewIfNull(InScene, InViewIfDynamicMeshCommand))
...
bool FMeshPassProcessor::PipelineVariableRateShadingEnabled()
{
...
FMeshDrawCommand& FCachedPassMeshDrawListContext::AddCommand(FMeshDrawCommand& Initializer, uint32 NumElements)
{
	...
	ensureMsgf(CommandInfo.StateBucketId == -1, TEXT("GetCommandInfoAndReset() wasn't called since the last command was added"));
