	...
	bMobileDynamicLocalLight(false)
{
	...
	bProxySupportsGPUScene = PrimitiveSceneInfo->Proxy->AllVertexFactoriesSupportGPUScene();
...
void FLightPrimitiveInteraction::FlushCachedShadowMapData()
{
	if (LightSceneInfo && PrimitiveSceneInfo && PrimitiveSceneInfo->Proxy && PrimitiveSceneInfo->Scene)
	{
		...
		{
			TArray<FCachedShadowMapData>* CachedShadowMapDatas = PrimitiveSceneInfo->Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapDatas(LightSceneInfo->Id);
	...
	DirectionalInscatteringColor(State.DirectionalInscatteringLuminance)
{
	...
	SkyLightCaptureAffectsHeightFogRoughness = State.SkyLightCaptureAffectsHeightFogRoughness;
	SkyLightCaptureAffectsHeightFogStrength = State.SkyLightCaptureAffectsHeightFogStrength;
	...
	bEnableFSSS = State.bEnableFSSS;
	FSSSSceneColorScatteringAmountScale = State.FSSSSceneColorScatteringAmountScale;
	FSSSSceneColorScatteringAmountPower = State.FSSSSceneColorScatteringAmountPower;
	FSSSSpreadScale = State.FSSSSpreadScale;
	FSSSBlurControl = State.FSSSBlurControl;
