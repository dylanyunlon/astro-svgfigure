...
EMobileLocalLightSetting GetMobileForwardLocalLightSetting(EShaderPlatform ShaderPlatform)
{
	...
	return MobileForwardLocalLightsIniValue > 0 ? EMobileLocalLightSetting::LOCAL_LIGHTS_ENABLED: EMobileLocalLightSetting::LOCAL_LIGHTS_DISABLED;
}
	...
	bool bKeepStencilDither)
{
	...
	else if (bKeepStencilDither && IsStencilForLODDitherEnabled(Material.GetShaderPlatform()))
	{
		// If the object is not transitioning and StencilLODDither is enabled, we need to preserve the dither pattern, 
		// as well as the other stencil bits (RECEIVE_DECAL, lighting channels, etc.).
		DrawRenderState.SetDepthStencilState(TStaticDepthStencilState<
			true, CF_DepthNearOrEqual,
			true, CF_Always, SO_Keep, SO_Keep, SO_Replace,
			false, CF_Always, SO_Keep, SO_Keep, SO_Keep,
			0x00, (0xff & ~STENCIL_SANDBOX_MASK) >::GetRHI());
	}
	else
	{
		DrawRenderState.SetDepthStencilState(TStaticDepthStencilState<
...
bool MobileBasePass::GetShaders(
	...
	const FVertexFactoryType* VertexFactoryType,
	TShaderRef<TMobileBasePassVSPolicyParamType<LightMapPolicyType>>& VertexShader,
	TShaderRef<TMobileBasePassPSPolicyParamType<LightMapPolicyType>>& PixelShader)
{
...
// Force instantination for FMobileSingleLayerWaterPolicy as it's used in a different translation unit
template bool MobileBasePass::GetShaders<FMobileSingleLayerWaterPolicy>(
	const FMobileSingleLayerWaterPolicy& RESTRICT LightMapPolicy,
	EMobileLocalLightSetting LocalLightSetting,
	const FMaterial& MaterialResource,
	const FVertexFactoryType* VertexFactoryType,
	TShaderRef<TMobileBasePassVSPolicyParamType<FMobileSingleLayerWaterPolicy>>& VertexShader,
	TShaderRef<TMobileBasePassPSPolicyParamType<FMobileSingleLayerWaterPolicy>>& PixelShader);

// Force instantination for FUniformLightMapPolicy as it's used in a different translation unit
template bool MobileBasePass::GetShaders<FUniformLightMapPolicy>(
	const FUniformLightMapPolicy& RESTRICT LightMapPolicy,
	EMobileLocalLightSetting LocalLightSetting,
	const FMaterial& MaterialResource,
	const FVertexFactoryType* VertexFactoryType,
...
void MobileBasePass::SetOpaqueRenderState(FMeshPassProcessorRenderState& DrawRenderState, const FPrimitiveSceneProxy* PrimitiveSceneProxy, const FMaterial& Material, FMaterialShadingModelField ShadingModels, bool bCanUseDepthStencil, bool bUsesDeferredShading)
{
	...
	{
		SetMobileBasePassDepthState(DrawRenderState, PrimitiveSceneProxy, Material, ShadingModels, bUsesDeferredShading, TOptional<uint32>(), false);
	}
	else
	{
...
static FMeshDrawCommandSortKey GetBasePassStaticSortKey(EDepthDrawingMode EarlyZPassMode, const bool bIsMasked, bool bBackground, const TShaderRef<FMeshMaterialShader>& VertexShader, const TShaderRef<FMeshMaterialShader>& PixelShader)
{
	FMeshDrawCommandSortKey SortKey;
	SortKey.BasePass.Background = (bBackground ? 1 : 0); // background flag in second bit
	SortKey.BasePass.VertexShaderHash = VertexShader.GetSortKey() & 0xFFFF;
	SortKey.BasePass.PixelShaderHash = PixelShader.GetSortKey();
...
template <typename LightMapPolicyType>
void TMobileBasePassPSPolicyParamType<LightMapPolicyType>::GetShaderBindings(
	const FScene* Scene,
...
// Force instantination for FMobileSingleLayerWaterPolicy as it's used in a different translation unit
template void TMobileBasePassPSPolicyParamType<FMobileSingleLayerWaterPolicy>::GetShaderBindings(
	const FScene* Scene,
	const FStaticFeatureLevel FeatureLevel,
	const FPrimitiveSceneProxy* PrimitiveSceneProxy,
	const FMaterialRenderProxy& MaterialRenderProxy,
	const FMaterial& Material,
	const TMobileBasePassShaderElementData<FMobileSingleLayerWaterPolicy>& ShaderElementData,
	FMeshDrawSingleShaderBindings& ShaderBindings) const;

// Force instantination for FUniformLightMapPolicy as it's used in a different translation unit
template void TMobileBasePassPSPolicyParamType<FUniformLightMapPolicy>::GetShaderBindings(
	const FScene* Scene,
	const FStaticFeatureLevel FeatureLevel,
	const FPrimitiveSceneProxy* PrimitiveSceneProxy,
	const FMaterialRenderProxy& MaterialRenderProxy,
...
bool FMobileBasePassMeshProcessor::ShouldDraw(const FMaterial& Material) const
{
	...
	const bool bIsTranslucentSLW = bIsSLW && IsUsingSingleLayerWaterFallback(GetFeatureLevelShaderPlatform(ERHIFeatureLevel::ES3_1));
...
static bool MaterialCanReceiveCSM(const FMaterial& Material)
{
	...
	return !bIsTranslucent || bIsSLW || IsTranslucentPerPixelLighting;
}
...
bool FMobileBasePassMeshProcessor::TryAddMeshBatch(const FMeshBatch& RESTRICT MeshBatch, uint64 BatchElementMask, const FPrimitiveSceneProxy* RESTRICT PrimitiveSceneProxy, int32 StaticMeshId, const FMaterialRenderProxy& MaterialRenderProxy, const FMaterial& Material)
{
	if (ShouldDraw(Material))
	{
		...
		if ((uint16(Material.MaterialGetSubstrateMaterialBsdfFeatures_RenderThread()) & uint16(ESubstrateBsdfFeature::Toon)) > 0)
		{
			ShadingModels.AddShadingModel(EMaterialShadingModel::MSM_Strata); // This model is not only a DefaultLit model and will be schedule in the "all shading model" lighting pass.
		}
...
#endif
		const bool bIsSLW = ShadingModels.HasShadingModel(MSM_SingleLayerWater);
		// Only translucent SLW fallback takes this path. Opaque SLW must be handled by FMobileSingleLayerWaterPassMeshProcessor 
		check(!bIsSLW || IsUsingSingleLayerWaterFallback(ShaderPlatform)); 

		const EBlendMode BlendMode = Material.GetBlendMode();
		const bool bIsLitMaterial = ShadingModels.IsLit();
		const bool bIsTranslucent = IsTranslucentBlendMode(BlendMode) || bIsSLW; // Water fallback goes into the translucent pass;
		const bool bIsMasked = IsMaskedBlendMode(Material);		
		const bool bCanReceiveCSM = ((Flags & EFlags::CanReceiveCSM) == EFlags::CanReceiveCSM) && MaterialCanReceiveCSM(Material);
		ELightMapPolicyType Policy = MobileBasePass::SelectMeshLightmapPolicy(Scene, MeshBatch, PrimitiveSceneProxy, bCanReceiveCSM, bPassUsesDeferredShading, bIsLitMaterial, bIsTranslucent);

		return Process<FUniformLightMapPolicy>(
			MeshBatch,
			BatchElementMask,
			StaticMeshId,
			PrimitiveSceneProxy,
		...
		const bool bCanReceiveCSM)
{
	TMeshProcessorShaders<
		...
		TMobileBasePassPSPolicyParamType<LightMapPolicyType>> BasePassShaders;
	...
	else
	{
		...
		SortKey = GetBasePassStaticSortKey(EarlyZPassMode, bIsMasked, bBackground, BasePassShaders.VertexShader, BasePassShaders.PixelShader);
	}
	...
	TMobileBasePassShaderElementData<LightMapPolicyType> ShaderElementData(LightMapElementData, bCanReceiveCSM);
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	FUniformLightMapPolicy UniformLightmapPolicy(LightMapPolicyType);

	if (!MobileBasePass::GetShaders(
		UniformLightmapPolicy,
		LocalLightSetting,
		MaterialResource,
...
void FMobileBasePassMeshProcessor::CollectPSOInitializers(const FSceneTexturesConfig& SceneTexturesConfig, const FMaterial& Material, const FPSOPrecacheVertexFactoryData& VertexFactoryData, const FPSOPrecacheParams& PreCacheParams, TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	if (bTranslucentBasePass)
	{
		...
		else if (MeshPassType == EMeshPass::TranslucencyAll && !UPSOPrecacheSettingsManager::GetSettings().bTranslucencyAll)
		{
	...
	const bool bUseNoLocalLightPermutation = !bUseLocalLightPermutation || !MobileLocalLightsUseSinglePermutation(ShaderPlatform);
	check(bUseLocalLightPermutation || bUseNoLocalLightPermutation);
	...
	{
		if (bUseNoLocalLightPermutation)
		{
...
FMeshPassProcessor* CreateMobileBasePassProcessor(ERHIFeatureLevel::Type FeatureLevel, const FScene* Scene, const FSceneView* InViewIfDynamicMeshCommand, FMeshPassDrawListContext* InDrawListContext)
{
	...
	const bool bDepthWrite = (DefaultBasePassDepthStencilAccess & FExclusiveDepthStencil::Type::DepthWrite) != 0;
	PassDrawRenderState.SetDepthStencilState(bDepthWrite ? TStaticDepthStencilState<true, CF_DepthNearOrEqual>::GetRHI() : TStaticDepthStencilState<false, CF_DepthNearOrEqual>::GetRHI());
