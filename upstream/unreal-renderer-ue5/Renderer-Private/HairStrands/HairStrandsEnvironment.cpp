...
static int32 GHairScatterSceneLighting = 0;
static FAutoConsoleVariableRef CVarHairScatterSceneLighting(TEXT("r.HairStrands.ScatterSceneLighting"), GHairScatterSceneLighting, TEXT("Enable scene color lighting scattering into hair (valid for short hair only), disabled by default."));
...
enum class EHairLightingSourceType : uint8
{
	SceneColor = 0,
	ReflectionProbe = 1,
	LumenScreenProbeGather = 2,
	LumenIrradianceFieldGather = 3,
	Count
};
...
class FHairEnvironmentLightingPS : public FGlobalShader
{
	...
	class FLumen			: SHADER_PERMUTATION_INT("PERMUTATION_LUMEN", 3); 
	class FStaticLighting	: SHADER_PERMUTATION_BOOL("PERMUTATION_STATIC_LIGHTING"); 
	using FPermutationDomain = TShaderPermutationDomain<FIntegrationType, FLumen, FStaticLighting>;
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		if (PermutationVector.Get<FStaticLighting>() && PermutationVector.Get<FLumen>()>0)
		{
			return false;
		}
	...
	const EHairLightingSourceType LightingType)
{
	...
	// Sanity check
	if (bHasStaticLighting) { check(LightingType != EHairLightingSourceType::LumenScreenProbeGather && LightingType != EHairLightingSourceType::LumenIrradianceFieldGather); }
	...
	if (bScreenTrace)
	{
		...
		ParametersPS->ScreenTraceLength = 128;
	...
	const TCHAR* LightingTypeName = TEXT("Unknown");
	uint32 LumenPermutation = 0;
	switch (LightingType)
	{
		case EHairLightingSourceType::SceneColor: 
		{
			LightingTypeName = TEXT("SceneScatter") ; 
		}
		break;
		case EHairLightingSourceType::LumenScreenProbeGather : 
		{
			LightingTypeName = TEXT("LumenScreenProbe"); 
			LumenPermutation = 1; 
		...
		break;
		case EHairLightingSourceType::LumenIrradianceFieldGather : 
		{
			LightingTypeName = TEXT("LumenIrradianceField"); 
			LumenPermutation = 2; 

			const FRadianceCacheState& RadianceCacheState = View.ViewState->Lumen.RadianceCacheState;
			LumenRadianceCache::FRadianceCacheInputs RadianceCacheInputs = LumenIrradianceFieldGather::SetupRadianceCacheInputs(View);
			RadianceCacheInputs.UseAdaptiveProbes = 0;
			LumenRadianceCache::GetInterpolationParameters(View, GraphBuilder, RadianceCacheState, RadianceCacheInputs, ParametersPS->RadianceCache);
			ParametersPS->ProbeValid = LumenRadianceCache::GetProbeValidSRV(GraphBuilder, GraphBuilder.RegisterExternalBuffer(RadianceCacheState.ProbeValid));
			ParametersPS->ProbeOcclusionBias = LumenIrradianceFieldGather::GetProbeOcclusionBias();
			ParametersPS->MaxRoughnessToEvaluateRoughSpecular = LumenIrradianceFieldGather::GetMaxRoughnessToEvaluateRoughSpecular();
			ParametersPS->SkylightPrefilterMipBias = FMath::Clamp<float>(CVarLumenSkylightPrefilterMipBias.GetValueOnRenderThread(), 0.0f, 16.0f);
		}
		break;
		case EHairLightingSourceType::ReflectionProbe : 
		{
			LightingTypeName = TEXT("ReflectionProbe"); 
		}
		break;
	};
	...
	PermutationVector.Set<FHairEnvironmentLightingPS::FLumen>(LumenPermutation);
	...
	GraphBuilder.AddPass(
		RDG_EVENT_NAME("HairStrands::EnvLightingPS(%s)", LightingTypeName),
		ParametersPS,
		ERDGPassFlags::Raster,
	...
	TArrayView<const FViewInfo> Views)
{
	...
	for (const FViewInfo& View : Views)
	{
		...
		{
			AddHairStrandsEnvironmentLightingPassPS(GraphBuilder, Scene, View, VisibilityData, VoxelResources, SceneColorTexture, EHairLightingSourceType::SceneColor);
		}
	}
}
	...
	EHairLightingSourceType LightingType)
{
	...
	AddHairStrandsEnvironmentLightingPassPS(GraphBuilder, Scene, View, VisibilityData, VoxelResources, nullptr, LightingType);
}
...
void RenderHairStrandsLumenLighting(
	...
	const FViewInfo& View)
{
	const ELumenFinalGatherMethod FinalGatherMethod = Lumen::GetFinalGatherMethod(*View.Family, View.GetShaderPlatform());
	const EHairLightingSourceType SourceType = FinalGatherMethod == ELumenFinalGatherMethod::IrradianceFieldGather ? EHairLightingSourceType::LumenIrradianceFieldGather : EHairLightingSourceType::LumenScreenProbeGather;
	InternalRenderHairStrandsEnvironmentLighting(GraphBuilder, Scene, View, SourceType);
}
