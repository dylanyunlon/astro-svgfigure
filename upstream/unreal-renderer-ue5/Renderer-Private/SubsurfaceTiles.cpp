	...
	FRDGTextureRef Texture, const FScreenPassTextureViewport& ScreenPassViewport, FRDGBufferRef ConditionBuffer, uint32 Offset)
{
...

	{
		...
		TShaderMapRef<FClearUAVBuildIndirectDispatchBufferCS> ComputeShader(GetGlobalShaderMap(GMaxRHIShaderPlatform));
	...
	TShaderMapRef<FClearUAVCS> ComputeShader(GetGlobalShaderMap(GMaxRHIShaderPlatform));
...
{
	const float DistanceToProjectionWindow = View.ViewMatrices.GetViewToClip().M[0][0];
