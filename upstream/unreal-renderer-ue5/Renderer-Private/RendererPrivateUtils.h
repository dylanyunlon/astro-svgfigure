...
public:
	...
	RENDERER_API void UploadTo(FRDGBuilder& GraphBuilder, FRDGBuffer *DestBuffer, FRDGBuffer *ScatterOffsets, FRDGBuffer *Values, uint32 NumScatters, uint32 NumBytesPerElement, int32 NumValuesPerScatter, EPixelFormat ValuePixelFormat);
...
struct FFloat4BufferTraits
{
	template <typename ElementType, typename AllocatorType>
	static FRDGBuffer *CreateValueUploadBuffer(FRDGBuilder& GraphBuilder, const TCHAR* Name, const TArray<ElementType, AllocatorType> &InitialData)
	{
		static_assert(sizeof(ElementType) % OverrideBytesPerElement == 0);
		const int32 NumElements = (InitialData.Num() * sizeof(ElementType)) / OverrideBytesPerElement;
		FRDGBufferRef Buffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredUploadDesc(OverrideBytesPerElement, NumElements), Name);
		GraphBuilder.QueueBufferUpload(Buffer, TConstArrayView<ElementType>(InitialData), ERDGInitialDataFlags::NoCopy);
		return Buffer;
	}
	
	template <typename AllocatorType>
	static FRDGBuffer *CreateOffsetUploadBuffer(FRDGBuilder& GraphBuilder, const TCHAR* Name, const TArray<uint32, AllocatorType> &InitialData)
	{
	...
	{
		return FRDGBufferDesc::CreateBufferDesc(BytesPerElement, NumElements);
	}

	static constexpr bool bAutoValuesPerScatter = false;
	static constexpr uint32 OverrideBytesPerElement = 16;
...
struct FStructuredBufferTraits
{
	...
	static constexpr bool bAutoValuesPerScatter = false;
	static constexpr uint32 OverrideBytesPerElement = 0;
	static constexpr EPixelFormat PixelFormat = PF_Unknown;
};
...
public:
	...
	static constexpr int32 BytesPerElement = BufferTraits::OverrideBytesPerElement != 0 ? BufferTraits::OverrideBytesPerElement : sizeof(ValueType);
	...
	{
		int32 NewMinNumElements = GetNumElements(FMath::Max(MinimumNumElementsReserved, bRoundUpToPOT ? int32(FMath::RoundUpToPowerOfTwo(InNewMinNumElements)) : InNewMinNumElements));
	...
	{
#if DO_CHECK
		const int32 NumValues = (PooledBuffer->Desc.NumElements * BytesPerElement) / sizeof(ValueType);

		check(HostValues.Num() == 0 || PooledBuffer.IsValid() && HostValues.Num() <= NumValues);
...
private:
	int32 GetNumElements(int32 InNumValues) const
	{
		return (InNumValues * sizeof(ValueType)) / BytesPerElement;
	}
};
...
public:
	using BufferTraits = InBufferTraits;
	using ValueType = InValueType;
	static constexpr int32 NumBytesPerElement = BufferTraits::OverrideBytesPerElement != 0 ? BufferTraits::OverrideBytesPerElement : sizeof(ValueType);
	static constexpr int32 NumValuesPerScatter = InNumValuesPerScatter;

	static_assert((NumBytesPerElement % 4) == 0, "The struct used must be 4-byte aligned");
	...
	FRDGBuffer *ResizeAndUploadTo(FRDGBuilder& GraphBuilder, TPersistentBuffer<ValueType, BufferTraits> &DestDataBuffer, int32 DestDataMinimumSize, FScatterInfo &OutScatterInfo)
	{
		...
		if (OutScatterInfo.NumScatters != 0u)
		{
			...
			OutScatterInfo.ScatterOffsetsRDG = BufferTraits::CreateOffsetUploadBuffer(GraphBuilder, TEXT("ScatterUploader.Offsets"), UploadDataProxy->ScatterOffsets);
			FRDGBuffer *ValuesRDG = BufferTraits::CreateValueUploadBuffer(GraphBuilder, TEXT("ScatterUploader.Values"), UploadDataProxy->Values);

			uint32 ElementSize = NumBytesPerElement;
			...
			FBufferScatterUploader::UploadTo(GraphBuilder, DestBufferRDG, OutScatterInfo.ScatterOffsetsRDG, ValuesRDG, OutScatterInfo.NumScatters, ElementSize, ElementsPerScatter, BufferTraits::PixelFormat);
		}
...
using TPersistentFloat4Buffer = UE::RendererPrivateUtils::Implementation::TPersistentBuffer<InValueType, UE::RendererPrivateUtils::Implementation::FFloat4BufferTraits>;

template <typename InValueType, int32 InNumValuesPerScatter = 1>
using TFloat4BufferScatterUploader = UE::RendererPrivateUtils::Implementation::TBufferScatterUploader<InValueType, UE::RendererPrivateUtils::Implementation::FFloat4BufferTraits, InNumValuesPerScatter>;
 ...
 */
RENDERER_API FRDGBufferRef CreateAndClearIndirectDispatchArgs(FRDGBuilder& GraphBuilder, ERHIFeatureLevel::Type FeatureLevel, const TCHAR* Name, const FIntVector3 &DimClearValue, uint32 NumIndirectArgs, uint32 IndirectArgStride);
