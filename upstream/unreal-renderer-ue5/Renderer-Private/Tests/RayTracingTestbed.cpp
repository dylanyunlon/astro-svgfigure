...
#include "RayTracingGeometryInstance.h"
...
bool RunRayTracingTestbed_RenderThread(const FString& Parameters)
{
	...
	GeometryInitializer.IndexBufferStride = sizeof(uint16);
	...
	FRayTracingSceneRHIRef RayTracingSceneRHI;
	{
		...
		Initializer.DebugName = FName("FRayTracingScene");
...
public:
	...
	{
		return ShouldCompileRayTracingShadersForProject(Parameters.Platform) && !IsMetalPlatform(Parameters.Platform);
	}
