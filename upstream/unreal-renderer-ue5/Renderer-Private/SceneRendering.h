...
public:
	...
	FRDGPooledBuffer* PageFreeListAllocatorBuffer = nullptr;
	FRDGPooledBuffer* PageFreeListBuffer = nullptr;
	IPooledRenderTarget* PageAtlasTexture = nullptr;
	IPooledRenderTarget* CoverageAtlasTexture = nullptr;
	FRDGPooledBuffer* PageObjectGridBuffer = nullptr;
	IPooledRenderTarget* PageTableCombinedTexture = nullptr;
	IPooledRenderTarget* PageTableLayerTextures[GDF_Num] = {};
	IPooledRenderTarget* MipTexture = nullptr;
...
const int32 GMaxForwardFarShadowCascades = 4; 
...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT_WITH_CONSTRUCTOR(FForwardLightUniformParameters, )
	...
	SHADER_PARAMETER(uint32, LightNameDwords)
	...
	SHADER_PARAMETER(uint32, DirectionalLightLightFunctionAtlasLightIndex)
	SHADER_PARAMETER(uint32, DirectionalLightAffectsTranslucentLighting)
	...
	SHADER_PARAMETER_RDG_BUFFER_SRV(ByteAddressBuffer, LightNamesBuffer)
END_GLOBAL_SHADER_PARAMETER_STRUCT()

BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT_WITH_CONSTRUCTOR(FForwardDirectionalLightShadowMapParameters, )
	SHADER_PARAMETER(uint32, NumDirectionalLightCascades)
	SHADER_PARAMETER(FVector4f, CascadeEndDepths)
	SHADER_PARAMETER(uint32, NumFarShadowCascades)
	SHADER_PARAMETER(FVector4f, FarShadowEndDepths)
...
extern TRDGUniformBufferRef<FForwardDirectionalLightShadowMapParameters> CreateDummyForwardDirLightShadowUniformBuffer(FRDGBuilder& GraphBuilder);
...
public:
	...
	void SetShadowUniformBuffer(TRDGUniformBufferRef<FForwardDirectionalLightShadowMapParameters> UniformBuffer)
	{
		check(UniformBuffer);
		ForwardDirLightShadowUniformBuffer = UniformBuffer;
		ForwardDirLightShadowParameters = UniformBuffer->GetContents();
	}
	...
	const FForwardDirectionalLightShadowMapParameters* ForwardDirLightShadowParameters = nullptr;
	TRDGUniformBufferRef<FForwardDirectionalLightShadowMapParameters> ForwardDirLightShadowUniformBuffer = nullptr;

	const FLightSceneProxy* SelectedForwardDirectionalLightProxy = nullptr;
	int32 SelectedForwardDirectionalLightIndex = INDEX_NONE;
	...
	FRDGBufferRef LightPowerHistoryRatioBuffer = nullptr;

	TConstArrayView<VisualizeLightShape::FLightInfo> VisualizeLightShapeLightInfos;
	FRDGBufferRef VisualizeLightShapeLightInfoBuffer = nullptr;
};
...
// Maximum number of concurrent runtime-reflection-capture smooth blends (cubemap-array slots reserved as blend
// sources, plus the corresponding shader-side blend table size).  Actual blend slot count is defined in
// r.ReflectionCapture.Runtime.SmoothBlendSlots, and clamped to this value.
static constexpr int32 GMaxRuntimeReflectionCaptureSmoothBlendSlots = 4;
...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT(FReflectionCaptureShaderData,)
	...
	// W is smooth blend info for runtime captures -- integer portion is source cube map index, fractional is blend, with 0 representing no active blend
	SHADER_PARAMETER_ARRAY(FVector4f,PositionLow,[GMaxNumReflectionCaptures])
	// R is brightness, G is array index, B is shape, A is the fade-in alpha (0..1) for runtime captures
...
struct FPreviousViewInfo
{
	...
	// History of all mega filters.
	TRefCountPtr<UE::Renderer::Private::IMegaFilters::IHistory> MegaFiltersHistory;
	...
	// Temporal AA history for Fog Screen Space Scattering
	FTemporalAAHistory FSSSHistory;
	...
	TMap<FLightComponentId, TSharedPtr<FScreenSpaceDenoiserHistory>> ShadowHistories;
	...
	// View size of the EyeAdaptation texture
	FIntRect MobileBloomSetup_EyeAdaptationViewSize;
...
{
	// The copy contains all members for rendering. Must be done on the rendering thread.
	Main,

	// The copy contains only members relevant for shadow views.
	Shadow
};
...
public:
	...
	/* Final position of the view in the final render target (in pixels), potentially scaled by the Upper Bound of ScreenPercentage */
	FIntRect UpperBoundViewRect;
	...
	TSceneRendererLightArray<FVisibleLightViewInfo> VisibleLightInfos;
	...
	/** Whether the view has any Single Layer Water materials that use motion vector world offset. */
	uint32 bSLWUsesMotionVectorWorldOffset : 1;
	...
	bool bPrevTransformsReset = false;
...
public:
	FFrontLayerTranslucency FrontLayerTranslucency;
...
public:
	...
	RENDERER_API FRHIRayTracingScene* GetRayTracingSceneChecked(ERayTracingSceneLayer Layer) const;
	RENDERER_API FRDGBufferSRVRef GetRayTracingSceneLayerViewChecked(ERayTracingSceneLayer Layer) const;

	FRDGBufferUAVRef GetRayTracingInstanceHitCountUAV(FRDGBuilder& GraphBuilder) const;

	RENDERER_API const FRayTracingScene::FViewHandle& GetRayTracingSceneViewHandle() const;
	...
	virtual FRDGBufferRef GetInlineRayTracingBindingDataBuffer() const override
	{
		return InlineRayTracingBindingDataBuffer;
	}
	...
	* for stereo rendering, this will force the post-processing to use the same render target for both eyes*/
	IPooledRenderTarget* GetTonemappingLUT(FRHICommandList& RHICmdList, const int32 LUTSize, const bool bNeedFloatOutput) const;

	/** Gets the other rendertarget that will be populated by CombineLUTS post process */
	IPooledRenderTarget* GetInnerTonemappingLUT(FRHICommandList& RHICmdList, const int32 LUTSize, const bool bNeedFloatOutput) const;

	void ClearInnerTonemappingLUT() const;
	...
	FViewInfo* CreateSnapshot(EViewSnapshotType Type = EViewSnapshotType::Main) const;
...
public:
	...
	inline bool IsSceneTexturesOwner() const
	{
		return bIsViewFamilyInfo && SceneTextures->Owner == this;
	}
...
public:
	...
	static RENDERER_API FSceneRendererBase* GetActiveInstance(FRDGBuilder& GraphBuilder);
	...
	/**
	* Utility function to retrieve from a View the appropriate ViewRect used to create an HZB
	*/
	static FIntRect GetHZBViewRect(const FViewInfo& View);
...
public:
	...
	FVisibleLightInfoArray VisibleLightInfos;
	...
	// Set to true if a whole scene point light shadow is present
	// Used for warnings about unsupported features
	bool bUsedPointLightWholeSceneShadow = false;
#endif
...
public:
	...
	*/
	...
	static FIntPoint ApplyResolutionFraction(const FIntPoint& UnscaledViewSize, float ResolutionFraction);
...
protected:
	...
	*/
	...
	void RenderSkyAtmosphereLookUpTables(FRDGBuilder& GraphBuilder, class FSkyAtmospherePendingRDGResources& PendingRDGResources, FDynamicShadowsTaskData* FilterDynamicShadowTaskData = nullptr);
	 ...
	 */
	template <typename T>
	using FComputeLightGridArray = TArray<T, TInlineAllocator<2, SceneRenderingAllocator>>;
	FComputeLightGridOutput ComputeLightGrid(FRDGBuilder& GraphBuilder, bool bCullLightsToGrid, const FSortedLightSetSceneInfo& SortedLightSet, FComputeLightGridArray<FForwardLightUniformParameters*>& PerViewForwardLightUniformParameters, FComputeLightGridArray<FForwardDirectionalLightShadowMapParameters*>& PerViewForwardDirLightShadowParameters);
...
protected:
	...
	void ComputeMegaLightsLightPowerDelta(FRDGBuilder& GraphBuilder);
...
private:
	...
	void PrepareViewStatePrevFrameViewInfo();
...
protected:
	...
	void RenderCustomRenderPassBasePass(FRDGBuilder& GraphBuilder, TArrayView<FViewInfo> InViews, FRDGTextureRef ViewFamilyTexture, FSceneTextures& SceneTextures, FMobileBasePassTextures& BasePassTextures, FInstanceCullingManager& InstanceCullingManager, bool bIncludeTranslucent);
	...
	/** Renders the debug view pass for mobile in its own render pass */
	void RenderMobileDebugViewMode(
		FRDGBuilder& GraphBuilder,
		FViewInfo& View,
		FSceneTextures& SceneTextures,
		FInstanceCullingManager& InstanceCullingManager,
		const class FMobileRenderPassParameters& BasePassParameters);
	...
	/** Renders decals. */
	TConstArrayView<const FVisibleDecal*> GetRelevantDeferredDecals(int32 DecalViewOffset, int32 ViewIndex, EDecalRenderStage DecalRenderStage);
	void RenderDecals(FRHICommandList& RHICmdList, FViewInfo& View, const FDecalViewInfo& DecalViewInfo, const FInstanceCullingDrawParams* InstanceCullingDrawParams);
	void RenderDeferredDecalsMobile(FRHICommandList& RHICmdList, const FViewInfo& View, const FDecalViewInfo& DecalViewInfo);
	void RenderDBuffer(FRDGBuilder& GraphBuilder, TArray<FViewInfo>& InViews, FSceneTextures& SceneTextures, FDBufferTextures& DBufferTextures, FInstanceCullingManager& InstanceCullingManager);

	/** Utility to fetch DecalViewOffset to pass to decal related rendering functions */
	int32 GetDecalViewOffset(TConstArrayView<FViewInfo> InViews) const;
	...
	void RenderDeferredMultiPassDecals(FRDGBuilder& GraphBuilder, FViewInfo& View, int32 DecalViewOffset, uint32 ViewIndex, FSceneTextures& SceneTextures, FInstanceCullingManager& InstanceCullingManager);
	...
	void ClearMobileScreenSpaceShadows(FRDGBuilder& GraphBuilder, const FViewInfo& View, const FLightSceneProxy* LightSceneProxy, FRDGTextureRef ScreenShadowMaskTexture);
	...
	void GatherShadowPointLights(const FViewInfo& View, const FSortedLightSetSceneInfo& SortedLightSet, TArray<int32, TInlineAllocator<8>>& OutShadowPointLightIndexes);
...
private:
	...
	FDecalVisibilityTaskData* DecalVisibilityTaskData = nullptr;
...
/** Checks whether deferred primitive alpha holdout is supported in shaders.*/
bool IsDeferredPrimitiveAlphaHoldoutSupported();

/** Checks whether primitive alpha holdout is enabled.*/
bool IsPrimitiveAlphaHoldoutEnabled(const FViewInfo& View);

/** Checks whether primitive alpha holdout is enabled for any view.*/
