...
#include "MegaFiltersInterface.h"
...
// HZB reallocate it's RT when dynamic resolution changes. While this behaviour needs to be addressed we don't want to pay the allocation cost on some platforms keeping the size fixed.
#if PLATFORM_DESKTOP
static constexpr int32 GHZBUseUpperBoundViewRectDefault = 1;
#else
static constexpr int32 GHZBUseUpperBoundViewRectDefault = 0;
#endif

static TAutoConsoleVariable<int32> CVarHZBUseUpperBoundViewRect(
	TEXT("r.HZB.UseUpperBoundViewRect"),
	GHZBUseUpperBoundViewRectDefault,
	TEXT(" 0: HZB sized to View.ViewRect (variable, smaller HZB, reallocation cost).\n")
	TEXT(" 1: HZB sized to View.UpperBoundViewRect (fixed, larger HZB, no reallocation cost).\n"),
	ECVF_RenderThreadSafe);
...
void FViewInfo::Init()
{
	...
	const bool bCubeShareCascades = bIsSceneCaptureCube && IsSceneCaptureCubeShareCascadesEnabled(GetShaderPlatform());

	// When cascade shadows are shared across all cube faces, we only need a single shadow atlas regardless of the number of views,
	// so the view count is ignored for this calculation of the number of cascade shadows that can be rendered.
	const int32 MaxMobileShadowCascadeCount = MAX_MOBILE_SHADOWCASCADES / (bCubeShareCascades ? 1 : FMath::Max(Family->Views.Num(), 1));
	...
	ShaderMap = GetGlobalShaderMap(GetShaderPlatform());
	...
	bSLWUsesMotionVectorWorldOffset = false;
	...
	{
		VolumetricCloudShadowRenderTarget[LightIndex] = nullptr;
		VolumetricCloudShadowExtractedRenderTarget[LightIndex] = nullptr;
		if (ViewState != nullptr)
		{
			ViewState->VolumetricCloudShadowRenderTarget[LightIndex].Reset();
		}
	}
}
...
const FRayTracingScene::FViewHandle& FViewInfo::GetRayTracingSceneViewHandle() const
{
...
void SetupPhysicsFieldUniformBufferParameters(const FScene* Scene, FEngineShowFlags EngineShowFlags, FViewUniformShaderParameters& ViewUniformShaderParameters)
{
	if (Scene && Scene->PhysicsField && Scene->PhysicsField->FieldResource)
	{
		...
		if (FieldResource->FieldInfos.bBuildClipmap)
		{
			...
			ViewUniformShaderParameters.PhysicsFieldNodesParams = FieldResource->NodesParams.SRV.GetReference();
			ViewUniformShaderParameters.PhysicsFieldNodesOffsets = FieldResource->NodesOffsets.SRV.GetReference();
			ViewUniformShaderParameters.PhysicsFieldTargetsOffsets = FieldResource->TargetsOffsets.SRV.GetReference();
		}
		else
		{
			ViewUniformShaderParameters.PhysicsFieldClipmapBuffer = GWhiteVertexBufferWithSRV->ShaderResourceViewRHI;
			ViewUniformShaderParameters.PhysicsFieldNodesParams = GWhiteVertexBufferWithSRV->ShaderResourceViewRHI;
			ViewUniformShaderParameters.PhysicsFieldNodesOffsets = GWhiteVertexBufferWithSRV->ShaderResourceViewRHI;
			ViewUniformShaderParameters.PhysicsFieldTargetsOffsets = GWhiteVertexBufferWithSRV->ShaderResourceViewRHI;
		}
		...
		for (int32 Index = 0; Index < MAX_PHYSICS_FIELD_TARGETS; ++Index)
		{
			...
			ViewUniformShaderParameters.PhysicsFieldTargetMappings[Index].X = Index < VectorTypes.Num() ? VectorTypes[Index] : 0;
			ViewUniformShaderParameters.PhysicsFieldTargetMappings[Index].Y = Index < ScalarTypes.Num() ? ScalarTypes[Index] : 0;
			ViewUniformShaderParameters.PhysicsFieldTargetMappings[Index].Z = Index < IntegerTypes.Num() ? IntegerTypes[Index] : 0;
			ViewUniformShaderParameters.PhysicsFieldTargetMappings[Index].W = 0;
		}
	}
	else
	{
		...
		ViewUniformShaderParameters.PhysicsFieldTargetMappings = EmptyTargets;
	}
}
	...
	FViewUniformShaderParameters& ViewUniformShaderParameters) const
{
	...
	const FIntRect EffectiveViewRect = GetMultiViewCount() > 1 ? FIntRect(0, 0, ViewRect.Width(), ViewRect.Height()) : ViewRect;
	...
	ViewUniformShaderParameters.VolumetricFogUVMax = GetFroxelGridUVMaxForSampling(ViewRectSize, VolumetricFogResourceGridSize, VolumetricFogResourceGridPixelSize);
...

	{
		...
		// When the primary upscaler is not temporal AA, the resolution is the same between the rendering pass
		// and the post processing pass after upscaling. No need to modify the ResolutionFractionAndInv.
		// When ResolutionFraction > 1.0, it is used for other purpose, e.g., in Shadow. 
	...
	// Subsurface
	{
		ViewUniformShaderParameters.bSubsurfacePostprocessEnabled = IsSubsurfaceEnabled(Family->EngineShowFlags) ? 1.0f : 0.0f;
	...
	// Toon Profiles
	{
		FRHITexture* Texture = ToonProfile::GetToonProfileTextureAtlasWithFallback();
		FIntVector TextureSize = Texture->GetSizeXYZ();
		ViewUniformShaderParameters.ToonProfileTextureSizeAndInvSize = FVector4f(TextureSize.X, TextureSize.Y, 1.0f / TextureSize.X, 1.0f / TextureSize.Y);

		// See ToonProfile.usf for the baking/sampling description.
		const float  ToonProfileDitherPatternFullResolution = ViewUniformShaderParameters.ToonProfileTextureSizeAndInvSize.X;
		const float  ToonProfileDitherPatternBakeResolution = ToonProfileDitherPatternFullResolution - 2.0f;
		const float  ToonProfileDitherPatternBakeResolutionInv = 1.0f / ToonProfileDitherPatternBakeResolution;
		ViewUniformShaderParameters.ToonProfileDitherPatternSizeAndInvSize = FVector2f(ToonProfileDitherPatternBakeResolution, ToonProfileDitherPatternBakeResolutionInv);

		ViewUniformShaderParameters.ToonProfileTexture = Texture;
		ViewUniformShaderParameters.ToonProfileSampler = TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
	}
	...
	ViewUniformShaderParameters.RealTimeReflectionCapturePreExposure = EyeAdaptation::GetCachedLightingPreExposure();
	...
	if (Scene && Scene->SkyLight)
	{
		...
		{
			SkyPreExposureInv = 1.0f / EyeAdaptation::GetCachedLightingPreExposure();
		}
		...
		ViewUniformShaderParameters.SkyLightColor = Family->EngineShowFlags.SkyLighting ? SkyLight->GetEffectiveLightColor() * SkyPreExposureInv * SkylightScale : FLinearColor::Black;
	...
	// Shading energy conservation
	{
		...
		ViewUniformShaderParameters.ShadingEnergyGGXMaxSpecTexture	= ShadingEnergyConservationData.GGXMaxSpecEnergyTexture ? ShadingEnergyConservationData.GGXMaxSpecEnergyTexture->GetRHI() : nullptr;
	...
	ViewUniformShaderParameters.ShadingEnergyGGXMaxSpecTexture	 = OrBlack2DIfNull(ViewUniformShaderParameters.ShadingEnergyGGXMaxSpecTexture);
...
FViewInfo* FViewInfo::CreateSnapshot(EViewSnapshotType Type) const
{
	return ViewSnapshotCache::Create(this, Type);
}
...
IPooledRenderTarget* FViewInfo::GetTonemappingLUT(FRHICommandList& RHICmdList, const int32 LUTSize, const bool bNeedFloatOutput) const 
{
	FSceneViewState* EffectiveViewState = GetEyeAdaptationViewState();
	if (EffectiveViewState)
	{
		return EffectiveViewState->GetTonemappingLUT(RHICmdList, LUTSize, bNeedFloatOutput);
	}
	return nullptr;
}

IPooledRenderTarget* FViewInfo::GetInnerTonemappingLUT(FRHICommandList& RHICmdList, const int32 LUTSize, const bool bNeedFloatOutput) const
{
	FSceneViewState* EffectiveViewState = GetEyeAdaptationViewState();
	if (EffectiveViewState)
	{
		return EffectiveViewState->GetInnerTonemappingLUT(RHICmdList, LUTSize, bNeedFloatOutput);
	}
	return nullptr;
}

void FViewInfo::ClearInnerTonemappingLUT() const
{
	FSceneViewState* EffectiveViewState = GetEyeAdaptationViewState();
	if (EffectiveViewState)
	{
		EffectiveViewState->ClearInnerTonemappingLUT();
	}
}
...
static bool PreparePostProcessSettingTextureForRenderer(const FViewInfo& View, UTexture2D* Texture2D, const TCHAR* TextureUsageName)
{
	...
	if (bIsValid && (Texture2D->IsFullyStreamedIn() == false || Texture2D->HasPendingInitOrStreaming()))
	{
		...
		{
			UE_LOGF(LogRenderer, Warning, "The %ls texture is not streamed in.", TextureUsageName);
		}
	...
	if (bIsValid && Texture2D->bHasStreamingUpdatePending == true)
	{
		if ((View.Family->FrameNumber % FramesPerWarning) == 0)
		{
			UE_LOGF(LogRenderer, Warning, "The %ls texture has pending update.", TextureUsageName);
		}
...
#endif
	{
		if ((View.Family->FrameNumber % FramesPerWarning) == 0)
		{
			UE_LOGF(LogRenderer, Warning, "The %ls texture is still using the default texture proxy.", TextureUsageName);
		}
...
FIntRect FSceneRendererBase::GetHZBViewRect(const FViewInfo& View)
{
	return CVarHZBUseUpperBoundViewRect.GetValueOnRenderThread() != 0 ? View.UpperBoundViewRect : View.ViewRect;
}
...
,	bShadowDepthRenderCompleted(false)
{
	...
	for (int32 ViewIndex = 0;ViewIndex < InViewFamily->Views.Num();ViewIndex++)
	{
		...
		if (float ViewRollAngle = CVarTestViewRollAngle.GetValueOnGameThread())
		{
			...
			NewMatrices.ViewRotationMatrix  = CurrentMatrices.GetWorldToView().RemoveTranslation() * Rotation;
			NewMatrices.ProjectionMatrix    = CurrentMatrices.GetViewToClip();
	...
	// Fork the plugin interfaces of the view family.
	{
		...
		if (ViewFamily.MegaFiltersInterface)
		{
			ViewFamily.MegaFiltersInterface = nullptr;
			ViewFamily.SetMegaFiltersInterface(InViewFamily->MegaFiltersInterface->Fork_GameThread(ViewFamily));
		}
	...
	// If multiple CRPs are in use with MSAA active, we share the non-MSAA version of scene textures across them.
	FCustomRenderPassInfo* CustomRenderPassInfoNoMSAA = nullptr;
	...
	for (int32 i = 0; i < Scene->CustomRenderPassRendererInputs.Num(); i+=IncrementIfNotRemoved)
	{
		...
		if (SceneCaptureUserData.bExcludeFromSceneTextureExtents)
		{
			// If excluded from scene texture extents, generate its own scene textures struct, by not passing a source view family.
			CustomRenderPassInfo = &CustomRenderPassInfos.Emplace_GetRef(FamilyCVS);
		}
		else if (GetDefaultMSAACount(Scene->GetFeatureLevel(), GDynamicRHI->RHIGetPlatformTextureMaxSampleCount()) > 1)
		{
			...
			if (!CustomRenderPassInfoNoMSAA)
			{
				CustomRenderPassInfo = &CustomRenderPassInfos.Emplace_GetRef(FamilyCVS);
				CustomRenderPassInfoNoMSAA = CustomRenderPassInfo;
			}
			else
			{
				CustomRenderPassInfo = &CustomRenderPassInfos.Emplace_GetRef(FamilyCVS, CustomRenderPassInfoNoMSAA->ViewFamily);
			}
		}
		else
		{
		...
		ViewInitOptions.bExcludeFromSceneTextureExtents = SceneCaptureUserData.bExcludeFromSceneTextureExtents;
...
// static
FIntPoint FSceneRenderer::ApplyResolutionFraction(const FIntPoint& UnscaledViewSize, float ResolutionFraction)
{
	FIntPoint ViewSize;
...
{
	FIntPoint Out;
	// some features (e.g. Nanite, Substrate, Lumen, MegaLights) require that view rect is aligned on 8x8 boundary.
	QuantizeSceneBufferSize(ViewRectMin, Out, 8);
	return Out;
}
...
{
	// If this function is called for a CustomRenderPass view family, it means it is getting its own scene textures,
	// separate from the main renderer, and only its own views should be counted, not AllViews.
	if (ViewFamily.Views[0]->CustomRenderPass)
	{
	...
	if (!ViewFamily.SupportsScreenPercentage())
	{
		...
		{
			// If a custom render pass has bExcludeFromSceneTextureExtents set, it will get its own scene textures, and shouldn't be counted in the main view family.
			if (View->CustomRenderPass && View->bExcludeFromSceneTextureExtents)
			{
				continue;
			}
	...
	for (const FSceneView* View : ViewFamily.AllViews)
	{
		...
		FIntPoint ViewSize = ApplyResolutionFraction(View->UnconstrainedViewRect.Size(), AdjustedResolutionFractionUpperBounds);
...
void FSceneRenderer::PrepareViewRectsForRendering()
{
	...
	// The only configurations that do not support screen percentage are Forward Mobile non-HDR and Forward Mobile w/ Tonemap Subpass.
	// These configurations do not support post-processing or any features that require ViewRect quantization (e.g. Nanite, Substrate, Lumen, MegaLights), so it's safe to skip.
	if (!ViewFamily.SupportsScreenPercentage())
	{
		...
		for (FViewInfo& View : Views)
		{
			View.ViewRect = View.UpperBoundViewRect = View.UnscaledViewRect;
		}
	...
	const float PrimaryResolutionFraction = DynamicResolutionFractions[GDynamicPrimaryResolutionFraction];
	const float PrimaryMaxResolutionFraction = DynamicResolutionUpperBounds[GDynamicPrimaryResolutionFraction];
	{
	...
	// Compute final resolution fraction.
	const float ResolutionFraction = PrimaryResolutionFraction * ViewFamily.SecondaryViewFraction * LensDistortionResolutionFraction;
	const float ResolutionMaxFraction = PrimaryMaxResolutionFraction * ViewFamily.SecondaryViewFraction * LensDistortionResolutionFraction;
	...
	for (int32 i = 0; i < Views.Num(); i++)
	{
		...
		auto InitViewRect = [&View, &MaxOverscanResolutionFraction, ViewFamilySecondaryViewFraction = ViewFamily.SecondaryViewFraction](float PositionResFraction, float SizeResFraction)
			{
				auto GetViewResFraction = [&](float Fraction)
				{
					return View.SceneViewInitOptions.OverridePrimaryResolutionFraction > 0.0 ? (View.SceneViewInitOptions.OverridePrimaryResolutionFraction * ViewFamilySecondaryViewFraction) : Fraction;
				};

				const float ViewPositionFraction = GetViewResFraction(PositionResFraction);
				const float ViewSizeFraction = GetViewResFraction(SizeResFraction);

				FIntPoint ViewSize = ApplyResolutionFraction(View.UnscaledViewRect.Size(), ViewSizeFraction * View.SceneViewInitOptions.OverscanResolutionFraction);
				FIntPoint ViewRectMin = QuantizeViewRectMin(FIntPoint(
					FMath::CeilToInt(View.UnscaledViewRect.Min.X * ViewPositionFraction * MaxOverscanResolutionFraction),
					FMath::CeilToInt(View.UnscaledViewRect.Min.Y * ViewPositionFraction * MaxOverscanResolutionFraction)));

				// Use the bottom-left view rect if requested, instead of top-left
				if (CVarViewRectUseScreenBottom.GetValueOnRenderThread())
				{
					ViewRectMin.Y = FMath::CeilToInt(View.UnscaledViewRect.Max.Y * ViewFamilySecondaryViewFraction) - ViewSize.Y;
				}

				return FIntRect(ViewRectMin, ViewRectMin + ViewSize);
			};

		if (Views.Num() > 1)
		{
			// In split-screen, multiple views share a depth buffer.  Use ResolutionMaxFraction for position so that adjacent views' UpperBoundViewRects don't overlap. 
			View.ViewRect = InitViewRect(ResolutionMaxFraction, ResolutionFraction);
			View.UpperBoundViewRect = InitViewRect(ResolutionMaxFraction, ResolutionMaxFraction);
		}
		else
		{
			View.ViewRect = InitViewRect(ResolutionFraction, ResolutionFraction);
			View.UpperBoundViewRect = InitViewRect(ResolutionMaxFraction, ResolutionMaxFraction);
		}
		...
		check(View.ViewRect.Area() != 0);
		check(View.UpperBoundViewRect.Area() != 0);
#if DO_CHECK || USING_CODE_ANALYSIS
		check(View.VerifyMembersChecks());
#endif
		...
		if (!ViewFamily.EngineShowFlags.HitProxies)
		{
			...
			const float ViewResolutionFraction = View.SceneViewInitOptions.OverridePrimaryResolutionFraction > 0.0 ? (View.SceneViewInitOptions.OverridePrimaryResolutionFraction * ViewFamily.SecondaryViewFraction) : ResolutionFraction;
	...
	// views in FSceneViewFamily::RenderTarget whereever it was requested with FSceneView::UnscaledViewRect.
	{
		...
		for (int32 i = 0; i < Views.Num(); i++)
		{
			...
			Views[i].UpperBoundViewRect -= TopLeftShift;
		}
	}
	...
	#if !UE_BUILD_SHIPPING
	{
		...
		if (Views.Num() == 1 && ViewRectOffset > 0)
		{
			...
			if (!View.bIsSceneCapture && !View.bIsReflectionCapture)
			{
				...
				const FIntPoint NewOffset = QuantizeViewRectMin(NewViewRectMin) - View.ViewRect.Min;

				View.ViewRect += NewOffset;
				View.UpperBoundViewRect += NewOffset;
	...
	for (FCustomRenderPassInfo& PassInfo : CustomRenderPassInfos)
	{
		for (FViewInfo& View : PassInfo.Views)
		{
			...
			if (SceneCaptureUserData.bMainViewResolution)
			{
				...
				View.UpperBoundViewRect = View.UnconstrainedViewRect = View.ViewRect;
			}
			else
			{
				View.UpperBoundViewRect = View.ViewRect = View.UnscaledViewRect;
			}
		}	
	}
}
...
class FCrossGPUTransfersDeferred : public UE::Private::FQueryableRefCountedObject
{
public:
...
IVisibilityTaskData* FSceneRenderer::OnRenderBegin(FRDGBuilder& GraphBuilder, const FSceneRenderUpdateInputs* SceneUpdateInputs)
{
	...
	if (SceneUpdateInputs)
	{
		PrepareSceneTexturesConfigTask = UE::Tasks::Launch(UE_SOURCE_LOCATION, [SceneUpdateInputs]
		{
			...
			for (FSceneRenderer* Renderer : SceneUpdateInputs->Renderers)
			{
				...
				{
					// If this is excluded from scene texture extents, it computes its own scene texture config, rather than copying.
					if (CustomRenderPass.ViewFamily.Views[0]->bExcludeFromSceneTextureExtents)
					{
						InitializeSceneTexturesConfig(CustomRenderPass.ViewFamily.SceneTexturesConfig, CustomRenderPass.ViewFamily);
					}
					else
					{
	...
	SceneUpdateParameters.Callbacks.PostStaticMeshUpdate = [&] (const UE::Tasks::FTask& StaticMeshUpdateTask)
	{
		...
		// This must be done after ViewExtension->PreRenderView_RenderThread performs any updates to the camera position
		PrepareViewStatePrevFrameViewInfo();
		...
		// Notify the atmosphere system to update lights that are needed for this scene and all its renders.
		SkyAtmosphereOnRenderBegin(*Scene, ViewFamily.EngineShowFlags);
		...
		// Notify the volumetric fog system that we start a new frame, deal with its state, etc.
		VolumetricFogOnRenderBegin(*Scene, Views);

		// Allow scene extensions to set up data that may be needed in the visibility task processing.
		GetSceneExtensionsRenderers().PreVisibilityTasks(GraphBuilder);
...
void FSceneRenderer::OnRenderFinish(FRDGBuilder& GraphBuilder, FRDGTextureRef ViewFamilyTexture)
{
	...
	if (ViewFamilyTexture)
	{
		...
		const bool bShowPointLightWarning = !IsMobilePlatform(ShaderPlatform) ? bUsedPointLightWholeSceneShadow && !FReadOnlyCVARCache::EnablePointLightShadows(ShaderPlatform) : false;
		...
		{	
			...
			if (!View.bIsReflectionCapture && !View.bIsSceneCapture )
			{
				...
				const bool bMegaLightsHasWarning = MegaLights::HasWarning(View);
				const bool bEyeAdaptationHasWarning = EyeAdaptation::WriteWarnings(Scene, View, /*Writer*/ nullptr);
...
#endif
				const bool bViewHasWarnings = bAnyWarning
					|| bIsFrozen
					|| bLocked
					|| bStereoView
					|| bShowAmbientCubemapMipGenSettingsWarning
					|| bGPUSkinCacheVisualzationMode
					|| bMegaLightsHasWarning
					|| bEyeAdaptationHasWarning;
				...
				if ((GAreScreenMessagesEnabled && !GEngine->bSuppressMapWarnings) && bViewHasWarnings)
				{
					...
					const bool bIsInstancedStereo = View.IsInstancedStereo();
					const bool bIsMobileMultiViewEnabled = View.UsesMobileMultiViewShaders();
					...
					AddDrawCanvasPass(GraphBuilder, {}, View, Output,
						...
						bShowAmbientCubemapMipGenSettingsWarning, bLocalFogVolumeInSceneButProjectDisabled, bLumenHasWarnings, bMegaLightsHasWarning, bEyeAdaptationHasWarning,
						bStereoView, bPrimaryStereoView, bIsInstancedStereo, bIsMobileMultiViewEnabled, bContactShadowIntensityCvarUsed, bExpenssiveSkyLightRealTimeCaptureWithCloud]
						(FCanvas& Canvas)
					{
						...
						if (bIsFrozen)
						{
							...
							Writer.DrawLine(StateText, 10, FLinearColor(0.8f, 1.0f, 0.2f, 1.0f));
						}
						...
						{
							static const FText Message = NSLOCTEXT("Renderer", "PointLight", "A shadow casting point light is present but the project does not support whole scene point light shadows");
							Writer.DrawLine(Message);
						}
						...
						if (bLocked)
						{
							...
							Writer.DrawLine(Message, 10, FLinearColor(0.8f, 1.0f, 0.2f, 1.0f));
						}
						...
						{
							MegaLights::WriteWarnings(View, Writer);
						}

						if (bEyeAdaptationHasWarning)
						{
							EyeAdaptation::WriteWarnings(Scene, View, &Writer);
						}

						if (bNaniteEnabledButNoAtomics)
						{
							static const FText Message = NSLOCTEXT("Renderer", "NaniteNotSupported", "Nanite is used in the scene but not supported by your graphics hardware and/or driver. Meshes will not render using Nanite.");
							Writer.DrawLine(Message);
						}
						...
						{
							static const FTextFormat VideoMemoryExhaustedFmt = NSLOCTEXT("Renderer", "VideoMemoryExhausted", "Video memory has been exhausted ({0} MB over budget). Expect extremely poor performance.");
							Writer.DrawLine(FText::Format(VideoMemoryExhaustedFmt, FText::AsNumber(float(GDemotedLocalMemorySize) / 1048576.0f)));
						}
						...
						if (bStereoView)
						{
							const TCHAR* SecondaryOrInstanced = bIsInstancedStereo || bIsMobileMultiViewEnabled ? TEXT("Instanced") : TEXT("Secondary");
							...
							if (bPrimaryStereoView)
							{
								...
								{
									Technique = TEXT("Instanced-Stereo");
								}
						...
						if (bShowWaitingSkylight && SkyLight)
						{
							...
							FString String = "Sky Light waiting on ";
...
void FSceneRenderer::SetupMeshPass(FViewInfo& View, FExclusiveDepthStencil::Type BasePassDepthStencilAccess, FViewCommands& ViewCommands, FInstanceCullingManager& InstanceCullingManager)
{
	...
	for (int32 PassIndex = 0; PassIndex < EMeshPass::Num; PassIndex++)
	{
		...
		if ((FPassProcessorManager::GetPassFlags(ShadingPath, PassType) & EMeshPassFlags::MainView) != EMeshPassFlags::None)
		{
			...
			EInstanceCullingMode InstanceCullingMode = bIsMainViewPass && View.IsInstancedStereo() ? EInstanceCullingMode::Stereo : EInstanceCullingMode::Normal;
...
#if WITH_EDITOR
			if (PassType == EMeshPass::EditorSelection)
			{
				CullingFlags |= EInstanceCullingFlags::DrawOnlySelected;
			}
#endif
			...
			Pass.DispatchPassSetup(
				Scene,
				View,
				FInstanceCullingContext(GetMeshPassName(PassType), ShaderPlatform, &InstanceCullingManager, ViewIds, bAllowInstanceOcclusionCulling ? View.PrevViewInfo.HZB : nullptr, InstanceCullingMode, CullingFlags, EBatchProcessingMode::UnCulled, HierarchicalCullingQuery),
				PassType,
...
bool FSceneRenderer::ShouldCompositeEditorPrimitives(const FViewInfo& View)
{
	...
	if ((ShowFlags.VisualizeHDR && !ShowFlags.Wireframe) ||				// VisualizeHDR is skipped with in Wireframe mode.
		ShowFlags.VisualizeSkyLightIlluminance || ShowFlags.VisualizeToonProfileAtlas ||
	...
	const FSceneTextures& SceneTextures)
{
	if (PostOpaqueRenderDelegate.IsBound())
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			...
			RenderParameters.ViewMatrix = View.ViewMatrices.GetWorldToView();
			RenderParameters.ProjMatrix = View.ViewMatrices.GetViewToClip();
...
{
public:
	FScenePrimitiveRenderingContext(FRDGBuilder& InGraphBuilder, FScene& Scene, FSceneViewFamily* InViewFamily = nullptr, EBeginScenePrimitiveRenderingFlags BeginScenePrimitiveRenderingFlags = EBeginScenePrimitiveRenderingFlags::All) :
		...
		ViewFamily(InViewFamily)
	{
		...
		if (EnumHasAllFlags(BeginScenePrimitiveRenderingFlags, EBeginScenePrimitiveRenderingFlags::UpdateScenePrimitives))
		{
...
IScenePrimitiveRenderingContext* FRendererModule::BeginScenePrimitiveRendering(FRDGBuilder &GraphBuilder, FSceneViewFamily* ViewFamily, EBeginScenePrimitiveRenderingFlags BeginScenePrimitiveRenderingFlags)
{
	check(ViewFamily);
	...
	FScenePrimitiveRenderingContext* ScenePrimitiveRenderingContext = new FScenePrimitiveRenderingContext(GraphBuilder, *Scene, ViewFamily, BeginScenePrimitiveRenderingFlags);

	return ScenePrimitiveRenderingContext;
}

IScenePrimitiveRenderingContext* FRendererModule::BeginScenePrimitiveRendering(FRDGBuilder &GraphBuilder, FSceneInterface& InScene, EBeginScenePrimitiveRenderingFlags BeginScenePrimitiveRenderingFlags)
{
	FScene* Scene = InScene.GetRenderScene();
	check(Scene);

	FScenePrimitiveRenderingContext* ScenePrimitiveRenderingContext = new FScenePrimitiveRenderingContext(GraphBuilder, *Scene, nullptr, BeginScenePrimitiveRenderingFlags);
...
IMaterialCacheVirtualTextureAllocator* FRendererModule::GetMaterialCacheVirtualTextureAllocator()
{
	return &FMaterialCacheVirtualTextureAllocator::Get();
}
...
{
#if WITH_EDITOR
	FVirtualTextureSystem::Get().SetRequestRecordBuffer(Handle);
#endif
}

uint64 FRendererModule::GetVirtualTextureRequestRecordBuffer(TMap<uint64, uint32>& OutRequestData)
{
#if WITH_EDITOR
	return FVirtualTextureSystem::Get().GetRequestRecordBuffer(OutRequestData);
#else
	return (uint64)-1;
#endif
}

void FRendererModule::PackVirtualTextureRequestsToStream(TMap<uint64, uint32> const& InPageRequests, TArray<uint32>& OutRequestStream)
{
#if WITH_EDITOR
	FVirtualTextureSystem::PackRequestsToStream(InPageRequests, OutRequestStream);
#else
	OutRequestStream.Reset();
#endif
}

void FRendererModule::RequestVirtualTextureTiles(TArray<uint32>&& InRequestStream, FIntPoint const& InCaptureResolution)
{
	FVirtualTextureSystem::Get().RequestRecordedTiles(MoveTemp(InRequestStream), InCaptureResolution);
}
...
uint64 FRendererModule::GetNaniteRequestRecordBuffer(TMap<uint64, uint32>& OutPageRequests)
{
#if WITH_EDITOR
...
void FRendererModule::PackNaniteRequestsToStream(TMap<uint64, uint32> const& InPageRequests, TArray<uint32>& OutRequestStream)
{
#if WITH_EDITOR
	Nanite::FStreamingManager::PackRequestsToStream(InPageRequests, OutRequestStream);
#else
	OutRequestStream.Reset();
#endif 
}

void FRendererModule::RequestNanitePages(TArray<uint32>&& InRequestStream)
{
	Nanite::GStreamingManager.RequestNanitePages(MoveTemp(InRequestStream));
}
...
static void DisplayInternals(FRDGBuilder& GraphBuilder, FViewInfo& InView)
{
	...
	#define CANVAS_LINE(bHighlight, txt, ... ) \
			...
			CANVAS_LINE(false, TEXT("  ViewProjectionMatrix Hash: %x"), InView.ViewMatrices.GetWorldToClip().ComputeHash())
...
bool IsDeferredPrimitiveAlphaHoldoutSupported()
{
	// Read-only cvar is expected to match SUPPORT_PRIMITIVE_ALPHA_HOLDOUT shader definition
	return CVarPrimitiveAlphaHoldoutSupport.GetValueOnRenderThread();
}
...
bool DoMaterialAndPrimitiveModifyMeshPosition(const FMaterial& Material, const FPrimitiveSceneProxy* PrimitiveSceneProxy, const FMeshBatch& MeshBatch)
{
	...
	// HISM batches with dithered LOD transition also have special logic to hide non-visible LODs in the VS (see Intermediates.IsVisible in LocalVertexFactory.ush)
	// Using InstancedLODRange as a marker, since it's currently only set by HISMs
	const bool bHISMDitherModifiesPosition = MeshBatch.Elements.Num() > 0 && MeshBatch.Elements[0].InstancedLODRange != 0;
	return (bMaterialModifiesMeshPosition && bPrimitiveAllowsWPOEvaluation) || bIsFirstPerson || bHISMDitherModifiesPosition; 
}