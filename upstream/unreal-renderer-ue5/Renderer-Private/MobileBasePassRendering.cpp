...
#define IMPLEMENT_MOBILE_SHADING_BASEPASS_LIGHTMAPPED_PIXEL_SHADER_TYPE(LightMapPolicyType, LightMapPolicyName, LocalLightSettingType, LocalLightSettingName) \
	typedef TMobileBasePassPS< LightMapPolicyType, LocalLightSettingType, EMobileTranslucentColorTransmittanceMode::DEFAULT > TMobileBasePassPS##LightMapPolicyName##LocalLightSettingName; \
	...
	typedef TMobileBasePassPS< LightMapPolicyType, LocalLightSettingType, EMobileTranslucentColorTransmittanceMode::SINGLE_SRC_BLENDING > TMobileBasePassPS##LightMapPolicyName##LocalLightSettingName##ThinTranslGrey; \
...
#define IMPLEMENT_MOBILE_SHADING_BASEPASS_LIGHTMAPPED_SHADER_TYPE(LightMapPolicyType, LightMapPolicyName) \
	...
	IMPLEMENT_MOBILE_SHADING_BASEPASS_LIGHTMAPPED_PIXEL_SHADER_TYPE(LightMapPolicyType, LightMapPolicyName, LOCAL_LIGHTS_ENABLED, LocalLights)
...
IMPLEMENT_MOBILE_SHADING_BASEPASS_LIGHTMAPPED_SHADER_TYPE(FMobileSingleLayerWaterPolicy, FMobileSingleLayerWaterPolicy);
	...
	bool bIsRenderingDeferredBasepass)
{
	...
	if (InstancedView && View.UsesMobileMultiViewShaders())
	{
	...
	SetForwardLightUniformParametersFromView(GraphBuilder, View, BasePassParameters.Forward);
	...
	// We need some kind of "Dummy" type resource here, to be fully correct.
	// Otherwise UAVs break RDG pass auto-merging
	if (BasePass <= EMobileBasePass::Opaque || GMaxRHIFeatureLevel > ERHIFeatureLevel::ES3_1)
	{
	...
	FMobileDirectionalLightShaderParameters& Params)
{
	...
	FLightSceneInfo* Light = Scene.GetMobileDirectionalLightForView(ChannelIdx, SceneView);
	if (Light)
	{
		...
		const FVector2D FadeParams = GetDirectionalLightDistanceFadeParameters(*Light, FeatureLevel, Light->IsPrecomputedLightingValid(), SceneView.MaxShadowCascades);
		...
		if (bDynamicShadows && VisibleLightInfos.IsValidIndex(Light->Id) && VisibleLightInfos[Light->Id].AllProjectedShadows.Num() > 0)
		{
			...
			for (int32 i = 0; i < NumShadowsToCopy && OutShadowIndex < SceneView.MaxShadowCascades; ++i)
			{
				...
				const FViewInfo* DependentSceneView = IStereoRendering::IsAPrimaryView(SceneView) ? &SceneView : SceneView.GetPrimaryView();
				if (ShadowInfo->ShadowDepthView && !ShadowInfo->bRayTracedDistanceField && ShadowInfo->CacheMode != SDCM_StaticPrimitivesOnly && ShadowInfo->DependentView == DependentSceneView)
				{
					if (OutShadowIndex == 0)
					{
					...
					Params.DirectionalLightShadowDepthRange[OutShadowIndex] = ShadowInfo->MaxSubjectZ;										// TODO: Depth range is not MaxSubjectZ - MinSubjectZ as on the non-mobile it seems?
...
void FMobileSceneRenderer::RenderMobileBasePass(FRHICommandList& RHICmdList, const FViewInfo& View, const FInstanceCullingDrawParams* InstanceCullingDrawParams, const FInstanceCullingDrawParams* SkyPassInstanceCullingDrawParams)
{
	...
	if (bIsFullDepthPrepassEnabled)
	{
		...
		DrawRenderState.SetDepthStencilState(TStaticDepthStencilState<false>::GetRHI());
	}
	else
	{
		DrawRenderState.SetDepthStencilAccess(FExclusiveDepthStencil::DepthWrite_StencilWrite);
		DrawRenderState.SetDepthStencilState(TStaticDepthStencilState<true>::GetRHI());
	}
...
void FMobileSceneRenderer::RenderMobileEditorPrimitives(FRHICommandList& RHICmdList, const FViewInfo& View, const FMeshPassProcessorRenderState& DrawRenderState, const FInstanceCullingDrawParams* InstanceCullingDrawParams)
{
	...
	if (!View.Family->EngineShowFlags.CompositeEditorPrimitives)
	{
		...
		View.BatchedViewElements.Draw(RHICmdList, DrawRenderState, View, false);
		...
		{
			FRHIUniformBuffer* InstanceCullingBufferRHI = InstanceCullingDrawParams->InstanceCulling.GetUniformBuffer()->GetRHI();
			check(InstanceCullingBufferRHI);

			FUniformBufferStaticSlot InstanceCullingUBOSlot = FInstanceCullingContext::GetStaticUniformBufferSlot(View.GetShaderPlatform());
			RHICmdList.SetStaticUniformBuffer(InstanceCullingUBOSlot, InstanceCullingBufferRHI);
		}

		// Draw the view's batched simple elements(lines, sprites, etc).
		View.TopBatchedViewElements.Draw(RHICmdList, DrawRenderState, View, false);
	}
}
...
void FMobileSceneRenderer::RenderMobileDebugPrimitives(FRHICommandList& RHICmdList, const FViewInfo& View)
{
	...
	if (View.DebugSimpleElementCollector.HasAnyPrimitives())
	{
		...
		const bool bDepthWrite = !bIsFullDepthPrepassEnabled;
		if (bDepthWrite)
		{
			DrawRenderState.SetDepthStencilAccess(FExclusiveDepthStencil::DepthWrite_StencilWrite);
		}
		...
		DrawRenderState.SetDepthStencilState(bDepthWrite ? TStaticDepthStencilState<true, CF_DepthNearOrEqual>::GetRHI() : TStaticDepthStencilState<false, CF_DepthNearOrEqual>::GetRHI());
		...
		DrawRenderState.SetDepthStencilState(bDepthWrite ? TStaticDepthStencilState<true, CF_Always>::GetRHI() : TStaticDepthStencilState<false, CF_Always>::GetRHI());
