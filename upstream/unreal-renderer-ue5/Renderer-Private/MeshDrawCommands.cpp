...
static TAutoConsoleVariable<int32> CVarMeshSortingMethodWithoutEarlyZ(
	TEXT("r.MeshSortingMethodWithoutEarlyZ"),
	0,
	TEXT("How to sort mesh commands:\n")
...
static TAutoConsoleVariable<int32> CVarMobileMeshSortingMethod(
	...
	TEXT("\t-1: Use r.MeshSortingMethodWithoutEarlyZ (Default).\n")
	TEXT("\t0: Sort by state, roughly front to back.\n")
	TEXT("\t1: Strict front to back sorting.\n"),
	ECVF_RenderThreadSafe);
...
static uint64 GetSortKey_FrontToBack(bool bMasked, bool bBackground, uint32 PipelineId, int32 StateBucketId, float PrimitiveDistance)
{
	union
	{
		uint64 PackedInt;
...
static uint64 GetSortKey_ByState(bool bMasked, bool bBackground, uint32 PipelineId, uint32 StateBucketId, float PipelineDistance, float StateBucketDistance, float PrimitiveDistance)
{
...
/**
 * Compute mesh sort keys to reduce overdraw
 */
static void UpdateMeshSortKeys(
	bool bMobile,
	const FVector& ViewOrigin,
...
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_UpdateMeshSortKeys);

	// Object radius past which we treat object as part of 'background'
	constexpr float MIN_BACKGROUND_OBJECT_RADIUS = 500000;

	const int32 NumCmds = VisibleMeshCommands.Num();
	const int32 MobileMeshSortingMethod = bMobile ? CVarMobileMeshSortingMethod.GetValueOnAnyThread() : -1;
	const int32 MeshSortingMethod = (MobileMeshSortingMethod >= 0)
		? MobileMeshSortingMethod
		: CVarMeshSortingMethodWithoutEarlyZ.GetValueOnAnyThread();
	...
	if (MeshSortingMethod == 1) //strict front to back sorting
	{
		...
		for (int32 CmdIdx = 0; CmdIdx < NumCmds; ++CmdIdx)
		{
			...
			Cmd.SortKey.PackedData = GetSortKey_FrontToBack(bMasked, bBackground, PipelineId, StateBucketId, PrimitiveDistance);
		}
	}
	else
	{
		...
		for (int32 CmdIdx = 0; CmdIdx < NumCmds; ++CmdIdx)
		{
			...
			Cmd.SortKey.PackedData = GetSortKey_ByState(bMasked, bBackground, PipelineId, StateBucketId, MeanPipelineDistance, StateBucketDistance, PrimitiveDistance);
		}
	}
}
	...
	for (int32 DrawCommandIndex = 0; DrawCommandIndex < VisibleMeshDrawCommands.Num(); ++DrawCommandIndex)
	{
		...
		if (bUseIndirect || MeshDrawCommand->NumPrimitives == 0)
		{
			...
			else if (bSupportsGPUSceneInstancing)
			{
				...
				DrawData.NumBatches = MeshDrawCommandInfo.NumBatches;
			}
...
public:
	...
	void AnyThreadTask()
	{
		...
		// Also sort if EarlyZPassMode is disabled to reduce overdraw
		const bool bNeedsUpdateMeshSortKeys =
			(bMobile || (Context.Scene->EarlyZPassMode == DDM_None)) &&
			((Context.PassType == EMeshPass::DepthPass) ||
		...
		if (Context.MeshDrawCommands.Num() > 0)
		{
			...
			else if (Context.TranslucencyPass != ETranslucencyPass::TPT_MAX)
			{
				...
				{
					bInverseSorting &= OIT::IsSortedPixelsEnabledForPass(OITPass_SeparateTranslucency);
				}
...
void SortAndMergeDynamicPassMeshDrawCommands(
	...
	FRHIBuffer*& OutInstanceIdVertexBuffer,
	uint32 InstanceFactor,
	const FGPUScenePrimitiveCollector* DynamicPrimitiveCollector)
{
	OutInstanceIdVertexBuffer = nullptr;
	...
	if (NumDrawCommands > 0)
	{
		...
		VisibleMeshDrawCommands.Sort(FCompareFMeshDrawCommands());
		{
			...
			if (bUsesUniformView)
			{
				...
				static_assert(sizeof(FBatchedPrimitiveShaderData) == sizeof(FBatchedInstanceShaderData),
					"Primitive and Instance shader records must share the same stride for the parallel CPU writer.");

				// Parallel instance buffer holds one canonical instance record per draw command, 1:1 with the primitive array.
				const int32 InstanceIdBufferDataSize = PrimitiveIdBufferDataSize;
				FPrimitiveIdVertexBufferPoolEntry InstanceEntry = GPrimitiveIdVertexBufferPool.Allocate(RHICmdList, InstanceIdBufferDataSize);
				OutInstanceIdVertexBuffer = InstanceEntry.BufferRHI;
				void* InstanceIdBufferData = RHICmdList.LockBuffer(OutInstanceIdVertexBuffer, 0, InstanceIdBufferDataSize, RLM_WriteOnly);
				...
				auto WritePrimitiveDataFn = [&](int32 PrimitiveIndex, int32 DrawPrimitiveId, int32 ScenePrimitiveId)
				{
					...
					FBatchedInstanceShaderData* InstanceData = reinterpret_cast<FBatchedInstanceShaderData*>(InstanceIdBufferData) + PrimitiveIndex;
					const uint32 PrimitiveSlotIndex = static_cast<uint32>(PrimitiveIndex);
					...
					if ((DrawPrimitiveId & GPrimIDDynamicFlag) != 0)
					{
						...
						FBatchedInstanceShaderData::Emplace(InstanceData, *ShaderParams, PrimitiveSlotIndex);
					}
					else
					{
						...
						if (ScenePrimitiveId >= 0 && ScenePrimitiveId < Scene->Primitives.Num())
						{
							...
							FBatchedInstanceShaderData::Emplace(InstanceData, PrimitiveProxy, PrimitiveSlotIndex);
						}
						else
						{
							FBatchedPrimitiveShaderData::Emplace(PrimitiveData, *IdentityShaderParams);
							FBatchedInstanceShaderData::Emplace(InstanceData, *IdentityShaderParams, PrimitiveSlotIndex);
						}
					}
				};
				...
				RHICmdList.UnlockBuffer(OutInstanceIdVertexBuffer);
				GPrimitiveIdVertexBufferPool.ReturnToFreeList(InstanceEntry);
			}
			else
			{
...
)
{
	...
	check(!TaskEventRef.IsValid() && MeshPassProcessor != nullptr);
	check((PassType == EMeshPass::Num) == (DynamicMeshElementsPassRelevance == nullptr));
	...
	TaskContext.InstanceFactor = (bIsMainViewPass && View.IsInstancedStereo() && IStereoRendering::IsAPrimaryView(View)) ? 2 : 1;
	...
	TaskContext.ViewMatrix = View.ViewMatrices.GetWorldToView();
...
void FParallelMeshDrawCommandPass::DumpInstancingStats() const
{
	...
	{
		UE_LOGF(LogRenderer, Log, "Instancing stats for %ls", *PassNameForStats);
		UE_LOGF(LogRenderer, Log, "   %i Mesh Draw Commands in %i instancing state buckets", TaskContext.VisibleMeshDrawCommandsNum, TaskContext.NewPassVisibleMeshDrawCommandsNum);
		UE_LOGF(LogRenderer, Log, "   Largest %i", TaskContext.MaxInstances);
		UE_LOGF(LogRenderer, Log, "   %.1f Dynamic Instancing draw call reduction factor", TaskContext.VisibleMeshDrawCommandsNum / (float)TaskContext.NewPassVisibleMeshDrawCommandsNum);
	}
}
