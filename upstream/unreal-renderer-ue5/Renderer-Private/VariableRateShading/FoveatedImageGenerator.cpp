...
void FFoveatedImageGenerator::PrepareImages(FRDGBuilder& GraphBuilder, const FSceneViewFamily& ViewFamily, const FMinimalSceneTextures& SceneTextures, bool bPrepareHardwareImages, bool bPrepareSoftwareImages)
{
	...
	TShaderMapRef<FComputeVariableRateShadingImageGeneration> ComputeShader(GetGlobalShaderMap(GMaxRHIShaderPlatform)); // If in stereo, shader sets up a single side-by-side image for both eyes at once
...
{
	// On desktop, we always use palette rates, but may want this available for Software VRS even if DataType == VRSImage_Unsupported
	// On mobile, we may use palette or fractional rates, and this generator currently only supports palette rates
	return !IsMobilePlatform(View.GetShaderPlatform()) || GRHIVariableRateShadingImageDataType == VRSImage_Palette;
}
