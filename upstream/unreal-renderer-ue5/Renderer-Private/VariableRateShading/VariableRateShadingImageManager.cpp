...
void CVarVRSPreviewCallback(IConsoleVariable* Var)
{
	...
	{
		UE_LOGF(LogVRS, Warning, "Selected invalid preview mode, disabling preview");
	}
}
...
void CVarVRSDebugForceRateCallback(IConsoleVariable* Var)
{
	...
	{
		UE_LOGF(LogVRS, Warning, "Selected forced shading rate exceeds maximum available, defaulting to %ls", GRHISupportsLargerVariableRateShadingSizes ? TEXT("4x4") : TEXT("2x2"));
	}
}
...
static void CVarVRSImagePassTypeCallback(IConsoleVariable* Var)
{
	...
	{
		UE_LOGF(LogVRS, Warning, "Selected invalid image type, disabling VRS for pass");
	}
}
...
TAutoConsoleVariable<int32> CVarVRSBasePass(
	TEXT("r.VRS.BasePass"),
	2,
	TEXT("Whether to apply screen space VRS to the base pass.\n")
	TEXT("0: Disabled\n")
	TEXT("1: Full\n")
...
TAutoConsoleVariable<int32> CVarVRSTranslucency(
	...
	1,
	TEXT("Whether to apply screen space VRS to the translucency pass.\n")
	TEXT("0: Disabled\n")
	TEXT("1: Full (default)\n")
	TEXT("2: Conservative"),
	FConsoleVariableDelegate::CreateStatic(&CVarVRSImagePassTypeCallback),
...
TAutoConsoleVariable<int32> CVarVRSNaniteEmitGBuffer(
	...
	2,
	TEXT("Whether to apply screen space VRS to Nanite.\n")
	TEXT("0: Disabled\n")
	TEXT("1: Full\n")
	TEXT("2: Conservative (default)"),
	FConsoleVariableDelegate::CreateStatic(&CVarVRSImagePassTypeCallback),
...
TAutoConsoleVariable<int32> CVarVRS_SSAO(
	TEXT("r.VRS.SSAO"),
	0,
	TEXT("Whether to apply screen space VRS to SSAO (not currently recommended).\n")
...
TAutoConsoleVariable<int32> CVarVRS_SSR(
	TEXT("r.VRS.SSR"),
	2,
	TEXT("Whether to apply screen space VRS to screen space reflections (only supported when r.SSR.Compute=0).\n")
	TEXT("0: Disabled\n")
	TEXT("1: Full\n")
	TEXT("2: Conservative (default)"),
	FConsoleVariableDelegate::CreateStatic(&CVarVRSImagePassTypeCallback),
...
TAutoConsoleVariable<int32> CVarVRSReflectionEnvironmentSky(
	...
	2,
	TEXT("Whether to apply screen space VRS to reflections and indirect sky lighting (only supported when Lumen is disabled).\n")
	TEXT("0: Disabled\n")
	TEXT("1: Full\n")
	TEXT("2: Conservative (default)"),
	FConsoleVariableDelegate::CreateStatic(&CVarVRSImagePassTypeCallback),
...
TAutoConsoleVariable<int32> CVarVRSLightFunctions(
	...
	1,
	TEXT("Whether to apply screen space VRS to light functions.\n")
	TEXT("0: Disabled\n")
	TEXT("1: Full (default)\n")
	TEXT("2: Conservative"),
	FConsoleVariableDelegate::CreateStatic(&CVarVRSImagePassTypeCallback),
...
TAutoConsoleVariable<int32> CVarVRSDecals(
	TEXT("r.VRS.Decals"),
	2,
	TEXT("Whether to apply screen space VRS to decals.\n")
	TEXT("0: Disabled\n")
	TEXT("1: Full\n")
	TEXT("2: Conservative (default)"),
	FConsoleVariableDelegate::CreateStatic(&CVarVRSImagePassTypeCallback),
...
// If false, we are using fragment shading rate images (selects rates using a palette)
// If true, we are using fragment density map images (suggests rates with floats as a percentage of full resolution)
class FFragmentDensityMap : SHADER_PERMUTATION_BOOL("FRAGMENT_DENSITY_MAP");
...
public:
	...
	using FPermutationDomain = TShaderPermutationDomain<FFragmentDensityMap>;
...
class FDebugVariableRateShadingPS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, VariableRateShadingTextureIn)
...
void FVariableRateShadingImageManager::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	{
		UE_LOGF(LogVRS, Log, "Current RHI supports per-draw and screenspace Variable Rate Shading");
	}
	else if (GRHISupportsPipelineVariableRateShading)
	{
		UE_LOGF(LogVRS, Log, "Current RHI supports per-draw Variable Rate Shading");
	}
	else if (GRHISupportsAttachmentVariableRateShading)
	{
		UE_LOGF(LogVRS, Log, "Current RHI supports screenspace Variable Rate Shading");
	}
	else
	{
		UE_LOGF(LogVRS, Log, "Current RHI does not support Variable Rate Shading");
	}
}
...
bool FVariableRateShadingImageManager::IsPipelineVRSEnabled() const
{
	...
	return GRHISupportsPipelineVariableRateShading && CVarEnableVRS.GetValueOnAnyThread() != 0;
}
...
{
	// When using palette shading rate images, we will always support the four rates 1x1, 1x2, 2x1, and 2x2.
	...
	// When using fractional shading rate images, we still use a palette rate to set r.VRS.DebugForceRate. The selected palette rate is automatically converted into a fractional rate.
	// As such, we should allow selecting expanded rates so we can fully represent fractional rates 0.25 (4x), 0.5 (2x), and 1.0 (1x).
	return (GRHISupportsLargerVariableRateShadingSizes || GRHIVariableRateShadingImageDataType == VRSImage_Fractional) ? NumExpandedRates : NumBaseRates;
}
...
void FVariableRateShadingImageManager::DrawDebugPreview(FRDGBuilder& GraphBuilder, const FSceneViewFamily& ViewFamily, FRDGTextureRef OutputSceneColor)
{
	...
	for (const FSceneView* View : ViewFamily.Views)
	{
		...
		if (ViewInfo->ShouldRenderView() && IsVRSCompatibleWithView(*ViewInfo))
		{
			...
			FDebugVariableRateShadingPS::FPermutationDomain PermutationVector;
			PermutationVector.Set<FFragmentDensityMap>(GRHIVariableRateShadingImageDataType == VRSImage_Fractional);

			TShaderMapRef<FDebugVariableRateShadingVS> VertexShader(ViewInfo->ShaderMap);
			TShaderMapRef<FDebugVariableRateShadingPS> PixelShader(ViewInfo->ShaderMap, PermutationVector);

			auto* PassParameters = GraphBuilder.AllocParameters<FDebugVariableRateShadingPS::FParameters>();
			PassParameters->VariableRateShadingTextureIn = PreviewTexture;
			PassParameters->RenderTargets[0] = FRenderTargetBinding(OutputSceneColor, ERenderTargetLoadAction::ELoad);
			PassParameters->RenderTargets.MultiViewCount = ViewInfo->GetMultiViewCount();
...
void FVariableRateShadingImageManager::RegisterExternalImageGenerator(IVariableRateShadingImageGenerator* ExternalGenerator)
{
	...
	{
		UE_LOGF(LogVRS, Warning, "Trying to register a null VRS generator. Generator will be ignored.");
		return;
	}
	FWriteScopeLock GeneratorsLock(GeneratorsMutex);
...
void FVariableRateShadingImageManager::UnregisterExternalImageGenerator(IVariableRateShadingImageGenerator* ExternalGenerator)
{
	if (ExternalGenerator == nullptr)
	{
		UE_LOGF(LogVRS, Warning, "Trying to unregister a null VRS generator. Generator will be ignored.");
		return;
	}
	FWriteScopeLock GeneratorsLock(GeneratorsMutex);
...
FRDGTextureRef FVariableRateShadingImageManager::CombineShadingRateImages(FRDGBuilder& GraphBuilder, const FSceneViewFamily& ViewFamily, TArray<FRDGTextureRef> Sources)
{
	...
	else
	{
		...
		FCombineShadingRateTexturesCS::FPermutationDomain PermutationVector;
		PermutationVector.Set<FFragmentDensityMap>(GRHIVariableRateShadingImageDataType == VRSImage_Fractional);

		TShaderMapRef<FCombineShadingRateTexturesCS> ComputeShader(GetGlobalShaderMap(GMaxRHIShaderPlatform), PermutationVector);
...
FRDGTextureRef FVariableRateShadingImageManager::GetForceRateImage(FRDGBuilder& GraphBuilder, const FSceneViewFamily& ViewFamily, int RateIndex /* = 0*/, bool bGetSoftwareImage)
{
	...
	static const TArray<FVector2D> ValidFragmentDensities = { FVector2D(1.0, 1.0), FVector2D(1.0, 0.5), FVector2D(0.5, 1.0), FVector2D(0.5, 0.5), FVector2D(0.5, 0.25), FVector2D(0.25, 0.5), FVector2D(0.25, 0.25) };

	const bool bImageTypeAvailable = bGetSoftwareImage ? bSoftwareVRSEnabledForFrame : bHardwareVRSEnabledForFrame;
	if (!bImageTypeAvailable || (GRHIVariableRateShadingImageDataType == VRSImage_NotSupported))
	{
		return nullptr;
	}
	...
	{
		AddClearUAVPass(GraphBuilder, ForceShadingRateUAV, ValidFragmentDensities[RateIndex]);
	}
	else
	{
		checkNoEntry(); // Invalid GRHIVariableRateShadingImageDataType
		return nullptr;
	}
