...
#define REGISTER_VIZ_TYPE(name, value)	Map.Emplace(FName(name), value); CVarHelpText += TEXT(" - ") name TEXT("\n")
        ...
        REGISTER_VIZ_TYPE(TEXT("LightGridListHash")	   	, PATH_TRACER_DEBUG_VIZ_LIGHT_GRID_LIST_HASH    );
        REGISTER_VIZ_TYPE(TEXT("LightGridSingular")     , PATH_TRACER_DEBUG_VIZ_LIGHT_GRID_SINGULAR     );
        ...
        REGISTER_VIZ_TYPE(TEXT("DecalGridListHash")     , PATH_TRACER_DEBUG_VIZ_DECAL_GRID_LIST_HASH    );
		...
		REGISTER_VIZ_TYPE(TEXT("BackfacingNormal")      , PATH_TRACER_DEBUG_VIZ_BACKFACING_NORMAL       );
		REGISTER_VIZ_TYPE(TEXT("HairPrimitiveUV")		, PATH_TRACER_DEBUG_VIZ_HAIR_PRIMITIVE_UV		);
        REGISTER_VIZ_TYPE(TEXT("TriangleID")            , PATH_TRACER_DEBUG_VIZ_TRIANGLE_ID             );
        REGISTER_VIZ_TYPE(TEXT("InstanceID")            , PATH_TRACER_DEBUG_VIZ_INSTANCE_ID             );
        REGISTER_VIZ_TYPE(TEXT("ClusterID")             , PATH_TRACER_DEBUG_VIZ_CLUSTER_ID              );
        REGISTER_VIZ_TYPE(TEXT("DecalCoverage")         , PATH_TRACER_DEBUG_VIZ_DECAL_COVERAGE          );
        REGISTER_VIZ_TYPE(TEXT("DecalColor")            , PATH_TRACER_DEBUG_VIZ_DECAL_COLOR             );
        REGISTER_VIZ_TYPE(TEXT("DecalNormal")           , PATH_TRACER_DEBUG_VIZ_DECAL_NORMAL            );
        REGISTER_VIZ_TYPE(TEXT("DecalRoughMetalSpec")   , PATH_TRACER_DEBUG_VIZ_DECAL_RMS               );
        REGISTER_VIZ_TYPE(TEXT("DecalRadiance")         , PATH_TRACER_DEBUG_VIZ_DECAL_RADIANCE          );
        REGISTER_VIZ_TYPE(TEXT("DecalResponseMask")     , PATH_TRACER_DEBUG_VIZ_DECAL_RESPONSE_MASK     );
        REGISTER_VIZ_TYPE(TEXT("DecalUseDBuffer")       , PATH_TRACER_DEBUG_VIZ_DECAL_USE_DBUFFER       );
...
TAutoConsoleVariable<float> CVarPathTracingVisualizeAmbientOcclusionMaxDistance(
	TEXT("r.PathTracing.Visualize.AmbientOcclusionMaxDistance"),
	200.0f,
	TEXT("How far to trace ambient occlusion rays for fake shading effect? This only applies to some of the visualization modes. Negative values revert to a simple facing ratio shading."),
	ECVF_RenderThreadSafe
);

TAutoConsoleVariable<int32> CVarPathTracingVisualizeMaxCount(
	TEXT("r.PathTracing.Visualize.MaxCount"),
	16,
	TEXT("Number of lights (or decals) to use for the visualization modes involving turning a count into a color. The color ramp is blue to red, with pink indicating a cell beyond the target count. "),
	ECVF_RenderThreadSafe
);
...
TAutoConsoleVariable<int32> CVarPathTracingCompactionDepth(
	...
	6,
	TEXT("Enables path compaction to improve GPU occupancy for the path tracer. The value sets the bounce up to which compaction will happen, beyond that point the path tracer will handle bounces within the dispatch. Values <= 0 apply compaction to all bounces."),
	ECVF_RenderThreadSafe
);
...
TAutoConsoleVariable<bool> CVarPathTracingApproximateCaustics(
	TEXT("r.PathTracing.ApproximateCaustics"),
	true,
	TEXT("When true, the path tracer will approximate caustic paths to reduce noise. This reduces speckles and noise from low-roughness glass and metals."),
	ECVF_RenderThreadSafe
);
...
TAutoConsoleVariable<bool> CVarPathTracingUseDBuffer(
	...
	true,
	TEXT("When enabled, materials are allowed to read from d-buffer nodes. This carries a cost as the primary ray must be traced again to re-evaluate the material after the decal information has been gathered."),
	ECVF_RenderThreadSafe
);
...
TAutoConsoleVariable<int32> CVarPathTracingHeterogeneousVolumesPipeline(
	TEXT("r.PathTracing.HeterogeneousVolumes.Pipeline"),
	0,
	TEXT("Determines which pipeline to invoke when rendering Heterogeneous Volumes. The first method resamples the volume into a global voxel buffer. The second method isolates each volume, and invokes an individual callable shader to perform rendering options.\n")
	TEXT("0: Global voxel buffers (default)\n")
	TEXT("1: Callable shading\n")
...
BEGIN_SHADER_PARAMETER_STRUCT(FPathTracingData, )
	...
	SHADER_PARAMETER(float, ApproximateCaustics)
	...
	SHADER_PARAMETER(float, FilterSigma)
...
struct FPathTracingConfig
{
	...
	bool IsDifferent(const FPathTracingConfig& Other) const
	{
		...
		return
			...
			PathTracingData.FilterSigma != Other.PathTracingData.FilterSigma ||
...
struct FPathTracingState
{
	...
	TMap<FHeterogeneousVolumesPrimitiveInstanceKey, TRefCountPtr<IPooledRenderTarget> > HeterogeneousVolumesPersistentMajorantRT;
	TArray<FPathTracingHeterogeneousVolumesCallableInfo> HeterogeneousVolumesCallableInfo;
...
static void PreparePathTracingData(const FScene* Scene, const FViewInfo& View, FPathTracingData& PathTracingData)
{
	...
	PathTracingData.ApproximateCaustics = CVarPathTracingApproximateCaustics.GetValueOnRenderThread() ? 1.0f : 0.0f;
	PathTracingData.SamplerType = CVarPathTracingSamplerType.GetValueOnRenderThread();
	PathTracingData.FilterSigma = CVarPathTracingFilterWidth.GetValueOnRenderThread() / 6.0; // user-provided width covers +/-3*Sigma
...
IMPLEMENT_RT_PAYLOAD_TYPE(ERayTracingPayloadType::PathTracingMaterial, 76);
...
class FPathTracingRG : public FGlobalShader
{
	...
	class FKernelType : SHADER_PERMUTATION_INT("PATH_TRACER_KERNEL_TYPE", 3);
	...
	class FHeterogeneousVolumesPipeline : SHADER_PERMUTATION_INT("PATH_TRACER_HETEROGENEOUS_VOLUMES_PIPELINE", 2);
	using FPermutationDomain = TShaderPermutationDomain<FKernelType, FAdaptiveSampling, FCloudShader, FSubstrateComplexSpecialMaterial, FUseSER, FNeedTMinWorkaround, FHeterogeneousVolumesPipeline>;

    static bool IsPermutationExperimental(const FGlobalShaderPermutationParameters& Parameters)
    {
        const FPermutationDomain PermutationVector(Parameters.PermutationId);
        return PermutationVector.Get<FAdaptiveSampling>() ||
               PermutationVector.Get<FHeterogeneousVolumesPipeline>();
    }
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		const FPermutationDomain PermutationVector(Parameters.PermutationId);
		if (bUseExperimental == false && IsPermutationExperimental(Parameters))
		{
            // do not compile experimental features if they are not enabled
            return false;
		}
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWRadianceTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float2>, VarianceTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWAlbedoTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWNormalTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float> , RWSceneDepthTexture)
		...
		// NOTE: Remove in separate submission
		//SHADER_PARAMETER_RDG_UNIFORM_BUFFER(FPreshadingUniformBufferParameters, PreshadingUniformBuffer)

        // Heterogeneous volumes callable interface
		SHADER_PARAMETER(int32, HeterogeneousVolumesNum)
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<int>, HeterogeneousVolumesCallableShaderIdIntersect)
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<int>, HeterogeneousVolumesCallableShaderIdGetDensityBounds)
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<int>, HeterogeneousVolumesCallableShaderIdGetDensity)
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<int>, HeterogeneousVolumesCallableShaderIdSampleDistance)
		SHADER_PARAMETER_RDG_BUFFER_SRV(Buffer<int>, HeterogeneousVolumesCallableShaderIdGetTransmittance)
...
class FPathTracingDebugRG : public FGlobalShader
{
	...
	class FKernelType : SHADER_PERMUTATION_INT("PATH_TRACER_KERNEL_TYPE", 3);
	using FPermutationDomain = TShaderPermutationDomain<FUseSER, FUsePrimaryRays, FIncludeTranslucent, FKernelType>;
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		// If we are one of the debug modes (i.e.: not "PrimaryRays" mode)
		if (!PermutationVector.Get<FUsePrimaryRays>())
		{
			// then only the CAMERA kernel type applies
			if (PermutationVector.Get<FKernelType>() != PATH_TRACER_KERNEL_CAMERA)
			{
				return false;
			}
		}
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWRadianceTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWDiffuseColorTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWSpecularColorTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWNormalTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float>,  RWRoughnessTexture)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float>,  RWLinearDepthTexture)
		SHADER_PARAMETER_RDG_BUFFER_SRV(RaytracingAccelerationStructure, TLAS)
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, ViewUniformBuffer)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D<uint>, RandomSequenceSpaceFillingCurve)
        ...
        SHADER_PARAMETER(float, MaxPathIntensity)
        SHADER_PARAMETER(float, AmbientOcclusionMaxDistance)
        SHADER_PARAMETER(uint32, VisualizeMaxCount)
        SHADER_PARAMETER(uint32, EnabledDirectLightingContributions)   // PATHTRACER_CONTRIBUTION_*
    	SHADER_PARAMETER(uint32, EnabledIndirectLightingContributions) // PATHTRACER_CONTRIBUTION_*
        SHADER_PARAMETER(float, BaseExposure)
...
class FPathTracingCopyDepthPS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D<float>, LinearDepthTexture)
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, ViewUniformBuffer)
...
template <enum ECallableType>
class FPathTracingHeterogeneousVolumesMaterial : public FGlobalShader
//class FPathTracingHeterogeneousVolumesMaterial : public FMaterialShader
{
	//DECLARE_SHADER_TYPE(FPathTracingHeterogeneousVolumesMaterial, Material);
	DECLARE_GLOBAL_SHADER(FPathTracingHeterogeneousVolumesMaterial)
public:
	FPathTracingHeterogeneousVolumesMaterial() = default;

	FPathTracingHeterogeneousVolumesMaterial(const ShaderMetaType::CompiledShaderInitializerType& Initializer)
		//: FMaterialShader(Initializer)
		: FGlobalShader(Initializer)
	{
		PreshadingUniformBuffer.Bind(Initializer.ParameterMap, TEXT("PreshadingUniformBuffer"));
	}
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		return PathTracing::ShouldCompilePathTracingShadersForProject(Parameters.Platform) &&
			ShouldCompileRayTracingCallableShadersForProject(Parameters.Platform)
			//Parameters.MaterialParameters.bIsUsedWithHeterogeneousVolumes &&
			//Parameters.MaterialParameters.MaterialDomain == MD_Volume
			;
	}
	...
	void GetShaderBindings(
		const FViewInfo& View,
		const TUniformBufferRef<FPreshadingUniformBufferParameters>& PreshadingUniformBufferParameters,
		FMeshDrawSingleShaderBindings& ShaderBindings) const
	{
		//FMaterialShader::GetShaderBindings(Scene, FeatureLevel, MaterialRenderProxy, Material, ShaderBindings);

		ShaderBindings.Add(GetUniformBufferParameter<FViewUniformShaderParameters>(), View.ViewUniformBuffer);
		ShaderBindings.Add(GetUniformBufferParameter<FPreshadingUniformBufferParameters>(), PreshadingUniformBufferParameters);
	}

private:
	LAYOUT_FIELD(FShaderUniformBufferParameter, PreshadingUniformBuffer);
};

using FPathTracingHeterogeneousVolumesIntersectCS = FPathTracingHeterogeneousVolumesMaterial<ECallableType::Intersect>;
using FPathTracingHeterogeneousVolumesGetDensityBoundsCS = FPathTracingHeterogeneousVolumesMaterial<ECallableType::GetDensityBounds>;
using FPathTracingHeterogeneousVolumesGetDensityCS = FPathTracingHeterogeneousVolumesMaterial<ECallableType::GetDensity>;
using FPathTracingHeterogeneousVolumesSampleDistanceCS = FPathTracingHeterogeneousVolumesMaterial<ECallableType::SampleDistance>;
using FPathTracingHeterogeneousVolumesGetTransmittanceCS = FPathTracingHeterogeneousVolumesMaterial<ECallableType::GetTransmittance>;
IMPLEMENT_SHADER_TYPE(template<>, FPathTracingHeterogeneousVolumesIntersectCS, TEXT("/Engine/Private/PathTracing/PathTracingHeterogeneousVolumeMaterialShader.usf"), TEXT("PathTracingHeterogeneousVolumesIntersectCS"), SF_RayCallable);
IMPLEMENT_SHADER_TYPE(template<>, FPathTracingHeterogeneousVolumesGetDensityBoundsCS, TEXT("/Engine/Private/PathTracing/PathTracingHeterogeneousVolumeMaterialShader.usf"), TEXT("PathTracingHeterogeneousVolumesGetDensityBoundsCS"), SF_RayCallable);
IMPLEMENT_SHADER_TYPE(template<>, FPathTracingHeterogeneousVolumesGetDensityCS, TEXT("/Engine/Private/PathTracing/PathTracingHeterogeneousVolumeMaterialShader.usf"), TEXT("PathTracingHeterogeneousVolumesGetDensityCS"), SF_RayCallable);
IMPLEMENT_SHADER_TYPE(template<>, FPathTracingHeterogeneousVolumesSampleDistanceCS, TEXT("/Engine/Private/PathTracing/PathTracingHeterogeneousVolumeMaterialShader.usf"), TEXT("PathTracingHeterogeneousVolumesSampleDistanceCS"), SF_RayCallable);
IMPLEMENT_SHADER_TYPE(template<>, FPathTracingHeterogeneousVolumesGetTransmittanceCS, TEXT("/Engine/Private/PathTracing/PathTracingHeterogeneousVolumeMaterialShader.usf"), TEXT("PathTracingHeterogeneousVolumesGetTransmittanceCS"), SF_RayCallable);
...
void PreparePathTracingCloudMaterial(FRDGBuilder& GraphBuilder, FScene* Scene, TArrayView<FViewInfo> Views)
{
	...
	{
		CallableShaderVisualize = GetGlobalShaderMap(Scene->GetShaderPlatform())->GetShader<FPathTracingVolumetricCloudMaterialVisualize>();
...
template <class CallableShaderType>
void BindCallableShader(
	FScene* Scene,
	const FViewInfo& View,
	const FMaterialRenderProxy* MaterialRenderProxy,
	const FMaterial* MaterialResource,
	const FMaterialShaderMap* MaterialShaderMap,
	TUniformBufferRef<FPreshadingUniformBufferParameters> PreshadingUniformBuffer,
	int32& CallableShaderIndex
)
{
	//auto CallableShader = MaterialShaderMap->GetShader<CallableShaderType>();
	auto CallableShader = GetGlobalShaderMap(Scene->GetShaderPlatform())->GetShader<CallableShaderType>();
	if (!CallableShader.IsValid())
	{
		return;
	}

	CallableShaderIndex = Scene->RayTracingSBT.NumCallableShaderSlots++;

	FRayTracingShaderCommand& Command = Scene->RayTracingSBT.CallableCommands.AddDefaulted_GetRef();
	Command.SetShader(CallableShader);
	Command.SlotInScene = CallableShaderIndex;

	// Bind resource to shader
	FMeshDrawSingleShaderBindings SingleShaderBindings = Command.ShaderBindings.GetSingleShaderBindings(SF_RayCallable);
	CallableShader->GetShaderBindings(View, PreshadingUniformBuffer, SingleShaderBindings);
}

void PreparePathTracingHeterogeneousVolumeMaterial(
	FRDGBuilder& GraphBuilder,
	FScene* Scene,
	const FViewInfo& View,
	FVolumetricMeshBatch& VolumetricMeshBatch,
	int32 VolumeInstanceIndex,
	const FPreshadingUniformBufferParameters& PreshadingUniformBufferParameters,
...
)
{
	...
	if (!ShouldRenderHeterogeneousVolumes(Scene))
	{
		return;
	}

	const FPrimitiveSceneProxy* PrimitiveSceneProxy = VolumetricMeshBatch.Proxy;
	if (!PrimitiveSceneProxy->IsHeterogeneousVolume())
	{
		return;
	}

	const FMaterialRenderProxy* MaterialRenderProxy = VolumetricMeshBatch.Mesh->MaterialRenderProxy;
	if (MaterialRenderProxy == nullptr)
	{
		return;
	}

	const FMaterial* MaterialResource = &MaterialRenderProxy->GetMaterialWithFallback(Scene->GetFeatureLevel(), MaterialRenderProxy);
	if (MaterialResource->GetMaterialDomain() != MD_Volume)
	{
		return;
	}

	const IHeterogeneousVolumeInterface* HeterogeneousVolumeInterface = (IHeterogeneousVolumeInterface*)VolumetricMeshBatch.Mesh->Elements[VolumeInstanceIndex].UserData;
	if (HeterogeneousVolumeInterface == nullptr)
	{
		return;
	}

	// Add preshading uniform buffer resource to SBT
	TUniformBufferRef<FPreshadingUniformBufferParameters> PreshadingUniformBuffer = CreateUniformBufferImmediate(PreshadingUniformBufferParameters, EUniformBufferUsage::UniformBuffer_SingleFrame);
	Scene->RayTracingSBT.TransientUniformBuffers.Add(PreshadingUniformBuffer);

	const FMaterialShaderMap* MaterialShaderMap = MaterialResource->GetRenderingThreadShaderMap();
	BindCallableShader<FPathTracingHeterogeneousVolumesIntersectCS>(
		Scene,
		View,
		MaterialRenderProxy,
		MaterialResource,
		...
		CallableInfo.IntersectShaderId
	);

	BindCallableShader<FPathTracingHeterogeneousVolumesGetDensityBoundsCS>(
		Scene,
		View,
		MaterialRenderProxy,
		MaterialResource,
		MaterialShaderMap,
		PreshadingUniformBuffer,
		CallableInfo.GetDensityBoundsShaderId
	);

	BindCallableShader<FPathTracingHeterogeneousVolumesGetDensityCS>(
		Scene,
		View,
		MaterialRenderProxy,
		MaterialResource,
		MaterialShaderMap,
		PreshadingUniformBuffer,
		CallableInfo.GetDensityShaderId
	);

	BindCallableShader<FPathTracingHeterogeneousVolumesSampleDistanceCS>(
		Scene,
		View,
		MaterialRenderProxy,
		MaterialResource,
		MaterialShaderMap,
		PreshadingUniformBuffer,
		CallableInfo.SampleDistanceShaderId
	);

	BindCallableShader<FPathTracingHeterogeneousVolumesGetTransmittanceCS>(
		Scene,
		View,
		MaterialRenderProxy,
		MaterialResource,
		MaterialShaderMap,
		PreshadingUniformBuffer,
		CallableInfo.GetTransmittanceShaderId
	);

	// TODO: check that callable shader id index corresponds to volume id
}

enum class EPathTracingHeterogeneousVolumesPipeline
{
	Megavoxel,
	Callable
};

EPathTracingHeterogeneousVolumesPipeline GetPathTracingHeterogeneousVolumesPipeline()
{
	EPathTracingHeterogeneousVolumesPipeline Pipeline = EPathTracingHeterogeneousVolumesPipeline::Megavoxel;

	const bool bUseExperimental = CVarPathTracingExperimental.GetValueOnRenderThread();
	if (bUseExperimental &&
		CVarPathTracingHeterogeneousVolumesPipeline.GetValueOnRenderThread() == 1)
	{
		Pipeline = EPathTracingHeterogeneousVolumesPipeline::Callable;
	}
	return Pipeline;
}

static TAutoConsoleVariable<int32> CVarHeterogeneousVolumesMajorantFlush(
	TEXT("r.HeterogeneousVolumes.Majorant.Flush"),
	0,
	TEXT("Flushes the majorant cache (Default = 0)"),
	ECVF_RenderThreadSafe
);

void PreparePathTracingHeterogeneousVolumes(
	FRDGBuilder& GraphBuilder,
	FScene* Scene,
...
)
{
	...
	if (GetPathTracingHeterogeneousVolumesPipeline() == EPathTracingHeterogeneousVolumesPipeline::Megavoxel)
	{
		return;
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		// Empty shader ids
		PathTracingState->HeterogeneousVolumesCallableInfo.Empty();

		if (CVarHeterogeneousVolumesMajorantFlush.GetValueOnAnyThread())
		{
			PathTracingState->HeterogeneousVolumesPersistentMajorantRT.Empty();
		}
		...
		if (ShouldRenderHeterogeneousVolumesForView(View))
		{
			...
			{
				FVolumetricMeshBatch VolumetricMeshBatch = View.HeterogeneousVolumesMeshBatches[MeshBatchIndex];
				...
				for (int32 VolumeInstanceIndex = 0; VolumeInstanceIndex < Mesh->Elements.Num(); ++VolumeInstanceIndex)
				{
					const IHeterogeneousVolumeInterface* HeterogeneousVolumeInterface = (IHeterogeneousVolumeInterface*)Mesh->Elements[VolumeInstanceIndex].UserData;
					...
					FHeterogeneousVolumesPrimitiveInstanceKey Key;
					Key.PersistentPrimitiveIndex = PrimitiveSceneProxy->GetPrimitiveSceneInfo()->GetPersistentIndex().Index;
					Key.InstanceIndex = VolumeInstanceIndex;

					TRefCountPtr<IPooledRenderTarget>& PersistentMajorantRT = PathTracingState->HeterogeneousVolumesPersistentMajorantRT.FindOrAdd(Key);
					FPathTracingHeterogeneousVolumesCallableInfo& CallableInfo = PathTracingState->HeterogeneousVolumesCallableInfo.AddDefaulted_GetRef();

					FPreshadingUniformBufferParameters PreshadingUniformBufferParameters;
					BuildPreshadingGrid(GraphBuilder, Scene, View, HeterogeneousVolumeInterface, MaterialRenderProxy,
						PersistentMajorantRT,
						CallableInfo,
						PreshadingUniformBufferParameters);
					PreparePathTracingHeterogeneousVolumeMaterial(
						GraphBuilder,
						Scene,
						View,
						VolumetricMeshBatch,
		...
		// Initialize shader ids if there are no volumes..
		if (PathTracingState->HeterogeneousVolumesCallableInfo.IsEmpty())
		{
			FPathTracingHeterogeneousVolumesCallableInfo& CallableInfo = PathTracingState->HeterogeneousVolumesCallableInfo.AddDefaulted_GetRef();
			CallableInfo.IntersectShaderId = -1;
			CallableInfo.GetDensityBoundsShaderId = -1;
			CallableInfo.GetDensityShaderId = -1;
			CallableInfo.SampleDistanceShaderId = -1;
			CallableInfo.GetTransmittanceShaderId = -1;
		}
	}
}
...
)
{
	...
	int32 MissShaderPipelineIndex = FindRayTracingMissShaderIndex(View.MaterialRayTracingData.PipelineState, Shader.GetRayTracingShader(), false);

	if (MissShaderPipelineIndex == INDEX_NONE)
	{
		MissShaderPipelineIndex = FindRayTracingMissShaderIndex(View.MaterialRayTracingData.PipelineState, GetPathTracingDefaultMissShader(View.ShaderMap), true);
	}
...
)
{
	...
	// run a simple compute shader to sample the cubemap and prep the top level of the mipmap hierarchy
	{
		...
		TShaderMapRef<FPathTracingSkylightPrepareCS> ComputeShader(GetGlobalShaderMap(GShaderPlatformForFeatureLevel[FeatureLevel]));
	...
	{
		//ue-todo: Refactor to use EShaderPlatform directly instead of converting from FeatureLevel
		TShaderMapRef<FPathTracingSkylightMISCompensationCS> ComputeShader(GetGlobalShaderMap(GShaderPlatformForFeatureLevel[FeatureLevel]));
...
RENDERER_API void PrepareLightGrid(FRDGBuilder& GraphBuilder, ERHIFeatureLevel::Type FeatureLevel, FPathTracingLightGrid* LightGridParameters, const FPathTracingLight* Lights, uint32 NumLights, uint32 NumInfiniteLights, FRDGBufferSRV* LightsSRV)
{
	...
	if (NumFiniteLights > 0)
	{
		...
		//ue-todo: Refactor to use EShaderPlatform directly instead of converting from FeatureLevel
		TShaderMapRef<FPathTracingBuildLightGridCS> ComputeShader(GetGlobalShaderMap(GShaderPlatformForFeatureLevel[FeatureLevel]));
		FComputeShaderUtils::AddPass(
			GraphBuilder,
...
)
{
	...
	*SceneVisibleLightCount = 0;
	...
	if (PrepareSkyTexture(GraphBuilder, Scene, View, bEnableSkydome, UseMISCompensation, SkylightParameters))
	{
		...
		DestLight.CameraColorScale = FVector3f(1, 1, 1);
		if ((Scene->SkyLight->bRealTimeCaptureEnabled && (View.SkyAtmosphereUniformShaderParameters == nullptr ||  !IsSkyAtmosphereHoldout(View.CachedViewUniformShaderParameters->EnvironmentComponentsFlags))) || CVarPathTracingVisibleLights.GetValueOnRenderThread() == 2)
		{
			...
			*SceneVisibleLightCount = 1;
			...
			if (Scene->SkyLight->bRealTimeCaptureEnabled)
			{
				...
				DestLight.CameraColorScale /= FVector3f(Scene->SkyLight->GetEffectiveLightColor());
			}
		}
	}
	...
	if (View.Family->EngineShowFlags.DirectionalLights)
	{
		for (const FLightSceneInfoCompact& Light : Scene->Lights)
		{
			...
			DestLight.IESAtlasIndex = INDEX_NONE;
			DestLight.MissShaderIndex = 0;
			DestLight.CameraColorScale = FVector3f(Light.LightSceneInfo->Proxy->GetAtmosphereSunDiskColorScale());
			...
			if (bUseAtmosphere)
			{
				// we want the sky color to come in _without_ the sky transmittance baked in, so need to go back to the proxy
				DestLight.Color = FVector3f(Light.LightSceneInfo->Proxy->GetOuterSpaceIlluminance());
			}
			else
			{
				// this color has the sky transmittance already factored in
				DestLight.Color = FVector3f(LightParameters.Color);
			}

			DestLight.Color *= LightParameters.GetLightExposureScale(View.GetLastEyeAdaptationExposure());
...
static FPathTracingRG::FPermutationDomain GetPathTracingRGPermutation(const FViewInfo& View, const FScene& Scene, const bool bIsCameraRays, const bool bCompactKernel)
{
	const bool bUseExperimental = CVarPathTracingExperimental.GetValueOnRenderThread();
	...
	const bool bHasComplexSpecialRenderPath = Substrate::IsSubstrateEnabled() && UsesSubstrateTileType(View.GetSceneTextures().SubstrateSceneData->UsesTileTypeMask, ESubstrateTileType::EComplexSpecial);
	...
	Out.Set<FPathTracingRG::FKernelType>(bIsCameraRays ? PATH_TRACER_KERNEL_CAMERA : (bCompactKernel ? PATH_TRACER_KERNEL_INNER : PATH_TRACER_KERNEL_TAIL));
	...
	Out.Set<FPathTracingRG::FHeterogeneousVolumesPipeline>(static_cast<int32>(GetPathTracingHeterogeneousVolumesPipeline()));
	return Out;
}

static FPathTracingDebugRG::FPermutationDomain GetPathTracingDebugRGPermutation(const FViewInfo& View, const FScene& Scene, const bool bIsCameraRays, const bool bCompactKernel, const int DebugMode)
{
	const bool bUsePrimaryRays = DebugMode == PATH_TRACER_DEBUG_VIZ_PRIMARY_RAYS;
	const bool bUseSER = GRHIGlobals.SupportsShaderExecutionReordering && CVarPathTracingShaderExecutionReordering.GetValueOnRenderThread();
	FPathTracingDebugRG::FPermutationDomain Out;
	Out.Set<FPathTracingDebugRG::FKernelType>((bIsCameraRays || !bUsePrimaryRays) ? PATH_TRACER_KERNEL_CAMERA : (bCompactKernel ? PATH_TRACER_KERNEL_INNER : PATH_TRACER_KERNEL_TAIL));
	Out.Set<FPathTracingDebugRG::FUseSER>(bUseSER && bUsePrimaryRays);
	Out.Set<FPathTracingDebugRG::FUsePrimaryRays>(bUsePrimaryRays);
	Out.Set<FPathTracingDebugRG::FIncludeTranslucent>(CVarPathTracingVisualizeIncludeTranslucent.GetValueOnRenderThread());
	return Out;
}

template <typename TShaderType>
static void AddRayGenShader(FGlobalShaderMap* ShaderMap, typename TShaderType::FPermutationDomain PermutationVector, TArray<FRHIRayTracingShader*>& OutRayGenShaders)
{
    auto RayGenShader = ShaderMap->GetShader<TShaderType>(PermutationVector);
...
void FDeferredShadingSceneRenderer::PreparePathTracing(const FViewInfo& View, const FScene& Scene, TArray<FRHIRayTracingShader*>& OutRayGenShaders)
{
	...
	{
		FGlobalShaderMap* ShaderMap = GetGlobalShaderMap(ViewFamily.GetShaderPlatform());
		...
		{
			{
				AddRayGenShader<FPathTracingDebugRG>(ShaderMap, GetPathTracingDebugRGPermutation(View, Scene, true, false, DebugMode), OutRayGenShaders);
			}
			if (DebugMode == PATH_TRACER_DEBUG_VIZ_PRIMARY_RAYS)
			{
				if (CompactionDepth != 1)
				{
                    AddRayGenShader<FPathTracingDebugRG>(ShaderMap, GetPathTracingDebugRGPermutation(View, Scene, false, true, DebugMode), OutRayGenShaders);
				}
				if (CompactionDepth > 0)
				{
                    AddRayGenShader<FPathTracingDebugRG>(ShaderMap, GetPathTracingDebugRGPermutation(View, Scene, false, false, DebugMode), OutRayGenShaders);
				}
			}
		}
		else
		{
			...
			{
				// camera ray kernel
                AddRayGenShader<FPathTracingRG>(ShaderMap, GetPathTracingRGPermutation(View, Scene, true, false), OutRayGenShaders);
			}
			if (CompactionDepth != 1)
			{
				// at least one compacted innner bounce
                AddRayGenShader<FPathTracingRG>(ShaderMap, GetPathTracingRGPermutation(View, Scene, false, true), OutRayGenShaders);
			}
			if (CompactionDepth > 0)
			{
				// non-compacted tail handling
                AddRayGenShader<FPathTracingRG>(ShaderMap, GetPathTracingRGPermutation(View, Scene, false, false), OutRayGenShaders);
			}
			{
                ...
                AddRayGenShader<FPathTracingInitExtinctionCoefficientRG>(ShaderMap, PermutationVector, OutRayGenShaders);
			}
		}
	}
}
	...
	ENQUEUE_RENDER_COMMAND(PreparePathTracingRTPSO)([](FRHICommandListImmediate& RHICmdList)
		{
			...
			for (int PermutationId = 0; PermutationId < FPathTracingRG::FPermutationDomain::PermutationCount; PermutationId++)
			{
                ...
                if (FPathTracingRG::IsPermutationExperimental(Parameters))
                {
                    // Permutation is for an experimental feature, don't compile it on startup
                    continue;
                }
			...
			UE_LOGF(LogRenderer, Log, "Requested compilation of Path Tracing RTPSOs (%d permutations).", NumValidPermutations);
		}
	);
}
...
TAutoConsoleVariable<int32> CVarPathTracingVolumeIndex(
	TEXT("r.PathTracing.VolumeIndex"),
	0,
	TEXT("Heterogeneous volume index for rendering"),
	ECVF_RenderThreadSafe
);

BEGIN_SHADER_PARAMETER_STRUCT(FHeterogeneousVolumesCallableShadersParameters, )
	RDG_BUFFER_ACCESS(IntersectShaderIds, ERHIAccess::CopyDest)
	RDG_BUFFER_ACCESS(GetDensityBoundsShaderIds, ERHIAccess::CopyDest)
	RDG_BUFFER_ACCESS(GetDensityShaderIds, ERHIAccess::CopyDest)
	RDG_BUFFER_ACCESS(SampleDistanceShaderIds, ERHIAccess::CopyDest)
	RDG_BUFFER_ACCESS(GetTransmittanceShaderIds, ERHIAccess::CopyDest)
END_SHADER_PARAMETER_STRUCT()
	...
	FPathTracingResources& PathTracingResources)
{
	...
	if (int DebugMode = GetPathTracingVisualizationMode(); DebugMode >= 0)
	{
		...
		FRDGTextureRef LinearDepthTexture = GraphBuilder.CreateTexture(DepthDesc, TEXT("PathTracer.LinearDepth"));
		...
		int32 MaxBounces = bUsePrimaryRays ? CVarPathTracingMaxBounces.GetValueOnRenderThread() : 0;
		if (MaxBounces < 0)
		{
			MaxBounces = View.FinalPostProcessSettings.PathTracingMaxBounces;
		}
		int32 CompactionDepth = bUsePrimaryRays ? CVarPathTracingCompactionDepth.GetValueOnRenderThread() : -1;
		if (CompactionDepth <= 0 || CompactionDepth > MaxBounces)
		{
			CompactionDepth = MaxBounces;
		}

		FRDGBuffer* ActivePaths[2] = {};
		FRDGBuffer* NumPathStates = nullptr;
		FRDGBuffer* PathStateData = nullptr;
		{
			const int32 NumPaths = DispatchResX * DispatchResY;
			...
			NumPathStates = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateIndirectDesc<FRHIDispatchIndirectParameters>(CompactionDepth + 1), TEXT("PathTracer.NumPathStates"));
		...
		{
			AddClearUAVPass(GraphBuilder, GraphBuilder.CreateUAV(NumPathStates, PF_R32_UINT), 0);
			for (int Bounce = 0; Bounce <= CompactionDepth; Bounce++)
			{
				...
				{
					PassParameters->RWRadianceTexture = GraphBuilder.CreateUAV(SceneColorOutputTexture);
					...
					PassParameters->MaxBounces = MaxBounces;
					PassParameters->EnableEmissive = CVarPathTracingEnableEmissive.GetValueOnRenderThread();
					if (PassParameters->EnableEmissive < 0)
					{
						PassParameters->EnableEmissive = View.FinalPostProcessSettings.PathTracingEnableEmissiveMaterials;
					}
                    PassParameters->MaxPathIntensity = CVarPathTracingMaxPathIntensity.GetValueOnRenderThread();
                    if (PassParameters->MaxPathIntensity <= 0)
                    {
                        // cvar clamp disabled, use PPV value instad
                        PassParameters->MaxPathIntensity = View.FinalPostProcessSettings.PathTracingMaxPathIntensity;
                    }
                    PassParameters->AmbientOcclusionMaxDistance = CVarPathTracingVisualizeAmbientOcclusionMaxDistance.GetValueOnRenderThread();
                    PassParameters->VisualizeMaxCount = uint32(FMath::Max(1, CVarPathTracingVisualizeMaxCount.GetValueOnRenderThread()));
                    PassParameters->EnabledDirectLightingContributions = PATHTRACER_CONTRIBUTION_ALL;
                    PassParameters->EnabledIndirectLightingContributions = PATHTRACER_CONTRIBUTION_ALL;
                    PassParameters->BaseExposure = View.PreExposure;
					{
						SetLightParameters(
							GraphBuilder,
							Scene,
					...
					PassParameters->RWDiffuseColorTexture = GraphBuilder.CreateUAV(DiffuseColorTexture);
					PassParameters->RWSpecularColorTexture = GraphBuilder.CreateUAV(SpecularColorTexture);
					PassParameters->RWNormalTexture = GraphBuilder.CreateUAV(NormalTexture);
					PassParameters->RWRoughnessTexture = GraphBuilder.CreateUAV(RoughnessTexture);
					PassParameters->RWLinearDepthTexture = GraphBuilder.CreateUAV(LinearDepthTexture);
                    ...
                    PassParameters->DecalRoughnessCutoff = PathTracing::UsesDecals(ViewFamily) && View.bHasRayTracingDecals ? CVarPathTracingDecalRoughnessCutoff.GetValueOnRenderThread() : -1;
                    PassParameters->MeshDecalRoughnessCutoff = PathTracing::UsesDecals(ViewFamily) && Scene->RayTracingScene.GetNumNativeInstances(ERayTracingSceneLayer::Decals, View.GetRayTracingSceneViewHandle()) > 0 ? CVarPathTracingMeshDecalRoughnessCutoff.GetValueOnRenderThread() : -1;
                    PassParameters->MeshDecalBias = CVarPathTracingMeshDecalBias.GetValueOnRenderThread();
					PassParameters->EnableDBuffer = CVarPathTracingUseDBuffer.GetValueOnRenderThread();
				...
				PassParameters->NumPathStates = GraphBuilder.CreateUAV(NumPathStates, PF_R32_UINT);
				...
				const bool bUseCompaction = Bounce < CompactionDepth || Bounce == MaxBounces;
				FPathTracingDebugRG::FPermutationDomain PermutationVector = GetPathTracingDebugRGPermutation(View, *Scene, Bounce == 0, bUseCompaction, DebugMode);
				TShaderMapRef<FPathTracingDebugRG> RayGenShader(View.ShaderMap, PermutationVector);
				ClearUnusedGraphResources(RayGenShader, PassParameters);
				GraphBuilder.AddPass(
					RDG_EVENT_NAME("Path Tracing Visualize [%d/%d] - Bounce %d%s", SampleIndex + 1, SamplesPerPixel, Bounce, bUseCompaction ? TEXT("") : TEXT("+")),
					PassParameters,
...

		{
			...
			DisplayParameters->LinearDepthTexture = GraphBuilder.CreateSRV(FRDGTextureSRVDesc::Create(LinearDepthTexture));
	...
	else if (!PathTracingState->StartingMediumData.IsValid() || PathTracingState->SampleIndex == 0 || View.bIsOfflineRender)
	{
		...
		auto RayGenShader = GetGlobalShaderMap(View.GetShaderPlatform())->GetShader<FPathTracingInitExtinctionCoefficientRG>(PermutationVector);
	...
	if ((Config.PathTracingData.VolumeFlags & PATH_TRACER_VOLUME_ENABLE_ATMOSPHERE) != 0)
	{
		...
		if (!PathTracingState->AtmosphereOpticalDepthLUT.IsValid() || PathTracingState->LastAtmosphereConfig.IsDifferent(AtmoConfig))
		{
			...
			TShaderMapRef<FPathTracingBuildAtmosphereOpticalDepthLUTCS> ComputeShader(GetGlobalShaderMap(View.GetShaderPlatform()));
			FComputeShaderUtils::AddPass(
				GraphBuilder,
	...
	// NOTE: Remove in separate submission
	TRDGUniformBufferRef<FPreshadingUniformBufferParameters> PreshadingUniformBuffer = nullptr;
	bool bCreateVolumeGrids = false;

	// Upload heterogeneous volume callable interface
	bool bCallableShaderHeterogeneousVolumesPipeline = GetPathTracingHeterogeneousVolumesPipeline() == EPathTracingHeterogeneousVolumesPipeline::Callable;
	int32 ElementCount = bCallableShaderHeterogeneousVolumesPipeline ? FMath::Max(PathTracingState->HeterogeneousVolumesCallableInfo.Num(), 1) : 1;
	FRDGBufferDesc ShaderIdsDesc = FRDGBufferDesc::CreateBufferDesc(sizeof(int32), ElementCount);
	FRDGBufferRef IntersectShaderIdsBuffer = GraphBuilder.CreateBuffer(ShaderIdsDesc, TEXT("IntersectShaderId"));
	FRDGBufferRef GetDensityBoundsShaderIdsBuffer = GraphBuilder.CreateBuffer(ShaderIdsDesc, TEXT("GetDensityBoundsShaderId"));
	FRDGBufferRef GetDensityShaderIdsBuffer = GraphBuilder.CreateBuffer(ShaderIdsDesc, TEXT("GetDensityShaderId"));
	FRDGBufferRef SampleDistanceShaderIdsBuffer = GraphBuilder.CreateBuffer(ShaderIdsDesc, TEXT("SampleDistanceShaderId"));
	FRDGBufferRef GetTransmittanceShaderIdsBuffer = GraphBuilder.CreateBuffer(ShaderIdsDesc, TEXT("GetTransmittanceShaderId"));
	if (bCallableShaderHeterogeneousVolumesPipeline)
	{
		FHeterogeneousVolumesCallableShadersParameters* HeterogeneousVolumesCallableShadersParameters = GraphBuilder.AllocParameters<FHeterogeneousVolumesCallableShadersParameters>();
		{
			HeterogeneousVolumesCallableShadersParameters->IntersectShaderIds = IntersectShaderIdsBuffer;
			HeterogeneousVolumesCallableShadersParameters->GetDensityBoundsShaderIds = GetDensityBoundsShaderIdsBuffer;
			HeterogeneousVolumesCallableShadersParameters->GetDensityShaderIds = GetDensityShaderIdsBuffer;
			HeterogeneousVolumesCallableShadersParameters->SampleDistanceShaderIds = SampleDistanceShaderIdsBuffer;
			HeterogeneousVolumesCallableShadersParameters->GetTransmittanceShaderIds = GetTransmittanceShaderIdsBuffer;
		}

		GraphBuilder.AddPass(
			RDG_EVENT_NAME("UploadShaderIds"),
			HeterogeneousVolumesCallableShadersParameters,
			...
			{
				int32 SizeInBytes = ShaderIdsDesc.NumElements * ShaderIdsDesc.BytesPerElement;

				// Intersect
				{
					// TODO: Convert to SoA style
					TArray<int32> IntersectShaderIds;
					IntersectShaderIds.Reserve(ShaderIdsDesc.NumElements);
					for (uint32 i = 0; i < ShaderIdsDesc.NumElements; ++i)
					{
						IntersectShaderIds.Add(PathTracingState->HeterogeneousVolumesCallableInfo[i].IntersectShaderId);
					}

					void* Dest = RHICmdList.LockBuffer(HeterogeneousVolumesCallableShadersParameters->IntersectShaderIds->GetRHI(), 0, SizeInBytes, RLM_WriteOnly);
					FPlatformMemory::Memcpy(Dest, IntersectShaderIds.GetData(), SizeInBytes);
					RHICmdList.UnlockBuffer(HeterogeneousVolumesCallableShadersParameters->IntersectShaderIds->GetRHI());
				}

				// GetDensityBounds
				{
					// TODO: Convert to SoA style
					TArray<int32> GetDensityBoundsShaderId;
					GetDensityBoundsShaderId.Reserve(ShaderIdsDesc.NumElements);
					for (uint32 i = 0; i < ShaderIdsDesc.NumElements; ++i)
					{
						GetDensityBoundsShaderId.Add(PathTracingState->HeterogeneousVolumesCallableInfo[i].GetDensityBoundsShaderId);
					}

					void* Dest = RHICmdList.LockBuffer(HeterogeneousVolumesCallableShadersParameters->GetDensityBoundsShaderIds->GetRHI(), 0, SizeInBytes, RLM_WriteOnly);
					FPlatformMemory::Memcpy(Dest, GetDensityBoundsShaderId.GetData(), SizeInBytes);
					RHICmdList.UnlockBuffer(HeterogeneousVolumesCallableShadersParameters->GetDensityBoundsShaderIds->GetRHI());
				}

				// GetDensity
				{
					// TODO: Convert to SoA style
					TArray<int32> GetDensityShaderId;
					GetDensityShaderId.Reserve(ShaderIdsDesc.NumElements);
					for (uint32 i = 0; i < ShaderIdsDesc.NumElements; ++i)
					{
						GetDensityShaderId.Add(PathTracingState->HeterogeneousVolumesCallableInfo[i].GetDensityShaderId);
					}

					void* Dest = RHICmdList.LockBuffer(HeterogeneousVolumesCallableShadersParameters->GetDensityShaderIds->GetRHI(), 0, SizeInBytes, RLM_WriteOnly);
					FPlatformMemory::Memcpy(Dest, GetDensityShaderId.GetData(), SizeInBytes);
					RHICmdList.UnlockBuffer(HeterogeneousVolumesCallableShadersParameters->GetDensityShaderIds->GetRHI());
				}

				// SampleDistance
				{
					// TODO: Convert to SoA style
					TArray<int32> SampleDistanceShaderId;
					SampleDistanceShaderId.Reserve(ShaderIdsDesc.NumElements);
					for (uint32 i = 0; i < ShaderIdsDesc.NumElements; ++i)
					{
						SampleDistanceShaderId.Add(PathTracingState->HeterogeneousVolumesCallableInfo[i].SampleDistanceShaderId);
					}

					void* Dest = RHICmdList.LockBuffer(HeterogeneousVolumesCallableShadersParameters->SampleDistanceShaderIds->GetRHI(), 0, SizeInBytes, RLM_WriteOnly);
					FPlatformMemory::Memcpy(Dest, SampleDistanceShaderId.GetData(), SizeInBytes);
					RHICmdList.UnlockBuffer(HeterogeneousVolumesCallableShadersParameters->SampleDistanceShaderIds->GetRHI());
				}

				//  GetTransmittance
				{
					// TODO: Convert to SoA style
					TArray<int32> GetTransmittanceShaderId;
					GetTransmittanceShaderId.Reserve(ShaderIdsDesc.NumElements);
					for (uint32 i = 0; i < ShaderIdsDesc.NumElements; ++i)
					{
						GetTransmittanceShaderId.Add(PathTracingState->HeterogeneousVolumesCallableInfo[i].GetTransmittanceShaderId);
					}

					void* Dest = RHICmdList.LockBuffer(HeterogeneousVolumesCallableShadersParameters->GetTransmittanceShaderIds->GetRHI(), 0, SizeInBytes, RLM_WriteOnly);
					FPlatformMemory::Memcpy(Dest, GetTransmittanceShaderId.GetData(), SizeInBytes);
					RHICmdList.UnlockBuffer(HeterogeneousVolumesCallableShadersParameters->GetTransmittanceShaderIds->GetRHI());
				}
			}
		);
	}
	...
	for (int32 FramePassIndex = 0; FramePassIndex < FramePassCount; FramePassIndex++)
	{
		...
		Config.PathTracingData.SampleBlendFactor = 1.0f / (Config.PathTracingData.Iteration + 1);
		...
		if (bNeedsMoreRays)
		{
			...
			{
				bool bMegavoxelPipeline = GetPathTracingHeterogeneousVolumesPipeline() == EPathTracingHeterogeneousVolumesPipeline::Megavoxel;

				FVoxelGridBuildOptions BuildOptions;
				BuildOptions.bBuildFrustumGrid &= bMegavoxelPipeline;
				BuildOptions.bBuildOrthoGrid &= bMegavoxelPipeline;
			...
			// Preshading interface
			// NOTE: Removing in separate submission
			if (false)
			{
				RDG_EVENT_SCOPE(GraphBuilder, "Heterogeneous Volumes: Preshading Pipeline");
				...
				CollectRenderableHeterogeneousVolumes(
					GraphBuilder,
					Scene,
					View,
					HeterogeneousVolumeInterfaces,
					MaterialRenderProxies
				);

				if (!HeterogeneousVolumeInterfaces.IsEmpty())
				{
					FHeterogeneousVolumesPrimitiveInstanceKey Key;
					Key.PersistentPrimitiveIndex = 0;
					Key.InstanceIndex = 0;
					...
					BuildPreshadingGrid(
						...
						Scene,
						View,
						HeterogeneousVolumeInterfaces[0],
						MaterialRenderProxies[0],
						PersistentMajorantRT,
						PreshadingUniformBuffer
					);
				}
				...
				{
					BuildEmptyPreshadingUniformBuffer(GraphBuilder, PreshadingUniformBuffer);
				}
			}
            ...
            const int TotalPixels = DispatchResX * DispatchResY;
            const int DispatchPixels = DispatchSize * DispatchSize;

            // How many tiles do we need to stay under DispatchPixels per tile?
            const int NumTiles = FMath::DivideAndRoundUp(TotalPixels, DispatchPixels);
            // compute Y tile count first, to prefer horizontal aspect ratio
            const int NumTilesY = FMath::FloorToInt(FMath::Sqrt(float(NumTiles)));
            // compute X tile count by estimating how far above the DispatchPixels total we are with the current number of Y tiles
			const int NumTilesX = FMath::DivideAndRoundUp(FMath::DivideAndRoundUp(DispatchResY, NumTilesY) * DispatchResX, DispatchPixels);
            check(NumTilesX * NumTilesY >= NumTiles);
            const int NumXPixels = FMath::DivideAndRoundUp(DispatchResX, NumTilesX);
            const int NumYPixels = FMath::DivideAndRoundUp(DispatchResY, NumTilesY);
            check(NumXPixels * NumYPixels <= DispatchPixels);
            const int32 NumPaths = FMath::DivideAndRoundUp(NumXPixels * NumYPixels, NumGPUs);
			...
			for (int32 GPUIndex : GPUMask)
			{
                ...
                for (int32 TileYId = 0; TileYId < NumTilesY; TileYId++)
                {
                    for (int32 TileXId = 0; TileXId < NumTilesX; TileXId++)
                    {
                        const int32 TileX = DispatchResX * (TileXId + 0) / NumTilesX;
                        const int32 TileY = DispatchResY * (TileYId + 0) / NumTilesY;
                        const int32 DispatchSizeX = DispatchResX * (TileXId + 1) / NumTilesX - TileX;
                        const int32 DispatchSizeY = DispatchResY * (TileYId + 1) / NumTilesY - TileY;
                        ...
                        check(DispatchSizeX * DispatchSizeYLocal <= NumPaths);

						RDG_EVENT_SCOPE_CONDITIONAL(GraphBuilder, NumTiles > 1, "Tile=(%d,%d - %dx%d)", TileX, TileY, DispatchSizeX, DispatchSizeYLocal);
						...
						if (Config.UseAdaptiveSampling && Config.PathTracingData.Iteration > 0)
						{
							...
							TShaderMapRef<FPathTracingAdaptiveStartCS> ComputeShader(GetGlobalShaderMap(View.GetShaderPlatform()));
						...
						for (int Bounce = 0; Bounce <= CompactionDepth; Bounce++)
						{
							...
							PassParameters->RWRadianceTexture = GraphBuilder.CreateUAV(RadianceTexture);
							PassParameters->RWNormalTexture = GraphBuilder.CreateUAV(NormalTexture);
							PassParameters->RWSceneDepthTexture = GraphBuilder.CreateUAV(DepthTexture);
                            PassParameters->RWAlbedoTexture = GraphBuilder.CreateUAV(AlbedoTexture);
							...
							// Heterogeneous volume callable interface
							if (bCallableShaderHeterogeneousVolumesPipeline)
							{
								PassParameters->HeterogeneousVolumesNum = PathTracingState->HeterogeneousVolumesCallableInfo.Num();
								PassParameters->HeterogeneousVolumesCallableShaderIdIntersect = GraphBuilder.CreateSRV(IntersectShaderIdsBuffer, PF_R32_SINT);
								PassParameters->HeterogeneousVolumesCallableShaderIdGetDensityBounds = GraphBuilder.CreateSRV(GetDensityBoundsShaderIdsBuffer, PF_R32_SINT);
								PassParameters->HeterogeneousVolumesCallableShaderIdGetDensity = GraphBuilder.CreateSRV(GetDensityShaderIdsBuffer, PF_R32_SINT);
								PassParameters->HeterogeneousVolumesCallableShaderIdSampleDistance = GraphBuilder.CreateSRV(SampleDistanceShaderIdsBuffer, PF_R32_SINT);
								PassParameters->HeterogeneousVolumesCallableShaderIdGetTransmittance = GraphBuilder.CreateSRV(GetTransmittanceShaderIdsBuffer, PF_R32_SINT);
							}
							...
							{
								PassParameters->PathTracingIndirectArgs = NumPathStates;
							}
							...
							TShaderMapRef<FPathTracingRG> RayGenShader(View.ShaderMap, GetPathTracingRGPermutation(View, *Scene, Bounce == 0, bUseCompaction));
							ClearUnusedGraphResources(RayGenShader, PassParameters, { PassParameters->PathTracingIndirectArgs });
							...
							GraphBuilder.AddPass(
								RDG_EVENT_NAME("Path Tracer Sample=%d/%d NumLights=%d (Bounce=%d%s%s)", PathTracingState->SampleIndex, MaxSPP, PassParameters->SceneLightCount, PassParameters->FirstBounce, bUseCompaction ? TEXT("") : TEXT("+"), bUseIndirectDispatch && Bounce > 0 ? TEXT(" indirect") : TEXT("")),
								PassParameters,
								ERDGPassFlags::Compute,
								[PassParameters, RayGenShader, DispatchSizeX, DispatchSizeYLocal, bUseIndirectDispatch, bUse1DDispatch, GPUIndex, &View](FRDGAsyncTask, FRHICommandList& RHICmdList)
								{
									FRHIBatchedShaderParameters& GlobalResources = RHICmdList.GetScratchShaderParameters();
									SetShaderParameters(GlobalResources, RayGenShader, *PassParameters);
									...
									if (bUseIndirectDispatch && PassParameters->FirstBounce > 0)
									{
										...
										RHICmdList.RayTraceDispatchIndirect(
											...
											RayGenShader.GetRayTracingShader(),
											View.MaterialRayTracingData.ShaderBindingTable, GlobalResources,
											PassParameters->PathTracingIndirectArgs->GetIndirectRHICallBuffer(), PassParameters->FirstBounce * sizeof(FRHIDispatchIndirectParameters)
										);
									}
	...
	if (NumGPUs > 1)
	{
		...
		TShaderMapRef<FPathTracingSwizzleScanlinesCS> ComputeShader(GetGlobalShaderMap(View.GetShaderPlatform()));
	...
	if (Config.UseAdaptiveSampling && bNeedsMoreRays)
	{
		...
		TShaderMapRef<FPathTracingBuildAdaptiveErrorTextureCS> ComputeShader(GetGlobalShaderMap(View.GetShaderPlatform()));
