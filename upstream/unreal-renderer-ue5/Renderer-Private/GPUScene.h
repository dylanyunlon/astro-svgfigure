...
//Non-RDG parameters, untracked, use with care (or preferably, don't)
...
public:
	 ...
	 */
	void Update(FRDGBuilder& GraphBuilder, FRDGExternalAccessQueue& ExternalAccessQueue, const FUpdateFromComputeCommands& UpdatesFromCompute, const UE::Tasks::FTask& UpdateTaskPrerequisites, FSceneExtensionsUpdaters& SceneExtensionsUpdaters);
	 ...
	 */
	RENDERER_API FGPUSceneResourceParameters GetShaderParameters(FRDGBuilder& GraphBuilder) const;
	 ...
	 */
	RENDERER_API TRDGUniformBufferRef<FGPUSceneWriterUniformParameters> GetWriteParameters(FRDGBuilder& GraphBuilder, FGPUSceneWriterParameters* LegacyParameters = nullptr, bool bSkipBarriers = true);
...
private:
	...
	// One-way latch that allows allocating GPU Scene as a reserved resource (when it is large enough)
	bool bAllowReservedResources = false;
	...
	int32 PrevDynamicPrimitivesMaxSize = 0;
	int32 PrevDynamicPrimitivePayloadMaxSize = 0;
	int32 PrevDynamicPrimitiveInstanceMaxSize = 0;
	...
	FRegisteredBuffers UpdateBufferAllocations(FRDGBuilder& GraphBuilder);
	...
	void UpdateInternal(FRDGBuilder& GraphBuilder, FRDGExternalAccessQueue& ExternalAccessQueue, const UE::Tasks::FTask& UpdateTaskPrerequisites, const FUpdateFromComputeCommands& UpdatesFromCompute, FSceneExtensionsUpdaters& SceneExtensionsUpdaters);
	...
	bool bLoggedPayloadBufferOverflow = false;
#endif // UE_BUILD_SHIPPING
	uint64 InstancePayloadDataSizeReserveUnclamped = 0;
	// Used to track the GetCachedScalabilityCVars().ViewDistanceScale such that we can trigger a full upload if it changes, this is required as the value is baked into some values such as in FPrimitiveUniformShaderParametersBuilder::InstanceWorldPositionOffsetDisableDistance
	float TrackedViewDistanceScle = -1.0f;
};
...
{
	static constexpr uint32 DataStrideInFloat4s = BATCHED_PRIMITIVE_DATA_STRIDE_FLOAT4;
...
// Per-instance record for the mobile UBO path, paired with FBatchedPrimitiveShaderData.
// PrimitiveSlotIndex links each instance record back to its primitive record in the parallel FBatchedPrimitiveShaderData array
struct FBatchedInstanceShaderData
{
	static constexpr uint32 DataStrideInFloat4s = BATCHED_INSTANCE_DATA_STRIDE_FLOAT4;

	TStaticArray<FVector4f, DataStrideInFloat4s> Data;

	FBatchedInstanceShaderData()
		: Data(InPlace, NoInit)
	{
		Setup(GetIdentityPrimitiveParameters(), 0u);
	}

	FBatchedInstanceShaderData(const FPrimitiveUniformShaderParameters& PrimitiveUniformShaderParameters, uint32 PrimitiveSlotIndex)
		: Data(InPlace, NoInit)
	{
		Setup(PrimitiveUniformShaderParameters, PrimitiveSlotIndex);
	}

	FBatchedInstanceShaderData(const FPrimitiveSceneProxy* Proxy, uint32 PrimitiveSlotIndex);

	static void Emplace(FBatchedInstanceShaderData* Dest, const FPrimitiveUniformShaderParameters& ShaderParams, uint32 PrimitiveSlotIndex);
	static void Emplace(FBatchedInstanceShaderData* Dest, const FPrimitiveSceneProxy* Proxy, uint32 PrimitiveSlotIndex);

private:
	void Setup(const FPrimitiveUniformShaderParameters& PrimitiveUniformShaderParameters, uint32 PrimitiveSlotIndex);
};
static_assert(sizeof(FBatchedInstanceShaderData) == BATCHED_INSTANCE_DATA_STRIDE_FLOAT4 * sizeof(FVector4f),
	"FBatchedInstanceShaderData must be exactly BATCHED_INSTANCE_DATA_STRIDE_FLOAT4 float4");
...
struct FInstanceUploadInfo
{
	...
	void SetPrimitiveData(FPrimitiveSceneProxy* InPrimitiveSceneProxy, const FPersistentPrimitiveIndex& PrimitiveIndex, uint32 SceneFrameNumber);
	void SetInstancePayloadData(const FInstanceSceneDataBuffers* InInstanceSceneDataBuffers);
	void WritePayloadData(TArrayView<FVector4f> InstancePayloadData, int32 InstanceIndex);
	uint32 CalculatePayloadStrideFloat4() const;
};
