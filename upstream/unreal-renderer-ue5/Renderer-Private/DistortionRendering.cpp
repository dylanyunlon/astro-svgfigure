...
DEFINE_GPU_DRAWCALL_STAT(Distortion);
...
static TAutoConsoleVariable<int32> CVarRefractionBlur(
	...
	TEXT("Enables rough refractions, i.e. blurring of the background. Only enabled when Substrate is enabled."),
...
static TAutoConsoleVariable<float> CVarRefractionBlurDitheringScaleFactor(
	TEXT("r.Refraction.Blur.DitheringScaleFactor"),
	0.3f,
	TEXT("Represents the radius scale factor of the dithered noise applied on the rough refraction UV sample. A value of 1 means that the UV offset will be rotated on circle of radius = pixel standard deviation. A large value can confuse TAA/TSR and result on color splotches. Disabled when 0, accept [0,1] range."),
	ECVF_Scalability | ECVF_RenderThreadSafe);
...
void SetupDistortionParams(FVector4f& DistortionParams, const FViewInfo& View)
{
	...
	DistortionParams.X = View.ViewMatrices.GetViewToClip().M[0][0];
	...
	// When ISR or MMV is enabled we store two FOVs in the distortion parameters and compute the aspect ratio in the shader instead.
	if (View.bIsSinglePassStereo)
	{
		const FSceneView* InstancedView = View.GetInstancedView();
		if (InstancedView)
		{
			DistortionParams.Y = InstancedView->ViewMatrices.GetViewToClip().M[0][0];
		}
	}
}
...
{
	return Substrate::IsSubstrateEnabled() && !CVarDisableDistortion.GetValueOnAnyThread() && CVarRefractionBlur.GetValueOnAnyThread() > 0; // Any thread since it can be called when creating the PassProcessor
}
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, BlurDitheringScaleFactor)
...
static void AddCopySceneColorDepthPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureRef SceneColorTexture, FRDGTextureRef SceneDepthTexture, FRDGTextureRef SceneColorCopyTexture, FRDGTextureRef SceneDepthCopyTexture)
{
	...
	PassParameters->SceneDepthSampler = TStaticSamplerState<SF_Bilinear>::GetRHI();
	...
	AddDrawScreenPass(GraphBuilder, RDG_EVENT_NAME("Distortion::CopySceneColorDepth"), View, InputViewport, OutputViewport, VertexShader, PixelShader, PassParameters);
}
	...
	FTranslucencyPassResourcesMap& TranslucencyResourceMap)
{
	...
	const bool bUseRoughRefraction = GetSubstrateEnabledUseRoughRefraction();
	if(bUseRoughRefraction)
	{
		...
		for (int32 ViewIndex = 0, Num = Views.Num(); ViewIndex < Num; ++ViewIndex)
		{
...

			{
				...
				{
					const FVector2D InverseFilterTextureExtent(1.0f / static_cast<float>(DstMipResolution.X), 1.0f / static_cast<float>(DstMipResolution.Y));
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			if (auto* Pass = View.ParallelMeshDrawCommandPasses[EMeshPass::Distortion])
			{
				...
				GraphBuilder.AddPass(
					RDG_EVENT_NAME("Distortion::Accumulate View%d", ViewIndex),
					PassParameters,
	...
	if (bUseRoughRefraction)
	{
		...
		CommonParameters.BlueNoise = CreateUniformBufferImmediate(BlueNoise, EUniformBufferUsage::UniformBuffer_SingleDraw);
	}
	...
	CommonParameters.BlurDitheringScaleFactor = FMath::Clamp(CVarRefractionBlurDitheringScaleFactor.GetValueOnRenderThread(), 0.0f, 1.0f);
	...
	// Apply distortion and store off-screen.
	{
		...
		for (int32 ViewIndex = 0, Num = Views.Num(); ViewIndex < Num; ++ViewIndex)
		{
			...
			*PassParameters = CommonParameters;
			...
			AddDrawScreenPass(GraphBuilder, RDG_EVENT_NAME("Distortion::Apply View%d", ViewIndex), FScreenPassViewInfo(View, 1), Viewport, Viewport, PipelineState, PassParameters,
	...
	// Merge distortion back to scene color.
	{
		...
		for (int32 ViewIndex = 0, Num = Views.Num(); ViewIndex < Num; ++ViewIndex)
		{
			...
			*PassParameters = CommonParameters;
			...
			AddDrawScreenPass(GraphBuilder, RDG_EVENT_NAME("Distortion::Merge View%d", ViewIndex), FScreenPassViewInfo(View, 1), Viewport, Viewport, PipelineState, PassParameters,
...
{
	return (GetGlobalRefractionQuality() > 0
...
void FDistortionMeshProcessor::CollectPSOInitializers(const FSceneTexturesConfig& SceneTexturesConfig, const FMaterial& Material, const FPSOPrecacheVertexFactoryData& VertexFactoryData, const FPSOPrecacheParams& PreCacheParams, TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	if (GetSubstrateEnabledUseRoughRefraction())
	{
...
FMeshPassProcessor* CreateMobileDistortionPassProcessor(ERHIFeatureLevel::Type FeatureLevel, const FScene* Scene, const FSceneView* InViewIfDynamicMeshCommand, FMeshPassDrawListContext* InDrawListContext)
{
	...
	{
		return CreateDistortionPassProcessor(FeatureLevel, Scene, InViewIfDynamicMeshCommand, InDrawListContext);
	}
	else
	{
		FMeshPassProcessorRenderState DistortionPassState;
