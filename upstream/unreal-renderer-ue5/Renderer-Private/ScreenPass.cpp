...
// static
FScreenPassTexture FScreenPassTexture::CopyFromSlice(FRDGBuilder& GraphBuilder, const FScreenPassTextureSlice& ScreenTextureSlice, FScreenPassTexture OverrideOutput, uint32 MultiViewCount /*= 0*/)
{
	...
	if (!OutputTexture)
	{
		...
		if (MultiViewCount > 0)
		{
			check(Desc.Dimension == ETextureDimension::Texture2DArray);
			check(Desc.ArraySize >= MultiViewCount);

			Desc.ArraySize = MultiViewCount;
		}
		else
		{
	...
	if (MultiViewCount > 0)
	{
		CopyInfo.NumSlices = MultiViewCount;
	}
...
FScreenPassRenderTarget FScreenPassRenderTarget::CreateViewFamilyOutput(FRDGTextureRef ViewFamilyTexture, const FViewInfo& View)
{
	...
	if (!View.IsFirstInFamily() || View.Family->bAdditionalViewFamily || View.IsRuntimeReflectionCapture())
	{
...
FScreenPassViewInfo::FScreenPassViewInfo(const FSceneView& View)
	: FeatureLevel(View.FeatureLevel)
	, ShaderPlatform(View.GetShaderPlatform())
...
FScreenPassViewInfo::FScreenPassViewInfo(const FSceneView& View, const uint32 OverrideInstanceCount)
	: FeatureLevel(View.FeatureLevel)
	, ShaderPlatform(View.GetShaderPlatform())
	, StereoViewIndex(View.StereoViewIndex)
	...
	FRHISamplerState* Sampler)
{
	...
	TShaderMapRef<FCopyRectPS> PixelShader(GetGlobalShaderMap(ViewInfo.ShaderPlatform));
	...
	Parameters->InputSampler = Sampler ? Sampler : TStaticSamplerState<>::GetRHI();
	...
	FScreenPassRenderTarget Output)
{
	...
	TShaderMapRef<FCopyRectSrvPS> PixelShader(GetGlobalShaderMap(ViewInfo.ShaderPlatform));
...
FCanvas& CreateCanvasForRenderTarget(FRDGBuilder& GraphBuilder, const FSceneView& View, const FScreenPassRenderTarget& RenderTarget)
{
	const FSceneViewFamily& ViewFamily = *View.Family;
	FCanvas& Canvas = *FCanvas::Create(GraphBuilder, RenderTarget.Texture, nullptr, ViewFamily.Time, View.GetFeatureLevel(), ViewFamily.DebugDPIScale);
	Canvas.SetRenderTargetRect(RenderTarget.ViewRect);
	return Canvas;
}
