...
#include "Skinning/AnimRuntimeTransformProvider.h"
#include "Skinning/AnimSequenceTransformProvider.h"
#include "Skinning/SkinningTransformProvider.h"
#include "PathTracing.h"
#include "SceneViewState.h"
#include "RayTracing/RaytracingOptions.h"
#include "RenderTargetPool.h"
#include "ScenePrimitiveDataRenderer.h"
...
static TAutoConsoleVariable<int32> CVarVisibilityLocalLightPrimitiveInteraction(
	...
	TEXT("0 - disabled\n")
	TEXT("1 - enabled\n")
	TEXT("2 - enabled if MegaLights is disabled"),
	ECVF_RenderThreadSafe
);
...
static TAutoConsoleVariable<int32> CVarLumenSceneSurfaceCacheAllowPersistOnVisibilityChange(
	...
	TEXT("Only supported through FScene::UpdatePrimitivesIsDrawn_RenderThread currently."),
	ECVF_RenderThreadSafe
);
...
void FSceneViewState::ResetVolumetricFogState()
{
	LightScatteringHistoryPreExposure = 1.0f;
	PrevLightScatteringViewGridUVToViewRectVolumeUV = FVector2f::One();
	VolumetricFogPrevViewGridRectUVToResourceUV = FVector2f::One();
	VolumetricFogPrevUVMax = FVector2f::One();
	VolumetricFogPrevUVMaxForTemporalBlend = FVector2f::One();
	VolumetricFogPrevResourceGridSize = FIntVector(1, 1, 1);
	LightScatteringHistory = nullptr;
	PrevLightScatteringConservativeDepthTexture = nullptr;
}
...
FSceneViewState::~FSceneViewState()
{
	...
	if (ExposureCompensationCurveLUT.Curve_GameThread.IsValid()
		&& ExposureCompensationCurveLUT.CurveUpdateDelegateHandle.IsValid())
	{
...
uint32 FScene::GetShareOriginViewKey(const FSceneViewState& ViewState)
{
	return ViewState.GetShareOriginViewKey();
}
...
void FScene::CheckPrimitiveArrays(int MaxTypeOffsetIndex)
{
	...
	for (int32 i = 0; i < MaxTypeOffsetIndex; i++)
	{
		...
		for (uint32 Index = NextOffset; Index < Entry.Offset; Index++)
		{
			...
			checkfSlow(PrimitiveSceneProxyType == Entry.PrimitiveSceneProxyType, TEXT("TypeHash: %zu not matching, expected: %zu"), PrimitiveSceneProxyType.ProxyTypeHash, Entry.PrimitiveSceneProxyType.ProxyTypeHash);
		}
...
void FScene::DumpMeshDrawCommandMemoryStats()
{
	...
	for (int32 i = 0; i < EMeshPass::Num; i++)
	{
		...
		{
			UE_LOGF(LogRenderer, Log, "%ls: %.1fKb for %u CachedMeshDrawCommands", GetMeshPassName((EMeshPass::Type)i), PassStats.CachedMeshDrawCommandBytes / 1024.0f, PassStats.NumCommands);

			if (PassStats.CachedMeshDrawCommandBytes > 1024 && i <= EMeshPass::BasePass)
			{
				UE_LOGF(LogRenderer, Log, "     avg %.1f bytes PSO", PassStats.PSOBytes / (float)PassStats.NumCommands);
				UE_LOGF(LogRenderer, Log, "     avg %.1f bytes ShaderBindingInline", PassStats.ShaderBindingInlineBytes / (float)PassStats.NumCommands);
				UE_LOGF(LogRenderer, Log, "     avg %.1f bytes ShaderBindingHeap", PassStats.ShaderBindingHeapBytes / (float)PassStats.NumCommands);
				UE_LOGF(LogRenderer, Log, "     avg %.1f bytes VertexStreamsInline", PassStats.VertexStreamsInlineBytes / (float)PassStats.NumCommands);
				UE_LOGF(LogRenderer, Log, "     avg %.1f bytes DebugData", PassStats.DebugDataBytes / (float)PassStats.NumCommands);
				UE_LOGF(LogRenderer, Log, "     avg %.1f bytes DrawCommandParameters", PassStats.DrawCommandParameterBytes / (float)PassStats.NumCommands);
				...
				UE_LOGF(LogRenderer, Log, "     avg %.1f bytes Other", Other / (float)PassStats.NumCommands);
			}
		}
	}

	UE_LOGF(LogRenderer, Log, "sizeof(FMeshDrawCommand) %zu", sizeof(FMeshDrawCommand));
	UE_LOGF(LogRenderer, Log, "Total cached MeshDrawCommands %.3fMb", TotalCachedMeshDrawCommands / 1024.0f / 1024.0f);
	UE_LOGF(LogRenderer, Log, "Primitive StaticMeshCommandInfos %.1fKb", TotalStaticMeshCommandInfos / 1024.0f);
	UE_LOGF(LogRenderer, Log, "GPUScene CPU structures %.1fKb", GPUScene.GetAllocatedSize() / 1024.0f);
	UE_LOGF(LogRenderer, Log, "PSO persistent Id table %.1fKb %d elements", FGraphicsMinimalPipelineStateId::GetPersistentIdTableSize() / 1024.0f, FGraphicsMinimalPipelineStateId::GetPersistentIdNum());
	UE_LOGF(LogRenderer, Log, "PSO one frame Id %.1fKb", FGraphicsMinimalPipelineStateId::GetLocalPipelineIdTableSize() / 1024.0f);
}
...
,	World(InWorld)
,	FXSystem(nullptr)
,	PrimitiveDataImpl(MakePimpl<FScenePrimitiveDataRenderer>())
,	PrimitiveData(*PrimitiveDataImpl.Get())
...
{
	// Populate the FSceneInterface exposed FScenePrimitiveDataEngine pointer to allow access outside the Renderer 
	InitializeScenePrimitiveDataEngineImpl(&PrimitiveData);
	...
	checkf(ShaderPlatform != SP_NumPlatforms, TEXT("Invalid feature level %s for platform (max feature level %s)"), *LexToString(FeatureLevel), *LexToString(GMaxRHIFeatureLevel));

	for (int32 NanitePass = 0; NanitePass < ENaniteMeshPass::Num; ++NanitePass)
	{
		const bool bBasePass = (ENaniteMeshPass::Type)NanitePass == ENaniteMeshPass::BasePass;	// Only basepass for now
		NaniteShadingPipelines[NanitePass].Init(UseNaniteBindlessShading(ShaderPlatform) && bBasePass);
	}
	...
	if (World->FXSystem)
	{
		FFXSystemInterface::MarkPendingKill(World->FXSystem.Get());
	}
	...
	DefaultLumenSceneData = new FLumenSceneData(GShaderPlatformForFeatureLevel[InFeatureLevel], World->WorldType, World->AllowLumenPrimitiveTrackingInPreviewWorld());
...
FScene::~FScene()
{
	...
	for (FThreadSafeObjectIterator Iter(UActorComponent::StaticClass()); Iter; ++Iter)
	{
		...
		{
			UE_LOGF(LogRenderer, Log, "%ls's scene is going to get invalidated", *ActorComp->GetName());
		}
	}
#endif
...
void FScene::BatchAddPrimitivesInternal(TArrayView<T*> InPrimitives)
{
	...
	{
		if (UObject* PrimitiveObject = ToUObject(InPrimitives.Last()))
		{
			GetRendererModule().DebugLogRenderProxies(PrimitiveObject->GetClass()->GetFName(), InPrimitives.Num());
		}
	}
#endif
	...
	LLM_SCOPE_DYNAMIC_STAT_OBJECTPATH_FNAME(ToUObject(InPrimitives[0]) ? FName(UPackage::StaticClass()->GetPathName()) : NAME_None, ELLMTagSet::AssetClasses);
	UE_TRACE_METADATA_SCOPE_ASSET_FNAME(NAME_None, ToUObject(InPrimitives[0]) ? FName(UPackage::StaticClass()->GetPathName()) : NAME_None,
		ToUObject(InPrimitives[0]) ? InPrimitives[0]->GetOutermost()->GetFName() : NAME_None);
	...
	for (T* Primitive : InPrimitives)
	{
		...
		if (Primitive->GetPrimitiveComponentInterface() && !Primitive->GetSceneProxy())
		{
		...
		FSimpleStreamableAssetManager::Register(Primitive->GetPrimitiveComponentInterface(), PrimitiveSceneProxy, RenderMatrix, WorldBounds);
...
static int32 GSkipRedundantTransformUpdate = 0;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
					...
					UpdateParams.AttachmentRootPosition))
				{
					...
					else
					{
						...
						UE_LOGF(LogRenderer, Warning,
							"Redundant UpdatePrimitiveTransform for Primitive %ls: Owner: %ls, Resource: %ls, Level: %ls",
							*Primitive->GetName(),
							*Primitive->GetSceneProxy()->GetOwnerName().ToString(),
							*Primitive->GetSceneProxy()->GetResourceName().ToString(),
							*Primitive->GetSceneProxy()->GetLevelName().ToString()
				...
				if (bNeedsTransformCommand)
				{
					ENQUEUE_RENDER_COMMAND(UpdateTransformCommand)(
						[UpdateParams](FRHICommandListBase&)
						{
							...
							FSimpleStreamableAssetManager::UpdateStreamingState(UpdateParams.PrimitiveSceneProxy, UpdateParams.LocalToWorld, UpdateParams.WorldBounds);
						}
					);
				}
			}
...
void FScene::UpdatePrimitiveTransforms(IPrimitiveTransformUpdater* InUpdater)
{
	if (auto* Updater = static_cast<FPrimitiveTransformUpdater*>(InUpdater))
	{
		...
		{
			PHYS_SYNC_SCOPE("PhysSync_RT_ConsumeTransforms");
			PHYS_SYNC_BOOKMARK(TEXT("PhysSync RT ConsumeTransforms F:%" UINT64_FMT), (uint64)GFrameNumberRenderThread);
...
void FScene::UpdatePrimitivesIsDrawn_RenderThread(TArrayView<FPrimitiveSceneProxy*> InPrimitiveSceneProxies, bool bInIsDrawn)
{
	check(IsInRenderingThread());

	for (FPrimitiveSceneProxy* PrimitiveSceneProxy : InPrimitiveSceneProxies)
	{
		if ((PrimitiveSceneProxy->IsDrawnInGame() != PrimitiveSceneProxy->IsDrawnInEditor()) || (PrimitiveSceneProxy->IsDrawnInGame() != bInIsDrawn))
		{
			FPrimitiveSceneInfo* PrimitiveSceneInfo = PrimitiveSceneProxy->GetPrimitiveSceneInfo();
			...
			if (!bInIsDrawn && bPrimitiveIndexValid)
			{
			...
			PrimitiveSceneProxy->SetIsDrawn_RenderThread(bInIsDrawn);
...
#if RHI_RAYTRACING
			if (bPrimitiveIndexValid)
			{
				UpdateCachedRayTracingState(PrimitiveSceneProxy, EPrimitiveDirtyRayTracingState::Default | EPrimitiveDirtyRayTracingState::DrawInGame);
			}
#endif
		}
	}
...
{
	FPrimitiveSceneInfo* const* Found = PrimitiveComponentIdToInfoMap.Find(PrimitiveId);
...
void FScene::BatchRemovePrimitivesInternal(TArrayView<T*> InPrimitives)
{
	...
	for (T* Primitive : InPrimitives)
	{
		...
		if (PrimitiveSceneProxy)
		{
			FSimpleStreamableAssetManager::Unregister(Primitive->GetPrimitiveComponentInterface(), PrimitiveSceneProxy);
...
void FScene::BatchRemovePrimitives(TArray<FPrimitiveSceneProxy*>&& InPrimitives)
{
	if (!InPrimitives.IsEmpty())
	{
			...
			[this, InPrimitives = MoveTemp(InPrimitives)](FRHICommandListBase&)
		{
			...
			{
				// Proxy-only removal: no component to touch, so Unregister's component-field write
				// is skipped via the nullptr arg (which also lets the GT check short-circuit
				// -- we're on RT here inside the render command).
				FSimpleStreamableAssetManager::Unregister(nullptr, PrimitiveSceneProxy);
...
void FScene::AddLightSceneInfo_RenderThread(FLightSceneInfo* LightSceneInfo)
{
		...
		if(GetFeatureLevelShadingPath(FeatureLevel) == EShadingPath::Mobile)
		{
		    ...
		    if (FirstLightingChannel >= 0 && MobileDirectionalLights[FirstLightingChannel] == nullptr)
		    {
			    ...
			    if (bUsesDirectionalLightForLighmapPolicySelection && (!LightSceneInfo->Proxy->HasStaticShadowing() || bUseCSMForDynamicObjects))
				{
					...
					UE_CLOGF(!GIsEditor, LogRenderer, Log, "Forcing update for all mesh draw commands: Add directional light");
				}
		    }
		}
	}
	...
	if (LightType == LightType_Rect)
	{
		...
		RectProxy->RectAtlasId = RectLightAtlas::AddTexture(ShaderPlatform, RectProxy->SourceTexture, RectProxy->SourceTextureScaleOffset);
	}
...
void FScene::AddLight(ULightComponent* Light)
{
	...
	if (!Light->SceneProxy)
	{
		...
		Light->SceneProxy = Light->CreateSceneProxy();
	}

	FLightSceneDesc LightSceneDesc{ .Transform = Light->GetComponentTransform(), .SceneProxy = Light->SceneProxy };
	BatchAddLights(MakeArrayView(&LightSceneDesc, 1));
}

void FScene::BatchAddLights(TArrayView<FLightSceneDesc> InLights)
{
	LLM_SCOPE(ELLMTag::SceneRender);

	TArray<FLightSceneInfo*, SceneRenderingAllocator> LightsToAdd;
	LightsToAdd.Reserve(InLights.Num());

	for (const FLightSceneDesc& LightDesc : InLights)
	{
		if (FLightSceneProxy* Proxy = LightDesc.GetSceneProxy())
		{
			// Update the light's transform and position.
			Proxy->SetTransform(LightDesc.GetTransform().ToMatrixNoScale(), LightDesc.GetPosition());
			...
			// Adding a new light
			++NumVisibleLights_AnyThread;

			LightsToAdd.Emplace(Proxy->LightSceneInfo);
		}
	}

	if (!LightsToAdd.IsEmpty())
	{
		// Send a command to the rendering thread to add the lights to the scene.
		ENQUEUE_RENDER_COMMAND(BatchAddLightsCommand)(
			[this, LightsToAdd = MoveTemp(LightsToAdd)](FRHICommandListBase& RHICmdList)
			{
				CSV_SCOPED_TIMING_STAT_EXCLUSIVE(Scene_AddLight);
				for (FLightSceneInfo* LightSceneInfo : LightsToAdd)
				{
					FScopeCycleCounter Context(LightSceneInfo->Proxy->GetStatId());
					// It should not have been added to the scene already
					check(!LightSceneInfo->Id.IsValid());
...
void FScene::AddInvisibleLight(ULightComponent* Light)
{
	...
	FLightSceneDesc LightSceneDesc{ .Transform = Light->GetComponentTransform(), .SceneProxy = Light->SceneProxy };
	BatchAddInvisibleLights(MakeArrayView(&LightSceneDesc, 1));
}

void FScene::BatchAddInvisibleLights(TArrayView<FLightSceneDesc> InLights)
{
	TArray<FLightSceneInfo*, SceneRenderingAllocator> LightsToAdd;
	LightsToAdd.Reserve(InLights.Num());
	...
	{
		// Send a command to the rendering thread to add the lights to the scene.
		ENQUEUE_RENDER_COMMAND(BatchAddInvisibleLightsCommand)(
			[this, LightsToAdd = MoveTemp(LightsToAdd)](FRHICommandListBase& RHICmdList)
			{
				for (FLightSceneInfo* LightSceneInfo : LightsToAdd)
				{
					FScopeCycleCounter Context(LightSceneInfo->Proxy->GetStatId());
					LightSceneInfo->Id = InvisibleLights.Add(FLightSceneInfoCompact(LightSceneInfo));
				}
			});
	}
}
...
void FScene::SetSkyLight(FSkyLightSceneProxy* LightProxy)
{
	...
	ENQUEUE_RENDER_COMMAND(FSetSkyLightCommand)(
		[Scene, LightProxy] (FRHICommandListBase&)
		{
			...
			Scene->ReflectionSceneData.bSkylightHasChanged = true;
...
{
	// Set the shader print/debug values if
...
{
	if (PhysicsField && PhysicsField->FieldResource)
	{
...
void FScene::AddHairStrands(FHairStrandsInstance* Proxy)
{
	if (Proxy)
	{
		const int32 PackedIndex = HairStrandsSceneData.RegisteredProxies.Add(TRefCountPtr<FHairStrandsInstance>(Proxy));
...
void FScene::RemoveHairStrands(FHairStrandsInstance* Proxy)
{
	if (Proxy)
	{
		...
		if (HairStrandsSceneData.RegisteredProxies.IsValidIndex(ProxyIndex))
		{
			HairStrandsSceneData.RegisteredProxies[ProxyIndex]->RegisteredIndex = ProxyIndex;
		}
	}
}

IAnimBankTransformProvider* FScene::GetAnimBankTransformProvider()
{
	return SceneExtensions.GetExtensionPtr<FAnimBankTransformProvider>();
}

IAnimRuntimeTransformProvider* FScene::GetAnimRuntimeTransformProvider()
{
	return SceneExtensions.GetExtensionPtr<FAnimRuntimeTransformProvider>();
}

IAnimSequenceTransformProvider* FScene::GetAnimSequenceTransformProvider()
{
	return SceneExtensions.GetExtensionPtr<FAnimSequenceTransformProvider>();
}

ISkinningSceneExtension* FScene::GetSkinningSceneExtension()
{
	return SceneExtensions.GetExtensionPtr<FSkinningSceneExtension>();
}
...
void FScene::CreateReflectionCaptureProxy(UReflectionCaptureComponent* Component)
{
	if (!Component->SceneProxy)
	{
		Component->SceneProxy = Component->CreateSceneProxy();
		...
		ENQUEUE_RENDER_COMMAND(FAddCaptureCommand)(
			[Scene, Component, bIsRuntime = Component->IsRuntimeCapture(), Proxy, Position](FRHICommandListBase& RHICmdList)
		{
			...
			Scene->ReflectionSceneData.SetRegisteredReflectionCapturesHasChanged();
			...
			// For runtime captures, suppress shading until its cubemap has been rendered.
			if (bIsRuntime)
			{
				FCaptureComponentSceneState* State = Scene->ReflectionSceneData.AllocatedReflectionCaptureState.Find(Component);
				if (State && State->bRenderedForShading)
				{
...
void FScene::DestroyReflectionCaptureProxy(UReflectionCaptureComponent* Component)
{
	if (Component->SceneProxy)
	{
		FScene* Scene = this;
			...
			[Scene, Proxy] (FRHICommandListBase&)
		{
			...
			// Only mobile and desktop forward populate CachedReflectionCaptureProxy; on deferred there is nothing to clear.
			if (Scene->ShouldCacheReflectionCaptures())
			{
				for (int32 PrimitiveIndex = 0; PrimitiveIndex < Scene->Primitives.Num(); ++PrimitiveIndex)
				{
					Scene->Primitives[PrimitiveIndex]->RemoveCachedReflectionCapture(Proxy);
				}
			}
		...
		// Reset runtime capture related values to defaults, so it starts at a fresh state if its proxy is re-created.
		Component->RuntimeCaptureResetState();
	}
}

void FScene::AddReflectionCapture(UReflectionCaptureComponent* Component)
{
	// Always track runtime captures as budget candidates so the budget CVar can be changed at runtime.
	if (Component->IsRuntimeCapture())
	{
		ReflectionSceneData.RuntimeCaptureBudgetCandidates.AddUnique(Component);

		// If budgeting is active, defer proxy creation.  The budget evaluation in AllocateReflectionCaptures will activate it later.
		static const auto CVarBudget = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.ReflectionCapture.Runtime.Budget"));
		const int32 Budget = CVarBudget ? CVarBudget->GetValueOnGameThread() : 0;
		if (Budget > 0 && !bIsEditorScene)
		{
			return;
		}
	}

	CreateReflectionCaptureProxy(Component);
}

void FScene::RemoveReflectionCapture(UReflectionCaptureComponent* Component)
{
	ReflectionSceneData.RuntimeCaptureBudgetCandidates.Remove(Component);
	DestroyReflectionCaptureProxy(Component);
}
...
void FScene::UpdateReflectionCaptureTransform(UReflectionCaptureComponent* Component)
{
	if (Component->SceneProxy)
	{
		...
		float InfluenceRadius = Component->GetInfluenceBoundingRadius();

		ENQUEUE_RENDER_COMMAND(UpdateReflectionCaptureTransformCommand)(
			[Scene, Proxy, Transform, InfluenceRadius, bUsingPreviewCaptureData](FRHICommandListBase& RHICmdList)
		{
			if (Proxy->bUsingPreviewCaptureData)
			{
				FPlatformAtomics::InterlockedDecrement(&Scene->NumUnbuiltReflectionCaptures);
			...
			Proxy->InfluenceRadius = InfluenceRadius;

			int32 CaptureIndex = Proxy->PackedIndex;
			Scene->ReflectionSceneData.RegisteredReflectionCapturePositionAndRadius[CaptureIndex] = FSphere(Transform.GetOrigin(), InfluenceRadius);
	...
	if (bRemoved)
	{
		...
		ENQUEUE_RENDER_COMMAND(RemoveCaptureCommand)(
			[CaptureComponent, Scene] (FRHICommandListBase&)
			{
				...
				// Free any in-flight blends owned by this component so the slot becomes available for other blends.
				for (FReflectionEnvironmentSceneData::FSmoothBlendEntry& Entry : Scene->ReflectionSceneData.RuntimeSmoothBlendSlots)
				{
					if (Entry.OwningComponent == CaptureComponent)
					{
						Entry = {};
					}
				}
...
void FVolumetricLightmapSceneData::AddLevelVolume(FRHICommandListImmediate& RHICmdList, const FPrecomputedVolumetricLightmap* InVolume, EShadingPath ShadingPath, bool bIsPersistentLevel)
{
	...
	InVolume->Data->AddToSceneData(RHICmdList, &GlobalVolumetricLightmapData);
	
	// Invalidate CPU lightmap lookup cache
	CPUInterpolationCache.Empty();
}

void FVolumetricLightmapSceneData::RemoveLevelVolume(FRHICommandListImmediate& RHICmdList, const FPrecomputedVolumetricLightmap* InVolume)
{
	LevelVolumetricLightmaps.Remove(InVolume);

	InVolume->Data->RemoveFromSceneData(RHICmdList, &GlobalVolumetricLightmapData, PersistentLevelVolumetricLightmap ? PersistentLevelVolumetricLightmap->Data->BrickDataBaseOffsetInAtlas : 0);
...
void FScene::AddPrecomputedVolumetricLightmap(const FPrecomputedVolumetricLightmap* Volume, bool bIsPersistentLevel)
{
	...
	{
		Scene->VolumetricLightmapSceneData.AddLevelVolume(RHICmdList, Volume, Scene->GetShadingPath(), bIsPersistentLevel);
	});
}
...
void FScene::RemovePrecomputedVolumetricLightmap(const FPrecomputedVolumetricLightmap* Volume)
{
	...
	{
		Scene->VolumetricLightmapSceneData.RemoveLevelVolume(RHICmdList, Volume);
	});
}
...
void FScene::GetPrimitiveUniformShaderParameters_RenderThread(const FPrimitiveSceneInfo* PrimitiveSceneInfo, bool& bHasPrecomputedVolumetricLightmap, FMatrix& PreviousLocalToWorld, int32& SingleCaptureIndex, bool& bOutputVelocity) const 
{
	...
	// NOTE: The logic here is duplicated in BuildPrimitiveUniformShaderParameters_RenderThread since that allows reordering the logic to avoid transform copies & needlessly setting Builder.PreviousLocalToWorld - which avoids 
	//       a matrix inverse.
...
void FScene::BuildPrimitiveUniformShaderParameters_RenderThread(const FPrimitiveSceneInfo* PrimitiveSceneInfo, FPrimitiveUniformShaderParametersBuilder& Builder) const
{
	const FPrimitiveSceneProxy* Proxy = PrimitiveSceneInfo->Proxy;

	const FMatrix LocalToWorld = Proxy->GetLocalToWorld();

	bool bOutputVelocity = Proxy->AlwaysHasVelocity() || (Proxy->IsNaniteMesh() && Proxy->IsSkinnedMesh());
	const bool bHasPrecomputedVolumetricLightmap = VolumetricLightmapSceneData.HasData();

	// Get index if proxy exists, otherwise fall back to index 0 which will contain the default black cubemap
	const int32 SingleCaptureIndex = PrimitiveSceneInfo->CachedReflectionCaptureProxy ? PrimitiveSceneInfo->CachedReflectionCaptureProxy->SortedCaptureIndex : 0;

	Builder.LocalToWorld(LocalToWorld)
			.SingleCaptureIndex(SingleCaptureIndex)
			.UseVolumetricLightmap(bHasPrecomputedVolumetricLightmap);

	// Only set up previous transform if we actually have one.
	if (const FMatrix* RESTRICT PreviousLocalToWorld = VelocityData.GetComponentPreviousLocalToWorld(PrimitiveSceneInfo->PrimitiveComponentId))
	{
		Builder.PreviousLocalToWorld(*PreviousLocalToWorld);
		bOutputVelocity = bOutputVelocity || !LocalToWorld.Equals(*PreviousLocalToWorld, 0.0001f);
	}

	Builder.OutputVelocity(bOutputVelocity)
			.LightmapDataIndex(PrimitiveSceneInfo->GetLightmapDataOffset())
...
bool DoesPlatformNeedLocalLightPrimitiveInteraction(EShaderPlatform ShaderPlatform)
{
	...
	if (CVarVisibilityLocalLightPrimitiveInteraction.GetValueOnRenderThread() == 2 && MegaLights::IsEnabledInProject(ShaderPlatform))
	{
		return false;
	}
...
void FScene::UpdateLightTransform_RenderThread(FLightSceneId LightId, FLightSceneInfo* LightSceneInfo, const FUpdateLightTransformParameters& Parameters)
{
	...
	const bool bHasId = LightId.IsValid();
...
void FScene::UpdateLightInternal(FLightSceneProxy* LightSceneProxy, UpdatePayloadType&& InUpdatePayload)
{
	if (LightSceneProxy)
	{
		...
		if (LightSceneInfo->bVisible)
		{
			ENQUEUE_RENDER_COMMAND(UpdateLightInternal)(
...
void FScene::SetShadowDepthCaptureView(ULightComponent* Light, const FShadowDepthCaptureView& View)
{
	FLightSceneProxy* LightSceneProxy = Light ? Light->SceneProxy : nullptr;
	if (!LightSceneProxy)
	{
		return;
	}

	ENQUEUE_RENDER_COMMAND(SetShadowDepthCaptureView)(
		[this, LightSceneProxy, View](FRHICommandListBase&)
		{
			if (const FLightSceneInfo* LightSceneInfo = LightSceneProxy->GetLightSceneInfo())
			{
				GetExtension<FCSMSceneExtension>().SetCapturedView(LightSceneInfo->Id, View);
			}
		});
}

void FScene::ClearShadowDepthCaptureView(ULightComponent* Light)
{
	FLightSceneProxy* LightSceneProxy = Light ? Light->SceneProxy : nullptr;
	if (!LightSceneProxy)
	{
	...
	ENQUEUE_RENDER_COMMAND(ClearShadowDepthCaptureView)(
		[this, LightSceneProxy](FRHICommandListBase&)
		{
			if (const FLightSceneInfo* LightSceneInfo = LightSceneProxy->GetLightSceneInfo())
			{
				GetExtension<FCSMSceneExtension>().ClearCapturedView(LightSceneInfo->Id);
			}
		});
}
	...
	if(GetFeatureLevelShadingPath(FeatureLevel) == EShadingPath::Mobile)
	{
		...
		for (int32 LightChannelIdx = 0; LightChannelIdx < UE_ARRAY_COUNT(MobileDirectionalLights); LightChannelIdx++)
		{
			if (LightSceneInfo == MobileDirectionalLights[LightChannelIdx])
			{
				...
				{
					bScenesPrimitivesNeedStaticMeshElementUpdate = true;
					UE_CLOGF(!GIsEditor, LogRenderer, Log, "Forcing update for all mesh draw commands: Remove directional light");
				}
				break;
			}
		}
...
{
	FLightSceneDesc LightSceneDesc{ .SceneProxy = Light->SceneProxy };
	BatchRemoveLights(MakeArrayView(&LightSceneDesc, 1));

	// Disassociate the primitive's render info.
	Light->SceneProxy = nullptr;
}

void FScene::BatchRemoveLights(TArrayView<FLightSceneDesc> InLights)
{
	TArray<FLightSceneInfo*, SceneRenderingAllocator> LightsToRemove;
	LightsToRemove.Reserve(InLights.Num());

	for (const FLightSceneDesc& LightDesc : InLights)
	{
		if (LightDesc.SceneProxy)
		{
			FLightSceneInfo* LightSceneInfo = LightDesc.SceneProxy->GetLightSceneInfo();
			...
			LightsToRemove.Emplace(LightSceneInfo);
		}
	}
	...
	if (!LightsToRemove.IsEmpty())
	{
		...
		ENQUEUE_RENDER_COMMAND(BatchRemoveLightsCommand)(
			[this, LightsToRemove = MoveTemp(LightsToRemove)](FRHICommandListBase& RHICmdList)
			{
				for (FLightSceneInfo* LightSceneInfo : LightsToRemove)
				{
					FScopeCycleCounter Context(LightSceneInfo->Proxy->GetStatId());
...
FLightSceneInfo* FScene::GetMobileDirectionalLightForView(int32 ChannelIdx, const FViewInfo& View) const
{
	// TODO:  Should this call "ShouldRenderLight(View)" instead of just checking the view lighting channel mask?  Main
	// difference is that ShouldRenderLight filters out dynamic lights when the view is set to render static only, typically
	// for sky and reflection captures.  The immediate goal of this function is to make the view lighting channel mask work
	// on mobile, and otherwise avoid changes to existing behavior, so we'll err on the side of caution for now.  If this
	// gets changed, the other view lighting channel mask checks in FMobileSceneRenderer::InitDynamicShadows and
	// FMobileSceneRenderer::BuildCSMVisibilityState should also be updated to use "ShouldRenderLight"!
	if (MobileDirectionalLights[ChannelIdx] &&
		MobileDirectionalLights[ChannelIdx]->Proxy &&
		(MobileDirectionalLights[ChannelIdx]->Proxy->GetViewLightingChannelMask() & View.ViewLightingChannelMask))
	{
		return MobileDirectionalLights[ChannelIdx];
	}
	return nullptr;
}
...
void FScene::GetRelevantLights_RenderThread( const FPrimitiveSceneProxy* PrimitiveSceneProxy, TArray<const FLightSceneProxy*> &OutRelevantLights ) const
{
	if (ShouldSkipNaniteLPIs(GetShaderPlatform()) && PrimitiveSceneProxy->IsNaniteMesh())
	{
		if (PrimitiveSceneProxy->GetLightingChannelMask() != 0)
		{
			...
			for (FLightSceneId LightID : DirectionalShadowCastingLightIDs)
			{
		...
		ENQUEUE_RENDER_COMMAND(FGetRelevantLightsCommand)(
			[this, PrimitiveSceneProxy = Primitive->GetSceneProxy(), RelevantLights] (FRHICommandListBase&)
			{
				...
				for (const FLightSceneProxy* LightSceneProxy : RelevantLightProxies)
				{
					...
					if (const ULightComponent* LightComponent = LightSceneProxy->LightComponent)
					{
						RelevantLights->Add(LightComponent);
					}
				}
			});
...
int32 FScene::GetRelevantLightCount(UPrimitiveComponent* Primitive) const
{
	int32 Count = 0;
	if (Primitive && Primitive->GetSceneProxy() != nullptr)
	{
		// Add interacting lights to the array.
		ENQUEUE_RENDER_COMMAND(FGetRelevantLightsCommand)(
			[this, PrimitiveSceneProxy = Primitive->GetSceneProxy(), &Count](FRHICommandListBase&)
			{
				TArray<const FLightSceneProxy*> RelevantLightProxies;
				GetRelevantLights_RenderThread(PrimitiveSceneProxy, RelevantLightProxies);
				for (const FLightSceneProxy* LightSceneProxy : RelevantLightProxies)
				{
					if (LightSceneProxy->GetLightComponentId().IsValid())
					{
						++Count;
					}
				}
	...
	// Flush rendering commands to wait for command to execute
	FlushRenderingCommands();
...
void FScene::UpdateCachedRayTracingState(FPrimitiveSceneProxy* SceneProxy, EPrimitiveDirtyRayTracingState PrimitiveDirtyState)
{
	check(IsInRenderingThread());

	if (FPrimitiveSceneInfo* PrimitiveSceneInfo = SceneProxy->GetPrimitiveSceneInfo())
	{
		if (EnumHasAnyFlags(PrimitiveDirtyState, EPrimitiveDirtyRayTracingState::MeshCommandsAndInstances))
		{
			PrimitiveSceneInfo->RemoveCachedRayTracingPrimitives();
		}

		const int32 PrimitiveIndex = PrimitiveSceneInfo->GetIndex();
		if (PrimitiveIndex != INDEX_NONE)
		{
			if (EnumHasAnyFlags(PrimitiveDirtyState, EPrimitiveDirtyRayTracingState::DrawInGame))
			{
				PrimitiveRayTracingDatas[PrimitiveIndex].bDrawInGame = SceneProxy->IsDrawnInGame();
			}

			if (EnumHasAnyFlags(PrimitiveDirtyState, EPrimitiveDirtyRayTracingState::LightingChannelMask))
			{
				PrimitiveRayTracingDatas[PrimitiveIndex].LightingChannelMask = SceneProxy->GetLightingChannelMask();
			}
...
#if WITH_STATE_STREAM
	if (FStateStreamManagerImpl* StateStreamManager = static_cast<FStateStreamManagerImpl*>(World->GetStateStreamManager()))
	{
		ENQUEUE_RENDER_COMMAND(FReleaseStateStream)(
			[Scene, StateStreamManager](FRHICommandListImmediate& RHICmdList)
			{
				//StateStreamManager->Render_Exit();
				//GetRendererModule().FreeStateStream(StateStreamManager);
				if (FSkeletalMeshUpdater* Smu = Scene->SkeletalMeshUpdater)
				{
					Smu->Shutdown();
				}
			});
...
void FScene::DumpUnbuiltLightInteractions( FOutputDevice& Ar ) const
{
	...
	for(const FLightSceneInfoCompact& LightCompactInfo : Lights)
	{
...
void FScene::ApplyWorldOffset_RenderThread(FRHICommandListBase& RHICmdList, const FVector& InOffset)
{
	...
	for(FLightSceneInfoCompact& LightSceneInfoCompact : Lights)
	{
		LightSceneInfoCompact.BoundingSphereVector = VectorAdd(LightSceneInfoCompact.BoundingSphereVector, OffsetReg);
		LightSceneInfoCompact.LightSceneInfo->Proxy->ApplyWorldOffset(InOffset);
	}
...
void FScene::ProcessAtmosphereLightRemoval_RenderThread(FLightSceneInfo* LightSceneInfo)
{
	...
	if (AtmosphereLights[Index] == LightSceneInfo)
	{
		...
		for (const FLightSceneInfoCompact& LightInfo : Lights)
		{
...
void FScene::UpdateLights(FRDGBuilder& GraphBuilder, FSceneExtensionsUpdaters& SceneExtensionsUpdaters)
{
	...
	{
		TArray<FLightSceneId, SceneRenderingAllocator> RemovedLightIds;
		TArray<FLightSceneId, SceneRenderingAllocator> AddedLightIds;
		FFLightSceneChangeSetAllocation(FSceneLightInfoUpdates&& InSceneLightInfoUpdates, int32 InPreUpdateMaxIndex) 
			...
			, PreUpdateChangeSet{TConstArrayView<FLightSceneId>(RemovedLightIds), TConstArrayView<FLightSceneId>(), SceneLightInfoUpdates, RemovedLightsMask, InPreUpdateMaxIndex, -1 }
			, PostUpdateChangeSet{TConstArrayView<FLightSceneId>(RemovedLightIds), TConstArrayView<FLightSceneId>(AddedLightIds), SceneLightInfoUpdates, RemovedLightsMask, InPreUpdateMaxIndex, -1 }
		{}
		FSceneLightInfoUpdates SceneLightInfoUpdates;
		FSceneRendererLightBitArray RemovedLightsMask;
	...
	{
		FLightSceneId Id = UpdateLightCommand.GetSceneInfo()->Id;
	...
	for (auto LightId : ChangeSetAlloc.RemovedLightIds)
	{
	...
	ChangeSetAlloc.SceneLightInfoUpdates.ForEachCommand(ESceneUpdateCommandFilter::AddedUpdated, [&](const FUpdateLightCommand& UpdateLightCommand)
	{
		...
		const FLightSceneId Id = LightSceneInfo->Id;
		...
		if (FUpdateLightColorParameters* ColorParameters = ChangeSetAlloc.SceneLightInfoUpdates.GetPayloadPtr<FUpdateLightColorParameters>(UpdateLightCommand))
		{
			...
			{
				bScenesPrimitivesNeedStaticMeshElementUpdate = true;
				UE_CLOGF(!GIsEditor, LogRenderer, Log, "Forcing update for all mesh draw commands: Toggle directional light");
			}
...
extern TAutoConsoleVariable<float> CVarReflectionCaptureRuntimeFadeInTime;
extern TAutoConsoleVariable<float> CVarReflectionCaptureRuntimeSmoothBlendTime;
...
static void CreateReflectionCaptureUniformBuffer(const TArray<FReflectionCaptureSortData>& SortedCaptures, TUniformBufferRef<T>& OutReflectionCaptureUniformBuffer)
{
	...
	for (int32 CaptureIndex = 0; CaptureIndex < SortedCaptures.Num(); CaptureIndex++)
	{
		...
		// PositionLow.w carries the packed refresh-blend state: integer = source cubemap array index, fractional = alpha.
		// A value of 0 means no active blend; the shader skips the cross-fade with a single compare.
		SamplePositionsBuffer.PositionLow[CaptureIndex] = FVector4f(SortedCaptures[CaptureIndex].Position.Low, SortedCaptures[CaptureIndex].SmoothBlendEncoded);
...
void UpdateReflectionSceneData(FScene* Scene)
{
	...
	const int32 MaxCubemaps = ReflectionSceneData.CubemapArray.GetMaxRegularCubemaps();
	...
	// Blend-in parameters for runtime captures.  When any capture is mid-fade we need to rebuild the uniform buffer
	// each frame so its CaptureProperties.a value advances even when nothing else has changed.
	const float FadeInDuration = FMath::Max(0.0f, CVarReflectionCaptureRuntimeFadeInTime.GetValueOnRenderThread());
	const double CurrentTime = FApp::GetCurrentTime();
	bool bAnyBlendInProgress = false;

	// Sweep refresh-blend slots: free any whose cross-fade has completed.  A slot's BlendStartTime is stamped by
	// StampRefreshBlendStartTime once the new render lands; before that the slot stays pinned (BlendStartTime < 0)
	// and the shader keeps the source at full strength.
	const float RefreshBlendDuration = FMath::Max(0.0f, CVarReflectionCaptureRuntimeSmoothBlendTime.GetValueOnRenderThread());
	for (FReflectionEnvironmentSceneData::FSmoothBlendEntry& Entry : ReflectionSceneData.RuntimeSmoothBlendSlots)
	{
		if (Entry.OwningComponent != nullptr && Entry.BlendStartTime >= 0.0)
		{
			const float Elapsed = (float)(CurrentTime - Entry.BlendStartTime);
			if (RefreshBlendDuration <= 0.0f || Elapsed >= RefreshBlendDuration)
			{
				Entry = {};
			}
		}
	...
	for (int32 ReflectionProxyIndex = 0; ReflectionProxyIndex < ReflectionSceneData.RegisteredReflectionCaptures.Num() && ReflectionSceneData.SortedCaptures.Num() < PlatformMaxNumReflectionCaptures; ReflectionProxyIndex++)
	{
		...
		if (SupportsTextureCubeArray(Scene->GetFeatureLevel()))
		{
			...
			// Skip captures that haven't finished rendering for the current activation, to avoid sampling a stale cubemap slot.
			if (!ComponentStatePtr->bRenderedForShading)
			{
				continue;
			}
			...
			// Match this capture against any active refresh-blend slot and pack (source cube index, alpha) into a
			// single float for PositionLow.w: integer part = SourceCubemapArrayIndex, fractional part = blend alpha.
			// Alpha is clamped strictly below 1 so floor() in the shader recovers the cube index even when the
			// fade has just started.  Setting any non-zero value forces a UB rebuild so the alpha advances per frame.
			for (int32 SlotIdx = 0; SlotIdx < ReflectionSceneData.RuntimeSmoothBlendSlots.Num(); SlotIdx++)
			{
				const FReflectionEnvironmentSceneData::FSmoothBlendEntry& Slot = ReflectionSceneData.RuntimeSmoothBlendSlots[SlotIdx];
				if (Slot.OwningComponent == CurrentCapture->Component && Slot.SourceCubemapArrayIndex >= 0)
				{
					float BlendAlpha = 1.0f;
					if (Slot.BlendStartTime >= 0.0)
					{
						BlendAlpha = (RefreshBlendDuration > 0.0f)
							? 1.0f - FMath::Clamp((float)(CurrentTime - Slot.BlendStartTime) / RefreshBlendDuration, 0.0f, 1.0f)
							: 0.0f;
					}
					BlendAlpha = FMath::Min(BlendAlpha, 0.99999f);
					NewSortEntry.SmoothBlendEncoded = (float)Slot.SourceCubemapArrayIndex + BlendAlpha;
					bAnyBlendInProgress = true;
					break;
				}
			}

			// Compute fade alpha from the (Start, Target, StartTime) triple.  Non-steady state forces a uniform buffer rebuild
			// so CaptureProperties.a advances each frame.  Once Alpha reaches Target we latch Start = Target so the next frame
			// can skip the rebuild if no fades are active.
			const float Alpha = ComponentStatePtr->ComputeCurrentFade(CurrentTime, FadeInDuration);
			NewSortEntry.FadeAlpha = Alpha;
			if (ComponentStatePtr->FadeStartValue != ComponentStatePtr->FadeTargetValue)
			{
				bAnyBlendInProgress = true;
				if (Alpha == ComponentStatePtr->FadeTargetValue)
				{
					ComponentStatePtr->FadeStartValue = ComponentStatePtr->FadeTargetValue;
				}
			}
		}
		...
		NewSortEntry.CaptureProperties = FVector4f(CurrentCapture->Brightness, NewSortEntry.CubemapIndex, ShapeTypeValue, NewSortEntry.FadeAlpha);
	...
	// Create uniform buffers with sorted captures.  Also rebuild while any capture is mid blend-in, since
	// CaptureProperties.a changes every frame until the fade completes.
	...
	if (Scene->ShouldCacheReflectionCaptures())
	{
		...
		const bool bPlatformNeedsStaticMeshUpdate = GetFeatureLevelShadingPath(Scene->GetFeatureLevel()) == EShadingPath::Mobile;

		if (ReflectionSceneData.bRegisteredReflectionCapturesHasChanged)
		{
			// Check if the sorted capture list changed.  If it did, we need to update all primitives that point to a reflection capture.
			bool bSortedCapturesChanged = false;
			if (ReflectionSceneData.SortedCaptures.Num() != ReflectionSceneData.PreChangeSortOrder.Num())
			{
				bSortedCapturesChanged = true;
			}
			else
			{
				for (int32 CaptureIndex = 0; CaptureIndex < ReflectionSceneData.SortedCaptures.Num(); CaptureIndex++)
				{
					if (ReflectionSceneData.SortedCaptures[CaptureIndex] != ReflectionSceneData.PreChangeSortOrder[CaptureIndex])
					{
						bSortedCapturesChanged = true;
						break;
					}
			...
			ReflectionSceneData.PreChangeSortOrder.Empty();

			// Mark all primitives as needing an update, that are affected by a capture that was updated.
			for (FPrimitiveSceneInfo* Primitive : Scene->Primitives)
			{
				if (bPlatformNeedsStaticMeshUpdate)
				{
					// If we have been flagged as requiring update, track that we need an update, even if CacheReflectionCaptures (which clears this flag)
					// returns false.  This handles the case where a primitive pointed to a modified reflection capture, but no longer does.  In that case,
					// CacheReflectionCaptures() will return false, since the capture pointers will already have been nulled out, and won't change.  We
					// need to generate new draw calls in that case, that don't use the reflection capture.
					bool bNeedsUpdate = Primitive->bNeedsCachedReflectionCaptureUpdate;

					// We need to call "CacheReflectionCaptures" for every primitive, even ones not flagged as affected by a changed capture, because it's
					// possible a change to the size or location of a capture causes the primitive to now intersect it.
					if (Primitive->CacheReflectionCaptures())
					{
						bNeedsUpdate = true;
					}

					// Finally, if the sorted captures changed, any primitive pointing to a capture whatsoever must be updated, since the sort indices
					// will have changed globally.
					if (bSortedCapturesChanged && Primitive->CachedReflectionCaptureProxy)
					{
						bNeedsUpdate = true;
					}
				...
				{
					// For platforms that don't need caching, remove the cache.
					Primitive->RemoveCachedReflectionCapture(nullptr);
					Primitive->RemoveCachedPlanarReflection(nullptr);
				}
			}

			ReflectionSceneData.bRegisteredReflectionCapturesHasChanged = false;
		}
		
		if (ReflectionSceneData.bSkylightHasChanged)
		{
			if (bPlatformNeedsStaticMeshUpdate)
			{
				...
				for (int32 PrimitiveIndex = 0; PrimitiveIndex < NumPrimitives; PrimitiveIndex++)
				{
					Scene->Primitives[PrimitiveIndex]->RequestStaticMeshUpdate();
				}
			}
			ReflectionSceneData.bSkylightHasChanged = false;
		}
	}
}
...
FViewSceneChangeSet* FScene::ProcessViewChanges(FRDGBuilder& GraphBuilder, TConstArrayView<FViewInfo*> Views)
{
	...
	// Extract scene time from the view family. Views should always be non-empty here;
	// ProcessViewChanges is only called when SceneUpdateInputs provides rendered views.
	if (ensureMsgf(Views.Num() > 0, TEXT("ProcessViewChanges called with no views")))
	{
		ChangeSet->CurrentTime = Views[0]->Family->Time;
	}

	return ChangeSet;
}

void FScene::ProcessLevelCommands(FLevelCommand::EOp OpToProcess)
{
	// TODO: Explicit invalidation of various cached systems does not scale & is very error prone. 
	// In the future this should be changed to use the data manager & a tracked visibility state & any and all down stream systems 
	// (GPU scene, VSM, Occlusion culling) monitor changes to this.
	FVirtualShadowMapArrayCacheManager* VirtualShadowMapArrayCacheManager = GetExtensionPtr<FVirtualShadowMapArrayCacheManager>();
	UE::Renderer::Private::IShadowInvalidatingInstances* ShadowInvalidation = VirtualShadowMapArrayCacheManager ? VirtualShadowMapArrayCacheManager->GetInvalidatingInstancesInterface() : nullptr;

	auto FlushCachedShadowMaps = [&](FPrimitiveSceneInfo* PrimitiveSceneInfo)
	{
		FLightPrimitiveInteraction* NodeLightList = PrimitiveSceneInfo->LightList;
		while (NodeLightList)
		{
	...
	{
		if (Command.Op == OpToProcess)
		{
			if (const TArray<FPrimitiveSceneInfo*>* LevelPrimitives = PrimitivesNeedingLevelUpdateNotification.Find(Command.Name))
			{
				for (FPrimitiveSceneInfo* PrimitiveSceneInfo : *LevelPrimitives)
				{
					// Skip primitives pending delete as they need no further action
					if (!PrimitiveSceneInfo->bPendingDelete)
					{
						if (OpToProcess == FLevelCommand::EOp::Add)
						{
							PrimitiveSceneInfo->Proxy->OnLevelAddedToWorld_RenderThread();
						}
						else
						{
							PrimitiveSceneInfo->Proxy->OnLevelRemovedFromWorld_RenderThread();
						}

						// Force hidden flag has changed, update it's dependent flags.
						PrimitiveSceneInfo->UpdateOcclusionFlags();
						PrimitiveFlagsCompact[PrimitiveSceneInfo->GetIndex()] = FPrimitiveFlagsCompact(PrimitiveSceneInfo->Proxy);

						// Note: this works because the explicitly queued invalidations are not filtered by primitive flags (otherwise the bIsForceHidden would kill it).
						if (ShadowInvalidation)
						{
							ShadowInvalidation->AddPrimitive(PrimitiveSceneInfo);
						}
						FlushCachedShadowMaps(PrimitiveSceneInfo);

						GPUScene.AddPrimitiveToUpdate(PrimitiveSceneInfo->GetPersistentIndex());
					}
				}
			}
		}		
...
void FScene::Update(FRDGBuilder& GraphBuilder, const FUpdateParameters& Parameters)
{
	...
	const FSkeletalMeshUpdater::FSubmitData* SkeletalMeshSubmitData = &FSkeletalMeshUpdater::FSubmitData::EmptyData;
	...
	if (SkeletalMeshUpdater)
	{
		SkeletalMeshSubmitData = &SkeletalMeshUpdater->Submit(GraphBuilder, GPUSkinCachePipeline);
		GPUSkinUpdateTask = SkeletalMeshSubmitData->GetSkinCacheTask();
	}
	...
	{
		GPUSkinCacheTask = GPUSkinCache->Dispatch(GraphBuilder, SkeletalMeshSubmitData->GetSkinCacheTask(), GPUSkinCachePipeline);
	}
	...
	// Remove ops shoud be processed before scene update to allow scene extensions PreSceneUpdate to handle effects before the scene is changed.
	// In particular VSM cache invalidation depends on this ordering.
	ProcessLevelCommands(FLevelCommand::EOp::Remove);
	...
	bool bNeedPathTracedInvalidation = false;
	{
		...
		while (RemovedLocalPrimitiveSceneInfos.Num())
		{
			...
			for (int32 RemoveIndex = StartIndex; RemoveIndex < RemovedLocalPrimitiveSceneInfos.Num(); RemoveIndex++)
			{
				...
				PrimitiveComponentIdToInfoMap.Remove(PrimitiveSceneInfo->PrimitiveComponentId);
...
#if RHI_RAYTRACING
				ensureMsgf(PrimitiveSceneInfo->bHasRayTracingRepresentation == SceneProxy->HasRayTracingRepresentation(),
					TEXT("Stale bHasRayTracingRepresentation detected. Primitive might have missed requests to update cached ray tracing state."));
				if (PrimitiveSceneInfo->bHasRayTracingRepresentation)
				{
					const RayTracing::FGeometryGroupHandle RayTracingGeometryGroupHandle = SceneProxy->GetRayTracingGeometryGroupHandle();
	...
	const auto QueueAddStaticMeshes = [&](FPrimitiveSceneInfo* SceneInfo)
	{
		...
		{
			UE_LOGF(LogRenderer, Fatal, "SceneInfo %ls is marked for deletion but is being queued for static mesh updates. This is a race condition and will crash.", *SceneInfo->Proxy->GetOwnerName().ToString());
		}
		...
		while (AddedLocalPrimitiveSceneInfos.Num())
		{
			...
			for (int32 AddIndex = StartIndex; AddIndex < AddedLocalPrimitiveSceneInfos.Num(); AddIndex++)
			{
...
#if RHI_RAYTRACING
				PrimitiveSceneInfo->bHasRayTracingRepresentation = SceneProxy->HasRayTracingRepresentation();
				if (PrimitiveSceneInfo->bHasRayTracingRepresentation)
				{
...

				{
					...
					PrimitiveRayTracingData.bRetainWhileHidden = SceneProxy->CastsHiddenShadow() || SceneProxy->AffectsIndirectLightingWhileHidden();
					...
					PrimitiveRayTracingData.LightingChannelMask = SceneProxy->GetLightingChannelMask();
					...
					PrimitiveSceneInfo->bIsRelevantToPathTracing =
						...
						(PrimitiveRayTracingData.bDrawInGame || PrimitiveRayTracingData.bRetainWhileHidden) &&
...
// TODO: Support skip always visible in the editor (need to handle dynamic relevance)
	...
	if (GVisibilitySkipAlwaysVisible != 0)
	{
		...
		for (int32 TypeOffsetIndex = 0; TypeOffsetIndex < TypeOffsetTable.Num(); ++TypeOffsetIndex)
		{
			...
			checkfSlow (Type == TypeEntry.PrimitiveSceneProxyType, TEXT("TypeHash: %zu not matching TypeOffsetTable, expected: %zu"), Type.ProxyTypeHash, TypeEntry.PrimitiveSceneProxyType.ProxyTypeHash);
		#endif
	...
	ProcessLevelCommands(FLevelCommand::EOp::Add);
	...
	}, MakeArrayView({ UpdateUniformExpressionsTask, SkeletalMeshSubmitData->GetInlineTask() }), UE::Tasks::ETaskPriority::High, bAsyncCacheMeshDrawCommands);
...

	{
		...
		if (bCachedShouldRenderSkylightInBasePass != bLastFrameShouldRenderSkylightInBasePass)
		{
			...
			UE_CLOGF(!GIsEditor, LogRenderer, Log, "Forcing update for all mesh draw commands: SkyLight change");
			bScenesPrimitivesNeedStaticMeshElementUpdate = true;
		}
		...
		if (bCachedSkyLightRealTimeCapture != bLastFrameSkyLightRealTimeCapture)
		{
			...
			UE_CLOGF(!GIsEditor, LogRenderer, Log, "Forcing update for all mesh draw commands: SkyLight real-time capture change");
			bScenesPrimitivesNeedStaticMeshElementUpdate = true;
		}

		if (Parameters.ViewUpdateChangeSet && Parameters.ViewUpdateChangeSet->bIsRequiresDebugMaterialChanged)
		{
			UE_CLOGF(!GIsEditor, LogRenderer, Log, "Forcing update for all mesh draw commands: Scene view debug rendering change");
			bScenesPrimitivesNeedStaticMeshElementUpdate = true;
		}

		if (GVRSImageManager.GetNeedStaticMeshUpdate())
		{
			UE_CLOGF(!GIsEditor, LogRenderer, Log, "Forcing update for all mesh draw commands: Variable rate shading toggled");
			bScenesPrimitivesNeedStaticMeshElementUpdate = true;
	...
	CreateLightPrimitiveInteractionsTask = GraphBuilder.AddSetupTask([this, &SceneInfosWithAddToScene]
	{
		...
		for (FPrimitiveSceneInfo* PrimitiveSceneInfo : SceneInfosWithAddToScene)
		{
			...
			if (Proxy->GetLightingChannelMask() != 0)
			{
				...
				// Also loop through non-local (directional) shadow-casting lights
				for (auto LightID : DirectionalShadowCastingLightIDs)
				{
					const FLightSceneInfoCompact& LightSceneInfoCompact = Lights[LightID];
					LightSceneInfoCompact.LightSceneInfo->CreateLightPrimitiveInteraction(LightSceneInfoCompact, PrimitiveSceneInfoCompact);
				}
	...
	if (IsRayTracingEnabled(GetShaderPlatform()))
	{
		...
		Nanite::GRayTracingManager.Update(GraphBuilder.RHICmdList);
	}
#endif
	...
	{
		SkinningUpdater->PostMeshUpdate(GraphBuilder, SceneInfosWithStaticDrawListUpdate, SkeletalMeshSubmitData);
	}
...

	{
		...
		GPUScene.Update(GraphBuilder, ExternalAccessQueue, SceneUpdateChangeSetStorage.PrimitiveUpdates.GetRangeView<FUpdateInstanceFromComputeCommand>(), Parameters.GPUSceneUpdateTaskPrerequisites, SceneExtensionsUpdaters);

		ExternalAccessQueue.Submit(GraphBuilder);
	}

	SceneExtensionsUpdaters.PostGPUSceneUpdate(GraphBuilder);
...
public:
	...
	virtual void GetPrimitiveUniformShaderParameters_RenderThread(const FPrimitiveSceneInfo* PrimitiveSceneInfo, bool& bHasPrecomputedVolumetricLightmap, FMatrix& PreviousLocalToWorld, int32& SingleCaptureIndex, bool& OutputVelocity) const override 
	{
		PreviousLocalToWorld = FMatrix::Identity; 
		SingleCaptureIndex = 0;
		bHasPrecomputedVolumetricLightmap = OutputVelocity = false;
	}
	...
	virtual IAnimBankTransformProvider* GetAnimBankTransformProvider() { return nullptr; }
	virtual IAnimRuntimeTransformProvider* GetAnimRuntimeTransformProvider() { return nullptr; }
	virtual IAnimSequenceTransformProvider* GetAnimSequenceTransformProvider() { return nullptr; }
	virtual ISkinningSceneExtension* GetSkinningSceneExtension() override { return nullptr; }
...
{
	check(IsInGameThread());
	SceneRemovedEvent.Broadcast(Scene);
	AllocatedScenes.Remove(Scene);
}

FSceneRemovedEvent::RegistrationType& FRendererModule::GetSceneRemovedEvent()
{
	return SceneRemovedEvent;
}

void FRendererModule::UpdateStaticDrawLists()
{
	ensureMsgf(false, TEXT("UpdateStaticDrawLists is no longer implemented and will be removed, see UpdateStaticMeshesForMaterials."));
}
...
void FScene::SwapMobileReflectionCaptureCachedShadowState()
{
	for (FLightSceneInfo* MobileDirectionalLightSceneInfo : MobileDirectionalLights)
	{
		if (MobileDirectionalLightSceneInfo && MobileDirectionalLightSceneInfo->Proxy->TimesliceAtlasFrame == ReflectionSceneData.RuntimeCaptureShadows.TimesliceAtlasFrame)
		{
			FReflectionEnvironmentCachedShadow& CachedShadow = ReflectionSceneData.RuntimeCaptureShadows.Shadows[MobileDirectionalLightSceneInfo->Proxy->TimesliceAtlasIndex[0]];

			// Generate payload for update command
			FUpdateLightTransformParameters TransformParameters;
			TransformParameters.LightToWorld = CachedShadow.LightToWorld;
			TransformParameters.Position     = CachedShadow.Position;

			// Store off the old transformation, so we can swap back to it later with a second call to this function
			CachedShadow.LightToWorld = MobileDirectionalLightSceneInfo->Proxy->LightToWorld;
			CachedShadow.Position     = MobileDirectionalLightSceneInfo->Proxy->Position;

			SceneLightInfoUpdates->Enqueue(MobileDirectionalLightSceneInfo, MoveTemp(TransformParameters));
		}
	}
}
