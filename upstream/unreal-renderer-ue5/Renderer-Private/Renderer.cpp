...
class FRendererModule::FRendererStateStreamManager : public FStateStreamManagerImpl
{
public:
	virtual void Game_DebugRender(IStateStreamDebugRenderer& Renderer, uint32 LaneId) override
	{
		FStateStreamManagerImpl::Game_DebugRender(Renderer, LaneId);

		TStringBuilder<1024> DebugLine;

		FScopeLock Lock(&ProxyTypeAndCountLock);
		const int32 NumElementCounts = ProxyTypeAndCount.Num();
		if (NumElementCounts == 0)
		{
			return;
		}
		const float NumElementColumns = FMath::Min(10.0f, NumElementCounts);
		const int32 NumElementRows = FMath::CeilToInt32(NumElementCounts / NumElementColumns);

		// Display the number of elements
		DebugLine.Reset();
		DebugLine.Appendf(TEXT("Num Render proxies = %d"), NumElementCounts);
		Renderer.DrawText(*DebugLine, FColor::Blue);

		// Display per element informations
		auto ElementIterator = ProxyTypeAndCount.CreateConstIterator();
		for (int32 ColumnIndex = 0; ColumnIndex < NumElementColumns; ++ColumnIndex)
		{
			DebugLine.Reset();
			//DebugLine.Appendf(TEXT("%s : "), *DebugName);
			for (int32 RowIndex = 0; RowIndex < NumElementRows; ++RowIndex)
			{
				if (ElementIterator)
				{
					DebugLine.Appendf(TEXT("[%s | %d]"), *ElementIterator.Key().ToString(), ElementIterator.Value());
					//UE_LOGF(LogTemp, Log, "%ls : [%ls | %d]", *DebugName, *ElementIterator.Key(), ElementIterator.Value());
					++ElementIterator;
				}
			}
	...
	FCriticalSection ProxyTypeAndCountLock;
	TMap<FName, uint32> ProxyTypeAndCount;
};
#endif
...
void FRendererModule::StartupModule()
{
	...
	FMaterialCacheVirtualTextureAllocator::Initialize();
	...
	if (FApp::CanEverRender() && !FApp::IsUnattended())
	{
		FCoreDelegates::GetOnPostEngineInit().AddLambda([]()
		{
			...
			InitVisualizeColorGradingExtension();
		});
	}

#if WITH_STATE_STREAM
	StateStreamManager = new FRendererStateStreamManager();
	FStateStreamCreator::RegisterStateStreams(FStateStreamRegisterContext{*StateStreamManager});
	//uint32 RenderCommandsLane = StateStreamManager->Game_CreateLane(); // Create the lane that non-tick based render commands can be stored in
	//check(RenderCommandsLane == 0);(void)RenderCommandsLane;
#endif
}
...
void FRendererModule::ShutdownModule()
{
	...
	FMaterialCacheVirtualTextureAllocator::Shutdown();
...
#if WITH_STATE_STREAM
IStateStreamManager* FRendererModule::GetStateStreamManager()
{
	return StateStreamManager;
}

void FRendererModule::DebugLogRenderProxies(FName Type, uint32 Num)
{
	FScopeLock Lock(&StateStreamManager->ProxyTypeAndCountLock);
	StateStreamManager->ProxyTypeAndCount.FindOrAdd(Type, 0) += Num;
}
#endif
...
{
	return CreateSinglePrimitiveSceneUniformBuffer(GraphBuilder, nullptr, FeatureLevel, Mesh);
}

FSceneUniformBuffer* FRendererModule::CreateSinglePrimitiveSceneUniformBuffer(FRDGBuilder& GraphBuilder, const FPrimitiveSceneProxy* PrimitiveSceneProxy, ERHIFeatureLevel::Type FeatureLevel, FMeshBatch& Mesh)
{
	FSceneUniformBuffer& SceneUniforms = *GraphBuilder.AllocObject<FSceneUniformBuffer>();

	if (Mesh.VertexFactory->SupportsGPUScene(FeatureLevel))
	{
		...
		if (MeshElement.PrimitiveUniformBufferResource)
		{
			...
			FRDGBufferRef InstancePayloadDataBuffer = nullptr;

			uint32 InstancePayloadFlags = 0;
			uint32 CustomDataCount = 0;
			float InstanceRandomID = 0.0f;

			// If a primitive scene proxy is provided, add any extra instance data that this proxy needs.
			if(PrimitiveSceneProxy)
			{
				if (const FInstanceSceneDataBuffers* InstanceSceneDataBuffersPtr = PrimitiveSceneProxy->GetInstanceSceneDataBuffers())
				{
					checkf(InstanceSceneDataBuffersPtr->GetNumInstances() == 1, TEXT("CreateSinglePrimitiveSceneUniformBuffer does not currently support instancing"));

					FInstanceUploadInfo InstanceUploadInfo;
					InstanceUploadInfo.SetInstancePayloadData(InstanceSceneDataBuffersPtr);
					
					uint32 PayloadFloat4Size = InstanceUploadInfo.CalculatePayloadStrideFloat4();
					if (PayloadFloat4Size > 0)
					{
						TArray<FVector4f, SceneRenderingAllocator> InstanceUploadInfoBufferData = GraphBuilder.AllocArray<FVector4f>();
						InstanceUploadInfoBufferData.SetNum(PayloadFloat4Size);
						InstanceUploadInfo.WritePayloadData(InstanceUploadInfoBufferData, 0);
						InstancePayloadDataBuffer = CreateStructuredBuffer(GraphBuilder, TEXT("InstancePayloadDataBuffer"), TConstArrayView<FVector4f>(InstanceUploadInfoBufferData));

						PrimitiveParams.InstancePayloadDataOffset = 0;
						PrimitiveParams.InstancePayloadDataStride = PayloadFloat4Size;

						InstancePayloadFlags = InstanceUploadInfo.InstanceFlags;
						CustomDataCount = InstanceUploadInfo.InstanceCustomDataCount;
					}
				}
			}
			...
			(
				...
				InstancePayloadFlags /* Payload Data Flags */,
				INVALID_LAST_UPDATE_FRAME,
				CustomDataCount /* Custom Data Count */,
				0.0f /* Random ID */,
			...
			InstancePayloadDataBuffer = InstancePayloadDataBuffer ? InstancePayloadDataBuffer : GSystemTextures.GetDefaultStructuredBuffer(GraphBuilder, sizeof(FVector4f));
...
TRDGUniformBufferRef<FInstanceCullingGlobalUniforms> FRendererModule::CreateSinglePrimitiveInstanceCullingUniformBuffer(FRDGBuilder& GraphBuilder, FStaticShaderPlatform ShaderPlatform, FMeshBatch& Mesh)
{
	if (!PlatformGPUSceneUsesUniformBufferView(ShaderPlatform) ||
		!Mesh.VertexFactory->SupportsGPUScene(GetMaxSupportedFeatureLevel(ShaderPlatform)))
	{
		return FInstanceCullingContext::CreateDummyInstanceCullingUniformBuffer(GraphBuilder);
	}

	FMeshBatchElement& MeshElement = Mesh.Elements[0];
	checkf(Mesh.Elements.Num() == 1, TEXT("Only 1 batch element currently supported by CreateSinglePrimitiveInstanceCullingUniformBuffer"));
	checkf(MeshElement.PrimitiveUniformBuffer == nullptr, TEXT("CreateSinglePrimitiveInstanceCullingUniformBuffer does not currently support an explicit primitive uniform buffer on vertex factories which manually fetch primitive data.  Use PrimitiveUniformBufferResource instead."));
	...
	FRDGBufferRef SinglePrimitiveBuffer = GraphBuilder.CreateBuffer(UBODesc, TEXT("SinglePrimitiveData"));
	FRDGBufferRef SingleInstanceBuffer = GraphBuilder.CreateBuffer(UBODesc, TEXT("SingleInstanceData"));

	// Identity parameters
	FBatchedPrimitiveShaderData PrimitiveShaderData{};
	FBatchedInstanceShaderData InstanceShaderData{};
	if (MeshElement.PrimitiveUniformBufferResource)
	{
		checkf(MeshElement.NumInstances == 1, TEXT("CreateSinglePrimitiveInstanceCullingUniformBuffer does not currently support instancing"));
		// Force PrimitiveId to be 0 in the shader
		MeshElement.PrimitiveIdMode = PrimID_ForceZero;
		FPrimitiveUniformShaderParameters PrimitiveParams = *(const FPrimitiveUniformShaderParameters*)MeshElement.PrimitiveUniformBufferResource->GetContents();
		// Now we just need to fill out the first entry of a batched primitive data in a buffer
		PrimitiveShaderData = FBatchedPrimitiveShaderData(PrimitiveParams);
		InstanceShaderData = FBatchedInstanceShaderData(PrimitiveParams, 0);
	}
	GraphBuilder.QueueBufferUpload(SinglePrimitiveBuffer, PrimitiveShaderData.Data.GetData(), PrimitiveShaderData.Data.Num() * sizeof(FVector4f));
	GraphBuilder.QueueBufferUpload(SingleInstanceBuffer, InstanceShaderData.Data.GetData(), InstanceShaderData.Data.Num() * sizeof(FVector4f));
	
	InstanceCullingGlobalUniforms->BatchedPrimitiveData = GraphBuilder.CreateSRV(SinglePrimitiveBuffer, PF_A32B32G32R32F);
	InstanceCullingGlobalUniforms->BatchedInstanceData = GraphBuilder.CreateSRV(SingleInstanceBuffer, PF_A32B32G32R32F);
...
void FRendererModule::DrawTileMesh(FCanvasRenderContext& RenderContext, FMeshPassProcessorRenderState& DrawRenderState, const FSceneView& SceneView, FMeshBatch& Mesh, bool bIsHitTesting, const FHitProxyId& HitProxyId, bool bUse128bitRT)
{
	if (!GUsingNullRHI)
	{
		...
		View.ForwardLightingResources.SetShadowUniformBuffer(CreateDummyForwardDirLightShadowUniformBuffer(GraphBuilder));
		...
		PassParameters->RenderTargets[0] = FRenderTargetBinding(RenderContext.GetRenderTarget(), ERenderTargetLoadAction::ELoad, /*MipIndex*/0, RenderContext.GetSliceIndex());
		...
		else
		{
			...
			else
			{
				if (ShadingPath == EShadingPath::Deferred)
				{
					...
					const bool bEnableSkylight = Scene ? Scene->ShouldRenderSkylightInBasePass(/*bIsTranslucent*/false) : true;
					PassParameters->OpaqueBasePass = CreateOpaqueBasePassUniformBuffer(GraphBuilder, View, ViewIndex, {}, {}, bEnableSkylight);
