...
DEFINE_GPU_STAT(ReflectionEnvironment);
...
class FDiffuseIndirectCompositePS : public FGlobalShader
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FApplyDiffuseIndirectDim, FUpscaleSSGIDim, FScreenBentNormalMode, FUpscaleLumenGIDim, FShortRangeGI, Substrate::FSubstrateTileType, FEnableDualSrcBlending, FScreenSpaceReflectionTiledComposition, FHairComplexTransmittance>;
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		if (PermutationVector.Get<FApplyDiffuseIndirectDim>() != 2 
			&& (PermutationVector.Get<FScreenBentNormalMode>() != 0 || PermutationVector.Get<FShortRangeGI>() || PermutationVector.Get<FUpscaleLumenGIDim>()))
		{
			return false;
		}
	...
	static EShaderPermutationPrecacheRequest ShouldPrecachePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		extern int32 GLumenShortRangeGlobalIllumination;
		if (PermutationVector.Get<FShortRangeGI>() != (GLumenShortRangeGlobalIllumination != 0))
		{
			return EShaderPermutationPrecacheRequest::NotUsed;
		}
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, LumenAmbientOcclusionIntensity)
	...
	{
		const bool bSeparatedSubsurfaceDiffuse = PlatformSupportsSeparatedSubsurfaceDiffuse(Parameters.Platform);
		OutEnvironment.SetDefine(TEXT("SSS_SUPPORT_SEPARATED_SUBSURFACE_DIFFUSE"), bSeparatedSubsurfaceDiffuse  ? 1 : 0);
		OutEnvironment.CompilerFlags.Add(CFLAG_ForceOptimization);
		if ((Substrate::IsSubstrateEnabled() && Substrate::IsOpaqueRoughRefractionEnabled(Parameters.Platform))
			|| bSeparatedSubsurfaceDiffuse)
		{
...
class FReflectionEnvironmentSkyLightingPS : public FGlobalShader
{
	...
	static FPermutationDomain RemapPermutation(FPermutationDomain PermutationVector)
	{
		...
		// Hair shading is classified only for complex & complex special tiles
		if (Substrate::IsSubstrateEnabled() && PermutationVector.Get<FHairComplexTransmittance>() && PermutationVector.Get<Substrate::FSubstrateTileType>() < ESubstrateTileType::EComplex) 
		{
			PermutationVector.Set<Substrate::FSubstrateTileType>(0);
		}
	...
	for (int32 ViewIndex = 0; ViewIndex < AllViews.Num(); ViewIndex++)
	{
		...
		if (ViewFamily.EngineShowFlags.Lighting)
		{
			...
			else if (ScreenSpaceRayTracing::IsScreenSpaceDiffuseIndirectSupported(View))
			{
				...
				static bool bHasWarnedSSGIDeprecated = false;
				if (!bHasWarnedSSGIDeprecated)
				{
					bHasWarnedSSGIDeprecated = true;
					UE_LOGF(LogRenderer, Warning, "Screen Space Global Illumination (SSGI) is deprecated (from 5.8) and will be removed in a future release. Please migrate to Lumen Global Illumination.");
				}
			}
...
void SetupReflectionUniformParameters(FRDGBuilder& GraphBuilder, const FViewInfo& View, FReflectionUniformParameters& OutParameters)
{
	...
	if (SkyLightTextureResource)
	{
		...
		// If this is not a realtime sky light capture, support visualizing the cube map.  Realtime sky light cube maps can be
		// visualized by setting r.SkyLight.RealTimeReflectionCapture.TimeSlice 0, without needing an explicit AddVisualizeTexturePass,
		// as it gets rendered to every frame.  A similar approach (setting r.SkylightUpdateEveryFrame 1) doesn't work for the
		// non-realtime code path, because it's written by a copy, which the visualizer ignores.
		const bool bIsRealtimePath = Scene && Scene->CanSampleSkyLightRealTimeCaptureData();
		if (!bIsRealtimePath && View.Family->EngineShowFlags.ReflectionEnvironment)
		{
			FVisualizeTexture::AddVisualizeTexturePass(GraphBuilder, GetGlobalShaderMap(GMaxRHIShaderPlatform), SkyLightTexture, /*bUseConfig=*/ true);
		}
	}
	else
	{
	...
	{
		FVisualizeTexture::AddVisualizeTexturePass(GraphBuilder, GetGlobalShaderMap(GMaxRHIShaderPlatform), GraphBuilder.RegisterExternalTexture(Scene->ReflectionSceneData.CubemapArray.GetRenderTarget()), /*bUseConfig=*/ true);
	...
	FAsyncLumenIndirectLightingOutputs& AsyncLumenIndirectLightingOutputs)
{
	...
	auto ShouldSkipView = [bCompositeRegularLumenOnly, &AsyncLumenIndirectLightingOutputs](const FPerViewPipelineState& ViewPipelineState, bool bApplyAmbientCubmap)
	{
		...
		// If Static lighting is disabled and this pass only composes (SS)AO on top of the scene color, we can skip this step all together.
		const bool bSkipSSAOComposition = Substrate::IsSubstrateEnabled() && ViewPipelineState.DiffuseIndirectMethod == EDiffuseIndirectMethod::Disabled && ViewPipelineState.AmbientOcclusionMethod == EAmbientOcclusionMethod::SSAO && !IsStaticLightingAllowed() && !bApplyAmbientCubmap;
		return bCompositeRegularLumenOnly != (bOnlyCompositeStepLeft && bRegularLumenView) || bSkipSSAOComposition;
	};
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		const FPerViewPipelineState& ViewPipelineState = GetViewPipelineState(View);
		const bool bApplyAmbientCubmap = IsAmbientCubemapPassRequired(View) && !bIsVisualizePass;

		if (ShouldSkipView(ViewPipelineState, bApplyAmbientCubmap))
		{
			continue;
		}
		...
		else if (ViewPipelineState.DiffuseIndirectMethod == EDiffuseIndirectMethod::Lumen)
		{
			...
			{
				// No Lumen Reflections and no SSR - use Lumen Rough Reflections only.
				OutTextures.Textures[3] = OutTextures.Textures[2];
			}
		...
		if (View.ViewState)
		{
			...
			{
				// Release sub-feature resources when their specific feature is disabled while MegaLights itself remains enabled

				if (!MegaLights::UseFrontLayerTranslucencyDirectLighting(View))
				{
					View.ViewState->MegaLights.FrontLayerTranslucency.SafeRelease();
				}

				if (!MegaLights::IsHairStrandsEnabled(View))
				{
					View.ViewState->MegaLights.HairStrands.SafeRelease();
				}
			}
		...
		auto ApplyDiffuseIndirect = [&](ESubstrateTileType TileType)
		{
			...
			PSParameters->LumenAmbientOcclusionIntensity = FMath::Clamp<float>(View.FinalPostProcessSettings.LumenAmbientOcclusionIntensity, 0.0f, 1.0f);
			...
			if (UseSeparatedSubsurfaceDiffuse(ShaderPlatform))
			{
				if (bEnableCopyPass)
				{
					PSParameters->RenderTargets[1] = FRenderTargetBinding(
						View.GetSceneTextures().SeparatedSubsurfaceDiffuse, ERenderTargetLoadAction::ELoad);
				}
				else
				{
					SetupSubsurfaceDiffuseWriteParameters(GraphBuilder, View, PSParameters->SubsurfaceDiffuseParameters);
				}
			}

			const bool bWriteStencil = !bCompositeRegularLumenOnly && Substrate::IsSubstrateEnabled() && Substrate::GetStencilPassStage() == Substrate::EStencilPassStage::DuringIndirectLighting;
			if (bWriteStencil)
			{
				PSParameters->RenderTargets.DepthStencil = FDepthStencilBinding(SceneDepthTexture, ERenderTargetLoadAction::ELoad, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthRead_StencilWrite);
			}
			...
			const bool bNeedHairComplexTransmittance = View.HairCardsMeshElements.Num() > 0 || IsHairComplexTransmittanceEnabled(View.GetShaderPlatform());
			PermutationVector.Set<FDiffuseIndirectCompositePS::FHairComplexTransmittance>(bNeedHairComplexTransmittance);
			...
			else
			{
				if (UseSeparatedSubsurfaceDiffuse(ShaderPlatform))
				{
					BlendState = TStaticBlendState<CW_RGBA, BO_Add, BF_One, BF_Zero, BO_Add, BF_One, BF_Zero,
			...
			else
			{
				...
				if (Substrate::IsOpaqueRoughRefractionEnabled(View.GetShaderPlatform()))
				{
					...
					PSParameters->OutOpaqueRoughRefractionSceneColor = GraphBuilder.CreateUAV(SubstrateSceneData.SeparatedOpaqueRoughRefractionSceneColor);
					PSParameters->OutSubSurfaceSceneColor = GraphBuilder.CreateUAV(SubstrateSceneData.SeparatedSubSurfaceSceneColor);
				}
				...
				GraphBuilder.AddPass(
					...
					PassParameters,
					ERDGPassFlags::Raster,
					[&View, TileVertexShader, PixelShader, PassParameters, TileType, BlendState, PrimitiveType, bWriteStencil](FRDGAsyncTask, FRHICommandList& RHICmdList)
				{
					FGraphicsPipelineStateInitializer GraphicsPSOInit;
					...
					if (bWriteStencil)
					{
						Substrate::GetSubstrateTiledDepthStencilStateWrite(TileType, GraphicsPSOInit, StencilRef);
					}
		...
		if (bApplyAmbientCubmap)
		{
			...
			{			
				...
				const bool bUseSeparatedSubsurfaceDiffuse = UseSeparatedSubsurfaceDiffuse(View.GetShaderPlatform());
				if (bUseSeparatedSubsurfaceDiffuse)
				{
					PSParameters->RenderTargets[1] = FRenderTargetBinding(View.GetSceneTextures().SeparatedSubsurfaceDiffuse, ERenderTargetLoadAction::ELoad);
				}
				...
				VSPermutationVector.Set< Substrate::FSubstrateTilePassVS::FEnableDebug >(false);
				VSPermutationVector.Set< Substrate::FSubstrateTilePassVS::FEnableTexCoordScreenVector >(false);
				VSPermutationVector.Set< Substrate::FSubstrateTilePassVS::FClassificationTileSize16 >(bSubstrateEnabled && Substrate::UseTileSize16());
				TShaderMapRef<Substrate::FSubstrateTilePassVS> TileVertexShader(View.ShaderMap, VSPermutationVector);
				...
				if (bUseSeparatedSubsurfaceDiffuse)
				{
					BlendState = TStaticBlendState<CW_RGBA, BO_Add, BF_One, BF_One, BO_Add, BF_One, BF_One,
				...
				GraphBuilder.AddPass(
					...
					PassParameters,
					ERDGPassFlags::Raster,
					[PassParameters, &View, FullScreenVertexShader, TileVertexShader, PixelShader, BlendState, PrimitiveType, TileType, bSubstrateEnabled](FRDGAsyncTask, FRHICommandList& RHICmdList)
				{				
	...
	ESubstrateTileType SubstrateTileMaterialType)
{
	...
	if (UseSeparatedSubsurfaceDiffuse(View.GetShaderPlatform()))
	{
		PassParameters->RenderTargets[1] = FRenderTargetBinding(View.GetSceneTextures().SeparatedSubsurfaceDiffuse, ERenderTargetLoadAction::ELoad);
	}
	...
	PassParameters->RenderTargets.ShadingRateTexture = GVRSImageManager.GetVariableRateShadingImage(GraphBuilder, View, FVariableRateShadingImageManager::EVRSPassType::ReflectionEnvironmentAndSky);
	...
	auto PermutationVector = FReflectionEnvironmentSkyLightingPS::BuildPermutationVector(
		...
		bLumenStandaloneReflections, bIsTiledSSR, bNeedHairComplexTransmittance,
	...
	VSPermutationVector.Set< Substrate::FSubstrateTilePassVS::FEnableDebug >(false);
	VSPermutationVector.Set< Substrate::FSubstrateTilePassVS::FEnableTexCoordScreenVector >(false);
	VSPermutationVector.Set< Substrate::FSubstrateTilePassVS::FClassificationTileSize16 >(bSubstrateEnabled && bSubstrateLightingPassesTiled && Substrate::UseTileSize16());
	...
	TArray<FRDGTextureRef>& DynamicBentNormalAOTextures)
{
	...
	for (FViewInfo& View : Views)
	{
		...
		if (bRequiresApply)
		{
			...
			{
				Substrate::AddSubstrateStencilPass(GraphBuilder, Scene, Views, SceneTextures, *SceneTextures.SubstrateSceneData);
			}
