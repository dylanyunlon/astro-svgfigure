...
FDepthPassInfo GetDepthPassInfo(const FScene* Scene)
{
	...
	Info.bDitheredLODTransitionsUseStencil = IsStencilForLODDitherEnabled(Scene ? Scene->GetShaderPlatform() : GMaxRHIShaderPlatform);
...
TOptional<uint32> GetOptionalDitheringFlag(
	const FSceneView* SceneView,
	const FMeshBatch& RESTRICT Mesh,
	int32 StaticMeshId)
{
#if UE_ENABLE_DEBUG_DRAWING
	// Check cached value from GetDynamicMeshElements first for debug view mode.
	if (Mesh.CachedLODDither != FMeshBatch::EDitherState::None)
	{
		if (Mesh.CachedLODDither == FMeshBatch::EDitherState::FadeOut)
		{
			return STENCIL_SANDBOX_MASK;
		}
		else if (Mesh.CachedLODDither == FMeshBatch::EDitherState::FadeIn)
		{
			return 0;
		}
	}
...
void AddDitheredStencilFillPass(FRDGBuilder& GraphBuilder, TConstArrayView<FViewInfo> Views, FRDGTextureRef DepthTexture, const FDepthPassInfo& DepthPass)
{
	...
	if (DepthPass.StencilDitherPassFlags == ERDGPassFlags::Raster)
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			...
			PassParameters->RenderTargets.DepthStencil = IsMobilePlatform(View.GetShaderPlatform()) ?
				FDepthStencilBinding(DepthTexture, ERenderTargetLoadAction::EClear, ERenderTargetLoadAction::EClear, FExclusiveDepthStencil::DepthWrite_StencilWrite):
				FDepthStencilBinding(DepthTexture, ERenderTargetLoadAction::ELoad, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthWrite_StencilWrite);
...
{
	if (bClearAllStencilBits)
	{
		return TStaticDepthStencilState<false, CF_Always,
...
{
	auto* PassParameters = GraphBuilder.AllocParameters<TOneColorPixelShaderMRT::FParameters>();
	PassParameters->RenderTargets.DepthStencil = FDepthStencilBinding(DepthTexture, ERenderTargetLoadAction::ELoad, ERenderTargetLoadAction::ELoad, FExclusiveDepthStencil::DepthWrite_StencilWrite);
	FIntPoint DepthSize = DepthTexture->Desc.Extent;
	const auto ShaderMap = GetGlobalShaderMap(ShaderPlatform);
	...
	FRHIDepthStencilState* DepthStencilState = GetDitheredStencilClearPassDepthStencilState(bClearAllStencilBits);

	FPixelShaderUtils::AddFullscreenPass(
		GraphBuilder, 
		ShaderMap,
		RDG_EVENT_NAME("AddDitheredStencilClearPass"),
		PixelShader, 
		PassParameters, 
		FIntRect(0, 0, DepthSize.X, DepthSize.Y), 
		nullptr, 
		nullptr, 
		DepthStencilState, 
		0);
	...
	EDepthDrawingMode DepthDrawingMode)
{
		...
		[&View, DrawRenderState, DepthDrawingMode](FRHICommandList& RHICmdList)
	{
		...
		View.SimpleElementCollector.DrawBatchedElements(RHICmdList, DrawRenderState, View, EBlendModeFilter::OpaqueAndMasked, SDPG_World, View.GetStereoPassInstanceFactor());
		View.SimpleElementCollector.DrawBatchedElements(RHICmdList, DrawRenderState, View, EBlendModeFilter::OpaqueAndMasked, SDPG_Foreground, View.GetStereoPassInstanceFactor());
		...
		if (!View.Family->EngineShowFlags.CompositeEditorPrimitives)
		{
			...
			View.BatchedViewElements.Draw(RHICmdList, DrawRenderState, View, false, 1.0f, EBlendModeFilter::All, View.GetStereoPassInstanceFactor());
			...
			// Draw the view's batched simple elements(lines, sprites, etc).
			View.TopBatchedViewElements.Draw(RHICmdList, DrawRenderState, View, false, 1.0f, EBlendModeFilter::All, View.GetStereoPassInstanceFactor());
		}
	});
}
...
void FDeferredShadingSceneRenderer::RenderPrePass(FRDGBuilder& GraphBuilder, TArrayView<FViewInfo> InViews, FRDGTextureRef SceneDepthTexture, FInstanceCullingManager& InstanceCullingManager, FRDGTextureRef* FirstStageDepthBuffer)
{
	...
	{
		AddDitheredStencilClearPass(GraphBuilder, SceneDepthTexture, ShaderPlatform, true);
	}
	...
	for (const FViewInfo& View : InViews)
	{
		...
		if (IStereoRendering::IsStereoEyeView(View) && !bIsCapture)
		{
				...
				[this, &View, HMDDevice](FRDGAsyncTask, FRHICommandList& RHICmdList)
			{
				...
				TShaderMapRef<TOneColorVS<true>> VertexShader(GetGlobalShaderMap(GMaxRHIShaderPlatform));
...
FMeshDrawCommandSortKey CalculateDepthPassMeshStaticSortKey(bool bIsMasked, const TShaderRef<FMeshMaterialShader>& VertexShader, const TShaderRef<FMeshMaterialShader>& PixelShader)
{
	...
	ERasterizerCullMode MeshCullMode)
{
	...
	const FMeshDrawCommandSortKey SortKey = CalculateDepthPassMeshStaticSortKey(bIsMasked, DepthPassShaders.VertexShader, DepthPassShaders.PixelShader);
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	EShaderPlatform ShaderPlatform = GetFeatureLevelShaderPlatform(SceneTexturesConfig.FeatureLevel);
	bool bUseVirtualShadowMaps = UseVirtualShadowMaps(ShaderPlatform, SceneTexturesConfig.FeatureLevel);
	if (UPSOPrecacheSettingsManager::GetSettings().bDepthProjectedShadowsState && !bUseVirtualShadowMaps)
	{
		// Set stencil to one.
...
void FDepthPassMeshProcessor::AddMeshBatch(const FMeshBatch& RESTRICT MeshBatch, uint64 BatchElementMask, const FPrimitiveSceneProxy* RESTRICT PrimitiveSceneProxy, int32 StaticMeshId)
{
	...
	if (EarlyZPassMode == DDM_AllOpaqueNoVelocity && PrimitiveSceneProxy)
	{
		...
		if (FOpaqueVelocityMeshProcessor::PrimitiveCanHaveVelocity(ShaderPlatform, PrimitiveSceneProxy))
		{
			if (ViewIfDynamicMeshCommand)
			{
				if (FOpaqueVelocityMeshProcessor::PrimitiveHasVelocityForFrame(ShaderPlatform, PrimitiveSceneProxy))
				{
...
{		
	...
	if (MeshPassType == EMeshPass::DitheredLODFadingOutMaskPass && !UPSOPrecacheSettingsManager::GetSettings().bDitheredLODFadingOutMask)
	{
		return;
	}
