 ...
 *
 * Á≠ñÁ-• (Âè-Ê®°Â-è):
 *   1. Â∞-ËØ-‰ª-Á-ÜÂà∞ Python FastAPI Â-éÁ´Ø (BACKEND_URL/api/cells)
 *   2. Â-éÁ´ØÁ-ªÁ∫øÊ-∂ÂõûÈ--: ËØªÂèñ channels/cell/*/params.json -Üí ÁªÑË-Ö CellDescriptor[]
 *      Ê-ìÊâë (topology.outgoing_edges) Á-±Âõ∫ÂÆ- Transformer È°∫Â∫èÊé®Êñ≠
 *
 * GitHub ËÉ-‰π-: withastro/astro (API Routes), ResearAI/AutoFigure, xiaodi #17
 */
...
function readFromCompositeParams(): { cells: unknown[]; edges: unknown[] } | null {
  ...
  // Build a lookup: cell_id -Üí list of incoming source IDs (derived from edges)
  ...
  // Map composite cells -Üí CellDescriptor
  ...
  // Sort by bbox.y (top-Üíbottom visual order)
  cells.sort((a: any, b: any) => (a.bbox?.y ?? 0) - (b.bbox?.y ?? 0))

  // Map composite edges -Üí EdgeDescriptor
